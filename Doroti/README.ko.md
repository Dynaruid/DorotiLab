...
Windows 기본값은 계속 opaque child-HWND ANGLE presenter입니다. Windows 11 24H2 build 26100 이상에서 `WindowBackdropMode.experimentalAcrylic`을 명시한 창만 같은 hardware D3D11 device의 ANGLE/Skia와 3-slot Composition Swapchain, 하나의 ContentIsland를 사용합니다. capability, adapter, 초기화가 실패하면 창을 보이기 전에 opaque로 결정하고 fallback 사유를 기록하며, 표시된 창의 topology는 바꾸지 않습니다. Interactive resize에서는 shell의 `WM_SIZING` rect를 수정하지 않습니다. 실제 child `WM_SIZE` physical extent만 metrics authority로 사용하며, interactive terminal wait는 최대 16ms이고 종료 exact wait는 최대 100ms입니다. Raster worker는 interactive frame마다 `DwmFlush`하지 않고 종료/비대화형 exact present에만 수행합니다. ContentIsland는 physical surface와 1:1 scale을 사용하고, 256px retained overscan과 top-HWND의 어두운 erase fill이 빠른 성장 중 흰색 노출을 막습니다. 3초 geometry는 active edge `min(12 physical px, ceil(6 logical px × scale))`와 inactive edge 1px를 유지합니다. 600ms responsiveness는 최소 40fps(측정 허용치 0.5fps), cursor lag 26px, 종료 후 9 refresh/50ms exact 복귀를 사용합니다. 안정형 `acrylic` 이름은 이 경로에 연결하지 않습니다.

`DOROTI_WINDOWS_PRESENTER=Vulkan`은 실험적 Vulkan/Skia presenter를 선택하며 ANGLE이 계속 기본값이고 조용한 fallback은 없습니다. 제품은 System32 Vulkan 1.1, 일치하는 adapter LUID와 dedicated importable D3D11-texture external memory를 요구하고 Windows Presentation으로 표시하므로 Vulkan WSI swapchain은 0개입니다. SkiaSharp 계열은 `4.152.0-rc.1.26426.14`로 고정하며 Windows host는 `SkiaSharp.Vulkan.Silk.NET`의 `GRSilkNetBackendContext`를 사용합니다. Top-level HWND의 native topmost DirectComposition target이 full-capacity identity source를 소유하고 top-level client가 유일한 visible clip입니다. 모든 interactive edge가 proposed-size Skia raster, synchronous Vulkan copy, Presentation 제출을 USER32 geometry 전에 완료합니다. 원점이 고정되는 `Right`/`Bottom`은 DWM 표시 경계까지 기다립니다. 원점이 움직이는 `Left`/`Top`은 제출 직후 반환해 새 frame이 다음 HWND transaction과 합쳐질 수 있게 하며 이전 screen origin에서 강제로 한 번 표시하지 않습니다. Programmatic/mismatch geometry는 bounded exact-render fallback을 사용하고 별도의 100 ms terminal 제한은 fail-safe입니다. Windows 11 24H2 이상에서는 Vulkan과 `experimentalAcrylic`을 함께 선택할 수 있습니다. Host-backdrop-enabled non-topmost `DesktopWindowTarget`이 active `DesktopAcrylicController`를 소유하고 HWND-wide transient system backdrop은 content 아래에 유지하며, topmost Vulkan overlay는 premultiplied입니다. Controller는 공통 kind/theme/tint/luminosity option channel을 적용합니다. `validate-windows-vulkan-live-resize.ps1`의 strict app-raster pixel gate는 material underlay가 보일 때 PASS로 재분류하지 않습니다. 실제 물리 확인에서는 standard-HWND moving-origin transaction의 떨림이 남았으므로 이 동작은 acceptance하지 않습니다. App-owned fixed-envelope/custom-non-client 대안은 채택하지 않으며, 좌측·상단 물리 acceptance가 해결되지 않은 상태로 Vulkan을 실험적 opt-in으로 유지합니다.

Material demo는 `DOROTI_DEMO_EXPERIMENTAL_ACRYLIC=1`일 때만 opt-in합니다. Runtime option은 `doroti/windows/experimental-acrylic` platform channel을 사용합니다. ANGLE과 Vulkan 모두 active `DesktopAcrylicController`에 kind/theme/tint/luminosity를 적용하며, 빈 payload로 effective mode, adapter LUID, transport, state, budget과 option counter를 조회할 수 있습니다. 전용 validator 결과는 자동 partial evidence이고 전체 방향/DPI/refresh matrix와 실물 검증은 완료되지 않아 이 mode는 아직 stable qualification이 아닙니다.
