...
`DOROTI_WINDOWS_PRESENTER=Vulkan`은 별도의 실험적 direct Vulkan/Skia presenter를 선택합니다. System32 Vulkan 1.1과 `VK_KHR_surface`, `VK_KHR_win32_surface`, `VK_KHR_swapchain`을 요구하지만 swapchain-maintenance는 요구하지 않습니다. stale work는 acquire 전에 버리고, acquire 성공 뒤에는 항상 copy/present합니다. capable GPU가 둘 이상이거나 adapter를 재현 가능하게 고정해야 할 때 `DOROTI_WINDOWS_VULKAN_DEVICE`를 지정합니다. 자동 fallback은 없고 `experimentalAcrylic`과 충돌합니다. ANGLE이 계속 기본값입니다. `validate-windows-vulkan-live-resize.ps1`로 WGC/current-monitor resize를 자동 검증할 수 있지만 실제 물리 resize/scan-out acceptance는 아직 `notVerified`입니다.

Material demo는 `DOROTI_DEMO_EXPERIMENTAL_ACRYLIC=1`일 때만 opt-in합니다. Runtime kind/theme/tint 변경은 `doroti/windows/experimental-acrylic` platform channel을 사용하며, 빈 payload로 effective mode, adapter LUID, budget과 option counter를 조회할 수 있습니다. 전용 validator는 `pwsh -File ./Doroti/eng/validate-windows-experimental-acrylic.ps1`입니다. 현재 200% DPI/165Hz TopLeft의 3초 geometry와 600ms responsiveness는 모두 `PASS-automated-partial`이지만 전체 방향/DPI/refresh 3회 matrix와 실물 검증은 완료되지 않아 이 mode는 아직 stable qualification이 아닙니다.
