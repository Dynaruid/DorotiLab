...
`DOROTI_WINDOWS_PRESENTER=Vulkan`은 실험적 Vulkan/Skia presenter를 선택하며 ANGLE이 계속 기본값이고 조용한 fallback은 없습니다. 제품은 System32 Vulkan 1.1, 일치하는 adapter LUID와 dedicated importable D3D11-texture external memory를 요구하고 Windows Presentation으로 표시하므로 Vulkan WSI swapchain은 0개입니다. SkiaSharp 계열은 `4.152.0-rc.1.26426.14`로 고정하며 Windows host는 `SkiaSharp.Vulkan.Silk.NET`의 `GRSilkNetBackendContext`를 사용합니다. Top-level HWND의 native topmost DirectComposition target이 full-capacity identity source를 소유하고 HWND client clip만 exact viewport를 정합니다. 좌·상단처럼 origin이 움직이는 edge는 현재 raster를 같은 HWND geometry에 둔 채 actual `WM_SIZE` 작업을 latest frame으로 coalesce하고 per-step platform wait를 하지 않으며, origin 고정 edge의 bounded exact-frame handshake는 유지합니다. Windows 11 24H2 이상에서는 Vulkan과 `experimentalAcrylic`을 함께 선택할 수 있습니다. Host-backdrop-enabled non-topmost `DesktopWindowTarget`이 active `DesktopAcrylicController`를 소유하고 topmost Vulkan overlay는 premultiplied입니다. Controller는 공통 kind/theme/tint/luminosity option channel을 적용합니다. `validate-windows-vulkan-live-resize.ps1`로 WGC/current-monitor 검증을 수행할 수 있지만 실제 물리 resize/scan-out과 Acrylic acceptance는 아직 `notVerified`입니다.
