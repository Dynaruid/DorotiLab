...
internal sealed unsafe class WindowsManagedAngleEglPresenter : WindowsManagedHwndPresenterBase
{
    ...
    private bool _flushAfterSurfaceCreationPresent;
    ...
    internal override bool EnsureTarget(nint childWindow, int width, int height)
    {
        ...
        _flushAfterSurfaceCreationPresent = true;
        return true;
    }
    ...
    internal override T RenderAndPresent<T>(Func<SKSurface, T> paint, Predicate<T> shouldPresent)
    {
        ...
        // so an EGL surface recreation cannot inherit an offscreen viewport,
        // scissor, or framebuffer binding.
    ...
    internal void ResetWindowSurfaceAfterInteractiveMove()
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
        ReleaseWindowSurface();
    }
