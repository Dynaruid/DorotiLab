...
internal sealed unsafe partial class WindowsManagedVulkanPresenter
{
    ...
    private static string? _actualGraphiteLibraryPath,
        _graphiteNativeHash;
    ...
    private uint RequiredVulkanApiVersion =>
        _usesPackagedGraphiteAsset ? (1u << 22) | (2u << 12) : VulkanApiVersion11;
    ...
    internal static void ConfigureGraphiteLibrary()
    {
        ...
        var asset = string.IsNullOrWhiteSpace(manifest)
            ? GraphiteNativeLibrary.GetPackagedAsset()
        ...
        lock (GraphiteLibraryGate)
        {
            ...
            _actualGraphiteLibraryPath = Process
                .GetCurrentProcess()
                .Modules.Cast<ProcessModule>()
                .Single(module =>
                    module.ModuleName.Equals("libSkiaSharp.dll", StringComparison.OrdinalIgnoreCase)
                )
                .FileName;
            if (
                !_actualGraphiteLibraryPath.Equals(
                    _graphiteLibraryPath,
                    StringComparison.OrdinalIgnoreCase
                )
            )
            {
                throw new InvalidOperationException(
                    "Actual loaded official Graphite path differs from provenance."
                );
            }
    ...
    private void CreateGraphiteContext(IReadOnlyList<string> deviceExtensions)
    {
        ...
        _graphite = SkiaGraphiteSession.CreateVulkan(
            new(
                _instance.Handle,
                ...
                image =>
                {
                    var s = _stockObserver.State((ulong)image);
    ...
    private void WrapGraphiteBacking(int width, int height)
    {
        ...
        {
            _stockObserver.RegisterHostTarget(
                _backingImage.Handle,
                new ImageCreateInfo
                {
                    ImageType = ImageType.Type2D,
                    Format = _format,
                    Extent = new((uint)_backingCapacityWidth, (uint)_backingCapacityHeight, 1),
                    MipLevels = 1,
                    ArrayLayers = 1,
        ...
        _graphiteTarget = _graphite!.CreateVulkanTarget(
            width,
            height,
    ...
    {
        if (_graphiteFrame is null)
        {
            return;
        }

        if (_graphiteSubmissionAttempted)
        {
            _graphiteFrame.CompleteGpuWork();
        }
        else
        {
            _graphiteFrame.CancelRecording();
        }
    ...
    private void ObservedPipelineBarrier(
        ...
        PipelineStageFlags source,
        PipelineStageFlags destination,
        DependencyFlags flags,
        uint memoryCount,
        MemoryBarrier* memory,
        uint bufferCount,
        BufferMemoryBarrier* buffers,
        uint imageCount,
    ...
    )
    {
        ...
        {
            _stockObserver.Call<VulkanObserver.CmdPipelineBarrierDelegate>("vkCmdPipelineBarrier")(
                command,
                source,
                destination,
                flags,
