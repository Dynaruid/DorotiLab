...
internal sealed class PreparedMovingFrameLedger
{
    ...
    {
        if (
            frame.SlotIndex is < 0 or >= 3
            || frame.Key.ResizeEpoch == 0
            || frame.Key.MetricsGeneration == 0
            || frame.Key.Width <= 0
            || frame.Key.Height <= 0
            || !double.IsFinite(frame.Key.Scale)
            || frame.Key.Scale <= 0
        )
        {
