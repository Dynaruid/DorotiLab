...
internal sealed record WindowsCompositionSlice(
...
{
    internal static readonly PlatformEffectSupport Effects = new(
        true,
        4,
        32,
        Saturation: true,
    ...
    private readonly Dictionary<
        int,
        (double Sigma, double Saturation, C.SpriteVisual Visual, C.CompositionEffectBrush Brush)
    > _effects = [];
    ...
    private sealed record RasterSlot(
        WindowsCompositionSlice Slice,
        C.SpriteVisual Visual,
        C.CompositionSurfaceBrush Brush,
        C.CompositionDrawingSurface Surface
    ) : IDisposable
    {
    ...
    private long _nativeMessages,
        _mouseEvents,
        _pointerEvents;
    private const long _rasterUploadBytes = 0,
        _rasterUploads = 0;
    private long _rasterReuses,
        _rasterGpuCopies;
    internal long RasterUploadBytes => _rasterUploadBytes;
    internal object Evidence =>
        new
        {
            strategy = "WebView2-CompositionController",
            apiFamily = "Windows.UI.Composition",
            runtimeVersion = _environment is { IsCompletedSuccessfully: true } environment
                ? environment.Result.BrowserVersionString
                : null,
            liveInstances = _instances.Count,
            effects = _effects.Count,
            nativeMessages = _nativeMessages,
            ...
            navigationEvents = _instances
                .Values.Select(instance => instance.NavigationEvents.ToArray())
                .ToArray(),
            rasterUploadBytes = _rasterUploadBytes,
            rasterUploads = _rasterUploads,
            rasterReuses = _rasterReuses,
            rasterGpuCopies = _rasterGpuCopies,
            effectParameters = _effects
                .Values.Select(effect => new
                {
                    physicalSigma = effect.Sigma,
                    saturation = effect.Saturation,
                })
                .ToArray(),
            nativeContentCaptured = false,
            observation = "BackendAccepted",
        };

    internal WindowsWebViewComposition(
        nint parent,
        Action invalidate,
        Action yieldText,
        bool transportAvailable,
        IApplicationResourceHostCapability resources,
        Func<(uint Low, int High)> adapter
    )
    {
        _adapter = adapter;
        _parent = parent;
    ...
    public PlatformViewSupport QuerySupport(PlatformViewRequest request) =>
        new(
            ...
            NativeBackdropBlur: _transportAvailable && !_disposed,
            Capabilities: new(
                PlatformViewRepresentation.CompositionVisual,
    ...
    private void Initialize()
    {
        if (!_transportAvailable)
        {
            throw new NotSupportedException(
                "WebView composition requires the Graphite/Vulkan raster transport."
            );
        }
        ...
        {
            var options = new Native.QueueOptions
            {
                Size = 12,
                ThreadType = 2,
                ApartmentType = 2,
            ...
            Marshal.ThrowExceptionForHR(
                Native.CreateDispatcherQueueController(options, out var queue)
            );
            try
            {
                _queue = WinRT.MarshalInterface<Windows.System.DispatcherQueueController>.FromAbi(
                    queue
                );
            }
            finally
        ...
        _target.Root = _root;
        Marshal.ThrowExceptionForHR(
            Native.CreateGraphics(Abi(_compositor), _adapter().Low, _adapter().High, out _graphics)
        );
    ...
    )
    {
        ...
        Initialize();
        var folder =
            Environment.GetEnvironmentVariable("DOROTI_WEBVIEW_USER_DATA")
        ...
        if (_disposed)
        {
            throw new WebViewException(
                WebViewError.Closed,
                "WebView owner closed during environment creation."
            );
        }

        var profile = environment.CreateCoreWebView2ControllerOptions();
        profile.ProfileName =
            options.Profile == WebViewProfile.Ephemeral
                ? "private-" + Guid.NewGuid().ToString("N")
                : "DorotiShared";
        profile.IsInPrivateModeEnabled = options.Profile == WebViewProfile.Ephemeral;
        var controller = await environment.CreateCoreWebView2CompositionControllerAsync(
            CoreWebView2ControllerWindowReference.CreateFromWindowHandle((ulong)_parent),
            profile
        );
        ...
        try
        {
            ...
            if (_disposed)
            {
                throw new WebViewException(
                    WebViewError.Closed,
                    "WebView owner closed during controller creation."
                );
            }
            ...
            if (!_instances.TryAdd(handle, instance))
            {
                throw new InvalidOperationException("Duplicate WebView identity.");
            }
    ...
    internal unsafe void Commit(
        PlatformCompositionPlan plan,
        WindowsSharedRaster? shared,
        WindowsCompositionSlice[] slices,
    ...
    )
    {
        ...
        {
            throw new NotSupportedException(
                "HWND controls and WebView2 composition visuals require separate owner scenes."
            );
        }
        ...
        try
        {
            foreach (var slice in slices)
            {
                if (slice.Reused)
                {
                    if (!_rasters.TryGetValue(slice.Segment.PaintOrder, out var retained))
                    {
                        throw new InvalidOperationException(
                            "Missing retained WebView composition raster."
                        );
                    }
                ...
                if (shared is null)
                {
                    throw new InvalidOperationException("Missing changed WebView shared raster.");
                }

                Marshal.ThrowExceptionForHR(
                    Native.CreateSharedSurface(
                        _graphics,
                        shared.Handle,
                        (uint)slice.AtlasY,
                ...
                try
                {
                    drawing = WinRT.MarshalInterface<C.CompositionDrawingSurface>.FromAbi(surface);
                }
                finally
                {
                ...
                prepared.Add(slot);
                nextRasters.Add(slice.Segment.PaintOrder, slot);
            ...
            foreach (var effect in next.OfType<PlatformBackdropSegment>())
            {
                if (
                    ...
                    || plan.Token.DeviceScaleX != plan.Token.DeviceScaleY
                )
                {
                    throw new NotSupportedException(
                ...
                if (
                    _effects.TryGetValue(effect.PaintOrder, out var cached)
                    && cached.Sigma == sigma
                    && cached.Saturation == saturation
                )
                {
                    nextEffects.Add(effect.PaintOrder, cached);
                    continue;
                }
                Marshal.ThrowExceptionForHR(
                    Native.CreateEffect(
                ...
                try
                {
                    brush = WinRT.MarshalInterface<C.CompositionEffectBrush>.FromAbi(value);
                }
                finally
                {
            ...
            {
                Complete(batch.AttachAsync(native.Placement));
            }

            var nextHandles = next.OfType<PlatformNativeSegment>()
                .Select(p => p.Placement.Handle)
                .ToHashSet();
            foreach (var old in _visible.OfType<PlatformNativeSegment>())
            {
                if (
                    !nextHandles.Contains(old.Placement.Handle)
            ...
            foreach (var part in next)
            {
                switch (part)
                {
                    case PlatformRasterSegment raster
                        when visuals.TryGetValue(raster.PaintOrder, out var rasterVisual):
                        ordered.Add(rasterVisual);
                        break;
                    case PlatformNativeSegment native:
                        var nativeVisual = _instances[native.Placement.Handle].Visual;
                        ordered.Add(nativeVisual);
                        break;
                    case PlatformBackdropSegment effect:
                        var visual = nextEffects[effect.PaintOrder].Visual;
                        visual.Offset = new(
                            (float)(effect.Bounds.left * plan.Token.DeviceScaleX),
                            (float)(effect.Bounds.top * plan.Token.DeviceScaleY),
                            0
                        );
                        visual.Size = new(
                            (float)(effect.Bounds.width * plan.Token.DeviceScaleX),
                            (float)(effect.Bounds.height * plan.Token.DeviceScaleY)
                        );
                        ordered.Add(visual);
                        break;
                }
            ...
            {
                var replacement = nextRasters.Values.FirstOrDefault(r =>
                    ReferenceEquals(r.Visual, old.Visual)
                );
                if (replacement is null)
                {
                    old.Dispose();
                }
                else if (!ReferenceEquals(replacement.Surface, old.Surface))
                {
                    old.DisposeImage();
                }
            }
            _rasters.Clear();
            foreach (var item in nextRasters)
            {
                _rasters.Add(item.Key, item.Value);
            }
            ...
            _effects.Clear();
            foreach (var item in nextEffects)
            {
            ...
            _visible = next;
            _token = plan.Token;
            _orderedVisuals = ordered.ToArray();
        ...
        catch
        {
            if (mutationStarted)
            {
                foreach (var raster in _rasters.Values)
                {
                    raster.Visual.Brush = raster.Brush;
                    raster.Visual.Size = new(raster.Slice.Bounds.Width, raster.Slice.Bounds.Height);
                    raster.Visual.Offset = new(
                        raster.Slice.Bounds.Left,
                        raster.Slice.Bounds.Top,
                        0
                    );
                }
                var previousHandles = _visible
                    .OfType<PlatformNativeSegment>()
                    .Select(p => p.Placement.Handle)
                    .ToHashSet();
                foreach (var native in next.OfType<PlatformNativeSegment>())
                {
                    if (!previousHandles.Contains(native.Placement.Handle))
                    {
                        Complete(batch.DetachAsync(native.Placement.Handle));
                    }
                }

                foreach (var native in _visible.OfType<PlatformNativeSegment>())
                {
                    if (batch.Contains(native.Placement.Handle))
                    {
                        Complete(batch.AttachAsync(native.Placement));
                    }
                }
                ...
                foreach (var effect in _visible.OfType<PlatformBackdropSegment>())
                {
                    ...
                    visual.Offset = new(
                        (float)(effect.Bounds.left * _token.DeviceScaleX),
                        (float)(effect.Bounds.top * _token.DeviceScaleY),
                        0
                    );
                    visual.Size = new(
                        (float)(effect.Bounds.width * _token.DeviceScaleX),
                        (float)(effect.Bounds.height * _token.DeviceScaleY)
                    );
                }
                _root!.Children.RemoveAll();
                foreach (var visual in _orderedVisuals)
                {
                    _root.Children.InsertAtTop(visual);
                }
            }
        ...
        finally
        {
            ...
            {
                if (!_rasters.Values.Any(r => ReferenceEquals(r.Visual, item.Visual)))
                {
                    item.Visual.Dispose();
                }
            ...
            foreach (var item in allocatedEffects)
            {
                item.Visual.Dispose();
                item.Brush.Dispose();
            }
    ...
    private static void Complete(ValueTask task)
    {
        ...
        {
            throw new InvalidOperationException(
                "Reserved native operation must complete on the UI thread."
            );
        }
    ...
    internal void Clear(IPlatformViewPlacementBatch batch)
    {
        ...
        _hover = null;
        _root?.Children.RemoveAll();
        foreach (var resource in _rasters.Values)
        {
            resource.Dispose();
        }
        ...
        {
            effect.Visual.Dispose();
            effect.Brush.Dispose();
        }
        _effects.Clear();
        _visible = [];
        _orderedVisuals = [];
    ...
    )
    {
        ...
        if (
            message == 0x20
            && _hover is { } cursorOwner
            && _instances.TryGetValue(cursorOwner, out var cursorView)
        )
        {
            ...
            Native.SetCursor(Native.LoadCursorW(0, cursor));
            return 1;
        }
        if (message is >= 0x200 and <= 0x20e && HasVisibleContent)
        {
            ...
            if (message == 0x200 && handle != _hover)
            {
                if (_hover is { } previous && _instances.TryGetValue(previous, out var hovered))
                {
                    hovered.Controller.SendMouseInput(
                ...
                _hover = handle;
                var tracking = new Native.TrackMouse
                {
                    Size = (uint)Marshal.SizeOf<Native.TrackMouse>(),
                    Flags = 2,
                    Window = hwnd,
                };
            ...
            if (
                ...
                && instance.Placement is { } placement
            )
            {
                ...
                var local = new Windows.Foundation.Point(
                    point.X - (int)Math.Round(origin.dx * _token.DeviceScaleX),
                    point.Y - (int)Math.Round(origin.dy * _token.DeviceScaleY)
                );
                instance.Controller.SendMouseInput(
                    (CoreWebView2MouseEventKind)message,
                    (CoreWebView2MouseEventVirtualKeys)(wparam & 0xffff),
                    message is 0x20a or 0x20e ? (uint)(short)(wparam >> 16)
                        : message is 0x20b or 0x20c or 0x20d ? (uint)(wparam >> 16)
                        : 0,
                    local
                );
                _mouseEvents++;
    ...
    private sealed partial class Instance : IPlatformViewInstance, IPlatformWebViewInstance
    {
        ...
        internal Instance(
            WindowsWebViewComposition owner,
            PlatformViewHandle handle,
            CoreWebView2Environment environment,
            CoreWebView2CompositionController controller,
            C.ContainerVisual visual,
        ...
        {
            TraceNavigation(
                $"completed:{args.NavigationId}:active={_nativeNavigation}:success={args.IsSuccess}"
            );
            ...
            CompleteNavigation(args);
            Loaded = args.IsSuccess;
            NavigationStatus = args.IsSuccess ? "Loaded" : args.WebErrorStatus.ToString();
            _owner._invalidate();
        }
        ...
        public ValueTask ApplyAsync(PlatformViewPlacement placement)
        {
            ...
            if (Placement == placement && scale == _scale)
            {
            ...
            var origin = placement.Transform.Map(placement.Bounds.topLeft);
            Controller.Bounds = new(
                0,
                0,
                (int)Math.Round(placement.Bounds.width * scale),
                (int)Math.Round(placement.Bounds.height * scale)
            );
            Visual.Offset = new((float)(origin.dx * scale), (float)(origin.dy * scale), 0);
            var bounds = Bounds(placement);
            var previousClip = Visual.Clip;
            Visual.Clip = _owner._compositor!.CreateInsetClip(
                (float)((bounds.left - origin.dx) * scale),
                (float)((bounds.top - origin.dy) * scale),
                (float)((origin.dx + placement.Bounds.width - bounds.right) * scale),
            ...
            Controller.IsVisible = placement.Visible && !bounds.isEmpty;
            Placement = placement;
        ...
        {
            if (focused && Placement is { Visible: true })
            {
                FocusNative();
            }
        ...
        public ValueTask DisposeAsync()
        {
            ...
            _disposed = true;
            _owner._instances.TryRemove(_handle, out _);
            foreach (
                var id in _owner
                    ._pointerTargets.Where(p => p.Value == _handle)
                    .Select(p => p.Key)
                    .ToArray()
            )
            {
                _owner._pointerTargets.Remove(id);
            }
    ...
    public void Dispose()
    {
        ...
        {
            Complete(instance.DisposeAsync());
        }
        ...
        if (_graphics != 0)
        {
            Native.DestroyGraphics(_graphics);
        }

        _compositor?.Dispose();
        if (_queue is not null)
        {
            _ = _queue.ShutdownQueueAsync();
        }
    }
    ...
    private static class Native
    {
        ...
        [DllImport(
            ...
            EntryPoint = "doroti_windows_platform_shared_surface_v1",
            CallingConvention = CallingConvention.Cdecl
        )]
        internal static extern int CreateSharedSurface(
            nint context,
            nint handle,
            uint sourceY,
            uint width,
        ...
        [DllImport("user32.dll")]
        internal static extern nint LoadCursorW(nint module, nint id);

        [DllImport("user32.dll")]
        internal static extern nint SetCursor(nint cursor);

        [DllImport("user32.dll")]
        internal static extern bool TrackMouseEvent(ref TrackMouse tracking);

        [DllImport(WindowsNativeV1.LibraryName, EntryPoint = "doroti_windows_platform_effect_v2")]
        internal static extern int CreateEffect(
            nint compositor,
            float sigma,
            float saturation,
            out nint brush
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_platform_graphics_create_v2"
        )]
        internal static extern int CreateGraphics(
            nint compositor,
            uint low,
            int high,
            out nint context
        ...
        internal static extern int CreateSurface(
            nint context,
            nint pixels,
            uint width,
            uint height,
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_platform_graphics_destroy_v1"
        )]
