...
{
    internal static readonly PlatformEffectSupport Support = new(
        true,
        4,
        32,
    ...
    private readonly bool _omitNativeProbe =
    ...
    internal WindowsWinUiBackdrop(
        WindowsWinUiControls controls,
        nint parent,
    ...
    internal object Evidence =>
        new
        {
            strategy = "WinUI CompositionVisualSurface live prefix sampling",
            effects = _effects.Count,
            nativeSources = _native.Count,
            nativeReadbackBytes = 0,
            rasterUploads = 0,
            rasterGpuCopies = _gpuCopies,
            rasterReuses = _reuses,
            omittedNativeProbe = _omitNativeProbe,
            commits = _commits,
            input = "direct native HWND; effect bridges input-transparent",
            bounds = _effects
                .Select(pair => new
                {
                    order = pair.Key,
                    sigma = pair.Value.Sigma,
                    area = Coordinates(pair.Value.Area),
                    clip = Coordinates(pair.Value.Bounds),
                })
                .ToArray(),
        };

    private static double[] Coordinates(Rect bounds) =>
        [bounds.left, bounds.top, bounds.right, bounds.bottom];

    internal unsafe void Prepare(
        PlatformCompositionPlan plan,
        WindowsSharedRaster? shared,
        WindowsCompositionSlice[] slices,
        int width,
        int height,
        Func<PlatformViewHandle, nint> getWindow
    )
    {
        ...
        {
            var first =
                nativeParts.FirstOrDefault()
                ?? throw new InvalidOperationException(
                    "WinUI backdrop requires a live native source."
                );
        ...
        var areas = effects.ToDictionary(
            effect => effect.PaintOrder,
            effect =>
                Scale(effect.Bounds, scale)
                    .inflate(Math.Ceiling(3 * effect.SigmaX * scale))
                    .intersect(viewport)
        );
        // Source and output surfaces plus current/staging GPU rasters are bounded.
        var bytes =
            areas.Values.Sum(area => area.width * area.height * 16)
            + slices.Sum(slice => (double)slice.Bounds.Width * slice.Bounds.Height * 16);
        if (bytes > 256L * 1024 * 1024)
        {
            throw new InvalidOperationException("WinUI backdrop staging exceeds 256 MiB.");
        }
        ...
        foreach (var slice in slices)
        {
            if (
                slice.Reused
                && _rasters.TryGetValue(slice.Segment.PaintOrder, out var previous)
                && previous.Slice.Bounds == slice.Bounds
                && SkiaPlatformRasterContent.Equivalent(
                    previous.Slice.Segment.Commands,
            ...
            if (slice.Reused || shared is null)
            {
                throw new InvalidOperationException("Missing retained WinUI backdrop source.");
            }
            ...
            try
            {
                drawing = WinRT.MarshalInterface<CompositionDrawingSurface>.FromAbi(surface);
            }
            finally
            {
            ...
            if (_rasters.Remove(slice.Segment.PaintOrder, out var old))
            {
                _retired.Add(old);
            }
        ...
        foreach (var hwnd in _native.Keys.Except(liveWindows).ToArray())
        {
            _retired.Add(_native[hwnd]);
            _native.Remove(hwnd);
        }
        foreach (var part in nativeParts)
        {
            ...
            if (!_native.TryGetValue(hwnd, out var source))
            {
                ...
                if (Abi(visual.Compositor) != Abi(_compositor))
                {
                    throw new InvalidOperationException("WinUI sources must share one compositor.");
                }
            ...
            source.Surface.SourceSize = new(
                (float)part.Placement.Bounds.width,
                (float)part.Placement.Bounds.height
            );
        }
        var orders = effects.Select(effect => effect.PaintOrder).ToHashSet();
        foreach (var order in _effects.Keys.Except(orders).ToArray())
        {
            _effects[order].Dispose();
            _effects.Remove(order);
        }
        foreach (var effect in effects)
        {
            if (!_effects.TryGetValue(effect.PaintOrder, out var layer))
            {
                layer = new(_compositor, _parent);
                _effects.Add(effect.PaintOrder, layer);
            }
            layer.ResetPrefix();
            foreach (var part in plan.Parts.TakeWhile(part => part.PaintOrder < effect.PaintOrder))
            {
                if (
                    part is PlatformRasterSegment raster
                    && _rasters.TryGetValue(raster.PaintOrder, out var source)
                )
                {
                    ...
                    part is PlatformNativeSegment native
                ...
                )
                {
                    ...
                    layer.Add(
                        _native[getWindow(placement.Handle)].Brush,
                        bounds,
            ...
            layer.Update(
                areas[effect.PaintOrder],
                Scale(effect.Bounds, scale).intersect(viewport),
        ...
        foreach (
            var order in _rasters
                .Keys.Except(slices.Select(slice => slice.Segment.PaintOrder))
                .ToArray()
        )
        {
            _rasters[order].Dispose();
            _rasters.Remove(order);
        }
    ...
    internal void Clear()
    {
        ...
        foreach (var effect in _effects.Values.Reverse())
        {
            effect.Dispose();
        }

        _effects.Clear();
        foreach (var source in _native.Values)
        {
            source.Dispose();
        }
    ...
    private sealed record RasterSource(
    ...
    private sealed class EffectLayer : IDisposable
    {
        ...
        internal EffectLayer(Compositor compositor, nint parent)
        {
            ...
            _bridge = DesktopChildSiteBridge.Create(
                compositor,
                Win32Interop.GetWindowIdFromWindow(parent)
            );
            ...
            if (
                Native.SetWindowLongPtrW(Hwnd, -20, style | 0x20) == 0
        ...
        {
            if (bounds.isEmpty || clipping is { isEmpty: true })
            {
                return;
            }
            ...
            if (!ReferenceEquals(node.Brush, brush))
            {
                node.Visual.Brush = brush;
                node.Brush = brush;
            }
            ...
            if (node.Bounds != bounds || node.ClipBounds != clipping)
            {
                ...
                node.Visual.Clip = inset;
                node.Clip?.Dispose();
                node.Clip = inset;
            }
            node.Bounds = bounds;
            node.ClipBounds = clipping;
        }

        private InsetClip MakeClip(Rect bounds, Rect clip) =>
            _compositor.CreateInsetClip(
                (float)Math.Max(0, clip.left - bounds.left),
                (float)Math.Max(0, clip.top - bounds.top),
                (float)Math.Max(0, bounds.right - clip.right),
                (float)Math.Max(0, bounds.bottom - clip.bottom)
            );
        ...
        internal void Update(Rect area, Rect bounds, float sigma, int width, int height)
        {
            while (_children.Count > _nextChild)
            {
                ...
                _prefix.Children.Remove(child.Visual);
                child.Dispose();
            }
            if (Brush is null || Sigma != sigma)
            {
                Marshal.ThrowExceptionForHR(
                    Native.CreateEffect(Abi(_compositor), sigma, out var pointer)
                );
                CompositionEffectBrush brush;
                try
                {
                    brush = WinRT.MarshalInterface<CompositionEffectBrush>.FromAbi(pointer);
                }
                finally
                {
            ...
            _sample.SourceSize = new(
                (float)Math.Max(1, area.width),
                (float)Math.Max(1, area.height)
            );
    ...
    private static class Native
    {
        [DllImport(
            ...
            EntryPoint = "doroti_windows_winui_shared_surface_v1",
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_winui_graphics_create_v2"
        )]
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_winui_graphics_destroy_v1"
        )]
