...
internal sealed class WindowsSharedRaster(nint resource, nint handle, SKCanvas canvas) : IDisposable
{
    ...
    public void Dispose()
    {
        var value = Interlocked.Exchange(ref _resource, 0);
        if (value != 0)
        {
...
internal sealed unsafe partial class WindowsManagedVulkanPresenter
{
    ...
    internal WindowsSharedRaster CreateSharedRaster(int width, int height)
    {
        ...
        {
            throw new InvalidOperationException("Shared raster requires an active Graphite frame.");
        }

        if (_sharedRasterDevice == 0)
        {
            Marshal.ThrowExceptionForHR(
                CreateSharedRasterDevice(_adapterLuidLow, _adapterLuidHigh, out _sharedRasterDevice)
            );
        }

        Marshal.ThrowExceptionForHR(
            CreateSharedRasterResource(
                _sharedRasterDevice,
                (uint)width,
                (uint)height,
                out var resource,
        ...
        try
        {
            ...
            var output = new VulkanSharedRaster(
                _vk,
                _instance,
                _physicalDevice,
                _device,
    ...
    [DllImport(
        ...
        EntryPoint = "doroti_windows_shared_raster_device_v1",
    ...
    [DllImport(
        WindowsNativeV1.LibraryName,
        EntryPoint = "doroti_windows_shared_raster_create_v1",
        CallingConvention = CallingConvention.Cdecl
    )]
    private static extern int CreateSharedRasterResource(
        nint device,
        uint width,
        uint height,
        out nint resource,
