...
internal sealed class WindowsWinUiControls : IDisposable
{
    ...
    internal Island Create(nint parent, bool editor, string text)
    {
        try
        {
            if (_initializationError is not null)
            {
                throw new InvalidOperationException(
                    "The WinUI environment could not be initialized.",
            ...
            if (_manager is null)
            {
                ...
                {
                    _application = new IslandApplication(resourceManager);
                }
        ...
        {
            throw new InvalidOperationException(
                $"WinUI island initialization failed: {error}",
                error
            );
        }
    }
    ...
    {
        foreach (var island in _islands.ToArray())
        {
            island.Dispose();
        }
    }
    ...
    public void Dispose()
    {
        ...
        {
            throw new InvalidOperationException(
                "WinUI islands must retire before the XAML environment."
            );
        }
    ...
    private sealed class IslandApplication : Application, IXamlMetadataProvider
    {
        ...
        internal IslandApplication(
            Microsoft.Windows.ApplicationModel.Resources.ResourceManager resources
        )
        {
    ...
    internal sealed class Island : IDisposable
    {
        ...
        {
            if (_shieldBounds.SequenceEqual(bounds))
            {
                return;
            }
            ...
            foreach (var rect in bounds)
            {
                ...
                Canvas.SetLeft(shield, rect.left);
                Canvas.SetTop(shield, rect.top);
                _shields.Children.Add(shield);
            }
            _shieldBounds = bounds;
        }

        private void OnShieldPressed(object sender, PointerRoutedEventArgs args) =>
            ForwardPointer(args, 0x201);

        private void OnShieldMoved(object sender, PointerRoutedEventArgs args) =>
            ForwardPointer(args, 0x200);

        private void OnShieldReleased(object sender, PointerRoutedEventArgs args) =>
            ForwardPointer(args, 0x202);
        ...
        private void OnWheel(object sender, PointerRoutedEventArgs args)
        {
            if (!args.Handled)
            {
                ForwardPointer(
                    args,
                    args.GetCurrentPoint(_root).Properties.IsHorizontalMouseWheel ? 0x20eu : 0x20au
                );
            }
        }
        ...
        private void ForwardPointer(PointerRoutedEventArgs args, uint message)
        {
            try
            {
                ...
                {
                    X = (int)Math.Round(point.Position.X * scale),
                    Y = (int)Math.Round(point.Position.Y * scale),
                };
                Native.ClientToScreen(Hwnd, ref position);
                var wheel = message is 0x20a or 0x20e;
                if (!wheel)
                {
                    Native.ScreenToClient(_parent, ref position);
                }

                uint keys =
                    (properties.IsLeftButtonPressed ? 1u : 0u)
                ...
                if (wheel)
                {
                    keys |= (uint)(ushort)properties.MouseWheelDelta << 16;
                }
                ...
                Native.SendMessageW(
                    ...
                    message,
                    keys,
                    (nint)((ushort)position.X | ((uint)(ushort)position.Y << 16))
                );
            }
            catch (Exception error)
            {
        ...
        {
            if ((int)args.Key is not (16 or 17 or 18 or 160 or 161 or 162 or 163 or 164 or 165))
            {
                return;
            }
            ...
            try
            {
                ...
                var flags =
                    status.RepeatCount
                    | (status.ScanCode << 16)
                    | (status.IsExtendedKey ? 1u << 24 : 0)
                    | (status.WasKeyDown ? 1u << 30 : 0)
                    | (status.IsKeyReleased ? 1u << 31 : 0);
                ModifierKey?.Invoke(
                    status.IsKeyReleased ? 0x101u : 0x100u,
                    (nuint)args.Key,
                    unchecked((nint)flags)
                );
            }
            catch (Exception error)
            {
        ...
        internal object Snapshot() =>
            new
            {
                type = _control.GetType().FullName,
                hwnd = Hwnd.ToInt64(),
                loaded = _control.IsLoaded,
                width = _control.ActualWidth,
                height = _control.ActualHeight,
                templateChildren = Microsoft.UI.Xaml.Media.VisualTreeHelper.GetChildrenCount(
                    _control
                ),
