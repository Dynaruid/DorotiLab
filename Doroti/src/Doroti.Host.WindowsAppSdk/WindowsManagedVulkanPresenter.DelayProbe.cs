...
{
    private readonly int _diagnosticDelayMilliseconds = int.TryParse(
        Environment.GetEnvironmentVariable("DOROTI_WINDOWS_GRAPHITE_DELAY_MS"),
        out var delay
    )
        ? Math.Clamp(delay, 0, 15000)
        : 0;
    ...
    {
        if (_diagnosticDelayMilliseconds == 0 || _diagnosticDelayUsed || PresentCount == 0)
        {
            return;
        }

        if (!_usesPackagedGraphiteAsset)
        {
            throw new InvalidOperationException(
                "The delay fixture requires the official Vulkan 1.2 path."
            );
        }
        ...
        var ci = new SemaphoreCreateInfo
        {
            SType = StructureType.SemaphoreCreateInfo,
            PNext = &type,
        };
        Check(
            _vk.CreateSemaphore(_device, &ci, null, out _diagnosticDelaySemaphore),
            "qualification timeline"
        );
        _diagnosticProducer = new Thread(() =>
        {
            Thread.Sleep(_diagnosticDelayMilliseconds);
            var signal = (delegate* unmanaged<nint, SemaphoreSignalInfo*, Result>)
                (nint)_vk.GetDeviceProcAddr(_device, "vkSignalSemaphore");
            var info = new SemaphoreSignalInfo
            {
                SType = StructureType.SemaphoreSignalInfo,
                Semaphore = _diagnosticDelaySemaphore,
                Value = 1,
            };
        ...
        {
            File.WriteAllText(marker, "real-timeline-wait-submitted");
        }
    }

    private void ReleaseDiagnosticProducer()
    {
        _diagnosticProducer?.Join();
        _diagnosticProducer = null;
        if (_diagnosticDelaySemaphore.Handle == 0)
        {
            return;
        }

        _vk.DestroySemaphore(_device, _diagnosticDelaySemaphore, null);
        _diagnosticDelaySemaphore = default;
