...
internal sealed class WindowsPlatformViewHost : IDisposable
{
    ...
    private sealed record WindowPosition(
        nint Hwnd,
        int X,
        int Y,
        int Width,
    ...
    internal IEnumerable<IPlatformViewFactory> CreateFactories() =>
        [
            new DeferredFactory(this, "doroti/native-button"),
            new DeferredFactory(this, "doroti/native-editor"),
            new DeferredFactory(this, "doroti/webview"),
        ];

    private sealed class DeferredFactory(WindowsPlatformViewHost owner, string type)
        : IPlatformViewFactory
    {
        public string ViewType => type;

        private IPlatformViewFactory Resolve() =>
            owner._factories.GetValueOrDefault(type)
            ?? throw new InvalidOperationException(
                "Windows PlatformView parent/presenter is not bound yet."
            );

        public PlatformViewSupport QuerySupport(PlatformViewRequest request) =>
            Resolve().QuerySupport(request);
        ...
        public ValueTask<IPlatformViewInstance> CreateAsync(
            ...
            Resolve()
                .CreateAsync(
                    handle,
                    parameters,
                    value => owner._host!.DispatchPlatformViewEvent(() => focused(value)),
                    cancellationToken
                );
    }

    internal WindowsPlatformViewDispatcher Bind(
        WindowsManagedProductHost host,
        WindowsManagedVulkanPresenter? presenter,
        IApplicationResourceHostCapability resources
    )
    {
        if (_parent != 0)
        {
            throw new InvalidOperationException("PlatformView owner already bound.");
        }
        ...
        _webViews = new(
            ...
            () =>
            {
                ...
                if (RequestFrameworkFrame is { } request)
                {
                    request();
                }
                else
        ...
        if (presenter is not null)
        {
            Native.SetWindowLongPtrW(
                _parent,
                -16,
                Native.GetWindowLongPtrW(_parent, -16) | 0x02000000
            );
        ...
        foreach (var editor in new[] { false, true })
        {
            var factory = new WindowsHwndPlatformViewFactory(_parent, editor, false)
            {
                    ...
                    if (!string.IsNullOrWhiteSpace(path))
                    {
                        File.WriteAllText(
                            ...
                            System.Text.Json.JsonSerializer.Serialize(
                                new
                                {
                                    nativePointerDowns = _nativePointerDowns,
                                    scriptPointerDowns = _scriptPointerDowns,
                                    screenX = (short)(screenPoint.ToInt64() & 0xffff),
                                    screenY = (short)((screenPoint.ToInt64() >> 16) & 0xffff),
                                    extra = extra.ToInt64(),
                                    handle,
                                }
                            )
                ...
                StagePlacement = (hwnd, x, y, width, height, visible) =>
                    Position(
                        new(hwnd, x, y, width, height, 0x0014u | (visible ? 0x0040u : 0x0080u))
                    ),
    ...
    private static bool HasPlatformCommands(IReadOnlyList<SceneCommand> commands, int depth = 0)
    {
        if (depth > 256)
        {
            throw new InvalidDataException("PlatformView retained depth limit exceeded.");
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("Unretired PlatformView raster frame.");
        }
        ...
        var token = new PlatformCompositionToken(
            ...
            Interlocked.Increment(ref _compositionFrame),
        ...
        )
        {
            ...
            throw new NotSupportedException(
                "Windows cannot mix WebView composition visuals and sibling HWNDs in one frame."
            );
        }
        if (
            nativeParts.Any(p => !_webViews!.Contains(p.Placement.Handle))
            && plan.Parts.OfType<PlatformBackdropSegment>()
                .Any(p => p.Style is { Saturation: not 1 })
        )
        {
            plan.Dispose();
            throw new NotSupportedException(
                "Saturation adjustment requires the WebView2 Windows Composition tree; the WinUI island effect does not support it."
            );
        }
        if (
            plan
                ...
                .Any(p =>
                    Math.Abs(p.SigmaX - p.SigmaY) > .001
                    || Math.Abs(token.DeviceScaleX - token.DeviceScaleY) > .001
                )
        )
        {
            plan.Dispose();
            throw new NotSupportedException(
                "Windows Composition requires isotropic backdrop sigma and device scale."
            );
        }
        if (
            plan
                .Parts.OfType<PlatformBackdropSegment>()
                .Any(p => p.SigmaX * token.DeviceScaleX > 128)
        )
        {
            plan.Dispose();
            throw new NotSupportedException(
                "Windows Composition backdrop physical sigma must not exceed 128."
            );
        }
        ...
        try
        {
            ...
            if (
                plan
                    ...
                    .Any(p => !p.Shield.Transform.IsAxisAligned)
            )
            {
                throw new NotSupportedException(
                    "Windows native input shields require axis-aligned rectangles."
                );
            }
            ...
            if (
                ...
                && !(
                    Volatile.Read(ref _hasVisibleParts) && Volatile.Read(ref _liveHwndSources) != 0
                )
            )
            {
                ...
                _recording = new(plan, width, height, [])
                {
                    Started = started,
            ...
            var visualComposition =
                ...
                && nativeParts.All(p => _webViews!.Contains(p.Placement.Handle));
            ...
            var topology =
                (visualComposition ? "visual:" : "hwnd:")
                + string.Join(",", plan.Parts.Select(p => $"{p.GetType().Name}:{p.PaintOrder}"));
            WindowsCompositionSlice[] previous;
            long cacheRevision;
            lock (_gate)
            {
                previous =
                    _rasterCacheScope == cacheScope && _rasterCacheTopology == topology
                        ? _rasterCache
                        : [];
            ...
            foreach (var raster in rasters)
            {
                ...
                if (raster.PaintOrder != 0)
                {
                    ...
                    if (coverage.Count == 0 && visualComposition)
                    {
                        continue;
                    }
                    ...
                    bounds =
                        coverage.Count == 0
                            ? new(0, 0, 1, 1)
                            : new(
                                coverage.Min(s => s.Bounds.Left),
                                coverage.Min(s => s.Bounds.Top),
                                coverage.Max(s => s.Bounds.Right),
                                coverage.Max(s => s.Bounds.Bottom)
                            );
                }
                ...
                var reused =
                    ...
                    && SkiaPlatformRasterContent.CanReuse(
                        cacheScope,
                        new(prior.Segment.Commands, prior.Bounds),
                        cacheScope,
                        new(raster.Commands, bounds),
                        allowTranslation: visualComposition
                    );
                slices.Add(new(raster, bounds, reused ? -1 : atlasHeight, reused));
                if (!reused)
                {
                    atlasHeight = checked(atlasHeight + bounds.Height);
                }
            }
            ...
            if (changes.Length == 0)
            {
                _recording = new(plan, width, height, rasters)
                {
                    Started = started,
                    NativeRevision = nativeRevision,
                    ...
                    ExpectedCacheRevision = expectedRevision,
                };
                return;
            }
            ...
            var reservedBytes =
                ...
                + slices.Sum(s => (long)s.Bounds.Width * s.Bounds.Height * 8);
            if (atlasWidth > 16384 || atlasHeight > 16384 || reservedBytes > 256L * 1024 * 1024)
            {
                throw new InvalidOperationException(
                    "Windows PlatformView active/staging resources exceed 256 MiB / 16384 pixels."
                );
            }
            ...
            {
                shared.Canvas.Save();
                shared.Canvas.ClipRect(
                    SKRect.Create(0, slice.AtlasY, slice.Bounds.Width, slice.Bounds.Height),
                    SKClipOperation.Intersect,
                    false
                );
                shared.Canvas.Translate(-slice.Bounds.Left, slice.AtlasY - slice.Bounds.Top);
                if (slice.Segment.PaintOrder == 0)
                {
                    shared.Canvas.DrawColor(renderer.PlatformBackgroundColor);
                }
        ...
        try
        {
            if (accepted)
            {
                lock (_gate)
                {
                    AddTiming(
                        _rasterMilliseconds,
                        System.Diagnostics.Stopwatch.GetElapsedTime(frame.Started).TotalMilliseconds
                    );
    ...
    internal bool Terminal(long causalFrameId, bool presented, long generation)
    {
        ...
        lock (_gate)
        {
            _ready.Remove(causalFrameId, out frame);
        }
        ...
        using (frame)
        {
            ...
            if (
                !presented
                || _closed
                || frame.Plan.Token.SurfaceGeneration != generation
                || !_host!.IsLatestResizeGeneration(checked((ulong)generation))
            )
            {
                return false;
            }
            ...
            {
                if (
                    frame.ExpectedCacheRevision >= 0
                    && frame.ExpectedCacheRevision != _rasterCacheRevision
                )
                {
            ...
            try
            {
                if (!_session.CommitRetiredFrame(frame.Plan, () => CommitFrame(frame, started)))
                {
                    return false;
                }
            }
            catch
            {
                ...
                {
                    _rasterCache = [];
                    _rasterCacheRevision++;
                }
                throw;
            }
            lock (_gate)
            {
                _rasterCache = frame.Slices;
                _rasterCacheScope = frame.CacheScope;
                _rasterCacheTopology = frame.RasterTopology;
                _rasterCacheRevision++;
            }
            ...
            catch (Exception error) when (error is IOException or UnauthorizedAccessException)
            {
                System.Diagnostics.Trace.TraceWarning(
                    $"PlatformView evidence could not be written: {error.Message}"
                );
            }
            return true;
        }
    ...
    private bool CommitFrame(Frame frame, long started)
    {
        ...
        {
            if (
                next.OfType<PlatformNativeSegment>()
                    .Any(p =>
                        _coordinator!.GetState(p.Placement.Handle)
                            is not (
                                PlatformViewState.Ready
        ...
        var handles = next.OfType<PlatformNativeSegment>()
            .Select(p => p.Placement.Handle)
            .Concat(_visible.OfType<PlatformNativeSegment>().Select(p => p.Placement.Handle))
            .Where(handle => _controls.ContainsKey(handle) || _webViews!.Contains(handle))
            .Distinct()
            .ToArray();
        ...
        if (next.OfType<PlatformNativeSegment>().Any(p => _webViews!.Contains(p.Placement.Handle)))
        {
            ...
            {
                if (
                    _controls.ContainsKey(old.Placement.Handle)
                    && batch.Contains(old.Placement.Handle)
                )
                {
            ...
            _sceneOutput?.Hide();
            foreach (var layer in _banks[_visibleBank])
            {
                layer.Hide();
            }
        ...
        if (
            _winUiSceneVisible
            || next.OfType<PlatformNativeSegment>()
                .Any(p => _controls.ContainsKey(p.Placement.Handle))
        )
        {
            CommitWinUiFrame(frame, next, batch, started);
            return true;
        }
        if (_webViews!.HasVisibleContent)
        {
            _webViews.Clear(batch);
        }
        ...
        var placementChanged =
            !reuseVisible
            || frame.Plan.Token.DeviceScaleX != _visibleScaleX
            || frame.Plan.Token.DeviceScaleY != _visibleScaleY
            || !next.OfType<PlatformNativeSegment>()
        ...
        try
        {
            ...
            foreach (var part in next)
            {
                ...
                else if (part is PlatformNativeSegment native && native.Placement.Visible)
                {
                    ...
                    sources.Add(
                        new()
                        {
                            Hwnd = (ulong)_controls[placement.Handle],
                            Generation = (ulong)placement.Handle.InstanceGeneration,
                            X = Pixel(origin.dx),
                            Y = Pixel(origin.dy),
                            Left = Pixel(clip.left) - Pixel(origin.dx),
                            Top = Pixel(clip.top) - Pixel(origin.dy),
                            Right = Pixel(clip.right) - Pixel(origin.dx),
                            Bottom = Pixel(clip.bottom) - Pixel(origin.dy),
                        }
                    );
            ...
            if (placementChanged)
            {
                ...
                if (!reuseVisible)
                {
                    foreach (var old in _banks[_visibleBank])
                    {
                        Position(new(old.Hwnd, 0, 0, 0, 0, 0x0097));
                    }
                }
            ...
            if (layerCount != 0)
            {
                ...
                if (!Native.IsWindowVisible(_sceneOutput.Hwnd))
                {
                    ...
                    Marshal.ThrowExceptionForHR(
                        Native.WaitForCompositionCommit(_sceneOutput.Surface)
                    );
                    _positions = [];
    ...
    private void CommitWinUiFrame(
        Frame frame,
        PlatformCompositionPart[] next,
    ...
    )
    {
        ...
        if (frame.Slices.Length == 0)
        {
            ...
            foreach (var old in _visible.OfType<PlatformNativeSegment>())
            {
                if (batch.Contains(old.Placement.Handle))
                {
                    RunNow(_ => batch.DetachAsync(old.Placement.Handle));
                }
            }
        ...
        var pool = _banks[bank];
        while (pool.Count < frame.Slices.Length)
        {
            pool.Add(new(this));
        }
        ...
        _winUiBackdrop!.Prepare(
            frame.Plan,
            frame.Shared,
            frame.Slices,
            frame.Width,
            frame.Height,
            handle => _controls[handle]
        );
        foreach (var native in next.OfType<PlatformNativeSegment>())
        {
            ...
            _winUiControls.SetInputShields(
                _controls[placement.Handle],
                next.OfType<PlatformShieldSegment>()
                    .Where(shield => shield.PaintOrder > native.PaintOrder)
                    .Select(shield => ShieldBounds(shield.Shield).intersect(bounds))
        ...
        if (frame.Slices.Length != 0)
        {
            Marshal.ThrowExceptionForHR(Native.CommitComposition(pool[0].Surface));
        }
        ...
        try
        {
            ...
            foreach (var part in next)
            {
                ...
                else if (part is PlatformNativeSegment native)
                {
                    ...
                    if (!sameTopology)
                    {
                        Position(new(_controls[native.Placement.Handle], 0, 0, 0, 0, 0x0013));
                    }
                }
                else if (
                    part is PlatformBackdropSegment effect
                    && (!sameTopology || viewportChanged)
                )
                {
                    Position(
                        new(
                            _winUiBackdrop.WindowFor(effect.PaintOrder),
                            0,
                            0,
                            frame.Width,
                            frame.Height,
            ...
            for (var i = raster; i < pool.Count; i++)
            {
                Position(new(pool[i].Hwnd, 0, 0, 0, 0, 0x0097));
            }
            ...
            _winUiWidth = frame.Width;
            _winUiHeight = frame.Height;
            _visibleScaleX = frame.Plan.Token.DeviceScaleX;
            _visibleScaleY = frame.Plan.Token.DeviceScaleY;
            Volatile.Write(ref _hasVisibleParts, next.Length != 0);
            Volatile.Write(ref _needsReplay, false);
    ...
    {
        var previousWindows = _visible
            .Where(p =>
                p is PlatformRasterSegment or PlatformNativeSegment or PlatformBackdropSegment
            )
            .ToArray();
        var nextWindows = next.Where(p =>
                p is PlatformRasterSegment or PlatformNativeSegment or PlatformBackdropSegment
            )
            .ToArray();
            ...
            && previousWindows
                ...
                .All(pair =>
                    (pair.First, pair.Second) switch
                    {
                        ...
                        (PlatformNativeSegment before, PlatformNativeSegment after) => before
                            .Placement
                            .Handle == after.Placement.Handle,
                        _ => false,
                    }
                );
    }
    ...
    private void Apply(
        PlatformCompositionPart[] parts,
        List<CompositionRaster> layers,
        int rasterCount,
        IPlatformViewPlacementBatch batch
    )
    {
        ...
        foreach (var old in _visible.OfType<PlatformNativeSegment>())
        {
            if (!handles.Contains(old.Placement.Handle) && batch.Contains(old.Placement.Handle))
            {
                ...
                { /* Removal already disabled this retiring instance. */
                }
            }
        }
        ...
        for (var index = raster; index < layers.Count; index++)
        {
            Position(new(layers[index].Hwnd, 0, 0, 0, 0, 0x0097));
        }
    }

    private void Position(WindowPosition position)
    {
        if (_positions is { } positions)
        {
    ...
    private static void RunNow(Func<CancellationToken, ValueTask> action)
    {
        ...
        if (!task.IsCompleted)
        {
            ...
            _ = Observe(task);
            throw new InvalidOperationException(
                "Native placement is retiring; composition was not committed."
            );
        }
    ...
    private bool Intercepts(int nativeOrder, nint packedScreenPoint)
    {
        ...
        {
            X = (short)(packedScreenPoint.ToInt64() & 0xffff),
            Y = (short)((packedScreenPoint.ToInt64() >> 16) & 0xffff),
        };
        if (!Native.ScreenToClient(_parent, ref point))
        {
            return true;
        }
        ...
        return _visible
            ...
            .Any(p => p.PaintOrder > nativeOrder && ShieldBounds(p.Shield).contains(logical));
    }
    ...
    private nint NativeAt(Native.Point point)
    {
        ...
        foreach (var native in _visible.OfType<PlatformNativeSegment>().Reverse())
        {
            ...
            if (
                placement.Visible
                && bounds.contains(logical)
                && !_visible
                    .OfType<PlatformShieldSegment>()
                    .Any(shield =>
                        shield.PaintOrder > native.PaintOrder
                        && ShieldBounds(shield.Shield).contains(logical)
                    )
            )
            {
    ...
    public void Dispose()
    {
        ...
        if (_session is { } session)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Windows composition retirement remained after raster shutdown."
                );
            }
        ...
        lock (_gate)
        {
            foreach (var frame in _ready.Values)
            {
                frame.Dispose();
            }
            _ready.Clear();
        ...
        {
            _dispatcher.DrainShutdown(coordinator.DisposalCompletion);
        }
    ...
    private void WriteEvidence(Frame frame)
    {
        ...
        )
        {
            ...
            var count = int.TryParse(
                Environment.GetEnvironmentVariable("DOROTI_WINDOWS_PLATFORM_CAPTURE_FRAMES"),
                out var requested
            )
                ? Math.Clamp(requested, 1, 60)
                : 1;
        ...
        lock (_gate)
        {
            rasterTimings = _rasterMilliseconds.ToArray();
        }

        var payload = new
        {
            backend = _winUiSceneVisible
                ? "Graphite/Vulkan raster slices + live WinUI 3 XAML Islands"
            : _webViews?.HasVisibleContent == true
            ...
            captureProbeStatus = _captureProbe?.IsCompletedSuccessfully == true
                ? _captureProbe.Result
                : (int?)null,
            frame = frame.Plan.Token,
            native = _visible
                .OfType<PlatformNativeSegment>()
                .Select(p => new
                {
                    handle = p.Placement.Handle,
                    hwnd = _controls.GetValueOrDefault(p.Placement.Handle).ToInt64(),
                    bounds = Coordinates(p.Placement.Bounds),
                    transform = p.Placement.Transform,
                    order = p.PaintOrder,
                }),
            shields = _visible
                .OfType<PlatformShieldSegment>()
                .Select(p => new
                {
                    bounds = Coordinates(ShieldBounds(p.Shield)),
                    order = p.PaintOrder,
                }),
            sessionCommit = _session?.LastCommit,
            sessionPendingRetirements = _session?.PendingRetirements,
            rasterCount = frame.Rasters.Length,
            physicalAtomicDisplay = "notVerified",
            hwndBackdrops = _visible
                .OfType<PlatformBackdropSegment>()
                .Select(effect => new
                {
                    bounds = Coordinates(effect.Bounds),
                    effect.SigmaX,
                    effect.SigmaY,
                    source = "DirectComposition live layered HWND and preceding raster surfaces",
                    nativeContentCaptured = false,
                }),
            rasterSlices = frame.Slices.Select(slice => new
            {
                order = slice.Segment.PaintOrder,
                bounds = new[]
                {
                    slice.Bounds.Left,
                    slice.Bounds.Top,
            ...
            effectProbe = Environment.GetEnvironmentVariable("DOROTI_PLATFORM_EFFECT_PROBE_STATE"),
            webView = _webViews?.Evidence,
            timings = new
            {
                sampleLimit = 128,
                rasterSamples = rasterTimings.Length,
                uiSamples = _uiMilliseconds.Count,
                rasterAndReadbackP50Ms = Percentile(rasterTimings, .5),
                rasterAndReadbackP95Ms = Percentile(rasterTimings, .95),
                rasterAndReadbackP99Ms = Percentile(rasterTimings, .99),
                uiCommitP50Ms = Percentile(_uiMilliseconds.ToArray(), .5),
                uiCommitP95Ms = Percentile(_uiMilliseconds.ToArray(), .95),
    ...
    private static double Percentile(double[] values, double percentile)
    {
        ...
        return values.Length == 0
            ? 0
            : values[Math.Max(0, (int)Math.Ceiling(values.Length * percentile) - 1)];
    }
    ...
    private sealed class CompositionRaster : IDisposable
    {
        ...
        internal CompositionRaster(WindowsPlatformViewHost owner)
        {
            ...
            try
            {
                ...
                {
                    Marshal.ThrowExceptionForHR(
                        Native.CreateSharedCompositionRaster(
                            shared.Surface,
                            (ulong)_hwnd,
                            out _surface
                        )
        ...
        )
        {
            if (message == 0x0084)
            {
                if (_owner._winUiSceneVisible)
                {
                    ...
                    if (_owner.NativeAt(point) != 0)
                    {
                        return -1; // HTTRANSPARENT: native WinUI input.
                    }
                }
                return 1;
            }
            if (message == 0x0021)
            {
                return Native.SendMessageW(_owner._parent, message, (nuint)_owner._parent, lparam);
            }
        ...
        internal void Prepare(
            WindowsSharedRaster? image,
            WindowsCompositionSlice slice,
            int frameWidth,
            int frameHeight
        )
        {
            if (
                ...
                && _width == frameWidth
                && _height == frameHeight
                && _rasterContent is { } previous
                && previous.Bounds == slice.Bounds
                && SkiaPlatformRasterContent.Equivalent(
                    previous.Segment.Commands,
                    slice.Segment.Commands
                )
            )
            {
            ...
            if (slice.Reused || image is null)
            {
                throw new InvalidOperationException("Missing shared Windows raster surface.");
            }

            Marshal.ThrowExceptionForHR(
                Native.UpdateCompositionRasterShared(
                    _surface,
                    image.Handle,
                    (uint)slice.AtlasY,
                    (uint)slice.Bounds.Width,
                    (uint)slice.Bounds.Height,
                    slice.Bounds.Left,
                    slice.Bounds.Top
                )
            );
        ...
        internal void PrepareBackdrop(
            Native.CompositionSource[] sources,
        ...
        )
        {
            ...
            var bounds = effect.Bounds.intersect(
                Rect.fromLTWH(0, 0, frame.Width / scale, frame.Height / scale)
            );
            if (bounds.isEmpty)
            {
            ...
            Marshal.ThrowExceptionForHR(
                Native.UpdateCompositionBackdrop(
                    _surface,
                    sources,
                    (uint)sources.Length,
        ...
        public void Dispose()
        {
            if (_surface != 0)
            {
                Native.DestroyCompositionRaster(_surface);
            }
    ...
    private static class Native
    {
        [DllImport(
            ...
            EntryPoint = "doroti_windows_composition_raster_shared_v1",
            CallingConvention = CallingConvention.Cdecl
        )]
        internal static extern int UpdateCompositionRasterShared(
            nint raster,
            nint handle,
            uint sourceY,
            uint width,
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_platform_capture_probe_v1",
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_composition_scene_update_v1"
        )]
        internal static extern int UpdateCompositionScene(
            nint surface,
            [In] CompositionSource[] sources,
            uint count,
            float width,
            float height
        );
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_composition_raster_create_shared_v1"
        )]
        internal static extern int CreateSharedCompositionRaster(
            nint shared,
            ulong hwnd,
            out nint raster
        );
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_composition_backdrop_update_v1"
        )]
        internal static extern int UpdateCompositionBackdrop(
            nint raster,
            [In] CompositionSource[] sources,
            uint count,
            float left,
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_composition_raster_update_region_v1"
        )]
        internal static extern int UpdateCompositionRasterRegion(
            nint surface,
            nint pixels,
            uint width,
            uint height,
        ...
        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_composition_commit_v1"
        )]
        internal static extern int CommitComposition(nint surface);

        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_composition_wait_commit_v1"
        )]
        internal static extern int WaitForCompositionCommit(nint surface);

        [DllImport(
            WindowsNativeV1.LibraryName,
            EntryPoint = "doroti_windows_composition_raster_destroy_v1"
        )]
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        internal static extern bool RedrawWindow(nint hwnd, nint rect, nint region, uint flags);
        ...
        [DllImport("user32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        internal static extern bool EndDeferWindowPos(nint batch);
