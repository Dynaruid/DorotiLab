...
internal abstract class WindowsManagedHwndPresenterBase : IDisposable
{
    ...
    internal virtual string VisibleOwner =>
        UsesCompositionTopology ? "ContentIsland" : "child HWND";
    internal virtual string TopologySlug =>
        UsesCompositionTopology ? "content-island" : "cpp-child-hwnd";
    ...
    internal virtual void AttachWindow(nint topLevelWindow) =>
        throw new NotSupportedException(
            $"{GetType().Name} does not own a Composition window topology."
        );
    ...
    internal virtual void ResetDeviceAfterRendererGpuResourceRelease(bool deviceLost) =>
        ResetDevice();
