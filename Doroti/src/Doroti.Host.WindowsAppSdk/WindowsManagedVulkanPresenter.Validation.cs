...
internal sealed unsafe partial class WindowsManagedVulkanPresenter
{
    private readonly bool _validationEnabled =
        Environment.GetEnvironmentVariable("DOROTI_WINDOWS_VULKAN_VALIDATION") == "1";
    ...
    )
    {
        try
        {
            ...
            {
                Interlocked.Increment(ref _validationWarnings);
            }

            var message =
                data == null
                    ? severity.ToString()
                    : Marshal.PtrToStringUTF8((nint)data->PMessage) ?? severity.ToString();
            lock (_validationMessages)
            {
                if (_validationMessages.Count < 128)
                {
    ...
    private void CreateValidationMessenger(DebugUtilsMessengerCreateInfoEXT* info)
    {
        ...
        if (create == null)
        {
            throw new PlatformNotSupportedException("Vulkan debug utils missing.");
        }
    ...
    {
        if (_validationMessenger.Handle == 0)
        {
            return;
        }

        var destroy = (delegate* unmanaged<nint, DebugUtilsMessengerEXT, void*, void>)
            (nint)_vk.GetInstanceProcAddr(_instance, "vkDestroyDebugUtilsMessengerEXT");
        destroy(_instance.Handle, _validationMessenger, null);
        _validationMessenger = default;
    }
}
