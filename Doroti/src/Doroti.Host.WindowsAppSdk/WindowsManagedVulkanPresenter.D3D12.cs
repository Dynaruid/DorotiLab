...
internal sealed unsafe partial class WindowsManagedVulkanPresenter
{
    ...
    {
        if (_presentationContext == 0) return _lastD3D12Snapshot;
        ...
        return _lastD3D12Snapshot = new(native.SubmittedCopies, native.CompletedCopies,
            native.ResourceAllocations, native.SwapchainResizes, native.DebugEnabled != 0,
            native.DebugErrors, native.DebugWarnings, native.ActiveSwapchains,
    ...
    private void ImportD3D12ProducerFence(ulong sharedHandle)
    {
        ...
        var properties = new ExternalSemaphoreProperties { SType = StructureType.ExternalSemaphoreProperties };
        _vk.GetPhysicalDeviceExternalSemaphoreProperties(_physicalDevice, &external, &properties);
        if ((properties.ExternalSemaphoreFeatures & ExternalSemaphoreFeatureFlags.ImportableBit) == 0 ||
            (properties.CompatibleHandleTypes & ExternalSemaphoreHandleTypeFlags.D3D12FenceBit) == 0)
            throw new PlatformNotSupportedException("The selected Vulkan device cannot import a D3D12 timeline fence.");
        var create = new SemaphoreCreateInfo { SType = StructureType.SemaphoreCreateInfo, PNext = &type };
        Check(_vk.CreateSemaphore(_device, &create, null, out _d3d12ProducerSemaphore), "vkCreateSemaphore(D3D12 timeline)");
        if (!_vk.TryGetDeviceExtension<KhrExternalSemaphoreWin32>(_instance, _device, out var api))
            throw new PlatformNotSupportedException("VK_KHR_external_semaphore_win32 is unavailable.");
        using (api)
        {
            ...
            Check(api.ImportSemaphoreWin32Handle(_device, &import), "vkImportSemaphoreWin32HandleKHR(D3D12 fence)");
        }
        _d3d12ProducerValue = 0;
        RecordEvent("D3D12 output: same-LUID resources, shared timeline fence, DIRECT queue, DXGI flip-sequential");
    }
    ...
    [LibraryImport(WindowsNativeV1.LibraryName, EntryPoint = "doroti_windows_d3d12_output_ready_v1")]
    ...
    {
        internal ulong SubmittedCopies, CompletedCopies, ResourceAllocations, SwapchainResizes;
        internal ulong DebugEnabled, DebugErrors, DebugWarnings, ActiveSwapchains;
    }

    [LibraryImport(WindowsNativeV1.LibraryName, EntryPoint = "doroti_windows_d3d12_output_snapshot_v1")]
...
internal sealed record D3D12OutputSnapshot(
    ulong SubmittedCopies, ulong CompletedCopies, ulong ResourceAllocations, ulong SwapchainResizes,
    bool DebugEnabled, ulong DebugErrors, ulong DebugWarnings, ulong ActiveSwapchains,
    string Retirement, string DisplayWait);
