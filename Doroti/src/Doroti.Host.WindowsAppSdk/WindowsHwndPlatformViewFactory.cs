...
public sealed class WindowsHwndPlatformViewFactory : IPlatformViewFactory
{
    ...
    public WindowsHwndPlatformViewFactory(nint parent, bool editor, bool lowerCompositionTarget)
    {
        ...
        if (!Native.IsWindow(parent) || Native.GetWindowThreadProcessId(parent, out _) != _uiThread)
        {
            throw new ArgumentException(
                "The native parent must belong to the current UI thread.",
                nameof(parent)
            );
        }
    }
    ...
    public PlatformViewSupport QuerySupport(PlatformViewRequest request)
    {
        ...
        var backdrop =
            supported
            && SiblingRasterTopology
            && request.Composition == PlatformViewComposition.InterleavedComposition;
        return new(
            WinUiControls is null ? "Windows-HWND" : "Windows-WinUI3",
            ...
            Capabilities: new(
                ...
                WinUiControls is null
                    ...
                    : PlatformViewTransport.Native,
                PlatformViewInputPolicy.DirectNative,
                backdrop
                    ? WinUiControls is null
                        ? WindowsWebViewComposition.Effects
                        : WindowsWinUiBackdrop.Support
    ...
    )
    {
        ...
        var hwnd =
            ...
            ?? Native.CreateWindowExW(
                (_editor ? 0x200u : 0u) | (SiblingRasterTopology ? 0x00080000u : 0u),
                _editor ? "EDIT" : "BUTTON",
                text,
                0x40000000u | 0x00010000u | (_editor ? 0x00800080u : 0u),
                0,
                0,
                0,
                0,
    ...
    {
        if (Native.GetCurrentThreadId() != _uiThread)
        {
            throw new InvalidOperationException(
                "HWND attachment operation must run on its owner UI thread."
            );
        }
    }
    ...
    private sealed class Instance : IPlatformViewInstance
    {
        ...
        public Instance(
            WindowsHwndPlatformViewFactory owner,
            nint hwnd,
            PlatformViewHandle handle,
            Action<PlatformViewHandle> onFocused,
        ...
        )
        {
            ...
            _island = island;
            if (!Native.SetWindowSubclass(hwnd, _callback, 1, 0))
            {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
            ...
            if (island is not null)
            {
                island.Focused = () =>
                {
                    if (_inputEnabled && _visible)
                    {
                        _owner.YieldFrameworkTextInput?.Invoke();
                        _onFocused(_handle);
                    }
                };
                island.ModifierKey = (message, key, flags) =>
                    Native.SendMessageW(_owner._parent, message, key, flags);
                island.TakeFocus = () =>
                {
                    YieldFocus();
        ...
        )
        {
            ...
            if (
                _island is null
                && message is 0x100 or 0x101 or 0x104 or 0x105
                && (wparam is 9 or 16 or 17 or 18 or 160 or 161 or 162 or 163 or 164 or 165)
                && _owner.SiblingRasterTopology
            )
            {
            ...
            if (_island is null && message == 0x102 && wparam == 9 && _owner.SiblingRasterTopology)
            {
                return 0;
            }
            ...
            if (message == 0x84)
            {
                try
                {
                    if (_owner.InterceptsPoint?.Invoke(_paintOrder, lparam) == true)
                    {
                        return 1;
                    }
                }
            ...
            if (message == 0x82)
            {
                Native.RemoveWindowSubclass(hwnd, _callback, 1);
                _hwnd = 0;
                try
                {
                    _owner.Destroyed?.Invoke(_handle);
                }
                catch (Exception error)
                {
                    System.Diagnostics.Trace.TraceError(error.ToString());
        ...
        private static nint Pack(Native.Point point) =>
            (nint)((ushort)point.X | ((uint)(ushort)point.Y << 16));
        ...
        public ValueTask ApplyAsync(PlatformViewPlacement placement)
        {
            ...
            {
                throw new InvalidOperationException(
                    "HWND only supports translation and rectangular clipping."
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    "A platform HWND cannot be reparented outside its owner."
                );
            }
            ...
            if (parentDpi == 0)
            {
                throw new InvalidOperationException("Native HWND parent has no valid DPI.");
            }
            ...
            int x = Pixel(bounds.left),
                y = Pixel(bounds.top),
                right = Pixel(bounds.right),
                bottom = Pixel(bounds.bottom);
            if (
                _owner.StagePlacement is null
                && !Native.SetWindowPos(_hwnd, 0, x, y, right - x, bottom - y, 0x0010 | 0x0004)
            )
            {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
            ...
            {
                clipLeft = clipTop = 0;
                clipRight = sourceWidth;
                clipBottom = sourceHeight;
            }
            ...
            if (_clipRegion != nextClip)
            {
                ...
                if (Native.SetWindowRgn(_hwnd, region, false) == 0)
                {
            ...
            else if (_owner.StagePlacement is { } placementStage)
            {
                placementStage(_hwnd, x, y, right - x, bottom - y, _visible);
            }
            else
            {
                Native.ShowWindow(_hwnd, _visible ? 4 : 0);
            }
        ...
        private void YieldFocus()
        {
            if (
                _hwnd != 0
                && (
                    Native.GetFocus() == _hwnd
                    || Native.IsChild(_hwnd, Native.GetFocus())
                    || _island?.HasFocus == true
                )
            )
            {
            ...
            if (
                _hwnd != 0
                && (Native.GetCapture() == _hwnd || Native.IsChild(_hwnd, Native.GetCapture()))
            )
            {
        ...
        public ValueTask SetFocusAsync(bool focused)
        {
            ...
            if (focused)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "A hidden, detached, or retiring platform HWND cannot receive focus."
                    );
                }
                ...
                if (_island is not null)
                {
                    if (!_island.Focus())
                    {
                        throw new InvalidOperationException("WinUI island focus request failed.");
                    }
                }
                else
                {
                    Native.SetFocus(_hwnd);
                    if (Native.GetFocus() != _hwnd)
                    {
                        throw new InvalidOperationException("Native HWND focus request failed.");
                    }
                }
            }
            else
        ...
        public ValueTask DisableInputAsync()
        {
            ...
            {
                YieldFocus();
                Native.EnableWindow(_hwnd, false);
            }
            return ValueTask.CompletedTask;
        }
        ...
        public ValueTask DisposeAsync()
        {
            ...
            {
                YieldFocus();
                if (_island is null && !Native.DestroyWindow(_hwnd))
                {
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                }
            }
    ...
    private static class Native
    {
        ...
        [DllImport("user32.dll")]
        [return: MarshalAs(UnmanagedType.Bool)]
        internal static extern bool IsChild(nint parent, nint hwnd);
