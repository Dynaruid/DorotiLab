...
public class _AxisSize__wrap
{
    ...
    public virtual _AxisSize__wrap op_Add(_AxisSize__wrap other) =>
        Create_(
            new Size(_size.width + other._size.width, Math.Max(_size.height, other._size.height))
        );

    public virtual _AxisSize__wrap op_Subtract(_AxisSize__wrap other) =>
        Create_(new Size(_size.width - other._size.width, _size.height - other._size.height));
}
...
public static class WrapAlignmentMembers
{
    ...
    )
    {
        ...
        return value switch
        {
            ...
            WrapAlignment.spaceBetween when itemCount < 2L => WrapAlignment.start._distributeSpace(
                freeSpace,
                itemSpacing,
                itemCount,
                flipped
            ...
            WrapAlignment.spaceBetween => (0, (freeSpace / (itemCount - 1L)) + itemSpacing),
...
{
    internal static WrapCrossAlignment _flipped(this WrapCrossAlignment value) =>
        value switch
        {
...
internal class _RunMetrics__wrap
{
    ...
    public virtual _RunMetrics__wrap? tryAddingNewChild(
        RenderBox child,
        _AxisSize__wrap childSize,
        bool flipMainAxis,
        double spacing,
    ...
    )
    {
        ...
        {
            axisSize = axisSize.op_Add(
                childSize.op_Add(new _AxisSize__wrap(mainAxisExtent: spacing, crossAxisExtent: 0.0))
            );
            childCount += 1L;
            if (flipMainAxis)
            {
        ...
        RenderBoxContainerDefaultsMixin<RenderBox, WrapParentData>
{
    ...
    internal virtual bool _debugHasNecessaryDirections
    {
        get
        {
            ...
            if (
                Equals(runAlignment, WrapAlignment.start) || Equals(runAlignment, WrapAlignment.end)
            )
            {
                switch (direction)
                {
            ...
            if (
                Equals(crossAxisAlignment, WrapCrossAlignment.start)
                || Equals(crossAxisAlignment, WrapCrossAlignment.end)
            )
            {
                switch (direction)
                {
    ...
    public override double computeMinIntrinsicHeight(double width)
    {
        switch (direction)
        {
            ...
            case Axis.vertical:
            {
                ...
                while (child is not null)
                {
                    heightLocal = Math.Max(
                        heightLocal,
                        child.getMinIntrinsicHeight(double.PositiveInfinity)
                    );
                    child = childAfter(child);
                }
                return heightLocal;
    ...
    internal virtual (bool, bool) _areAxesFlipped
    {
        get
        {
            bool flipHorizontal = (textDirection ?? TextDirection.ltr) switch
            {
    ...
    public override double? computeDryBaseline(BoxConstraints constraints, TextBaseline baseline)
    {
        ...
        BoxConstraints childConstraints = direction switch
        {
        ...
        _AxisSize__wrap containerAxisSize = childrenAxisSize.applyConstraints(
            constraints,
            direction
        );
        ...
        void findHighestBaseline(Offset offset, RenderBox child)
        {
            baselineOffset = baselineOffset.minOf(
                new BaselineOffset(child.getDryBaseline(childConstraints, baseline)).op_Add(
                    offset.dy
                )
            );
        }
    ...
    internal new virtual Size _computeDryLayout(
    ...
    )
    {
        ...
        {
            Axis.horizontal => ((BoxConstraints, double))
    ...
    internal virtual (_AxisSize__wrap, List<_RunMetrics__wrap>) _computeRuns(
        BoxConstraints constraints,
    ...
    )
    {
        ...
        {
            var childSize = _AxisSize__wrap.CreateFromSize(
                size: layoutChild(child, childConstraints),
                direction: direction
            );
            _RunMetrics__wrap? newRun =
                (currentRun is null)
                    ? new _RunMetrics__wrap(child, childSize)
            ...
            if (newRun is not null)
            {
                ...
                childrenAxisSize = childrenAxisSize.op_Add(
                    currentRun?.axisSize.flipped ?? _AxisSize__wrap.empty
                );
                currentRun = newRun;
            }
        }
        ...
        childrenAxisSize = childrenAxisSize.op_Add(
            new _AxisSize__wrap(mainAxisExtent: totalRunSpacing, crossAxisExtent: 0.0).op_Add(
    ...
    internal virtual void _positionChildren(
        List<_RunMetrics__wrap> runMetrics,
        _AxisSize__wrap freeAxisSize,
        _AxisSize__wrap containerAxisSize,
        Action<Offset, RenderBox> positionChild,
        Func<RenderBox, Size> getChildSize
    )
    {
        DartRuntimePrimitives.Assert(() => checked((long)runMetrics.Count) != 0);
        double spacingLocal = spacing;
        ...
        WrapCrossAlignment effectiveCrossAlignment = flipCrossAxis
            ? WrapCrossAlignmentMembers._flipped(crossAxisAlignment)
            : crossAxisAlignment;
        var (runLeadingSpace, runBetweenSpace) = runAlignment._distributeSpace(
            crossAxisFreeSpace,
            runSpacing,
            checked(runMetrics.Count),
            flipCrossAxis
        );
        Func<RenderBox, RenderBox?> nextChild = flipMainAxis ? childBefore : childAfter;
        var runCrossAxisOffset = runLeadingSpace;
        IEnumerable<_RunMetrics__wrap> runs = flipCrossAxis
            ? Enumerable.Reverse(runMetrics)
            : runMetrics;
        foreach (var run in runs)
        {
            double runCrossAxisExtent = run.axisSize.crossAxisExtent;
            long childCountLocal = run.childCount;
            double mainAxisFreeSpace = Math.Max(
                0.0,
                containerAxisSize.mainAxisExtent - run.axisSize.mainAxisExtent
            );
            var (childLeadingSpace, childBetweenSpace) = alignment._distributeSpace(
                mainAxisFreeSpace,
                spacingLocal,
                childCountLocal,
                flipMainAxis
            ...
            for (
                ...
                (child is not null) && (remainingChildCount > 0L);
                child = nextChild(child), remainingChildCount -= 1L
            )
            {
                var __pattern28999 = _AxisSize__wrap.CreateFromSize(
                ...
                double childCrossAxisOffset =
                    WrapCrossAlignmentMembers._alignment(effectiveCrossAlignment)
        ...
        else
        {
            ...
            else
            {
                ...
                var childPreviousSiblingParentData = (
                    (WrapParentData?)(object?)childParentData.previousSibling!.parentData!
                )!;
                var childNextSiblingParentData = (
                    (WrapParentData?)(object?)childParentData.nextSibling!.parentData!
                )!;
