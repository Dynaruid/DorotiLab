...
public sealed class RenderPlatformView : RenderBox
{
    ...
    public override Size computeDryLayout(BoxConstraints constraints)
    {
        ...
        if (!value.isFinite)
        {
            throw new InvalidOperationException("PlatformView requires bounded width and height.");
        }
    ...
    {
        if (_handle is { } handle && !size.isEmpty)
        {
            context.addLayer(new TypedPlatformViewLayer(handle, offset & size));
        }
    }
...
public sealed class TypedPlatformViewLayer(PlatformViewHandle handle, Rect bounds) : Layer
{
    ...
    public override void addToScene(SceneBuilder builder) =>
        builder.addPlatformView(handle, bounds.topLeft, bounds.width, bounds.height);
}
...
public sealed class RenderPointerInterceptor : RenderProxyBox
{
    ...
    public override void paint(PaintingContext context, Offset offset)
    {
        if (Intercepting && !size.isEmpty)
        {
            context.addLayer(new InputShieldLayer(offset & size, Debug));
        }
