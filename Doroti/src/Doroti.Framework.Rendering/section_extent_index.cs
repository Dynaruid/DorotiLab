...
public sealed class SectionExtentIndex
{
    ...
    public int CachedConfigurations =>
        _current is null ? 0
        : _previous is null ? 1
        : 2;
    ...
    public SectionExtentIndex(int count, double estimatedExtent = 300)
    {
        ...
        if (!double.IsFinite(estimatedExtent) || estimatedExtent <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(estimatedExtent));
        }

        Count = count;
        _estimate = estimatedExtent;
    }

    public void RequestItem(int index, double offset = 0)
    {
        if ((uint)index >= Count || !double.IsFinite(offset) || offset < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(index));
        }

        RequestedAnchor = (index, offset);
        Changed?.Invoke();
    }
    ...
    public void Configure(double width, object? metricRevision)
    {
        ...
        if (_current is { } same && same.Width == width && Equals(same.Revision, metricRevision))
        {
            return;
        }
    ...
    {
        for (var i = index + 1; i < config.Tree.Length; i += i & -i)
        {
            config.Tree[i] += delta;
        }
    }
    ...
    public void Invalidate(int index)
    {
        ...
        {
            current.States[index] = Measurement.Invalidated;
        }

        if (_previous is { } previous)
        {
            previous.States[index] = Measurement.Invalidated;
        }
    }

    public void Measure(int index, double extent)
    {
        if ((uint)index >= Count || !double.IsFinite(extent) || extent < 0)
        {
        ...
        config.Heights[index] = extent;
        config.States[index] = Measurement.Measured;
    }
}
