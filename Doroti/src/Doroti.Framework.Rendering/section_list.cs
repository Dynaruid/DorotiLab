...
public sealed class RenderSectionList : RenderSliverMultiBoxAdaptor, KeepAliveReleaseListener
{
    ...
    public RenderSectionList(
        RenderSliverBoxChildManager manager,
        SectionExtentIndex index,
        object? metricRevision,
        int[] retain,
    ...
    {
        _index = index;
        _revision = metricRevision;
        _retain = retain;
        _suspended = suspended;
        index.Changed = markNeedsLayout;
    }
    ...
    public void Update(
        ...
        int[] retain,
        bool suspended = false,
        bool restoreRetainedChildren = false
    )
    {
        if (!ReferenceEquals(_index, index))
        {
            if (_index.Changed == markNeedsLayout)
            {
                _index.Changed = null;
            }
            _index = index;
    ...
    public override void performLayout()
    {
        ...
        {
            var releases = _pendingReleases.ToArray();
            ...
            invokeLayoutCallback<SliverConstraints>(_ =>
            {
                foreach (var child in releases)
                {
                    if (
                        ...
                        && child.parentData
                            is SliverMultiBoxAdaptorParentData { keepAlive: false, keptAlive: true }
                    )
                    {
        ...
        if (
            _configured
            && (_width != c.crossAxisExtent || !Equals(_configuredRevision, _revision))
            && _index.RequestedAnchor is null
        )
        {
        ...
        _configured = true;
        _width = c.crossAxisExtent;
        _configuredRevision = _revision;
        ...
        if (_restore)
        {
            ...
            {
                if (_keepAliveBucket.ContainsKey(i))
                {
                    continue;
                }

                if (addInitialChild(i, _index.Prefix(i)))
                {
                    Trim(1, 0);
                }
            }
        ...
        {
            if (start < indexOf(first) || start > indexOf(lastChild!))
            {
                Trim(childCount, 0);
            }
            else
        ...
        for (var i = start; i < _index.Count; i++)
        {
            ...
            else
            {
                ...
                if (next is not null)
                {
                    next.layout(box, parentUsesSize: true);
                }
                else
                {
                    next = insertAndLayoutChild(box, child, parentUsesSize: true);
                }
            ...
            _index.Measure(i, extent);
            end += extent;
            last = i;
            if (end >= target && i >= anchor)
            {
                break;
            }
        }
        ...
        {
            geometry = new SliverGeometry(scrollOffsetCorrection: correctionAfterMeasure);
            return;
        }
        _index.AnchorIndex = anchor;
        _index.AnchorOffset = inside;
        geometry = new SliverGeometry(
            scrollExtent: _index.Total,
            maxPaintExtent: _index.Total,
