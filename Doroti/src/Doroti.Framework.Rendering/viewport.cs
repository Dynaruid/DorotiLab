...
public class RevealedOffset
{
    ...
    )
    {
        ...
        (smaller, larger) = inverted
            ? (((RevealedOffset, RevealedOffset))(leadingEdgeOffset, trailingEdgeOffset))
            : (((RevealedOffset, RevealedOffset))(trailingEdgeOffset, leadingEdgeOffset));
...
public abstract class RenderViewportBase<ParentDataClass>
    : RenderBox,
        ContainerRenderObjectMixin<RenderSliver, ParentDataClass>
    where ParentDataClass : ContainerParentDataMixin<RenderSliver>
{
    ...
    )
    {
        ...
        System.Diagnostics.Debug.Assert(
            (cacheExtent is not null) || Equals(cacheExtentStyle, CacheExtentStyle.pixel)
        );
    }
        ...
        childrenInPaintOrder
            .where(sliver =>
            {
                ...
                return geometry is not null
                    && (geometry.visible || geometry.cacheExtent > 0.0 || sliver.ensureSemantics);
            })
            .forEach(visitor);
    }
    ...
    public virtual ScrollCacheExtent scrollCacheExtent
    {
        ...
        {
            var __value = value;
            ScrollCacheExtent effectiveValue =
                __value
    ...
    public virtual double layoutChildSequence(
        RenderSliver? child,
        double scrollOffset,
        double overlap,
        double layoutOffset,
    ...
    )
    {
        ...
        ScrollDirection adjustedUserScrollDirection =
            SliverLibrary.applyGrowthDirectionToScrollDirection(
        ...
        while (child is not null)
        {
            ...
            child.layout(
                new SliverConstraints(
                    ...
                    precedingScrollExtent: precedingScrollExtentLocal,
    ...
    public override Rect? describeApproximatePaintClip(RenderObject child)
    {
        ...
        if (
            __child.ensureSemantics
            && !(__child.geometry!.visible || (__child.geometry!.cacheExtent > 0.0))
        )
        {
            return null;
        }
        ...
        if (
            (__child.constraints.overlap == 0L)
            || !double.IsFinite(__child.constraints.viewportMainAxisExtent)
        )
        {
            return viewportClip;
        }
        ...
        double startOfOverlap =
            __child.constraints.viewportMainAxisExtent - __child.constraints.remainingPaintExtent;
    ...
    {
        var (mainAxisPositionLocal, crossAxisPositionLocal) = axis switch
        {
                ...
                hitTest: (result) =>
                {
                    return child.hitTest(
                        ...
                        mainAxisPosition: computeChildMainAxisPosition(
                            child,
                            mainAxisPositionLocal
                        ),
                        crossAxisPosition: crossAxisPositionLocal
                    );
                }
            );
            if (isHit)
    ...
    )
    {
        ...
        ) switch
        {
            AxisDirection.up => new Offset(
                0.0,
                size.height - layoutOffset - child.geometry!.paintExtent
            ),
            AxisDirection.left => new Offset(
                size.width - layoutOffset - child.geometry!.paintExtent,
                0.0
            ),
    ...
    public abstract void updateOutOfBandData(
        GrowthDirection growthDirection,
        SliverGeometry childLayoutGeometry
    );
    public abstract void updateChildLayoutOffset(
        RenderSliver child,
        double layoutOffset,
        GrowthDirection growthDirection
    );
    ...
    public abstract double computeChildMainAxisPosition(
        RenderSliver child,
    ...
    )
    {
        ...
        Rect? newRect = RenderViewportBase<ParentDataClass>.showInViewport(
    ...
    public static Rect? showInViewport(
        RenderObject? descendant = null,
        Rect? rect = null,
        RenderViewportBase<ParentDataClass> viewport = default!,
        ViewportOffset offset = default!,
        Duration duration = default,
        Curve curve = default!
    )
        ...
        else
        {
            ...
            else
            {
                ...
                var childPreviousSiblingParentData = (
                    (ParentDataClass?)(object?)childParentData.previousSibling!.parentData!
                )!;
                var childNextSiblingParentData = (
                    (ParentDataClass?)(object?)childParentData.nextSibling!.parentData!
                )!;
...
public class RenderViewport : RenderViewportBase<SliverPhysicalContainerParentData>
{
    ...
    public static SemanticsTag excludeFromScrolling = new SemanticsTag(
    ...
    public override void performLayout()
    {
        ...
        var (mainAxisExtent, crossAxisExtent) = axis switch
        {
        ...
        do
        {
            correction = _attemptLayout(
                mainAxisExtent,
                crossAxisExtent,
                offset.pixels + centerOffsetAdjustmentLocal
            );
            if (correction != 0.0)
            {
    ...
    internal virtual double _attemptLayout(
        double mainAxisExtent,
        double crossAxisExtent,
    ...
    )
    {
        ...
        return layoutChildSequence(
            ...
            overlap: (leadingNegativeChild is null) ? Math.Min(0.0, -centerOffset) : 0.0,
    ...
    public override void updateOutOfBandData(
        GrowthDirection growthDirection,
        SliverGeometry childLayoutGeometry
    )
    {
    ...
    public override void updateChildLayoutOffset(
        RenderSliver child,
        double layoutOffset,
        GrowthDirection growthDirection
    )
    ...
    public override double computeChildMainAxisPosition(
        RenderSliver child,
        double parentMainAxisPosition
    )
    {
        ...
        {
            AxisDirection.down => parentMainAxisPosition - paintOffsetLocal.dy,
            AxisDirection.right => parentMainAxisPosition - paintOffsetLocal.dx,
            AxisDirection.up => child.geometry!.paintExtent
...
public class RenderShrinkWrappingViewport : RenderViewportBase<SliverLogicalContainerParentData>
{
    ...
    public override void performLayout()
    {
        ...
        size = axis switch
        {
            Axis.vertical => constraintsLocal.constrainDimensions(crossAxisExtent, effectiveExtent),
            Axis.horizontal => constraintsLocal.constrainDimensions(
                effectiveExtent,
                crossAxisExtent
            ),
            _ => throw new InvalidOperationException("Non-exhaustive Dart switch value."),
    ...
    public override double scrollOffsetOf(RenderSliver child, double scrollOffsetWithinChild)
    {
        DartRuntimePrimitives.Assert(() => Equals(child.parent, this));
        DartRuntimePrimitives.Assert(() =>
            Equals(child.constraints.growthDirection, GrowthDirection.forward)
        );
        var scrollOffsetToChild = 0.0;
    ...
    )
    {
        ...
        {
            AxisDirection.down => parentMainAxisPosition - layoutOffsetLocal,
