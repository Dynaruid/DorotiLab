...
public class KeyboardInsertedContent
{
    ...
    public static KeyboardInsertedContent CreateFromJson(DartMap<string, object?> metadata)
    {
        ...
        __instance.data =
            (metadata.GetValueOrDefault("data") is not null)
                ? new Uint8List(
                    new List<long>(
                        DartRuntimePrimitives.ConvertEnumerable<long>(
                            ((IEnumerable<object>?)metadata.GetValueOrDefault("data"))!
                        )
                    )
                )
                : null;
    ...
    public override string ToString() =>
        $"{objectRuntimeTypeFunctions.objectRuntimeType(this, "KeyboardInsertedContent")}({mimeType}, {uri}, {data})";
    ...
    public override bool Equals(object? other)
    {
        ...
        return (__other is KeyboardInsertedContent)
            && (__other.mimeType == mimeType)
            && (__other.uri == uri)
            && Equals(__other.data, data);
    }
