...
{
    public static async ValueTask<UrlLaunchResult> launchUrl(
        string url,
    ...
    )
    {
        ...
        {
            return new(
                UrlLaunchStatus.invalidUrl,
                "An absolute HTTP or HTTPS URL without credentials is required."
            );
        }
        ...
        if (view is null)
        {
            return new(UrlLaunchStatus.unsupported, "URL launch requires an attached view.");
        }
        ...
        {
            return new(
                UrlLaunchStatus.unsupported,
                "This host does not support opening external URLs."
            );
        }
        try
        {
            return await host.LaunchUrlAsync(uri.AbsoluteUri, cancellationToken);
        }
