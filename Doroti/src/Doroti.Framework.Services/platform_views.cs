...
public abstract class AndroidViewController : PlatformViewController
{
    ...
    {
        if (!Equals(_state, _AndroidViewState.created))
        {
            return Future.value();
        }
    ...
    public override async Future dispose()
    {
        ...
        _service.RemoveFocus(viewId);
        if (Equals(state, _AndroidViewState.creating) || Equals(state, _AndroidViewState.created))
        {
...
internal class _TextureAndroidViewControllerInternals : _AndroidViewControllerInternals
{
    ...
    public virtual async Future setOffset(Offset offset, long viewId, _AndroidViewState viewState)
    {
        ...
        if (!Equals(viewState, _AndroidViewState.created))
        {
            return;
        }
        _offset = offset;
