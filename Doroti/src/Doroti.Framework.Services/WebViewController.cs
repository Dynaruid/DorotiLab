...
public sealed class WebViewController : IAsyncDisposable
{
    ...
    public WebViewController(DorotiView owner, WebViewOptions? options = null)
    {
        ...
        if (!support.Supported || !support.WebViewCommands)
        {
            throw new WebViewException(
                ...
                support.Reason ?? "This backend has no WebView controller adapter."
            );
        }

        _web =
            host as IWebViewHostCapability
            ?? throw new WebViewException(
                WebViewError.Unsupported,
                "Host has no WebView command adapter."
            );
        _client = new PlatformViewClient(
            host,
    ...
    {
        if (_closed != 0 || !Ready.IsCompletedSuccessfully || value.Handle != Ready.Result)
        {
            return;
        }
    ...
    )
    {
        ...
        if (!Ready.IsCompletedSuccessfully)
        {
            throw new WebViewException(
                ...
                "Await Ready before issuing commands."
            );
        }
    ...
    public Task<WebViewResult> LoadAppContentAsync(
        string resourceKey,
        CancellationToken cancellationToken = default
    )
    {
        var paths = _resources
            .Where(pair => pair.Value.ResourceKey == resourceKey)
        ...
        if (paths.Length != 1)
        {
            throw new WebViewException(
                ...
                "App content requires one registered URL path for this resource key."
            );
        }

        return ExecuteAsync(
            new(WebViewOperation.Navigate, "doroti-app://content" + paths[0]),
            cancellationToken
        );
    }
    ...
    public void AttachWidget()
    {
        ...
        if (Interlocked.CompareExchange(ref _attached, 1, 0) != 0)
        {
            throw new InvalidOperationException(
                "A WebViewController can be attached to one widget at a time."
            );
        }
    }
