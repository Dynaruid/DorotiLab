...
internal sealed partial class BrowserPlatformViewHost : IPlatformViewDispatcher, IDisposable
{
    ...
    private readonly SynchronizationContext _context =
        ...
        ?? throw new InvalidOperationException(
            "Browser platform views require the JS owner context."
        );
    ...
    private Dictionary<int, CachedRaster> _rasters = [],
        _nextRasters = [];
    ...
    {
        if (!Hosts.TryGetValue(hostId, out var host) || host._closed)
        {
            return;
        }
        ...
        var handle = new PlatformViewHandle(
            ulong.Parse(identity.GetProperty("owner").GetString()!),
            long.Parse(identity.GetProperty("id").GetString()!),
            long.Parse(identity.GetProperty("generation").GetString()!)
        );
        if (!host._instances.TryGetValue(handle, out var instance))
        {
            return;
        }

        if (e.TryGetProperty("focused", out _))
        {
            instance.Focused(handle);
            return;
        }
        instance.Notify(
            new(
                handle,
                e.GetProperty("NavigationId").GetInt64(),
                e.GetProperty("DocumentGeneration").GetInt64(),
                (WebViewEventKind)e.GetProperty("Kind").GetInt32(),
                Text("Url"),
                Text("Error"),
                Text("MessageName"),
                Text("MessageJson"),
        ...
        string? Text(string key) =>
            e.TryGetProperty(key, out var value) && value.ValueKind == JsonValueKind.String
                ? value.GetString()
                : null;
    }
    ...
    private static object Identity(PlatformViewHandle h) =>
        new
        {
            owner = h.OwnerViewId.ToString(),
            id = h.InstanceId.ToString(),
            generation = h.InstanceGeneration.ToString(),
        };
    ...
    private async Task<string> Request(object payload)
    {
        ...
        try
        {
            json = await RequestAsync(_hostId, JsonSerializer.Serialize(payload));
        }
        catch (JSException transportError)
        {
            throw new WebViewException(
                WebViewError.ProcessFailed,
                "DOM control transport failed: " + transportError.Message
            );
        }
    ...
    private async ValueTask Cleanup(object payload)
    {
        ...
        { /* Main owner already reclaimed its DOM. */
        }
    }
    ...
    private sealed class Factory(BrowserPlatformViewHost? host) : IPlatformViewFactory
    {
        ...
        public PlatformViewSupport QuerySupport(PlatformViewRequest request) =>
            new(
                ...
                NativeBackdropBlur: host?._effects.LiveSourceSampling == true,
                Capabilities: new(
                    PlatformViewRepresentation.Dom,
                    PlatformViewTransport.CpuUpload,
        ...
        )
        {
            ...
            var owner =
                host ?? Hosts.Values.Single(h => h.Coordinator.OwnerViewId == handle.OwnerViewId);
    ...
    ) : IPlatformViewInstance, IPlatformWebViewInstance
    {
        ...
        public async Task<WebViewResult> ExecuteAsync(
        ...
        )
        {
            ...
            if (
                ...
                || Encoding.UTF8.GetByteCount(command.Text ?? "") > 2 * 1024 * 1024
            )
            {
                throw new WebViewException(
                    WebViewError.InvalidRequest,
                    "Invalid operation or command larger than 2 MiB."
                );
            }
            ...
            {
                _requests--;
                throw new WebViewException(WebViewError.Busy, "WebView has 32 pending requests.");
            }
            ...
            try
            {
                ...
                return (
                    JsonSerializer.Deserialize<WebViewResult>(result)
                    ?? throw new InvalidDataException("Missing WebView result.")
                ) with
                {
                    RequestId = requestId,
                };
            ...
            catch (TimeoutException)
            {
                throw new WebViewException(
                    ...
                    "WebView command exceeded 30 seconds."
                );
            }
        ...
        public ValueTask ApplyAsync(PlatformViewPlacement placement) =>
            throw new NotSupportedException(
                "Browser attachment is committed by the owning scene frame, not an independent placement call."
            );

        public ValueTask DetachAsync() =>
            host.Cleanup(new { action = "hide", identity = Identity(handle) });

        public ValueTask DisableInputAsync() =>
            host.Cleanup(new { action = "disable", identity = Identity(handle) });

        public async ValueTask SetFocusAsync(bool focused) =>
            await host.Request(
                new
                {
                    action = "focus",
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            try
            {
                await host.Cleanup(new { action = "remove", identity = Identity(handle) });
            }
            finally
            {
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("An unacknowledged DOM frame is still live.");
        }
        ...
        try
        {
                ...
                switch (part)
                {
                    ...
                    case PlatformRasterSegment raster:
                    {
                        ...
                        for (var index = 0; index < slices.Count; index++)
                        {
                            ...
                            if (bytes > RasterByteBudget)
                            {
                                throw new NotSupportedException(
                                    "Browser composition exceeds the 64 MiB raster frame budget."
                                );
                            }
                            ...
                            fixed (byte* destination = pixels)
                            {
                                ...
                                {
                                    throw new InvalidOperationException(
                                        "Browser raster RGBA conversion failed."
                                    );
                                }
                            }
                    ...
                    case PlatformNativeSegment native:
                        ...
                        views.Add(
                            new
                            {
                                identity = Identity(p.Handle),
                                bounds = Bounds(Map(p.Bounds, p.Transform)),
                                clip = p.Clip is { } c ? Bounds(c) : null,
                                visible = p.Visible,
                                order = p.PaintOrder * RasterSlots,
                            }
                        );
                        break;
                    case PlatformShieldSegment shield:
                        ...
                        shields.Add(
                            new
                            {
                                id = s.PaintOrder.ToString(),
                                bounds = Bounds(Map(s.Bounds, s.Transform)),
                                clip = s.Clip is { } sc ? Bounds(sc) : null,
                                order = s.PaintOrder * RasterSlots,
                                debug = s.Debug,
                            }
                        );
                        break;
                    case PlatformBackdropSegment effect:
                        ...
                        {
                            throw new NotSupportedException(
                                "CSS backdrop requires isotropic blur."
                            );
                        }
                        ...
                        effects.Add(
                            new
                            {
                                id = effect.PaintOrder.ToString(),
                                bounds = Bounds(effect.Bounds),
                                order = effect.PaintOrder * RasterSlots,
                                strength = effect.SigmaX / 16,
                                tint = "#00000000",
                                saturation = effect.Style?.Saturation ?? 1,
                            }
                        );
                        break;
                }
            ...
            StageFrame(
                JsonSerializer.Serialize(
                    ...
                    {
                        version = 2,
                        owner = token.OwnerViewId.ToString(),
                        epoch = token.ViewEpoch,
                        surfaceGeneration = token.SurfaceGeneration,
                        frame = token.FrameNumber,
                        views,
                        shields,
                        effects,
                    }
    ...
    )
    {
        ...
        foreach (var raster in plan.Parts.OfType<PlatformRasterSegment>())
        {
            var slices =
                raster.PaintOrder == 0
                    ? Whole(raster)
                    : SkiaPlatformRasterContent.Split(raster.Commands, width, height);
        ...
        if (
            result.Values.SelectMany(s => s).Sum(s => (long)s.Bounds.Width * s.Bounds.Height * 4)
            > RasterByteBudget
        )
        {
            foreach (var raster in plan.Parts.OfType<PlatformRasterSegment>())
            {
                result[raster.PaintOrder] = Whole(raster);
            }
        }
    ...
    private static object Bounds(Rect r) =>
        new
        {
            left = r.left,
            top = r.top,
    ...
    private static Rect Map(Rect rect, PlatformViewTransform transform)
    {
        ...
        {
            throw new NotSupportedException(
                "Browser platform views require axis-aligned placements."
            );
        }

        var a = transform.Map(rect.topLeft);
        var b = transform.Map(rect.bottomRight);
        return new(a.dx, a.dy, b.dx, b.dy);
    }
    ...
    internal void Complete(bool accepted = false)
    {
        ...
        {
            _composed = _nextComposed;
            _rasters = _nextRasters;
        }
        _nextRasters = [];
        _pending?.Dispose();
        _pending = null;
    ...
    public void Dispose()
    {
        ...
        Complete();
        _rasters.Clear();
        Hosts.Remove(_hostId);
        Coordinator.Dispose();
        lock (Retirements)
        {
            Retirements.Add(Coordinator.DisposalCompletion);
        }
    }
    ...
    private static partial void StageRaster(
        int order,
        double left,
        double top,
        double width,
