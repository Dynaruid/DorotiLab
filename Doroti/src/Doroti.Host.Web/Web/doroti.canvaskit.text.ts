...
function paragraphGraphemeSnapshots(
  ...
  for (let index = 0; index < text.length; index++) {
    ...
    const strutRects = paragraph.getRectsForRange(
      start, end, canvasKit.RectHeightStyle.Strut, canvasKit.RectWidthStyle.Tight);
    if (strutRects.length !== 1)
      throw new Error(
        `Doroti CanvasKit returned ${strutRects.length} strut rectangles for grapheme [${start}, ${end}).`);
    ...
    if (![strutLeft, strutTop, strutRight, strutBottom].every(Number.isFinite) ||
      ...
      throw new Error(`Doroti CanvasKit returned invalid strut bounds for [${start}, ${end}).`);
    if (Math.abs(strutLeft - left) > 0.001 || Math.abs(strutRight - right) > 0.001)
      throw new Error(`Doroti CanvasKit strut bounds changed the tight width for [${start}, ${end}).`);
