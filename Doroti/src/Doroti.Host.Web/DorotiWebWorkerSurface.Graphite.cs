...
public static partial class DorotiWebWorkerSurface
{
    ...
    {
        if (_graphiteContext is not null)
        {
            throw new InvalidOperationException("Graphite already initialized.");
        }
        ...
        using var module =
            JSHost.DotnetInstance.GetPropertyAsJSObject("Module")
        ...
        _graphiteContext =
            SKGraphiteContext.CreateDawn(
                new SKGraphiteDawnBackendContext
                {
        ...
        _graphiteRecorder =
            _graphiteContext.CreateRecorder(
                ResourceCacheBytes,
    ...
    )
    {
        ...
        if (
            _graphiteOwner != Environment.CurrentManagedThreadId
            || _graphiteContext is null
            || _graphiteRecorder is null
        )
        {
        ...
        {
            throw new InvalidOperationException("Graphite device lost.");
        }
        ...
        {
            using var backend =
                SKGraphiteBackendTexture.CreateDawn(texture)
                ?? throw new InvalidOperationException(
                    "Graphite current texture could not be wrapped."
                );
            using var surface = SkiaGpuSurfaces.Register(
                SKSurface.Create(_graphiteRecorder, backend, SKColorType.Bgra8888)
                    ?? throw new InvalidOperationException(
                        "Graphite current texture surface failed."
                    ),
                _graphiteRecorder
            );
    ...
    public static async Task DisposeGraphite()
    {
        if (_graphiteOwner == 0)
        {
            return; // Device initialization can fail before native attachment.
        }
