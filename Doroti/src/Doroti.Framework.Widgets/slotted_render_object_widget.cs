...
public abstract class SlottedMultiChildRenderObjectWidget<SlotType, ChildType>
    : RenderObjectWidget,
        SlottedMultiChildRenderObjectWidgetMixin<SlotType, ChildType>
    where SlotType : notnull
    where ChildType : RenderObject
{
    protected SlottedMultiChildRenderObjectWidget(Key? key = null)
        : base(key: key) { }
    ...
    public override SlottedRenderObjectElement<SlotType, ChildType> createElement() =>
        new SlottedRenderObjectElement<SlotType, ChildType>(this);
}

public interface SlottedMultiChildRenderObjectWidgetMixin<SlotType, ChildType>
    where SlotType : notnull
    where ChildType : RenderObject
{
...
public interface SlottedContainerRenderObjectMixin<SlotType, ChildType>
    where SlotType : notnull
    where ChildType : RenderObject
{
...
public class SlottedRenderObjectElement<SlotType, ChildType> : RenderObjectElement
    where SlotType : notnull
    where ChildType : RenderObject
{
    internal virtual DartMap<SlotType, Element> _slotToChild { get; set; } =
        new DartMap<SlotType, Element>();
    internal virtual DartMap<Key, Element> _keyedChildren { get; set; } =
        new DartMap<Key, Element>();
    internal virtual List<SlotType>? _debugPreviousSlots { get; set; } = default;

    public SlottedRenderObjectElement(
        SlottedMultiChildRenderObjectWidgetMixin<SlotType, ChildType> widget
    )
        : base((RenderObjectWidget)widget) { }

    public override RenderObject renderObject =>
        DartRuntimePrimitives.ConvertValue<RenderObject>(
            ((SlottedContainerRenderObjectMixin<SlotType, ChildType>?)base.renderObject)!
        );
    ...
    public override void forgetChild(Element child)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
            _slotToChild.ContainsKey(
                child.slot is SlotType childSlot
                    ? childSlot
                    : throw new ArgumentException(
                        "A slotted child requires its declared slot type.",
                        nameof(child)
                    )
            )
        );
    ...
    {
        var slottedMultiChildRenderObjectWidgetMixin = (
            (SlottedMultiChildRenderObjectWidgetMixin<SlotType, ChildType>?)widget
        )!;
        DartRuntimePrimitives.Assert(
            () =>
            {
                ...
                return CollectionsLibrary.listEquals(
                    _debugPreviousSlots,
        ...
        DartRuntimePrimitives.Assert(
            () =>
                checked((long)slottedMultiChildRenderObjectWidgetMixin.slots.toSet().Count)
                == slottedMultiChildRenderObjectWidgetMixin.slots.Count(),
            () => (object?)"slots must be unique"
        );
        ...
        foreach (SlotType slotLocal in slottedMultiChildRenderObjectWidgetMixin.slots)
        {
            ...
            else
            {
                ...
                {
                    DartRuntimePrimitives.Assert(() =>
                        !Equals(oldSlotChild!.widget.key, newWidgetKey)
                    );
                    fromElement = null;
                }
            }
            ...
            if (newChild is not null)
            {
                ...
                if (newWidgetKey is not null)
                {
                    DartRuntimePrimitives.Assert(() =>
                    {
                        ...
                        {
                            (debugDuplicateKeys ??= new DartMap<Key, List<Element>>())
                                .putIfAbsent(
                                    newWidgetKey,
                                    () => new List<Element> { existingElement }
                                )
                                .Add(newChild);
                        }
                        return true;
        ...
        DartRuntimePrimitives.Assert(
            () => _keyedChildren.Values.All(_slotToChild.Values.contains),
            () =>
                (object?)
        ...
        foreach (MapEntry<Key, List<Element>> duplicateKey in debugDuplicateKeys.entries)
        {
            throw DartRuntimePrimitives.AsException(
                new FlutterError(
                    ...
                    {
                        new ErrorSummary(
                            $"Multiple widgets used the same key in {DartRuntimePrimitives.RuntimeType(widget)}."
                        ),
    ...
    public override void insertRenderObjectChild(RenderObject child, object? slot)
    {
        ...
        var __slot = slot is SlotType typedslot
            ? typedslot
            : throw new ArgumentException(
                "A slotted child requires its declared slot type.",
                nameof(slot)
            );
        ((SlottedContainerRenderObjectMixin<SlotType, ChildType>)renderObject)._setChild(
            __child,
            __slot
        );
        DartRuntimePrimitives.Assert(() =>
            EqualityComparer<ChildType>.Default.Equals(
                (
                    (SlottedContainerRenderObjectMixin<SlotType, ChildType>)renderObject
                )._slotToChild.GetValueOrDefault(__slot),
                __child
            )
        );
    }
    ...
    public override void removeRenderObjectChild(RenderObject child, object? slot)
    {
        ...
        )
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                !(
                    (SlottedContainerRenderObjectMixin<SlotType, ChildType>)renderObject
                )._slotToChild.ContainsKey(__slot)
            );
        }
    }
    ...
    {
        var __child = (ChildType)child;
        var __oldSlot = oldSlot is SlotType typedoldSlot
            ? typedoldSlot
            : throw new ArgumentException(
                "A slotted child requires its declared slot type.",
                nameof(oldSlot)
            );
        var __newSlot = newSlot is SlotType typednewSlot
            ? typednewSlot
            : throw new ArgumentException(
                "A slotted child requires its declared slot type.",
                nameof(newSlot)
            );
        ((SlottedContainerRenderObjectMixin<SlotType, ChildType>)renderObject)._moveChild(
            __child,
            __newSlot,
            __oldSlot
        );
