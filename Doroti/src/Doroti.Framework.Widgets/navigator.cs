...
public abstract class Route<T> : RouteBase
{
    ...
    internal virtual ValueNotifier<string?> _restorationScopeId { get; private set; } =
        new ValueNotifier<string?>(null);
    ...
    public override bool requestFocus =>
        DartRuntimePrimitives.ConvertValue<bool>(
            (_requestFocus ?? navigator?.widget.requestFocus) ?? false
        );
    public override NavigatorState? navigator => _navigator;
    internal override bool _installed => _navigator is not null;

    internal override bool _isInstalledIn(NavigatorState state) =>
        DartRuntimePrimitives.ConvertValue<bool>(Equals(_navigator, state));

    public override RouteSettings settings => _settings;
    internal override bool _isPageBased => settings is Page<object?>;
    public override ValueListenable<string?> restorationScopeId =>
        DartRuntimePrimitives.ConvertValue<ValueListenable<string?>>(_restorationScopeId);
    ...
    public override bool isCurrent
    {
        get
        {
            ...
            _RouteEntry__navigator? currentRouteEntry = _navigator!._lastRouteEntryWhereOrNull(
    ...
    public override bool isFirst
    {
        get
        {
            ...
            _RouteEntry__navigator? currentRouteEntry = _navigator!._firstRouteEntryWhereOrNull(
                _RouteEntry__navigator.isPresentPredicate
            );
            if (currentRouteEntry is null)
            {
    ...
    public override bool isActive
    {
        ...
        {
            return _navigator
                    ?._firstRouteEntryWhereOrNull(_RouteEntry__navigator.isRoutePredicate(this))
                    ?.isPresent
                ?? false;
        }
    }
        ...
        if (result is not null && result is not T)
        {
            throw DartRuntimePrimitives.AsException(
                new FlutterError(
                    ...
                    {
                        new ErrorSummary(
                            $"A request was made to pop a route with a result of type {DartRuntimePrimitives.RuntimeType(result)}, but the route expected a value of type {typeof(T)}."
                        ),
                        ...
                        new DiagnosticsProperty<object>("The route was", this),
                        new DiagnosticsProperty<object?>("The provided result was", result),
                    }
                )
            );
        }
...
public class RouteSettings
{
    ...
    public override string ToString() =>
        $"{objectRuntimeTypeFunctions.objectRuntimeType(this, "RouteSettings")}({((name is null) ? "none" : $"\"{name}\"")}, {arguments})";
}
...
public abstract class Page<T> : RouteSettings
{
    ...
    protected Page(
        ...
        bool canPop = true,
        Action<bool, T?> onPopInvoked = default!
    )
        : base(name: name, arguments: arguments)
    {
        Action<bool, T?> __onPopInvoked =
    ...
    public override string ToString() =>
        $"{objectRuntimeTypeFunctions.objectRuntimeType(this, "Page")}(\"{name}\", {key}, {arguments})";
}
...
public class HeroControllerScope : InheritedWidget
{
    ...
    public HeroControllerScope(
        Key? key = null,
        HeroController controller = default!,
...
public abstract class TransitionDelegate<T>
{
    ...
    internal virtual IEnumerable<RouteTransitionRecord> _transition(
        List<RouteTransitionRecord> newPageRouteHistory,
        DartMap<RouteTransitionRecord?, RouteTransitionRecord> locationToExitingPageRoute,
        DartMap<RouteTransitionRecord?, List<RouteTransitionRecord>> pageRouteToPagelessRoutes
    )
    {
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            HashSet<RouteTransitionRecord> exitingPageRoutes =
            ...
            foreach (var exitingPageRoute in exitingPageRoutes)
            {
                ...
                {
                    foreach (
                        RouteTransitionRecord pagelessRoute in pageRouteToPagelessRoutes.GetValueOrDefault(
                            exitingPageRoute
                        )!
                    )
                    {
            ...
            foreach (
                _RouteEntry__navigator routeEntry in resultsToVerify.cast<_RouteEntry__navigator>()
            )
            {
                DartRuntimePrimitives.Assert(() =>
                    !routeEntry.isWaitingForEnteringDecision
                    && !routeEntry.isWaitingForExitingDecision
                );
                if (
                    (indexOfNextRouteInNewHistory >= checked(newPageRouteHistory.Count))
                    || (!Equals(routeEntry, newPageRouteHistory[(int)indexOfNextRouteInNewHistory]))
                )
                {
            ...
            DartRuntimePrimitives.Assert(
                () =>
                    (indexOfNextRouteInNewHistory == checked(newPageRouteHistory.Count))
                    && !Enumerable.Any(exitingPageRoutes),
                () =>
                    (object?)$"The merged result from the {GetType()}.resolve does not include all "
...
public class DefaultTransitionDelegate<T> : TransitionDelegate<T>
{
    ...
    )
    {
        ...
        {
            RouteTransitionRecord? exitingPageRoute = locationToExitingPageRoute.GetValueOrDefault(
                DartRuntimePrimitives.RequireReference(location)
            );
            ...
            if (exitingPageRoute.isWaitingForExitingDecision)
            {
                ...
                if (hasPagelessRoute)
                {
                    List<RouteTransitionRecord> pagelessRoutes = pageRouteToPagelessRoutes
                        .GetValueOrDefault(exitingPageRoute)!
                        .ToList();
                    foreach (var pagelessRoute in pagelessRoutes)
                    {
                        if (pagelessRoute.isWaitingForExitingDecision)
                        {
                            if (
                                ...
                                && Equals(pagelessRoute, pagelessRoutes.Last())
                            )
                            {
                            ...
                            else
                            {
                                pagelessRoute.markForComplete(
                                    pagelessRoute.route.currentResultObject
                                );
                            }
                        }
                    }
...
public static partial class NavigatorLibrary
{
    public static TraversalEdgeBehavior kDefaultRouteTraversalEdgeBehavior =
        TraversalEdgeBehavior.parentScope;
}

public static partial class NavigatorLibrary
{
    public static TraversalEdgeBehavior kDefaultRouteDirectionalTraversalEdgeBehavior =
        TraversalEdgeBehavior.stop;
}

public class Navigator : StatefulWidget
{
    internal static RouteBase _requireRoute(object? route) =>
        route as RouteBase
        ?? throw new ArgumentException(
            "Navigator route callbacks must return a Route<T> instance.",
            nameof(route)
        );
    ...
    public virtual TraversalEdgeBehavior routeTraversalEdgeBehavior { get; private set; } =
        default!;
    public virtual TraversalEdgeBehavior routeDirectionalTraversalEdgeBehavior
    {
        get;
        private set;
    } = default!;
    public const string defaultRouteName = "/";
    public virtual Func<NavigatorState, string, List<dynamic>> onGenerateInitialRoutes
    {
        get;
        private set;
    } = default!;
    ...
    public Navigator(
        Key? key = null,
        List<Page<object?>> pages = default!,
        Func<dynamic, object?, bool>? onPopPage = null,
        string? initialRoute = null,
        Func<NavigatorState, string, List<dynamic>> onGenerateInitialRoutes = default!,
        Func<RouteSettings, dynamic?>? onGenerateRoute = null,
        Func<RouteSettings, dynamic?>? onUnknownRoute = null,
        TransitionDelegate<object> transitionDelegate = default!,
        bool reportsRouteUpdateToEngine = false,
        Clip clipBehavior = Clip.hardEdge,
        List<NavigatorObserver> observers = default!,
        bool requestFocus = true,
        string? restorationScopeId = null,
        TraversalEdgeBehavior? routeTraversalEdgeBehavior = null,
        TraversalEdgeBehavior? routeDirectionalTraversalEdgeBehavior = null,
        Action<Page<object?>>? onDidRemovePage = null
    )
        : base(key: key)
    {
        List<Page<object?>> __pages = pages ?? _defaultPages;
        Func<NavigatorState, string, List<dynamic>> __onGenerateInitialRoutes =
            onGenerateInitialRoutes ?? defaultGenerateInitialRoutes;
        TransitionDelegate<object> __transitionDelegate =
            transitionDelegate ?? new DefaultTransitionDelegate<object>();
        List<NavigatorObserver> __observers = observers ?? new List<NavigatorObserver>();
        TraversalEdgeBehavior __routeTraversalEdgeBehavior =
            routeTraversalEdgeBehavior ?? NavigatorLibrary.kDefaultRouteTraversalEdgeBehavior;
        TraversalEdgeBehavior __routeDirectionalTraversalEdgeBehavior =
            routeDirectionalTraversalEdgeBehavior
            ?? NavigatorLibrary.kDefaultRouteDirectionalTraversalEdgeBehavior;
        this.pages = __pages;
    ...
    public static Future<T?> pushNamed<T>(
    ...
    public static string restorablePush<T>(
        BuildContext context,
        Func<BuildContext, object?, Route<T>> routeBuilder,
        object? arguments = null
    )
    {
    ...
    public static string restorablePushAndRemoveUntil<T>(
        BuildContext context,
        Func<BuildContext, object?, Route<T>> newRouteBuilder,
        Func<dynamic, bool> predicate,
        object? arguments = null
    )
    {
    ...
    public static string restorableReplace<T>(
        BuildContext context,
        dynamic oldRoute,
        Func<BuildContext, object?, Route<T>> newRouteBuilder,
        object? arguments = null
    )
    {
        return of(context)
            .restorableReplace(
                oldRoute: _requireRoute((object?)oldRoute),
    ...
    )
    {
        of(context)
            .replaceRouteBelow(
                anchorRoute: _requireRoute((object?)anchorRoute),
                newRoute: newRoute
            );
        return;
    }

    public static string restorableReplaceRouteBelow<T>(
        BuildContext context,
        dynamic anchorRoute,
        Func<BuildContext, object?, Route<T>> newRouteBuilder,
        object? arguments = null
    ...
    public static void popUntilWithResult<T>(
        BuildContext context,
        Func<dynamic, bool> predicate,
        T? result
    )
    ...
    public static void removeRouteBelow<T>(
        BuildContext context,
        Route<T> anchorRoute,
        T? result = default
    )
    ...
    )
    {
        ...
        if (initialRouteName.startsWith("/") && (initialRouteName.Length > 1L))
        {
            ...
            if (initialRouteName.Length != 0)
            {
                ...
                foreach (var part in routeParts)
                {
                    ...
                    result.Add(
                        navigator._routeNamed<object>(routeName, arguments: null, allowNull: true)
                    );
                }
            }
        ...
        {
            result.Add(
                navigator._routeNamed<object>(defaultRouteName, arguments: null)
                    ?? throw new InvalidOperationException(
                        "The navigator must provide the default route."
                    )
            );
        }
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new NavigatorState());
}
...
public class _RouteEntry__navigator : RouteTransitionRecord
{
    ...
    public override RouteBase route
    {
        get => __field_route;
    }
    ...
    public virtual WeakReference<object> lastAnnouncedPoppedNextRoute { get; set; } =
        new WeakReference<object>(notAnnounced);
    ...
    internal _RouteEntry__navigator(
        RouteBase route,
        _RouteLifecycle__navigator initialState,
        bool pageBased,
        _RestorationInformation__navigator? restorationInformation = null
    )
    {
        __field_route = route;
        ...
        System.Diagnostics.Debug.Assert(
            Equals(initialState, _RouteLifecycle__navigator.staging)
                || Equals(initialState, _RouteLifecycle__navigator.add)
                || Equals(initialState, _RouteLifecycle__navigator.push)
                || Equals(initialState, _RouteLifecycle__navigator.pushReplace)
                || Equals(initialState, _RouteLifecycle__navigator.replace)
        );
    }
    ...
    public virtual void handleAdd(NavigatorState navigator, RouteBase? previousPresent)
    {
        ...
        navigator._observedRouteAdditions.Enqueue(
            new _NavigatorPushObservation__navigator(route, previousPresent)
        );
    }

    public virtual void handlePush(
        NavigatorState navigator,
        bool isNewFirst,
        RouteBase? previous,
        RouteBase? previousPresent
    )
    {
        DartRuntimePrimitives.Assert(() =>
            Equals(currentState, _RouteLifecycle__navigator.push)
            || Equals(currentState, _RouteLifecycle__navigator.pushReplace)
            || Equals(currentState, _RouteLifecycle__navigator.replace)
        );
        DartRuntimePrimitives.Assert(() => navigator._debugLocked);
        DartRuntimePrimitives.Assert(
            () => !route._installed,
            () =>
                (object?)"The pushed route has already been used. When pushing a route, a new "
        ...
        if (
            Equals(previousState, _RouteLifecycle__navigator.replace)
            || Equals(previousState, _RouteLifecycle__navigator.pushReplace)
        )
        {
            navigator._observedRouteAdditions.Enqueue(
                new _NavigatorReplaceObservation__navigator(route, previousPresent)
            );
        ...
        {
            DartRuntimePrimitives.Assert(() =>
                Equals(previousState, _RouteLifecycle__navigator.push)
            );
            navigator._observedRouteAdditions.Enqueue(
                new _NavigatorPushObservation__navigator(route, previousPresent)
            );
    ...
    public virtual void handleDidPopNext(RouteBase poppedRoute)
    {
        ...
        if (lastFocusNode is not null)
        {
            DartRuntimePrimitives.Ignore(
                poppedRoute
                    .disposalCompleted.then(
                        (Func<object?, Future<object?>>)(
                            async (result) =>
                            {
                                switch (PlatformLibrary.defaultTargetPlatform)
                                {
                                    case TargetPlatform.android:
                                    {
                                        ...
                                        await new Future(
                                            NavigatorLibrary._kAndroidRefocusingDelayDuration
                                        );
                                        await SystemChannels.accessibility.send(
                                            new FocusSemanticEvent().toMap(nodeId: reFocusNode)
                                        );
                                        break;
                                    }
                                    ...
                                    {
                                        await SystemChannels.accessibility.send(
                                            new FocusSemanticEvent().toMap(nodeId: lastFocusNode)
                                        );
                                        break;
                                    }
                                    default:
                            ...
                            FlutterError.reportError(
                                new FlutterErrorDetails(
                                    ...
                                    context: new ErrorDescription(
                                        "while restoring focus in the navigator"
                                    )
                                )
                            );
                        }
    ...
    public virtual void handleRemoval(NavigatorState navigator, RouteBase? previousPresent)
    {
        ...
        {
            navigator._observedRouteDeletions.Enqueue(
                new _NavigatorRemoveObservation__navigator(route, previousPresent)
            );
        }
    }
    ...
    {
        if (
            FoundationRuntimePorts.EnumIndex(currentState)
            >= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.remove)
        )
        {
            return;
        }
    ...
    public virtual void finalize()
    {
        DartRuntimePrimitives.Assert(() =>
            FoundationRuntimePorts.EnumIndex(currentState)
            < FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.dispose)
        );
        currentState = _RouteLifecycle__navigator.dispose;
    }
    ...
    public virtual void forcedDispose()
    {
        DartRuntimePrimitives.Assert(() =>
            FoundationRuntimePorts.EnumIndex(currentState)
            < FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.disposed)
        );
        DartRuntimePrimitives.Assert(() =>
            Foundation.DebugLibrary.debugMaybeDispatchDisposed(this)
        );
    ...
    public virtual void dispose()
    {
        DartRuntimePrimitives.Assert(() =>
            FoundationRuntimePorts.EnumIndex(currentState)
            < FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.disposing)
        );
            ...
            listener = () =>
            {
                ...
                if (mountedLocal == 0L)
                {
                    ...
                    DartAsyncRuntime.scheduleMicrotask(() =>
                    {
                        ...
                        {
                            DartRuntimePrimitives.Assert(() =>
                                !route._installed && !navigator.mounted
                            );
                            return;
                        }
                        DartRuntimePrimitives.Assert(() =>
                            Equals(currentState, _RouteLifecycle__navigator.disposing)
                        );
                        forcedDispose();
                    });
                    return;
    ...
    public virtual bool willBePresent
    {
        get
        {
            return (
                    FoundationRuntimePorts.EnumIndex(currentState)
                    <= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.idle)
                )
                && (
                    FoundationRuntimePorts.EnumIndex(currentState)
                    >= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.add)
                );
        }
    }
    ...
    public virtual bool isPresentForRestoration =>
        DartRuntimePrimitives.ConvertValue<bool>(
            FoundationRuntimePorts.EnumIndex(currentState)
                <= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.idle)
        );
    public virtual bool suitableForAnnouncement
    {
        get
        {
            return (
                    FoundationRuntimePorts.EnumIndex(currentState)
                    <= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.removing)
                )
                && (
                    FoundationRuntimePorts.EnumIndex(currentState)
                    >= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.push)
                );
        }
    }
    ...
    public virtual bool shouldAnnounceChangeToNext(object? nextRoute)
    {
        ...
        return !(
            (nextRoute is null)
            && Equals(
                DartCoreExtensions.weakTarget(lastAnnouncedPoppedNextRoute),
    ...
    public static bool suitableForTransitionAnimationPredicate(_RouteEntry__navigator entry) =>
    ...
    public override bool isWaitingForEnteringDecision =>
        DartRuntimePrimitives.ConvertValue<bool>(
            Equals(currentState, _RouteLifecycle__navigator.staging)
        );
    ...
    public override void markForPush()
    {
        DartRuntimePrimitives.Assert(
            ...
            () =>
                (object?)"This route cannot be marked for push. Either a decision has already been "
    ...
    public override void markForAdd()
    {
        DartRuntimePrimitives.Assert(
            () => isWaitingForEnteringDecision && !isWaitingForExitingDecision,
            () =>
                (object?)"This route cannot be marked for add. Either a decision has already been "
                + "made or it does not require an explicit decision on how to transition in."
        );
    ...
    public override void markForPop(dynamic? result = null)
    {
        DartRuntimePrimitives.Assert(
            ...
            () =>
                (object?)"This route cannot be marked for pop. Either a decision has already been "
    ...
    public override void markForComplete(dynamic? result = null)
    {
        DartRuntimePrimitives.Assert(
            () => !isWaitingForEnteringDecision && isWaitingForExitingDecision && isPresent,
            () =>
                (object?)"This route cannot be marked for complete. Either a decision has already "
...
{
    internal _NavigatorPushObservation__navigator(RouteBase primaryRoute, RouteBase? secondaryRoute)
...
{
    internal _NavigatorPopObservation__navigator(RouteBase primaryRoute, RouteBase? secondaryRoute)
        : base(primaryRoute, secondaryRoute) { }
...
{
    internal _NavigatorRemoveObservation__navigator(
        RouteBase primaryRoute,
        RouteBase? secondaryRoute
    )
        : base(primaryRoute, secondaryRoute) { }
...
{
    internal _NavigatorReplaceObservation__navigator(
        RouteBase primaryRoute,
        RouteBase? secondaryRoute
    )
        : base(primaryRoute, secondaryRoute) { }
...
{
    internal virtual List<_RouteEntry__navigator> _value { get; private set; } =
        new List<_RouteEntry__navigator>();
        ...
        RestorationMixin<Navigator>
{
    ...
    internal virtual HashSet<_RouteEntry__navigator> _entryWaitingForSubTreeDisposal
    {
        get;
        private set;
    } = new HashSet<_RouteEntry__navigator>();
    internal virtual _HistoryProperty__navigator _serializableHistory { get; private set; } =
        new _HistoryProperty__navigator();
    internal virtual Queue<_NavigatorObservation__navigator> _observedRouteAdditions
    {
        get;
        private set;
    } = new Queue<_NavigatorObservation__navigator>();
    internal virtual Queue<_NavigatorObservation__navigator> _observedRouteDeletions
    {
        get;
        private set;
    } = new Queue<_NavigatorObservation__navigator>();
    public virtual FocusNode focusNode { get; private set; } =
        new FocusNode(debugLabel: "Navigator");
    ...
    internal virtual RestorableNum<long> _rawNextPagelessRestorationScopeId { get; private set; } =
        new RestorableNum<long>(0L);
    ...
    public virtual ValueNotifier<bool> userGestureInProgressNotifier { get; private set; } =
    ...
    internal virtual void _handleHistoryChanged()
    {
        switch (Scheduler.SchedulerBinding.instance.schedulerPhase)
        {
            ...
            {
                new NavigationNotification(canHandlePop: _getNavigatorCanHandlePop()).dispatch(
                    context
                );
                break;
            }
            ...
            case Scheduler.SchedulerPhase.transientCallbacks:
            {
                Scheduler.SchedulerBinding.instance.addPostFrameCallback(
                    (timeStamp) =>
                    {
                        ...
                        new NavigationNotification(
                            canHandlePop: _getNavigatorCanHandlePop()
                        ).dispatch(context);
                    },
    ...
    internal virtual bool _getNavigatorCanHandlePop()
    {
        ...
        _RouteEntry__navigator? lastEntry = _lastRouteEntryWhereOrNull(
            _RouteEntry__navigator.isPresentPredicate
        );
    ...
    {
        _RouteEntry__navigator? entry = _history
            .where(_RouteEntry__navigator.isPresentPredicate)
            .LastOrDefault();
        entry?.lastFocusNode = ServicesBinding.instance.accessibilityFocus.value;
    }

    internal virtual long _nextPagelessRestorationScopeId =>
        DartRuntimePrimitives.ConvertValue<long>(_rawNextPagelessRestorationScopeId.value++);
    ...
    public virtual void restoreState(RestorationBucket? oldBucket, bool initialRestore)
    {
        ...
        {
            var entry = new _RouteEntry__navigator(
                page.createRoute(context),
                pageBased: true,
                initialState: _RouteLifecycle__navigator.add
            );
            DartRuntimePrimitives.Assert(
                () => Equals(entry.route.settings, page),
                () =>
                    (object?)"The settings getter of a page-based Route must return a Page object. "
        ...
        DartRuntimePrimitives.Assert(
            () => Enumerable.Any(_history),
            () =>
                (object?)"All routes returned by onGenerateInitialRoutes are not restorable. "
    ...
    {
        _entryWaitingForSubTreeDisposal.removeWhere(
            (entry) =>
            {
                entry.forcedDispose();
                return true;
                    ...
                    if (newHeroController.navigator is not null)
                    {
                        ...
                        ServicesBinding.instance.addPostFrameCallback(
                            (timestamp) =>
                            {
                                if (Equals(_heroControllerFromScope, newHeroController))
                                {
                                    var hasHeroControllerOwnerShip = Equals(
    ...
    internal virtual void _updateEffectiveObservers()
    {
        ...
        {
            _effectiveObservers = widget
                .observers.Concat(new List<NavigatorObserver> { _heroControllerFromScope! })
                .ToList();
        }
        else
        {
    ...
    internal virtual void _updatePages()
    {
        ...
        var pageRouteToPagelessRoutesLocal =
            new DartMap<_RouteEntry__navigator?, List<_RouteEntry__navigator>>();
        ...
        while (oldEntriesBottom <= oldEntriesTop)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                !Equals(oldEntry.currentState, _RouteLifecycle__navigator.disposed)
            );
            if (!oldEntry.pageBased)
            {
                List<_RouteEntry__navigator> pagelessRoutes = pageRouteToPagelessRoutesLocal
                    .putIfAbsent(
        ...
        while (oldEntriesBottom <= oldEntriesTop && newPagesBottom <= newPagesTop)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                !Equals(oldEntryLocal.currentState, _RouteLifecycle__navigator.disposed)
            );
            ...
            {
                pageRouteToPagelessRoutesLocal.putIfAbsent(
                    oldEntryLocal,
                    () =>
                        new List<_RouteEntry__navigator>(
                            DartRuntimePrimitives.ConvertEnumerable<_RouteEntry__navigator>(
                                unattachedPagelessRoutes
                            )
                        )
                );
        ...
        while (oldEntriesBottomToScan <= oldEntriesTop)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                !Equals(oldEntryAlternate.currentState, _RouteLifecycle__navigator.disposed)
            );
        ...
        while (newPagesBottom <= newPagesTop)
        {
            ...
            newPagesBottom += 1L;
            if (
                (nextPage.key is null)
                || !pageKeyToOldEntry.ContainsKey(nextPage.key)
                || !pageKeyToOldEntry
                    .GetValueOrDefault(DartRuntimePrimitives.RequireReference(nextPage.key))!
                    .canUpdateFrom(nextPage)
            )
            {
                var newEntry = new _RouteEntry__navigator(
                    nextPage.createRoute(context),
                    pageBased: true,
                    initialState: _RouteLifecycle__navigator.staging
                );
                needsExplicitDecision = true;
                DartRuntimePrimitives.Assert(
                    () => Equals(newEntry.route.settings, nextPage),
                    () =>
                        (object?)
        ...
        var locationToExitingPageRouteLocal =
            new DartMap<RouteTransitionRecord?, RouteTransitionRecord>();
        while (oldEntriesBottom <= oldEntriesTop)
        {
            ...
            if (!potentialEntryToRemove.pageBased)
            {
                ...
                List<_RouteEntry__navigator> pagelessRoutesLocal = pageRouteToPagelessRoutesLocal
                    .putIfAbsent(
                        previousOldPageRouteEntry,
                        () => new List<_RouteEntry__navigator>()
                    )
                ...
                if (
                    previousOldPageRouteEntry!.isWaitingForExitingDecision
            ...
            if (
                (potentialPageToRemove.key is null)
                || pageKeyToOldEntry.ContainsKey(potentialPageToRemove.key)
                || phantomEntries.Contains(potentialEntryToRemove)
            )
            {
                locationToExitingPageRouteLocal[
                    DartRuntimePrimitives.RequireReference(previousOldPageRouteEntry)
                ] = DartRuntimePrimitives.ConvertValue<RouteTransitionRecord>(
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            if (oldEntriesBottom <= oldEntriesTop)
            {
                return (newPagesBottom <= newPagesTop)
                    ...
                    && _history[oldEntriesBottom].canUpdateFrom(widget.pages[(int)newPagesBottom]);
            }
            else
            {
                return newPagesBottom > newPagesTop;
        ...
        while (oldEntriesBottom <= oldEntriesTop && newPagesBottom <= newPagesTop)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                !Equals(oldEntryNested.currentState, _RouteLifecycle__navigator.disposed)
            );
            if (!oldEntryNested.pageBased)
            {
                DartRuntimePrimitives.Assert(() => previousOldPageRouteEntry is not null);
                List<_RouteEntry__navigator> pagelessRoutesAlternate =
                    pageRouteToPagelessRoutesLocal
                        .putIfAbsent(
                            previousOldPageRouteEntry,
                            () => new List<_RouteEntry__navigator>()
                        )
        ...
        needsExplicitDecision =
            needsExplicitDecision || Enumerable.Any(locationToExitingPageRouteLocal);
        IEnumerable<_RouteEntry__navigator> results = newHistory;
        if (needsExplicitDecision)
        {
            results = widget
                .transitionDelegate._transition(
                    newPageRouteHistory: newHistory.Cast<RouteTransitionRecord>().ToList(),
                    locationToExitingPageRoute: locationToExitingPageRouteLocal,
                    pageRouteToPagelessRoutes: pageRouteToPagelessRoutesLocal.cast<
    ...
    internal virtual void _flushHistoryUpdates(bool rearrangeOverlay = true)
    {
        ...
        while (index >= 0L)
        {
            switch (entry!.currentState)
            {
                case _RouteLifecycle__navigator.add:
                {
                    ...
                    DartRuntimePrimitives.Assert(() =>
                        Equals(entry.currentState, _RouteLifecycle__navigator.adding)
                    );
                    continue;
                }
                case _RouteLifecycle__navigator.adding:
                {
                    if (canRemoveOrAdd || (next is null))
                    {
                        ...
                        DartRuntimePrimitives.Assert(() =>
                            Equals(entry.currentState, _RouteLifecycle__navigator.idle)
                        );
                        continue;
                    }
                    break;
                ...
                case _RouteLifecycle__navigator.replace:
                {
                    ...
                    DartRuntimePrimitives.Assert(() =>
                        !Equals(entry.currentState, _RouteLifecycle__navigator.push)
                    );
                    DartRuntimePrimitives.Assert(() =>
                        !Equals(entry.currentState, _RouteLifecycle__navigator.pushReplace)
                    );
                    DartRuntimePrimitives.Assert(() =>
                ...
                case _RouteLifecycle__navigator.pop:
                {
                    if (
                        !entry.handlePop(
                            ...
                            previousPresent: _getRouteBefore(
                                index,
                                _RouteEntry__navigator.willBePresentPredicate
                            )?.route
                        )
                    )
                    {
                    ...
                    _observedRouteDeletions.Enqueue(
                        new _NavigatorPopObservation__navigator(
                            entry.route,
                            _getRouteBefore(
                                index,
                                _RouteEntry__navigator.willBePresentPredicate
                    ...
                    DartRuntimePrimitives.Assert(() =>
                        Equals(entry.currentState, _RouteLifecycle__navigator.popping)
                    );
                    canRemoveOrAdd = true;
                    break;
                }
                ...
                case _RouteLifecycle__navigator.complete:
                {
                    ...
                    DartRuntimePrimitives.Assert(() =>
                        Equals(entry.currentState, _RouteLifecycle__navigator.remove)
                    );
                    continue;
                }
                case _RouteLifecycle__navigator.remove:
                {
                    ...
                    DartRuntimePrimitives.Assert(() =>
                        FoundationRuntimePorts.EnumIndex(entry.currentState)
                        >= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.removing)
                    );
                    continue;
                }
        ...
        if (widget.reportsRouteUpdateToEngine)
        {
            ...
            {
                DartRuntimePrimitives.Ignore(
                    SystemNavigator.routeInformationUpdated(uri: DartUri.parse(routeName))
                );
    ...
    internal virtual void _flushObserverNotifications()
    {
        ...
        while (Enumerable.Any(_observedRouteAdditions))
        {
            ...
            _effectiveObservers.forEach(
                (__arg0) => ((Action<NavigatorObserver>)observation.notify)(__arg0)
            );
        }
        while (Enumerable.Any(_observedRouteDeletions))
        {
            ...
            _effectiveObservers.forEach(
                (__arg0) => ((Action<NavigatorObserver>)observationLocal.notify)(__arg0)
            );
        }
    }
    ...
    internal virtual void _flushRouteAnnouncement()
    {
        ...
        while (index >= 0L)
        {
            ...
            _RouteEntry__navigator? next = _getRouteAfter(
                index + 1L,
                _RouteEntry__navigator.suitableForTransitionAnimationPredicate
            );
            ...
            _RouteEntry__navigator? previous = _getRouteBefore(
                index - 1L,
                _RouteEntry__navigator.suitableForTransitionAnimationPredicate
            );
    ...
    internal virtual _RouteEntry__navigator? _getRouteBefore(
        long index,
        Func<_RouteEntry__navigator, bool> predicate
    )
    {
                    ...
                    throw DartRuntimePrimitives.AsException(
                        new FlutterError(
                            new List<DiagnosticsNode>
                            {
                                ...
                                new DiagnosticsProperty<NavigatorState>(
                                    "The Navigator was",
                                    this,
    ...
    {
        DartRuntimePrimitives.Assert(
            () => RestorationLibrary.debugIsSerializableForRestoration(arguments),
            () => (object?)"The arguments object must be serializable via the StandardMessageCodec."
        );
        _RouteEntry__navigator entry = _RestorationInformation__navigator
            .CreateNamed(
                name: routeName,
                arguments: arguments,
                restorationScopeId: _nextPagelessRestorationScopeId
    ...
    {
        _pushEntry(
            new _RouteEntry__navigator(
                route,
                pageBased: false,
                initialState: _RouteLifecycle__navigator.push
            )
        );
        return route.popped;
        throw new InvalidOperationException("Control flow completed without returning a value.");
    ...
    )
    {
        DartRuntimePrimitives.Assert(
            ...
            () => (object?)"The provided routeBuilder must be a static function."
        );
        DartRuntimePrimitives.Assert(
            () => RestorationLibrary.debugIsSerializableForRestoration(arguments),
            () => (object?)"The arguments object must be serializable via the StandardMessageCodec."
    ...
    internal virtual void _afterNavigation(RouteBase? route)
    {
        if (!Foundation.ConstantsLibrary.kReleaseMode)
        {
            ...
            if (route is not null)
            {
                ...
                var settingsJsonable = new DartMap<string, object?>
                {
                    ["name"] = settingsLocal.name,
                };
                if (settingsLocal.arguments is not null)
                {
                    settingsJsonable["arguments"] = Dart_convertLibrary.jsonEncode(
                        settingsLocal.arguments,
            ...
            Dart_developerLibrary.postEvent(
                "Flutter.Navigation",
                new DartMap<string, object> { ["route"] = routeJsonable }
            );
        }
    ...
    public virtual Future<T?> pushReplacement<T, TO>(Route<T> newRoute, TO? result = default)
    {
        ...
        _pushReplacementEntry(
            new _RouteEntry__navigator(
                newRoute,
                pageBased: false,
                initialState: _RouteLifecycle__navigator.pushReplace
            ),
            result
        );
    ...
    internal virtual void _pushReplacementEntry<TO>(_RouteEntry__navigator entry, TO? result)
    {
        ...
        DartRuntimePrimitives.Assert(() => Enumerable.Any(_history));
        DartRuntimePrimitives.Assert(
            () => _history.any(__item => _RouteEntry__navigator.isPresentPredicate(__item)),
            () => (object?)"Navigator has no active routes to replace."
        );
        DartRuntimePrimitives.Assert(() =>
            Equals(entry.currentState, _RouteLifecycle__navigator.pushReplace)
        );
        _history
            .lastWhere(_RouteEntry__navigator.isPresentPredicate)
    ...
    public virtual Future<T?> pushAndRemoveUntil<T>(
        Route<T> newRoute,
        Func<dynamic, bool> predicate
    )
    {
    ...
    )
    {
        ...
        while ((index >= 0L) && !predicate(_history[index].route))
        {
            ...
            {
                _history[index]
                    .complete((Navigator?)null, isReplaced: false, imperativeRemoval: true);
            }
            index -= 1L;
        }
        _flushHistoryUpdates();
    ...
    public virtual void replace<T>(dynamic oldRoute, Route<T> newRoute)
    {
        ...
        _replaceEntry(
            new _RouteEntry__navigator(
                newRoute,
                pageBased: false,
                initialState: _RouteLifecycle__navigator.replace
            ),
            typedOldRoute
        );
    }

    public virtual string restorableReplace<T>(
        dynamic oldRoute,
        Func<BuildContext, object?, Route<T>> newRouteBuilder,
        object? arguments = null
    )
    ...
    internal virtual void _replaceEntry(_RouteEntry__navigator entry, RouteBase oldRoute)
    {
        ...
        DartRuntimePrimitives.Assert(
            () => index >= 0L,
            () => (object?)"This Navigator does not contain the specified oldRoute."
        );
        DartRuntimePrimitives.Assert(
            () => _history[index].isPresent,
            () => (object?)"The specified oldRoute has already been removed from the Navigator."
        );
    ...
    public virtual string restorableReplaceRouteBelow<T>(
        dynamic anchorRoute,
        Func<BuildContext, object?, Route<T>> newRouteBuilder,
        object? arguments = null
    )
    ...
    internal virtual void _replaceEntryBelow(_RouteEntry__navigator entry, RouteBase anchorRoute)
    {
        ...
        DartRuntimePrimitives.Assert(
            () => anchorIndex >= 0L,
            () => (object?)"This Navigator does not contain the specified anchorRoute."
        );
        DartRuntimePrimitives.Assert(
            () => _history[anchorIndex].isPresent,
            () => (object?)"The specified anchorRoute has already been removed from the Navigator."
        );
        ...
        DartRuntimePrimitives.Assert(
            () => index >= 0L,
            () => (object?)"There are no routes below the specified anchorRoute."
        );
        _history.insert(index + 1L, entry);
        _history[index].complete((Navigator?)null, isReplaced: true, imperativeRemoval: true);
        _flushHistoryUpdates();
    ...
    {
        IEnumerator<_RouteEntry__navigator> iterator = _history
            .where(_RouteEntry__navigator.isPresentPredicate)
            .GetEnumerator();
    ...
    public virtual async Future<bool> maybePop<T>(T? result = default)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
            lastEntry.route._debugCheckCanConsumeResult(result, methodName: "maybePop")
        );
        ...
        _RouteEntry__navigator? newLastEntry = _lastRouteEntryWhereOrNull(
            _RouteEntry__navigator.isPresentPredicate
        );
    ...
    public virtual void pop<T>(T? result = default)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            _RouteEntry__navigator? entry = _lastRouteEntryWhereOrNull(
                _RouteEntry__navigator.isPresentPredicate
            );
        ...
        if (entryLocal.pageBased && (widget.onPopPage is not null))
        {
            ...
            {
                if (
                    FoundationRuntimePorts.EnumIndex(entryLocal.currentState)
                    <= FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.idle)
                )
                {
        ...
        else
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                Equals(entryLocal.currentState, _RouteLifecycle__navigator.pop)
            );
        }
        ...
        DartRuntimePrimitives.Assert(() =>
            Equals(entryLocal.currentState, _RouteLifecycle__navigator.idle)
            || entryLocal.route.popCompleted
        );
        DartRuntimePrimitives.Assert(() =>
        {
            _debugLocked = false;
    ...
    {
        _RouteEntry__navigator? candidate = _lastRouteEntryWhereOrNull(
            _RouteEntry__navigator.isPresentPredicate
        );
    ...
    public virtual void popUntilWithResult<T>(Func<dynamic, bool> predicate, T? result)
    {
        ...
        while (candidate is not null)
        {
            ...
            _RouteEntry__navigator? next = _lastRouteEntryWhereOrNull(
                (e) => _RouteEntry__navigator.isPresentPredicate(e) && (!Equals(e, candidate))
            );
    ...
    public virtual void removeRoute<T>(Route<T> route, T? result = default)
    {
        ...
        _RouteEntry__navigator entry = _history.firstWhere(
            _RouteEntry__navigator.isRoutePredicate(route)
        );
        ...
        {
            _afterNavigation(
                _lastRouteEntryWhereOrNull(_RouteEntry__navigator.isPresentPredicate)?.route
            );
        }
    }
    ...
    public virtual void finalizeRoute(dynamic route)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
            _history
                .where(
                    (entry) =>
                        _RouteEntry__navigator.isRoutePredicate(typedRoute)(
                            DartRuntimePrimitives.ConvertValue<_RouteEntry__navigator>(entry)
                        )
                )
                .Count() == 1L
        );
        ...
        if (
            entryLocal.pageBased
            && (
                FoundationRuntimePorts.EnumIndex(entryLocal.currentState)
                < FoundationRuntimePorts.EnumIndex(_RouteLifecycle__navigator.pop)
            )
        )
        {
            _observedRouteDeletions.Enqueue(
        ...
        {
            DartRuntimePrimitives.Assert(() =>
                Equals(entryLocal.currentState, _RouteLifecycle__navigator.popping)
            );
        }
        entryLocal.finalize();
        if (!_flushingHistory)
    ...
    public virtual void didStartUserGesture()
    {
        ...
        {
            long routeIndex = _getIndexBefore(
                _history.Count() - 1L,
                _RouteEntry__navigator.willBePresentPredicate
            );
    ...
    internal virtual void _cancelActivePointers()
    {
        ...
        _activePointers
            .ToList()
            .forEach((__arg0) => ((Action<long>)WidgetsBinding.instance.cancelPointer)(__arg0));
    }

    internal virtual _RouteEntry__navigator? _firstRouteEntryWhereOrNull(
        Func<_RouteEntry__navigator, bool> test
    )
    {
    ...
    public override Widget build(BuildContext context)
    {
        ...
        return new Overlay(
            ...
            initialEntries: overlay is null
...
public abstract class _RestorationInformation__navigator
{
    ...
    internal static _RestorationInformation__navigator CreateNamed(
        string name,
        object? arguments,
        long restorationScopeId
    ) => new _NamedRestorationInformation__navigator(name, arguments, restorationScopeId);

    internal static _RestorationInformation__navigator CreateAnonymous(
        Func<BuildContext, object?, dynamic> routeBuilder,
        object? arguments,
        long restorationScopeId
    ) =>
        new _AnonymousRestorationInformation__navigator(
            routeBuilder,
            arguments,
            restorationScopeId
        );
    ...
    internal static _RestorationInformation__navigator CreateFromSerializableData(object data)
    {
        ...
        _RouteRestorationType__navigator @type = Enum.GetValues<_RouteRestorationType__navigator>()
            .ToList()[(int)(long)casted[(int)0L]!];
        switch (@type)
        {
            case _RouteRestorationType__navigator.named:
            {
                return _NamedRestorationInformation__navigator.CreateFromSerializableData(
                    casted.Skip(checked((int)1L)).ToList()
                );
            }
            case _RouteRestorationType__navigator.anonymous:
            {
                return _AnonymousRestorationInformation__navigator.CreateFromSerializableData(
                    casted.Skip(checked((int)1L)).ToList()
                );
            }
            default:
    ...
    public virtual _RouteEntry__navigator toRouteEntry(
        NavigatorState navigator,
        _RouteLifecycle__navigator initialState = _RouteLifecycle__navigator.add
    )
    {
...
internal class _NamedRestorationInformation__navigator : _RestorationInformation__navigator
{
    ...
    public override long restorationScopeId
    {
        get => __field_restorationScopeId;
    }
    public virtual string name { get; private set; } = default!;
    public virtual object? arguments { get; private set; }

    internal _NamedRestorationInformation__navigator(
        string name,
        object? arguments,
        long restorationScopeId
    )
        : base(_RouteRestorationType__navigator.named)
    {
        this.name = name;
        this.arguments = arguments;
    ...
    internal static _NamedRestorationInformation__navigator CreateFromSerializableData(
        List<object?> data
    )
    {
...
internal class _AnonymousRestorationInformation__navigator : _RestorationInformation__navigator
{
    ...
    public virtual Func<BuildContext, object?, dynamic> routeBuilder { get; private set; } =
        default!;
    public virtual object? arguments { get; private set; }

    internal _AnonymousRestorationInformation__navigator(
        Func<BuildContext, object?, dynamic> routeBuilder,
        object? arguments,
        long restorationScopeId
    )
        : base(_RouteRestorationType__navigator.anonymous)
    {
    ...
    internal static _AnonymousRestorationInformation__navigator CreateFromSerializableData(
        List<object?> data
    )
    {
        var __instance = new _AnonymousRestorationInformation__navigator(
            default!,
            default!,
            default!
        );
        ...
        return (
            (Func<List<object>>)(
                () =>
                {
                    var __cascade = base.computeSerializableData();
                    __cascade.AddRange(
                        new List<object> { restorationScopeId, handle!.toRawHandle() }
                    );
                    if (arguments is not null)
                    {
                        __cascade.Add(arguments);
...
internal class _HistoryProperty__navigator : RestorableProperty<DartMap<string?, List<object>>?>
{
    ...
    public virtual void update(_History__navigator history)
    {
        ...
        List<object> oldRoutesForCurrentPage = (
            _pageToPagelessRoutes!.GetValueOrDefault(null) ?? new List<object>()
        ).ToList();
        ...
        foreach (var entry in history)
        {
            ...
            if (entry.pageBased)
            {
                needsSerialization =
                    needsSerialization
                    || (
                        checked(newRoutesForCurrentPage.Count)
                        != checked((long)oldRoutesForCurrentPage.Count)
                    );
                ...
                if (restorationEnabledLocal)
                {
                    ...
                    oldRoutesForCurrentPage =
                        _pageToPagelessRoutes!.GetValueOrDefault(
                            DartRuntimePrimitives.RequireReference(entry.restorationId)
                        ) ?? new List<object>();
                }
                else
                {
            ...
            restorationEnabledLocal =
                restorationEnabledLocal && (entry.restorationInformation?.isRestorable ?? false);
            entry.restorationEnabled = restorationEnabledLocal;
            if (restorationEnabledLocal)
            {
                DartRuntimePrimitives.Assert(() => entry.restorationId is not null);
                DartRuntimePrimitives.Assert(() =>
                    (currentPage is null) || (currentPage.restorationId is not null)
                );
                ...
                needsSerialization =
                    needsSerialization
                    || (
                        checked(oldRoutesForCurrentPage.Count)
                        <= checked((long)newRoutesForCurrentPage.Count)
                    )
                    || (
                        !Equals(
                            oldRoutesForCurrentPage[
                                (int)checked((long)newRoutesForCurrentPage.Count)
                            ],
                            serializedData
                        )
                    );
    ...
    internal virtual void _finalizeEntry(
        List<object> routes,
        _RouteEntry__navigator? page,
        DartMap<string?, List<object>> pageToRoutes,
    ...
    internal virtual bool _debugMapsEqual(
        DartMap<string?, List<object>> a,
        DartMap<string?, List<object>> b
    )
    {
    ...
    public virtual bool hasData =>
        DartRuntimePrimitives.ConvertValue<bool>(_pageToPagelessRoutes is not null);

    public virtual List<_RouteEntry__navigator> restoreEntriesForPage(
        _RouteEntry__navigator? page,
        NavigatorState navigator
    )
    {
        ...
        List<object>? serializedData = _pageToPagelessRoutes!
            .GetValueOrDefault(page?.restorationId)
            ?.ToList();
        ...
        {
            result.Add(
                _RestorationInformation__navigator
                    .CreateFromSerializableData(data)
    ...
    {
        var casted = DartRuntimePrimitives.ConvertMap<object, object>(
            (System.Collections.IDictionary)data!
        );
        return casted.map(
            (key, value) =>
                new MapEntry<string?, List<object>>(
                    ((string?)key)!,
                    new List<object>(
                        DartRuntimePrimitives.ConvertEnumerable<object>(((List<object>?)value)!)
                    )
                )
        );
        throw new InvalidOperationException("Control flow completed without returning a value.");
...
{
    public virtual Func<BuildContext, NavigatorState> navigatorFinder { get; private set; } =
        default!;
    ...
    public RestorableRouteFuture(
        Func<BuildContext, NavigatorState> navigatorFinder = default!,
        Func<NavigatorState, object?, string> onPresent = default!,
        Action<T>? onComplete = null
    )
    {
        Func<BuildContext, NavigatorState> __navigatorFinder =
    ...
    public override string? createDefaultValue() =>
        DartRuntimePrimitives.ConvertValue<string>(null);
    ...
    public override bool enabled =>
        DartRuntimePrimitives.ConvertValue<bool>(route?.restorationScopeId.value is not null);
    ...
    internal static NavigatorState _defaultNavigatorFinder(BuildContext context) =>
