...
{
    public PlatformEffect(PlatformEffectStyle? style = null, Widget? child = null, Key? key = null)
        : base(key: key)
    {
        Style = style ?? new();
        Child = child;
            ...
            builder: (_, constraints) =>
            {
                if (!constraints.hasBoundedWidth || !constraints.hasBoundedHeight)
                {
                    throw new InvalidOperationException(
                        "PlatformEffect requires bounded width and height."
                    );
                }
                ...
                Widget foreground = new RepaintBoundary(
                    child: new Container(color: new Color(Style.Tint), child: Child)
                );
                return new ClipRect(
                    child: (
                        Style.Match == PlatformEffectMatchPolicy.SolidTint
                        || (Style.Sigma == 0 && Style.Saturation == 1)
                    )
                        ? foreground
                        : new BackdropFilter(
                            filterConfig: new NativeEffectFilterConfig(Style),
                            child: foreground
                        )
                );
            }
    ...
    {
        public override ImageFilter resolve(
            Doroti.Framework.Rendering.ImageFilterContext context
        ) =>
            new(
                sigmaX: style.Sigma,
                sigmaY: style.Sigma,
        ...
        public override bool Equals(object? other) =>
            other is NativeEffectFilterConfig config && style == config.Style;
