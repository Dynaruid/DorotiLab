...
public class ScrollableDetails
{
    ...
    public static ScrollableDetails CreateVertical(
        bool reverse = false,
        ScrollController? controller = null,
        ScrollPhysics? physics = null,
        Clip? decorationClipBehavior = null
    ...
    {
        var __instance = new ScrollableDetails(
            default!,
            controller,
            physics,
            default!,
    ...
    public static ScrollableDetails CreateHorizontal(
        bool reverse = false,
        ScrollController? controller = null,
        ScrollPhysics? physics = null,
        Clip? decorationClipBehavior = null
    ...
    public virtual Clip? clipBehavior =>
        DartRuntimePrimitives.ConvertValue<Clip>(decorationClipBehavior);

    public virtual ScrollableDetails copyWith(
    ...
    public override bool Equals(object? other)
    {
        ...
        return (__other is ScrollableDetails)
            && Equals(__other.direction, direction)
            && Equals(__other.controller, controller)
            && Equals(__other.physics, physics)
            && Equals(__other.decorationClipBehavior, decorationClipBehavior);
    }
}
...
public class EdgeDraggingAutoScroller
{
    ...
    internal virtual Axis _scrollDirection =>
        Basic_typesLibrary.axisDirectionToAxis(_axisDirection);
    ...
    internal virtual async Future _scroll()
    {
        ...
        Rect transformedDragTarget = MatrixUtils.transformRect(
            transform,
        ...
        DartRuntimePrimitives.Assert(
            () =>
                (
                    globalRect.size.width + Foundation.ConstantsLibrary.precisionErrorTolerance
                    >= transformedDragTarget.size.width
                )
                && (
                    globalRect.size.height + Foundation.ConstantsLibrary.precisionErrorTolerance
                    >= transformedDragTarget.size.height
                ),
            () =>
                (object?)"Drag target size is larger than scrollable size, which may cause bouncing"
        );
        _scrolling = true;
        ...
        double proxyStart = _offsetExtent(
            _dragTargetRelatedToScrollOrigin.topLeft,
            _scrollDirection
        );
        double proxyEnd = _offsetExtent(
        ...
        switch (_axisDirection)
        {
                ...
                else
                {
                    ...
                    )
                    {
                        ...
                        newOffset = Math.Min(
                            ...
                            scrollable.position.pixels + overDragLocal
                        );
                    }
                }
                break;
            ...
            case AxisDirection.down:
            {
                ...
                )
                {
                    ...
                    newOffset = Math.Max(
                        scrollable.position.minScrollExtent,
                        scrollable.position.pixels - overDragAlternate
                    );
                }
                else
                {
                    ...
                    )
                    {
                        ...
                        newOffset = Math.Min(
                            scrollable.position.maxScrollExtent,
                            scrollable.position.pixels + overDragNested
                        );
                    }
                }
                break;
...
public class ScrollAction : ContextAction<ScrollIntent>
{
    ...
    )
    {
        ...
        if (state.widget.incrementCalculator is not null)
        {
            return state.widget.incrementCalculator!(
                new ScrollIncrementDetails(type: type, metrics: state.position)
            );
        }
        return type switch
        {
    ...
    public static double getDirectionalIncrement(ScrollableState state, ScrollIntent intent)
    {
        if (
            Equals(
                Basic_typesLibrary.axisDirectionToAxis(intent.direction),
                Basic_typesLibrary.axisDirectionToAxis(state.axisDirection)
            )
        )
        {
    ...
    public override object? invoke(ScrollIntent intent, BuildContext? context = null)
    {
        DartRuntimePrimitives.Assert(
            () => context is not null,
            () => (object?)"Cannot scroll without a context."
        );
        ...
        DartRuntimePrimitives.Assert(
            ...
            () =>
                (object?)"Scrollable must be laid out before it can be scrolled via a ScrollAction"
        );
        if (
            (state.resolvedPhysics is not null)
            && !state.resolvedPhysics!.shouldAcceptUserOffset(state.position)
        )
        {
            return default!;
