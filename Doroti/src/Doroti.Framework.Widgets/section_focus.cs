...
public sealed class SectionFocusCoordinator : IDisposable
{
    ...
    public int? FocusedSection =>
        _nodes.Where(pair => pair.Value.hasFocus).Select(pair => (int?)pair.Key).FirstOrDefault();

    public void CancelPendingTraversal()
    {
        _request++;
        _pending = false;
        _queuedTabs.Clear();
    ...
    public Widget Wrap(int id, Widget child)
    {
        ...
        {
            _nodes[id] = node = new FocusNode(skipTraversal: true, debugLabel: $"section-{id}");
        }
    ...
    private KeyEventResult Handle(int id, FocusNode scope, KeyEvent key)
    {
        ...
        if (
            key is not (KeyDownEvent or KeyRepeatEvent)
            || !Equals(key.logicalKey, LogicalKeyboardKey.tab)
        ...
        if (_pending)
        {
            _queuedTabs.Enqueue(keyboard.isShiftPressed);
        ...
        if (members.Length == 0 || !ReferenceEquals(primary, reverse ? members[0] : members[^1]))
        {
            return KeyEventResult.ignored;
        }

        var next = _positions[id] + (reverse ? -1 : 1);
        if ((uint)next >= _order.Length)
        {
            return KeyEventResult.ignored;
        }
    ...
    private void Request(int position, bool reverse, FocusNode primary, long request)
    {
        ...
        WidgetsBinding.instance.addPostFrameCallback(_ =>
        {
            ...
            if (!ReferenceEquals(FocusManager.instance.primaryFocus, primary))
            {
                CancelPendingTraversal();
                return;
            }
            var scope = _nodes.GetValueOrDefault(_order[position]);
            var members = scope?.context is null
                ? []
                : _policy.sortDescendants(scope.traversalDescendants, scope).ToArray();
            if (members.Length != 0)
            {
                FocusTraversalPolicy.defaultTraversalRequestFocusCallback(
                    reverse ? members[^1] : members[0],
                    alignmentPolicy: reverse
            ...
            if ((uint)next < _order.Length)
            {
                Request(next, reverse, primary, request);
            }
            else
            {
                if (reverse)
    ...
    private void DrainAfterFrame(long request)
    {
        ...
        WidgetsBinding.instance.addPostFrameCallback(_ =>
        {
            if (_disposed || request != _request || _pending || _queuedTabs.Count == 0)
            {
                return;
            }
            ...
            {
                var members = _policy
                    .sortDescendants(_nodes[section].traversalDescendants, primary)
                    .ToArray();
                var next = _positions[section] + (reverse ? -1 : 1);
                if (
                    members.Length != 0
                    && ReferenceEquals(primary, reverse ? members[0] : members[^1])
                    && (uint)next < _order.Length
                )
                {
                    Request(next, reverse, primary, request);
                    return;
