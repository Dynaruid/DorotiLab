...
public class DefaultTextStyle : InheritedTheme
{
    ...
    {
        var __instance = new DefaultTextStyle(
            key,
            default!,
            default!,
            default!,
    ...
    public override bool updateShouldNotify(InheritedWidget oldWidget)
    {
        ...
        return (!Equals(style, __oldWidget.style))
            || (!Equals(textAlign, __oldWidget.textAlign))
            || (softWrap != __oldWidget.softWrap)
            || (!Equals(overflow, __oldWidget.overflow))
...
public class DefaultTextHeightBehavior : InheritedTheme
{
    ...
    public DefaultTextHeightBehavior(
        Key? key = null,
        TextHeightBehavior textHeightBehavior = default!,
        Widget child = default!
    )
        : base(key: key, child: child)
    {
...
public class Text : StatelessWidget
{
    ...
    public override Widget build(BuildContext context)
    {
        ...
        TextSpan effectiveTextSpan =
            _OverridingTextStyleTextSpanUtils__text.applyTextSpacingOverrides(
                ...
                textSpan: new TextSpan(
                    ...
                    text: data,
                    locale: locale,
                    children: (textSpan is not null) ? new List<InlineSpan> { textSpan! } : null
                )
            );
        Painting.StrutStyle? effectiveStrutStyle = strutStyle?.merge(
            new Painting.StrutStyle(height: lineHeightScaleFactorLocal)
        );
        SelectionRegistrar? registrar = SelectionContainer.maybeOf(context);
        TextScaler textScalerLocal = (textScaler, textScaleFactor) switch
        {
        ...
        if (registrar is not null)
        {
            result = DartRuntimePrimitives.ConvertValue<Widget>(
                new MouseRegion(
                    ...
                    child: new _SelectableTextContainer__text(
                        textAlign: (textAlign ?? defaultTextStyle.textAlign) ?? TextAlign.start,
                        textDirection: textDirection,
                        locale: locale,
                        softWrap: softWrap ?? defaultTextStyle.softWrap,
                        overflow: (overflow ?? effectiveTextStyle?.overflow)
            ...
            result = DartRuntimePrimitives.ConvertValue<Widget>(
                new Semantics(
                    ...
                    child: new ExcludeSemantics(
                        excluding: semanticsLabel is not null,
                        child: result
                    )
                )
            );
    ...
    public override void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        ...
        {
            properties.add(
                ((Diagnosticable)textSpan!).toDiagnosticsNode(
                    name: "textSpan",
        ...
        properties.add(new IntProperty("maxLines", maxLines, defaultValue: null));
        properties.add(
            new EnumProperty<TextWidthBasis>("textWidthBasis", textWidthBasis, defaultValue: null)
        );
        properties.add(
            new DiagnosticsProperty<TextHeightBehavior>(
                "textHeightBehavior",
...
internal class _SelectableTextContainer__text : StatefulWidget
{
    ...
    internal _SelectableTextContainer__text(
        TextSpan text,
        TextAlign textAlign,
        TextDirection? textDirection = null,
        bool softWrap = default!,
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new _SelectableTextContainerState__text());
}

internal class _SelectableTextContainerState__text : State<_SelectableTextContainer__text>
{
    internal virtual _SelectableTextContainerDelegate__text _selectionDelegate
    {
        get;
        private set;
    } = default!;
...
internal class _RichText__text : StatelessWidget
{
    ...
    internal _RichText__text(
        GlobalKey<IState>? textKey = null,
...
internal class _SelectableTextContainerDelegate__text : StaticSelectionContainerDelegate
{
    ...
    public virtual RenderParagraph paragraph =>
        ((RenderParagraph?)_textKey.currentContext!.findRenderObject()!)!;
    ...
    internal virtual SelectionResult _handleSelectParagraph(SelectParagraphSelectionEvent @event)
    {
        ...
        {
            bool selectableIsPlaceholder = !paragraph.selectableBelongsToParagraph(
                selectables[(int)indexLocal]
            );
            if (
                selectableIsPlaceholder
                && Enumerable.Any(selectables[(int)indexLocal].boundingBoxes)
            )
            {
                foreach (Rect rect in selectables[(int)indexLocal].boundingBoxes)
                {
                    Rect globalRect = MatrixUtils.transformRect(
                        selectables[(int)indexLocal].getTransformTo(null),
                        rect
                    );
        ...
        for (
            var indexAlternate = 0L;
            indexAlternate < checked(selectables.Count);
            indexAlternate += 1L
        )
        {
            if (!paragraph.selectableBelongsToParagraph(selectables[(int)indexAlternate]))
            {
                if (foundStart)
                {
                    SelectionEvent synthesizedEvent = new SelectParagraphSelectionEvent(
                        globalPosition: @event.globalPosition,
                        absorb: true
                    );
                    SelectionResult result = dispatchSelectionEventToChild(
                        selectables[(int)indexAlternate],
                        synthesizedEvent
                    );
            ...
            )
            {
                ...
                if (
                    (!Equals(selectables[(int)indexAlternate].value, existingGeometry))
                    && !foundStart
                )
                {
                    DartRuntimePrimitives.Assert(() =>
                        Enumerable.Any(selectables[(int)indexAlternate].boundingBoxes)
                    );
                    DartRuntimePrimitives.Assert(() =>
                        Enumerable.Any(selectables[(int)indexAlternate].value.selectionRects)
                    );
                    bool selectionAtStartOfSelectable = selectables[(int)indexAlternate]
                        .boundingBoxes[(int)0L]
                        .overlaps(selectables[(int)indexAlternate].value.selectionRects[(int)0L]);
                    var startIndex = 0L;
                    ...
                    {
                        startIndex =
                            ((lastNextIndex is null) && selectionAtStartOfSelectable)
                                ? 0L
                                : indexAlternate;
                    }
                    for (var i = startIndex; i < indexAlternate; i += 1L)
                    {
                        SelectionEvent synthesizedEventLocal = new SelectParagraphSelectionEvent(
                            globalPosition: @event.globalPosition,
                            absorb: true
                        );
            ...
            if (!Equals(selectables[(int)indexAlternate].value, existingGeometry))
            {
                if (!foundStart && (lastNextIndex is null))
                {
                    ...
                    {
                        SelectionEvent synthesizedEventAlternate =
                            new SelectParagraphSelectionEvent(
                                globalPosition: @event.globalPosition,
                                absorb: true
                            );
                        dispatchSelectionEventToChild(
                            selectables[(int)iLocal],
    ...
    internal new virtual SelectionResult _adjustSelection(
    ...
    )
    {
        ...
        var isCurrentEdgeWithinViewport = isEnd
            ? (value.endSelectionPoint is not null)
            : (value.startSelectionPoint is not null);
    ...
    internal static new long _compareScreenOrder(Selectable a, Selectable b)
    {
    ...
    // Dart library-private member: distinct from the same name in the base library.
    internal new virtual SelectedContentRange? _calculateLocalRange(
    ...
    )
    {
        ...
        for (var index = 0L; index < checked(selections.Count); index++)
        {
            ...
            if (!foundStart)
            {
                bool shouldConsiderContentStart =
                    (index > 0L) && paragraph.selectableBelongsToParagraph(selectables[(int)index]);
                startOffsetLocal += (
                        ...
                        shouldConsiderContentStart
                            ? paragraph
                                .getPositionForOffset(
                                    selectables[(int)index].boundingBoxes.First().centerLeft
                                )
                                .offset
                            : 0L
                    )
