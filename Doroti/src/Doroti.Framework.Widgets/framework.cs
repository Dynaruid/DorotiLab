...
public class ObjectKey : LocalKey
{
    ...
    public override int GetHashCode() =>
        DartRuntimePrimitives.ConvertValue<int>(
            FoundationRuntimePorts.ObjectHash(GetType(), Dart_coreLibrary.identityHashCode(value))
        );
...
{
    internal virtual Element? _currentElement =>
        WidgetsBinding.instance.buildOwner!._globalKeyRegistry.GetValueOrDefault(this);
}

public class GlobalKey<T> : GlobalKeyBase
    where T : IState
{
    public static new GlobalKey<T> Create(string? debugLabel = null) =>
        new LabeledGlobalKey<T>(debugLabel);

    public GlobalKey() { }

    public virtual BuildContext? currentContext =>
        DartRuntimePrimitives.ConvertValue<BuildContext>(_currentElement);
    public virtual Widget? currentWidget => _currentElement?.widget;
    public virtual T? currentState =>
        _currentElement switch
        {
            StatefulElement { state: T stateLocal } __object7625 => stateLocal,
            _ => default,
        };

    public GlobalKey(string? debugLabel)
    {
        _ = debugLabel;
    }
}

public class LabeledGlobalKey<T> : GlobalKey<T>
    where T : IState
{
...
public class GlobalObjectKey<T> : GlobalKey<T>
    where T : IState
{
    ...
    public override bool Equals(object? other)
    {
        ...
        return (__other is GlobalObjectKey<T>)
            && DartRuntimePrimitives.Identical(__other.value, value);
    }

    public override int GetHashCode() =>
        DartRuntimePrimitives.ConvertValue<int>(Dart_coreLibrary.identityHashCode(value));
...
public abstract class Widget : DiagnosticableTree
{
    ...
    public override int GetHashCode() =>
        DartRuntimePrimitives.ConvertValue<int>(base.GetHashCode());
...
{
    protected StatelessWidget(Key? key = null)
        : base(key: key) { }
...
public abstract class State<T> : IState, Diagnosticable
    where T : StatefulWidget
{
    internal virtual T? _widget { get; set; } = default;
    internal virtual _StateLifecycle__framework _debugLifecycleState { get; set; } =
        _StateLifecycle__framework.created;
    ...
    {
        DartRuntimePrimitives.Assert(() =>
            Equals(_debugLifecycleState, _StateLifecycle__framework.created)
        );
        DartRuntimePrimitives.Assert(() =>
            Foundation.DebugLibrary.debugMaybeDispatchCreated("widgets", "State", this)
        );
    }
    ...
    {
        DartRuntimePrimitives.Assert(() =>
            Equals(_debugLifecycleState, _StateLifecycle__framework.ready)
        );
        DartRuntimePrimitives.Assert(() =>
        {
    ...
    public virtual void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        ...
        {
            properties.add(
                new EnumProperty<_StateLifecycle__framework>(
                    "lifecycle state",
                    _debugLifecycleState,
                    defaultValue: _StateLifecycle__framework.ready
                )
            );
            return true;
            throw new InvalidOperationException("Dart closure completed without a value.");
        ...
        properties.add(
            new ObjectFlagProperty<StatefulElement>("_element", _element, ifNull: "not mounted")
        );
    }

    StatefulWidget? IState._widget
    {
        get => _widget;
        set => _widget = (T?)value;
    }
    _StateLifecycle__framework IState._debugLifecycleState
    {
        get => _debugLifecycleState;
        set => _debugLifecycleState = value;
    }
    StatefulElement? IState._element
    {
        get => _element;
        set => _element = value;
    }
...
public abstract class ProxyWidget : Widget
{
    ...
    protected ProxyWidget(Key? key = null, Widget child = default!)
        : base(key: key)
    {
        this.child = child;
    }
...
{
    protected ParentDataWidget(Key? key = null, Widget child = default!)
    ...
    internal virtual IEnumerable<DiagnosticsNode> _debugDescribeIncorrectParentDataType(
...
{
    protected InheritedWidget(Key? key = null, Widget child = default!)
        : base(key: key, child: child) { }
...
{
    protected RenderObjectWidget(Key? key = null)
        : base(key: key) { }
...
{
    protected LeafRenderObjectWidget(Key? key = null)
        : base(key: key) { }
...
public abstract class SingleChildRenderObjectWidget : RenderObjectWidget
{
    ...
    protected SingleChildRenderObjectWidget(Key? key = null, Widget? child = null)
        : base(key: key)
    {
        this.child = child;
    }
...
public abstract class MultiChildRenderObjectWidget : RenderObjectWidget
{
    ...
    protected MultiChildRenderObjectWidget(Key? key = null, List<Widget> children = default!)
        : base(key: key)
    {
    ...
    protected MultiChildRenderObjectWidget(Key? key = null, IEnumerable<Widget> children = default!)
        : this(key, children.ToList()) { }
}
...
internal class _InactiveElements__framework
{
    ...
    {
        DartRuntimePrimitives.Assert(() =>
            Equals(element._lifecycleState, _ElementLifecycle__framework.inactive)
        );
        DartRuntimePrimitives.Assert(() =>
        {
        ...
        element.unmount();
        DartRuntimePrimitives.Assert(() =>
            Equals(element._lifecycleState, _ElementLifecycle__framework.defunct)
        );
    }
    ...
    {
        DartRuntimePrimitives.Assert(() =>
            Equals(element._lifecycleState, _ElementLifecycle__framework.active)
        );
        try
        {
            element.deactivate();
    ...
    public virtual void add(Element element)
    {
        ...
        switch (element._lifecycleState)
        {
            ...
            case _ElementLifecycle__framework.initial
            or _ElementLifecycle__framework.failed
            or _ElementLifecycle__framework.defunct:
            {
                DartRuntimePrimitives.Assert(
                    () => false,
                    () =>
...
public interface BuildContext
{
    ...
    public InheritedWidget dependOnInheritedElement(
...
public class BuildScope
{
    ...
    internal virtual void _scheduleBuildFor(Element element)
    {
        ...
        if (FrameworkWorkTrace.Enabled)
        {
            FrameworkWorkTrace.Record(
                FrameworkWorkTrace.Kind.Enqueue,
                element,
                this,
                element._inDirtyList ? 1 : 0
            );
        }

        if (_building)
        {
            FrameworkWorkCounters.Add(FrameworkWork.BuildDuringFlush);
        }

        if (element._inDirtyList)
        {
            FrameworkWorkCounters.Add(FrameworkWork.BuildEnqueueDuplicate);
        }

        DartRuntimePrimitives.Assert(() =>
            DartRuntimePrimitives.Identical(element.buildScope, this)
        );
        if (!element._inDirtyList)
        {
    ...
    internal virtual void _tryRebuild(Element element)
    {
        ...
        bool isTimelineTracked =
            !Foundation.ConstantsLibrary.kReleaseMode
            && FrameworkLibrary._isProfileBuildsEnabledFor(element.widget);
        if (isTimelineTracked)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
            {
                if (
                    ...
                    && DebugLibrary.debugEnhanceBuildTimelineArguments
                )
                {
                    debugTimelineArguments = ((Diagnosticable)element.widget)
                        .toDiagnosticsNode()
                        .toTimelineArguments();
                }
                return true;
        ...
        catch (Exception e)
        {
            ...
            FrameworkLibrary._reportException(
    ...
    internal virtual void _flushDirtyElements(Element debugBuildRoot)
    {
        ...
        DartRuntimePrimitives.Assert(
            () => _dirtyElementsNeedsResorting is null,
            () => (object?)"_flushDirtyElements must be non-reentrant"
        );
        ...
        try
        {
            for (
                var index = 0L;
                index < checked(_dirtyElements.Count);
            ...
            )
            {
                ...
                {
                    DartRuntimePrimitives.Assert(() =>
                        _debugAssertElementInScope(elementLocal, debugBuildRoot)
                    );
            ...
            DartRuntimePrimitives.Assert(() =>
            {
                IEnumerable<Element> missedElements = _dirtyElements.where(
                    (element) =>
...
public class BuildOwner
{
    ...
    internal virtual _InactiveElements__framework _inactiveElements { get; private set; } =
        new _InactiveElements__framework();
    ...
    internal virtual DartMap<
        Element,
        HashSet<GlobalKeyBase>
    >? _debugElementsThatWillNeedToBeRebuiltDueToGlobalKeyShenanigans { get; set; } = default;
    internal virtual DartMap<GlobalKeyBase, Element> _globalKeyRegistry { get; private set; } =
        new DartMap<GlobalKeyBase, Element>();
    internal virtual HashSet<Element>? _debugIllFatedElements { get; private set; } =
        Foundation.ConstantsLibrary.kDebugMode ? new HashSet<Element>() : null;
    internal virtual DartMap<Element, DartMap<Element, GlobalKeyBase>>? _debugGlobalKeyReservations
    {
        get;
        private set;
    } =
        Foundation.ConstantsLibrary.kDebugMode
            ? new DartMap<Element, DartMap<Element, GlobalKeyBase>>()
            : null;
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            if (DebugLibrary.debugPrintScheduleBuildForStacks)
            {
                AssertionsLibrary.debugPrintStack(
                    label: $"scheduleBuildFor() called for {element}{(element.buildScope._dirtyElements.Contains(element) ? " (ALREADY IN LIST)" : "")}"
                );
            }
            if (!element.dirty)
            {
            ...
            if (DebugLibrary.debugPrintScheduleBuildForStacks && element._inDirtyList)
            {
                AssertionsLibrary.debugPrintStack(
                    label: "BuildOwner.scheduleBuildFor() called; "
                        + $"_dirtyElementsNeedsResorting was {buildScopeLocal._dirtyElementsNeedsResorting} (now true); "
                        + $"The dirty list for the current build scope is: {buildScopeLocal._dirtyElements}"
                );
            }
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            if (DebugLibrary.debugPrintScheduleBuildForStacks)
            {
                PrintLibrary.debugPrint(
                    $"...the build scope's dirty list is now: {buildScopeLocal._dirtyElements}"
                );
            }
            return true;
            throw new InvalidOperationException("Dart closure completed without a value.");
    ...
    internal virtual bool _debugStateLocked =>
        DartRuntimePrimitives.ConvertValue<bool>(_debugStateLockLevel > 0L);
            ...
            if (DebugLibrary.debugPrintBuildScope)
            {
                PrintLibrary.debugPrint(
                    $"buildScope called with context {context}; "
                        + $"its build scope's dirty list is: {buildScopeLocal._dirtyElements}"
                );
            }
            _debugStateLockLevel += 1L;
            _debugBuilding = true;
                ...
                if (DebugLibrary.debugEnhanceBuildTimelineArguments)
                {
                    debugTimelineArguments = new DartMap<string, string>
                    {
                        ["build scope dirty count"] =
                            $"{checked((long)buildScopeLocal._dirtyElements.Count)}",
                        ["build scope dirty list"] = $"{buildScopeLocal._dirtyElements}",
        ...
        try
        {
            ...
            if (callback is not null)
            {
                ...
                finally
                {
                    ...
                    {
                        DartRuntimePrimitives.Assert(() =>
                            Equals(_debugCurrentBuildTarget, context)
                        );
    ...
    internal virtual void _debugTrackElementThatWillNeedToBeRebuiltDueToGlobalKeyShenanigans(
        Element node,
        GlobalKeyBase key
    )
    {
        DartMap<Element, HashSet<GlobalKeyBase>> map =
            _debugElementsThatWillNeedToBeRebuiltDueToGlobalKeyShenanigans ??=
                new DartMap<Element, HashSet<GlobalKeyBase>>();
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            if (_globalKeyRegistry.ContainsKey(key))
            {
                ...
                DartRuntimePrimitives.Assert(() =>
                    !Equals(
                        DartRuntimePrimitives.RuntimeType(element.widget),
                        DartRuntimePrimitives.RuntimeType(oldElement.widget)
                    )
                );
    ...
    internal virtual void _unregisterGlobalKey(GlobalKeyBase key, Element element)
    {
        DartRuntimePrimitives.Assert(() =>
        {
            if (
                ...
                && (!Equals(_globalKeyRegistry.GetValueOrDefault(key), element))
            )
            {
                Element oldElement = _globalKeyRegistry.GetValueOrDefault(key)!;
                DartRuntimePrimitives.Assert(() =>
    ...
    internal virtual void _debugReserveGlobalKeyFor(
        Element parent,
        Element child,
        GlobalKeyBase key
    )
    {
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            {
                reservations.putIfAbsent(parent, () => new DartMap<Element, GlobalKeyBase>())[
                    child
                ] = key;
            }
    ...
    internal virtual void _debugVerifyGlobalKeyReservation()
    {
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            _debugGlobalKeyReservations?.forEach(
                ...
                {
                    if (
                        Equals(parent._lifecycleState, _ElementLifecycle__framework.defunct)
                    ...
                    childToKey.forEach(
                        (child, key) =>
                        {
                            ...
                            if (
                                keyToParent.ContainsKey(key)
                                && (!Equals(keyToParent.GetValueOrDefault(key), parent))
                            )
                            {
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            foreach (Element element in _debugIllFatedElements ?? new HashSet<Element>())
            {
                if (!Equals(element._lifecycleState, _ElementLifecycle__framework.defunct))
                {
                    ...
                    HashSet<Element> elements = duplicates.putIfAbsent(
                        keyLocal,
                        () => new HashSet<Element>()
                    );
            ...
            if (duplicates is not null)
            {
                ...
                foreach (GlobalKeyBase keyAlternate in duplicates.Keys)
                {
                    ...
                    information.Add(
                        Element.describeElements(
                            $"The key {keyAlternate} was used by {checked((long)elementsLocal.Count)} widgets",
                            elementsLocal
                        )
                    );
                }
    ...
    public virtual void finalizeTree()
    {
            ...
            DartRuntimePrimitives.Assert(() =>
            {
                try
                {
                    ...
                    )
                    {
                        ...
                        foreach (
                            Element elementLocal in _debugElementsThatWillNeedToBeRebuiltDueToGlobalKeyShenanigans!.Keys
                        )
                        {
                            if (
                                !Equals(
                                    elementLocal._lifecycleState,
                                    _ElementLifecycle__framework.defunct
                                )
                            )
                            {
                                keys.UnionWith(
                                    _debugElementsThatWillNeedToBeRebuiltDueToGlobalKeyShenanigans!.GetValueOrDefault(
                                        elementLocal
                                    )!
                                );
                            }
                        ...
                        if (Enumerable.Any(keys))
                        {
                            ...
                            foreach (
                                string? keyLocal in keys.map<GlobalKeyBase, string?>(
                                    (key) => key.ToString()
                                )
                            )
                            {
                            ...
                            foreach (
                                string elementAlternate in elements.map(
                            ...
                            var respective =
                                (checked(elementLabels.Count) == 1L) ? "" : " respective";
        ...
        catch (Exception e)
        {
            var stack = new System.Diagnostics.StackTrace();
            FrameworkLibrary._reportException(
                new ErrorSummary("while finalizing the widget tree"),
                e,
                stack
            );
        }
...
internal class _NotificationNode__framework
{
    ...
    internal _NotificationNode__framework(
        _NotificationNode__framework? parent,
...
public static partial class FrameworkLibrary
{
    ...
    {
        return DebugLibrary.debugProfileBuildsEnabled
            || (
...
public abstract class Element : DiagnosticableTree, BuildContext
{
    ...
    internal virtual _ElementLifecycle__framework _lifecycleState { get; set; } =
        _ElementLifecycle__framework.initial;
    internal virtual HashSet<Element>? _debugForgottenChildrenWithGlobalKey { get; private set; } =
        Foundation.ConstantsLibrary.kDebugMode ? new HashSet<Element>() : null;
    internal virtual PersistentHashMap<Type, InheritedElement>? _inheritedElements { get; set; } =
        default;
    ...
    public virtual BuildScope buildScope =>
        DartRuntimePrimitives.ConvertValue<BuildScope>(_parentBuildScope!);
        ...
        get
        {
            ...
            while (current is not null)
            {
                ...
                else
                {
                    ...
                    {
                        RenderObjectElement current__163649__as163793 =
                            (RenderObjectElement)current;
    ...
    public virtual List<DiagnosticsNode> describeMissingAncestor(Type expectedAncestorType)
    {
        ...
        {
            information.Add(
                describeElements("The ancestors of this widget were", ancestors.Cast<Element>())
            );
        }
        else
        {
    ...
    public static DiagnosticsNode describeElements(string name, IEnumerable<Element> elements)
    {
        return new DiagnosticsBlock(
            name: name,
            children: elements
                .map<Element, DiagnosticsNode>(
                    (element) => new DiagnosticsProperty<Element>("", element)
                )
                .ToList(),
            allowTruncate: true
        );
    ...
    public virtual void debugVisitOnstageChildren(Action<Element> visitor) =>
    ...
    public virtual Element? updateChild(Element? child, Widget? newWidget, object? newSlot)
    {
        ...
        {
            FrameworkWorkTrace.Record(
                FrameworkWorkTrace.Kind.UpdateChild,
                this,
                child,
                newWidget is null ? 0 : 1
            );
        }
        ...
        if (child is not null)
        {
            ...
            else
            {
                if (hasSameSuperclass && Widget.canUpdate(child.widget, newWidget))
                {
                    ...
                    bool isTimelineTracked =
                        !Foundation.ConstantsLibrary.kReleaseMode
                        && FrameworkLibrary._isProfileBuildsEnabledFor(newWidget);
                    if (isTimelineTracked)
                    {
                        DartMap<string, string>? debugTimelineArguments = default!;
                        DartRuntimePrimitives.Assert(() =>
                        {
                            ...
                            {
                                debugTimelineArguments = ((Diagnosticable)newWidget)
                                    .toDiagnosticsNode()
                                    .toTimelineArguments();
                            }
                            return true;
    ...
    {
        DartRuntimePrimitives.Assert(() =>
            (slots is null) || (checked(newWidgets.Count) == checked((long)slots.Count))
        );
        ...
        var newChildren = new List<Element>(
            Enumerable.Repeat<Element>(
                _NullElement__framework.instance,
                checked((int)checked((long)newWidgets.Count))
            )
        );
        ...
        while (oldChildrenTop <= oldChildrenBottom && newChildrenTop <= newChildrenBottom)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                (oldChild is null)
                || Equals(oldChild._lifecycleState, _ElementLifecycle__framework.active)
            );
            ...
            Element newChild = updateChild(
                oldChild,
                newWidget,
                slotFor(newChildrenTop, previousChildLocal)
            )!;
            DartRuntimePrimitives.Assert(() =>
                Equals(newChild._lifecycleState, _ElementLifecycle__framework.active)
            );
        ...
        while (oldChildrenTop <= oldChildrenBottom && newChildrenTop <= newChildrenBottom)
        {
            Element? oldChildLocal = replaceWithNullIfForgotten(
                oldChildren[(int)oldChildrenBottom]
            );
            Widget newWidgetLocal = newWidgets[(int)newChildrenBottom];
            DartRuntimePrimitives.Assert(() =>
                (oldChildLocal is null)
                || Equals(oldChildLocal._lifecycleState, _ElementLifecycle__framework.active)
            );
        ...
        if (haveOldChildren)
        {
            ...
            {
                Element? oldChildAlternate = replaceWithNullIfForgotten(
                    oldChildren[(int)oldChildrenTop]
                );
                DartRuntimePrimitives.Assert(() =>
                    (oldChildAlternate is null)
                    || Equals(
                        oldChildAlternate._lifecycleState,
                        _ElementLifecycle__framework.active
                    )
                );
        ...
        while (newChildrenTop <= newChildrenBottom)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                ...
                || Widget.canUpdate(oldChildNested.widget, newWidgetAlternate)
            );
            Element newChildLocal = updateChild(
                oldChildNested,
                newWidgetAlternate,
                slotFor(newChildrenTop, previousChildLocal)
            )!;
            DartRuntimePrimitives.Assert(() =>
                Equals(newChildLocal._lifecycleState, _ElementLifecycle__framework.active)
            );
            DartRuntimePrimitives.Assert(() =>
                Equals(oldChildNested, newChildLocal)
                || (oldChildNested is null)
                || (!Equals(oldChildNested._lifecycleState, _ElementLifecycle__framework.active))
            );
        ...
        DartRuntimePrimitives.Assert(() =>
            (checked(newWidgets.Count) - newChildrenTop)
            == (checked(oldChildren.Count) - oldChildrenTop)
        );
        ...
        while (oldChildrenTop <= oldChildrenBottom && newChildrenTop <= newChildrenBottom)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
                replaceWithNullIfForgotten(oldChildCurrent) is not null
            );
            DartRuntimePrimitives.Assert(() =>
                Equals(oldChildCurrent._lifecycleState, _ElementLifecycle__framework.active)
            );
            Widget newWidgetNested = newWidgets[(int)newChildrenTop];
            DartRuntimePrimitives.Assert(() =>
                Widget.canUpdate(oldChildCurrent.widget, newWidgetNested)
            );
            Element newChildAlternate = updateChild(
                oldChildCurrent,
                newWidgetNested,
                slotFor(newChildrenTop, previousChildLocal)
            )!;
            DartRuntimePrimitives.Assert(() =>
                Equals(newChildAlternate._lifecycleState, _ElementLifecycle__framework.active)
            );
            DartRuntimePrimitives.Assert(() =>
                Equals(oldChildCurrent, newChildAlternate)
                || (!Equals(oldChildCurrent._lifecycleState, _ElementLifecycle__framework.active))
            );
    ...
    {
        DartRuntimePrimitives.Assert(
            () => Equals(_lifecycleState, _ElementLifecycle__framework.initial),
            () =>
                (object?)
                    $"This element is no longer in its initial state ({_lifecycleState.ToString()})"
        );
        DartRuntimePrimitives.Assert(
            () => _parent is null,
            () =>
        ...
        DartRuntimePrimitives.Assert(
            () =>
                (parent is null)
                || Equals(parent._lifecycleState, _ElementLifecycle__framework.active),
            () =>
                (object?)
                    $"Parent ({parent}) should be null or in the active state ({parent?._lifecycleState.ToString()})"
        );
        DartRuntimePrimitives.Assert(
            () => slot is null,
            () => (object?)$"This element already has a slot ({slot}) and it shouldn't"
        );
        _parent = parent;
        _slot = newSlot;
    ...
    public virtual void update(Widget newWidget)
    {
        DartRuntimePrimitives.Assert(() =>
            ...
            && (!Equals(newWidget, widget))
            && Widget.canUpdate(widget, newWidget)
        );
        DartRuntimePrimitives.Assert(() =>
        {
            _debugForgottenChildrenWithGlobalKey?.forEach(
                (__arg0) => ((Action<Element>)_debugRemoveGlobalKeyReservation)(__arg0)
            );
    ...
    public virtual void updateSlot(object? newSlot)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
            Equals(_parent!._lifecycleState, _ElementLifecycle__framework.active)
        );
        _slot = newSlot;
    }
    ...
    internal virtual Element? _retakeInactiveElement(GlobalKeyBase key, Widget newWidget)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            {
                PrintLibrary.debugPrint(
                    $"Attempting to take {element} from {(object?)element._parent ?? (object?)"inactive elements list"} to put in {this}."
                );
            }
            return true;
            throw new InvalidOperationException("Dart closure completed without a value.");
                    ...
                    throw DartRuntimePrimitives.AsException(
                        new FlutterError(
                            new List<DiagnosticsNode>
                            {
                                ...
                                new DiagnosticsProperty<GlobalKeyBase>(
                                ...
                                new DiagnosticsProperty<Widget>(
                                    "The second child that was to get instantiated with that key was",
                                    widget,
                                    style: DiagnosticsTreeStyle.errorProperty
                                ),
                                new ErrorDescription(
    ...
    public virtual Element inflateWidget(Widget newWidget, object? newSlot)
    {
        ...
        {
            Key? keyLocal = newWidget.key;
            Element? inactiveChild =
                (keyLocal is GlobalKeyBase globalKey)
                    ? _retakeInactiveElement(globalKey, newWidget)
                    : null;
    ...
    {
        DartRuntimePrimitives.Assert(() =>
            Equals(_lifecycleState, _ElementLifecycle__framework.inactive)
        );
        _parent = parent;
        _owner = parent.owner;
        DartRuntimePrimitives.Assert(() =>
    ...
    public virtual void activate()
    {
        if (FrameworkWorkTrace.Enabled)
        {
            FrameworkWorkTrace.Record(FrameworkWorkTrace.Kind.Activate, this);
        }
        ...
        DartRuntimePrimitives.Assert(() => owner is not null);
        bool hadDependencies =
            (
                (
                    _dependencies is { } __items203339
                        ? System.Linq.Enumerable.Any(__items203339)
                        : (bool?)null
                ) ?? false
    ...
    public virtual void deactivate()
    {
        if (FrameworkWorkTrace.Enabled)
        {
            FrameworkWorkTrace.Record(FrameworkWorkTrace.Kind.Deactivate, this);
        }
    ...
    internal virtual void _ensureDeactivated()
    {
        if (_dependencies is HashSet<InheritedElement> dependencies && Enumerable.Any(dependencies))
        {
            foreach (var dependency in dependencies)
            {
                ...
                {
                    FrameworkWorkTrace.Record(
                        FrameworkWorkTrace.Kind.RemoveDependency,
                        this,
                        dependency
                    );
                }
    ...
    public virtual void unmount()
    {
        if (FrameworkWorkTrace.Enabled)
        {
            FrameworkWorkTrace.Record(FrameworkWorkTrace.Kind.Unmount, this);
        }
    ...
    public virtual Size? size
    {
        get
        {
            ...
            DartRuntimePrimitives.Assert(() =>
            {
                            ...
                            new List<DiagnosticsNode>
                            {
                                ...
                                new ErrorHint(
                                    "The render object associated with this element is a "
                                        + $"{DartRuntimePrimitives.RuntimeType(renderObject__212317__as213062)}, which is a subtype of RenderSliver. "
                                ...
                                renderObject__212317__as213062.describeForError(
                            ...
                            new List<DiagnosticsNode>
                            {
                                ...
                                new ErrorHint(
                                    "Instead of being a subtype of RenderBox, the render object associated "
                                        + $"with this element is a {DartRuntimePrimitives.RuntimeType(renderObject)}. If this type of "
                ...
                RenderBox box = DartRuntimePrimitives.ConvertValue<RenderBox>(
    ...
    )
    {
        if (FrameworkWorkTrace.Enabled)
        {
            FrameworkWorkTrace.Record(FrameworkWorkTrace.Kind.RegisterDependency, this, ancestor);
        }
    ...
    internal virtual void _updateInheritance()
    {
        ...
        _inheritedElements =
            _parent?._inheritedElements ?? PersistentHashMap<Type, InheritedElement>.CreateEmpty();
    }
    ...
    public virtual T? findAncestorWidgetOfExactType<T>()
    {
        ...
        while (
            ...
            && (!Equals(DartRuntimePrimitives.RuntimeType(ancestor.widget), typeof(T)))
        )
        {
    ...
    public virtual void didChangeDependencies()
    {
        ...
        if (FrameworkWorkTrace.Enabled)
        {
            FrameworkWorkTrace.Record(FrameworkWorkTrace.Kind.Dependency, this);
        }
            ...
            if (owner!._debugCurrentBuildTarget is null)
            {
                throw DartRuntimePrimitives.AsException(
                    new FlutterError(
                        ...
                        {
                            new ErrorSummary(
                                $"{methodName} for {DartRuntimePrimitives.RuntimeType(widget)} was called at an "
                                    + "inappropriate time."
                            ),
                            new ErrorDescription(
    ...
    public virtual string toStringShort() =>
        ((Diagnosticable?)_widget)?.toStringShort()
        ?? $"{DiagnosticsLibrary.describeIdentity(this)}(DEFUNCT)";
                    ...
                    () =>
                    {
                        ...
                        __cascade.sort(
                            (a, b) =>
                                ((Diagnosticable)a)
                                    .toStringShort()
                                    .CompareTo(((Diagnosticable)b).toStringShort())
                        );
                        return __cascade;
                    }
                )
            )().ToList();
            List<DiagnosticsNode> diagnosticsDependencies = sortedDependencies
                .map(
                    (element) =>
                        ((Diagnosticable)element.widget).toDiagnosticsNode(
                ...
                .ToList();
            properties.add(
                new DiagnosticsProperty<HashSet<InheritedElement>>(
                    "dependencies",
                    deps,
    ...
    public virtual void markNeedsBuild()
    {
        ...
        if (_dirty)
        {
            FrameworkWorkCounters.Add(FrameworkWork.MarkBuildAlreadyDirty);
        }

        DartRuntimePrimitives.Assert(() =>
            !Equals(_lifecycleState, _ElementLifecycle__framework.defunct)
        );
        if (!Equals(_lifecycleState, _ElementLifecycle__framework.active))
        {
            return;
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            if (owner!._debugBuilding)
            {
                ...
                var information = new List<DiagnosticsNode>
                {
                    ...
                    new ErrorDescription(
                        $"This {DartRuntimePrimitives.RuntimeType(widget)} widget cannot be marked as needing to build because the framework "
                    ...
                    throw DartRuntimePrimitives.AsException(
                        new FlutterError(
                            new List<DiagnosticsNode>
                            {
                                ...
                                new ErrorDescription(
                                    $"This {DartRuntimePrimitives.RuntimeType(widget)} widget cannot be marked as needing to build "
    ...
    public virtual void rebuild(bool force = false)
    {
        ...
        try
        {
            ...
            if (force)
            {
                FrameworkWorkCounters.Add(FrameworkWork.ForcedRebuild);
            }

            using var profile = FrameworkWorkCounters.Enabled
                ? FrameworkWorkProfile.Begin(widget.GetType())
                : default;
            if (FrameworkWorkTrace.Enabled)
            {
                FrameworkWorkTrace.Record(FrameworkWorkTrace.Kind.Build, this);
            }
...
internal class _ElementDiagnosticableTreeNode__framework
    : DiagnosticableTreeNode<DiagnosticableTree>
{
    public virtual bool stateful { get; private set; } = default!;

    internal _ElementDiagnosticableTreeNode__framework(
        string? name = null,
...
public class ErrorWidget : LeafRenderObjectWidget
{
    ...
    public override RenderObject createRenderObject(BuildContext context) =>
        DartRuntimePrimitives.ConvertValue<RenderObject>(new RenderErrorBox(message));
    ...
    public override void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        ...
        {
            properties.add(
                ((Diagnosticable)_flutterError).toDiagnosticsNode(
...
public abstract class ComponentElement : Element
{
    ...
    protected ComponentElement(Widget widget)
        : base(widget) { }
...
public class StatefulElement : ComponentElement
{
    ...
    {
        DartRuntimePrimitives.Assert(() =>
            Equals(state._debugLifecycleState, _StateLifecycle__framework.created)
        );
        object? debugCheckForReturnedFuture = DartRuntimePrimitives.CaptureVoid(() =>
            state.initState()
        );
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            {
                Future debugCheckForReturnedFuture__250900__as250986 =
                    (Future)debugCheckForReturnedFuture;
                throw DartRuntimePrimitives.AsException(
                    new FlutterError(
                        new List<DiagnosticsNode>
                        {
                            new ErrorSummary(
                                $"{DartRuntimePrimitives.RuntimeType(state)}.initState() returned a Future."
                            ),
                            new ErrorDescription(
    ...
    public override void update(Widget newWidget)
    {
        ...
        if (FrameworkWorkTrace.Enabled)
        {
            FrameworkWorkTrace.Record(FrameworkWorkTrace.Kind.DidUpdateWidget, this, state);
        }
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            if (debugCheckForReturnedFuture is Future)
            {
                Future debugCheckForReturnedFuture__252202__as252303 =
                    (Future)debugCheckForReturnedFuture;
                throw DartRuntimePrimitives.AsException(
                    new FlutterError(
                        new List<DiagnosticsNode>
                        {
                            new ErrorSummary(
                                $"{DartRuntimePrimitives.RuntimeType(state)}.didUpdateWidget() returned a Future."
                            ),
                            new ErrorDescription(
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            throw DartRuntimePrimitives.AsException(
                new FlutterError(
                    ...
                    {
                        new ErrorSummary(
                            $"{DartRuntimePrimitives.RuntimeType(state)}.dispose failed to call super.dispose."
                        ),
                        new ErrorDescription(
    ...
    public override InheritedWidget dependOnInheritedElement(
        InheritedElement ancestor,
        object? aspect = null
    )
    {
            ...
            if (Equals(state._debugLifecycleState, _StateLifecycle__framework.created))
            {
                throw DartRuntimePrimitives.AsException(
                    new FlutterError(
                        ...
                        {
                            new ErrorSummary(
                                $"dependOnInheritedWidgetOfExactType<{targetType}>() or dependOnInheritedElement() was called before {DartRuntimePrimitives.RuntimeType(state)}.initState() completed."
                            ),
                            new ErrorDescription(
    ...
    {
        return new _ElementDiagnosticableTreeNode__framework(
            name: name,
            value: this,
            style: style,
            stateful: true
...
{
    protected ProxyElement(ProxyWidget widget)
        : base(widget) { }
...
{
    public ParentDataElement(ParentDataWidget<T> widget)
        : base(widget) { }
    ...
    public virtual void applyWidgetOutOfTurn(ParentDataWidget<T> newWidget)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
            Equals(newWidget.child, ((ParentDataWidget<T>)widget).child)
        );
...
{
    internal virtual DartMap<Element, object?> _dependents { get; private set; } =
        new DartMap<Element, object?>();
    ...
    internal override void _updateInheritance()
    {
        ...
        PersistentHashMap<Type, InheritedElement> incomingWidgets =
            _parent?._inheritedElements ?? PersistentHashMap<Type, InheritedElement>.CreateEmpty();
...
public abstract class RenderObjectElement : Element
{
    ...
    public override RenderObject renderObject
    {
        ...
        {
            DartRuntimePrimitives.Assert(
                () => _renderObject is not null,
                () => (object?)$"{GetType()} unmounted"
            );
            return _renderObject!;
        }
    }
    public override Element? renderObjectAttachingChild =>
        DartRuntimePrimitives.ConvertValue<Element>(null);
    public override bool debugDoingBuild => _debugDoingBuild;
    ...
    internal virtual void _debugCheckCompetingAncestors(
        List<IParentDataElement> result,
    ...
    )
    {
        ...
        {
            if (
                (checked(debugAncestorTypes.Count) != checked((long)result.Count))
                || (checked(debugParentDataTypes.Count) != checked((long)result.Count))
            )
            {
                DartRuntimePrimitives.Assert(() =>
                    (checked(debugAncestorTypes.Count) < checked((long)result.Count))
        ...
        while ((ancestorLocal is not null) && (ancestorLocal is not RenderObjectElement))
        {
            if (ancestorLocal is IParentDataElement)
            {
                ...
                DartRuntimePrimitives.Assert(() =>
                {
                    ...
                    if (
                        !debugAncestorTypes.Add(DartRuntimePrimitives.RuntimeType(ancestor))
    ...
    public override void deactivate()
    {
        ...
        DartRuntimePrimitives.Assert(
            ...
            () =>
                (object?)"A RenderObject was still attached when attempting to deactivate its "
    ...
    public override void unmount()
    {
        ...
        DartRuntimePrimitives.Assert(
            () => !renderObject.attached,
            () =>
                (object?)"A RenderObject was still attached when attempting to unmount its "
                + $"RenderObjectElement: {renderObject}"
        );
    ...
    internal virtual void _updateParentData<T>(ParentDataWidget<T> parentDataWidget)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            ...
            {
                FrameworkLibrary._reportException(
                    new ErrorSummary("while applying parent data."),
                    e,
                    e.stackTrace
                );
            }
    ...
    public override void updateSlot(object? newSlot)
    {
        ...
        DartRuntimePrimitives.Assert(() => Equals(slot, newSlot));
        DartRuntimePrimitives.Assert(() =>
            Equals(_ancestorRenderObjectElement, _findAncestorRenderObjectElement())
        );
                            ...
                            new List<DiagnosticsNode>
                            {
                                ...
                                new ErrorHint(
                                    "Try wrapping your widget in a View widget or any other widget that is backed by "
                                        + $"a {typeof(RenderTreeRootElement)} to serve as the root of the render tree."
                                ),
                            }
                        )
                    )
    ...
    public abstract void moveRenderObjectChild(
        RenderObject child,
        object? oldSlot,
        object? newSlot
    );
    ...
    public override void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        DiagnosticableDefaults.debugFillProperties(properties);
        properties.add(
            new DiagnosticsProperty<RenderObject>("renderObject", _renderObject, defaultValue: null)
        );
    }
}
...
{
    protected RootRenderObjectElement(RenderObjectWidget widget)
        : base(widget) { }
...
{
    public LeafRenderObjectElement(LeafRenderObjectWidget widget)
        : base(widget) { }
...
public class SingleChildRenderObjectElement : RenderObjectElement
{
    ...
    public SingleChildRenderObjectElement(SingleChildRenderObjectWidget widget)
        : base(widget) { }
...
public class MultiChildRenderObjectElement : RenderObjectElement
{
        ...
        : base(widget)
    {
        System.Diagnostics.Debug.Assert(
            !DebugLibrary.debugChildrenHaveDuplicateKeys(widget, widget.children.Cast<Widget>())
        );
    }

    public override RenderObject renderObject
    {
        get { return DartRuntimePrimitives.ConvertValue<RenderObject>(base.renderObject); }
    }
    ...
    public override void insertRenderObjectChild(RenderObject child, object? slot)
    {
        var __slot =
            ...
            ?? throw new ArgumentException(
                "A multi-child render slot must be an IndexedSlot.",
                nameof(slot)
            );
    ...
    public override void mount(Element? parent, object? newSlot)
    {
        ...
        var childrenLocal = new List<Element>(
            Enumerable.Repeat<Element>(
                _NullElement__framework.instance,
                checked((int)checked((long)multiChildRenderObjectWidget.children.Count))
            )
        );
        ...
        for (var i = 0L; i < checked(childrenLocal.Count); i += 1L)
        {
            Element newChild = inflateWidget(
                multiChildRenderObjectWidget.children[(int)i],
    ...
    public override void update(Widget newWidget)
    {
        ...
        DartRuntimePrimitives.Assert(() => Equals(widget, __newWidget));
        DartRuntimePrimitives.Assert(() =>
            !DebugLibrary.debugChildrenHaveDuplicateKeys(
                widget,
                multiChildRenderObjectWidget.children.Cast<Widget>()
            )
        );
...
{
    protected RenderTreeRootElement(RenderObjectWidget widget)
        : base(widget) { }
...
public static partial class FrameworkLibrary
{
    internal static FlutterErrorDetails _reportException(
    ...
    where T : Element?
{
    ...
    public override bool Equals(object? other)
    {
        ...
        return (__other is IndexedSlot<T>)
            && (index == __other.index)
            && Equals(value, __other.value);
    }

    public override int GetHashCode() =>
        DartRuntimePrimitives.ConvertValue<int>(FoundationRuntimePorts.ObjectHash(index, value));
}
...
internal class _NullElement__framework : Element
{
    ...
    internal _NullElement__framework()
        : base(new _NullWidget__framework()) { }
