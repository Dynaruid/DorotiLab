...
public class AnimatedCrossFade : StatefulWidget
{
    ...
    public virtual Func<Widget, Key, Widget, Key, Widget> layoutBuilder { get; private set; } =
        default!;
    ...
    public AnimatedCrossFade(
        Key? key = null,
        Widget firstChild = default!,
        Widget secondChild = default!,
        Curve firstCurve = default!,
        Curve secondCurve = default!,
        Curve sizeCurve = default!,
        AlignmentGeometry alignment = default!,
        CrossFadeState crossFadeState = default!,
        Duration duration = default!,
        Duration? reverseDuration = null,
        Func<Widget, Key, Widget, Key, Widget> layoutBuilder = default!,
        ...
        : base(key: key)
    {
        ...
        Func<Widget, Key, Widget, Key, Widget> __layoutBuilder =
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(
            new _AnimatedCrossFadeState__animated_cross_fade()
        );
...
internal class _AnimatedCrossFadeState__animated_cross_fade
    : State<AnimatedCrossFade>,
        TickerProviderStateMixin<AnimatedCrossFade>
{
    ...
    public override void initState()
    {
        ...
        _controller.addStatusListener(
            (status) =>
            {
                setState(() => { });
                if (
                    Equals(status, AnimationStatus.completed)
                    || Equals(status, AnimationStatus.dismissed)
                )
                {
                    widget.onEnd?.Invoke();
                }
    ...
    public override Widget build(BuildContext context)
    {
        ...
        bottomChild = DartRuntimePrimitives.ConvertValue<Widget>(
            new TickerMode(
                key: bottomKey,
                ...
                child: new IgnorePointer(
                    child: new ExcludeSemantics(
                        child: new ExcludeFocus(
                            ...
                            child: new FadeTransition(opacity: bottomAnimation, child: bottomChild)
                        )
                    )
                )
            )
        );
        topChild = DartRuntimePrimitives.ConvertValue<Widget>(
            new TickerMode(
                key: topKey,
                enabled: true,
                child: new IgnorePointer(
                    ...
                    child: new ExcludeSemantics(
                        ...
                        child: new ExcludeFocus(
                            excluding: false,
                            child: new FadeTransition(opacity: topAnimation, child: topChild)
                        )
                    )
                )
            )
    ...
    public override void debugFillProperties(DiagnosticPropertiesBuilder description)
    {
        ...
        description.add(
            new EnumProperty<Clip>("clipBehavior", widget.clipBehavior, defaultValue: Clip.hardEdge)
        );
    }
