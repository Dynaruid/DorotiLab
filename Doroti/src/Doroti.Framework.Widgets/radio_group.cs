...
public static class RadioGroup
{
    public static RadioGroupRegistry<T>? maybeOf<T>(BuildContext context) =>
        RadioGroup<T>.maybeOf<T>(context);
}
...
public class RadioGroup<T> : StatefulWidget
{
    ...
    {
        return DartRuntimePrimitives.NullAware(
            context.dependOnInheritedWidgetOfExactType<
                _RadioGroupStateScope__radio_group<TValue>
            >(),
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new _RadioGroupState__radio_group<T>());
}
...
internal class _RadioGroupState__radio_group<T> : State<RadioGroup<T>>, RadioGroupRegistry<T>
{
    ...
    internal virtual DartMap<ShortcutActivator, Intent> _radioGroupShortcuts
    {
        get
        {
            ...
            {
                __late__radioGroupShortcuts = new DartMap<ShortcutActivator, Intent>
                {
                    [new SingleActivator(LogicalKeyboardKey.arrowLeft)] = new VoidCallbackIntent(
                        () =>
                            _selectPreviousRadio()
                    ),
                    [new SingleActivator(LogicalKeyboardKey.arrowRight)] = new VoidCallbackIntent(
                        () =>
                            _selectNextRadio()
                    ),
                    [new SingleActivator(LogicalKeyboardKey.arrowDown)] = new VoidCallbackIntent(
                        () =>
                            _selectNextRadio()
                    ),
                    [new SingleActivator(LogicalKeyboardKey.arrowUp)] = new VoidCallbackIntent(() =>
                        _selectPreviousRadio()
                    ),
                    [new SingleActivator(LogicalKeyboardKey.space)] = new VoidCallbackIntent(() =>
                        _toggleFocusedRadio()
                    ),
                };
    ...
    internal virtual _RadioGroupShortcutManager__radio_group<T> _radioGroupShortcutManager
    {
        get
        {
            ...
            {
                __late__radioGroupShortcutManager = new _RadioGroupShortcutManager__radio_group<T>(
    ...
    internal virtual HashSet<RadioClient<T>> _radios { get; private set; } =
        new HashSet<RadioClient<T>>();
    ...
    internal virtual void _selectRadioInDirection(bool forward)
    {
        ...
        FocusNode? currentFocus = _radios
            .firstWhereOrNull((radio) => radio.focusNode.hasFocus)
            ?.focusNode;
        if (currentFocus is null)
        {
            return;
        }
        List<FocusNode> sorted = ReadingOrderTraversalPolicy
            .sort(
                _radios
                    .where((radio) => radio.enabled)
                    .map((radio) => radio.focusNode)
                    .Cast<FocusNode>()
            )
            .ToList()
            .ToList();
        DartRuntimePrimitives.Assert(() => Enumerable.Any(sorted));
        IEnumerable<FocusNode> nodesInEffectiveOrder = forward
            ? sorted
            : Enumerable.Reverse(sorted);
        ...
        RadioClient<T> radioToSelect = _radios.firstWhere(
            (radio) => Equals(radio.focusNode, nextFocus)
        );
        ...
        return new Semantics(
            ...
            child: Shortcuts.CreateManager(
                ...
                child: new FocusTraversalGroup(
                    policy: new _SkipUnselectedRadioPolicy__radio_group<T>(
                        _radios,
                        widget.groupValue
                    ),
                    child: new _RadioGroupStateScope__radio_group<T>(
                        state: this,
...
internal class _RadioGroupShortcutManager__radio_group<T> : ShortcutManager
{
    ...
    internal _RadioGroupShortcutManager__radio_group(
        DartMap<ShortcutActivator, Intent> shortcuts,
        _RadioGroupState__radio_group<T> state
    )
...
internal class _RadioGroupStateScope__radio_group<T> : InheritedWidget
{
    ...
    internal _RadioGroupStateScope__radio_group(
        _RadioGroupState__radio_group<T> state,
        T? groupValue,
        Widget child
    )
...
internal class _SkipUnselectedRadioPolicy__radio_group<T> : ReadingOrderTraversalPolicy
{
    ...
    internal virtual bool _radioSelected(RadioClient<T> radio) =>
        DartRuntimePrimitives.ConvertValue<bool>(
            EqualityComparer<T>.Default.Equals(radio.radioValue, groupValue)
        );
    ...
    {
        IEnumerable<FocusNode> nodesInReadOrder = base.sortDescendants(
        ...
        HashSet<FocusNode> nodeToSkip = radios
            .where((radio) => (!Equals(selected, radio)) && (!Equals(radio.focusNode, currentNode)))
            .map((radio) => radio.focusNode)
            .toSet();
        IEnumerable<FocusNode> skipsNonSelected = descendants.where(
