...
public abstract class Intent : Diagnosticable
{
...
public interface IIntentAction : IActionListenerSource
{
    ...
    KeyEventResult ToKeyEventResultForIntent(Intent intent, object? invokeResult) =>
        ConsumesKeyForIntent(intent)
            ? KeyEventResult.handled
            : KeyEventResult.skipRemainingHandlers;
    void UpdateCallingAction(IIntentAction? value);
}

public abstract class IntentAction<T> : Diagnosticable, IActionListenerSource, IIntentAction
    where T : Intent
{
    internal virtual ObserverList<Action<object>> _listeners { get; private set; } =
        new ObserverList<Action<object>>();
    internal virtual IIntentAction? _currentCallingAction { get; set; }

    protected IntentAction() { }

    public static IntentAction<T> CreateOverridable(
        IntentAction<T> defaultAction,
    ...
    internal virtual bool _debugCanHandleIntent<I>(I? intent)
        where I : Intent
    {
        object? badIntentString = intent switch
        {
            T __object8942 => null,
            object runtimeType => (object?)runtimeType,
            null when new List<I>() is List<T> => null,
            null => (object?)typeof(I).ToString(),
        };
        DartRuntimePrimitives.Assert(
            () => badIntentString is null,
            () =>
                (object?)
    ...
    internal virtual bool _isEnabled(T intent, BuildContext? context) =>
        this switch
        {
            ContextAction<T> action => action.isEnabled(intent, context),
            _ => isEnabled(intent),
        };
    ...
    internal virtual object? _invoke(T intent, BuildContext? context) =>
        this switch
        {
            ContextAction<T> action => action.invoke(intent, context),
            _ => invoke(intent),
        };

    bool IIntentAction.IsEnabledForIntent(Intent intent, BuildContext? context) =>
        _isEnabled((T)intent, context);

    object? IIntentAction.InvokeIntent(Intent intent, BuildContext? context) =>
        _invoke((T)intent, context);
    ...
    KeyEventResult IIntentAction.ToKeyEventResultForIntent(Intent intent, object? invokeResult) =>
        toKeyEventResult((T)intent, invokeResult);
    ...
    public virtual void removeActionListener(Action<object> listener) =>
        ...
        foreach (var listener in localListeners)
        {
            ...
            DartRuntimePrimitives.Assert(() =>
            {
                collector = () =>
                    ...
                    {
                        new DiagnosticsProperty<IntentAction<T>>(
...
public class ActionListener : StatefulWidget
{
    ...
    public ActionListener(
        Key? key = null,
        Action<object> listener = default!,
        dynamic action = default!,
        ...
        : base(key: key)
    {
        ...
        this.action = action;
        listenerSource =
            (object?)action as IActionListenerSource
            ?? throw new ArgumentException(
                "ActionListener.action must implement IActionListenerSource.",
                nameof(action)
            );
        this.child = child;
    }
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new _ActionListenerState__actions());
}
...
internal class _ActionListenerState__actions : State<ActionListener>
{
    ...
    public override void didUpdateWidget(ActionListener oldWidget)
    {
        ...
        if (
            Equals(oldWidget.listenerSource, widget.listenerSource)
            && Equals(oldWidget.listener, widget.listener)
        )
        {
            return;
        }
...
public abstract class ContextAction<T> : IntentAction<T>
    where T : Intent
{
    public override bool isEnabled(T intent, BuildContext? context = null) =>
        base.isEnabled(intent);
    ...
    {
        return new _OverridableContextAction__actions<T>(
            defaultAction: this,
...
public delegate object? OnInvokeCallback<T>(T intent)
    where T : Intent;

public class CallbackAction<T> : IntentAction<T>
    where T : Intent
{
    public virtual Func<T, object?> onInvoke { get; private set; } = default!;

    public CallbackAction(Func<T, object?> onInvoke)
    {
...
public class ActionDispatcher : Diagnosticable
{
    ...
    public virtual object? invokeAction(dynamic action, Intent intent, BuildContext? context = null)
    {
        ...
        DartRuntimePrimitives.Assert(
            () => intentAction.IsEnabledForIntent(intent, target),
...
public class Actions : StatefulWidget
{
    ...
    public Actions(
        ...
        DartMap<Type, dynamic> actions = default!,
        Widget child = default!
    )
        : base(key: key)
    {
    ...
    internal static IIntentAction _requireAction(object? action, Type? intentType = null)
    {
        if (action is IIntentAction typedAction)
        {
            ...
            {
                throw new ArgumentException(
                    $"Action for {intentType} declares intent type {typedAction.IntentType}.",
                    nameof(action)
                );
            }
            return typedAction;
    ...
    internal static bool _visitActionsAncestors(
        BuildContext context,
        Func<InheritedElement, bool> visitor
    )
    {
        if (!context.mounted)
        {
        ...
        InheritedElement? actionsElement =
            context.getElementForInheritedWidgetOfExactType<_ActionsScope__actions>();
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            if (action is null)
            {
                ...
                throw DartRuntimePrimitives.AsException(
                    FlutterError.Create(
                        $"Unable to find an action for a {@type} in an {typeof(Actions)} widget "
                            + "in the given context.\n"
                            + $"{typeof(Actions)}.find() was called on a context that doesn't contain an "
                            + $"{typeof(Actions)} widget with a mapping for the given intent type.\n"
        ...
        DartRuntimePrimitives.Assert(() =>
        {
            throw DartRuntimePrimitives.AsException(
                new FlutterError(
                    ...
                    {
                        new ErrorSummary(
                            $"An {DartRuntimePrimitives.RuntimeType(action)} cannot be cast to an Action<{typeof(T)}>."
                        ),
                        new ErrorDescription(
                            $"A valid action {action} was found but could not be returned by Actions.maybeFind<{typeof(T)}>."
                        ),
                        new ErrorHint(
    ...
    internal static IIntentAction? _getActionForIntent<T>(
        _ActionsScope__actions actionsMarker,
        T? intent
    )
        where T : Intent
    {
        IIntentAction? mappedAction = actionsMarker.actions.GetValueOrDefault(
            DartRuntimePrimitives.RuntimeType(intent) ?? typeof(T)
        );
        ...
        where T : Intent
    {
        ...
        bool actionFound = _visitActionsAncestors(
            context,
            (element) =>
            {
                var actions = ((_ActionsScope__actions?)element.widget)!;
                ...
                throw DartRuntimePrimitives.AsException(
                    FlutterError.Create(
                        "Unable to find an action for an Intent with type "
                            + $"{DartRuntimePrimitives.RuntimeType(intent)} in an {typeof(Actions)} widget in the given context.\n"
                            + $"{typeof(Actions)}.invoke() was unable to find an {typeof(Actions)} widget that "
                            + "contained a mapping for the given intent, or the intent type isn't the "
                            + $"same as the type argument to invoke (which is {typeof(T)} - try supplying a "
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new _ActionsState__actions());

    public override void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        DiagnosticableDefaults.debugFillProperties(properties);
        properties.add(new DiagnosticsProperty<ActionDispatcher>("dispatcher", dispatcher));
        properties.add(new DiagnosticsProperty<DartMap<Type, object>>("actions", actions));
    }
}

internal class _ActionsState__actions : State<Actions>
{
    public virtual HashSet<IIntentAction>? listenedActions { get; set; } =
        new HashSet<IIntentAction>();
    ...
    {
        return new _ActionsScope__actions(
            actions: widget.typedActions,
...
internal class _ActionsScope__actions : InheritedWidget
{
    ...
    internal _ActionsScope__actions(
        ActionDispatcher? dispatcher,
        DartMap<Type, IIntentAction> actions,
        object rebuildKey,
        Widget child
    )
        : base(child: child)
    ...
    public override bool updateShouldNotify(InheritedWidget oldWidget)
    {
        ...
        bool actionsEqual =
            __oldWidget.actions.Count == actions.Count
            && __oldWidget.actions.All(entry =>
                actions.ContainsKey(entry.Key)
                && Equals(actions.GetValueOrDefault(entry.Key), entry.Value)
            );
...
public class FocusableActionDetector : StatefulWidget
{
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new _FocusableActionDetectorState__actions());
}
...
internal class _FocusableActionDetectorState__actions : State<FocusableActionDetector>
{
    ...
    internal virtual GlobalKey<IState> _mouseRegionKey { get; private set; } =
    ...
    internal virtual void _updateHighlightMode(FocusHighlightMode mode)
    {
        ...
        {
            _canShowHighlight = FocusManager.instance.highlightMode switch
            {
    ...
    internal virtual void _handleMouseEnter(PointerEnterEvent @event)
    {
        if (!_hovering)
        {
            _mayTriggerCallback(task: () =>
    ...
    internal virtual void _handleMouseExit(PointerExitEvent @event)
    {
        if (_hovering)
        {
            _mayTriggerCallback(task: () =>
    ...
    internal virtual void _mayTriggerCallback(
        Action? task = null,
    ...
    )
    {
        ...
        {
            return MediaQuery.maybeNavigationModeOf(context) switch
            {
    ...
    public override Widget build(BuildContext context)
    {
        ...
        if (widget.enabled && (widget.actions is not null) && Enumerable.Any(widget.actions!))
        {
            childLocal = DartRuntimePrimitives.ConvertValue<Widget>(
                new Actions(actions: widget.actions!, child: childLocal)
            );
        }
        if (widget.enabled && (widget.shortcuts is not null) && Enumerable.Any(widget.shortcuts!))
        {
            childLocal = DartRuntimePrimitives.ConvertValue<Widget>(
                new Shortcuts(shortcuts: widget.shortcuts!, child: childLocal)
            );
        }
        return childLocal;
        throw new InvalidOperationException("Control flow completed without returning a value.");
...
{
    public virtual Action callback { get; private set; } = default!;

    public VoidCallbackIntent(Action callback)
    {
...
{
    public static DoNothingIntent Create() => new DoNothingIntent();
...
public class PrioritizedAction : ContextAction<PrioritizedIntents>
{
    ...
    public override bool isEnabled(PrioritizedIntents intent, BuildContext? context = null)
    {
        ...
        {
            IIntentAction? candidateAction = Actions._maybeFindWithoutDependingOn(
                focus.context!,
                candidateIntent,
                declareDependency: true
            );
            if (
                (candidateAction is not null)
                && candidateAction.IsEnabledForIntent(candidateIntent, context)
            )
            {
...
internal interface _OverridableActionMixin__actions<T>
    where T : Intent
{
...
internal class _OverridableAction__actions<T> : ContextAction<T>
    where T : Intent
{
    ...
    internal override ContextAction<T> _makeOverridableAction(BuildContext context)
    {
        return new _OverridableAction__actions<T>(
    ...
    public virtual IIntentAction? _getOverrideAction<U>(U? intent, bool declareDependency = false)
        where U : Intent
    {
        IIntentAction? @override = declareDependency
    ...
    public virtual object? _invokeOverride(
        IIntentAction overrideAction,
        T intent,
        BuildContext? context
    )
    {
...
internal class _OverridableContextAction__actions<T> : ContextAction<T>
    where T : Intent
{
    ...
    internal _OverridableContextAction__actions(
...
internal class _ContextActionToActionAdapter__actions<T> : IntentAction<T>
    where T : Intent
{
    ...
    internal _ContextActionToActionAdapter__actions(
    ...
    public override bool isEnabled(T intent, BuildContext? context = null) =>
        action.isEnabled(intent, invokeContext);
    ...
    public override object? invoke(T intent, BuildContext? context = null) =>
        action.invoke(intent, invokeContext);
}
