...
public sealed class PlatformView : StatefulWidget
{
    public PlatformView(
        ...
        Action<PlatformViewHandle>? onCreated = null,
...
internal sealed class PlatformViewStateImpl : State<PlatformView>
{
    ...
    private void Start()
    {
        _handle = null;
        _error = null;
        var client = widget.Descriptor is { } descriptor
            ? new PlatformViewClient(widget.Owner, descriptor)
    ...
    private void NativeFocused()
    {
        if (mounted)
        {
            widget.Owner.DispatchPlatformEvent(() =>
            {
                if (mounted)
                {
    ...
    private void FocusChanged(bool focused)
    {
        ...
        {
            _ = SetNativeFocusAsync(client, focused);
        }
    }
    ...
    private async Task SetNativeFocusAsync(PlatformViewClient client, bool focused)
    {
        try
        {
            await client.SetFocusAsync(focused);
        }
        ...
        catch (Exception error)
        {
            if (mounted && ReferenceEquals(_client, client))
            {
                FlutterError.reportError(
                    new FlutterErrorDetails(exception: error, library: "platform view focus")
                );
            }
        }
    }
    ...
    private async Task InitializeAsync(PlatformViewClient client, DorotiView owner)
    {
        ...
        catch (Exception exception)
        {
            ...
            { /* The owner closed before the completion event. */
            }
        }
    }
    ...
    public override void didUpdateWidget(PlatformView oldWidget)
    {
        ...
        )
        {
            throw new InvalidOperationException(
                "Changing PlatformView owner/request requires a new widget key so disposal completes before ID reuse."
            );
        }
    }
    ...
    public override Widget build(BuildContext context)
    {
        ...
        {
            throw new InvalidOperationException("PlatformView creation failed.", _error);
        }
    ...
    public override void dispose()
    {
        ...
        if (client is not null)
        {
            _ = DisposeClientAsync(client, widget.OnError);
        }
    ...
    )
    {
        ...
        catch (Exception exception)
        {
            ...
            {
                FlutterError.reportError(
                    new FlutterErrorDetails(exception: exception, library: "platform views")
                );
            }
        }
    }
...
internal sealed class PlatformViewLeaf(PlatformViewHandle? handle) : LeafRenderObjectWidget
{
    public override RenderObject createRenderObject(BuildContext context) =>
        new RenderPlatformView { Handle = handle };

    public override void updateRenderObject(BuildContext context, RenderObject renderObject) =>
        ((RenderPlatformView)renderObject).Handle = handle;
}

public sealed class PointerInterceptor : SingleChildRenderObjectWidget
{
    public PointerInterceptor(
        Widget child,
    ...
    public override RenderObject createRenderObject(BuildContext context) =>
        new RenderPointerInterceptor { Intercepting = Intercepting, Debug = Debug };
