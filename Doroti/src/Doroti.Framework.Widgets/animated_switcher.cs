...
internal class _ChildEntry__animated_switcher
{
    ...
    internal _ChildEntry__animated_switcher(
    ...
    public override string ToString() =>
        $"Entry#{DiagnosticsLibrary.shortHash(this)}({widgetChild})";
}
...
public class AnimatedSwitcher : StatefulWidget
{
    ...
    public virtual Func<Widget, Animation<double>, Widget> transitionBuilder { get; private set; } =
        default!;
    public virtual Func<Widget?, List<Widget>, Widget> layoutBuilder { get; private set; } =
        default!;
    ...
    public AnimatedSwitcher(
        ...
        Curve switchInCurve = default!,
        Curve switchOutCurve = default!,
        Func<Widget, Animation<double>, Widget> transitionBuilder = default!,
        Func<Widget?, List<Widget>, Widget> layoutBuilder = default!
    )
        : base(key: key)
    {
        Curve __switchInCurve = switchInCurve ?? Curves.linear;
        Curve __switchOutCurve = switchOutCurve ?? Curves.linear;
        Func<Widget, Animation<double>, Widget> __transitionBuilder =
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new _AnimatedSwitcherState__animated_switcher());
...
internal class _AnimatedSwitcherState__animated_switcher
    : State<AnimatedSwitcher>,
        TickerProviderStateMixin<AnimatedSwitcher>
{
    internal virtual _ChildEntry__animated_switcher? _currentEntry { get; set; } = default;
    internal virtual HashSet<_ChildEntry__animated_switcher> _outgoingEntries
    {
        get;
        private set;
    } = new HashSet<_ChildEntry__animated_switcher>();
    ...
    public override void didUpdateWidget(AnimatedSwitcher oldWidget)
    {
        ...
        {
            _outgoingEntries.forEach(
                (__arg0) =>
                    ((Action<_ChildEntry__animated_switcher>)_updateTransitionForEntry)(__arg0)
            );
    ...
    internal virtual _ChildEntry__animated_switcher _newEntry(
        Widget child,
        Func<Widget, Animation<double>, Widget> builder,
        AnimationController controller,
        CurvedAnimation animation
    )
    {
        var entry = new _ChildEntry__animated_switcher(
            widgetChild: child,
            transition: KeyedSubtree.CreateWrap(builder(child, animation), _childNumber),
            animation: animation,
    ...
    {
        entry.transition = DartRuntimePrimitives.ConvertValue<Widget>(
            new KeyedSubtree(
    ...
    {
        _outgoingWidgets ??= new List<Widget>(
            DartRuntimePrimitives.ConvertEnumerable<Widget>(
                _outgoingEntries.map((entry) => entry.transition)
            )
        );
        DartRuntimePrimitives.Assert(() =>
            checked(_outgoingEntries.Count) == checked((long)_outgoingWidgets!.Count)
        );
        DartRuntimePrimitives.Assert(() =>
            !Enumerable.Any(_outgoingEntries)
            || Equals(_outgoingEntries.Last().transition, _outgoingWidgets!.Last())
        );
    }
    ...
    public override Widget build(BuildContext context)
    {
        ...
        return widget.layoutBuilder(
            ...
            _outgoingWidgets!
                .where((outgoing) => !Equals(outgoing.key, _currentEntry?.transition.key))
                .toSet()
                .ToList()
        );
        throw new InvalidOperationException("Dart control flow completed without a value.");
