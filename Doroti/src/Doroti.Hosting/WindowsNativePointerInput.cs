...
public sealed class WindowsNativePointerInput : IDisposable
{
    ...
    public WindowsNativePointerInput(nint window, ulong viewId, Action<PointerDataPacket> dispatch)
    {
        ...
        if (!SetWindowSubclass(window, _procedure, _id, 0))
        {
            throw new InvalidOperationException(
                "Cannot attach native touch/pen input to the window thread."
            );
        }
    }
        ...
        if (!_disposed)
        {
            try
            {
                ...
                if (
                    ...
                    && (unchecked((uint)GetMessageExtraInfo()) & 0xffffff00u) == 0xff515700u
                )
                {
                    return 0;
                }
                ...
                {
                    if (message == 0x024a && _pointers.ContainsKey(pointer))
                    {
                        Remove(pointer, _down.Contains(pointer));
                        return 0;
                    }
                    if (GetPointerInfo(pointer, out var info) && info.Type is 2 or 3)
                    {
                        if (message == 0x024a)
                        {
                            return 0; // A touch was already removed on up.
                        }

                        PenInfo pen = default;
                        if (info.Type == 3 && !GetPointerPenInfo(pointer, out pen))
                        {
                            pen = default;
                        }
                ...
                if (message is 0x0008 or 0x001f)
                {
                    Cancel();
                }
            ...
            {
                Console.Error.WriteLine($"[Doroti pointer] {error.Message}");
                Cancel();
            }
        }
    ...
    private void Process(uint message, in PointerInfo info, in PenInfo pen, double x, double y)
    {
        var id = info.Id;
        var kind =
            info.Type == 2 ? PointerDeviceKind.touch
            : (pen.Flags & 6) != 0 ? PointerDeviceKind.invertedStylus
            : PointerDeviceKind.stylus;
        if (_pointers.TryGetValue(id, out var previous) && previous.kind != kind)
        {
            Remove(id, true);
        }
        ...
        if (
            kind is PointerDeviceKind.stylus or PointerDeviceKind.invertedStylus
            && (pen.Flags & 1) != 0
        )
        {
            buttons |= 2;
        ...
        var packet = new PointerData(
            ...
            change,
            kind,
            (1UL << 52) | ((ulong)kind << 48) | id,
            x,
            y,
            exists ? x - previous.physicalX : 0,
        ...
        if (change == PointerChange.down || (change == PointerChange.move && !_down.Contains(id)))
        {
            packet = packet with
            {
                pointerIdentifier = checked((ulong)Interlocked.Increment(ref _nextPointer)),
                change = PointerChange.down,
                physicalDeltaX = 0,
                physicalDeltaY = 0,
                synthesized = change != PointerChange.down,
            };
            _down.Add(id);
        }
        Emit(id, packet);
        ...
        {
            _down.Remove(id);
            if (kind == PointerDeviceKind.touch || change == PointerChange.cancel)
            {
                Remove(id, false);
            }
        }
    ...
    private void Emit(uint id, PointerData packet)
    {
        _pointers[id] = packet;
        _dispatch(new([packet]));
    }

    private void Remove(uint id, bool cancel)
    {
        if (!_pointers.TryGetValue(id, out var packet))
        {
            return;
        }
    ...
    private void Cancel()
    {
        foreach (var id in _pointers.Keys.ToArray())
        {
            Remove(id, true);
        }
    }
    ...
    [DllImport("user32.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool GetPointerInfo(uint id, out PointerInfo info);
