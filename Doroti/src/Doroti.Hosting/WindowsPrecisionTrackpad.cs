...
public sealed unsafe class WindowsPrecisionTrackpad : IDisposable
{
    ...
    private readonly nint _window, _root;
    private readonly ulong _viewId;
    private readonly Action<PointerDataPacket> _dispatch;
    private readonly SubclassProc _subclass;
    private nint _manager, _updates, _viewport;
    private Callback* _callback;
    private uint _cookie;
    private nuint _subclassId;
    private bool _attached, _rootAttached, _disposed, _running, _inertia, _resetting, _added;
    private double _initialScale = 1, _initialX, _initialY, _lastX, _lastY, _lastDeltaX, _lastDeltaY;
    ...
    {
        var clsid = ManagerClass; var iid = ManagerId;
        nint result;
        Check(CoCreateInstance(&clsid, 0, 1, &iid, &result)); _manager = result;
        iid = UpdateId;
        Check(((delegate* unmanaged[Stdcall]<nint, Guid*, nint*, int>)V(_manager, 7))(_manager, &iid, &result)); _updates = result;
        iid = ViewportId;
        Check(((delegate* unmanaged[Stdcall]<nint, nint, nint, Guid*, nint*, int>)V(_manager, 8))(_manager, 0, _window, &iid, &result)); _viewport = result;
        ...
        uint cookie;
        Check(((delegate* unmanaged[Stdcall]<nint, nint, Callback*, uint*, int>)V(_viewport, 25))(_viewport, _window, _callback, &cookie));
        _cookie = cookie;
        Resize();
        ...
        if (!_attached) throw new InvalidOperationException("Cannot attach precision trackpad input to the window thread.");
    ...
    private static int Call(nint value, int slot) => ((delegate* unmanaged[Stdcall]<nint, int>)V(value, slot))(value);
    private static int U32(nint value, int slot, uint argument) => ((delegate* unmanaged[Stdcall]<nint, uint, int>)V(value, slot))(value, argument);
    private void Update() => Check(((delegate* unmanaged[Stdcall]<nint, nint, int>)V(_updates, 5))(_updates, 0));
    private void Resize()
    {
        if (!GetClientRect(_window, out var rect)) return;
        rect.Right = Math.Max(1, rect.Right); rect.Bottom = Math.Max(1, rect.Bottom);
        Check(((delegate* unmanaged[Stdcall]<nint, Rect*, int>)V(_viewport, 12))(_viewport, &rect));
    }

    private nint HandleMessage(nint hwnd, uint message, nuint wparam, nint lparam, nuint id, nuint data)
    {
        try
        {
            if (!_disposed)
            {
                if (message == 0x0250 && GetPointerType((uint)(wparam & 0xffff), out var type) && type == 5)
                {
                ...
                if (message == 0x0113 && wparam == _subclassId) { Update(); return 0; }
                if (message == 0x0005) Resize();
                if (message == 0x001f || (message == 0x001c && wparam == 0)) Cancel();
                if (message == 0x0082 && hwnd == _window) Dispose();
            }
        }
        catch (Exception error) { Console.Error.WriteLine($"[Doroti trackpad] {error.Message}"); Cancel(); }
        return DefSubclassProc(hwnd, message, wparam, lparam);
    }

    private void Emit(PointerData data) { _last = data; _dispatch(new([data])); }
    private void Start(nint viewport)
    {
        var iid = ContentId;
        nint content = 0;
        Check(((delegate* unmanaged[Stdcall]<nint, Guid*, nint*, int>)V(viewport, 16))(viewport, &iid, &content));
        try { ReadTransform(content, out _initialScale, out _initialX, out _initialY); }
        finally { Release(content); }
        _lastX = _lastY = _lastDeltaX = _lastDeltaY = 0;
        GetCursorPos(out var point); ScreenToClient(_window, ref point);
        var pointer = checked((ulong)Interlocked.Increment(ref _nextPointer));
        var packet = new PointerData(_viewId, DorotiFrameClock.Now, PointerChange.panZoomStart,
            PointerDeviceKind.trackpad, (1UL << 62) | unchecked((ulong)_window), point.X, point.Y, 0, 0, 0, pointerIdentifier: pointer);
        if (!_added) { Emit(packet with { change = PointerChange.add }); _added = true; }
        _running = true;
        Emit(packet);
    }
    ...
    private void End()
    {
        if (!_running) return;
        _running = false;
        Emit(_last with { timeStamp = DorotiFrameClock.Now, change = PointerChange.panZoomEnd, panDeltaX = 0, panDeltaY = 0 });
    }

    private void Status(nint viewport, uint current, uint previous)
    {
        if (_disposed) return;
        if (_resetting) { _resetting = current != 5; return; }
        _inertia = current == 4;
        if (previous == 4 && current != 4)
        {
            if (Math.Max(Math.Abs(_lastDeltaX), Math.Abs(_lastDeltaY)) > .01)
                Emit(_last with { timeStamp = DorotiFrameClock.Now, change = PointerChange.hover,
                    signalKind = PointerSignalKind.scrollInertiaCancel });
        }
        if (current == 3) { if (!_running) Start(viewport); }
        else if (previous == 3) { End(); _lastDeltaX = _lastDeltaY = 0; }
        if (current == 5)
        {
            KillTimer(_window, _subclassId);
            _resetting = true;
            GetClientRect(_window, out var rect);
            Check(((delegate* unmanaged[Stdcall]<nint, float, float, float, float, int, int>)V(viewport, 13))
    ...
    private static void ReadTransform(nint content, out double scale, out double x, out double y)
    {
        ...
        Check(((delegate* unmanaged[Stdcall]<nint, float*, uint, int>)V(content, 9))(content, matrix, 6));
    ...
    private void Content(nint content)
    {
        ...
        x -= _initialX; y -= _initialY;
        _lastDeltaX = x - _lastX; _lastDeltaY = y - _lastY;
        _lastX = x; _lastY = y;
        if (_running && !_inertia && _initialScale > 0)
            Emit(_last with { timeStamp = DorotiFrameClock.Now, change = PointerChange.panZoomUpdate,
                panX = x, panY = y, panDeltaX = _lastDeltaX, panDeltaY = _lastDeltaY, scale = scale / _initialScale });
    }

    private void Cancel()
    {
        End();
        if (_added) Emit(_last with { timeStamp = DorotiFrameClock.Now, change = PointerChange.remove, signalKind = PointerSignalKind.none });
        _added = false;
        _resetting = true;
        if (_viewport != 0) { Call(_viewport, 7); Call(_viewport, 29); }
        _resetting = false;
        _inertia = false;
    ...
    public void Dispose()
    {
        ...
        _disposed = true;
        Cancel();
        if (_attached) RemoveWindowSubclass(_window, _subclass, _subclassId);
        if (_rootAttached) RemoveWindowSubclass(_root, _subclass, _subclassId);
        if (_viewport != 0)
        {
            Call(_viewport, 4);
            if (_callback != null) U32(_viewport, 26, _cookie);
            Call(_viewport, 30); Release(_viewport); _viewport = 0;
        }
        if (_manager != 0) ((delegate* unmanaged[Stdcall]<nint, nint, int>)V(_manager, 4))(_manager, _window);
        Release(_updates); Release(_manager); _updates = _manager = 0;
        if (_callback != null) { ReleaseCallback(_callback); _callback = null; }
    }

    private static void Release(nint value) { if (value != 0) ((delegate* unmanaged[Stdcall]<nint, uint>)V(value, 2))(value); }
    private struct Callback { internal nint* Vtable; internal nint Owner; internal int References; }
    private static WindowsPrecisionTrackpad Owner(Callback* self) => (WindowsPrecisionTrackpad)GCHandle.FromIntPtr(self->Owner).Target!;
    private static nint* CreateVtable()
    {
        var table = (nint*)RuntimeHelpers.AllocateTypeAssociatedMemory(typeof(WindowsPrecisionTrackpad), 6 * sizeof(nint));
    ...
    private static int Query(Callback* self, Guid* iid, nint* result)
    {
        ...
        *result = 0;
        if (*iid != UnknownId && *iid != HandlerId) return unchecked((int)0x80004002);
        *result = (nint)self; Interlocked.Increment(ref self->References); return 0;
    }
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvStdcall)])]
    ...
    private static uint ReleaseCallback(Callback* self)
    {
        ...
        if (count == 0) { GCHandle.FromIntPtr(self->Owner).Free(); NativeMemory.Free(self); }
        return (uint)count;
    }
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvStdcall)])]
    ...
    { try { Owner(self).Status(viewport, current, previous); return 0; } catch (Exception e) { return Marshal.GetHRForException(e); } }
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvStdcall)])]
    ...
    { try { Owner(self).Content(content); return 0; } catch (Exception e) { return Marshal.GetHRForException(e); } }
    ...
    [DllImport("ole32.dll")] private static extern int CoCreateInstance(Guid* clsid, nint outer, uint context, Guid* iid, nint* result);
    ...
    [DllImport("user32.dll")] [return: MarshalAs(UnmanagedType.Bool)] private static extern bool GetPointerType(uint id, out uint type);
    [DllImport("user32.dll")] private static extern nuint SetTimer(nint hwnd, nuint id, uint interval, nint callback);
    [DllImport("user32.dll")] [return: MarshalAs(UnmanagedType.Bool)] private static extern bool KillTimer(nint hwnd, nuint id);
}
