...
public sealed record PlatformRasterSegment(int PaintOrder, IReadOnlyList<SceneCommand> Commands)
    : PlatformCompositionPart(PaintOrder);

public sealed record PlatformNativeSegment(PlatformViewPlacement Placement)
    : PlatformCompositionPart(Placement.PaintOrder);

public sealed record PlatformShieldSegment(PlatformInputShield Shield)
    : PlatformCompositionPart(Shield.PaintOrder);

/// <summary>Samples only preceding parts. Output clipping is separate from the expanded kernel input.</summary>
public abstract record PlatformEffectSegment(int PaintOrder, Rect Bounds)
    : PlatformCompositionPart(PaintOrder);

public sealed record PlatformBackdropSegment(
    int PaintOrder,
    Rect Bounds,
    double SigmaX,
    double SigmaY
) : PlatformEffectSegment(PaintOrder, Bounds)
{
    public PlatformEffectStyle? Style { get; init; }
    public Rect SampleBounds =>
        Rect.fromLTRB(
            Bounds.left - (3 * SigmaX),
            Bounds.top - (3 * SigmaY),
            Bounds.right + (3 * SigmaX),
            Bounds.bottom + (3 * SigmaY)
        );
}
...
public sealed class PlatformCompositionPlan : IDisposable
{
    ...
    internal PlatformCompositionPlan(
        PlatformCompositionToken token,
        IEnumerable<PlatformCompositionPart> parts,
        IDisposable[] leases
    )
    {
        Token = token;
    ...
    internal void Admit(IDisposable[] leases)
    {
        lock (_gate)
        {
            ...
            if (_admitted)
            {
                throw new InvalidOperationException("Frame was already admitted.");
            }
...
public static class PlatformCompositionPlanner
{
    ...
    public static PlatformCompositionPlan Build(
        Scene scene,
        PlatformCompositionToken token,
        PlatformViewCoordinator coordinator,
    ...
    )
    {
        ...
        if (scene.viewId != token.OwnerViewId)
        {
            throw new InvalidOperationException("Platform composition scene owner is invalid.");
        }
    ...
    public static PlatformCompositionPlan Analyze(
        IReadOnlyList<SceneCommand> commands,
        PlatformCompositionToken token,
        PlatformViewSnapshot snapshot,
        PlatformEffectSupport? effects = null
    )
    {
        ...
        if (
            snapshot.OwnerViewId != token.OwnerViewId
            || token.ViewEpoch < 0
            || token.FrameNumber < 0
            || token.SurfaceGeneration < 0
        )
        {
            throw new InvalidOperationException(
                "Platform composition token/scene owner is invalid."
            );
        }

        if (
            !double.IsFinite(token.DeviceScaleX)
            || !double.IsFinite(token.DeviceScaleY)
            || token.DeviceScaleX <= 0
            || token.DeviceScaleY <= 0
        )
        {
        ...
        if (nativeCount == 0 && !flattened.Any(command => command.Operation == "inputShield"))
        {
            return new(
                token,
                [new PlatformRasterSegment(0, Array.AsReadOnly(flattened.ToArray()))],
                []
            );
        }

        if (nativeCount > MaximumNativeViews)
        {
            throw Failure("native/overlay limit exceeded");
        }
        // Backdrop sampling may reach earlier native siblings even outside their scope.
        if (
            nativeCount != 0
            && flattened.Any(command => command.Operation == "backdropFilter")
            && (
                !allowNativeBackdrop
                || composition != PlatformViewComposition.InterleavedComposition
            )
        )
        {
        ...
        var state = new State(
            new(1 / token.DeviceScaleX, 0, 0, 1 / token.DeviceScaleY, 0, 0),
            null,
            null
        );
        ...
        foreach (var command in flattened)
        {
            ...
            {
                if (
                    command.HostPayload is not SceneBackdropFilterPayload backdrop
                    || state.Clip is not { } clip
                    || state.Unsupported is not null
                    || !state.Transform.IsAxisAligned
                    || backdrop.BlendMode != BlendMode.srcOver
                    || backdrop.BackdropId is not null
                    || backdrop.Filter
                        is not {
                            Outer: null,
                            Inner: null,
                    ...
                    || (
                        ...
                        && !(
                            filter.SigmaX == 0
                            && filter.SigmaY == 0
                            && filter.PlatformEffectIntent is { Saturation: not 1 }
                        )
                    )
                )
                {
                    throw Failure(
                        "native backdrop requires a clipped, ungrouped srcOver Gaussian blur with nonnegative finite sigma (zero requires a saturation effect)"
                    );
                }
                ...
                {
                    throw Failure(
                        "This native backdrop backend does not support saturation adjustment."
                    );
                }
                ...
                {
                    effects!.Validate(
                        filter.SigmaX * state.Transform.M11,
                ...
                parts.Add(
                    ...
                    {
                        Style = filter.PlatformEffectIntent,
                    }
                );
                ...
                var scope = new SceneCommand("offset", null)
                {
                    HostPayload = new SceneOffsetPayload(0, 0),
                };
                states.Push(state);
                scopes.Add(scope);
                ...
                state = state with
                {
                    Unsupported = "native views inside a backdrop scope are unsupported",
                };
                continue;
            }
            if (command.Operation == "platformView")
            {
                if (command.HostPayload is not ScenePlatformViewPayload native)
                {
                    throw Failure("untyped native payload");
                }

                if (
                    native.Handle.OwnerViewId != token.OwnerViewId
                    || !snapshot.Contains(native.Handle)
                )
                {
                ...
                {
                    throw Failure($"simultaneous multiple attachment of {native.Handle}");
                }

                if (state.Unsupported is not null)
                {
                ...
                {
                    if (!placement.Transform.IsAxisAligned)
                    {
                        throw Failure("NativeOverlay requires axis-aligned native bounds");
                    }
                    ...
                    if (!bounds.isEmpty)
                    {
                        precedingNativeBounds.Add(bounds);
                    }
                }
                raster.AddRange(scopes);
                continue;
            ...
            {
                if (command.HostPayload is not SceneInputShieldPayload shield)
                {
                    throw Failure("untyped input shield");
                }
                ...
                if (
                    state.Unsupported is not null
                    && state.Unsupported != "opacity across native content is unsupported"
                )
                {
                    throw Failure(state.Unsupported);
                }
                ...
                FlushRaster();
                parts.Add(
                    new PlatformShieldSegment(
                        new(shield.Bounds, state.Transform, state.Clip, parts.Count, shield.Debug)
                    )
                );
                raster.AddRange(scopes);
                continue;
            ...
            if (command.Operation == "pop")
            {
                if (states.Count == 0)
                {
                    throw Failure("unbalanced scene pop");
                }
            ...
            )
            {
                ...
                {
                    throw Failure(
                        "NativeOverlay cannot prove that foreground raster avoids earlier native content; interleaved composition or an explicit disjoint rectangular clip is required"
                    );
                }
            }
            raster.Add(command);
        ...
        if (states.Count != 0)
        {
            throw Failure("unclosed scene scopes");
        }
        ...
        if (composition == PlatformViewComposition.NativeOverlay)
        {
            ...
            {
                throw Failure(
                    "NativeOverlay cannot display foreground UI protected by an input shield; interleaved composition is required"
                );
            }
        ...
        void FlushRaster()
        {
            ...
            var commands = raster
                .Concat(scopes.Select(_ => new SceneCommand("pop", null)))
                .ToArray();
            parts.Add(new PlatformRasterSegment(parts.Count, Array.AsReadOnly(commands)));
            raster.Clear();
        }
        DorotiCapabilityException Failure(string reason) =>
            new(
    ...
    )
    {
        ...
        foreach (var command in commands)
        {
            ...
            {
                if (
                    command.HostPayload is not SceneRetainedPayload retained
                    || retained.ViewId != owner
    ...
    private static bool IsScope(string operation) =>
        operation
            is "offset"
                or "transform"
                or "clipRect"
    ...
    private static State Apply(State state, SceneCommand command)
    {
        switch (command.HostPayload)
        {
            case SceneOpacityPayload opacity:
                return state with
                {
                    Transform = state.Transform.ThenLocal(
                        new(1, 0, 0, 1, opacity.Offset.dx, opacity.Offset.dy)
                    ),
                    Unsupported =
                        state.Unsupported ?? "opacity across native content is unsupported",
                };
            case SceneOffsetPayload offset:
                return state with
                {
                    Transform = state.Transform.ThenLocal(new(1, 0, 0, 1, offset.Dx, offset.Dy)),
                };
            case SceneTransformPayload transform:
                var m = transform.Matrix4;
                if (
                    m.Count != 16
                    || m.Any(value => !double.IsFinite(value))
                    || m[2] != 0
                    || m[3] != 0
                    || m[6] != 0
                    || m[7] != 0
            ...
            case SceneClipRectPayload clip:
                ...
                if (clip.Behavior == Clip.antiAliasWithSaveLayer)
                {
                    return state with
                    {
                        Unsupported = "saveLayer clip across native content is unsupported",
                    };
                }

                if (!state.Transform.IsAxisAligned)
                {
                    return state with
                    {
                        Unsupported = "transformed non-rectangular clip is unsupported",
                    };
                }
                ...
                return state with
                {
                    Clip = state.Clip is { } parent ? parent.intersect(bounds) : bounds,
                };
            default:
                return state with
                {
                    Unsupported = $"{command.Operation} across native content is unsupported",
                };
        }
    }
}
...
{
    ValueTask<IPreparedPlatformComposition> PrepareAsync(
...
public sealed class PlatformCompositionSession(
    ulong ownerViewId,
    IPlatformCompositionPresenter presenter
) : IAsyncDisposable
{
    public PlatformCompositionSession(ulong ownerViewId)
        : this(ownerViewId, new RetiredFramePresenter()) { }
    ...
    {
        if (
            presenter is not RetiredFramePresenter inline
            || inline.Commit is not null
            || _gate.CurrentCount == 0
        )
        {
            plan.Dispose();
            throw new InvalidOperationException(
                "Retired frame commit cannot reenter or await the UI thread."
            );
        }
        try
        {
            ...
            {
                throw new InvalidOperationException("UI epoch update unexpectedly suspended.");
            }
            ...
            {
                throw new InvalidOperationException("Retired frame commit unexpectedly suspended.");
            }

            submission.GetAwaiter().GetResult();
            return true;
        }
        catch (RetiredFrameRejectedException)
        {
            return false;
        }
        finally
    ...
    private sealed class RetiredFramePresenter : IPlatformCompositionPresenter
    {
        ...
        )
        {
            ...
            return ValueTask.FromResult<IPreparedPlatformComposition>(
                new Prepared(
                    Commit ?? throw new InvalidOperationException("No native commit staged.")
                )
            );
        }
        ...
        private sealed class Prepared(Func<bool> commit) : IPreparedPlatformComposition
        {
            ...
            public ValueTask CommitAsync(CancellationToken cancellationToken)
            {
                ...
                if (!commit())
                {
                    throw new RetiredFrameRejectedException();
                }
    ...
    public async ValueTask SetEpochAsync(long epoch, long surfaceGeneration)
    {
        ...
        try
        {
            ...
            if (epoch < _epoch || surfaceGeneration < _surfaceGeneration)
            {
                throw new InvalidOperationException("Composition epoch cannot regress.");
            }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    "Composition plan requires lifetime admission."
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    "Composition retirement backlog exceeded three frames."
                );
            }

            var token = plan.Token;
            if (
                token.OwnerViewId != ownerViewId
                || token.ViewEpoch != _epoch
                || token.SurfaceGeneration != _surfaceGeneration
                || token.FrameNumber <= _frame
            )
            {
    ...
    private async Task RetireAsync(
        IPreparedPlatformComposition prepared,
        PlatformCompositionPlan plan
    )
    {
        ...
        try
        {
            await prepared.Retirement.ConfigureAwait(false);
        }
        finally
        {
            try
            ...
            {
                plan.Dispose();
                Interlocked.Decrement(ref _pendingRetirements);
            }
        }
    }
