...
public sealed class HapticFeedbackPlatformMessageCapability(
    ...
    Func<HapticFeedbackKind, CancellationToken, ValueTask> perform
) : IPlatformMessageHostCapability
{
    ...
    )
    {
        ...
        if (channel == "flutter/platform" && data is { } message)
        {
            ...
            if (
                ...
                && method.GetString() == "HapticFeedback.vibrate"
            )
            {
    ...
    private static HapticFeedbackKind? ParseKind(string? value) =>
        value switch
        {
