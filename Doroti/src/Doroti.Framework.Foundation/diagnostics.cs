...
public interface DiagnosticableTree : Diagnosticable
{
    ...
    new DiagnosticsNode toDiagnosticsNode(
        string? name = null,
...
public class DiagnosticsNode
{
    ...
    public virtual DartMap<string, string> toTimelineArguments() =>
        new() { ["name"] = name ?? string.Empty, ["description"] = toDescription() };

    public virtual DartMap<string, object?> toJsonMap(
        DiagnosticsSerializationDelegate? serializer = null
    )
    {
        ...
        if (serializer.includeProperties)
        {
            result["properties"] = getProperties()
                .Where(item => !item.isFiltered(serializer.minLevel))
                .Select(item => item.toJsonMap(serializer))
                .ToArray();
        }
        if (serializer.subtreeDepth > 0)
        {
            result["children"] = getChildren()
                .Where(item => !item.isFiltered(serializer.minLevel))
                .Select(item =>
                    item.toJsonMap(serializer.copyWith(subtreeDepth: serializer.subtreeDepth - 1))
                )
                .ToArray();
        }
        return result;
    ...
    public virtual DartMap<string, object?> toJsonMapIterative(
        DiagnosticsSerializationDelegate? serializer = null
    ...
    where T : class, Diagnosticable
{
    public DiagnosticableNode(string? name, T value, DiagnosticsTreeStyle? style = null)
        : base(
            ...
            description: value.toStringShort,
...
public class DiagnosticsProperty<T> : DiagnosticsNode
{
    ...
    public DiagnosticsProperty(
        ...
        : base(
            ...
            showName,
            showSeparator,
            description: value is null && ifNull is not null ? () => ifNull
    ...
    public T? propertyValue =>
        _computeValue is null
            ? base.value is T typed
                ? typed
                : default
            : _computeValue();
    ...
    public override string toDescription()
    {
        ...
        return currentValue is null && _ifNullDescription is not null
            ? _ifNullDescription
            : _valueDescription ?? currentValue?.ToString() ?? "null";
    }

    public override bool isFiltered(DiagnosticLevel minLevel) =>
        base.isFiltered(minLevel)
...
public sealed class PercentProperty : DiagnosticsProperty<double?>
{
    ...
    public override string toDescription() =>
        propertyValue is { } value ? $"{value * 100:0.0}{unit ?? "%"}" : "null";
}
    ...
    where T : struct, Enum
{
    public EnumProperty(
        ...
        : base(name, value, level: level, defaultValue: defaultValue) { }
}
...
public sealed class ObjectFlagProperty<T> : DiagnosticsProperty<T>
{
    public ObjectFlagProperty(
        string? name,
        T? value,
        string? ifPresent = "present",
        DiagnosticLevel level = DiagnosticLevel.info,
        string? ifNull = null
    )
        : base(name, value, level: level, description: value is null ? ifNull : null) =>
        this.ifPresent = ifPresent;

    public string? ifPresent { get; }

    public override string toDescription() =>
        propertyValue is null || ifPresent is null ? base.toDescription() : ifPresent;
...
public sealed class IterableProperty<T> : DiagnosticsProperty<IEnumerable<T>>
{
    ...
    public override string toDescription() =>
        ...
        : propertyValue.Any() ? $"[{string.Join(", ", propertyValue)}]"
        : _ifEmpty ?? "[]";
}

public sealed class MessageProperty : DiagnosticsNode
{
    public MessageProperty(string message, DiagnosticLevel level = DiagnosticLevel.info)
        : base(null, message, level: level, showName: false) { }
...
public class DiagnosticsBlock : DiagnosticsNode
{
    ...
    public DiagnosticsBlock(
        ...
        DiagnosticsTreeStyle style = DiagnosticsTreeStyle.whitespace,
        bool allowTruncate = false,
        object? propertiesCompatibility = null
    )
...
public class DiagnosticsSerializationDelegate
{
    ...
    public DiagnosticsSerializationDelegate copyWith(
        int? subtreeDepth = null,
        bool? includeProperties = null,
        DiagnosticLevel? minLevel = null
    ) =>
        new(
    ...
    public virtual List<DiagnosticsNode> truncateNodesList(
        List<DiagnosticsNode> nodes,
...
public sealed class TextTreeRenderer
{
    ...
    )
    {
        ...
        var children = node.getProperties()
            .Concat(node.getChildren())
            .Where(item => !item.isFiltered(minLevel))
            .ToArray();
...
internal sealed class _DefaultDiagnosticsSerializationDelegate
    : DiagnosticsSerializationDelegate { }

internal sealed class _JsonDiagnosticsNode(string? name, object? value)
    : DiagnosticsNode(name, value);

internal sealed class _NoDefaultValue
{
    public static _NoDefaultValue instance { get; } = new();

    private _NoDefaultValue() { }
}

internal sealed class _NodesToJsonifyStack : Stack<DiagnosticsNode>;

internal sealed class _NumProperty(string? name, double? value)
    : DiagnosticsProperty<double?>(name, value);
...
internal enum _DiagnosticsWordWrapParseMode
{
    inSpace,
    inWord,
    atBreak,
...
public static class DiagnosticsLibrary
{
    ...
    public static TextTreeConfiguration dashedTextConfiguration { get; } =
        new(prefixLastChildLineOne: "└╌");
    ...
    public static TextTreeConfiguration errorTextConfiguration { get; } =
        new(prefixLastChildLineOne: "└─");
    public static TextTreeConfiguration whitespaceTextConfiguration { get; } =
        new(prefixLastChildLineOne: "  ");
    ...
    public static string shortHash(object? value) =>
        ((value?.GetHashCode() ?? 0) & 0xFFFFF).ToString("x5", CultureInfo.InvariantCulture);

    public static string describeIdentity(object? value) =>
        $"{DartRuntimePrimitives.RuntimeTypeName(value)}#{shortHash(value)}";

    internal static bool _isSingleLine(string value) =>
