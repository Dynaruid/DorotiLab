...
public abstract class BindingBase : IDisposable
{
    ...
    private readonly Dictionary<string, ServiceExtensionCallback> _serviceExtensions = new(
    ...
    protected BindingBase(PlatformDispatcher? platformDispatcher = null)
    {
        ...
        using var initialization = ConstantsLibrary.kReleaseMode
            ? null
    ...
    public void registerBoolServiceExtension(
        string name,
    ...
    )
    {
        registerServiceExtension(
            ...
            async parameters =>
            {
                ...
                {
                    await setter(string.Equals(raw, "true", StringComparison.OrdinalIgnoreCase))
                        .ConfigureAwait(false);
                }
                return new Dictionary<string, object?>
                {
                    ["enabled"] = (await getter().ConfigureAwait(false))
                        .ToString()
                        .ToLowerInvariant(),
                };
            }
    ...
    public void registerNumericServiceExtension(
        string name,
    ...
    )
    {
        registerServiceExtension(
            ...
            async parameters =>
            {
                ...
                return new Dictionary<string, object?>
                {
                    [name] = await getter().ConfigureAwait(false),
                };
            }
        );
    }
    ...
    public void registerStringServiceExtension(
        string name,
    ...
    public void registerServiceExtension(string name, ServiceExtensionCallback callback)
    {
        ...
        if (!_serviceExtensions.TryAdd(name, callback))
        {
            throw new InvalidOperationException(
                $"Service extension '{name}' is already registered."
            );
        }
    }
    ...
    public async Task<IReadOnlyDictionary<string, object?>> invokeServiceExtension(
        string name,
    ...
    )
    {
        ...
        var result = await callback(parameters ?? new Dictionary<string, string>())
            .ConfigureAwait(false);
        result["method"] = name;
...
{
    internal static Task _exitApplication(int exitCode = 0) =>
        Task.FromException(
