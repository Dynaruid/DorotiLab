...
public static class ConsolidateResponseLibrary
{
    ...
    )
    {
        ...
        await foreach (
            var chunk in response.WithCancellation(cancellationToken).ConfigureAwait(false)
        )
        {
        ...
        if (contentLength is >= 0 && output.length != contentLength.Value)
        {
            throw new InvalidDataException(
                $"Content size below specified contentLength. Expected {contentLength}, but received {output.length} bytes."
            );
        }
