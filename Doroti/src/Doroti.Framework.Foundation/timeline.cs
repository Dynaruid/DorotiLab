...
{
    private readonly Dictionary<string, (int Count, TimeSpan Total)> _values = new(
        StringComparer.Ordinal
    );

    public IReadOnlyList<AggregatedTimedBlock> blocks =>
        _values
            .OrderBy(item => item.Key, StringComparer.Ordinal)
            .Select(item => new AggregatedTimedBlock(item.Key, item.Value.Count, item.Value.Total))
            .ToArray();
...
{
    public static TimedBlock startSync(
        string name,
    ...
    public static Task<T> timeSync<T>(
        string name,
    ...
    private static async Task<T> TimeAsync<T>(
        string name,
        Func<Task<T>> function,
...
internal static class TimelineLibrary
{
    internal const int _kSliceSize = 500;
}
