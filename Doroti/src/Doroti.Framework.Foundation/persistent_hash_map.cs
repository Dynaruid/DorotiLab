    ...
    where TKey : notnull
{
    ...
    private PersistentHashMap(PersistentHashTrie<TKey, TValue>? root, int count)
    {
        _root = root;
        Count = count;
    }
    ...
    {
        using var profile = Ui.FrameworkComponentProfile.Begin(
            Ui.FrameworkComponentProfile.Kind.InheritancePut,
            Count
        );
        var hash = Hash(key);
        if (_root is null)
        {
            return new(new PersistentHashTrie<TKey, TValue>.Leaf(hash, [new(key, value)]), 1);
        }
    ...
    {
        using var profile = Ui.FrameworkComponentProfile.Begin(
            Ui.FrameworkComponentProfile.Kind.InheritanceRemove,
            Count
        );
        var hash = Hash(key);
        var removed = false;
    ...
    public bool TryGetValue(TKey key, out TValue value)
    {
        ...
        {
            return _root.Find(key, hash, 0, out value!);
        }
