...
internal abstract class PersistentHashTrie<TKey, TValue>
    where TKey : notnull
{
    internal abstract bool Find(TKey key, uint hash, int shift, out TValue value);
    internal abstract PersistentHashTrie<TKey, TValue> Put(
        TKey key,
        TValue value,
        uint hash,
        int shift,
    ...
    internal abstract PersistentHashTrie<TKey, TValue>? Remove(
        TKey key,
        uint hash,
        int shift,
        ref bool removed
    ...
    internal sealed class Leaf(uint hash, KeyValuePair<TKey, TValue>[] entries)
        : PersistentHashTrie<TKey, TValue>
    {
        ...
        internal override PersistentHashTrie<TKey, TValue> Put(
            TKey key,
            TValue value,
            uint hash,
            int shift,
        ...
        internal override PersistentHashTrie<TKey, TValue>? Remove(
            TKey key,
            uint hash,
            int shift,
            ref bool removed
    ...
    private sealed class Branch(uint bitmap, PersistentHashTrie<TKey, TValue>[] children)
        : PersistentHashTrie<TKey, TValue>
    {
        internal override bool Find(TKey key, uint hash, int shift, out TValue value)
        {
            ...
            if ((bitmap & bit) != 0)
            {
                return children[Index(bitmap, bit)].Find(key, hash, shift + 5, out value);
            }
        ...
        )
        {
            ...
            {
                if (children.Length == 1 && child is Leaf)
                {
                    return child;
                }
            ...
            if (remaining.Length == 1 && remaining[0] is Leaf)
            {
                return remaining[0];
            }
        ...
        internal override IEnumerable<KeyValuePair<TKey, TValue>> Entries()
        {
            ...
            {
                foreach (var pair in child.Entries())
                {
                    yield return pair;
                }
            }
    ...
    private static PersistentHashTrie<TKey, TValue> Merge(Leaf left, Leaf right, int shift)
    {
        ...
        if (a == b)
        {
            return new Branch(a, [Merge(left, right, shift + 5)]);
        }
