...
internal static class AndroidGraphiteApkAsset
{
    ...
    {
        var paths = new[] { baseApkPath }
            .Concat(splitApkPaths ?? [])
            ...
            foreach (var entry in apk.Entries.Where(entry => entry.FullName == entryName))
            {
                if (containingApk is not null)
                {
                    throw new InvalidDataException(
                        "Installed APKs contain more than one Skia library for this ABI."
                    );
                }
                ...
                {
                    throw new InvalidDataException(
                        "APK native Skia differs from the pinned official package: " + path
                    );
                }
        ...
        return containingApk
            ?? throw new InvalidDataException(
                "Installed APKs must contain exactly one official Skia library for this ABI."
            );
    }

    internal static bool IsVerifiedLoadPath(
        string loadedPath,
        string containingApk,
        string abi,
        string? verifiedExtractedPath
    ) =>
        loadedPath == verifiedExtractedPath
        || loadedPath == containingApk + "!/lib/" + abi + "/libSkiaSharp.so";
}
