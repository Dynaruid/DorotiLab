...
internal sealed class SubmissionJournal
{
    ...
    {
        if (Images.Remove(image, out var state))
        {
            state.Alive = false;
        }
    }
    ...
    public void Allocate(nint handle, ulong pool)
    {
        ...
        {
            throw new InvalidOperationException("Live command handle reused.");
        }
    ...
    {
        if (Buffers.Remove(handle, out var b))
        {
            b.Alive = false;
            b.Operations.Clear();
        }
    ...
    {
        foreach (var b in Buffers.Values.Where(b => b.Pool == pool))
        {
            Begin(b.Handle);
        }
    }

    public void FreePool(ulong pool)
    {
        foreach (var b in Buffers.Values.Where(b => b.Pool == pool).ToArray())
        {
            Free(b.Handle);
        }
    }
    ...
    {
        if (!Images.TryGetValue(image, out var state))
        {
            return; // Only registered host targets are exported.
        }
    ...
    public void Secondary(nint command, nint child)
    {
        ...
        if (!c.Executable)
        {
            throw new InvalidOperationException("Secondary is not executable.");
        }
    ...
    private void Add(nint command, Operation operation)
    {
        ...
        if (b.Executable)
        {
            throw new InvalidOperationException("Recording after EndCommandBuffer.");
        }

        if (b.Operations.Count >= 65536)
        {
            throw new NotSupportedException("Command journal limit exceeded.");
        }

        b.Operations.Add(operation);
        PeakOperations = Math.Max(PeakOperations, b.Operations.Count);
    }
        ...
        void Replay(Commands commands, int revision)
        {
            ...
            )
            {
                throw new InvalidOperationException(
                    "Stale, reset, cyclic or non-executable command journal."
                );
            }

            foreach (var op in commands.Operations)
            {
                if (op is Execute execute)
                {
                    Replay(execute.Child, execute.Revision);
                    continue;
                }
                var t = (Transition)op;
                if (
                    !t.Image.Alive
                    || !Images.TryGetValue(t.Image.Handle, out var live)
                    || !ReferenceEquals(live, t.Image)
                )
                {
                    throw new InvalidOperationException(
                        "Stale image generation in submitted command."
                    );
                }
                ...
                {
                    throw new InvalidOperationException(
                        $"Image {t.Image.Handle:x}/{t.Image.Epoch}: expected {t.Old}, scheduled {current.Layout}."
                    );
                }

                if (
                    t.SourceFamily == Vk.QueueFamilyIgnored
                    != (t.DestinationFamily == Vk.QueueFamilyIgnored)
                )
                {
                ...
                )
                {
                    throw new NotSupportedException(
                        "Host Graphite target must stay in its single queue family."
                    );
                }
