...
public sealed unsafe partial class GraphiteVulkanWindow : IDisposable
{
    ...
    public object ContextIdentity =>
        _session ?? throw new InvalidOperationException("No Vulkan session.");
    ...
    public static GraphiteVulkanWindow CreateAndroid(nint nativeWindow)
    {
        if (!GraphiteNativeLibrary.IsAssetSelected)
        {
            throw new InvalidOperationException(
                "Configure the official APK asset before creating an Android surface."
            );
        }
        ...
        try
        {
            ...
            var create = (delegate* unmanaged[Cdecl]<
                nint,
                AndroidSurfaceInfo*,
                void*,
                ulong*,
            ...
            if (create == null)
            {
                throw new PlatformNotSupportedException(
                    "Vulkan Android surface extension is unavailable."
                );
            }
        ...
        catch
        {
            if (surface != 0 && vk.TryGetInstanceExtension<KhrSurface>(instance, out var surfaces))
            {
                using (surfaces)
                {
                    surfaces.DestroySurface(instance, new(surface), null);
                }
            }
    ...
    public static GraphiteVulkanWindow FromQt(
        nint instance,
        ulong surface,
        string[] enabledInstanceExtensions,
        uint instanceApiVersion
    )
    {
        if (instanceApiVersion < Api12)
        {
            throw new PlatformNotSupportedException(
                "Qt must create and report a real Vulkan 1.2 instance."
            );
        }
    ...
    private GraphiteVulkanWindow(
        ...
        bool pipelinedWindowFrames = false
    )
    {
        _vk = vk;
        _instance = instance;
        ...
        try
        {
            ...
            foreach (var candidate in devices)
            {
                ...
                if (properties.ApiVersion < Api12)
                {
                    continue;
                }
                ...
                if (adapterLuid is { } requiredLuid)
                {
                    ...
                    if (!identity.DeviceLuidvalid || *(long*)identity.DeviceLuid != requiredLuid)
                    {
                        continue;
                    }
                }
                ...
                for (uint i = 0; i < families; i++)
                {
                    ...
                    {
                        Check(
                            _surfaces.GetPhysicalDeviceSurfaceSupport(
                                candidate,
                                i,
                                surface,
                                out var present
                    ...
                    _family = i;
                    DeviceName =
                        Marshal.PtrToStringUTF8((nint)properties.DeviceName) ?? "Vulkan GPU";
                    IsSoftwareDevice = properties.DeviceType == PhysicalDeviceType.Cpu;
                    if (OperatingSystem.IsLinux())
                    {
                        Console.WriteLine(
                            $"DorotiGraphite Vulkan device={DeviceName} type={properties.DeviceType}"
                        );
                    }
            ...
            var extension =
                surface.Handle != 0 ? "VK_KHR_swapchain" : "VK_KHR_external_memory_win32";
                    ...
                    || (
                        ...
                        && name
                            is "VK_ANDROID_external_memory_android_hardware_buffer"
                                or "VK_EXT_queue_family_foreign"
                    )
                )
                {
            ...
            SupportsHardwareBuffer =
                extensionNames.Contains("VK_ANDROID_external_memory_android_hardware_buffer")
            ...
            {
                foreach (var value in enabledExtensions)
                {
            ...
            if (
                surface.Handle != 0
                && !_vk.TryGetDeviceExtension(_instance, _device, out _swapchains)
            )
            {
                throw new PlatformNotSupportedException("VK_KHR_swapchain is required.");
            }

            _stockObserver = new VulkanObserver(
                _vk,
                _instance,
                _device,
                _queue,
            ...
            )
            {
                ...
                Console.WriteLine(
                    "DorotiGraphite Vulkan API ceiling=1.1: device RenderPass2 unavailable"
                );
            }
            _session = SkiaGraphiteSession.CreateVulkan(
                new(
                    ...
                    _family,
                    graphiteApiVersion,
                    _stockObserver.Resolve,
                    image =>
                    {
                        var state = _stockObserver.State((ulong)image);
                        return ((int)state.Layout, state.Family);
                    },
    ...
    public VulkanSharedRaster CreateHardwareBufferRaster(nint buffer, int width, int height)
    {
        ...
        {
            throw new PlatformNotSupportedException(
                "An active frame with Vulkan hardware-buffer sharing is required."
            );
        }

        var raster = new VulkanSharedRaster(
            _vk,
            _instance,
            _physical,
            _device,
    ...
    public Task<SkiaGraphiteReadback> RequestPlatformReadback(SKSurface surface, SKImageInfo info)
    {
        ...
        var frame =
            _platformRecording
            ?? throw new InvalidOperationException(
                "Platform readback requires an active paint callback."
            );
    ...
    )
    {
        ...
        if (_recreate || Width != width || Height != height)
        {
            Resize(width, height);
        }
        ...
        uint index = 0;
        var acquired = _swapchains.AcquireNextImage(
            _device,
            _swapchain,
            _pipelinedWindowFrames ? 0 : Timeout,
            slot.Acquired,
            default,
            &index
        );
        if (acquired is Result.NotReady or Result.Timeout)
        {
            UnavailableWindowImages++;
            return false;
        }
        ...
        if (acquired != Result.SuboptimalKhr)
        {
            CheckDevice(acquired, "acquire image");
        }
        ...
        try
        {
            ...
            Barrier(
                command,
                _images[index],
                _initialized[index] ? ImageLayout.PresentSrcKhr : ImageLayout.Undefined,
            ...
            var restoreLayout = _stockObserver is null
            ...
            MaximumWindowFramesInFlight = Math.Max(
            ...
            {
                CheckDevice(result, "vkQueuePresentKHR");
            }

            _nextWindowFrame = (_nextWindowFrame + 1) % _windowFrames.Count;
            var presentedTime = FrameTimestamp();
            LastFrameTiming = new(
                Milliseconds(startTime, acquiredTime),
                Milliseconds(acquiredTime, paintedTime),
                Milliseconds(paintedTime, submittedTime),
                Milliseconds(submittedTime, copiedTime),
                Milliseconds(copiedTime, completedTime),
                Milliseconds(completedTime, presentedTime),
                Milliseconds(startTime, presentedTime)
            );
        ...
        finally
        {
            ...
            if (submitted && !sharedCompleted && _platformShared.Count != 0)
            {
                ...
                {
                    Check(idle, "failed shared raster drain");
                }
            }
            foreach (var raster in _platformShared)
            {
    ...
    private void Resize(int width, int height)
    {
        ...
        Check(
            _surfaces.GetPhysicalDeviceSurfaceCapabilities(_physical, _surface, out var caps),
        ...
        var preTransform = OperatingSystem.IsAndroid()
            ? SurfaceTransformFlagsKHR.IdentityBitKhr
            : caps.CurrentTransform;
        if ((caps.SupportedTransforms & preTransform) == 0)
        {
            throw new PlatformNotSupportedException(
                "The surface does not support the renderer's orientation transform."
            );
        }

        uint count = 0;
        Check(
            _surfaces.GetPhysicalDeviceSurfaceFormats(_physical, _surface, &count, null),
            "format count"
        );
        ...
        {
            Check(
                _surfaces.GetPhysicalDeviceSurfaceFormats(_physical, _surface, &count, values),
                "formats"
            );
        }
        ...
        {
            selected = formats.FirstOrDefault(value =>
        ...
        if (
            selected.Format == Format.Undefined
            || (caps.SupportedUsageFlags & ImageUsageFlags.TransferDstBit) == 0
        )
        {
            throw new PlatformNotSupportedException(
                "Vulkan output requires an RGBA/BGRA UNORM transfer-destination surface."
            );
        }
        ...
        var extent =
            OperatingSystem.IsAndroid() || caps.CurrentExtent.Width == uint.MaxValue
                ? new Extent2D(
        ...
        {
            images = Math.Min(images, caps.MaxImageCount);
        }
        ...
        try
        {
            ...
            Check(
                _swapchains.CreateSwapchain(_device, &info, null, out var replacement),
                "create swapchain"
            );
        ...
        finally
        {
            ...
            {
                _swapchains.DestroySwapchain(_device, oldSwapchain, null);
            }
        }
        ...
        {
            Check(
                _swapchains.GetSwapchainImages(_device, _swapchain, &count, values),
                "swapchain images"
            );
        }
        ...
        {
            Check(
                _vk.CreateSemaphore(_device, &semaphore, null, out _presentReady[i]),
                "present semaphore"
            );
        }
        ...
        {
            CreateWindowBacking(slot, extent);
        }
        ...
        {
            Console.WriteLine(
                $"DorotiGraphite swapchain generation={Generation} requested={width}x{height} extent={Width}x{Height} currentTransform={caps.CurrentTransform} preTransform={preTransform}"
            );
        }
    }
    ...
    private void CreateWindowBacking(WindowFrameSlot slot, Extent2D extent)
    {
        ...
        if (requirements.Size > 512UL * 1024 * 1024 / (ulong)_windowFrames.Count)
        {
            throw new InvalidOperationException(
                "Vulkan surfaces exceed the total 512 MiB backing budget."
            );
        }
        ...
        for (uint i = 0; i < memory.MemoryTypeCount; i++)
        {
            if (
                ...
                && (memory.MemoryTypes[(int)i].PropertyFlags & MemoryPropertyFlags.DeviceLocalBit)
                    != 0
            )
            {
                memoryType = i;
        ...
        {
            throw new PlatformNotSupportedException("No device-local Vulkan image memory.");
        }
        ...
        slot.Target = _session!.CreateVulkanTarget(
            Width,
            Height,
    ...
    )
    {
        ...
        _stockObserver?.Journal.Barrier(
            command.Handle,
            image.Handle,
            oldLayout,
            newLayout,
    ...
    private nint GetProcedure(string name, nint instance, nint device) =>
        device != 0
            ? _vk.GetDeviceProcAddr(new(device), name)
            : _vk.GetInstanceProcAddr(new(instance), name);
        ...
        try
        {
            ...
            fixed (nint* extensions = strings)
            {
                var info = new InstanceCreateInfo
                {
                    ...
                    EnabledExtensionCount = (uint)strings.Length,
    ...
    private void ReleaseImages(bool releaseSwapchain = true)
    {
        ...
        foreach (var slot in _windowFrames)
        {
            slot.Target?.Dispose();
            slot.Target = null;
            if (slot.Backing.Handle != 0)
            {
                _stockObserver?.ForgetHostTarget(slot.Backing.Handle);
            }

            if (slot.Backing.Handle != 0)
            {
                _vk.DestroyImage(_device, slot.Backing, null);
            }
            ...
            slot.Backing = default;
            slot.Memory = default;
        }
        _target?.Dispose();
        _target = null;
        ReleaseIntermediateTarget();
        if (_backing.Handle != 0)
        {
            _vk.DestroyImage(_device, _backing, null);
        }
    ...
    private void ReleaseDevice()
    {
        ...
        foreach (var slot in _windowFrames)
        {
            ...
            if (slot.Acquired.Handle != 0)
            {
                _vk.DestroySemaphore(_device, slot.Acquired, null);
            }
        }
        _windowFrames.Clear();
        if (_fence.Handle != 0)
    ...
    public void Dispose()
    {
        ...
        if (_ownsSurface)
        {
            _surfaces.DestroySurface(_instance, _surface, null);
        }
    ...
    public Task DisposeAfterOwnerDetachedAsync()
    {
        ...
        if (_terminalShutdown)
        {
            throw new InvalidOperationException("Vulkan surface is already retiring.");
        }
    ...
    public void TakeShutdownOwnershipAfterThreadJoined()
    {
        ...
        {
            Check(result, "terminal owner transfer drain");
        }
    ...
    private void CheckOwner()
    {
        ...
        {
            throw new InvalidOperationException("Vulkan WSI accessed outside its owner thread.");
        }
    }
    ...
    private struct AndroidSurfaceInfo
    {
        public uint Type;
        public nint Next;
        public uint Flags;
