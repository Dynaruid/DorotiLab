...
public sealed unsafe class VulkanSharedRaster : IDisposable
{
    ...
    private VkImage _render,
        _output;
    private DeviceMemory _renderMemory,
        _outputMemory;
    ...
    )
    {
        ...
        try
        {
            ...
            _target = session.CreateVulkanTarget(
                width,
                height,
            ...
            var external = new ExternalMemoryImageCreateInfo
            {
                ...
                HandleTypes = android
                    ? ExternalMemoryHandleTypeFlags.AndroidHardwareBufferBitAndroid
            ...
            if (android)
            {
                ...
                var get = (delegate* unmanaged[Cdecl]<
                    ...
                    Result>)
                    (nint)
                        vk.GetDeviceProcAddr(device, "vkGetAndroidHardwareBufferPropertiesANDROID");
                if (get == null)
                {
                    throw new PlatformNotSupportedException(
                        "Vulkan AHardwareBuffer import is unavailable."
                    );
                }
            ...
            else
            {
                ...
                if (
                    !vk.TryGetDeviceExtension<KhrExternalMemoryWin32>(instance, device, out var api)
                )
                {
                    throw new PlatformNotSupportedException(
                        "Vulkan Win32 memory import is unavailable."
                    );
                }
                ...
                using (api)
                {
                    ...
                    Check(
                        api.GetMemoryWin32HandleProperties(
                            device,
                    ...
                    var import = new ImportMemoryWin32HandleInfoKHR
                    {
                        ...
                        PNext = &dedicated,
                        HandleType = external.HandleTypes,
                        Handle = sharedHandle,
                    };
                    Allocate(
                        physical,
    ...
    )
    {
        ...
        {
            throw new NotSupportedException("Shared raster allocation exceeds 256 MiB.");
        }
        ...
        {
            throw new PlatformNotSupportedException("No compatible shared raster memory.");
        }
    ...
    public void RecordCopy(CommandBuffer command)
    {
        if (_disposed || _copied || Environment.CurrentManagedThreadId != _owner)
        {
            throw new InvalidOperationException(
                "Shared raster must be published exactly once on its GPU owner."
            );
        }
    ...
    private static ImageMemoryBarrier Transition(
        ...
        new()
        {
            ...
            OldLayout = from,
            NewLayout = to,
            SrcQueueFamilyIndex = source,
            DstQueueFamilyIndex = destination,
            SrcAccessMask = read,
            DstAccessMask = write,
    ...
    public void Dispose()
    {
        ...
        if (_render.Handle != 0)
        {
            _observer.ForgetHostTarget(_render.Handle);
            _vk.DestroyImage(_device, _render, null);
        }
        if (_output.Handle != 0)
        {
            _vk.DestroyImage(_device, _output, null);
        }

        if (_renderMemory.Handle != 0)
        {
            _vk.FreeMemory(_device, _renderMemory, null);
        }

        if (_outputMemory.Handle != 0)
        {
            _vk.FreeMemory(_device, _outputMemory, null);
        }
