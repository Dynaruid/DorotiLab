...
public sealed unsafe partial class GraphiteVulkanWindow
{
    ...
    private void CreateIntermediateTarget(int width, int height, ImageUsageFlags usage)
    {
        ...
        Check(_vk.CreateImage(_device, &info, null, out _intermediateTarget), "official private image");
        ...
        Check(_vk.AllocateMemory(_device, &allocate, null, out _intermediateTargetMemory), "official private allocation");
        Check(_vk.BindImageMemory(_device, _intermediateTarget, _intermediateTargetMemory, 0), "official private bind");
        ...
        _target = _session!.CreateVulkanTarget(width, height, new SKGraphiteVkTextureInfo {
            ...
            (int)ImageLayout.Undefined, _family, (nint)_intermediateTarget.Handle, SKColorType.Rgba8888);
    }
    private void ReleaseIntermediateTarget()
    {
        if (_intermediateTarget.Handle != 0)
        {
            ...
            _vk.DestroyImage(_device, _intermediateTarget, null); _intermediateTarget = default;
        }
        if (_intermediateTargetMemory.Handle != 0)
        { _vk.FreeMemory(_device, _intermediateTargetMemory, null); _intermediateTargetMemory = default; }
    }
    private void CopyIntermediateTargetToD3D12()
    {
        ...
        transitions[0] = Transition(_intermediateTarget, (ImageLayout)state.Layout, ImageLayout.TransferSrcOptimal, uint.MaxValue, uint.MaxValue,
        ...
        _vk.CmdCopyImage(_command, _intermediateTarget, ImageLayout.TransferSrcOptimal, _backing, ImageLayout.TransferDstOptimal, 1, &copy);
        transitions[0] = Transition(_intermediateTarget, ImageLayout.TransferSrcOptimal, (ImageLayout)state.Layout, uint.MaxValue, uint.MaxValue,
