...
public sealed unsafe class GraphiteVulkanQuick : IDisposable
{
    ...
    {
        internal VkImage R, P;
        internal DeviceMemory RMemory, PMemory;
    ...
    public GraphiteVulkanQuick(nint instance, nint physical, nint device, nint queue, uint family, uint apiVersion)
    {
        ...
        if (apiVersion < ((1u << 22) | (2u << 12))) throw new NotSupportedException("Qt Quick requires Vulkan 1.2.");
        _device = new(device); _physical = new(physical); _queue = new(queue); _family = family;
        try
        {
            _vk.GetPhysicalDeviceProperties(_physical, out var properties);
            if (properties.ApiVersion < ((1u << 22) | (2u << 12))) throw new NotSupportedException("Qt selected a device below Vulkan 1.2.");
            IsSoftwareDevice = properties.DeviceType == PhysicalDeviceType.Cpu;
            _observer = new(_vk, new(instance), _device, _queue, family);
            _session = SkiaGraphiteSession.CreateVulkan(new(instance, physical, device, queue, family, apiVersion,
                _observer.Resolve, image => { var s = _observer.State((ulong)image); return ((int)s.Layout,s.Family); }, _observer.Check), 1, 1);
            var pool = new CommandPoolCreateInfo { SType = StructureType.CommandPoolCreateInfo,
            ...
            Check(_vk.AllocateCommandBuffers(_device, &allocation, out _command), "Quick copy command");
        ...
        {
            if (_session is not null) Dispose();
            else { _observer?.Dispose(); _vk.Dispose(); }
            throw;
        }
    }
    ...
    {
        Verify();
        if (_frame is not null) throw new InvalidOperationException("Unretired Quick frame.");
        if (width < 1 || height < 1 || width > 16384 || height > 16384) throw new NotSupportedException("Quick raster extent exceeds limits.");
    ...
    public void Cancel()
    {
        ...
        _frame = null; _submitted = false;
    }
    public void Complete()
    {
        Verify();
        if (_frame is null) throw new InvalidOperationException("No Quick recording.");
        _submitted = true;
        _frame.Submit();
        var reset = _observer.Call<VulkanObserver.ResetCommandBufferDelegate>("vkResetCommandBuffer");
        ...
        Check(_observer.Call<VulkanObserver.BeginCommandBufferDelegate>("vkBeginCommandBuffer")(_command,&begin), "Quick copy begin");
        ...
        for (int i=0;i<_used;i++)
        {
            ...
            transitions[1]=Transition(layer.P,layer.Initialized?ImageLayout.ShaderReadOnlyOptimal:ImageLayout.Undefined,
            ...
            transitions[1]=Transition(layer.P,ImageLayout.TransferDstOptimal,ImageLayout.ShaderReadOnlyOptimal,
                AccessFlags.TransferWriteBit,AccessFlags.ShaderReadBit);
            barrier(_command,PipelineStageFlags.AllCommandsBit,PipelineStageFlags.AllCommandsBit,0,0,null,0,null,2,transitions);
        }
        Check(_observer.Call<VulkanObserver.EndCommandBufferDelegate>("vkEndCommandBuffer")(_command),"Quick copy end");
        ...
        Check(_observer.Call<VulkanObserver.QueueSubmitDelegate>("vkQueueSubmit")(_queue,1,&submit,_fence),"Quick copy submit");
    ...
    private static ImageMemoryBarrier Transition(VkImage image, ImageLayout from, ImageLayout to, AccessFlags source, AccessFlags destination) =>
    ...
    private Layer CreateLayer(int width, int height)
    {
        ...
        catch { Destroy(layer); throw; }
    }
    private void Allocate(ImageCreateInfo info,out VkImage image,out DeviceMemory memory,Layer layer)
    {
        memory=default;
        ...
        if(requirements.Size>Budget || _bytes>Budget-requirements.Size) throw new NotSupportedException("Quick R/P and retiring images exceed the 128 MiB budget.");
        ...
        if(type==uint.MaxValue) throw new NotSupportedException("No device-local Quick image memory.");
    ...
    private void Destroy(Layer layer)
    {
        ...
        foreach(var image in new[]{layer.R,layer.P}) if(image.Handle!=0) { _observer.ForgetHostTarget(image.Handle);_vk.DestroyImage(_device,image,null); }
        if(layer.RMemory.Handle!=0) _vk.FreeMemory(_device,layer.RMemory,null);
        if(layer.PMemory.Handle!=0) _vk.FreeMemory(_device,layer.PMemory,null);
        _bytes-=layer.Bytes;
    }
    ...
    private void Verify() { ObjectDisposedException.ThrowIf(_disposed,this);if(Environment.CurrentManagedThreadId!=_owner) throw new InvalidOperationException("Quick GPU owner thread mismatch."); }
    public void Dispose()
    {
        if(_disposed)return;Verify();
        Check(_vk.QueueWaitIdle(_queue),"Quick terminal GPU drain");Cancel();
        ResourcesReleasing?.Invoke();
        foreach(var layer in _layers.Concat(_retired))Destroy(layer);
        _layers.Clear();_retired.Clear();_session.Dispose();
        if(_fence.Handle!=0)_vk.DestroyFence(_device,_fence,null);
        if(_pool.Handle!=0)_vk.DestroyCommandPool(_device,_pool,null);
        _observer.Journal.FreePool(_pool.Handle);_observer.Check();_observer.Dispose();
        _vk.Dispose();_disposed=true; // Qt's borrowed device/instance are never destroyed here.
    }
}
