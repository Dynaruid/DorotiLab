...
public sealed unsafe partial class GraphiteVulkanWindow
{
        ...
        using (api)
        {
            ...
            for (uint i = 0; i < memory.MemoryTypeCount; i++)
            {
                if (
                    (bits & (1u << (int)i)) != 0
                    && (
                        memory.MemoryTypes[(int)i].PropertyFlags
            ...
            {
                throw new PlatformNotSupportedException(
                    "D3D12 import has no compatible memory or exceeds the image budget."
                );
            }
            ...
            Check(
                _vk.AllocateMemory(_device, &allocation, null, out _memory),
                "D3D12 import memory"
            );
        }
    ...
    public void FlushD3D12Frame()
    {
        ...
        {
            throw new InvalidOperationException("No recording frame to flush.");
        }
    ...
    public void ReleaseD3D12Frame()
    {
        ...
        if (_externalSubmitted)
        {
            ...
            {
                Check(result, "D3D12 frame drain");
            }
