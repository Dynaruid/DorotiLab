...
{
    internal static Physics.SpringDescription _kFlingSpringDescription =
        Physics.SpringDescription.CreateWithDampingRatio(mass: 1.0, stiffness: 500.0);
}

public static partial class Animation_controllerLibrary
{
    internal static Physics.Tolerance _kFlingTolerance = new Physics.Tolerance(
...
{
    internal static bool _enableAnimations(this AnimationBehavior value) =>
        value switch
        {
            AnimationBehavior.normal => !PlatformDispatcher
                .instance
        ...
        AnimationLocalStatusListenersMixin
{
    ...
    internal virtual _AnimationDirection__animation_controller _direction { get; set; } =
    ...
    public static AnimationController CreateUnbounded(
        double value = 0.0,
        Duration? duration = null,
        Duration? reverseDuration = null,
        string? debugLabel = null,
    ...
    )
    {
        ...
        DartRuntimePrimitives.Assert(() =>
            Foundation.DebugLibrary.debugMaybeDispatchCreated(
                "animation",
                "AnimationController",
                __instance
            )
    ...
    public virtual TickerFuture toggle(double? from = null)
    {
        ...
        _direction = isForwardOrCompleted
            ? _AnimationDirection__animation_controller.reverse
            : _AnimationDirection__animation_controller.forward;
        if (from is not null)
        {
    ...
    )
    {
        ...
        if (simulationDuration is null)
        {
            DartRuntimePrimitives.Assert(() =>
                !(
                    (this.duration is null)
                    && Equals(_direction, _AnimationDirection__animation_controller.forward)
                )
            );
            DartRuntimePrimitives.Assert(() =>
                !(
                    ...
                    && (reverseDuration is null)
                )
            );
            double range = upperBound - lowerBound;
            double remainingFraction = double.IsFinite(range)
    ...
    public virtual TickerFuture fling(
        double velocity = 1.0,
        Physics.SpringDescription? springDescription = null,
    ...
    )
    {
        ...
        double target =
            (velocity < 0.0)
                ? (lowerBound - Animation_controllerLibrary._kFlingTolerance.distance)
                : (upperBound + Animation_controllerLibrary._kFlingTolerance.distance);
        ...
        var simulation = (
            (Func<Physics.SpringSimulation>)(
                () =>
                {
                    var __cascade = new Physics.SpringSimulation(
                        springDescription,
                        value,
                        target,
                        velocity * scale
        ...
        DartRuntimePrimitives.Assert(() =>
            !Equals(simulation.type, Physics.SpringType.underDamped)
        );
        stop();
...
internal class _InterpolationSimulation__animation_controller : Physics.Simulation
{
    ...
    internal _InterpolationSimulation__animation_controller(
        double _begin,
        double _end,
        Duration duration,
        Curve _curve,
...
internal delegate void _DirectionSetter__animation_controller(
    _AnimationDirection__animation_controller direction
);
...
internal class _RepeatingSimulation__animation_controller : Physics.Simulation
{
    ...
    public virtual Action<_AnimationDirection__animation_controller> directionSetter
    {
        get;
        private set;
    } = default!;
    ...
    internal _RepeatingSimulation__animation_controller(
        double initialValue,
        double min,
        double max,
        bool reverse,
        Duration period,
        Action<_AnimationDirection__animation_controller> directionSetter,
        long? count
    )
    {
        this.min = min;
