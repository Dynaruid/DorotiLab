...
/// <summary>A backend-neutral managed BGRA8888 surface for host framebuffer adapters.</summary>
public sealed class ManagedBgraRenderSurface : IRenderSurface
{
    ...
    public ManagedBgraRenderSurface(IBgra8888FramebufferTarget target)
    {
    ...
    public ISurfaceFrame BeginFrame()
    {
        ...
        RecreateIfNeeded();
        return new Frame(this, Generation, _metricsGeneration, PixelSize, _pixels);
    }
    ...
    private void Present(SurfaceGeneration generation, byte[] pixels, int width, int height)
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
        RecreateIfNeeded();
    ...
    private sealed class Frame(
        ManagedBgraRenderSurface owner,
        SurfaceGeneration generation,
        long metricsGeneration,
        Size size,
        byte[] pixels) : ISurfaceFrame, IPixelReadableSurfaceFrame, IMetricsBoundSurfaceFrame
    {
        private bool _disposed;
        ...
        public Size PixelSize { get; } = size;

        public IRasterCanvas Canvas { get; } = new ManagedRasterCanvas(pixels, checked((int)size.Width), checked((int)size.Height));

        public void Clear(Color color)
        {
            ObjectDisposedException.ThrowIf(_disposed, this);
            ((ManagedRasterCanvas)Canvas).Clear(color);
        }
        ...
        public bool TryReadPixels(Span<byte> destination, int rowBytes)
        {
            ...
            {
                pixels.AsSpan(row * requiredRowBytes, requiredRowBytes).CopyTo(destination.Slice(row * rowBytes, requiredRowBytes));
            }
            return true;
        }
        ...
        public void Present()
        {
            ...
            owner.Present(Generation, pixels, checked((int)PixelSize.Width), checked((int)PixelSize.Height));
            _presented = true;
        }
...
internal sealed class ManagedRasterCanvas : IRasterCanvas
{
    ...
    internal ManagedRasterCanvas(byte[] pixels, int width, int height)
    {
        _pixels = pixels;
        _width = width;
        _height = height;
    ...
    public void ClipPath(PathGeometry path)
    {
        ...
        var bounds = BoundsOf(points).Intersect(_state.ClipBounds);
        _state = _state with
        {
            ClipBounds = bounds,
    ...
    public void DrawImage(ImageResourceSnapshot image, Rect source, Rect destination, double opacity = 1)
    {
        ...
        VisitPixels(target, (x, y) =>
        {
            ...
            Blend(x, y, Color.FromArgb(sourcePixels[index + 3], sourcePixels[index + 2], sourcePixels[index + 1], sourcePixels[index]), opacity * _state.Opacity);
        });
    }
