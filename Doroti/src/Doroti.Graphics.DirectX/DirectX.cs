...
{
    private static string WindowsLibrary(string name) =>
    ...
    {
        internal static readonly DX.DXGI Api = new(
            DX.DXGI.CreateDefaultContext([WindowsLibrary("dxgi.dll")])
        );
    }

    private static class D3D12Module
    {
        internal static readonly D12.D3D12 Api = new(
            D12.D3D12.CreateDefaultContext([WindowsLibrary("d3d12.dll")])
        );
    }

    private static class D3D11Module
    {
        internal static readonly D11.D3D11 Api = new(
            D11.D3D11.CreateDefaultContext([WindowsLibrary("d3d11.dll")])
        );
    }

    private static class On12Module
    {
        internal static readonly On12.D3D11On12 Api = new(
            On12.D3D11On12.CreateDefaultContext([WindowsLibrary("d3d11.dll")])
        );
    }
    ...
    public static HResult D3D11CreateDevice(
        ...
        D3DDriverType driver,
        D11.CreateDeviceFlag flags,
        D3DFeatureLevel[] levels,
        out ID3D11Device device,
        out D3DFeatureLevel chosen,
    ...
    )
    {
        ...
        {
            hr = api.CreateDevice(
                (DX.IDXGIAdapter*)(adapter?.NativePointer ?? 0),
                driver,
                0,
                (uint)flags,
                levelPointer,
                (uint)levels.Length,
                D11.D3D11.SdkVersion,
                &nativeDevice,
                &nativeLevel,
                &nativeContext
            );
    ...
    public static HResult D3D11On12CreateDevice(
    ...
    )
    {
        ...
        {
            hr = api.On12CreateDevice(
                (IUnknown*)device12.NativePointer,
                (uint)flags,
                levelPointer,
                (uint)levels.Length,
                (IUnknown**)queuePointer,
                (uint)queues.Length,
                nodeMask,
                &nativeDevice,
                &nativeContext,
    ...
    private static HResult WrapDevice(
        int hr,
        D11.ID3D11Device* nativeDevice,
        D11.ID3D11DeviceContext* nativeContext,
        out ID3D11Device device,
        out ID3D11DeviceContext context
    )
    {
