...
public abstract class DragGestureRecognizer : OneSequenceGestureRecognizer
{
    ...
    public virtual Func<PointerEvent, VelocityTracker> velocityTrackerBuilder { get; set; } =
        default!;
    ...
    internal virtual DartMap<long, VelocityTracker> _velocityTrackers { get; private set; } =
        new DartMap<long, VelocityTracker>();
    internal virtual DartMap<long, Offset> _moveDeltaBeforeFrame { get; private set; } =
        new DartMap<long, Offset>();
    ...
    protected DragGestureRecognizer(
        ...
        Func<PointerEvent, VelocityTracker> velocityTrackerBuilder = default!,
    ...
    {
        Func<PointerEvent, VelocityTracker> __velocityTrackerBuilder =
            velocityTrackerBuilder ?? _defaultBuilder;
    ...
    internal static VelocityTracker _defaultBuilder(PointerEvent @event) =>
        new VelocityTracker(@event.kind);
    ...
    public abstract DragEndDetails? considerFling(
    ...
    internal virtual void _addPointer(PointerEvent @event)
    {
        ...
        switch (_state)
        {
            case _DragState__monodrag.ready:
            {
                ...
                _initialPosition = new OffsetPair(
    ...
    internal virtual void _recordMoveDeltaForMultitouch(long pointer, Offset localDelta)
    {
        ...
        DartRuntimePrimitives.Assert(() =>
            Equals(_frameTimeStamp, SchedulerBinding.instance.currentSystemFrameTimeStamp)
        );
    ...
    internal virtual Offset _resolveLocalDeltaForMultitouch(long pointer, Offset localDelta)
    {
        ...
        Duration currentSystemFrameTimeStampLocal = SchedulerBinding
            .instance
        ...
        if (
            (!Equals(_state, _DragState__monodrag.accepted))
            || Equals(localDelta, Offset.zero)
            || ((checked((long)_moveDeltaBeforeFrame.Count) == 0) && (axisLocal is not null))
        )
        {
            return localDelta;
        }
        ...
        {
            dxLocal = _resolveDelta(
                pointer: pointer,
                axis: _DragDirection__monodrag.horizontal,
        ...
        else
        {
            ...
            {
                dxLocal = 0.0;
                dyLocal = _resolveDelta(
                    pointer: pointer,
                    axis: _DragDirection__monodrag.vertical,
                    localDelta: localDelta
                );
            ...
            {
                double averageX = _resolveDeltaForPanGesture(
                    axis: _DragDirection__monodrag.horizontal,
                    localDelta: localDelta
                );
                double averageY = _resolveDeltaForPanGesture(
                    axis: _DragDirection__monodrag.vertical,
                    localDelta: localDelta
                );
    ...
    internal virtual double _resolveDelta(
        long pointer,
        _DragDirection__monodrag axis,
        Offset localDelta
    )
    {
        bool positiveLocal = Equals(axis, _DragDirection__monodrag.horizontal)
            ? (localDelta.dx > 0L)
            : (localDelta.dy > 0L);
        double delta = Equals(axis, _DragDirection__monodrag.horizontal)
            ? localDelta.dx
            : localDelta.dy;
        ...
        else
        {
            ...
            double curPointerSumDelta = _getSumDelta(
                pointer: pointer,
                positive: positiveLocal,
                axis: axis
            );
    ...
    public override void handleEvent(PointerEvent @event)
    {
        ...
        {
            Offset positionLocal = @event switch
            {
                PointerPanZoomStartEvent __object24693 => Offset.zero,
                PointerPanZoomUpdateEvent __object24744 => __object24744.pan,
                _ => @event.localPosition,
            };
            _velocityTrackers
                .GetValueOrDefault(@event.pointer)!
                .addPosition(@event.timeStamp, positionLocal);
        }
        ...
        )
        {
            Offset deltaLocal =
                (@event is PointerMoveEvent)
                    ? ((PointerMoveEvent)@event).delta
                    : (
                        (PointerPanZoomUpdateEvent?)
                            (object?)((PointerPanZoomUpdateEvent?)(object?)@event)!
                    )!.panDelta;
            Offset localDeltaLocal =
                (@event is PointerMoveEvent)
                    ? ((PointerMoveEvent)@event).localDelta
                    : (
                        (PointerPanZoomUpdateEvent?)
                            (object?)((PointerPanZoomUpdateEvent?)(object?)@event)!
                    )!.localPanDelta;
            Offset positionAlternate =
                (@event is PointerMoveEvent)
                    ? ((PointerMoveEvent)@event).position
                    : (
                        @event.position
                        + (
                            (PointerPanZoomUpdateEvent?)
            ...
            Offset localPositionLocal =
                (@event is PointerMoveEvent)
                    ? ((PointerMoveEvent)@event).localPosition
                    : (
                        @event.localPosition
                        + (
                            (PointerPanZoomUpdateEvent?)
            ...
            switch (_state)
            {
                ...
                {
                    _pendingDragOffset = _pendingDragOffset.op_Add(
                        new OffsetPair(local: localDeltaLocal, global: deltaLocal)
                    );
                    _lastPendingEventTimestamp = @event.timeStamp;
                    _lastTransform = @event.transform;
                    ...
                    if (
                        hasSufficientGlobalDistanceToAccept(@event.kind, gestureSettings?.touchSlop)
                    )
                    {
    ...
    internal virtual void _giveUpPointer(long pointer)
    {
        ...
        {
            _activePointer =
                (checked((long)_acceptedActivePointers.Count) != 0)
    ...
    internal virtual void _checkDrag(long pointer)
    {
        ...
        if ((!Equals(localUpdateDelta, Offset.zero)) && (onUpdate is not null))
        {
            Matrix4? localToGlobal =
                (transformLocal is not null) ? Matrix4.tryInvert(transformLocal) : null;
    ...
    internal virtual void _checkUpdate(
...
public class VerticalDragGestureRecognizer : DragGestureRecognizer
{
    ...
    public override DragEndDetails? considerFling(VelocityEstimate estimate, PointerDeviceKind kind)
    {
        ...
        return new DragEndDetails(
            velocity: new Velocity(pixelsPerSecond: new Offset(0, dyLocal)),
    ...
    internal override _DragDirection__monodrag? _getPrimaryDragAxis() =>
        _DragDirection__monodrag.vertical;
...
public class HorizontalDragGestureRecognizer : DragGestureRecognizer
{
    ...
    public override DragEndDetails? considerFling(VelocityEstimate estimate, PointerDeviceKind kind)
    {
        ...
        return new DragEndDetails(
            velocity: new Velocity(pixelsPerSecond: new Offset(dxLocal, 0)),
    ...
    internal override _DragDirection__monodrag? _getPrimaryDragAxis() =>
        _DragDirection__monodrag.horizontal;
...
public class PanGestureRecognizer : DragGestureRecognizer
{
    ...
    public override DragEndDetails? considerFling(VelocityEstimate estimate, PointerDeviceKind kind)
    {
        ...
        Velocity velocityLocal = new Velocity(
            pixelsPerSecond: estimate.pixelsPerSecond
        ).clampMagnitude(
            minFlingVelocity ?? ConstantsLibrary.kMinFlingVelocity,
            maxFlingVelocity ?? ConstantsLibrary.kMaxFlingVelocity
        );
        return new DragEndDetails(
