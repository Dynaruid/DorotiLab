...
public abstract class MultiDragPointerState
{
    ...
    public virtual global::Doroti.Ui.Offset? pendingDelta => this._pendingDelta;
    ...
    internal virtual void _handleEvent(PointerEvent @event)
    {
        ...
        else
        {
            if ((@event is PointerUpEvent))
            {
                ...
                DartRuntimePrimitives.Assert(() => (Equals(((PointerUpEvent)@event__as8876).delta, Offset.zero)));
                state._up();
            ...
            else
            {
                if ((@event is PointerCancelEvent))
                {
                    ...
                    DartRuntimePrimitives.Assert(() => (Equals(((PointerCancelEvent)@event__as9050).delta, Offset.zero)));
                    state._cancel();
