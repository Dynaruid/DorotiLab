...
public sealed class PlatformDispatcher : IDisposable
{
    ...
    public static PlatformDispatcher instance => ActiveDispatcher.Value ??
        throw new FlutterCapabilityException(
            FlutterCapabilityIds.DartPerformanceMode,
            null,
    ...
    public IReadOnlyList<FlutterView> views
    {
        get
        {
            lock (_gate)
    ...
    public FlutterView? implicitView => views.Count == 1 ? views[0] : null;
    ...
    public FlutterView? view(long id)
    {
        lock (_gate)
        {
    ...
    public Action<FlutterView>? onMetricsChanged { get; set; }
    ...
    public Action<FlutterView, AppLifecycleState>? onAppLifecycleStateChanged { get; set; }

    public event Action<FlutterView, AppLifecycleState>? lifecycleChanged;

    public Action<Doroti.Flutter.Runtime.Duration>? onBeginFrame { get; set; }
    ...
    public event Action<FlutterView, TimeSpan>? beginFrame;

    public event Action<FlutterView>? drawFrame;

    public Action<FlutterView, PointerDataPacket>? onPointerDataPacket { get; set; }
    ...
    public event Action<FlutterView, KeyData>? keyData;

    public Action<FlutterView, RawFocusData>? onFocusData { get; set; }

    public event Action<FlutterView, RawFocusData>? focusData;

    public Action<FlutterView>? onViewFocusChanged { get; set; }
    ...
    public Action<FlutterView, PlatformConfiguration>? onPlatformConfigurationChanged { get; set; }
    ...
    public FlutterView RegisterView(ulong viewId, FlutterViewCapabilities capabilities)
    {
        if (viewId == 0)
        {
        ...
        lock (_gate)
        {
            ...
            try
            {
                view = new FlutterView(this, viewId, capabilities);
            }
            catch
            {
    ...
    public FlutterView GetView(ulong viewId, DartUiInvocation invocation)
    {
        lock (_gate)
        {
            ObjectDisposedException.ThrowIf(_disposed, this);
    ...
    internal void Remove(FlutterView view)
    {
        lock (_gate)
        {
    ...
    internal void DispatchMetrics(FlutterView view) => DispatchWithEnvironment(view, () => onMetricsChanged?.Invoke(view));

    internal void DispatchLifecycle(FlutterView view, AppLifecycleState state) =>
    ...
    internal void DispatchPointer(FlutterView view, PointerDataPacket packet) =>
        DispatchWithEnvironment(view, () => onPointerDataPacket?.Invoke(view, packet));

    internal void DispatchKey(FlutterView view, KeyData data) =>
        DispatchWithEnvironment(view, () =>
        {
    ...
    internal void DispatchFocus(FlutterView view, RawFocusData data)
    {
        DispatchWithEnvironment(view, () =>
        {
    ...
    internal void DispatchPlatformConfiguration(FlutterView view, PlatformConfiguration configuration)
    {
    ...
    internal void DispatchSemanticsAction(FlutterView view, SemanticsActionEvent action) =>
        DispatchWithEnvironment(view, () => onSemanticsActionEvent?.Invoke(action));

    internal void DispatchFrame(FlutterView view, TimeSpan timestamp)
    {
        DispatchWithEnvironment(view, () =>
        {
    ...
    private FlutterView RequireMessagingView()
    {
        lock (_gate)
        {
            ObjectDisposedException.ThrowIf(_disposed, this);
...
public sealed class FlutterView : IDisposable
{
    private readonly PlatformDispatcher _dispatcher;
    private readonly FlutterViewCapabilities _capabilities;
    ...
    internal FlutterView(PlatformDispatcher dispatcher, ulong viewId, FlutterViewCapabilities capabilities)
    {
        ...
        _viewHost = capabilities.Require<IViewHostCapability>(
            viewId,
            FlutterCapabilityIds.ViewLifecycleMetrics,
            DartUiInvocation.Managed("dart:ui#FlutterView"));
        ...
        if (capabilities.RegisteredIds.Contains(FlutterCapabilityIds.InputEvents, StringComparer.Ordinal))
        {
        ...
        if (capabilities.RegisteredIds.Contains(FlutterCapabilityIds.PlatformEnvironment, StringComparer.Ordinal))
        {
        ...
        if (capabilities.RegisteredIds.Contains(FlutterCapabilityIds.AccessibilitySemantics, StringComparer.Ordinal))
        {
    ...
    public IDisposable EnterPlatformEnvironmentScope()
    {
        ...
        if (_environmentHost is null)
        {
            throw new FlutterCapabilityException(
                FlutterCapabilityIds.PlatformEnvironment,
                viewId,
                DartUiInvocation.Managed("dart:ui#FlutterView.EnterPlatformEnvironmentScope"),
                "the active host did not register it",
                targetIdentity);
        }
    ...
    public void SubmitScene(Scene scene, DartUiInvocation invocation)
    {
        ...
        _capabilities.Require<ISceneHostCapability>(viewId, FlutterCapabilityIds.GraphicsScene, invocation)
            .Submit(viewId, scene, invocation);
    }

    public void render(Scene scene) => SubmitScene(scene, DartUiInvocation.Managed("dart:ui#FlutterView.render"));

    public void render(Scene scene, Size size)
    {
        ArgumentNullException.ThrowIfNull(scene);
        _ = size;
        SubmitScene(scene, DartUiInvocation.Managed("dart:ui#FlutterView.render"));
    }

    public Paragraph LayoutParagraph(ParagraphRequest request, DartUiInvocation invocation) =>
        _capabilities.Require<IParagraphHostCapability>(viewId, FlutterCapabilityIds.GraphicsText, invocation)
    ...
    public ValueTask<Image> DecodeImageAsync(
        ...
        DartUiInvocation invocation,
        CancellationToken cancellationToken = default) =>
        _capabilities.Require<IImageHostCapability>(viewId, FlutterCapabilityIds.GraphicsImage, invocation)
    ...
    public void UpdateSemantics(SemanticsUpdate update, DartUiInvocation invocation) =>
        _capabilities.Require<ISemanticsHostCapability>(viewId, FlutterCapabilityIds.AccessibilitySemantics, invocation)
    ...
    public void updateSemantics(SemanticsUpdate update) =>
        UpdateSemantics(update, DartUiInvocation.Managed("dart:ui#FlutterView.updateSemantics"));
