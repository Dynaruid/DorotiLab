...
public class CircleBorder : OutlinedBorder
{
    ...
    public override ShapeBorder scale(double t) => new CircleBorder(side: side.scale(t), eccentricity: eccentricity);
    public override ShapeBorder? lerpFrom(ShapeBorder? a, double t)
    {
        if (a is CircleBorder)
        {
            CircleBorder a__as2194 = (CircleBorder)a;
            return new CircleBorder(side: BorderSide.lerp(a__as2194.side, side, t), eccentricity: Dart_uiLibrary.clampDouble(DartRuntimePrimitives.RequireValue(Dart_uiLibrary.lerpDouble(a__as2194.eccentricity, eccentricity, t)), 0.0, 1.0));
        }
    ...
    public override ShapeBorder? lerpTo(ShapeBorder? b, double t)
    {
        if (b is CircleBorder)
        {
            ...
            return new CircleBorder(side: BorderSide.lerp(side, b__as2507.side, t), eccentricity: Dart_uiLibrary.clampDouble(DartRuntimePrimitives.RequireValue(Dart_uiLibrary.lerpDouble(eccentricity, b__as2507.eccentricity, t)), 0.0, 1.0));
        }
        ...
        return ((Func<Path>)(() =>
{
            ...
            case BorderStyle.solid:
                {
                    if (eccentricity == 0.0)
                    {
                        canvas.drawCircle(rect.center, (rect.shortestSide + side.strokeOffset) / 2L, side.toPaint());
                    }
                    else
                    {
    ...
    public override bool Equals(object? other)
    {
        ...
        return (__other is CircleBorder) && Equals(__other.side, side) && (__other.eccentricity == eccentricity);
    }
