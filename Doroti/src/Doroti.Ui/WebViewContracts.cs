...
public sealed record WebViewEvent(
    PlatformViewHandle Handle,
    long NavigationId,
    long DocumentGeneration,
    WebViewEventKind Kind,
    string? Url,
    string? Error = null,
    string? MessageName = null,
    string? MessageJson = null,
    long MessageRequestId = 0
);

public sealed record WebViewCommand(
    WebViewOperation Operation,
    string? Text = null,
    long DocumentGeneration = 0
);

public sealed record WebViewFeatures(
    bool Navigation,
    bool JavaScript,
    bool EphemeralProfile,
    bool SharedPersistentProfile,
    bool ClearAllData,
    bool ScriptMessages = false,
    bool AppContentScheme = false,
    bool IsolatedPersistentProfile = false,
    bool FirstContentFrame = false
)
{
...
public sealed record WebViewOptions(
    ...
    Dictionary<string, WebViewResource>? Resources = null,
...
)
{
    ...
    public ReadOnlyMemory<byte> Encode()
    {
        ...
        return Encoding.UTF8.GetBytes(
            Prefix + JsonSerializer.Serialize(this, WebViewJsonContext.Default.WebViewOptions)
        );
    }
    ...
    public void Validate()
    {
        if (Profile == WebViewProfile.BrowserDefault && !OperatingSystem.IsBrowser())
        {
            throw new WebViewException(
                ...
                "BrowserDefault is only available in a browser host."
            );
        }
        ...
        if (!Enum.IsDefined(Profile) || Encoding.UTF8.GetByteCount(Html ?? "") > 2 * 1024 * 1024)
        {
            throw new WebViewException(
                ...
                "Invalid WebView profile or HTML larger than 2 MiB."
            );
        }

        if (
            (Resources?.Count ?? 0) > 256
            || (AllowedOrigins?.Length ?? 0) > 128
            || (MessageOrigins?.Length ?? 0) > 128
        )
        {
            throw new WebViewException(
                WebViewError.InvalidRequest,
                "Too many content routes or origins."
            );
        }
        ...
        foreach (var (path, resource) in Resources ?? [])
        {
            if (
                ...
                || string.IsNullOrWhiteSpace(resource.MimeType)
                || resource.MimeType.Any(char.IsControl)
            )
            {
                throw new WebViewException(
                    WebViewError.InvalidRequest,
                    "App content requires an exact absolute URL path and a manifest resource key/MIME type."
                );
            }
        }
        ...
        foreach (var origin in MessageOrigins ?? [])
        {
            ...
            )
            {
                throw new WebViewException(
                    WebViewError.InvalidRequest,
                    "MessageOrigins must contain canonical HTTP(S) or doroti-app://content origins."
                );
            }
        }
        ...
        foreach (var origin in AllowedOrigins ?? [])
        {
            ...
            )
            {
                throw new WebViewException(
                    WebViewError.InvalidRequest,
                    "AllowedOrigins must contain canonical HTTP(S) origins without paths."
                );
            }
        }
    }
...
{
    Task<WebViewResult> ExecuteWebViewAsync(
        PlatformViewHandle handle,
