...
public readonly record struct PlatformViewHandle(
    ulong OwnerViewId,
    long InstanceId,
    long InstanceGeneration
);

public enum PlatformViewState
{
    Requested,
    Creating,
    Ready,
...
public enum PlatformViewComposition
{
    NativeOverlay,
...
public sealed record PlatformViewSupport(
    ...
    PlatformViewEffects Effects,
    bool CaptureIncludesNative = false,
    bool GestureMediation = false,
    bool Accessibility = false,
    bool SynchronizedPlacement = false,
    string? Reason = null,
...
public readonly record struct PlatformViewTransform(
    double M11,
    double M12,
    double M21,
    double M22,
...
)
{
    ...
    public Offset Map(Offset point) =>
        new((M11 * point.dx) + (M21 * point.dy) + Dx, (M12 * point.dx) + (M22 * point.dy) + Dy);

    public PlatformViewTransform ThenLocal(PlatformViewTransform b) =>
        new(
            (M11 * b.M11) + (M21 * b.M12),
            (M12 * b.M11) + (M22 * b.M12),
            (M11 * b.M21) + (M21 * b.M22),
            (M12 * b.M21) + (M22 * b.M22),
            (M11 * b.Dx) + (M21 * b.Dy) + Dx,
            (M12 * b.Dx) + (M22 * b.Dy) + Dy
        );
}
    ...
    public void Validate()
    {
        ...
        )
        {
            throw new ArgumentOutOfRangeException(
                nameof(Bounds),
                "PlatformView layout and clip must be finite and nonnegative."
            );
        }
    }
}
...
public sealed record PlatformInputShield(
    Rect Bounds,
    PlatformViewTransform Transform,
    Rect? Clip,
    int PaintOrder,
...
public readonly record struct PlatformCompositionToken(
    ulong OwnerViewId,
    long ViewEpoch,
    long FrameNumber,
    long SurfaceGeneration,
    double DeviceScaleX = 1,
    double DeviceScaleY = 1
);
...
public interface IPlatformViewHostCapability
{
    ...
    ValueTask<PlatformViewHandle> CreateAsync(
        ...
        throw new NotSupportedException(
            "This host does not implement descriptor-based allocation."
        );
    long AllocateInstanceId() =>
        throw new NotSupportedException("This host does not allocate instance IDs.");
