...
public static class FrameworkWorkTrace
{
    ...
    public static void Start(int capacity = 65536)
    {
        ...
        if (Enabled)
        {
            throw new InvalidOperationException("A contract capture is already active.");
        }
    ...
    public static void Register(object node, int logicalId)
    {
        if (_buffer is not { } b)
        {
            throw new InvalidOperationException("Start capture before registering nodes.");
        }

        if (logicalId <= 0 || b.Nodes.Any(n => n.LogicalId == logicalId))
        {
            throw new ArgumentException(
                "Logical IDs must be positive and unique.",
                nameof(logicalId)
            );
        }
        ...
        if (id != 0)
        {
            if (b.Nodes[id - 1].LogicalId != 0)
            {
                throw new InvalidOperationException("A node already has a logical ID.");
            }
    ...
    {
        if (node is null || _buffer is not { } b)
        {
            return 0;
        }

        if (b.Ids.TryGetValue(node, out var identity))
        {
            return identity.Id;
        }

        if (b.Nodes.Count == b.Events.Length)
        {
            b.Dropped++;
            return 0;
        }
    ...
    )
    {
        ...
        if (buffer.Count == buffer.Events.Length)
        {
            buffer.Dropped++;
            return;
        }
        buffer.Events[buffer.Count++] = new(
            buffer.Tick,
            kind,
            Id(node),
            Id(related),
    ...
    public static Capture Stop()
    {
        ...
        return new(
            b.Dropped == 0,
            b.Dropped,
            b.Nodes.Select((n, i) => new Node(i + 1, n.LogicalId, n.Type.FullName ?? n.Type.Name))
                .ToArray(),
            b.Events[..b.Count]
        );
    }
