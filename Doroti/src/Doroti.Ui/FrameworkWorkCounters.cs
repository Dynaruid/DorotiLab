...
public enum FrameworkWork
{
    ...
    ForcedRebuild,
    MarkBuild,
    MarkBuildAlreadyDirty,
    SetState,
    DependencyChanged,
    LayoutEntry,
    LayoutFastPath,
    LayoutWork,
    LayoutDirtySameConstraints,
    MarkLayout,
    MarkLayoutAlreadyDirty,
    LayoutBoundary,
    LayoutParentPropagation,
    MarkPaint,
    MarkPaintAlreadyDirty,
    PaintBoundary,
    PaintParentPropagation,
    RepaintBoundary,
    NewPicture,
    MediaUpdate,
    MediaChanged,
    MediaDependentCheck,
    MediaDependentNotified,
    MediaAspectSubscriptions,
    HostSnapshotApply,
    HostMetricsNotified,
    DelegateUpdate,
    DelegateRebuild,
    RetainedChildVisit,
    ImplicitTweenCheck,
    ImplicitAnimationRestart,
    SemanticsJsonCacheHit,
    SemanticsJsonSerializedNode,
    SemanticsPayloadBytes,
    Count,
}
...
public static class FrameworkWorkCounters
{
    public static bool Enabled { get; } =
        Environment.GetEnvironmentVariable("DOROTI_STAGE_TRACE") == "1";
    ...
    public static void Add(FrameworkWork kind, long count = 1)
    {
        if (Enabled)
        {
            (_buffer ??= new Buffer()).Totals[(int)kind] += count;
        }
    }
    ...
    )
    {
        if (
            ...
            || phase
                is not (
                    DorotiFramePhase.metrics
                    or DorotiFramePhase.build
                    or DorotiFramePhase.layout
                    or DorotiFramePhase.paint
                    or DorotiFramePhase.sceneBuild
                    or DorotiFramePhase.sceneSubmitted
                    or DorotiFramePhase.drawFrame
                )
        )
        {
            return;
    ...
    public static Capture Snapshot()
    {
        ...
        if (buffer is null)
        {
            return new(Enabled, 0, Enum.GetNames<FrameworkWork>()[..Width], []);
        }
        ...
        return new(
            Enabled,
            Math.Max(0, buffer.Count - Capacity),
            Enum.GetNames<FrameworkWork>()[..Width],
            samples
        );
    }
}
