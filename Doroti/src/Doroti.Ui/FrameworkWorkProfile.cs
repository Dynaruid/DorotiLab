...
public static class FrameworkWorkProfile
{
    ...
    private sealed class Buffer
    {
        ...
        public readonly long[] Calls = new long[Capacity],
            Inclusive = new long[Capacity],
            Self = new long[Capacity];
        public readonly long[] Children = new long[Capacity];
        public readonly long[] Allocated = new long[Capacity],
            SelfAllocated = new long[Capacity],
            ChildAllocated = new long[Capacity];
        public readonly long[] StartCalls = new long[Capacity],
            StartSelf = new long[Capacity],
            StartInclusive = new long[Capacity];
    ...
    public readonly struct Scope : IDisposable
    {
        ...
        public void Dispose()
        {
            if (!_active || _buffer is not { } buffer)
            {
                return;
            }
            ...
            if (_depth > 0)
            {
                buffer.Children[_depth - 1] += elapsed;
            }
            ...
            if (AllocationEnabled)
            {
                ...
                if (_depth > 0)
                {
                    buffer.ChildAllocated[_depth - 1] += allocated;
                }
            }
        }
    }
    ...
    internal static void Record(long sequence, DorotiFramePhase phase)
    {
        ...
        if (phase != DorotiFramePhase.layout)
        {
            return;
        }
    ...
    private static long[][] Frames()
    {
        ...
        return Enumerable
            .Range(0, (int)Math.Min(buffer.FrameCount, 512))
            .Select(index =>
            {
    ...
    public sealed record Entry(
        ...
        long SelfMicroseconds,
        long AllocatedBytes = 0,
        long SelfAllocatedBytes = 0
    );

    public static Entry[] CaptureEntries() =>
        _buffer
            ?.Ids.Select(pair => new Entry(
                pair.Value,
                pair.Key.Item1.FullName ?? pair.Key.Item1.Name,
                pair.Key.Item2,
                _buffer.Calls[pair.Value],
                _buffer.Inclusive[pair.Value] / 10,
                _buffer.Self[pair.Value] / 10,
                _buffer.Allocated[pair.Value],
                _buffer.SelfAllocated[pair.Value]
            ))
            .ToArray()
        ?? [];
