...
public sealed class WindowsTarget : IDesktopFrameworkTarget
{
    ...
    public DesktopFrameworkTargetDiagnostics CaptureDiagnostics(ulong viewId)
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
        return _host.GetTargetDiagnostics(viewId);
    }

    public DesktopFrameworkRetainedDiagnostics CaptureRetainedDiagnosticsForValidation(ulong viewId)
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
    ...
    {
        using var stream = typeof(WindowsTarget).Assembly.GetManifestResourceStream("Doroti.Target.Manifest")
