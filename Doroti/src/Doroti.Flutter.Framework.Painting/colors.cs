...
public static partial class ColorsLibrary
{
    internal static Color _colorFromHue(double alpha, double hue, double chroma, double secondary, double match)
    {
        ...
        return global::Doroti.Flutter.Ui.Color.fromARGB(((alpha * 255L)).round(), ((((red__912 + match)) * 255L)).round(), ((((green__924 + match)) * 255L)).round(), ((((blue__938 + match)) * 255L)).round());
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
}
...
public class HSVColor
{
    ...
    public virtual global::Doroti.Flutter.Ui.Color toColor()
    {
...
public class ColorSwatch<T> : Color where T : notnull
{
    ...
    public global::Doroti.Flutter.Ui.Color? this[T key]
    {
        get
        {
    ...
    public override bool Equals(object? other)
    {
        ...
        return ((base.Equals(__other) && (__other is ColorSwatch<T>)) && global::Doroti.Generated.Framework.Foundation.CollectionsLibrary.mapEquals<T, global::Doroti.Flutter.Ui.Color>(((ColorSwatch<T>)((ColorSwatch<T>)__other))._swatch, this._swatch));
    }
    ...
    public static ColorSwatch<T>? lerp<T>(ColorSwatch<T>? a, ColorSwatch<T>? b, double t) where T : notnull
    {
        ...
        DartMap<T, global::Doroti.Flutter.Ui.Color> swatch__18030 = default!;
        if ((b is null))
        {
            swatch__18030 = a!._swatch.map<T, Color, T, Color>(((key, color) => new MapEntry<T, global::Doroti.Flutter.Ui.Color>(key, Dart_uiLibrary.Color.lerp(color, null, t)!)));
        }
        else
        {
            if ((a is null))
            {
                swatch__18030 = ((ColorSwatch<T>)b)._swatch.map<T, Color, T, Color>(((key, color) => new MapEntry<T, global::Doroti.Flutter.Ui.Color>(key, Dart_uiLibrary.Color.lerp(null, color, t)!)));
            }
            else
            {
                swatch__18030 = ((ColorSwatch<T>)a)._swatch.map<T, Color, T, Color>(((key, color) => new MapEntry<T, global::Doroti.Flutter.Ui.Color>(key, Dart_uiLibrary.Color.lerp(color, b[key], t)!)));
            }
        }
