...
internal static class QtTitlebarPainter
{
    internal static void Paint(
        SKCanvas canvas,
        string title,
        in QtNativeV2.Surface surface,
        WindowTitlebarTheme? theme = null
    )
    {
        if (surface.TitlebarHeight == 0)
        {
            return;
        }
        ...
        var dark = theme is not null
            ? theme.Brightness == Brightness.dark
            : (surface.TitlebarState & 1) != 0;
        var background =
            theme is not null ? new SKColor(unchecked(theme.BackgroundColor.value))
            : dark ? new SKColor(32, 32, 32, 153)
            : new SKColor(245, 245, 245, 153);
        ...
        try
        {
            ...
            var foreground = (dark ? SKColors.White : SKColors.Black).WithAlpha(
                active ? (byte)230 : (byte)150
            );
            ...
            canvas.DrawText(
                title,
                12,
                (height - font.Metrics.Ascent - font.Metrics.Descent) / 2,
                SKTextAlign.Left,
                font,
                ink
            );
            ...
            for (uint button = 1; button <= 3; button++)
            {
                ...
                if (surface.TitlebarHovered == button)
                {
                    ink.Color =
                        button == 3
                            ? new SKColor(210, 40, 45)
                            : (dark ? SKColors.White : SKColors.Black).WithAlpha(
                                surface.TitlebarPressed == button ? (byte)55 : (byte)30
                            );
                    canvas.DrawRect(left, 0, 46, height, ink);
                }
                ink.Color =
                    button == 3 && surface.TitlebarHovered == button ? SKColors.White : foreground;
                ...
                if (button == 1)
                {
                    canvas.DrawLine(x, y + 9, x + 10, y + 9, ink);
                }
                else if (button == 2)
                {
                    if (maximized)
                    {
                        canvas.DrawRect(x + 2, y - 2, 8, 8, ink);
                    }
