...
internal sealed partial class QtPlatformViewHost : IPlatformViewDispatcher, IDisposable
{
    ...
    internal unsafe struct Api
    {
        ...
        public delegate* unmanaged[Cdecl]<
            ulong,
            delegate* unmanaged[Cdecl]<nint, int, void>,
            nint,
            int> Post;
        public delegate* unmanaged[Cdecl]<
            ulong,
            uint,
            QtNativeV2.Utf8,
            delegate* unmanaged[Cdecl]<nint, ulong, void>,
            nint,
            ulong*,
            int> Create;
    ...
    private sealed record QuickRaster(
    ...
    private static readonly PlatformEffectSupport QuickEffects = new(
        true,
        1,
        32,
        Saturation: true,
    ...
    internal unsafe void Bind(
        nint window,
        Action yieldText,
        Action<Exception> fatal,
        Action<Action> dispatchFocus
    )
    {
        if (_owner != 0)
        {
            throw new InvalidOperationException("Qt PlatformView owner is already bound.");
        }
    ...
    internal static void Check(int result)
    {
        if (result != 0)
        {
            throw new InvalidOperationException(
                $"Qt PlatformView operation rejected (status {result})."
            );
        }
    }
    ...
    private sealed class Factory(QtPlatformViewHost host, uint kind) : IPlatformViewFactory
    {
        public string ViewType =>
            kind switch
            {
                0 => "doroti/native-button",
                1 => "doroti/native-editor",
                _ => "doroti/webview",
            };

        public PlatformViewSupport QuerySupport(PlatformViewRequest request) =>
            new(
                host.IsQuick ? "linux/qt-quick-gpu" : "linux/qt-native-child-widgets",
                "Qt6",
                ViewType,
                !host._closed
                    && host._owner != 0
                    && request.ViewType == ViewType
                    && (kind != 2 || host.HasWebEngine)
                    && (
                ...
                host.IsQuick ? request.Composition : PlatformViewComposition.NativeOverlay,
                PlatformViewEffects.RectClip,
                NativeBackdropBlur: host.HasEffects,
                WebViewCommands: kind == 2 && host.HasWebEngine,
                Capabilities: new(
                    ...
                    host.HasEffects ? QuickEffects : PlatformEffectSupport.Unsupported
                ),
                Reason: kind == 2 && !host.HasWebEngine
                        ? "WebEngine Quick requires DorotiQtQuick=true and DorotiQtWebEngine=true; rebuild the native shim."
                    : host.IsQuick
                        ? "Live Qt Quick items and Graphite Vulkan GPU images; translation and rect clip. Physical presentation atomicity is not qualified."
        ...
        )
        {
            ...
            if (kind == 2 && !host.HasWebEngine)
            {
                throw new WebViewException(
                    ...
                    "Qt WebEngine Quick is disabled."
                );
            }

            var bytes =
                kind == 2
                    ? await QtWebViewSession
                        .PrepareAsync(
                            parameters,
                            host.Resources,
                            host.ApplicationId,
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                result = Create(handle, focused, bytes);
                return ValueTask.CompletedTask;
            });
            return result!;
        }

        private unsafe IPlatformViewInstance Create(
            PlatformViewHandle handle,
            Action<PlatformViewHandle> focused,
            byte[] bytes
        )
        {
            ...
            {
                ulong id;
                var status = host._api.Create(
                    host._owner,
                    kind,
                    new(text, (ulong)bytes.Length),
                    &Focused,
            ...
            try
            {
                if (kind == 2)
                {
                    instance.Web = new QtWebViewSession(
                        host,
                        handle,
                        host._owner,
                        instance.Id,
            ...
            {
                host._api.Remove(host._owner, instance.Id);
                throw;
            }
    ...
    ) : IPlatformViewInstance, IPlatformWebViewInstance
    {
        ...
        public Task<WebViewResult> ExecuteAsync(
            ...
            Web?.ExecuteAsync(command, cancellationToken)
            ?? throw new WebViewException(WebViewError.Unsupported, "This item is not a WebView.");

        internal void NotifyFocus()
        {
            host._yieldText?.Invoke();
            focused(handle);
        }
        ...
        public ValueTask ApplyAsync(PlatformViewPlacement placement)
        {
            ...
            {
                var next = host
                    ._visible.Where(p => p.Id != Id)
                    .Append(Translate(placement, Id, host.IsQuick))
                    .ToArray();
                host.Commit(next, apply: true);
                host._visible = next;
                host._visibleHandles = host
                    ._visibleHandles.Where(h => h != handle)
                    .Append(handle)
                    .ToArray();
            }
            return ValueTask.CompletedTask;
        ...
        public unsafe ValueTask SetFocusAsync(bool focused)
        {
            ...
            {
                host.Verify();
                Check(host._api.Focus(host._owner, Id, focused ? 1u : 0u));
            }
            return ValueTask.CompletedTask;
        }
        ...
        {
            Web?.Close(host._closed);
            Web = null;
            WebViewChanged = null;
            if (!host._closed)
            {
                host.Verify();
                Check(host._api.Remove(host._owner, Id));
            }
            host._instances.Remove(handle);
            host._nativeInstances.Remove(Id);
            return ValueTask.CompletedTask;
        }
    }
    ...
    private static void Focused(nint context, ulong id)
    {
        ...
        try
        {
            ...
            if (!host._closed && host._nativeInstances.TryGetValue(id, out var instance))
            {
                host._dispatchFocus?.Invoke(instance.NotifyFocus);
            }
        }
        catch (Exception error)
        {
            host?._fatal?.Invoke(error);
        }
    }
    ...
    private static void RunWork(nint context, int result)
    {
        ...
        try
        {
            ...
            {
                _ = CompleteWorkAsync(task, work.Completion);
            }
        }
        catch (Exception error)
        {
    ...
    internal static Placement Translate(PlatformViewPlacement p, ulong id, bool quick = false)
    {
        ...
        NativeRect Convert(Rect r, bool inward)
        {
            foreach (var value in new[] { r.left, r.top, r.right, r.bottom })
            {
                ...
                {
                    throw new NotSupportedException(
                        "Qt native child Widgets require finite geometry within 1,000,000 logical pixels."
                    );
                }
            }
            ...
            if (quick)
            {
                return new(r.left, r.top, Math.Max(0, r.width), Math.Max(0, r.height));
            }
        ...
        {
            Size = 80,
            Visible = p.Visible && !clip.isEmpty ? 1u : 0u,
            Id = id,
            Bounds = Convert(bounds, false),
            Clip = clip.isEmpty ? default : Convert(clip, true),
        };
    }

    private unsafe void Commit(Placement[] placements, bool apply)
    {
        fixed (Placement* items = placements)
        {
            Check(_api.Commit(_owner, items, (ulong)placements.Length, apply ? 1u : 0u));
        }
    }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("Qt PlatformView has an unretired frame.");
        }
        ...
        try
        {
            if (
                ...
                && plan.Parts.OfType<PlatformShieldSegment>().Any()
            )
            {
                throw new NotSupportedException(
                    "Qt NativeOverlay does not support input shields; interleaving is required."
                );
            }
            ...
            var placements = plan
                ...
                .ToArray();
            var native = placements
                .Select(p => Translate(p, _instances[p.Handle].Id, IsQuick))
                .ToArray();
            if (_reservation is null || placements.Any(p => !_reservation.Contains(p.Handle)))
            {
                throw new InvalidOperationException(
                    "Qt PlatformView frame references an unreserved instance."
                );
            }
            ...
            if (IsQuick)
            {
                DrawQuick(renderer, plan, width, height, descriptor);
            }
            else
            {
    ...
    )
    {
        var surface =
            QuickSurface
            ?? throw new InvalidOperationException("Qt Quick surface is not configured.");
        var gpu =
            surface.QuickGpu
            ?? throw new InvalidOperationException("Qt Quick GPU is not initialized.");
        if (!ReferenceEquals(_quickCacheOwner, gpu))
        {
            _quickRasters.Clear();
            _quickCacheOwner = gpu;
        }
        _nextQuickRasters = [];
        ...
        var rasterIndex = 0;
        var viewport = new NativeRect(
            0,
            0,
            width / descriptor.DeviceScaleX,
            height / descriptor.DeviceScaleY
        );
        foreach (var part in plan.Parts)
        {
            switch (part)
            {
                case PlatformRasterSegment raster:
                {
                    ...
                    if (
                        !_quickRasters.TryGetValue(raster.PaintOrder, out var old)
                    ...
                    )
                    {
                        ...
                        if (rasterIndex == 0)
                        {
                            target.DrawColor(renderer.PlatformBackgroundColor);
                        }
                    ...
                    var logicalBounds = new NativeRect(
                        bounds.Left / descriptor.DeviceScaleX,
                        bounds.Top / descriptor.DeviceScaleY,
                        bounds.Width / descriptor.DeviceScaleX,
                        bounds.Height / descriptor.DeviceScaleY
                    );
                    _nextQuickRasters.Add(
                        raster.PaintOrder,
                        new(scope, slice, gpu.Identity(rasterIndex))
                    );
                    parts.Add(
                        new()
                        {
                            Size = 96,
                            Kind = 0,
                            Id = gpu.Identity(rasterIndex),
                            Image = gpu.Image(rasterIndex++),
                            PixelWidth = (uint)bounds.Width,
                            PixelHeight = (uint)bounds.Height,
                ...
                case PlatformNativeSegment native:
                    var placement = Translate(
                        native.Placement,
                        _instances[native.Placement.Handle].Id,
                        quick: true
                    );
                    parts.Add(
                        new()
                        {
                            ...
                            Clip = placement.Visible != 0 ? placement.Clip : default,
                        }
                    );
                    break;
                case PlatformBackdropSegment effect:
                    if (effect.SigmaX != effect.SigmaY)
                    {
                        throw new NotSupportedException(
                            "Qt Quick backdrop requires isotropic Gaussian blur."
                        );
                    }
                    ...
                    parts.Add(
                        new()
                        {
                            Size = 96,
                            Kind = 4,
                            Id = BitConverter.DoubleToUInt64Bits(effect.SigmaX),
                ...
                case PlatformShieldSegment shield:
                    ...
                    {
                        throw new NotSupportedException(
                            "Qt Quick input shields require axis-aligned bounds."
                        );
                    }

                    var a = t.Map(shield.Shield.Bounds.topLeft);
                    var b = t.Map(shield.Shield.Bounds.bottomRight);
                    var shieldBounds = new Rect(a.dx, a.dy, b.dx, b.dy);
                    var clip = shield.Shield.Clip is { } c
                        ? shieldBounds.intersect(c)
                        : shieldBounds;
                    parts.Add(
                        new()
                        {
                            ...
                            Clip = new(
                                clip.left,
                                clip.top,
                                Math.Max(0, clip.width),
                                Math.Max(0, clip.height)
                            ),
                        }
                    );
                    break;
                default:
                    throw new NotSupportedException(
                        $"Unsupported Qt Quick composition part {part.GetType().Name}."
                    );
            }
        }
    ...
    {
        if (!presented || _pending is null || _closed)
        {
            FinishFrameCore(presented);
            return;
        }
        var plan = _pending;
        _session ??= new(plan.Token.OwnerViewId);
        if (
            _session.CommitRetiredFrame(
                plan,
                () =>
                {
                    FinishFrameCore(presented);
    ...
    private void FinishFrameCore(bool presented)
    {
        ...
        try
        {
            ...
            foreach (var placement in _nextPlacements)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Qt placement unexpectedly suspended on GUI thread."
                    );
                }
            ...
            foreach (
                var old in _visibleHandles.Where(h =>
                    !nextHandles.Contains(h) && _reservation!.Contains(h)
                )
            )
            {
            ...
            if (IsQuick)
            {
                _quickRasters = _nextQuickRasters;
            }
        }
        finally
        {
    ...
    private static partial int GetApi(
        nint view,
        uint version,
        uint size,
        out ulong owner,
