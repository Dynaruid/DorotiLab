...
internal static partial class QtQuickNative
{
    ...
    internal struct Part
    {
        ...
        internal uint PixelWidth,
            PixelHeight;
        internal QtPlatformViewHost.NativeRect Bounds,
            Clip;
    }
    ...
    private static unsafe partial int CommitParts(
        nint window,
        Part* parts,
        ulong count,
        uint apply
    ...
    internal static unsafe Gpu Get(nint window)
    {
        ...
        if (gpu.Instance == 0 || gpu.Physical == 0 || gpu.Device == 0 || gpu.Queue == 0)
        {
            throw new InvalidDataException("Qt Quick GPU descriptor is incomplete.");
        }
    ...
    {
        fixed (Part* data = parts)
        {
            QtPlatformViewHost.Check(
                CommitParts(window, data, (ulong)parts.Length, apply ? 1u : 0u)
            );
        }
    }
}
