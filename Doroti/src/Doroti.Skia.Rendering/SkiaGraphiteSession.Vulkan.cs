...
public sealed partial class SkiaGraphiteSession
{
    ...
    public void TakeVulkanShutdownOwnershipAfterGpuDrain()
    {
        ...
        if (_vulkanOwner is null)
        {
            throw new InvalidOperationException("Shutdown handoff requires a Vulkan session.");
        }
    ...
    public void NotifyVulkanDeviceLost()
    {
        ...
        if (_vulkanOwner is null)
        {
            throw new InvalidOperationException("This is not a Vulkan session.");
        }
        ...
        if (!_vulkanOwner.ReportDeviceLost(_context.Handle))
        {
            throw new InvalidOperationException(
                "Could not forward external Vulkan device loss to Graphite."
            );
        }
    }

    /// <summary>Official asset path. Host state observation and safe retirement are mandatory.</summary>
    public static SkiaGraphiteSession CreateVulkan(
        SkiaGraphiteVulkanOptions options,
        long generation,
        int maxFrames = 3
    )
    {
        ...
        try
        {
            return new(owner.Context, generation, maxFrames) { _vulkanOwner = owner };
        }
        catch
        {
            owner.Dispose();
    ...
    public VulkanTarget CreateVulkanTarget(
        int width,
        int height,
    ...
    )
    {
        ...
        if (image == 0)
        {
            throw new ArgumentException("A live Vulkan image is required.", nameof(image));
        }

        var backend =
            SKGraphiteBackendTexture.CreateVulkan(width, height, info, layout, queueFamily, image)
        ...
        try
        {
            ...
            surface =
                SKSurface.Create(_recorder, backend, colorType)
                ?? throw new InvalidOperationException(
                    "Graphite Vulkan surface creation failed; verify format and INPUT_ATTACHMENT usage."
                );
            var target = new VulkanTarget(
                this,
                backend,
                SkiaGpuSurfaces.Register(surface, _recorder)
            );
        ...
        {
            surface?.Dispose();
            _vulkanOwner.UntrackTarget(backend.Handle);
            backend.Dispose();
            throw;
        }
    }
    ...
    public Frame BeginVulkanFrame(VulkanTarget target)
    {
        ...
        {
            throw new InvalidOperationException(
                "Vulkan target belongs to another generation, is busy, or the session cannot admit a frame."
            );
        }
    ...
    public sealed class VulkanTarget : IDisposable
    {
        ...
        public void SetStateAfterGpuCompletion(int layout, uint queueFamily)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Complete GPU work before updating the persistent target state."
                );
            }
        ...
        public void Dispose()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Return the Vulkan frame before disposing its target."
                );
            }
