...
internal static class SkiaRSuperellipsePath
{
    internal static SKPath Create(RSuperellipse shape)
    {
        ...
        if (r.isEmpty || !r.IsFinite)
        {
            return path.Detach();
        }

        var radii = new[] { shape.tlRadius, shape.trRadius, shape.brRadius, shape.blRadius }
            .Select(radius => radius.x <= 0 || radius.y <= 0 ? Radius.zero : radius)
            .ToArray();
        var scale = 1.0;
        ...
        {
            radii[i] = Radius.elliptical(radii[i].x * scale, radii[i].y * scale);
        }
        ...
        void Limit(double dimension, double sum)
        {
            if (sum > 0)
            {
                scale = Math.Min(scale, dimension / sum);
            }
        }
    }
    ...
    private static double Split(double start, double end, double first, double second) =>
        first + second == 0
            ? (start + end) / 2
            : ((start * second) + (end * first)) / (first + second);
    ...
    private static void Octant(
        SKPathBuilder path,
        Func<Offset, Offset> external,
        Offset offset,
        double a,
        double radius,
        bool reverse,
    ...
    {
        SKPoint Transform(Offset p) =>
            Point(external(offset + (flip ? new Offset(p.dy, p.dx) : p)));
        ...
        var center =
            ((j + m) / 2)
            - (
                new Offset(-chord.dy, chord.dx)
                / chord.distance
                * Math.Sqrt(
                    Math.Max(0, (circleRadius * circleRadius) - (chord.distanceSquared / 4))
                )
            );
        var start = j - center;
        var mid = m - center;
        var angle = Math.Atan2(
            (mid.dx * start.dy) - (mid.dy * start.dx),
            (mid.dx * start.dx) + (mid.dy * start.dy)
        );
        var end = new Offset(
    ...
    {
        if (Math.Abs(slope - otherSlope) < 1e-5)
        {
            return (p + q) / 2;
        }
    ...
    private static (double N, double XRatio) Parameters(double ratio)
    {
        ...
        {
            return (
                (1.559599389 * (ratio - 5)) + 6.43023796,
    ...
    private static (double W1, double W2, double YRatio) BezierFactors(
        double n,
        double xRatio,
        double yRatio
    )
    {
        ...
        return (
            Lerp(first.First, second.First, steps - index) * root,
            Lerp(first.Second, second.Second, steps - index) * xRatio,
            (root + yRatio) / (root + 1)
        );
    }
