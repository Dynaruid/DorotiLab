...
internal sealed class SkiaGraphiteVulkanInterop : SkiaGraphiteVulkanBinding
{
    ...
    internal SkiaGraphiteVulkanInterop(SkiaGraphiteVulkanOptions options)
    {
        ...
        if (!SKGraphiteContext.IsBackendAvailable(SKGraphiteBackend.Vulkan))
        {
            throw new PlatformNotSupportedException(
                "Selected Skia asset has no Graphite Vulkan backend."
            );
        }
        ...
        {
            VkInstance = options.Instance,
            VkPhysicalDevice = options.PhysicalDevice,
            VkDevice = options.Device,
            VkQueue = options.Queue,
            GraphicsQueueIndex = options.QueueFamily,
            MaxApiVersion = options.MaxApiVersion,
            GetProcedureAddress = (name, instance, device) =>
            {
                try
                {
                    var pointer = options.GetProcedure(name, instance, device);
                    if (pointer == 0 && unavailableProcedures.Count < 32)
                    {
                        unavailableProcedures.Add(name);
                    }
        ...
        var context = SKGraphiteContext.CreateVulkan(
            backend,
            new SKGraphiteContextOptions
            {
                GpuBudgetInBytes = SkiaGraphiteSession.ContextBudgetBytes,
            }
        );
        if (context is null)
        {
            ...
            throw new InvalidOperationException(
                "Official Graphite Vulkan context creation failed. Unavailable procedures: "
                    + string.Join(", ", unavailableProcedures),
                _dispatchFailure
            );
        }
        Context = context;
    ...
    internal override void CheckHostState()
    {
        ...
        {
            throw new InvalidOperationException(
                "Vulkan procedure callback failed.",
                _dispatchFailure
            );
        }
        ...
        {
            throw new InvalidOperationException(
                "Host observed Vulkan device loss; generation is permanently faulted."
            );
        }
    ...
    internal override bool Insert(
        nint context,
        nint recording,
        ReadOnlySpan<ulong> waits,
        ReadOnlySpan<ulong> signals
    )
    {
        CheckHostState();
        if (context != Context.Handle)
        {
            throw new InvalidOperationException("Context identity mismatch.");
        }

        if (!waits.IsEmpty || !signals.IsEmpty)
        {
            throw new NotSupportedException(
                "Official Graphite frames require host-owned same-queue semaphore submissions."
            );
        }

        var result = Context.InsertRecording(
            new SKGraphiteInsertRecordingInfo { Recording = recording }
        );
        CheckHostState();
    ...
    internal override void TrackTarget(nint texture, nint image, int layout, uint family)
    {
        ...
        if (
            state.QueueFamily != _options.QueueFamily
            || state.QueueFamily != family
            || state.Layout != layout
        )
        {
            throw new InvalidOperationException(
                "Target wrap state must match its observed initial layout and Graphite queue family."
            );
        }
    ...
    public override void Dispose()
    {
        ...
        {
            throw new InvalidOperationException(
                "Return persistent targets before disposing their official context."
            );
        }
