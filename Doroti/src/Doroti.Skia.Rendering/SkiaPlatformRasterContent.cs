...
public static class SkiaPlatformRasterContent
{
    ...
    public static SKRectI Coverage(IReadOnlyList<SceneCommand> commands, int width, int height)
    {
        ...
        return slices.Count == 0
            ? default
            : new(
                slices.Min(s => s.Bounds.Left),
                slices.Min(s => s.Bounds.Top),
                slices.Max(s => s.Bounds.Right),
                slices.Max(s => s.Bounds.Bottom)
            );
    }
    ...
    public static IReadOnlyList<SKRectI> SubtractOpaque(SKRectI bounds, SKRectI opaque)
    {
        ...
        var left = Math.Max(bounds.Left, opaque.Left);
        var top = Math.Max(bounds.Top, opaque.Top);
        var right = Math.Min(bounds.Right, opaque.Right);
        var bottom = Math.Min(bounds.Bottom, opaque.Bottom);
        ...
        if (bounds.Top < top)
        {
            remaining.Add(new(bounds.Left, bounds.Top, bounds.Right, top));
        }

        if (bottom < bounds.Bottom)
        {
            remaining.Add(new(bounds.Left, bottom, bounds.Right, bounds.Bottom));
        }

        if (bounds.Left < left)
        {
            remaining.Add(new(bounds.Left, top, left, bottom));
        }

        if (right < bounds.Right)
        {
            remaining.Add(new(right, top, bounds.Right, bottom));
        }
    ...
    public static IReadOnlyList<Slice> Split(
    ...
    )
    {
        ...
        foreach (var command in commands)
        {
            if (command.Operation == "pop")
            {
                if (stack.Count == 0)
                {
                    throw new InvalidDataException("Unbalanced platform raster scope.");
                }
            ...
            {
                if (
                    command.HostPayload is ScenePicturePayload picture
                    && !PictureHasDrawing(picture)
                )
                {
                    continue;
                }
                ...
                var bounds = new SKRectI(
                    Math.Max(0, (int)Math.Floor(clip.left) - 2),
                    Math.Max(0, (int)Math.Floor(clip.top) - 2),
                    Math.Min(width, (int)Math.Ceiling(clip.right) + 2),
                    Math.Min(height, (int)Math.Ceiling(clip.bottom) + 2)
                );
                slices.Add(
                    new(
                        scopes
            ...
            switch (command.HostPayload)
            {
                case SceneOffsetPayload offset:
                    state.Transform = state.Transform?.ThenLocal(
                        new(1, 0, 0, 1, offset.Dx, offset.Dy)
                    );
                    break;
                case SceneTransformPayload transform:
                    ...
                    break;
                case SceneClipRectPayload rect
                    when rect.Behavior != Clip.none && state.Transform is { } matrix:
                    var points = new[]
                    {
                        matrix.Map(rect.Rect.topLeft),
                        matrix.Map(rect.Rect.topRight),
                        matrix.Map(rect.Rect.bottomLeft),
                        matrix.Map(rect.Rect.bottomRight),
                    };
                    var mapped = new Rect(
                        points.Min(p => p.dx),
                        points.Min(p => p.dy),
                    ...
                    if (mapped.isFinite)
                    {
                        state.Clip = state.Clip.intersect(mapped);
                    }
        ...
        if (stack.Count != 0)
        {
            throw new InvalidDataException("Unclosed platform raster scope.");
        }
        ...
        foreach (var slice in slices)
        {
            ...
            if (compact.Count != 0)
            {
                ...
                var aa = (long)a.Width * a.Height;
                var ba = (long)b.Width * b.Height;
                if (
                    a == b
                    || (
                        aa >= (long)width * height / 4
                        && ba >= (long)width * height / 4
    ...
    public static bool HasDrawing(IReadOnlyList<SceneCommand> commands) =>
        commands.Any(command =>
            command.Operation switch
            {
                ...
                or "opacity"
                or "pop" => false,
                "picture" when command.HostPayload is ScenePicturePayload picture =>
                    PictureHasDrawing(picture),
                _ => true,
            }
        );
    ...
    public static bool Equivalent(
        ...
        IReadOnlyList<SceneCommand> next
    )
    {
        ...
        for (var i = 0; i < previous.Count; i++)
        {
            ...
            var equal = (a.Operation, a.HostPayload, b.HostPayload) switch
            {
                ...
                (
                    "clipRSuperellipse",
                    SceneClipRSuperellipsePayload x,
                    SceneClipRSuperellipsePayload y
                ) => x == y,
                ("transform", SceneTransformPayload x, SceneTransformPayload y) =>
                    x.Matrix4.SequenceEqual(y.Matrix4),
                ("opacity", SceneOpacityPayload x, SceneOpacityPayload y) => x == y,
                ("picture", ScenePicturePayload x, ScenePicturePayload y) => !x.WillChangeHint
                    && !y.WillChangeHint
                    && x.SnapshotIdentity == y.SnapshotIdentity
                    && x.Offset == y.Offset,
                _ => false,
            };
            if (!equal)
    ...
    private readonly record struct LocalPicture(
        long Identity,
        PlatformViewTransform Transform,
        Rect Clip
    );
    ...
    private static List<LocalPicture>? LocalPictures(Slice slice)
    {
        ...
        (PlatformViewTransform Transform, Rect Clip) state = (
            new(1, 0, 0, 1, -bounds.Left, -bounds.Top),
        ...
        foreach (var command in slice.Commands)
        {
            ...
            if (
                command.Operation == "picture"
                && command.HostPayload is ScenePicturePayload picture
            )
            {
                if (picture.WillChangeHint)
                {
                    return null;
                }
                ...
                if (PictureHasDrawing(picture) && !state.Clip.isEmpty)
                {
                    pictures.Add(
                        new(
                            ...
                            state.Transform.ThenLocal(
                                new(1, 0, 0, 1, picture.Offset.dx, picture.Offset.dy)
                            ),
                            state.Clip
                        )
                    );
            ...
            if (!state.Transform.IsFinite || !state.Transform.IsAxisAligned || !state.Clip.isFinite)
            {
                return null;
            }
        }
