...
public sealed class SkiaPlatformOverlayPool : IDisposable
{
    ...
    public SkiaPlatformOverlayPool(
        int maximumSurfaces = 17,
    ...
    )
    {
        ...
        _maximumSurfaces = maximumSurfaces;
        _maximumBytes = maximumBytes;
        _maximumIdle = Math.Min(maximumIdle, maximumSurfaces);
    }
    ...
    public Lease Rent(int width, int height, long allocationBytes, Func<SKSurface> create)
    {
        ...
        if (allocationBytes < checked((long)width * height * 4) || allocationBytes > _maximumBytes)
        {
            throw new InvalidOperationException(
                "Overlay allocation exceeds the byte budget or under-reports its pixel storage."
            );
        }
        ...
        for (var index = 0; index < _idle.Count; index++)
        {
            ...
            if (
                ...
                || candidate.Bytes != allocationBytes
            )
            {
                continue;
            }
        ...
        while (
            _idle.Count != 0
            && (LiveSurfaces >= _maximumSurfaces || ReservedBytes > _maximumBytes - allocationBytes)
        )
        {
            var slot = _idle[0];
            _idle.RemoveAt(0);
        ...
        {
            throw new InvalidOperationException(
                "Overlay pool capacity is still held by unretired frames."
            );
        }

        var surface =
            create() ?? throw new InvalidOperationException("Overlay surface allocation failed.");
        LiveSurfaces++;
    ...
    public void Invalidate(long contextGeneration)
    {
        VerifyThread();
        ObjectDisposedException.ThrowIf(_disposed, this);
        if (contextGeneration <= _generation)
        {
            throw new InvalidOperationException("Overlay context generation must advance.");
        }

        _generation = contextGeneration;
        foreach (var slot in _idle)
        {
            Destroy(slot);
        }
    ...
    public sealed class Lease : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            if (
                ...
                || owner._idle.Count >= owner._maximumIdle
            )
            {
                owner.Destroy(slot);
            }
    ...
    private void VerifyThread()
    {
        ...
        {
            throw new InvalidOperationException(
                "Overlay pool operations belong to the render owner thread."
            );
        }
    }
}
