...
public sealed class DisplayListEncodingCache
{
    ...
    private readonly Dictionary<DisplayListCommand, byte[]> _identities = new(
    ...
    internal bool TryGetStringTable(
        IReadOnlyList<DisplayListCommand> commands,
        out List<(string Value, byte[] Bytes)> strings,
        out Dictionary<string, uint> ids
    )
    {
        strings = null!;
        ids = null!;
        if (_stringRecipes is null)
        {
            return false;
        }
        ...
        {
            if (
                command is DisplayDrawParagraphCommand paragraph
                && (
                    index >= _stringRecipes.Length
                    || !SameStrings(_stringRecipes[index++], paragraph.Paragraph)
                )
            )
            {
                return false;
        ...
        if (index != _stringRecipes.Length)
        {
            return false;
        }
    ...
    private static bool SameStrings(DisplayParagraphRecipe a, DisplayParagraphRecipe b)
    {
        ...
        if (
            ...
            || a.TextRuns.Count != b.TextRuns.Count
        )
        {
            return false;
        }
        ...
        for (var i = 0; i < a.TextRuns.Count; i++)
        {
            ...
            if (
                ...
                || x.FontFeatures.Count != y.FontFeatures.Count
                || x.FontVariations.Count != y.FontVariations.Count
            )
            {
                return false;
            }

            for (var j = 0; j < x.FontFeatures.Count; j++)
            {
                if (x.FontFeatures[j].Name != y.FontFeatures[j].Name)
                {
                    return false;
                }
            }

            for (var j = 0; j < x.FontVariations.Count; j++)
            {
                if (x.FontVariations[j].Axis != y.FontVariations[j].Axis)
                {
                    return false;
                }
            }
    ...
    internal void RememberStringTable(
        IReadOnlyList<DisplayListCommand> commands,
    ...
    )
    {
        ...
        _stringTable = null;
        _stringIds = null;
        if (strings.Sum(s => 64L + (s.Value.Length * 2L) + s.Bytes.Length) > 2 * 1024 * 1024)
        {
            return;
        }
            ...
            foreach (var run in recipe.TextRuns)
            {
                charge +=
                    ...
                    + (
                        128L
                        * (
                            (long)run.Shadows.Count
                            + run.FontFeatures.Count
                            + run.FontVariations.Count
                            + run.FontFamilyFallback.Count
                        )
                    );
            }

            if (charge > 2 * 1024 * 1024 || recipes.Count >= 512)
            {
                return;
            }
        ...
        _stringRecipes = recipes.ToArray();
    ...
    internal void BeginFrame()
    {
        ...
        FrameMisses = 0;
        FrameBlockHits = 0;
        FrameStringTableHit = false;
    }

    internal void SetTables(
        IReadOnlyList<(string Value, byte[] Bytes)> strings,
    ...
    )
    {
        ...
        _tablesCacheable =
            strings.Sum(s => ((long)s.Value.Length * 2) + 32) <= 2 * 1024 * 1024
            && resources.Count <= 1024;
        var same =
            _tablesCacheable
            && _strings.Length == strings.Count
            && _resources.Length == resources.Count;
        for (var i = 0; same && i < strings.Count; i++)
        {
            same = _strings[i] == strings[i].Value;
        }

        for (var i = 0; same && i < resources.Count; i++)
        {
            same = _resources[i] == resources[i];
        }
        ...
        {
            foreach (
                var entry in _payloads.Where(p => p.Key is DisplayDrawParagraphCommand).ToArray()
            )
            {
                _payloads.Remove(entry.Key);
                _identities.Remove(entry.Key);
                RetainedBytes -= entry.Value.Charge;
            }
        }
    ...
    )
    {
        ...
        if (
            !_tablesCacheable
            || !_blocks.TryGetValue(start, out var block)
            || block.Commands.Length != count
        )
        {
            return false;
        }
        ...
        {
            if (
                !ReferenceEquals(block.Commands[i], commands[start + i])
                && !block.Commands[i].Equals(commands[start + i])
            )
            {
                return false;
            }
    ...
    {
        if (!_tablesCacheable || count < 8 || start >= 8192)
        {
            return;
        }

        if (_blocks.Remove(start, out var old))
        {
            _blockBytes -= old.Charge;
        }

        long charge = 128 + (count * (long)KeyCharge) + (bytes.Length * 8L);
        for (var i = 0; i < count; i++)
        {
            if (!Eligible(commands[start + i]))
            {
                return;
            }
        ...
        while (_blockBytes + charge > 4 * 1024 * 1024)
        {
            ...
            _blockBytes -= first.Value.Charge;
        }
        var keys = new DisplayListCommand[count];
        for (var i = 0; i < count; i++)
        {
            keys[i] = commands[start + i];
        }

        _blocks.Add(start, new(keys, bytes.ToArray(), (int)charge));
        _blockBytes += (int)charge;
    }
    ...
    {
        if (!_tablesCacheable || !Eligible(command))
        {
            return;
        }

        if (payload.Length > (MaximumChargedBytes - KeyCharge) / 8)
        {
            return;
        }

        var fullCharge = KeyCharge + ((long)payload.Length * 8) + TextCharge(command);
        if (fullCharge > MaximumChargedBytes)
        {
            return;
        }
    ...
    {
        if (command is not DisplayDrawParagraphCommand c)
        {
            return 0;
        }
        ...
        var p = c.Paragraph;
        long chars =
            (long)p.Text.Length + p.FontFamily.Length + p.Locale.Length + (p.Ellipsis?.Length ?? 0);
        foreach (var r in p.TextRuns)
        {
            chars += (long)r.Text.Length + r.FontFamily.Length + r.Locale.Length;
            foreach (var f in r.FontFamilyFallback)
            {
                chars += f.Length;
            }

            foreach (var f in r.FontFeatures)
            {
                chars += f.Name.Length;
            }

            foreach (var v in r.FontVariations)
            {
                chars += v.Axis.Length;
            }
        }
    ...
    public void Clear()
    {
        ...
        _strings = [];
        _resources = [];
        _tablesCacheable = true;
        RetainedBytes = 0;
        CommandCapacityHint = 256;
        BeginFrame();
    }

    private static bool Eligible(DisplayListCommand command) =>
        command switch
        {
            DisplaySaveCommand
            or DisplayRestoreCommand
            or DisplayTransformCommand
            or DisplayClipRectCommand
            or DisplayClipRoundedRectCommand
            or DisplayDrawColorCommand => true,
            ...
            DisplayClipPathCommand or DisplayDrawShadowCommand or DisplayDrawParagraphCommand =>
                true,
    ...
    private static bool Simple(DisplayPaint paint) =>
        paint is not null
        && paint.Shader is null
        && paint.ColorFilter is null
        && paint.MaskFilter is null
