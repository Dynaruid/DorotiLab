    ...
    where T : notnull
{
    ...
    public SegmentedButton(
        Key? key = null,
        List<ButtonSegment<T>> segments = default!,
        HashSet<T> selected = default!,
        Action<HashSet<T>>? onSelectionChanged = null,
    ...
    )
    {
        WidgetStateProperty<Color?>? overlayColorProp =
            (
                ...
                && (selectedForegroundColor is null)
                ...
                : (
                    overlayColor switch
                    {
                        Color overlayColorLocal when overlayColorLocal.value == 0L =>
                            DartRuntimePrimitives.ConvertValue<WidgetStateProperty<Color?>>(
                                new WidgetStatePropertyAll<Color?>(Colors.transparent)
                            ),
                        _ => _SegmentedButtonDefaultsM3__segmented_button.resolveStateColor(
                            foregroundColor,
        ...
        return TextButton
            ...
            .copyWith(
                foregroundColor: SegmentedButton<T>._defaultColor(
                    foregroundColor,
                    disabledForegroundColor,
                    selectedForegroundColor
                ),
                backgroundColor: SegmentedButton<T>._defaultColor(
                    backgroundColor,
        ...
        return (WidgetStateProperty<Color?>?)
            WidgetStateProperty<Color?>.CreateFromMap(
                new DartMap<WidgetStatesConstraint, Color?>
                {
                    ...
                    [WidgetState.selected.asConstraint()] = selected,
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new SegmentedButtonState<T>());
}

public class SegmentedButtonState<T> : State<SegmentedButton<T>>
    where T : notnull
{
    internal virtual bool _hovering { get; set; } = false;
    internal virtual bool _focused { get; set; } = false;
    public virtual DartMap<ButtonSegment<T>, WidgetStatesController> statesControllers
    {
        get;
        private set;
    } = new DartMap<ButtonSegment<T>, WidgetStatesController>();

    internal virtual bool _enabled =>
        DartRuntimePrimitives.ConvertValue<bool>(widget.onSelectionChanged is not null);
    ...
    internal virtual HashSet<WidgetState> _states =>
        (
            (Func<HashSet<WidgetState>>)(
                () =>
                {
                    var __collection16615 = new HashSet<WidgetState>();
                    if (!_enabled)
                    {
                        __collection16615.Add(WidgetState.disabled);
                    }
                    if (_hovering)
                    {
                        __collection16615.Add(WidgetState.hovered);
                    }
                    if (_focused)
                    {
                        __collection16615.Add(WidgetState.focused);
                    }
                    if (_selected)
                    {
                        __collection16615.Add(WidgetState.selected);
                    }
                    return __collection16615;
                }
            )
        )();
    ...
    internal virtual void _handleOnPressed(T segmentValue)
    {
        ...
        bool onlySelectedSegment =
            (checked(widget.selected.Count) == 1L) && widget.selected.Contains(segmentValue);
        ...
        if (validChange)
        {
            ...
            if (toggle)
            {
                updatedSelection = widget.selected.Contains(segmentValue)
                    ...
                    : widget.selected.Union(pressedSegment).ToHashSet();
            }
            else
            {
    ...
    public override Widget build(BuildContext context)
    {
        ...
        SegmentedButtonThemeData defaults = new _SegmentedButtonDefaultsM3__segmented_button(
            context
        );
        ...
        P? resolve<P>(
            ...
            HashSet<WidgetState>? states = null
        )
        {
                ...
                (style) =>
                    DartRuntimePrimitives.NullAware(
                        ...
                        __target =>
                            __target.resolve(
                                states
                                    ?? (HashSet<global::Doroti.Framework.Widgets.WidgetState>)
                                        _states
                            )
                    )
            );
        ...
        Widget? selectedIconLocal = widget.showSelectedIcon
        ...
        Widget buttonFor(ButtonSegment<T> segment)
        {
            ...
            Widget? iconLocal =
                (segmentSelected && widget.showSelectedIcon)
                    ? selectedIconLocal
                    : ((segment.label is not null) ? segment.icon : null);
            WidgetStatesController controller = statesControllers.putIfAbsent(
                segment,
            ...
            if (iconLocal is not null)
            {
                double defaultFontSize =
                    segmentStyle.textStyle?.resolve(new HashSet<WidgetState>())?.fontSize ?? 14.0;
                double effectiveTextScale =
                    MediaQuery.textScalerOf(context).scale(defaultFontSize) / 14.0;
                EdgeInsetsGeometry scaledPaddingLocal = ButtonStyleButton.scaledPadding(
                    new EdgeInsetsDirectional(12, 8, 16, 8),
                ...
                content = DartRuntimePrimitives.ConvertValue<Widget>(
                    new Row(
                        ...
                        children: Equals(effectiveIconAlignment, IconAlignment.start)
                            ? new List<Widget>
                            {
                                DartRuntimePrimitives.ConvertValue<Widget>(iconLocal),
                                DartRuntimePrimitives.ConvertValue<Widget>(
                                    new Flexible(child: labelLocal)
                                ),
                            }
                            : new List<Widget>
                            {
            ...
            Widget buttonWithTooltip =
                (segment.tooltip is not null)
                    ? new Tooltip(message: segment.tooltip, child: button)
                    : button;
        ...
        OutlinedBorder effectiveBorder =
            resolve((style) => style?.shape) ?? new RoundedRectangleBorder();
        OutlinedBorder resolvedDisabledBorder =
            resolve((style) => style?.shape, disabledState) ?? new RoundedRectangleBorder();
        ...
        VisualDensity resolvedVisualDensity =
            (segmentStyle.visualDensity ?? segmentThemeStyle.visualDensity)
        ...
        MaterialTapTargetSize resolvedTapTargetSize =
            (segmentStyle.tapTargetSize ?? segmentThemeStyle.tapTargetSize)
        ...
        double effectedButtonHeight = Math.Max(
            fontSizeLocal + effectiveVerticalPadding,
            adjustButtonMinHeight
        );
        double tapTargetVerticalPaddingLocal = resolvedTapTargetSize switch
        {
            var __constant25770 when Equals(__constant25770, MaterialTapTargetSize.shrinkWrap) =>
                0.0,
            var __constant25817 when Equals(__constant25817, MaterialTapTargetSize.padded) =>
                Math.Max(
                    0,
        ...
        return new Material(
            ...
            child: new TextButtonTheme(
                data: new TextButtonThemeData(style: segmentThemeStyle),
                child: new Padding(
                    padding: widget.expandedInsets ?? EdgeInsets.zero,
                    child: new _SegmentedButtonRenderWidget__segmented_button<T>(
                        tapTargetVerticalPadding: tapTargetVerticalPaddingLocal,
                        segments: widget.segments,
                        enabledBorder: _enabled ? enabledBorderLocal : disabledBorderLocal,
                        disabledBorder: disabledBorderLocal,
                        direction: widget.direction,
                        textDirection: textDirectionLocal,
                        isExpanded: widget.expandedInsets is not null,
                        children: buttons
                    )
                )
            )
...
internal class _SegmentedButtonRenderWidget__segmented_button<T> : MultiChildRenderObjectWidget
{
    ...
    internal _SegmentedButtonRenderWidget__segmented_button(
        Key? key = null,
        List<ButtonSegment<T>> segments = default!,
        OutlinedBorder enabledBorder = default!,
        OutlinedBorder disabledBorder = default!,
        Axis direction = default!,
        TextDirection textDirection = default!,
        double tapTargetVerticalPadding = default!,
    ...
    {
        return new _RenderSegmentedButton__segmented_button<T>(
            segments: segments,
    ...
    public override void updateRenderObject(BuildContext context, RenderObject renderObject)
    {
        ...
        DartRuntimePrimitives.Ignore(
            (
                (Func<_RenderSegmentedButton__segmented_button<T>>)(
                    () =>
                    {
...
internal class _SegmentedButtonContainerBoxParentData__segmented_button
...
public class _RenderSegmentedButton__segmented_button<T>
    : RenderBox,
        ...
        RenderBoxContainerDefaultsMixin<RenderBox, ContainerBoxParentData<RenderBox>>
{
    ...
    public override double computeMinIntrinsicWidth(double height)
    {
        ...
        {
            var childParentData = (
                (_SegmentedButtonContainerBoxParentData__segmented_button?)child.parentData!
            )!;
    ...
    {
        Rect borderRect =
            (offset + new Offset(0, tapTargetVerticalPadding / 2L))
            & new Size(size.width, size.height - tapTargetVerticalPadding);
        ...
        while (child is not null)
        {
            ...
            switch (textDirection)
            {
                case TextDirection.rtl:
                {
                    segmentLeft = Equals(child, lastChild)
                        ? (borderRect.left - borderOutset)
                        : childRect.left;
                    segmentRight = Equals(child, firstChild)
                ...
                case TextDirection.ltr:
                {
                    segmentLeft = Equals(child, firstChild)
                        ? (borderRect.left - borderOutset)
                        : childRect.left;
                    segmentRight = Equals(child, lastChild)
                        ? (borderRect.right + borderOutset)
                        : childRect.right;
            ...
            var segmentClipRect = Rect.fromLTRB(
                segmentLeft,
            ...
            {
                BorderSide divider =
                    (segments[(int)(index - 1L)].enabled || segments[(int)index].enabled)
...
internal class _SegmentedButtonDefaultsM3__segmented_button : SegmentedButtonThemeData
{
    ...
    public override ButtonStyle? style
    {
        get
        {
            return new ButtonStyle(
                ...
                surfaceTintColor: new WidgetStatePropertyAll<Color>(Colors.transparent),
                elevation: new WidgetStatePropertyAll<double?>(0),
                iconSize: new WidgetStatePropertyAll<double?>(18.0),
                ...
                minimumSize: new WidgetStatePropertyAll<Size?>(new Size(40.0))
            );
        }
    }
    public override Widget? selectedIcon =>
        DartRuntimePrimitives.ConvertValue<Widget>(new Icon(Icons.check));

    public static WidgetStateProperty<Color?> resolveStateColor(
    ...
    )
    {
        ...
        return WidgetStateProperty<Color?>.CreateFromMap(
            new DartMap<WidgetStatesConstraint, Color?>
            {
                [
                    WidgetState
                        .selected.asConstraint()
                        .op_BitwiseAnd(WidgetState.pressed.asConstraint())
                ] = selectedLocal?.withOpacity(0.1),
                [
                    WidgetState
                        .selected.asConstraint()
                        .op_BitwiseAnd(WidgetState.hovered.asConstraint())
                ] = selectedLocal?.withOpacity(0.08),
                [
                    WidgetState
                        .selected.asConstraint()
                        .op_BitwiseAnd(WidgetState.focused.asConstraint())
                ] = selectedLocal?.withOpacity(0.1),
                [WidgetState.pressed.asConstraint()] = unselected?.withOpacity(0.1),
                [WidgetState.hovered.asConstraint()] = unselected?.withOpacity(0.08),
                [WidgetState.focused.asConstraint()] = unselected?.withOpacity(0.1),
