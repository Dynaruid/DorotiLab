...
public class MaterialTextSelectionHandleControls : MaterialTextSelectionControls, TextSelectionHandleControls
{
...
{
    public override Size getHandleSize(double textLineHeight) => new Size(Text_selectionLibrary._kHandleSize, Text_selectionLibrary._kHandleSize);
    ...
    public override Widget buildHandle(BuildContext context, TextSelectionHandleType type, double textLineHeight, Action? onTap = null)
    {
        ...
        Color handleColor = TextSelectionTheme.of(context).selectionHandleColor ?? theme.colorScheme.primary;
        Widget handle = SizedBox.CreateSquare(dimension: Text_selectionLibrary._kHandleSize, child: new CustomPaint(painter: new _TextSelectionHandlePainter__text_selection(color: handleColor), child: new GestureDetector(onTap: onTap, behavior: HitTestBehavior.translucent)));
        return type switch { TextSelectionHandleType.left => DartRuntimePrimitives.ConvertValue<Widget>(Transform.CreateRotate(angle: Dart_mathLibrary.pi / 2.0, child: handle)), TextSelectionHandleType.right => handle, TextSelectionHandleType.collapsed => DartRuntimePrimitives.ConvertValue<Widget>(Transform.CreateRotate(angle: Dart_mathLibrary.pi / 4.0, child: handle)), _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    public override Offset getHandleAnchor(TextSelectionHandleType type, double textLineHeight)
    {
        return type switch { TextSelectionHandleType.collapsed => new Offset(Text_selectionLibrary._kHandleSize / 2L, -4), TextSelectionHandleType.left => new Offset(Text_selectionLibrary._kHandleSize, 0), TextSelectionHandleType.right => Offset.zero, _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
public class _TextSelectionControlsToolbar__text_selection : StatefulWidget
{
    ...
    internal _TextSelectionControlsToolbar__text_selection(ValueListenable<ClipboardStatus>? clipboardStatus, TextSelectionDelegate @delegate, List<TextSelectionPoint> endpoints, Rect globalEditableRegion, Action? handleCut, Action? handleCopy, Action? handlePaste, Action? handleSelectAll, Offset selectionMidpoint, double textLineHeight)
    {
...
public class _TextSelectionControlsToolbarState__text_selection : State<_TextSelectionControlsToolbar__text_selection>, TickerProviderStateMixin<_TextSelectionControlsToolbar__text_selection>
{
    ...
    public override Widget build(BuildContext context)
    {
        ...
        TextSelectionPoint endTextSelectionPoint = (checked(widget.endpoints.Count) > 1L) ? widget.endpoints[(int)1L] : widget.endpoints[(int)0L];
        ...
        var anchorAboveLocal = new Offset(widget.globalEditableRegion.left + widget.selectionMidpoint.dx, anchorTop);
        var anchorBelowLocal = new Offset(widget.globalEditableRegion.left + widget.selectionMidpoint.dx, widget.globalEditableRegion.top + endTextSelectionPoint.point.dy + Text_selectionLibrary._kToolbarContentDistanceBelow);
        ...
        return new TextSelectionToolbar(anchorAbove: anchorAboveLocal, anchorBelow: anchorBelowLocal, children: itemDatas.asMap().entries.map((entry) =>
        {
            return new TextSelectionToolbarTextButton(padding: TextSelectionToolbarTextButton.getPadding(entry.key, checked(itemDatas.Count)), alignment: AlignmentDirectional.centerStart, onPressed: () => entry.value.onPressed(), child: new Text(entry.value.label));
        ...
        var paintLocal = ((Func<Paint>)(() =>
{
    ...
    return __cascade;
        ...
        var circle = Rect.fromCircle(center: new Offset(radiusLocal, radiusLocal), radius: radiusLocal);
...
public static partial class Text_selectionLibrary
{
    public static TextSelectionControls materialTextSelectionHandleControls = new MaterialTextSelectionHandleControls();
}

public static partial class Text_selectionLibrary
{
    public static TextSelectionControls materialTextSelectionControls = new MaterialTextSelectionControls();
}
