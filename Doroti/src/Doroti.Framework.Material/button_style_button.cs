...
public abstract class ButtonStyleButton : StatefulWidget
{
    ...
    public static WidgetStateProperty<T>? allOrNull<T>(T? value) => DartRuntimePrimitives.ConvertValue<WidgetStateProperty<T>>((value is null) ? null : new WidgetStatePropertyAll<T>(value));
    public static WidgetStateProperty<Color?>? defaultColor(Color? enabled, Color? disabled)
    {
        ...
        return (WidgetStateProperty<Color?>?)WidgetStateProperty<Color?>.CreateFromMap(new DartMap<WidgetStatesConstraint, Color?> { [WidgetState.disabled.asConstraint()] = disabled, [WidgetStateMembers.any] = enabled }.cast<WidgetStatesConstraint, Color?>());
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    public static EdgeInsetsGeometry scaledPadding(EdgeInsetsGeometry geometry1x, EdgeInsetsGeometry geometry2x, EdgeInsetsGeometry geometry3x, double fontSizeMultiplier)
    {
...
internal class _ButtonStyleState__button_style_button : State<ButtonStyleButton>, TickerProviderStateMixin<ButtonStyleButton>
{
    ...
    public override Widget build(BuildContext context)
    {
        ...
        EdgeInsetsGeometry? resolvedPadding = resolve((style) => style?.padding) ?? EdgeInsets.CreateSymmetric(horizontal: 16, vertical: 8);
        ...
        Size? resolvedMaximumSize = resolve((style) => style?.maximumSize) ?? new Size(double.PositiveInfinity, double.PositiveInfinity);
        ...
        OutlinedBorder? resolvedShape = resolve((style) => style?.shape) ?? new RoundedRectangleBorder();
        WidgetStateMouseCursor mouseCursorLocal = new _MouseCursor__button_style_button((states) => effectiveValue((style) => style?.mouseCursor?.resolve(states)));
        WidgetStateProperty<Color?> overlayColorLocal = WidgetStateProperty.resolveWith((states) => effectiveValue((style) => style?.overlayColor?.resolve(states)));
        ...
        AlignmentGeometry? resolvedAlignment = effectiveValue((style) => style?.alignment) ?? Alignment.center;
        ...
        Func<BuildContext, HashSet<WidgetState>, Widget?, Widget>? resolvedBackgroundBuilder = effectiveValue((style) => style?.backgroundBuilder);
        Func<BuildContext, HashSet<WidgetState>, Widget?, Widget>? resolvedForegroundBuilder = effectiveValue((style) => style?.foregroundBuilder);
        Clip effectiveClipBehavior = widget.clipBehavior ?? (((resolvedBackgroundBuilder ?? resolvedForegroundBuilder) is not null) ? Clip.antiAlias : Clip.none);
        BoxConstraints effectiveConstraintsLocal = resolvedVisualDensity.effectiveConstraints(new BoxConstraints(minWidth: DartRuntimePrimitives.RequireValue(resolvedMinimumSize).width, minHeight: DartRuntimePrimitives.RequireValue(resolvedMinimumSize).height, maxWidth: DartRuntimePrimitives.RequireValue(resolvedMaximumSize).width, maxHeight: DartRuntimePrimitives.RequireValue(resolvedMaximumSize).height));
        if (resolvedFixedSize is not null)
        {
            ...
            Size sizeLocal = effectiveConstraintsLocal.constrain(DartRuntimePrimitives.RequireValue(DartRuntimePrimitives.RequireValue(resolvedFixedSize__16402__value18999)));
        ...
        EdgeInsetsGeometry paddingLocal = resolvedPadding!.add(new EdgeInsets(dxLocal, dyLocal, dxLocal, dyLocal)).clamp(EdgeInsets.zero, EdgeInsetsGeometry.infinity);
        if ((DartRuntimePrimitives.RequireValue(resolvedAnimationDuration) > Duration.zero) && (elevation is not null) && (backgroundColor is not null) && (elevation != resolvedElevation) && (backgroundColor!.value != resolvedBackgroundColor!.value) && (backgroundColor!.opacity == 1L) && (resolvedBackgroundColor.opacity < 1L) && (resolvedElevation == 0L))
        {
            ...
            {
                controller?.dispose();
                controller = ((Func<AnimationController>)(() =>
{
    var __cascade = new AnimationController(duration: DartRuntimePrimitives.RequireValue(resolvedAnimationDuration), vsync: this);
    ...
    return __cascade;
        ...
        Widget result = new Padding(padding: paddingLocal, child: new Align(alignment: resolvedAlignment!, widthFactor: 1.0, heightFactor: 1.0, child: (resolvedForegroundBuilder is not null) ? resolvedForegroundBuilder(context, statesController.value, widget.child) : widget.child));
        if (resolvedBackgroundBuilder is not null)
        {
            result = resolvedBackgroundBuilder(context, statesController.value, result);
        }
        result = DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedTheme(duration: DartRuntimePrimitives.RequireValue(DartRuntimePrimitives.RequireValue(resolvedAnimationDuration)), data: theme.copyWith(iconTheme: iconThemeLocal.merge(new IconThemeData(color: resolvedIconColor, size: resolvedIconSize))), child: new InkWell(onTap: widget.onPressed, onLongPress: widget.onLongPress, onHover: widget.onHover, mouseCursor: mouseCursorLocal, enableFeedback: resolvedEnableFeedback, focusNode: widget.focusNode, canRequestFocus: widget.enabled, onFocusChange: widget.onFocusChange, autofocus: widget.autofocus, splashFactory: resolvedSplashFactory, overlayColor: overlayColorLocal, highlightColor: Colors.transparent, customBorder: resolvedShape!.copyWith(side: resolvedSide), statesController: statesController, child: result)));
        if (widget.tooltip is not null)
        {
            result = DartRuntimePrimitives.ConvertValue<Widget>(new Tooltip(message: widget.tooltip, child: result));
        }
        ...
        switch (DartRuntimePrimitives.RequireValue(resolvedTapTargetSize))
        {
            case MaterialTapTargetSize.padded:
                {
                    minSizeLocal = new Size(Widgets.ConstantsLibrary.kMinInteractiveDimension + densityAdjustment.dx, Widgets.ConstantsLibrary.kMinInteractiveDimension + densityAdjustment.dy);
...
internal class _MouseCursor__button_style_button : WidgetStateMouseCursor
{
    ...
    internal _MouseCursor__button_style_button(Func<HashSet<WidgetState>, MouseCursor?> resolveCallback)
    {
        this.resolveCallback = resolveCallback;
    }

    public override MouseCursor resolve(HashSet<WidgetState> states) => DartRuntimePrimitives.ConvertValue<MouseCursor>(resolveCallback(states)!);
...
internal class _InputPadding__button_style_button : SingleChildRenderObjectWidget
{
    ...
    internal _InputPadding__button_style_button(Widget? child = null, Size minSize = default!) : base(child: child)
    {
...
public class _RenderInputPadding__button_style_button : RenderShiftedBox
{
    ...
    internal _RenderInputPadding__button_style_button(Size _minSize, RenderBox? child = null) : base(child)
    {
