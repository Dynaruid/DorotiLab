...
public class DropdownMenuFormField<T> : FormField<T>
{
    ...
    public DropdownMenuFormField(
        ...
        : base(
            ...
            builder: (field) =>
            {
                ...
                {
                    InputDecoration decoration = decorationBuilder is null
                        ? new InputDecoration()
                        : decorationBuilder.Invoke(context, menuController);
                    InputDecoration decorationWithLabels = decoration.copyWith(
                        label: label,
                        hintText: hintText,
                    ...
                    return (errorBuilder is not null)
                        ? decorationWithLabels.copyWith(
                            error: errorBuilder(state.context, errorTextLocal)
                        )
                ...
                return new UnmanagedRestorationScope(
                    bucket: (
                        (_DropdownMenuFormFieldState__dropdown_menu_form_field<T>)field
                    ).bucket,
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(
            new _DropdownMenuFormFieldState__dropdown_menu_form_field<T>()
        );
}
...
internal class _DropdownMenuFormFieldState__dropdown_menu_form_field<T> : FormFieldState<T>
{
    ...
    internal virtual DropdownMenuFormField<T> _dropdownMenuFormField =>
        ((DropdownMenuFormField<T>?)widget)!;
    public virtual TextEditingController textFieldController =>
        DartRuntimePrimitives.ConvertValue<TextEditingController>(
            _dropdownMenuFormField.controller
                ?? (_localTextFieldController ??= new TextEditingController())
        );
    ...
    internal virtual void _createRestorableController(T? initialValue)
    {
        ...
        _restorableController = new RestorableTextEditingController(
            new TextEditingValue(text: _findLabelByValue(initialValue))
        );
        if (!restorePending)
        {
    ...
    public override void didUpdateWidget(FormField<T> oldWidget)
    {
        ...
        if (
            !EqualityComparer<T>.Default.Equals(__oldWidget.initialValue, widget.initialValue)
            && !hasInteractedByUser
        )
        {
    ...
    internal virtual void _updateRestorableController(T? value)
    {
        ...
        {
            _restorableController!.value.value = new TextEditingValue(
                text: _findLabelByValue(value)
            );
        }
    }
