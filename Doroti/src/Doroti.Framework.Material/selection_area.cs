...
public class SelectionArea : StatefulWidget
{
    ...
    internal static Widget _defaultContextMenuBuilder(BuildContext context, SelectableRegionState selectableRegionState)
    {
...
{
    internal virtual GlobalKey<SelectableRegionState> _selectableRegionKey { get; private set; } = GlobalKey<SelectableRegionState>.Create();

    public virtual SelectableRegionState selectableRegion => DartRuntimePrimitives.ConvertValue<SelectableRegionState>(_selectableRegionKey.currentState!);
    public override Widget build(BuildContext context)
    {
        DartRuntimePrimitives.Assert(() => DebugLibrary.debugCheckHasMaterialLocalizations(context));
        TextSelectionControls controls = widget.selectionControls ?? (Theme.of(context).platform switch { TargetPlatform.android => Text_selectionLibrary.materialTextSelectionHandleControls, TargetPlatform.fuchsia => Text_selectionLibrary.materialTextSelectionHandleControls, TargetPlatform.linux => Desktop_text_selectionLibrary.desktopTextSelectionHandleControls, TargetPlatform.windows => Desktop_text_selectionLibrary.desktopTextSelectionHandleControls, TargetPlatform.iOS => Text_selectionLibrary.materialTextSelectionHandleControls, TargetPlatform.macOS => Desktop_text_selectionLibrary.desktopTextSelectionHandleControls, _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") });
        return new SelectableRegion(key: _selectableRegionKey, selectionControls: controls, focusNode: widget.focusNode, contextMenuBuilder: widget.contextMenuBuilder, magnifierConfiguration: widget.magnifierConfiguration ?? TextMagnifier.adaptiveMagnifierConfiguration, onSelectionChanged: widget.onSelectionChanged, child: widget.child);
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
