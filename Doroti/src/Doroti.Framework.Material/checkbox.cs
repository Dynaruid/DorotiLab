...
public class Checkbox : StatefulWidget
{
    ...
    public static Checkbox CreateAdaptive(
        Key? key = null,
        bool? value = default!,
        bool tristate = false,
        Action<bool?>? onChanged = default!,
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(new _CheckboxState__checkbox());
}

internal class _CheckboxState__checkbox
    : State<Checkbox>,
        TickerProviderStateMixin<Checkbox>,
        ToggleableStateMixin<Checkbox>
{
    ...
    public override Widget build(BuildContext context)
    {
        ...
        var defaultCheckColor =
            defaults.checkColor
            ?? throw new InvalidOperationException(
                "The built-in checkbox theme must provide checkColor."
            );
        var defaultFillColor =
            defaults.fillColor
            ?? throw new InvalidOperationException(
                "The built-in checkbox theme must provide fillColor."
            );
        ...
        {
            var __constant18361 when Equals(__constant18361, MaterialTapTargetSize.padded) =>
                new Size(
                    ConstantsLibrary.kMinInteractiveDimension,
                    ConstantsLibrary.kMinInteractiveDimension
                ),
            var __constant18488 when Equals(__constant18488, MaterialTapTargetSize.shrinkWrap) =>
                new Size(
        ...
        HashSet<WidgetState> checkStates = widget.isError
            ? (
                (Func<HashSet<WidgetState>>)(
                    () =>
                    {
        ...
        Color effectiveCheckColor =
            (widget.checkColor ?? (checkboxTheme.checkColor?.resolve(checkStates)))
                ...
                painter: (
                    (Func<_CheckboxPainter__checkbox>)(
                        () =>
                        {
                            ...
                            __cascade.shape =
                                (widget.shape ?? checkboxTheme.shape) ?? defaults.shape!;
...
internal class _CheckboxPainter__checkbox : ToggleablePainter
{
    ...
    internal virtual void _drawCheck(Canvas canvas, Offset origin, double t, Paint paint)
    {
        ...
        var path = new Path();
        var start = new Offset(
            CheckboxLibrary._kEdgeSize * 0.15,
            CheckboxLibrary._kEdgeSize * 0.45
        );
    ...
    public override void paint(Canvas canvas, Size size)
    {
        ...
        double tNormalized = position.status switch
        {
            AnimationStatus.forward => position.value,
            AnimationStatus.completed => position.value,
...
internal class _CheckboxDefaultsM3__checkbox : CheckboxThemeData
{
    ...
    public override WidgetStateBorderSide? side
    {
        get
        {
            return WidgetStateBorderSide.CreateResolveWith(
                (states) =>
                {
                    if (states.Contains(WidgetState.disabled))
                    {
                        ...
                        return new BorderSide(
                            width: 2.0,
                            color: _colors.onSurface.withOpacity(0.38)
                        );
                    }
                    if (states.Contains(WidgetState.selected))
                    {
    ...
    public override OutlinedBorder shape =>
        DartRuntimePrimitives.ConvertValue<OutlinedBorder>(
            new RoundedRectangleBorder(borderRadius: BorderRadius.CreateAll(Radius.circular(2.0)))
        );
}
