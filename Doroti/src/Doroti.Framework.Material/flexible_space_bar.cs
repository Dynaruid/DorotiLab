...
public class FlexibleSpaceBar : StatefulWidget
{
    ...
    public FlexibleSpaceBar(
        ...
        List<StretchMode> stretchModes = default!,
        double expandedTitleScale = 1.5
    )
        : base(key: key)
    {
        List<StretchMode> __stretchModes =
            stretchModes ?? new List<StretchMode> { StretchMode.zoomBackground };
        this.title = title;
    ...
    public static Widget createSettings(
        ...
        bool? hasLeading = null,
        double currentExtent = default!,
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(
            new _FlexibleSpaceBarState__flexible_space_bar()
        );
}
            ...
            builder: (context, constraints) =>
            {
                ...
                if (widget.background is not null)
                {
                    double fadeStart = Math.Max(
                        0.0,
                        1.0 - (ConstantsLibrary.kToolbarHeight / deltaExtent)
                    );
                    var fadeEnd = 1.0;
                    ...
                    if (
                        ...
                        && (constraints.maxHeight > heightLocal)
                    )
                    {
                    ...
                    childrenLocal.Add(
                        new Positioned(
                            ...
                            right: 0.0,
                            height: heightLocal,
                            child: new _FlexibleSpaceHeaderOpacity__flexible_space_bar(
                    ...
                    if (
                        widget.stretchModes.Contains(StretchMode.blurBackground)
                        ...
                        childrenLocal.Add(
                            Positioned.CreateFill(
                                child: new BackdropFilter(
                                    filter: new ImageFilter(sigmaX: blurAmount, sigmaY: blurAmount),
                                    child: new ColoredBox(color: Colors.transparent)
                                )
                            )
                        );
                    }
                ...
                if (widget.title is not null)
                {
                    ...
                    switch (theme.platform)
                    {
                        ...
                        case TargetPlatform.windows:
                        {
                            titleLocal = DartRuntimePrimitives.ConvertValue<Widget>(
                                new Widgets.Semantics(namesRoute: true, child: widget.title)
                            );
                            break;
                        }
                    }
                    if (
                        widget.stretchModes.Contains(StretchMode.fadeTitle)
                        && (constraints.maxHeight > settings.maxExtent)
                    )
                    {
                        ...
                        titleLocal = DartRuntimePrimitives.ConvertValue<Widget>(
                            new Opacity(opacity: stretchOpacity, child: titleLocal)
                        );
                    }
                    ...
                    if (opacityAlternate > 0.0)
                    {
                        ...
                        titleStyle = titleStyle.copyWith(
                            color: titleStyle.color!.withOpacity(opacityAlternate)
                        );
                        ...
                        double scaleValue = new Tween<double>(
...
public class FlexibleSpaceBarSettings : InheritedWidget
{
    ...
    public FlexibleSpaceBarSettings(
        Key? key = null,
        double toolbarOpacity = default!,
        double minExtent = default!,
    ...
    public override bool updateShouldNotify(InheritedWidget oldWidget)
    {
        ...
        return (toolbarOpacity != __oldWidget.toolbarOpacity)
            || (minExtent != __oldWidget.minExtent)
            || (maxExtent != __oldWidget.maxExtent)
...
internal class _FlexibleSpaceHeaderOpacity__flexible_space_bar : SingleChildRenderObjectWidget
{
    ...
    internal _FlexibleSpaceHeaderOpacity__flexible_space_bar(
        double opacity,
        Widget? child,
    ...
    {
        return new _RenderFlexibleSpaceHeaderOpacity__flexible_space_bar(
            opacity: opacity,
    ...
    public override void updateRenderObject(BuildContext context, RenderObject renderObject)
    {
        ...
        DartRuntimePrimitives.Ignore(
            (
                (Func<_RenderFlexibleSpaceHeaderOpacity__flexible_space_bar>)(
                    () =>
                    {
...
{
    internal _RenderFlexibleSpaceHeaderOpacity__flexible_space_bar(
        double opacity = 1.0,
        bool alwaysIncludeSemantics = false
    )
        : base(opacity: opacity, alwaysIncludeSemantics: alwaysIncludeSemantics) { }
