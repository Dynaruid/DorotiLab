...
public class Drawer : StatelessWidget
{
    ...
    public Drawer(Key? key = null, Color? backgroundColor = null, double? elevation = null, Color? shadowColor = null, Color? surfaceTintColor = null, ShapeBorder? shape = null, double? width = null, Widget? child = null, string? semanticLabel = null, Clip? clipBehavior = null) : base(key: key)
    {
...
internal class _DrawerControllerScope__drawer : InheritedWidget
{
    ...
    internal _DrawerControllerScope__drawer(DrawerController controller, Widget child) : base(child: child)
    {
...
public class DrawerControllerState : State<DrawerController>, SingleTickerProviderStateMixin<DrawerController>
{
    ...
    internal virtual GlobalKey<IState> _drawerKey { get; private set; } = GlobalKey<IState>.Create();
    internal virtual bool _previouslyOpened { get; set; } = false;
    internal virtual GlobalKey<IState> _gestureDetectorKey { get; private set; } = GlobalKey<IState>.Create();
    ...
    {
        base.initState();
        _controller = new AnimationController(value: widget.isDrawerOpen ? 1.0 : 0.0, duration: DrawerLibrary._kBaseSettleDuration, vsync: this);
        DartRuntimePrimitives.Ignore(((Func<AnimationController>)(() =>
{
    ...
    internal virtual void _ensureHistoryEntry()
    {
        if (_historyEntry is null)
        {
            ...
            {
                _historyEntry = new LocalHistoryEntry(onRemove: () => _handleHistoryEntryRemoved(), impliesAppBarDismissal: false);
    ...
    internal virtual AlignmentDirectional _drawerOuterAlignment => widget.alignment switch { DrawerAlignment.start => AlignmentDirectional.centerStart, DrawerAlignment.end => AlignmentDirectional.centerEnd, _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
    internal virtual AlignmentDirectional _drawerInnerAlignment => widget.alignment switch { DrawerAlignment.start => AlignmentDirectional.centerEnd, DrawerAlignment.end => AlignmentDirectional.centerStart, _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
    internal virtual Widget _buildDrawer(BuildContext context)
    {
        ...
        if (_controller.isDismissed)
        {
            ...
            {
                return new Align(alignment: _drawerOuterAlignment, child: new GestureDetector(key: _gestureDetectorKey, onHorizontalDragUpdate: _move, onHorizontalDragEnd: _settle, behavior: HitTestBehavior.translucent, excludeFromSemantics: true, dragStartBehavior: widget.dragStartBehavior, child: new LimitedBox(maxHeight: 0.0, child: new SizedBox(width: dragAreaWidth, height: double.PositiveInfinity))));
            }
            else
            {
        ...
        else
        {
            ...
            Color scrimColorLocal = (widget.scrimColor ?? DrawerTheme.of(context).scrimColor) ?? Colors.black54;
            Color effectiveScrimColor = scrimColorLocal.withValues(alpha: scrimColorLocal.a * _controller.value);
            Widget drawerScrim = new ColoredBox(color: effectiveScrimColor, child: new LimitedBox(maxWidth: 0.0, maxHeight: 0.0, child: SizedBox.CreateExpand()));
            Widget childLocal = new _DrawerControllerScope__drawer(controller: widget, child: new RepaintBoundary(child: new Stack(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new BlockSemantics(child: new ExcludeSemantics(excluding: platformHasBackButton, child: new GestureDetector(onTap: widget.drawerBarrierDismissible ? close : null, child: new Widgets.Semantics(label: MaterialLocalizations.of(context).modalBarrierDismissLabel, child: drawerScrim))))), DartRuntimePrimitives.ConvertValue<Widget>(new Align(alignment: _drawerOuterAlignment, child: new Align(alignment: _drawerInnerAlignment, widthFactor: _controller.value, child: new RepaintBoundary(child: new FocusScope(key: _drawerKey, node: _focusScopeNode, child: widget.child))))) })));
            if (isDesktop)
            {
                return childLocal;
            }
            return new GestureDetector(key: _gestureDetectorKey, onHorizontalDragDown: _handleDragDown, onHorizontalDragUpdate: _move, onHorizontalDragEnd: _settle, onHorizontalDragCancel: () => _handleDragCancel(), excludeFromSemantics: true, dragStartBehavior: widget.dragStartBehavior, child: childLocal);
        }
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _DrawerDefaultsM3__drawer : DrawerThemeData
{
    ...
    internal _DrawerDefaultsM3__drawer(BuildContext context) : base(elevation: 1.0, clipBehavior: Clip.hardEdge)
    {
        this.context = context;
    }

    public override Color? backgroundColor => DartRuntimePrimitives.ConvertValue<Color>(Theme.of(context).colorScheme.surfaceContainerLow);
    ...
    public override ShapeBorder? shape => DartRuntimePrimitives.ConvertValue<ShapeBorder>(new RoundedRectangleBorder(borderRadius: BorderRadiusDirectional.CreateHorizontal(end: Radius.circular(16.0)).resolve(direction)));
    public override ShapeBorder? endShape => DartRuntimePrimitives.ConvertValue<ShapeBorder>(new RoundedRectangleBorder(borderRadius: BorderRadiusDirectional.CreateHorizontal(start: Radius.circular(16.0)).resolve(direction)));
}
