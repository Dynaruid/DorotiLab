...
public class ControlsDetails
{
    ...
    public ControlsDetails(long currentStep, long stepIndex, Action? onStepCancel = null, Action? onStepContinue = null)
    {
...
public static partial class StepperLibrary
{
    internal static EdgeInsetsDirectional _kDefaultVerticalContentPadding = EdgeInsetsDirectional.CreateOnly(start: 60.0, end: 24.0, bottom: 24.0);
}
...
public static partial class StepperLibrary
{
    internal static EdgeInsetsGeometry _kDefaultHeaderPadding = EdgeInsets.CreateSymmetric(horizontal: 24.0);
}
...
public class Step
{
    ...
    public Step(Widget title, Widget? subtitle = null, Widget content = default!, StepState state = StepState.indexed, bool isActive = false, Widget? label = null, StepStyle? stepStyle = null)
    {
        this.title = title;
...
internal class _StepperState__stepper : State<Stepper>, TickerProviderStateMixin<Stepper>
{
    ...
    {
        base.initState();
        _keys = DartRuntimePrimitives.CreateList(checked(widget.steps.Count), (i) => GlobalKey<IState>.Create());
    ...
    internal virtual EdgeInsetsGeometry? _stepIconMargin => DartRuntimePrimitives.ConvertValue<EdgeInsetsGeometry>(widget.stepIconMargin);
    internal virtual double? _stepIconHeight => widget.stepIconHeight;
    internal virtual double? _stepIconWidth => widget.stepIconWidth;
    public virtual EdgeInsetsGeometry effectiveHeaderPadding => DartRuntimePrimitives.ConvertValue<EdgeInsetsGeometry>(widget.headerPadding ?? StepperLibrary._kDefaultHeaderPadding);
    ...
    internal virtual Color _connectorColor(bool isActive)
    {
        ...
        var states = ((Func<HashSet<WidgetState>>)(() => { var __collection15710 = new HashSet<WidgetState>(); if (isActive) { __collection15710.Add(WidgetState.selected); } else { __collection15710.Add(WidgetState.disabled); } return __collection15710; }))();
    ...
    {
        return new ColoredBox(color: _connectorColor(isActive), child: new SizedBox(width: visible ? (widget.connectorThickness ?? 1.0) : 0.0, height: 16.0));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    internal virtual Widget _buildCircleChild(long index, bool oldState)
    {
        ...
        Color iconColor = isDarkActive ? StepperLibrary._kCircleActiveDark : StepperLibrary._kCircleActiveLight;
        textStyle ??= (isDarkActive ? StepperLibrary._kStepStyle.copyWith(color: Colors.black87) : StepperLibrary._kStepStyle);
        return stateLocal switch { StepState.indexed => DartRuntimePrimitives.ConvertValue<Widget>(new Text($"{index + 1L}", style: textStyle)), StepState.disabled => DartRuntimePrimitives.ConvertValue<Widget>(new Text($"{index + 1L}", style: textStyle)), StepState.editing => DartRuntimePrimitives.ConvertValue<Widget>(new Icon(Icons.edit, color: iconColor, size: 18.0)), StepState.complete => DartRuntimePrimitives.ConvertValue<Widget>(new Icon(Icons.check, color: iconColor, size: 18.0)), StepState.error => DartRuntimePrimitives.ConvertValue<Widget>(new Center(child: new Text("!", style: StepperLibrary._kStepStyle))), _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    internal virtual Color _circleColor(long index)
    {
        ...
        ColorScheme colorSchemeLocal = Theme.of(context).colorScheme;
        var states = ((Func<HashSet<WidgetState>>)(() => { var __collection17276 = new HashSet<WidgetState>(); if (isActiveLocal) { __collection17276.Add(WidgetState.selected); } else { __collection17276.Add(WidgetState.disabled); } return __collection17276; }))();
        Color? resolvedConnectorColor = widget.connectorColor?.resolve(states);
    ...
    {
        return new Padding(padding: _stepIconMargin ?? EdgeInsets.CreateSymmetric(vertical: 8.0), child: new SizedBox(width: _stepIconWidth ?? StepperLibrary._kStepSize, height: _stepIconHeight ?? StepperLibrary._kStepSize, child: new AnimatedContainer(curve: Curves.fastOutSlowIn, duration: ThemeLibrary.kThemeAnimationDuration, decoration: new BoxDecoration(color: _stepStyle(index)?.color ?? _circleColor(index), shape: BoxShape.circle, border: _stepStyle(index)?.border, boxShadow: (_stepStyle(index)?.boxShadow is not null) ? new List<BoxShadow> { _stepStyle(index)!.boxShadow! } : null, gradient: _stepStyle(index)?.gradient), child: new Center(child: _buildCircleChild(index, oldState && Equals(widget.steps[(int)index].state, StepState.error))))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    internal virtual Widget _buildTriangle(long index, bool oldState)
    {
        ...
        return new Padding(padding: _stepIconMargin ?? EdgeInsets.CreateSymmetric(vertical: 8.0), child: new SizedBox(width: _stepIconWidth ?? StepperLibrary._kStepSize, height: _stepIconHeight ?? StepperLibrary._kStepSize, child: new Center(child: new SizedBox(width: _stepIconWidth ?? StepperLibrary._kStepSize, height: (_stepIconHeight is not null) ? (DartRuntimePrimitives.RequireValue(_stepIconHeight) * StepperLibrary._kTriangleSqrt) : StepperLibrary._kTriangleHeight, child: new CustomPaint(painter: new _TrianglePainter__stepper(color: colorLocal), child: new Align(alignment: new Alignment(0.0, 0.8), child: _buildCircleChild(index, oldState && (!Equals(widget.steps[(int)index].state, StepState.error)))))))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    internal virtual Widget _buildIcon(long index)
    {
        ...
        {
            return new AnimatedCrossFade(firstChild: _buildCircle(index, true), secondChild: _buildTriangle(index, true), firstCurve: new Interval(0.0, 0.6, curve: Curves.fastOutSlowIn), secondCurve: new Interval(0.4, 1.0, curve: Curves.fastOutSlowIn), sizeCurve: Curves.fastOutSlowIn, crossFadeState: Equals(widget.steps[(int)index].state, StepState.error) ? CrossFadeState.showSecond : CrossFadeState.showFirst, duration: ThemeLibrary.kThemeAnimationDuration);
        }
        else
        {
    ...
    internal virtual Widget _buildVerticalControls(long stepIndex)
    {
        ...
        Color cancelColor = Theme.brightnessOf(context) switch { Brightness.light => Colors.black54, Brightness.dark => Colors.white70, _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
        ThemeData themeData = Theme.of(context);
        ColorScheme colorSchemeLocal = themeData.colorScheme;
        MaterialLocalizations localizations = MaterialLocalizations.of(context);
        OutlinedBorder buttonShape = new RoundedRectangleBorder(borderRadius: BorderRadius.CreateAll(Radius.circular(2)));
        var buttonPadding = EdgeInsets.CreateSymmetric(horizontal: 16.0);
        return new Padding(padding: EdgeInsets.CreateOnly(top: 16.0), child: new SizedBox(height: 48.0, child: new Row(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new TextButton(onPressed: widget.onStepContinue, style: new ButtonStyle(foregroundColor: WidgetStateProperty.resolveWith((states) => {
...
throw new InvalidOperationException("Dart closure completed without a value.");
}), padding: new WidgetStatePropertyAll<EdgeInsetsGeometry>(buttonPadding), shape: new WidgetStatePropertyAll<OutlinedBorder>(buttonShape)), child: new Text(localizations.continueButtonLabel))), DartRuntimePrimitives.ConvertValue<Widget>(new Padding(padding: EdgeInsetsDirectional.CreateOnly(start: 8.0), child: new TextButton(onPressed: widget.onStepCancel, style: TextButton.styleFrom(foregroundColor: cancelColor, padding: buttonPadding, shape: buttonShape), child: new Text(localizations.cancelButtonLabel)))) })));
    }
    ...
    {
        return new Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: ((Func<List<Widget>>)(() => { var __collection25461 = new List<Widget>(); __collection25461.Add(DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedDefaultTextStyle(style: _titleStyle(index), duration: ThemeLibrary.kThemeAnimationDuration, curve: Curves.fastOutSlowIn, child: widget.steps[(int)index].title))); if (widget.steps[(int)index].subtitle is not null) { __collection25461.Add(DartRuntimePrimitives.ConvertValue<Widget>(new Padding(padding: EdgeInsets.CreateOnly(top: 2.0), child: new AnimatedDefaultTextStyle(style: _subtitleStyle(index), duration: ThemeLibrary.kThemeAnimationDuration, curve: Curves.fastOutSlowIn, child: widget.steps[(int)index].subtitle!)))); } return __collection25461; }))());
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    internal virtual Widget _buildLabelText(long index)
    {
        ...
        {
            return new AnimatedDefaultTextStyle(style: _labelStyle(index), duration: ThemeLibrary.kThemeAnimationDuration, child: widget.steps[(int)index].label!);
        }
    ...
    internal virtual Widget _buildVerticalHeader(long index)
    {
        ...
        return new Padding(padding: effectiveHeaderPadding, child: new Row(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new Column(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(_buildLine(!_isFirst(index), isPreviousActive)), DartRuntimePrimitives.ConvertValue<Widget>(_buildIcon(index)), DartRuntimePrimitives.ConvertValue<Widget>(_buildLine(!_isLast(index), isActiveLocal)) })), DartRuntimePrimitives.ConvertValue<Widget>(new Expanded(child: new Padding(padding: EdgeInsetsDirectional.CreateOnly(start: 12.0), child: _buildHeaderText(index)))) }));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    internal virtual Widget _buildVerticalBody(long index)
    {
        ...
        EdgeInsetsGeometry effectiveVerticalContentPadding = (widget.contentPadding ?? StepperLibrary._kDefaultVerticalContentPadding).add(EdgeInsetsDirectional.CreateOnly(start: marginLeft ?? 0.0));
        return new Stack(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new PositionedDirectional(start: 24.0 + (additionalMarginLeft ?? 0.0) + (additionalMarginRight ?? 0.0), top: 0.0, bottom: 0.0, width: _stepIconWidth ?? StepperLibrary._kStepSize, child: new Center(child: new SizedBox(width: !_isLast(index) ? (widget.connectorThickness ?? 1.0) : 0.0, height: double.PositiveInfinity, child: new ColoredBox(color: _connectorColor(widget.steps[(int)index].isActive)))))), DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedCrossFade(firstChild: new SizedBox(width: double.PositiveInfinity, height: 0), secondChild: new Padding(padding: effectiveVerticalContentPadding, child: new Column(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new ClipRect(clipBehavior: widget.clipBehavior, child: widget.steps[(int)index].content)), DartRuntimePrimitives.ConvertValue<Widget>(_buildVerticalControls(index)) })), firstCurve: new Interval(0.0, 0.6, curve: Curves.fastOutSlowIn), secondCurve: new Interval(0.4, 1.0, curve: Curves.fastOutSlowIn), sizeCurve: Curves.fastOutSlowIn, crossFadeState: _isCurrent(index) ? CrossFadeState.showSecond : CrossFadeState.showFirst, duration: ThemeLibrary.kThemeAnimationDuration)) });
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    internal virtual Widget _buildVertical()
    {
        return new ListView(controller: widget.controller, shrinkWrap: true, physics: widget.physics, children: ((Func<List<Widget>>)(() =>
        {
            var __collection29851 = new List<Widget>(); for (long i = 0L; i < checked(widget.steps.Count); i += 1L)
            {
                __collection29851.Add(DartRuntimePrimitives.ConvertValue<Widget>(new Column(key: _keys[(int)i], children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new InkWell(onTap: (!Equals(widget.steps[(int)i].state, StepState.disabled)) ? (() => {
...
widget.onStepTapped?.Invoke(i);
}) : null, canRequestFocus: !Equals(widget.steps[(int)i].state, StepState.disabled), child: _buildVerticalHeader(i))), DartRuntimePrimitives.ConvertValue<Widget>(_buildVerticalBody(i)) })));
            }
    ...
    {
        EdgeInsetsGeometry effectiveHorizontalContentPadding = widget.contentPadding ?? StepperLibrary._kDefaultHorizontalContentPadding;
        var childrenLocal = ((Func<List<Widget>>)(() =>
        {
            var __collection31062 = new List<Widget>(); for (long i = 0L; i < checked(widget.steps.Count); i += 1L)
            {
                __collection31062.AddRange(((Func<List<Widget>>)(() =>
                {
                    var __collection31130 = new List<Widget>(); __collection31130.Add(DartRuntimePrimitives.ConvertValue<Widget>(new InkResponse(onTap: (!Equals(widget.steps[(int)i].state, StepState.disabled)) ? (() =>
                    {
                        widget.onStepTapped?.Invoke(i);
                    }) : null, canRequestFocus: !Equals(widget.steps[(int)i].state, StepState.disabled), child: new Row(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new SizedBox(height: _isLabel() ? 104.0 : 72.0, child: new Column(mainAxisAlignment: MainAxisAlignment.center, children: ((Func<List<Widget>>)(() => { var __collection31654 = new List<Widget>(); if (widget.steps[(int)i].label is not null) { __collection31654.Add(DartRuntimePrimitives.ConvertValue<Widget>(new SizedBox(height: 24.0))); } __collection31654.Add(DartRuntimePrimitives.ConvertValue<Widget>(new Center(child: _buildIcon(i)))); if (widget.steps[(int)i].label is not null) { __collection31654.Add(DartRuntimePrimitives.ConvertValue<Widget>(new SizedBox(height: 24.0, child: _buildLabelText(i)))); } return __collection31654; }))()))), DartRuntimePrimitives.ConvertValue<Widget>(new Padding(padding: _stepIconMargin ?? EdgeInsetsDirectional.CreateOnly(start: 12.0), child: _buildHeaderText(i))) })))); if (!_isLast(i)) { __collection31130.Add(DartRuntimePrimitives.ConvertValue<Widget>(new Expanded(child: new Padding(padding: _stepIconMargin ?? EdgeInsets.CreateSymmetric(horizontal: 8.0), child: new SizedBox(height: (widget.steps[(int)i].stepStyle?.connectorThickness ?? widget.connectorThickness) ?? 1.0, child: new ColoredBox(color: widget.steps[(int)i].stepStyle?.connectorColor ?? _connectorColor(widget.steps[(int)i].isActive))))))); }
        ...
        for (var iLocal = 0L; iLocal < checked(widget.steps.Count); iLocal += 1L)
        {
            stepPanels.Add(new Visibility(maintainState: true, visible: iLocal == widget.currentStep, child: new ClipRect(clipBehavior: widget.clipBehavior, child: widget.steps[(int)iLocal].content)));
        }
        return new Column(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new Material(elevation: widget.elevation ?? 2, child: new Padding(padding: effectiveHeaderPadding, child: new SizedBox(height: (_stepIconHeight is not null) ? (DartRuntimePrimitives.RequireValue(_stepIconHeight) * _heightFactor) : null, child: new Row(children: childrenLocal))))), DartRuntimePrimitives.ConvertValue<Widget>(new Expanded(child: new ListView(controller: widget.controller, physics: widget.physics, padding: effectiveHorizontalContentPadding, children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedSize(curve: Curves.fastOutSlowIn, duration: ThemeLibrary.kThemeAnimationDuration, child: new Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: stepPanels))), DartRuntimePrimitives.ConvertValue<Widget>(_buildVerticalControls(widget.currentStep)) }))) });
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _TrianglePainter__stepper : CustomPainter
{
    ...
    public override void paint(Canvas canvas, Size size)
    {
        ...
        var points = new List<Offset> { new Offset(0.0, heightLocal), new Offset(@base, heightLocal), new Offset(halfBase, 0.0) };
...
public class StepStyle : Diagnosticable
{
    ...
    public StepStyle(Color? color = null, Color? errorColor = null, Color? connectorColor = null, double? connectorThickness = null, BoxBorder? border = null, BoxShadow? boxShadow = null, Painting.Gradient? gradient = null, TextStyle? indexStyle = null)
    {
        this.color = color;
    ...
    public virtual StepStyle copyWith(Color? color = null, Color? errorColor = null, Color? connectorColor = null, double? connectorThickness = null, BoxBorder? border = null, BoxShadow? boxShadow = null, Painting.Gradient? gradient = null, TextStyle? indexStyle = null)
    {
    ...
    public virtual void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        ...
        properties.add(new DoubleProperty("connectorThickness", connectorThickness, defaultValue: null));
        properties.add(new DiagnosticsProperty<BoxBorder>("border", border, defaultValue: null));
        properties.add(new DiagnosticsProperty<BoxShadow>("boxShadow", boxShadow, defaultValue: null));
        properties.add(new DiagnosticsProperty<Painting.Gradient>("gradient", gradient, defaultValue: null));
        properties.add(new DiagnosticsProperty<TextStyle>("indexStyle", indexStyle, defaultValue: defaultTextTheme.bodyLarge));
    }
