...
internal class _CalendarDatePickerState__calendar_date_picker : State<CalendarDatePicker>
{
    ...
    internal virtual GlobalKey<IState> _monthPickerKey { get; private set; } = GlobalKey<IState>.Create();
    internal virtual GlobalKey<IState> _yearPickerKey { get; private set; } = GlobalKey<IState>.Create();
    ...
    public override Widget build(BuildContext context)
    {
        ...
        var picker = new SizedBox(height: Calendar_date_pickerLibrary._subHeaderHeight + scaledMaxDayPickerHeight, child: _buildPicker());
        return new Stack(children: ((Func<List<Widget>>)(() => { var __collection15772 = new List<Widget>(); if (MediaQuery.maybeSupportsAnnounceOf(context) ?? false) { __collection15772.Add(DartRuntimePrimitives.ConvertValue<Widget>(picker)); } else { __collection15772.Add(DartRuntimePrimitives.ConvertValue<Widget>(new Widgets.Semantics(container: true, liveRegion: true, accessibilityFocusBlockType: AccessibilityFocusBlockType.blockNode, label: _announcementText, child: picker))); } __collection15772.Add(DartRuntimePrimitives.ConvertValue<Widget>(MediaQuery.withClampedTextScaling(maxScaleFactor: Calendar_date_pickerLibrary._kModeToggleButtonMaxScaleFactor, child: new _DatePickerModeToggleButton__calendar_date_picker(mode: _mode, title: widget.calendarDelegate.formatMonthYear(_currentDisplayedMonthDate, _localizations), onTitlePressed: () => { _handleModeChanged(_mode switch { DatePickerMode.day => DatePickerMode.year, DatePickerMode.year => DatePickerMode.day, _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") }); })))); return __collection15772; }))());
    }
...
public class _DatePickerModeToggleButton__calendar_date_picker : StatefulWidget
{
    ...
    internal _DatePickerModeToggleButton__calendar_date_picker(DatePickerMode mode, string title, Action onTitlePressed)
    {
        this.mode = mode;
        this.title = title;
...
public class _DatePickerModeToggleButtonState__calendar_date_picker : State<_DatePickerModeToggleButton__calendar_date_picker>, SingleTickerProviderStateMixin<_DatePickerModeToggleButton__calendar_date_picker>
{
    ...
    {
        base.initState();
        _controller = new AnimationController(value: Equals(widget.mode, DatePickerMode.year) ? 0.5 : 0, upperBound: 0.5, duration: Duration.Create(milliseconds: 200L), vsync: this);
    }
    ...
    public override Widget build(BuildContext context)
    {
        ...
        TextStyle? buttonTextStyle = datePickerTheme.toggleButtonTextStyle ?? defaultsLocal.toggleButtonTextStyle;
        Color? subHeaderForegroundColorLocal = datePickerTheme.subHeaderForegroundColor ?? defaultsLocal.subHeaderForegroundColor;
        Color? buttonTextColor = (datePickerTheme.toggleButtonTextStyle?.color ?? datePickerTheme.subHeaderForegroundColor) ?? defaultsLocal.toggleButtonTextStyle?.color;
        return new SizedBox(height: Calendar_date_pickerLibrary._subHeaderHeight, child: new Padding(padding: EdgeInsetsDirectional.CreateOnly(start: 16, end: 4), child: new Row(children: ((Func<List<Widget>>)(() => { var __collection19110 = new List<Widget>(); __collection19110.Add(DartRuntimePrimitives.ConvertValue<Widget>(new Flexible(child: new Widgets.Semantics(label: MaterialLocalizations.of(context).selectYearSemanticsLabel, button: true, container: true, child: new SizedBox(height: Calendar_date_pickerLibrary._subHeaderHeight, child: new InkWell(onTap: widget.onTitlePressed, child: new Padding(padding: EdgeInsets.CreateSymmetric(horizontal: 8), child: new Row(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new Flexible(child: new Text(widget.title, overflow: TextOverflow.ellipsis, style: buttonTextStyle?.apply(color: buttonTextColor)))), DartRuntimePrimitives.ConvertValue<Widget>(new RotationTransition(turns: _controller, child: new Icon(Icons.arrow_drop_down, color: subHeaderForegroundColorLocal))) })))))))); if (Equals(widget.mode, DatePickerMode.day)) { __collection19110.Add(DartRuntimePrimitives.ConvertValue<Widget>(new SizedBox(width: Calendar_date_pickerLibrary._monthNavButtonsWidth))); } return __collection19110; }))())));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
{
    internal virtual GlobalKey<IState> _pageViewKey { get; private set; } = GlobalKey<IState>.Create();
    internal virtual string _announcementText { get; set; } = "";
    ...
    public override void initState()
    {
        ...
        _pageController = new PageController(initialPage: widget.calendarDelegate.monthDelta(widget.firstDate, _currentMonth));
        _shortcutMap = new DartMap<ShortcutActivator, Intent> { [new SingleActivator(LogicalKeyboardKey.arrowLeft)] = new DirectionalFocusIntent(TraversalDirection.left), [new SingleActivator(LogicalKeyboardKey.arrowRight)] = new DirectionalFocusIntent(TraversalDirection.right), [new SingleActivator(LogicalKeyboardKey.arrowDown)] = new DirectionalFocusIntent(TraversalDirection.down), [new SingleActivator(LogicalKeyboardKey.arrowUp)] = new DirectionalFocusIntent(TraversalDirection.up) }.cast<ShortcutActivator, Intent>();
    ...
    internal virtual Widget _buildItems(BuildContext context, long index)
    {
        ...
        return new _DayPicker__calendar_date_picker(key: new ValueKey<DateTime>(month), calendarDelegate: widget.calendarDelegate, selectedDate: widget.selectedDate, currentDate: widget.currentDate, onChanged: _handleDateSelected, firstDate: widget.firstDate, lastDate: widget.lastDate, displayedMonth: month, selectableDayPredicate: widget.selectableDayPredicate);
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    public override Widget build(BuildContext context)
    {
        Color? subHeaderForegroundColorLocal = DatePickerTheme.of(context).subHeaderForegroundColor ?? DatePickerTheme.defaults(context).subHeaderForegroundColor;
        bool supportsAnnounce = MediaQuery.maybeSupportsAnnounceOf(context) ?? false;
        return new Widgets.Semantics(container: true, explicitChildNodes: true, liveRegion: !supportsAnnounce, accessibilityFocusBlockType: !supportsAnnounce ? AccessibilityFocusBlockType.blockNode : AccessibilityFocusBlockType.none, label: !supportsAnnounce ? _announcementText : null, child: new Column(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new SizedBox(height: Calendar_date_pickerLibrary._subHeaderHeight, child: new Padding(padding: EdgeInsetsDirectional.CreateOnly(start: 16, end: 4), child: new Row(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new Spacer()), DartRuntimePrimitives.ConvertValue<Widget>(new IconButton(icon: new Icon(Icons.chevron_left, semanticLabel: _isDisplayingFirstMonth ? _localizations.previousMonthTooltip : null), color: subHeaderForegroundColorLocal, tooltip: _isDisplayingFirstMonth ? null : _localizations.previousMonthTooltip, onPressed: _isDisplayingFirstMonth ? null : _handlePreviousMonth)), DartRuntimePrimitives.ConvertValue<Widget>(new IconButton(icon: new Icon(Icons.chevron_right, semanticLabel: _isDisplayingLastMonth ? _localizations.nextMonthTooltip : null), color: subHeaderForegroundColorLocal, tooltip: _isDisplayingLastMonth ? null : _localizations.nextMonthTooltip, onPressed: _isDisplayingLastMonth ? null : _handleNextMonth)) })))), DartRuntimePrimitives.ConvertValue<Widget>(new Expanded(child: new FocusableActionDetector(shortcuts: _shortcutMap, actions: _actionMap, focusNode: _dayGridFocus, onFocusChange: _handleGridFocusChange, child: new _FocusedDate__calendar_date_picker(calendarDelegate: widget.calendarDelegate, date: _dayGridFocus.hasFocus ? _focusedDay : null, child: new Material(type: MaterialType.transparency, child: PageView.CreateBuilder(key: _pageViewKey, controller: _pageController, itemBuilder: _buildItems, itemCount: widget.calendarDelegate.monthDelta(widget.firstDate, widget.lastDate) + 1L, onPageChanged: _handleMonthPageChanged)))))) }));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _FocusedDate__calendar_date_picker : InheritedWidget
{
    ...
    internal _FocusedDate__calendar_date_picker(Widget child, CalendarDelegate<DateTime> calendarDelegate, DateTime? date = null) : base(child: child)
    {
        this.calendarDelegate = calendarDelegate;
        this.date = date;
    }
...
internal class _DayPickerState__calendar_date_picker : State<_DayPicker__calendar_date_picker>
{
    ...
    internal virtual List<Widget> _dayHeaders(TextStyle? headerStyle, MaterialLocalizations localizations)
    {
    ...
    public override Widget build(BuildContext context)
    {
        ...
        while (day < daysInMonth)
        {
            ...
            else
            {
                ...
                dayItems.Add(new _Day__calendar_date_picker(dayToBuild, key: new ValueKey<DateTime>(dayToBuild), isDisabled: isDisabledLocal, isSelectedDay: isSelectedDayLocal, isToday: isTodayLocal, onChanged: widget.onChanged, focusNode: _dayFocusNodes[(int)(day - 1L)], calendarDelegate: widget.calendarDelegate));
            }
        }
        ...
        return new Padding(padding: EdgeInsets.CreateSymmetric(horizontal: monthPickerHorizontalPadding), child: MediaQuery.withClampedTextScaling(maxScaleFactor: isLandscapeOrientation ? Calendar_date_pickerLibrary._kDayPickerGridLandscapeMaxScaleFactor : Calendar_date_pickerLibrary._kDayPickerGridPortraitMaxScaleFactor, child: GridView.CreateCustom(physics: new ClampingScrollPhysics(), gridDelegate: new _DayPickerGridDelegate__calendar_date_picker(context), childrenDelegate: new SliverChildListDelegate(dayItems, addRepaintBoundaries: false))));
    }
...
internal class _DayState__calendar_date_picker : State<_Day__calendar_date_picker>
{
    ...
    public override Widget build(BuildContext context)
    {
        ...
        var statesLocal = ((Func<HashSet<WidgetState>>)(() => { var __collection44458 = new HashSet<WidgetState>(); if (widget.isDisabled) { __collection44458.Add(WidgetState.disabled); } if (widget.isSelectedDay) { __collection44458.Add(WidgetState.selected); } return __collection44458; }))();
        _statesController.value = statesLocal;
        Color? dayForegroundColorLocal = resolve((theme) => widget.isToday ? theme?.todayForegroundColor : theme?.dayForegroundColor, statesLocal);
        Color? dayBackgroundColorLocal = resolve((theme) => widget.isToday ? theme?.todayBackgroundColor : theme?.dayBackgroundColor, statesLocal);
        WidgetStateProperty<Color?> dayOverlayColorLocal = WidgetStateProperty.resolveWith((states) => effectiveValue((theme) => theme?.dayOverlayColor?.resolve(states)));
        ...
        BorderSide todayBorderSide = hasCustomBorderColor ? datePickerTheme.todayBorder! : (datePickerTheme.todayBorder ?? defaultsLocal.todayBorder!).copyWith(color: dayForegroundColorLocal);
        var decorationLocal = widget.isToday ? new ShapeDecoration(color: dayBackgroundColorLocal, shape: dayShapeLocal.copyWith(side: todayBorderSide)) : new ShapeDecoration(color: dayBackgroundColorLocal, shape: dayShapeLocal);
        Widget dayWidget = new Ink(decoration: decorationLocal, child: new Center(child: new Text(localizations.formatDecimal(widget.day.Day), style: dayStyleLocal?.apply(color: dayForegroundColorLocal))));
        Orientation orientation = MediaQuery.orientationOf(context);
        if (Equals(orientation, Orientation.portrait))
        {
            dayWidget = DartRuntimePrimitives.ConvertValue<Widget>(new Padding(padding: EdgeInsets.CreateAll(4.0), child: dayWidget));
        }
        dayWidget = DartRuntimePrimitives.ConvertValue<Widget>(new Widgets.Semantics(label: $"{localizations.formatDecimal(widget.day.Day)}, {widget.calendarDelegate.formatFullDate(widget.day, localizations)}{semanticLabelSuffix}", button: true, selected: widget.isSelectedDay, enabled: !widget.isDisabled, excludeSemantics: true, child: dayWidget));
        if (!widget.isDisabled)
        {
            dayWidget = DartRuntimePrimitives.ConvertValue<Widget>(new InkResponse(focusNode: widget.focusNode, onTap: () => { widget.onChanged(widget.day); }, statesController: _statesController, overlayColor: dayOverlayColorLocal, customBorder: dayShapeLocal, containedInkWell: true, child: dayWidget));
        }
        return dayWidget;
    }
...
internal class _DayPickerGridDelegate__calendar_date_picker : SliverGridDelegate
{
    ...
    public virtual SliverGridLayout getLayout(SliverConstraints constraints)
    {
        ...
        return new SliverGridRegularTileLayout(childCrossAxisExtent: tileWidth, childMainAxisExtent: tileHeight, crossAxisCount: columnCount, crossAxisStride: tileWidth, mainAxisStride: tileHeight, reverseCrossAxis: Basic_typesLibrary.axisDirectionIsReversed(constraints.crossAxisDirection));
    }
...
internal class _YearPickerState__calendar_date_picker : State<YearPicker>
{
    ...
    {
        base.initState();
        _scrollController = new ScrollController(initialScrollOffset: _scrollOffsetForYear(widget.selectedDate ?? widget.firstDate));
    }
    ...
    internal virtual Widget _buildYearItem(BuildContext context, long index)
    {
        ...
        var statesLocal = ((Func<HashSet<WidgetState>>)(() => { var __collection54093 = new HashSet<WidgetState>(); if (isDisabled) { __collection54093.Add(WidgetState.disabled); } if (isSelected) { __collection54093.Add(WidgetState.selected); } return __collection54093; }))();
        Color? textColor = resolve((theme) => isCurrentYear ? theme?.todayForegroundColor : theme?.yearForegroundColor, statesLocal);
        Color? background = resolve((theme) => isCurrentYear ? theme?.todayBackgroundColor : theme?.yearBackgroundColor, statesLocal);
        WidgetStateProperty<Color?> overlayColorLocal = WidgetStateProperty.resolveWith((states) => effectiveValue((theme) => theme?.yearOverlayColor?.resolve(states)));
        ...
        var decorationLocal = new ShapeDecoration(color: background, shape: yearShapeLocal.copyWith(side: borderSide));
        TextStyle? itemStyle = (datePickerTheme.yearStyle ?? defaultsLocal.yearStyle)?.apply(color: textColor);
        MaterialLocalizations localizations = MaterialLocalizations.of(context);
        Widget yearItem = new Center(child: new Container(decoration: decorationLocal, height: decorationHeight, width: decorationWidth, alignment: Alignment.center, child: new Widgets.Semantics(selected: isSelected, enabled: !isDisabled, button: true, child: new Text(widget.calendarDelegate.formatYear(year, localizations), style: itemStyle))));
        if (!isDisabled)
        {
            ...
            _statesController.value = statesLocal;
            yearItem = DartRuntimePrimitives.ConvertValue<Widget>(new InkWell(key: new ValueKey<long>(year), onTap: () => { widget.onChanged(date); }, statesController: _statesController, overlayColor: overlayColorLocal, child: yearItem));
        }
        return yearItem;
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    public override Widget build(BuildContext context)
    {
        return new Column(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new Divider()), DartRuntimePrimitives.ConvertValue<Widget>(new Expanded(child: new Material(type: MaterialType.transparency, child: GridView.CreateBuilder(controller: _scrollController, dragStartBehavior: widget.dragStartBehavior, gridDelegate: new _YearPickerGridDelegate__calendar_date_picker(context), itemBuilder: _buildYearItem, itemCount: Math.Max(_itemCount, minYears), padding: EdgeInsets.CreateSymmetric(horizontal: Calendar_date_pickerLibrary._yearPickerPadding))))), DartRuntimePrimitives.ConvertValue<Widget>(new Divider()) });
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
public static partial class Calendar_date_pickerLibrary
{
    internal static void _reportAnnouncementError(object exception, System.Diagnostics.StackTrace? stack)
    {
