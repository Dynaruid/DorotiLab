...
public class Tooltip : StatefulWidget
{
    ...
    public Tooltip(Key? key = null, string? message = null, InlineSpan? richMessage = null, double? height = null, BoxConstraints? constraints = null, EdgeInsetsGeometry? padding = null, EdgeInsetsGeometry? margin = null, double? verticalOffset = null, bool? preferBelow = null, bool? excludeFromSemantics = null, Decoration? decoration = null, TextStyle? textStyle = null, TextAlign? textAlign = null, Duration? waitDuration = null, Duration? showDuration = null, Duration? exitDuration = null, bool enableTapToDismiss = true, TooltipTriggerMode? triggerMode = null, bool? enableFeedback = null, Action? onTriggered = null, MouseCursor? mouseCursor = null, bool? ignorePointer = null, Func<TooltipPositionContext, Offset>? positionDelegate = null, Widget? child = null) : base(key: key)
    {
    ...
    public override void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        ...
        properties.add(new StringProperty("message", message, showName: message is null, defaultValue: (message is null) ? null : DiagnosticsLibrary.kNoDefaultValue.ToString()));
        properties.add(new StringProperty("richMessage", richMessage?.toPlainText(), showName: richMessage is null, defaultValue: (richMessage is null) ? null : DiagnosticsLibrary.kNoDefaultValue.ToString()));
        ...
        properties.add(new FlagProperty("semantics", value: excludeFromSemantics, ifTrue: "excluded", showName: true));
        properties.add(new DiagnosticsProperty<Duration>("wait duration", waitDuration, defaultValue: null));
        properties.add(new DiagnosticsProperty<Duration>("show duration", showDuration, defaultValue: null));
        properties.add(new DiagnosticsProperty<Duration>("exit duration", exitDuration, defaultValue: null));
        properties.add(new DiagnosticsProperty<TooltipTriggerMode>("triggerMode", triggerMode, defaultValue: null));
        properties.add(new FlagProperty("enableFeedback", value: enableFeedback, ifTrue: "true", showName: true));
        properties.add(new DiagnosticsProperty<TextAlign>("textAlign", textAlign, defaultValue: null));
        properties.add(new DiagnosticsProperty<Func<TooltipPositionContext, Offset>>("positionDelegate", positionDelegate, defaultValue: null));
    }
...
public class TooltipState : State<Tooltip>, SingleTickerProviderStateMixin<Tooltip>
{
    ...
    internal virtual GlobalKey<RawTooltipState> _tooltipKey { get; private set; } = GlobalKey<RawTooltipState>.Create();
    ...
    internal virtual Offset _getDefaultPositionDelegate(TooltipPositionContext context)
    {
        ...
        var resolvedContext = new TooltipPositionContext(target: context.target, targetSize: context.targetSize, tooltipSize: context.tooltipSize, overlaySize: context.overlaySize, verticalOffset: effectiveVerticalOffset, preferBelow: effectivePreferBelow);
    ...
    public override Widget build(BuildContext context)
    {
        ...
        var (defaultTextStyle, defaultDecoration) = Theme.of(context) switch { ThemeData { brightness: Brightness.dark, textTheme: TextTheme textThemeLocal, platform: TargetPlatform platformLocal } __object18397 => ((TextStyle, BoxDecoration))(textThemeLocal.bodyMedium!.copyWith(color: Colors.black, fontSize: _getDefaultFontSize(platformLocal)), new BoxDecoration(color: Colors.white.withOpacity(0.9), borderRadius: BorderRadius.CreateAll(Radius.circular(4)))), ThemeData { brightness: Brightness.light, textTheme: TextTheme textThemeAlternate, platform: TargetPlatform platformAlternate } __object18858 => ((TextStyle, BoxDecoration))(textThemeAlternate.bodyMedium!.copyWith(color: Colors.white, fontSize: _getDefaultFontSize(platformAlternate)), new BoxDecoration(color: Colors.grey[700L]!.withOpacity(0.9), borderRadius: BorderRadius.CreateAll(Radius.circular(4)))), _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
        var defaultConstraints = new BoxConstraints(minHeight: (widget.height ?? _tooltipTheme.height) ?? (double)_getDefaultTooltipHeight());
        Widget tooltipBox = new _TooltipBox__tooltip(constraints: (widget.constraints ?? _tooltipTheme.constraints) ?? defaultConstraints, textStyle: (widget.textStyle ?? _tooltipTheme.textStyle) ?? defaultTextStyle, textAlign: (widget.textAlign ?? _tooltipTheme.textAlign) ?? _defaultTextAlign, decoration: (widget.decoration ?? _tooltipTheme.decoration) ?? defaultDecoration, padding: (widget.padding ?? _tooltipTheme.padding) ?? _getDefaultPadding(), margin: (widget.margin ?? _tooltipTheme.margin) ?? _defaultMargin, richMessage: widget.richMessage ?? new TextSpan(text: widget.message));
        Widget effectiveChild = new MouseRegion(cursor: widget.mouseCursor ?? MouseCursor.defer, child: widget.child ?? SizedBox.CreateShrink());
...
internal class _TooltipBox__tooltip : StatelessWidget
{
    ...
    internal _TooltipBox__tooltip(BoxConstraints constraints, TextStyle textStyle, TextAlign textAlign, Decoration? decoration, EdgeInsetsGeometry? padding, EdgeInsetsGeometry? margin, InlineSpan richMessage)
    {
        this.constraints = constraints;
        this.textStyle = textStyle;
        this.textAlign = textAlign;
    ...
    public override Widget build(BuildContext context)
    {
        return new ConstrainedBox(constraints: constraints, child: new DefaultTextStyle(style: textStyle, textAlign: textAlign, child: new Container(decoration: decoration, padding: padding, margin: margin, child: new Center(widthFactor: 1.0, heightFactor: 1.0, child: Text.CreateRich(richMessage, style: textStyle, textAlign: textAlign)))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
