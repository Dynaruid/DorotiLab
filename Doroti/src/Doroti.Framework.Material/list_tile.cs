...
public class ListTile : StatelessWidget
{
    ...
    public static IEnumerable<Widget> divideTiles(BuildContext? context = null, IEnumerable<Widget> tiles = default!, Color? color = null)
    {
        ...
        {
            return new DecoratedBox(position: DecorationPosition.foreground, decoration: new BoxDecoration(border: new Border(bottom: Divider.createBorderSide(context, color: color))), child: tile);
            throw new InvalidOperationException("Dart control flow completed without a value.");
        }
        return ((Func<List<Widget>>)(() => { var __collection30919 = new List<Widget>(); __collection30919.AddRange(tiles.take(tiles.Count() - 1L).map(wrapTile)); __collection30919.Add(DartRuntimePrimitives.ConvertValue<Widget>(tiles.Last())); return __collection30919; }))();
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    public override Widget build(BuildContext context)
    {
        ...
        Color backgroundColor = ((tileColor ?? tileTheme.tileColor) ?? theme.listTileTheme.tileColor) ?? defaults.tileColor ?? new Color(0L);
        Color selectedBackgroundColor = ((selectedTileColor ?? tileTheme.selectedTileColor) ?? theme.listTileTheme.selectedTileColor) ?? defaults.tileColor ?? new Color(0L);
        ...
        var states = ((Func<HashSet<WidgetState>>)(() => { var __collection32361 = new HashSet<WidgetState>(); if (!enabled) { __collection32361.Add(WidgetState.disabled); } if (selected) { __collection32361.Add(WidgetState.selected); } return __collection32361; }))();
        ...
        Color? effectiveIconColor = (resolveColor(iconColor, selectedColor, iconColor) ?? resolveColor(tileTheme.iconColor, tileTheme.selectedColor, tileTheme.iconColor)) ?? resolveColor(theme.listTileTheme.iconColor, theme.listTileTheme.selectedColor, theme.listTileTheme.iconColor);
        Color? defaultEffectiveIconColor = resolveColor(defaults.iconColor, defaults.selectedColor, defaults.iconColor, theme.disabledColor);
        Color? effectiveIconButtonColor = (effectiveIconColor ?? (iconButtonTheme.style?.foregroundColor?.resolve(states))) ?? defaultEffectiveIconColor;
        effectiveIconColor ??= defaultEffectiveIconColor;
        Color? effectiveColor = ((resolveColor(textColor, selectedColor, textColor) ?? resolveColor(tileTheme.textColor, tileTheme.selectedColor, tileTheme.textColor)) ?? resolveColor(theme.listTileTheme.textColor, theme.listTileTheme.selectedColor, theme.listTileTheme.textColor)) ?? resolveColor(defaults.textColor, defaults.selectedColor, defaults.textColor, theme.disabledColor);
        var iconThemeData = new IconThemeData(color: effectiveIconColor);
        var iconButtonThemeData = new IconButtonThemeData(style: IconButtonTheme.of(context).style?.copyWith(foregroundColor: new WidgetStatePropertyAll<Color?>(effectiveIconButtonColor)) ?? IconButton.styleFrom(foregroundColor: effectiveIconButtonColor));
        ...
        {
            leadingIcon = DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedDefaultTextStyle(style: leadingAndTrailingStyle!, duration: ConstantsLibrary.kThemeChangeDuration, child: leading!));
        }
        ...
        Widget titleText = new AnimatedDefaultTextStyle(style: titleStyle, duration: ConstantsLibrary.kThemeChangeDuration, child: title ?? new SizedBox());
        ...
        if (subtitle is not null)
        {
            ...
            subtitleText = DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedDefaultTextStyle(style: subtitleStyle, duration: ConstantsLibrary.kThemeChangeDuration, child: subtitle!));
        }
        Widget? trailingIcon = default!;
        if (trailing is not null)
        {
            trailingIcon = DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedDefaultTextStyle(style: leadingAndTrailingStyle!, duration: ConstantsLibrary.kThemeChangeDuration, child: trailing!));
        }
        TextDirection textDirectionLocal = Directionality.of(context);
        EdgeInsets resolvedContentPadding = (contentPadding?.resolve(textDirectionLocal) ?? (tileTheme.contentPadding?.resolve(textDirectionLocal))) ?? defaults.contentPadding!.resolve(textDirectionLocal);
        var mouseStates = ((Func<HashSet<WidgetState>>)(() => { var __collection36732 = new HashSet<WidgetState>(); if (!enabled || (onTap is null) && (onLongPress is null)) { __collection36732.Add(WidgetState.disabled); } return __collection36732; }))();
        MouseCursor effectiveMouseCursor = (WidgetStateProperty.resolveAs(mouseCursor, mouseStates) ?? (tileTheme.mouseCursor?.resolve(mouseStates))) ?? WidgetStateMouseCursor.clickable.resolve(mouseStates);
        ListTileTitleAlignment effectiveTitleAlignment = (titleAlignment ?? tileTheme.titleAlignment) ?? ListTileTitleAlignment.threeLine;
        return new InkWell(customBorder: shape ?? tileTheme.shape, onTap: enabled ? onTap : null, onLongPress: enabled ? onLongPress : null, onFocusChange: onFocusChange, mouseCursor: effectiveMouseCursor, canRequestFocus: enabled, focusNode: focusNode, focusColor: focusColor, hoverColor: hoverColor, splashColor: splashColor, autofocus: autofocus, enableFeedback: (enableFeedback ?? tileTheme.enableFeedback) ?? true, statesController: statesController, child: new Widgets.Semantics(button: internalAddSemanticForOnTap && ((onTap is not null) || (onLongPress is not null)), selected: selected, enabled: enabled, child: new Ink(decoration: new ShapeDecoration(shape: (shape ?? tileTheme.shape) ?? new Border(), color: effectiveTileColor), child: new SafeArea(top: false, bottom: false, minimum: resolvedContentPadding, child: IconTheme.merge(data: iconThemeData, child: new IconButtonTheme(data: iconButtonThemeData, child: new _ListTile__list_tile(leading: leadingIcon, title: titleText, subtitle: subtitleText, trailing: trailingIcon, isDense: _isDenseLayout(theme, tileTheme), visualDensity: (visualDensity ?? tileTheme.visualDensity) ?? theme.visualDensity, isThreeLine: ((isThreeLine ?? tileTheme.isThreeLine) ?? theme.listTileTheme.isThreeLine) ?? false, textDirection: textDirectionLocal, titleBaselineType: titleStyle.textBaseline ?? DartRuntimePrimitives.RequireValue(defaults.titleTextStyle!.textBaseline), subtitleBaselineType: subtitleStyle?.textBaseline ?? DartRuntimePrimitives.RequireValue(defaults.subtitleTextStyle!.textBaseline), horizontalTitleGap: (horizontalTitleGap ?? tileTheme.horizontalTitleGap) ?? 16, minVerticalPadding: (minVerticalPadding ?? tileTheme.minVerticalPadding) ?? DartRuntimePrimitives.RequireValue(defaults.minVerticalPadding), minLeadingWidth: (minLeadingWidth ?? tileTheme.minLeadingWidth) ?? DartRuntimePrimitives.RequireValue(defaults.minLeadingWidth), minTileHeight: minTileHeight ?? tileTheme.minTileHeight, titleAlignment: effectiveTitleAlignment)))))));
    }

    public override void debugFillProperties(DiagnosticPropertiesBuilder properties)
    {
        DiagnosticableDefaults.debugFillProperties(properties);
        properties.add(new FlagProperty("isThreeLine", value: isThreeLine, ifTrue: "THREE_LINE", ifFalse: "TWO_LINE", showName: true));
        properties.add(new FlagProperty("dense", value: dense, ifTrue: "true", ifFalse: "false", showName: true));
        ...
        properties.add(new FlagProperty("enabled", value: enabled, ifTrue: "true", ifFalse: "false", showName: true, defaultValue: true));
        properties.add(new DiagnosticsProperty<Delegate>("onTap", onTap, defaultValue: null));
        properties.add(new DiagnosticsProperty<Delegate>("onLongPress", onLongPress, defaultValue: null));
        properties.add(new DiagnosticsProperty<MouseCursor>("mouseCursor", mouseCursor, defaultValue: null));
        properties.add(new FlagProperty("selected", value: selected, ifTrue: "true", ifFalse: "false", showName: true, defaultValue: false));
        ...
        properties.add(new FlagProperty("autofocus", value: autofocus, ifTrue: "true", ifFalse: "false", showName: true, defaultValue: false));
        ...
        properties.add(new FlagProperty("enableFeedback", value: enableFeedback, ifTrue: "true", ifFalse: "false", showName: true));
        ...
        DartRuntimePrimitives.Assert(() =>
            {
                ...
                {
                    FlutterError.reportError(new FlutterErrorDetails(exception: new FlutterError(new List<DiagnosticsNode> { new ErrorSummary("ListTile background color or ink splashes may be invisible."), new ErrorDescription($"The ListTile is wrapped in a {DartRuntimePrimitives.RuntimeType(intermediateWidget)} that has a background color. " + "Because ListTile paints its background and ink splashes on the nearest Material ancestor, " + $"this {DartRuntimePrimitives.RuntimeType(intermediateWidget)} will hide those effects."), new ErrorHint("To fix this, wrap the ListTile in its own Material widget, " + $"or remove the background color from the intermediate {DartRuntimePrimitives.RuntimeType(intermediateWidget)}.") }), informationCollector: () => new List<DiagnosticsNode> { new DiagnosticsProperty<ListTile>("ListTile", this, expandableValue: true), new DiagnosticsProperty<Widget>($"{DartRuntimePrimitives.RuntimeType(intermediateWidget)}", intermediateWidget, expandableValue: true) }));
                }
                return true;
            });
        return true;
    ...
    internal virtual Widget? _findIntermediateWidget(BuildContext context)
    {
        ...
        ((Element?)context)!.visitAncestorElements((ancestor) =>
        {
            ...
            Color? colorLocal = (Color?)(widgetLocal switch { ColoredBox { color: Color colorAlternate } __object45267 => colorAlternate, DecoratedBox { decoration: BoxDecoration { color: Color colorNested } __object45341 } __object45316 => colorNested, DecoratedBox { decoration: ShapeDecoration { color: Color colorCurrent } __object45420 } __object45395 => colorCurrent, _ => DartRuntimePrimitives.ConvertValue<Color>(null) });
...
internal class _ListTile__list_tile : SlottedMultiChildRenderObjectWidget<_ListTileSlot__list_tile, RenderBox>
{
    public virtual Widget? leading { get; private set; }
...
public class _RenderListTile__list_tile : RenderBox, SlottedContainerRenderObjectMixin<_ListTileSlot__list_tile, RenderBox>
{
    ...
    public virtual DartMap<_ListTileSlot__list_tile, RenderBox> _slotToChild { get; set; } = new DartMap<_ListTileSlot__list_tile, RenderBox>();
    ...
    public override double? computeDistanceToActualBaseline(TextBaseline baseline)
    {
        ...
        BaselineOffset offsetLocal = new BaselineOffset(title.getDistanceToActualBaseline(baseline)).op_Add(parentDataLocal.offset.dy);
        return offsetLocal.offset;
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    public virtual BoxConstraints maxIconHeightConstraint => new BoxConstraints(maxHeight: (isDense ? 48.0 : 56.0) + visualDensity.baseSizeAdjustment.dy);
    ...
    internal virtual (BoxConstraints textConstraints, Size tileSize, double titleY) _computeSizes(Func<RenderBox, BoxConstraints, TextBaseline, double?> getBaseline, Func<RenderBox, BoxConstraints, Size> getSize, BoxConstraints constraints, Action<RenderBox, Offset>? positionChild = null)
    {
        ...
        Size? trailingSize = (trailingLocal is null) ? null : getSize(trailingLocal, iconConstraints);
        DartRuntimePrimitives.Assert(() =>
            {
                if (tileWidth == 0.0)
                {
                ...
                throw DartRuntimePrimitives.AsException(new FlutterError(new List<DiagnosticsNode> { new ErrorSummary($"{overflowedWidget} widget consumes the entire tile width (including ListTile.contentPadding)."), new ErrorDescription($"Either resize the tile width so that the {overflowedWidget.toLowerCase()} widget plus any content padding " + "do not exceed the tile width, or use a sized widget, or consider replacing " + "ListTile with a custom widget."), new ErrorHint("See also: https://api.flutter.dev/flutter/material/ListTile-class.html#material.ListTile.4") }));
            });
        ...
        BoxConstraints textConstraintsLocal = looseConstraints.tighten(width: tileWidth - titleStart - adjustedTrailingWidth);
        ...
        else
        {
            ...
            positionChild?.Invoke(subtitleLocal, new Offset(isLTR ? titleStart : adjustedTrailingWidth, compact ? (minVerticalPadding + titleHeight) : idealSubtitleY));
        ...
        {
            positionChild(title, new Offset(isLTR ? titleStart : adjustedTrailingWidth, titleYLocal));
            if ((leadingLocal is not null) && (leadingSize is not null))
            {
                ...
                positionChild(leadingLocal, new Offset(isLTR ? 0.0 : (tileWidth - DartRuntimePrimitives.RequireValue(leadingSize__57061__value61002).width), titleAlignment._yOffsetFor(DartRuntimePrimitives.RequireValue(leadingSize__57061__value61002).height, tileHeight, this, true)));
            }
            if ((trailingLocal is not null) && (trailingSize is not null))
            {
                ...
                positionChild(trailingLocal, new Offset(isLTR ? (tileWidth - DartRuntimePrimitives.RequireValue(trailingSize__57151__value61289).width) : 0.0, titleAlignment._yOffsetFor(DartRuntimePrimitives.RequireValue(trailingSize__57151__value61289).height, tileHeight, this, false)));
            }
        }
        return (textConstraints: textConstraintsLocal, tileSize: new Size(tileWidth, tileHeight), titleY: titleYLocal);
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    public override double? computeDryBaseline(BoxConstraints constraints, TextBaseline baseline)
    {
        (BoxConstraints textConstraints, Size tileSize, double titleY) sizes = _computeSizes(ChildLayoutHelper.getDryBaseline, ChildLayoutHelper.dryLayoutChild, constraints);
        BaselineOffset titleBaseline = new BaselineOffset(title.getDryBaseline(sizes.textConstraints, baseline)).op_Add(sizes.titleY);
    ...
    {
        Size tileSizeLocal = _computeSizes(ChildLayoutHelper.getBaseline, ChildLayoutHelper.layoutChild, constraints, positionChild: _positionBox).tileSize;
    ...
    public virtual RenderBox? childForSlot(_ListTileSlot__list_tile slot) => _slotToChild.GetValueOrDefault(slot);
    ...
    public override List<DiagnosticsNode> debugDescribeChildren()
    {
        ...
        var childToSlot = new DartMap<RenderBox, _ListTileSlot__list_tile>(_slotToChild.Values, _slotToChild.Keys);
        foreach (RenderBox child in children)
        {
    ...
    public virtual void _moveChild(RenderBox child, _ListTileSlot__list_tile slot, _ListTileSlot__list_tile oldSlot)
    {
...
internal class _LisTileDefaultsM3__list_tile : ListTileThemeData
{
    ...
    internal _LisTileDefaultsM3__list_tile(BuildContext context) : base(contentPadding: EdgeInsetsDirectional.CreateOnly(start: 16.0, end: 24.0), minLeadingWidth: 24, minVerticalPadding: 8, shape: new RoundedRectangleBorder())
    {
        this.context = context;
    }

    public override Color? tileColor => DartRuntimePrimitives.ConvertValue<Color>(Colors.transparent);
    public override TextStyle? titleTextStyle => _textTheme.bodyLarge!.copyWith(color: _colors.onSurface);
    public override TextStyle? subtitleTextStyle => _textTheme.bodyMedium!.copyWith(color: _colors.onSurfaceVariant);
    public override TextStyle? leadingAndTrailingTextStyle => _textTheme.labelSmall!.copyWith(color: _colors.onSurfaceVariant);
    public override Color? selectedColor => DartRuntimePrimitives.ConvertValue<Color>(_colors.primary);
    public override Color? iconColor => DartRuntimePrimitives.ConvertValue<Color>(_colors.onSurfaceVariant);
}
