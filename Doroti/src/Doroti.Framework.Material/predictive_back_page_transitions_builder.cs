...
internal delegate Widget _PredictiveBackGestureDetectorWidgetBuilder__predictive_back_page_transitions_builder(BuildContext context, _PredictiveBackPhase__predictive_back_page_transitions_builder phase, PredictiveBackEvent? startBackEvent, PredictiveBackEvent? currentBackEvent);
...
internal class _PredictiveBackGestureDetector__predictive_back_page_transitions_builder : StatefulWidget
{
    public virtual Func<BuildContext, _PredictiveBackPhase__predictive_back_page_transitions_builder, PredictiveBackEvent?, PredictiveBackEvent?, Widget> builder { get; private set; } = default!;
    public virtual IModalRoute route { get; private set; } = default!;

    internal _PredictiveBackGestureDetector__predictive_back_page_transitions_builder(IModalRoute route, Func<BuildContext, _PredictiveBackPhase__predictive_back_page_transitions_builder, PredictiveBackEvent?, PredictiveBackEvent?, Widget> builder)
    {
        this.route = route;
...
internal class _PredictiveBackGestureDetectorState__predictive_back_page_transitions_builder : State<_PredictiveBackGestureDetector__predictive_back_page_transitions_builder>, WidgetsBindingObserver
{
    ...
    public virtual void handleCancelBackGesture()
    {
        ...
        startBackEvent = DartRuntimePrimitives.ConvertValue<PredictiveBackEvent>(currentBackEvent = null);
    }
...
public class _PredictiveBackSharedElementPageTransition__predictive_back_page_transitions_builder : StatefulWidget
{
    ...
    internal _PredictiveBackSharedElementPageTransition__predictive_back_page_transitions_builder(bool isDelegatedTransition, Animation<double> animation, Animation<double> secondaryAnimation, _PredictiveBackPhase__predictive_back_page_transitions_builder phase, PredictiveBackEvent? startBackEvent, PredictiveBackEvent? currentBackEvent, Widget child)
    {
...
internal class _PredictiveBackSharedElementPageTransitionState__predictive_back_page_transitions_builder : State<_PredictiveBackSharedElementPageTransition__predictive_back_page_transitions_builder>, SingleTickerProviderStateMixin<_PredictiveBackSharedElementPageTransition__predictive_back_page_transitions_builder>
{
    ...
    internal static Interval _kCommitInterval = new Interval(0.0, _kCommitMilliseconds / FadeForwardsPageTransitionsBuilder.kTransitionMilliseconds, curve: _kCurve);
    internal const double _kDeviceBorderRadius = 32.0;
    internal virtual Tween<double> _borderRadiusTween { get; private set; } = new Tween<double>(begin: 0.0, end: _kDeviceBorderRadius);
    internal virtual Tween<double> _opacityTween { get; private set; } = new Tween<double>(begin: 1.0, end: 0.0);
    internal virtual Tween<double> _scaleTween { get; private set; } = new Tween<double>(begin: 1.0, end: _kMinScale);
    ...
    {
        _animation.parent = widget.phase switch { _PredictiveBackPhase__predictive_back_page_transitions_builder.commit => DartRuntimePrimitives.ConvertValue<Animation<double>>(_curvedAnimationReversed), _ => widget.animation };
        _bounceAnimation.parent = widget.phase switch { _PredictiveBackPhase__predictive_back_page_transitions_builder.commit => new Tween<double>(begin: 0.0, end: _lastBounceAnimationValue).animate(_curvedAnimation!), _ => DartRuntimePrimitives.ConvertValue<Animation<double>>(new ReverseAnimation(widget.animation)) };
        _commitAnimation.parent = widget.phase switch { _PredictiveBackPhase__predictive_back_page_transitions_builder.commit => DartRuntimePrimitives.ConvertValue<Animation<double>>(_animation), _ => AnimationsLibrary.kAlwaysDismissedAnimation };
        double xShift = screenSize.width / _kDivisionFactor - _kMargin;
        _positionAnimation = _animation.drive(widget.phase switch { _PredictiveBackPhase__predictive_back_page_transitions_builder.commit => new Tween<Offset>(begin: _lastDrag, end: new Offset(screenSize.height * _kYPositionFactor, 0.0)), _ => new Tween<Offset>(begin: widget.currentBackEvent?.swipeEdge switch { SwipeEdge.left => new Offset(xShift, _getYShiftPosition(screenSize.height)), SwipeEdge.right => new Offset(-xShift, _getYShiftPosition(screenSize.height)), null => new Offset(xShift, _getYShiftPosition(screenSize.height)), _ when DartRuntimePrimitives.NonExhaustiveSwitchGuard => throw new InvalidOperationException("Non-exhaustive Dart switch value.") }, end: Offset.zero) });
    }
    ...
    internal virtual void _updateCurvedAnimations()
    {
        ...
        _curvedAnimationReversed = new CurvedAnimation(parent: new ReverseAnimation(widget.animation), curve: _kCommitInterval);
    }
    ...
    public override Widget build(BuildContext context)
    {
        return new AnimatedBuilder(animation: widget.animation, builder: (context, child) =>
        {
            _lastBounceAnimationValue = _bounceAnimation.value;
            return Transform.CreateScale(scale: _scaleTween.evaluate(_bounceAnimation), child: Transform.CreateTranslate(offset: widget.phase switch { _PredictiveBackPhase__predictive_back_page_transitions_builder.commit => _positionAnimation.value, _ => _lastDrag = new Offset(_positionAnimation.value.dx, _getYShiftPosition(MediaQuery.heightOf(context))) }, child: new Opacity(opacity: _opacityTween.evaluate(_commitAnimation), child: new ClipRRect(borderRadius: MediaQuery.displayCornerRadiiOf(context) ?? BorderRadius.CreateCircular(_borderRadiusTween.evaluate(_bounceAnimation)), child: child))));
            throw new InvalidOperationException("Dart closure completed without a value.");
        }, child: widget.child);
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _PredictiveBackFullscreenPageTransition__predictive_back_page_transitions_builder : StatefulWidget
{
    public virtual Animation<double> animation { get; private set; } = default!;
    public virtual Animation<double> secondaryAnimation { get; private set; } = default!;
    public virtual _PredictiveBackPhase__predictive_back_page_transitions_builder phase { get; private set; } = default!;
    ...
    internal _PredictiveBackFullscreenPageTransition__predictive_back_page_transitions_builder(Animation<double> animation, Animation<double> secondaryAnimation, Func<bool> getIsCurrent, _PredictiveBackPhase__predictive_back_page_transitions_builder phase, Widget child)
    {
        this.animation = animation;
        this.secondaryAnimation = secondaryAnimation;
...
internal class _PredictiveBackFullscreenPageTransitionState__predictive_back_page_transitions_builder : State<_PredictiveBackFullscreenPageTransition__predictive_back_page_transitions_builder>
{
    ...
    internal virtual Animatable<double> _primaryOpacityTween { get; private set; } = new Tween<double>(begin: _kOpacityStartTransition, end: _kOpacityFullyOpened);
    internal virtual Animatable<double> _primaryScaleTween { get; private set; } = new TweenSequence<double>(new List<TweenSequenceItem<double>> { new TweenSequenceItem<double>(tween: new Tween<double>(begin: _kScaleStart, end: _kScaleStart), weight: _kWeightPreCommit), new TweenSequenceItem<double>(tween: new Tween<double>(begin: _kScaleCommit, end: _kScaleStart), weight: _kWeightPostCommit) });
    internal virtual ConstantTween<double> _secondaryScaleTweenCurrent { get; private set; } = new ConstantTween<double>(_kScaleStart);
    internal virtual TweenSequence<double> _secondaryTweenScale { get; private set; } = new TweenSequence<double>(new List<TweenSequenceItem<double>> { new TweenSequenceItem<double>(tween: new Tween<double>(begin: _kScaleCommit, end: _kScaleStart), weight: _kWeightPreCommit), new TweenSequenceItem<double>(tween: new Tween<double>(begin: _kScaleStart, end: _kScaleStart), weight: _kWeightPostCommit) });
    internal virtual ConstantTween<double> _secondaryOpacityTweenCurrent { get; private set; } = new ConstantTween<double>(_kOpacityFullyOpened);
    internal virtual TweenSequence<double> _secondaryOpacityTween { get; private set; } = new TweenSequence<double>(new List<TweenSequenceItem<double>> { new TweenSequenceItem<double>(tween: new Tween<double>(begin: _kOpacityFullyOpened, end: _kOpacityStartTransition), weight: _kWeightPreCommit), new TweenSequenceItem<double>(tween: new Tween<double>(begin: _kOpacityFullyOpened, end: _kOpacityFullyOpened), weight: _kWeightPostCommit) });
    ...
    public override void didChangeDependencies()
    {
        ...
        _primaryPositionTween = DartRuntimePrimitives.ConvertValue<Animatable<Offset>>(new TweenSequence<Offset>(new List<TweenSequenceItem<Offset>> { new TweenSequenceItem<Offset>(tween: new Tween<Offset>(begin: Offset.zero, end: Offset.zero), weight: _kWeightPreCommit), new TweenSequenceItem<Offset>(tween: new Tween<Offset>(begin: new Offset(xShift, 0.0), end: Offset.zero), weight: _kWeightPostCommit) }.Cast<TweenSequenceItem<Offset>>().ToList()));
        _secondaryCurrentPositionTween = DartRuntimePrimitives.ConvertValue<Animatable<Offset>>(new ConstantTween<Offset>(Offset.zero));
        _secondaryPositionTween = DartRuntimePrimitives.ConvertValue<Animatable<Offset>>(new Tween<Offset>(begin: new Offset(xShift, 0.0), end: Offset.zero));
    }

    internal virtual Widget _secondaryAnimatedBuilder(BuildContext context, Widget? child)
    {
        bool isCurrent = widget.getIsCurrent();
        return Transform.CreateTranslate(offset: isCurrent ? _secondaryCurrentPositionTween.evaluate(widget.secondaryAnimation) : _secondaryPositionTween.evaluate(widget.secondaryAnimation), child: Transform.CreateScale(scale: isCurrent ? _secondaryScaleTweenCurrent.evaluate(widget.secondaryAnimation) : _secondaryTweenScale.evaluate(widget.secondaryAnimation), child: new Opacity(opacity: isCurrent ? _secondaryOpacityTweenCurrent.evaluate(widget.secondaryAnimation) : _secondaryOpacityTween.evaluate(widget.secondaryAnimation), child: child)));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    internal virtual Widget _primaryAnimatedBuilder(BuildContext context, Widget? child)
    {
        return Transform.CreateTranslate(offset: _primaryPositionTween.evaluate(widget.animation), child: Transform.CreateScale(scale: _primaryScaleTween.evaluate(widget.animation), child: new Opacity(opacity: _primaryOpacityTween.evaluate(widget.animation), child: new AnimatedOpacity(opacity: widget.phase switch { _PredictiveBackPhase__predictive_back_page_transitions_builder.commit => 0.0, _ => (widget.animation.value < _kCommitAt) ? 0.0 : 1.0 }, duration: _kCommitDuration, child: child))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    public override Widget build(BuildContext context)
    {
        return new AnimatedBuilder(animation: widget.secondaryAnimation, builder: _secondaryAnimatedBuilder, child: new AnimatedBuilder(animation: widget.animation, builder: _primaryAnimatedBuilder, child: new ClipRRect(borderRadius: MediaQuery.displayCornerRadiiOf(context) ?? BorderRadius.CreateAll(Radius.circular(_PredictiveBackSharedElementPageTransitionState__predictive_back_page_transitions_builder._kDeviceBorderRadius)), child: widget.child)));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
