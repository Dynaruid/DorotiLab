...
public class BottomAppBar : StatefulWidget
{
    ...
    public BottomAppBar(Key? key = null, Color? color = null, double? elevation = null, NotchedShape? shape = null, Clip clipBehavior = Clip.none, double notchMargin = 4.0, Widget? child = null, EdgeInsetsGeometry? padding = null, Color? surfaceTintColor = null, Color? shadowColor = null, double? height = null) : base(key: key)
    {
        this.color = color;
...
internal class _BottomAppBarState__bottom_app_bar : State<BottomAppBar>
{
    ...
    public override Widget build(BuildContext context)
    {
        ...
        CustomClipper<Path> clipperLocal = ((notchedShape is not null) && hasFab) ? new _BottomAppBarClipper__bottom_app_bar(geometry: geometryListenable, shape: notchedShape, materialKey: materialKey, notchMargin: widget.notchMargin) : new global::Doroti.Framework.Rendering.ShapeBorderClipper(shape: new global::Doroti.Framework.Painting.RoundedRectangleBorder());
        ...
        Color surfaceTintColorLocal = (widget.surfaceTintColor ?? babTheme.surfaceTintColor) ?? defaults.surfaceTintColor!;
        Color effectiveColor = ElevationOverlay.applySurfaceTint(colorLocal, surfaceTintColorLocal, elevationLocal);
        Color shadowColorLocal = (widget.shadowColor ?? babTheme.shadowColor) ?? defaults.shadowColor!;
        Widget childLocal = new SizedBox(height: heightLocal, child: new Padding(padding: (widget.padding ?? babTheme.padding) ?? EdgeInsets.CreateSymmetric(vertical: 12.0, horizontal: 16.0), child: widget.child));
        var material = new Material(key: materialKey, type: MaterialType.transparency, child: new SafeArea(child: childLocal));
        return new PhysicalShape(clipper: clipperLocal, elevation: elevationLocal, shadowColor: shadowColorLocal, color: effectiveColor, clipBehavior: widget.clipBehavior, child: material);
    }
...
internal class _BottomAppBarClipper__bottom_app_bar : CustomClipper<Path>
{
    ...
    internal _BottomAppBarClipper__bottom_app_bar(ValueListenable<ScaffoldGeometry> geometry, NotchedShape shape, GlobalKey<IState> materialKey, double notchMargin) : base(reclip: geometry)
    {
    ...
    {
        Rect? button = geometry.value.floatingActionButtonArea?.translate(0.0, bottomNavigationBarTop * -1.0);
...
internal class _BottomAppBarDefaultsM3__bottom_app_bar : BottomAppBarThemeData
{
    ...
    internal _BottomAppBarDefaultsM3__bottom_app_bar(BuildContext context) : base(elevation: 3.0, height: 80.0, shape: new AutomaticNotchedShape(new RoundedRectangleBorder()))
    {
