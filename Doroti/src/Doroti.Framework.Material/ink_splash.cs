...
{
    var __cascade = new AnimationController(duration: Ink_splashLibrary._kUnconfirmedSplashDuration, vsync: controller.vsync);
    __cascade.addListener(controller.markNeedsPaint);
    __cascade.forward();
    return __cascade;
}))();
        _radius = _radiusController.drive(new Tween<double>(begin: Ink_splashLibrary._kSplashInitialSize, end: _targetRadius));
        _alphaController = ((Func<AnimationController>)(() =>
{
    var __cascade = new AnimationController(duration: Ink_splashLibrary._kSplashFadeDuration, vsync: controller.vsync);
    __cascade.addListener(controller.markNeedsPaint);
    __cascade.addStatusListener(_handleAlphaStatusChanged);
    return __cascade;
}))();
        _alpha = _alphaController!.drive(new IntTween(begin: color.alpha, end: 0L));
