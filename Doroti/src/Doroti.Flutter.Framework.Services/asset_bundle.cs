...
public abstract class AssetBundle
{
    ...
    public async virtual Future<string> loadString(string key, bool cache = true)
    {
        ...
        {
            return global::Doroti.Flutter.Runtime.Dart_convertLibrary.utf8.decode(new Uint8List(data));
        }
...
public class NetworkAssetBundle : AssetBundle
{
    ...
    internal virtual global::Doroti.Flutter.Runtime.HttpClient _httpClient { get; private set; } = default!;

    public NetworkAssetBundle(DartUri baseUrl)
    {
        this._baseUrl = baseUrl;
        this._httpClient = new global::Doroti.Flutter.Runtime.HttpClient();
    }
    ...
    {
        global::Doroti.Flutter.Runtime.HttpClientRequest request = await _httpClient.getUrl(_urlFromKey(key));
        global::Doroti.Flutter.Runtime.HttpClientResponse response = await request.close();
...
public class PlatformAssetBundle : CachingAssetBundle
{
    ...
    {
        Uint8List encoded = global::Doroti.Flutter.Runtime.Dart_convertLibrary.utf8.encode(new DartUri(path: DartUri.encodeFull(key)).path);
