...
public class LengthLimitingTextInputFormatter : TextInputFormatter
{
    ...
    public static TextEditingValue truncate(TextEditingValue value, long maxLength)
    {
        ...
        return new TextEditingValue(text: truncated, selection: value.selection.copyWith(baseOffset: Math.Min(value.selection.start, truncated.Length), extentOffset: Math.Min(value.selection.end, truncated.Length)), composing: ((!value.composing.isCollapsed && (truncated.Length > value.composing.start)) ? new global::Doroti.Flutter.Ui.TextRange(start: value.composing.start, end: Math.Min(value.composing.end, truncated.Length)) : TextRange.empty));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
