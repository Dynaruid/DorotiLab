...
public abstract class TextBoundary
{
    ...
    public virtual TextRange getTextBoundaryAt(long position)
    {
        ...
        return new global::Doroti.Flutter.Ui.TextRange(start: start, end: end);
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
public class CharacterBoundary : TextBoundary
{
    ...
    public override TextRange getTextBoundaryAt(long position)
    {
        if ((position < 0L))
        {
            return new global::Doroti.Flutter.Ui.TextRange(start: -1L, end: (getTrailingTextBoundaryAt(position) ?? -1L));
        }
        else
        {
            if ((position >= _text.Length))
            {
                return new global::Doroti.Flutter.Ui.TextRange(start: (getLeadingTextBoundaryAt(position) ?? -1L), end: -1L);
            }
        }
        var rangeAtPosition = new CharacterRange(_text, position);
        return ((rangeAtPosition.Count != 0) ? new global::Doroti.Flutter.Ui.TextRange(start: rangeAtPosition.stringBeforeLength, end: (rangeAtPosition.stringBeforeLength + rangeAtPosition.Current.Length)) : new global::Doroti.Flutter.Ui.TextRange(start: rangeAtPosition.stringBeforeLength, end: (getTrailingTextBoundaryAt(position) ?? -1L)));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
public class LineBoundary : TextBoundary
{
    ...
    public override TextRange getTextBoundaryAt(long position) => _textLayout.getLineAtOffset(new global::Doroti.Flutter.Ui.TextPosition(offset: Math.Max(position, 0L)));
}
