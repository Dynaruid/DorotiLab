...
public class CupertinoPageScaffold : StatefulWidget
{
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(
            new _CupertinoPageScaffoldState__page_scaffold()
        );
}

internal class _CupertinoPageScaffoldState__page_scaffold
    : State<CupertinoPageScaffold>,
        WidgetsBindingObserver
{
    internal virtual GlobalKey<IState> _statusBarKey { get; private set; } =
    ...
    public virtual void handleStatusBarTap()
    {
        ...
        if (
            ...
            && _HitTestableAtOrigin__page_scaffold.hitTestableAtOrigin(_statusBarKey)
        )
        {
    ...
    public override Widget build(BuildContext context)
    {
        ...
        if (widget.navigationBar is not null)
        {
            ...
            double bottomPadding = widget.resizeToAvoidBottomInset
            ...
            if (fullObstruction)
            {
                paddedContent = DartRuntimePrimitives.ConvertValue<Widget>(
                    new MediaQuery(
        ...
        return new ScrollNotificationObserver(
            child: new DecoratedBox(
                decoration: new BoxDecoration(color: backgroundColorLocal),
                    ...
                    child: new Stack(
                        children: (
                            (Func<List<Widget>>)(
                                () =>
                                {
                                    var __collection8262 = new List<Widget>();
                                    __collection8262.Add(
                                        DartRuntimePrimitives.ConvertValue<Widget>(paddedContent)
                                    );
                                    if (widget.navigationBar is not null)
                                    {
                                        __collection8262.Add(
                                    ...
                                    __collection8262.Add(
                                        DartRuntimePrimitives.ConvertValue<Widget>(
                                            new Positioned(
                                                ...
                                                child: new _HitTestableAtOrigin__page_scaffold(
                                                    _statusBarKey
                                                )
                                            )
                                        )
                                    );
                                    return __collection8262;
                                }
                            )
                        )()
                    )
...
public class CupertinoPageScaffoldBackgroundColor : InheritedWidget
{
    ...
    public CupertinoPageScaffoldBackgroundColor(Widget child, Color color, Key? key = null)
