...
{
    internal static EdgeInsetsGeometry _kHorizontalItemPadding = EdgeInsets.CreateSymmetric(vertical: 2, horizontal: 3);
}
...
public static partial class Sliding_segmented_controlLibrary
{
    internal static CupertinoDynamicColor _kThumbColor = new CupertinoDynamicColor(color: new Color(4294967295L), darkColor: new Color(4284703590L));
}
...
public static partial class Sliding_segmented_controlLibrary
{
    internal static Physics.SpringSimulation _kThumbSpringAnimationSimulation = new Physics.SpringSimulation(new Physics.SpringDescription(mass: 1, stiffness: 503.551, damping: 44.8799), 0, 1, 0);
}
...
public class _Segment__sliding_segmented_control<T> : StatefulWidget
{
    ...
    internal _Segment__sliding_segmented_control(ValueKey<T> key, Widget child, bool pressed, bool highlighted, bool isDragging, bool enabled, _SegmentLocation__sliding_segmented_control segmentLocation, bool isMomentary) : base(key: key)
    {
        this.child = child;
...
public class _SegmentState__sliding_segmented_control<T> : State<_Segment__sliding_segmented_control<T>>, TickerProviderStateMixin<_Segment__sliding_segmented_control<T>>
{
    public virtual AnimationController highlightPressScaleController { get; private set; } = default!;
    ...
    {
        base.initState();
        highlightPressScaleController = new AnimationController(duration: Sliding_segmented_controlLibrary._kOpacityAnimationDuration, value: widget.shouldScaleContent ? 1 : 0, vsync: this);
        highlightPressScaleAnimation = highlightPressScaleController.drive(new Tween<double>(begin: 1.0, end: Sliding_segmented_controlLibrary._kMinThumbScale));
    }
    ...
    public override void didUpdateWidget(_Segment__sliding_segmented_control<T> oldWidget)
    {
        ...
        {
            Animatable<double> scaleAnimation = (widget.isMomentary && widget.shouldScaleContent) ? new global::Doroti.Framework.Animation.TweenSequence<double>(new List<global::Doroti.Framework.Animation.TweenSequenceItem<double>> { new global::Doroti.Framework.Animation.TweenSequenceItem<double>(tween: new global::Doroti.Framework.Animation.Tween<double>(begin: highlightPressScaleAnimation.value, end: Sliding_segmented_controlLibrary._kMaxThumbScaleForMomentary), weight: 50), new global::Doroti.Framework.Animation.TweenSequenceItem<double>(tween: new global::Doroti.Framework.Animation.Tween<double>(begin: Sliding_segmented_controlLibrary._kMaxThumbScaleForMomentary, end: 1.0), weight: 50) }) : new global::Doroti.Framework.Animation.Tween<double>(begin: highlightPressScaleAnimation.value, end: widget.shouldScaleContent ? Sliding_segmented_controlLibrary._kMinThumbScale : 1.0);
    ...
    {
        Alignment scaleAlignment = widget.segmentLocation switch { _SegmentLocation__sliding_segmented_control.leftmost => Alignment.centerLeft, _SegmentLocation__sliding_segmented_control.rightmost => Alignment.centerRight, _SegmentLocation__sliding_segmented_control.inbetween => Alignment.center, _ => throw new InvalidOperationException("Non-exhaustive Dart switch value.") };
        return new MetaData(behavior: HitTestBehavior.opaque, child: new IndexedStack(alignment: Alignment.center, children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedOpacity(opacity: widget.shouldFadeoutContent ? Sliding_segmented_controlLibrary._kContentPressedMinOpacity : 1, duration: Sliding_segmented_controlLibrary._kOpacityAnimationDuration, curve: Curves.ease, child: new AnimatedDefaultTextStyle(style: DefaultTextStyle.of(context).style.merge(new TextStyle(fontWeight: widget.shouldHighlightContent ? Sliding_segmented_controlLibrary._kHighlightedFontWeight : Sliding_segmented_controlLibrary._kFontWeight, fontSize: Sliding_segmented_controlLibrary._kFontSize, color: widget.enabled ? null : Sliding_segmented_controlLibrary._kDisabledContentColor)), duration: Sliding_segmented_controlLibrary._kHighlightAnimationDuration, curve: Curves.ease, child: new ScaleTransition(alignment: scaleAlignment, scale: highlightPressScaleAnimation, child: widget.child)))), DartRuntimePrimitives.ConvertValue<Widget>(DefaultTextStyle.merge(style: new TextStyle(fontWeight: Sliding_segmented_controlLibrary._kHighlightedFontWeight, fontSize: Sliding_segmented_controlLibrary._kFontSize), child: widget.child)) }));
...
public class _SegmentSeparator__sliding_segmented_control : StatefulWidget
{
    ...
    internal _SegmentSeparator__sliding_segmented_control(ValueKey<long> key, bool highlighted) : base(key: key)
    {
        this.highlighted = highlighted;
    }
...
public class _SegmentSeparatorState__sliding_segmented_control : State<_SegmentSeparator__sliding_segmented_control>, TickerProviderStateMixin<_SegmentSeparator__sliding_segmented_control>
{
    ...
    {
        base.initState();
        separatorOpacityController = new AnimationController(duration: Sliding_segmented_controlLibrary._kSpringAnimationDuration, value: widget.highlighted ? 0 : 1, vsync: this);
    }
    ...
    public override Widget build(BuildContext context)
    {
        return new AnimatedBuilder(animation: separatorOpacityController, child: new SizedBox(width: Sliding_segmented_controlLibrary._kSeparatorWidth), builder: (context, child) =>
        {
            return new Padding(padding: Sliding_segmented_controlLibrary._kSeparatorInset, child: new DecoratedBox(decoration: new BoxDecoration(color: Sliding_segmented_controlLibrary._kSeparatorColor.withOpacity(Sliding_segmented_controlLibrary._kSeparatorColor.opacity * separatorOpacityController.value), borderRadius: BorderRadius.CreateAll(Sliding_segmented_controlLibrary._kSeparatorRadius)), child: child));
...
public class CupertinoSlidingSegmentedControl<T> : StatefulWidget where T : notnull
{
    ...
    public CupertinoSlidingSegmentedControl(Key? key = null, DartMap<T, Widget> children = default!, Action<T?> onValueChanged = default!, HashSet<T> disabledChildren = default!, T? groupValue = default, Color thumbColor = default!, EdgeInsetsGeometry padding = default!, Color backgroundColor = default!, bool proportionalWidth = false, bool isMomentary = false) : base(key: key)
    {
        ...
        EdgeInsetsGeometry __padding = padding ?? Sliding_segmented_controlLibrary._kHorizontalItemPadding;
...
public class _SlidingSegmentButton__sliding_segmented_control<T> : StatefulWidget
{
    ...
    internal _SlidingSegmentButton__sliding_segmented_control(Key? key = null, T value = default!, Widget child = default!, bool enabled = default!) : base(key: key)
    {
        this.value = value;
        this.child = child;
        this.enabled = enabled;
...
internal class _SlidingSegmentButtonState__sliding_segmented_control<T> : State<_SlidingSegmentButton__sliding_segmented_control<T>>, RadioClient<T>
{
    ...
    {
        base.initState();
        _focusNode = new FocusNode(debugLabel: $"CupertinoSlidingSegmentedControl<{typeof(T)}>[{widget.value}]");
    }
...
public class _SegmentedControlState__sliding_segmented_control<T> : State<CupertinoSlidingSegmentedControl<T>>, TickerProviderStateMixin<CupertinoSlidingSegmentedControl<T>> where T : notnull
{
    ...
    public virtual AnimationController thumbController
    {
        get
        {
            ...
            {
                __late_thumbController = new AnimationController(duration: Sliding_segmented_controlLibrary._kSpringAnimationDuration, value: 0, vsync: this);
    ...
    public virtual AnimationController thumbScaleController
    {
        get
        {
            ...
            {
                __late_thumbScaleController = new AnimationController(duration: Sliding_segmented_controlLibrary._kSpringAnimationDuration, value: 0, vsync: this);
    ...
    public virtual Animation<double> thumbScaleAnimation
    {
        get
        {
            ...
            {
                __late_thumbScaleAnimation = thumbScaleController.drive(new Tween<double>(begin: 1, end: Sliding_segmented_controlLibrary._kMinThumbScale));
    ...
    public virtual Gestures.TapGestureRecognizer tap { get; private set; } = new Gestures.TapGestureRecognizer();
    public virtual Gestures.HorizontalDragGestureRecognizer drag { get; private set; } = new Gestures.HorizontalDragGestureRecognizer();
    public virtual Gestures.LongPressGestureRecognizer longPress { get; private set; } = new Gestures.LongPressGestureRecognizer();
    public virtual GlobalKey<IState> segmentedControlRenderWidgetKey { get; private set; } = GlobalKey<IState>.Create();
    internal virtual DartMap<T, GlobalKey<_SlidingSegmentButtonState__sliding_segmented_control<T>>> _segmentKeys { get; private set; } = new DartMap<T, GlobalKey<_SlidingSegmentButtonState__sliding_segmented_control<T>>>();
    ...
    public override void initState()
    {
        ...
        DartRuntimePrimitives.Ignore(((Func<Gestures.HorizontalDragGestureRecognizer>)(() =>
{
    var __cascade = drag;
    ...
    internal virtual bool _hasDraggedTooFar(Gestures.DragUpdateDetails details)
    {
        ...
        Offset offCenter = details.localPosition - new Offset(sizeLocal.width / 2L, sizeLocal.height / 2L);
    ...
    {
        thumbScaleAnimation = thumbScaleController.drive(new Tween<double>(begin: thumbScaleAnimation.value, end: isExpanding ? 1 : Sliding_segmented_controlLibrary._kMinThumbScale));
    ...
    public override Widget build(BuildContext context)
    {
        ...
        foreach (MapEntry<T, Widget> entry in widget.children.entries)
        {
            ...
            if (index != 0L)
            {
                childrenLocal.Add(new _SegmentSeparator__sliding_segmented_control(key: new ValueKey<long>(index), highlighted: isPreviousSegmentHighlighted || isHighlighted));
            }
            ...
            GlobalKey<_SlidingSegmentButtonState__sliding_segmented_control<T>> segmentKey = _segmentKeys.putIfAbsent(entry.key, () => GlobalKey<_SlidingSegmentButtonState__sliding_segmented_control<T>>.Create());
            childrenLocal.Add(new _SlidingSegmentButton__sliding_segmented_control<T>(key: segmentKey, value: entry.key, enabled: !widget.disabledChildren.Contains(entry.key), child: new Widgets.Semantics(button: true, onTap: () =>
            {
        ...
        }, child: new UnconstrainedBox(constrainedAxis: Axis.horizontal, child: new Container(clipBehavior: Clip.antiAlias, padding: widget.padding.resolve(Directionality.of(context)), decoration: new ShapeDecoration(shape: new RoundedSuperellipseBorder(borderRadius: BorderRadius.CreateAll(Radius.circular(DialogLibrary._kCornerRadius))), color: CupertinoDynamicColor.resolve(widget.backgroundColor, context)), child: new AnimatedBuilder(animation: thumbScaleAnimation, builder: (context, child) =>
        {
...
internal class _SegmentedControlRenderWidget__sliding_segmented_control<T> : MultiChildRenderObjectWidget where T : notnull
{
    ...
    internal _SegmentedControlRenderWidget__sliding_segmented_control(Key? key = null, List<Widget> children = default!, long? highlightedIndex = default!, Color thumbColor = default!, double thumbScale = default!, bool proportionalWidth = default!, _SegmentedControlState__sliding_segmented_control<T> state = default!) : base(key: key, children: children ?? new List<Widget>())
    {
...
internal class _SegmentedControlContainerBoxParentData__sliding_segmented_control : ContainerBoxParentData<RenderBox>
{
}
...
public class _RenderSegmentedControl__sliding_segmented_control<T> : RenderBox, ContainerRenderObjectMixin<RenderBox, ContainerBoxParentData<RenderBox>>, RenderBoxContainerDefaultsMixin<RenderBox, ContainerBoxParentData<RenderBox>> where T : notnull
{
    public virtual _SegmentedControlState__sliding_segmented_control<T> state { get; private set; } = default!;
    ...
    internal virtual Size _computeOverallSize(BoxConstraints constraints)
    {
        ...
        return constraints.constrain(new Size(_getChildWidths(constraints).sum() + totalSeparatorWidth, maxChildHeight));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    public override double? computeDryBaseline(BoxConstraints constraints, TextBaseline baseline)
    {
        ...
        while (child is not null)
        {
            var childConstraints = BoxConstraints.CreateTight(new Size(segmentWidths[(int)index], childHeight));
    ...
    public override void performLayout()
    {
        ...
        var separatorConstraints = new BoxConstraints(minHeight: childHeight, maxHeight: childHeight);
        RenderBox? child = firstChild;
        var index = 0L;
        double start = 0;
        while (child is not null)
        {
            var childConstraints = BoxConstraints.CreateTight(new Size(segmentWidths[(int)checked(index / 2L)], childHeight));
    ...
    public virtual Rect? moveThumbRectInBound(Rect? thumbRect, List<RenderBox> children)
    {
        ...
        Offset firstChildOffset = ((_SegmentedControlContainerBoxParentData__sliding_segmented_control?)children.First().parentData!)!.offset;
    ...
    public override void paint(PaintingContext context, Offset offset)
    {
        ...
        if (highlightedChildIndex is not null)
        {
            ...
            RenderBox selectedChild = children[(int)(DartRuntimePrimitives.RequireValue(highlightedChildIndex__50131__value50234) * 2L)];
            var childParentData = ((_SegmentedControlContainerBoxParentData__sliding_segmented_control?)selectedChild.parentData!)!;
            Rect newThumbRect = Sliding_segmented_controlLibrary._kThumbInsets.inflateRect(childParentData.offset & selectedChild.size);
            if (state.thumbController.isAnimating)
            {
                Animatable<Rect?>? thumbTween = state.thumbAnimatable;
                if (thumbTween is null)
                {
                    Rect startingRect = moveThumbRectInBound(currentThumbRect, children) ?? newThumbRect;
                ...
                else
                {
                    ...
                    {
                        Rect startingRectLocal = moveThumbRectInBound(currentThumbRect, children) ?? newThumbRect;
                        state.thumbAnimatable = new RectTween(begin: startingRectLocal, end: newThumbRect).chain(new CurveTween(curve: new Interval(DartRuntimePrimitives.RequireValue(state.thumbController.value), 1)));
                    }
                }
            }
            else
            ...
            Rect unscaledThumbRect = state.thumbAnimatable?.evaluate(state.thumbController) ?? newThumbRect;
            ...
            var thumbRect = Rect.fromCenter(center: unscaledThumbRect.center - new Offset(delta / 2L, 0), width: unscaledThumbRect.width * thumbScale, height: unscaledThumbRect.height * thumbScale);
    ...
    {
        var thumbShadow = new List<BoxShadow> { new BoxShadow(color: new Color(520093696L), offset: new Offset(0, 3), blurRadius: 8), new BoxShadow(color: new Color(167772160L), offset: new Offset(0, 3), blurRadius: 1) };
