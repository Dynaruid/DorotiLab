...
public static partial class Desktop_text_selection_toolbarLibrary
{
    internal static List<BoxShadow> _kToolbarShadow = new List<BoxShadow> { new BoxShadow(color: Color.fromARGB(60L, 0L, 0L, 0L), blurRadius: 10.0, spreadRadius: 0.5, offset: new Offset(0.0, 4.0)) };
}

public static partial class Desktop_text_selection_toolbarLibrary
{
    internal static CupertinoDynamicColor _kToolbarBorderColor = new CupertinoDynamicColor(color: new Color(4290295992L), darkColor: new Color(4284177243L));
}

public static partial class Desktop_text_selection_toolbarLibrary
{
    internal static CupertinoDynamicColor _kToolbarBackgroundColor = new CupertinoDynamicColor(color: new Color(3003121663L), darkColor: new Color(2989502512L));
}
...
public class CupertinoDesktopTextSelectionToolbar : StatelessWidget
{
    ...
    public CupertinoDesktopTextSelectionToolbar(Key? key = null, Offset anchor = default!, List<Widget> children = default!) : base(key: key)
    {
        this.anchor = anchor;
    ...
    {
        return new Container(width: Desktop_text_selection_toolbarLibrary._kToolbarWidth, clipBehavior: Clip.hardEdge, decoration: new ShapeDecoration(shadows: Desktop_text_selection_toolbarLibrary._kToolbarShadow, shape: new RoundedSuperellipseBorder(borderRadius: BorderRadius.CreateAll(Desktop_text_selection_toolbarLibrary._kToolbarBorderRadius))), child: new BackdropFilter(filter: new ImageFilter(outer: ColorFilter.matrix(_matrixWithSaturation(Desktop_text_selection_toolbarLibrary._kToolbarSaturationBoost)), inner: new ImageFilter(sigmaX: Desktop_text_selection_toolbarLibrary._kToolbarBlurSigma, sigmaY: Desktop_text_selection_toolbarLibrary._kToolbarBlurSigma)), child: new DecoratedBox(decoration: new ShapeDecoration(color: Desktop_text_selection_toolbarLibrary._kToolbarBackgroundColor.resolveFrom(context), shape: new RoundedSuperellipseBorder(side: new BorderSide(color: Desktop_text_selection_toolbarLibrary._kToolbarBorderColor.resolveFrom(context)), borderRadius: BorderRadius.CreateAll(Desktop_text_selection_toolbarLibrary._kToolbarBorderRadius))), child: new Padding(padding: Desktop_text_selection_toolbarLibrary._kToolbarPadding, child: child))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
