...
public class CupertinoExpansionTile : StatefulWidget
{
    ...
    public CupertinoExpansionTile(Key? key = null, Widget title = default!, Widget child = default!, ExpansibleController? controller = null, ExpansionTileTransitionMode transitionMode = ExpansionTileTransitionMode.fade) : base(key: key)
    {
        this.title = title;
        this.child = child;
...
{
    internal virtual GlobalKey<IState> _headerKey { get; private set; } = GlobalKey<IState>.Create();
    internal virtual OverlayPortalController _fadeController { get; private set; } = new OverlayPortalController();
    ...
    {
        _iconTurns = animation.drive(_quarterTween.chain(new CurveTween(curve: Expansion_tileLibrary._kAnimationCurve)));
        return (Widget?)new RotationTransition(turns: _iconTurns, child: SizedBox.CreateSquare(dimension: CupertinoTheme.of(context).textTheme.textStyle.fontSize, child: new Center(child: new Icon(CupertinoIcons.right_chevron, color: CupertinoColors.activeBlue, size: Expansion_tileLibrary._kIconFontSize, fontWeight: FontWeight.w900))));
    ...
    internal virtual Widget _buildHeader(BuildContext context, Animation<double> animation)
    {
        ...
        return new Widgets.Semantics(hint: semanticsHint, onTapHint: onTapHintLocal, child: new CupertinoListTile(key: _headerKey, onTap: () => { _onHeaderTap(); return null!; }, title: widget.title, trailing: _buildIcon(context, animation), backgroundColorActivated: CupertinoColors.transparent));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    internal virtual Widget _buildExpansible(BuildContext context, Widget header, Widget body, Animation<double> animation)
    {
        Widget childLocal = new Column(mainAxisSize: MainAxisSize.min, children: ((Func<List<Widget>>)(() => { var __collection7070 = new List<Widget>(); __collection7070.Add(DartRuntimePrimitives.ConvertValue<Widget>(header)); if (animation.isAnimating && Equals(widget.transitionMode, ExpansionTileTransitionMode.fade)) { __collection7070.Add(DartRuntimePrimitives.ConvertValue<Widget>(new Opacity(opacity: 0.0, child: body))); } else { __collection7070.Add(DartRuntimePrimitives.ConvertValue<Widget>(body)); } return __collection7070; }))());
        ...
        {
            return new OverlayPortal(controller: _fadeController, overlayChildBuilder: (context) =>
            {
                ...
                Offset headerOffset = headerBox.localToGlobal(Offset.zero, ancestor: overlay);
                return new Positioned(top: headerOffset.dy + Expansion_tileLibrary._kHeaderHeight, left: headerOffset.dx, child: new ConstrainedBox(constraints: constraints, child: new Visibility(visible: animation.isAnimating, child: new FadeTransition(opacity: animation, child: widget.child))));
    ...
    {
        return new Expansible(controller: _tileController, duration: Expansion_tileLibrary._kAnimationDuration, curve: Expansion_tileLibrary._kAnimationCurve, headerBuilder: _buildHeader, bodyBuilder: (context, animation) => widget.child, expansibleBuilder: _buildExpansible);
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
