...
public static partial class Activity_indicatorLibrary
{
    internal static Color _kActiveTickColor = new CupertinoDynamicColor(
        color: new Color(4282137668L),
        darkColor: new Color(4293651445L)
    );
}
...
public class CupertinoActivityIndicator : StatefulWidget
{
    ...
    public static CupertinoActivityIndicator CreatePartiallyRevealed(
        Key? key = null,
        Color? color = null,
        double? radius = null,
        double progress = 1.0
    ...
    public override IState createState() =>
        DartRuntimePrimitives.ConvertValue<IState>(
            new _CupertinoActivityIndicatorState__activity_indicator()
        );
}

internal class _CupertinoActivityIndicatorState__activity_indicator
        ...
        SingleTickerProviderStateMixin<CupertinoActivityIndicator>
{
    ...
    public override Widget build(BuildContext context)
    {
        return SizedBox.CreateSquare(
            dimension: widget.radius * 2L,
            child: new CustomPaint(
                painter: new _CupertinoActivityIndicatorPainter__activity_indicator(
                    position: _controller,
                    activeColor: widget.color
                        ?? CupertinoDynamicColor.resolve(
                            Activity_indicatorLibrary._kActiveTickColor,
                            context
                        ),
...
public static partial class Activity_indicatorLibrary
{
    internal static List<long> _kAlphaValues = new List<long>
    {
        47L,
        47L,
        47L,
...
internal class _CupertinoActivityIndicatorPainter__activity_indicator : CustomPainter
{
    ...
    internal _CupertinoActivityIndicatorPainter__activity_indicator(
        ...
        for (var i = 0L; i < (tickCount * progress); ++i)
        {
            ...
            paintLocal.color = activeColor.withAlpha(
                (progress < 1L)
                    ...
                    : Activity_indicatorLibrary._kAlphaValues[(int)t]
            );
...
public class CupertinoLinearActivityIndicator : StatelessWidget
{
    ...
    public override Widget build(BuildContext context)
    {
        return new ConstrainedBox(
            ...
            child: new CustomPaint(
                painter: new _CupertinoLinearActivityIndicator__activity_indicator(
                    progress: progress,
                    color: color
                )
            )
...
internal class _CupertinoLinearActivityIndicator__activity_indicator : CustomPainter
{
    ...
    internal _CupertinoLinearActivityIndicator__activity_indicator(
        double progress,
        Color? color = null
    )
    {
    ...
    {
        canvas.drawRRect(
            BorderRadius.CreateAll(Radius.circular(size.height / 2L)).toRRect(Offset.zero & size),
            _backgroundPaint
        );
        if (progress > 0L)
        {
            canvas.drawRRect(
                BorderRadius
                    .CreateAll(Radius.circular(size.height / 2L))
                    .toRRect(
                        Offset.zero
                            & new Size(
    ...
    public override bool shouldRepaint(CustomPainter oldDelegate) =>
        (((_CupertinoLinearActivityIndicator__activity_indicator)oldDelegate).progress != progress)
        || (
            !Equals(
                ((_CupertinoLinearActivityIndicator__activity_indicator)oldDelegate).color,
                color
            )
        );
}
