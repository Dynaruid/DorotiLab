...
public static partial class Context_menuLibrary
{
    internal static List<BoxShadow> _endBoxShadow = new List<BoxShadow> { new BoxShadow(color: new Color(1073741824L), blurRadius: 10.0, spreadRadius: 0.5) };
}

public static partial class Context_menuLibrary
{
    internal static Color _borderColor = new CupertinoDynamicColor(color: new Color(4289309103L), darkColor: new Color(4283914330L));
}

public static partial class Context_menuLibrary
{
    internal static Color _kBackgroundColor = new CupertinoDynamicColor(color: new Color(4294046193L), darkColor: new Color(4280361250L));
}

internal delegate void _DismissCallback__context_menu(BuildContext context, double scale, double opacity);

public delegate Widget CupertinoContextMenuBuilder(BuildContext context, Animation<double> animation);
...
public class CupertinoContextMenu : StatefulWidget
{
    ...
    public CupertinoContextMenu(Key? key = null, List<Widget> actions = default!, Widget child = default!, bool enableHapticFeedback = false) : base(key: key)
    {
    ...
    public static CupertinoContextMenu CreateBuilder(Key? key = null, List<Widget> actions = default!, Func<BuildContext, Animation<double>, Widget> builder = default!, bool enableHapticFeedback = false)
    {
...
internal class _CupertinoContextMenuState__context_menu : State<CupertinoContextMenu>, TickerProviderStateMixin<CupertinoContextMenu>
{
    internal virtual GlobalKey<IState> _childGlobalKey { get; private set; } = GlobalKey<IState>.Create();
    ...
    internal virtual Gestures.TapGestureRecognizer _tapGestureRecognizer { get; private set; } = default!;
    ...
    {
        base.initState();
        _openController = new AnimationController(duration: Context_menuLibrary._previewLongPressTimeout, vsync: this, upperBound: CupertinoContextMenu.animationOpensAt);
        _openController.addStatusListener(_onDecoyAnimationStatusChange);
        _tapGestureRecognizer = ((Func<Gestures.TapGestureRecognizer>)(() =>
{
    ...
    internal static Widget _defaultPreviewBuilder(BuildContext context, Animation<double> animation, Widget child)
    {
        return new FittedBox(fit: BoxFit.cover, child: new ClipRSuperellipse(borderRadius: BorderRadius.CreateCircular(Context_menuLibrary._previewBorderRadiusRatio * animation.value), child: child));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    internal virtual void _openContextMenu()
    {
        ...
        _route = new _ContextMenuRoute__context_menu<object?>(actions: widget.actions, barrierLabel: CupertinoLocalizations.of(context).menuDismissLabel, filter: new ImageFilter(sigmaX: 5.0, sigmaY: 5.0), contextMenuLocation: _contextMenuLocation, previousChildRect: DartRuntimePrimitives.RequireValue(_decoyChildEndRect), scaleFactor: _scaleFactor, builder: (context, animation) =>
        {
            if (widget.child is null)
            {
                Animation<double> localAnimation = new Tween<double>(begin: CupertinoContextMenu.animationOpensAt, end: 1).animate(animation);
    ...
    internal virtual void _onTapDown(Gestures.TapDownDetails details)
    {
        ...
        _lastOverlayEntry = new OverlayEntry(builder: (context) =>
        {
    ...
    {
        return new MouseRegion(cursor: Foundation.ConstantsLibrary.kIsWeb ? SystemMouseCursors.click : MouseCursor.defer, child: new Listener(onPointerDown: _tapGestureRecognizer.addPointer, child: new TickerMode(enabled: !_childHidden, child: new Visibility(key: _childGlobalKey, visible: !_childHidden, child: widget.builder(context, _openController)))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _DecoyChild__context_menu : StatefulWidget
{
    ...
    internal _DecoyChild__context_menu(Rect? beginRect = null, AnimationController controller = default!, Rect? endRect = null, Widget? child = null, Func<BuildContext, Animation<double>, Widget>? builder = null)
    {
...
internal class _DecoyChildState__context_menu : State<_DecoyChild__context_menu>, TickerProviderStateMixin<_DecoyChild__context_menu>
{
    ...
    public override void initState()
    {
        ...
        _rect = new TweenSequence<Rect?>(new List<TweenSequenceItem<Rect?>> { new TweenSequenceItem<Rect?>(tween: new RectTween(begin: widget.beginRect, end: widget.beginRect).chain(new CurveTween(curve: Curves.linear)), weight: beginPause), new TweenSequenceItem<Rect?>(tween: new RectTween(begin: widget.beginRect, end: widget.endRect).chain(new CurveTween(curve: Curves.easeOutSine)), weight: openAnimationLength), new TweenSequenceItem<Rect?>(tween: new RectTween(begin: widget.endRect, end: widget.endRect).chain(new CurveTween(curve: Curves.linear)), weight: endPause) }.Cast<TweenSequenceItem<Rect?>>().ToList()).animate(widget.controller);
        _boxDecorationCurvedAnimation = new CurvedAnimation(parent: widget.controller, curve: new Interval(0.0, CupertinoContextMenu.animationOpensAt));
        _boxDecoration = new DecorationTween(begin: new BoxDecoration(boxShadow: new List<BoxShadow>()), end: new BoxDecoration(boxShadow: Context_menuLibrary._endBoxShadow)).animate(_boxDecorationCurvedAnimation);
    }

    internal virtual Widget _buildAnimation(BuildContext context, Widget? child)
    {
        return Positioned.CreateFromRect(rect: DartRuntimePrimitives.RequireValue(_rect.value), child: new Container(decoration: _boxDecoration.value, child: widget.child));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    public override Widget build(BuildContext context)
    {
        return new Stack(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new AnimatedBuilder(builder: (widget.child is not null) ? _buildAnimation : _buildBuilder, animation: widget.controller)) });
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _ContextMenuRoute__context_menu<T> : PopupRoute<T>
{
    ...
    internal virtual GlobalKey<IState> _sheetGlobalKey { get; private set; } = GlobalKey<IState>.Create();
    ...
    internal static Animatable<Rect?> _rectAnimatableReverse = _rectTweenReverse.chain(_curveReverse);
    internal static RectTween _sheetRectTween = new RectTween();
    internal virtual Animatable<Rect?> _sheetRectAnimatable { get; private set; } = _sheetRectTween.chain(_curve);
    internal virtual Animatable<Rect?> _sheetRectAnimatableReverse { get; private set; } = _sheetRectTween.chain(_curveReverse);
    ...
    internal static Animatable<double> _sheetScaleAnimatableReverse = _sheetScaleTween.chain(_curveReverse);
    internal virtual Tween<double> _opacityTween { get; private set; } = new Tween<double>(begin: 0.0, end: 1.0);
    ...
    internal _ContextMenuRoute__context_menu(List<Widget> actions, _ContextMenuLocation__context_menu contextMenuLocation, string? barrierLabel = null, Func<BuildContext, Animation<double>, Widget>? builder = null, ImageFilter? filter = null, Rect previousChildRect = default!, double scaleFactor = default!, RouteSettings? settings = null) : base(filter: filter, settings: settings)
    {
    ...
    internal static Rect _getScaledRect(GlobalKey<IState> globalKey, double scale)
    {
        ...
        var offsetScaled = new Offset(childRect.left + ((childRect.size.width - sizeScaled.width) / 2L), childRect.top + ((childRect.size.height - sizeScaled.height) / 2L));
        return offsetScaled & sizeScaled;
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }

    public static AlignmentDirectional getSheetAlignment(_ContextMenuLocation__context_menu contextMenuLocation, Orientation orientation)
    {
    ...
    internal static Rect _getSheetRectBegin(Orientation? orientation, _ContextMenuLocation__context_menu contextMenuLocation, Rect childRect, Rect sheetRect)
    {
        switch (contextMenuLocation)
        {
            case _ContextMenuLocation__context_menu.center:
                {
                    Offset target = Equals(orientation, Orientation.portrait) ? childRect.bottomCenter : childRect.topCenter;
                    Offset centered = target - new Offset(sheetRect.width / 2L, 0.0);
                    return centered & sheetRect.size;
                }
            case _ContextMenuLocation__context_menu.right:
                {
                    Offset targetLocal = Equals(orientation, Orientation.portrait) ? childRect.bottomRight : childRect.topRight;
                    return targetLocal - new Offset(sheetRect.width, 0.0) & sheetRect.size;
                }
            case _ContextMenuLocation__context_menu.left:
                {
                    Offset targetAlternate = Equals(orientation, Orientation.portrait) ? childRect.bottomLeft : childRect.topLeft;
    ...
    internal virtual void _onDismiss(BuildContext context, double scale, double opacity)
    {
        ...
        _sheetOpacityCurvedAnimation = new CurvedAnimation(parent: animation!, curve: new Interval(0.9, 1.0));
    ...
    {
        Rect childRect = (_scale is null) ? Context_menuLibrary._getRect(_childGlobalKey) : _ContextMenuRoute__context_menu<T>._getScaledRect(_childGlobalKey, DartRuntimePrimitives.RequireValue(_scale));
        ...
        Rect sheetRectBegin = _ContextMenuRoute__context_menu<T>._getSheetRectBegin(_lastOrientation, _contextMenuLocation, childRectOriginal, sheetRect);
    ...
    public override Widget buildTransitions(BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation, Widget child)
    {
        return new OrientationBuilder(builder: (context, orientation) =>
        {
            ...
            if (!animation.isCompleted)
            {
                ...
                Rect rectLocal = reverseLocal ? DartRuntimePrimitives.RequireValue(_rectAnimatableReverse.evaluate(animation)) : DartRuntimePrimitives.RequireValue(_rectAnimatable.evaluate(animation));
                Rect sheetRect = reverseLocal ? DartRuntimePrimitives.RequireValue(_sheetRectAnimatableReverse.evaluate(animation)) : DartRuntimePrimitives.RequireValue(_sheetRectAnimatable.evaluate(animation));
                double sheetScale = reverseLocal ? _sheetScaleAnimatableReverse.evaluate(animation) : _sheetScaleAnimatable.evaluate(animation);
                return Stack.Create(children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(Positioned.CreateFromRect(rect: sheetRect, child: new FadeTransition(opacity: _sheetOpacity, child: Transform.CreateScale(alignment: _ContextMenuRoute__context_menu<T>.getSheetAlignment(_contextMenuLocation, DartRuntimePrimitives.RequireValue(DartRuntimePrimitives.RequireValue(orientation))), scale: sheetScale, child: new _ContextMenuSheet__context_menu(key: _sheetGlobalKey, actions: _actions, contextMenuLocation: _contextMenuLocation, orientation: DartRuntimePrimitives.RequireValue(orientation)))))), DartRuntimePrimitives.ConvertValue<Widget>(Positioned.CreateFromRect(key: _childGlobalKey, rect: rectLocal, child: _builder!(context, animation))) });
            }
...
internal class _ContextMenuRouteStatic__context_menu : StatefulWidget
{
    ...
    internal _ContextMenuRouteStatic__context_menu(List<Widget>? actions = null, Widget child = default!, GlobalKey<IState>? childGlobalKey = null, _ContextMenuLocation__context_menu contextMenuLocation = default!, Action<BuildContext, double, double>? onDismiss = null, Orientation orientation = default!, GlobalKey<IState>? sheetGlobalKey = null, Rect childRect = default!)
    {
        this.actions = actions;
        this.child = child;
...
internal class _ContextMenuRouteStaticState__context_menu : State<_ContextMenuRouteStatic__context_menu>, TickerProviderStateMixin<_ContextMenuRouteStatic__context_menu>
{
    ...
    internal virtual void _setDragOffset(Offset dragOffset)
    {
        ...
        setState(() =>
        {
            ...
            _moveAnimation = new Tween<Offset>(begin: Offset.zero, end: new Offset(Dart_uiLibrary.clampDouble(endX, -SliderLibrary._kPadding, SliderLibrary._kPadding), endY)).animate(_moveCurvedAnimation);
    ...
    internal virtual Widget _getChild(Orientation orientation, _ContextMenuLocation__context_menu contextMenuLocation)
    {
        ...
        Widget sheetLocal = new AnimatedBuilder(animation: _sheetController, builder: _buildSheetAnimation, child: new _ContextMenuSheet__context_menu(key: widget.sheetGlobalKey, actions: widget.actions!, contextMenuLocation: widget.contextMenuLocation, orientation: widget.orientation));
        Widget childLocal = new _ContextMenuAlignedChildren__context_menu(targetRect: widget.childRect, screenBounds: screenBoundsLocal, sheet: sheetLocal, contextMenuLocation: contextMenuLocation, orientation: widget.orientation, child: new AnimatedBuilder(animation: _moveController, builder: _buildChildAnimation, child: widget.child));
        return childLocal;
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    {
        return Transform.CreateScale(alignment: _ContextMenuRoute__context_menu<object>.getSheetAlignment(widget.contextMenuLocation, widget.orientation), scale: _sheetScaleAnimation.value, child: new FadeTransition(opacity: _sheetOpacityAnimation, child: child));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
    ...
    public override void initState()
    {
        ...
        _moveCurvedAnimation = new CurvedAnimation(parent: _moveController, curve: Curves.elasticIn);
        _sheetController = new AnimationController(duration: Duration.Create(milliseconds: 100L), reverseDuration: Duration.Create(milliseconds: 300L), vsync: this);
        _sheetCurvedAnimation = new CurvedAnimation(parent: _sheetController, curve: Curves.linear, reverseCurve: Curves.easeInBack);
        _sheetScaleAnimation = new Tween<double>(begin: 1.0, end: 0.0).animate(_sheetCurvedAnimation);
    ...
    public override Widget build(BuildContext context)
    {
        ...
        return new SafeArea(child: new Align(alignment: Alignment.topLeft, child: new GestureDetector(onPanEnd: _onPanEnd, onPanStart: _onPanStart, onPanUpdate: _onPanUpdate, child: new AnimatedBuilder(animation: _moveController, builder: _buildAnimation, child: childLocal))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _ContextMenuSheet__context_menu : StatefulWidget
{
    ...
    internal _ContextMenuSheet__context_menu(Key? key = null, List<Widget> actions = default!, _ContextMenuLocation__context_menu contextMenuLocation = default!, Orientation orientation = default!) : base(key: key)
    {
        this.actions = actions;
        this.contextMenuLocation = contextMenuLocation;
        this.orientation = orientation;
...
internal class _ContextMenuSheetState__context_menu : State<_ContextMenuSheet__context_menu>
{
    ...
    public override Widget build(BuildContext context)
    {
        return new SizedBox(width: _kMenuWidth, child: new IntrinsicHeight(child: new ClipRSuperellipse(borderRadius: BorderRadius.CreateAll(Radius.circular(13.0)), child: new ColoredBox(color: CupertinoDynamicColor.resolve(CupertinoContextMenu.kBackgroundColor, context), child: new ScrollConfiguration(behavior: ScrollConfiguration.of(context).copyWith(scrollbars: false), child: new CupertinoScrollbar(mainAxisMargin: _kScrollbarMainAxisMargin, controller: _controller, child: new SingleChildScrollView(controller: _controller, child: new Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: ((Func<List<Widget>>)(() => { var __collection49409 = new List<Widget>(); __collection49409.Add(DartRuntimePrimitives.ConvertValue<Widget>(widget.actions.First())); foreach (var action in widget.actions.skip(1L)) { __collection49409.Add(DartRuntimePrimitives.ConvertValue<Widget>(new DecoratedBox(decoration: new BoxDecoration(border: new Border(top: new BorderSide(color: CupertinoDynamicColor.resolve(Context_menuLibrary._borderColor, context), width: 0.4))), position: DecorationPosition.foreground, child: action))); } return __collection49409; }))()))))))));
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _ContextMenuAlignedChildren__context_menu : StatelessWidget
{
    ...
    internal _ContextMenuAlignedChildren__context_menu(Rect targetRect, Rect screenBounds, Widget child, Widget sheet, Orientation orientation, _ContextMenuLocation__context_menu contextMenuLocation)
    {
    ...
    public override Widget build(BuildContext context)
    {
        return new CustomMultiChildLayout(@delegate: new _ContextMenuAlignedChildrenDelegate__context_menu(targetRect: targetRect, screenBounds: screenBounds, orientation: orientation, contextMenuLocation: contextMenuLocation), children: new List<Widget> { DartRuntimePrimitives.ConvertValue<Widget>(new LayoutId(id: _ContextMenuChild__context_menu.child, child: child)), DartRuntimePrimitives.ConvertValue<Widget>(new LayoutId(id: _ContextMenuChild__context_menu.menuSheet, child: sheet)) });
        throw new InvalidOperationException("Dart control flow completed without a value.");
    }
...
internal class _ContextMenuAlignedChildrenDelegate__context_menu : MultiChildLayoutDelegate
{
    ...
    internal _ContextMenuAlignedChildrenDelegate__context_menu(Rect targetRect, Rect screenBounds, Orientation orientation, _ContextMenuLocation__context_menu contextMenuLocation)
    {
        this.targetRect = targetRect;
        this.screenBounds = screenBounds;
        this.orientation = orientation;
    ...
    public override void performLayout(Size size)
    {
        ...
        Size childSize = layoutChild(_ContextMenuChild__context_menu.child, constraints.copyWith(maxHeight: availableHeightForChild, maxWidth: availableWidthForChild));
        ...
        Size menuSize = layoutChild(_ContextMenuChild__context_menu.menuSheet, constraints.copyWith(maxHeight: availableHeightForMenu));
            ...
            case Orientation.portrait:
                {
                    ...
                    secondChildOffset = new Offset(secondChildDx, childSize.height + _ContextMenuRouteStaticState__context_menu._kPadding);
            ...
            case Orientation.landscape:
                {
                    ...
                    secondChildOffset = new Offset(secondChildDxLocal + _ContextMenuRouteStaticState__context_menu._kPadding, 0.0);
