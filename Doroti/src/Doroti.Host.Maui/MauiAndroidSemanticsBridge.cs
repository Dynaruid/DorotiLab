...
internal sealed partial class MauiAndroidSemanticsBridge : IMauiSemanticsBridge, IDisposable
{
    ...
    public MauiSemanticsDiagnostics Diagnostics =>
        new(
            ...
            0,
            _nodes.Count,
            UpdatesSuppressed: _suppressed,
            ApplyWorkMicroseconds: _applyMicros,
            MaxApplyWorkMicroseconds: _maxApplyMicros
        );
    ...
    {
        if (_disposed || update.generation < _generation)
        {
            return;
        }
        ...
        var pending = new Stack<int>(
            update.nodes.Where(node => node.flags?.isHidden == true).Select(node => node.id)
        );
        while (pending.TryPop(out var id))
        {
            if (!hidden.Add(id) || !nodes.TryGetValue(id, out var node))
            {
                continue;
            }
        ...
        hidden.UnionWith(
            nodes.Values.Where(node => node.platformViewId is not null).Select(node => node.id)
        );
        ...
        if (!SemanticsUpdateDiffer.Diff(_nodes, visible).HasChanges)
        {
            _suppressed++;
            return;
        }
    ...
    {
        if (_helper is null || _native is null)
        {
            return;
        }
        ...
        {
            _helper
                .GetAccessibilityNodeProvider(_native)
                ?.PerformAction(accessibility, (int)NativeAction.ClearAccessibilityFocus, null);
        }
    }
    ...
    private void Attach(object? sender, EventArgs args)
    {
        if (
            ...
            || ReferenceEquals(
                _native,
                (_element.Handler?.PlatformView as DorotiAndroidViewContainer)?.Surface
            )
        )
        {
            return;
        ...
        Detach();
        if (_element.Handler?.PlatformView is not DorotiAndroidViewContainer container)
        {
            return;
        }
    ...
    private sealed class VirtualNodes : ExploreByTouchHelper
    {
        ...
        internal VirtualNodes(DorotiAndroidVulkanView host, MauiAndroidSemanticsBridge owner)
            : base(host)
        {
            _host = host;
            _owner = owner;
        ...
        private VirtualNodes(IntPtr handle, JniHandleOwnership ownership)
            : base(handle, ownership) { }

        protected override int GetVirtualViewAt(float x, float y)
        {
            if (_owner is null || _host is null)
            {
                return InvalidId;
            }
        ...
        {
            if (_owner is null || virtualViewIds is null)
            {
                return;
            }
        ...
        private static bool IsExposed(SemanticsNodeUpdate node) =>
            ...
            || node.actions != SemanticsAction.none
            || node.flags is { isFocused: not Tristate.none };

        protected override void OnPopulateNodeForVirtualView(
            int virtualViewId,
        ...
        )
        {
            ...
            if (
                _owner is null
                || _host is null
                || !_owner._nodes.TryGetValue(virtualViewId, out var node)
            )
            {
                info.ContentDescription = string.Empty;
                using var empty = new Android.Graphics.Rect();
                SetBoundsInScreenFromBoundsInParent(info, empty);
                return;
            }
            ...
            info.ClassName =
                ...
                : flags?.isSlider == true ? "android.widget.SeekBar"
                : flags?.isInMutuallyExclusiveGroup == true ? "android.widget.RadioButton"
                : toggled ? "android.widget.Switch"
                : checkable ? "android.widget.CheckBox"
                : node.actions.HasFlag(SemanticsAction.tap) ? "android.widget.Button"
                : "android.view.View";
            info.Text = textField ? node.value ?? string.Empty : node.label ?? string.Empty;
            info.ContentDescription = textField
                ? node.label ?? string.Empty
                : string.Join(
                    " ",
                    new[] { node.label, node.value }.Where(value =>
            ...
            using var bounds = new Android.Graphics.Rect(
                (int)Math.Floor(node.rect.left * scale),
                (int)Math.Floor(node.rect.top * scale),
                (int)Math.Ceiling(node.rect.right * scale),
                (int)Math.Ceiling(node.rect.bottom * scale)
            );
            ...
            if (textField && node.textSelectionBase >= 0 && node.textSelectionExtent >= 0)
            {
                ...
                info.SetTextSelection(
                    (int)Math.Clamp(node.textSelectionBase, 0, length),
                    (int)Math.Clamp(node.textSelectionExtent, 0, length)
                );
            }
            if (
                flags?.isSlider == true
                && float.TryParse(node.minValue, CultureInfo.InvariantCulture, out var min)
                && float.TryParse(node.maxValue, CultureInfo.InvariantCulture, out var max)
                && max >= min
                && float.TryParse(node.value, CultureInfo.InvariantCulture, out var current)
                && float.IsFinite(min)
                && float.IsFinite(max)
                && float.IsFinite(current)
            )
            {
                using var range = AccessibilityNodeInfoCompat.RangeInfoCompat.Obtain(
                    1,
                    min,
                    max,
            ...
            foreach (var (native, action) in Actions)
            {
                ...
                {
                    if ((int)native <= (int)NativeAction.SetText)
                    {
                        info.AddAction((int)native);
                    }
                    else
                    {
                        using var custom =
        ...
        protected override bool OnPerformActionForVirtualView(
            int virtualViewId,
            int action,
            Bundle? arguments
        )
        {
            ...
            foreach (var (native, candidates) in Actions)
            {
                ...
                var chosen =
                    available.HasFlag(SemanticsAction.increase) ? SemanticsAction.increase
                    : available.HasFlag(SemanticsAction.decrease) ? SemanticsAction.decrease
                    : (SemanticsAction)((long)available & -(long)available);
                object? value =
                    chosen == SemanticsAction.setText
                        ? arguments?.GetCharSequence("ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE")
                            ?? string.Empty
                        : null;
                if (
                    native
                    is NativeAction.NextAtMovementGranularity
                        or NativeAction.PreviousAtMovementGranularity
                )
                {
                    ...
                    if (chosen == SemanticsAction.none || !node.actions.HasFlag(chosen))
                    {
                        return false;
                    }

                    value =
                        arguments?.GetBoolean("ACTION_ARGUMENT_EXTEND_SELECTION_BOOLEAN", false)
                        ?? false;
                }
                if (chosen == SemanticsAction.setSelection)
                {
                    ...
                    {
                        ["base"] =
                            arguments?.GetInt("ACTION_ARGUMENT_SELECTION_START_INT", -1) ?? -1,
                        ["extent"] =
                            arguments?.GetInt("ACTION_ARGUMENT_SELECTION_END_INT", -1) ?? -1,
                    };
                }
        ...
        protected override void OnPopulateEventForVirtualView(
            int virtualViewId,
        ...
        )
        {
            ...
            var action = e?.EventType switch
            {
                ...
                Android.Views.Accessibility.EventTypes.ViewAccessibilityFocusCleared =>
                    SemanticsAction.didLoseAccessibilityFocus,
                _ => SemanticsAction.none,
            };
            if (
                action != SemanticsAction.none
                && _owner?._nodes.TryGetValue(virtualViewId, out var node) == true
        ...
        protected override void OnVirtualViewKeyboardFocusChanged(int virtualViewId, bool hasFocus)
        {
            ...
            if (
                hasFocus
                && _owner?._nodes.TryGetValue(virtualViewId, out var node) == true
                && node.actions.HasFlag(SemanticsAction.focus)
            )
            {
        ...
        [
            (NativeAction.Click, SemanticsAction.tap),
            (NativeAction.LongClick, SemanticsAction.longPress),
            (
                NativeAction.ScrollForward,
                SemanticsAction.increase | SemanticsAction.scrollUp | SemanticsAction.scrollLeft
            ),
            (
                NativeAction.ScrollBackward,
                SemanticsAction.decrease | SemanticsAction.scrollDown | SemanticsAction.scrollRight
            ),
            (NativeAction.SetText, SemanticsAction.setText),
            (NativeAction.SetSelection, SemanticsAction.setSelection),
            (
                NativeAction.NextAtMovementGranularity,
                SemanticsAction.moveCursorForwardByCharacter
                    | SemanticsAction.moveCursorForwardByWord
            ),
            (
                NativeAction.PreviousAtMovementGranularity,
                SemanticsAction.moveCursorBackwardByCharacter
                    | SemanticsAction.moveCursorBackwardByWord
            ),
            (NativeAction.Copy, SemanticsAction.copy),
            (NativeAction.Cut, SemanticsAction.cut),
            (NativeAction.Paste, SemanticsAction.paste),
            (NativeAction.Expand, SemanticsAction.expand),
            (NativeAction.Collapse, SemanticsAction.collapse),
            (NativeAction.Dismiss, SemanticsAction.dismiss),
            ((NativeAction)16908342, SemanticsAction.showOnScreen),
        ];
    }
}
#endif
