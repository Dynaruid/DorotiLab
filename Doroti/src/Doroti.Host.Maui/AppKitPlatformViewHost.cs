...
internal sealed class AppKitPlatformViewHost : IDisposable
{
    private sealed class OverlayView : NSView
    {
        ...
        public override NSView? HitTest(CGPoint point)
        {
            ...
            {
                if (!child.Hidden && child.HitTest(local) is { } hit)
                {
                    return hit;
                }
            }
    ...
    {
        public override NSView? HitTest(CGPoint point) =>
            !Hidden && Frame.Contains(point) ? surface.NativeView : null;
    }
    ...
    internal IEnumerable<IPlatformViewFactory> CreateFactories(
        ...
        [
            new AppKitPlatformViewFactory(
                GetContainer,
                false,
                _textInput.YieldMacOSNativeFocus,
                RestoreFocus,
                interleaved: true
            ),
            new AppKitPlatformViewFactory(
    ...
    private NSView GetContainer()
    {
        ...
        var native =
            _surface.NativeView
            ?? throw new InvalidOperationException("AppKit render surface is not attached.");
        var parent =
            native.Superview
            ?? throw new InvalidOperationException("AppKit render surface has no container.");
        if (_window is not null && native.Window is not null && native.Window != _window)
        {
            throw new InvalidOperationException(
                "An AppKit PlatformView owner cannot move between windows."
            );
        }
        ...
        {
            _overlay.Appearance = NSAppearance.GetAppearance(
                _brightness == Brightness.dark ? NSAppearance.NameDarkAqua : NSAppearance.NameAqua
            );
    ...
    )
    {
        ...
        var coordinator =
            _coordinator
            ?? throw new InvalidOperationException("AppKit PlatformView coordinator is missing.");
        if (
            _placements.Length == 0
            && _visibleRasters.Length == 0
            && _visibleShields.Length == 0
        ...
        var plan = PlatformCompositionPlanner.Build(
            ...
            AppKitPlatformBlurView.Support
        );
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    "AppKit input shields require axis-aligned rectangular bounds."
                );
            }

            foreach (var part in plan.Parts.OfType<PlatformNativeSegment>())
            {
                gpuLeases.Add(coordinator.Retain(part.Placement.Handle));
            }

            foreach (var effect in plan.Parts.OfType<PlatformBackdropSegment>())
            {
                AppKitPlatformBlurView.Validate(effect);
            }
    ...
    private static void RunNow(Func<CancellationToken, ValueTask> action)
    {
        ...
        if (!task.IsCompleted)
        {
            ...
            throw new InvalidOperationException(
                "AppKit native placement is busy; the frame was not committed."
            );
        }
    ...
    private void ApplyLayers(
        (AppKitPlatformRasterSurface Slot, int Order, CGRect Bounds)[] rasters,
    ...
    internal sealed class PreparedFrame(
        AppKitPlatformViewHost host,
        PlatformCompositionPlan plan,
        AppKitPlatformRasterSurface.RasterFrame[] frames,
    ...
    ) : IDisposable, IPreparedPlatformComposition
    {
        ...
        public void Commit()
        {
            ...
            {
                throw new InvalidOperationException(
                    "AppKit composition epoch cannot wait on the UI thread."
                );
            }
            ...
            try
            {
                ...
                {
                    throw new InvalidOperationException(
                        "AppKit native commit unexpectedly suspended."
                    );
                }
    ...
    private sealed class CommitPresenter : IPlatformCompositionPresenter
    {
        ...
        )
        {
            ...
            return ValueTask.FromResult<IPreparedPlatformComposition>(
                Prepared ?? throw new InvalidOperationException("No AppKit frame prepared.")
            );
        }
    }
