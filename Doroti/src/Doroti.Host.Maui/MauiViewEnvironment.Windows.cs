...
internal sealed partial class MauiViewEnvironment
{
    ...
    private delegate nint WindowObserver(
        nint window,
        uint message,
        nuint wparam,
        nint lparam,
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    private static partial bool QueryHighContrast(
        uint action,
        uint size,
    ...
    {
        if (_element.Handler?.PlatformView is not FrameworkElement native)
        {
            return;
        }
        ...
        if (_element.Window?.Handler?.PlatformView is Microsoft.UI.Xaml.Window window)
        {
            ...
            if (!SetWindowSubclass(hwnd, callback, (nuint)callback, 0))
            {
                throw new InvalidOperationException(
                    "Could not observe native Windows environment changes."
                );
            }

            _detach.Add(() =>
            {
                RemoveWindowSubclass(hwnd, callback, (nuint)callback);
                GC.KeepAlive(observer);
            });
            _inputPane = InputPaneInterop.GetForWindow(hwnd);
            void PaneChanged(InputPane pane, InputPaneVisibilityEventArgs args)
            {
                args.EnsuredFocusedElementInView = true;
                Schedule();
            }
            _inputPane.Showing += PaneChanged;
            _inputPane.Hiding += PaneChanged;
            var pane = _inputPane;
            _detach.Add(() =>
            {
                pane.Showing -= PaneChanged;
                pane.Hiding -= PaneChanged;
                _inputPane = null;
            });
        }
        if (native.XamlRoot is { } root)
        {
            ...
            _detach.Add(() => root.Changed -= RootChanged);
        }
    }
    ...
    {
        if (
            _element.Handler?.PlatformView is not FrameworkElement native
            || native.XamlRoot is not { } root
        )
        {
            return;
        }
        ...
        NativePhysicalSize = new(
            Math.Round(native.ActualWidth * root.RasterizationScale),
            Math.Round(native.ActualHeight * root.RasterizationScale)
        );
        ...
        try
        {
            ...
            var localesSnapshot = Windows
                .System.UserProfile.GlobalizationPreferences.Languages.Select(ParseLocale)
                .ToArray();
        ...
        if (QueryHighContrast(0x42, contrast.Size, ref contrast, 0))
        {
            _highContrast = (contrast.Flags & 1) != 0;
        }

        Accessibility = new(false, false, !_animationsEnabled, false, _highContrast, false, false);
        Use24Hour =
            !System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.ShortTimePattern.Contains(
                't'
            );
    }
}
