...
{
    internal static async ValueTask PerformAsync(
        HapticFeedbackKind kind,
    ...
    private static void Perform(HapticFeedbackKind kind, IMauiSkiaSurface surface)
    {
        ...
        if (kind is HapticFeedbackKind.standard or HapticFeedbackKind.heavyImpact)
        {
            if (HapticFeedback.Default.IsSupported)
            {
                HapticFeedback.Default.Perform(
                    kind == HapticFeedbackKind.standard
        ...
        Android.Views.FeedbackConstants? feedback = kind switch
        {
            ...
            HapticFeedbackKind.successNotification
                when OperatingSystem.IsAndroidVersionAtLeast(30) => Android
                .Views
                .FeedbackConstants
                .Confirm,
            HapticFeedbackKind.warningNotification
                when OperatingSystem.IsAndroidVersionAtLeast(30) => Android
                .Views
                .FeedbackConstants
                .KeyboardTap,
            HapticFeedbackKind.errorNotification when OperatingSystem.IsAndroidVersionAtLeast(30) =>
                Android.Views.FeedbackConstants.Reject,
            _ => null,
        };
        ...
        if (kind == HapticFeedbackKind.selectionClick)
        {
            ...
            kind
            is HapticFeedbackKind.successNotification
                or HapticFeedbackKind.warningNotification
                or HapticFeedbackKind.errorNotification
        )
        {
            ...
            notification.NotificationOccurred(
                kind switch
                {
                    HapticFeedbackKind.successNotification => UIKit
                        .UINotificationFeedbackType
                        .Success,
                    HapticFeedbackKind.warningNotification => UIKit
                        .UINotificationFeedbackType
                        .Warning,
        ...
        else
        {
            ...
            if (surface.Element.Handler?.PlatformView is not UIView nativeView)
            {
                return;
            }
