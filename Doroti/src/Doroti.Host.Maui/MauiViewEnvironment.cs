...
internal sealed partial class MauiViewEnvironment : IDisposable
{
    ...
    internal static double ValidScale(double value) =>
    ...
    internal IReadOnlyList<DisplayFeature> Features { get; private set; } =
    ...
    internal void Refresh(object? sender = null, EventArgs? args = null)
    {
        ...
        if (
            ...
            || !previousFeatures.SequenceEqual(Features)
            || !(previousLocales ?? []).SequenceEqual(Locales ?? [])
        )
        {
            Changed?.Invoke();
        }
