...
) : NSObject, IWKUrlSchemeHandler
{
    ...
    {
        if (_tasks.Remove(task.Handle, out var cancellation))
        {
            cancellation.Cancel();
        }
    }
    ...
    private async Task Respond(IWKUrlSchemeTask task, CancellationTokenSource cancellation)
    {
        ...
        try
        {
            ...
            if (
                ...
                || !Uri.TryCreate(request.Url?.AbsoluteString, UriKind.Absolute, out var uri)
                || uri.Scheme != "doroti-app"
                || uri.Host != "content"
                || !string.IsNullOrEmpty(uri.UserInfo)
                || uri.AbsolutePath.Contains('%')
                || !routes.TryGetValue(uri.AbsolutePath, out var route)
                || request.Headers?["Range"] is not null
            )
            {
                throw new WebViewException(
                    WebViewError.InvalidRequest,
                    "Unknown app resource or unsupported method/Range request."
                );
            }

            var manifest =
                resources.Resources.SingleOrDefault(resource => resource.Key == route.ResourceKey)
                ?? throw new WebViewException(
                    WebViewError.InvalidRequest,
        ...
        catch (Exception error)
        {
            await new AppKitPlatformViewDispatcher().InvokeAsync(() =>
            {
                if (!_closed && !cancellation.IsCancellationRequested && _tasks.Remove(key))
                {
                    using var info = NSDictionary.FromObjectAndKey(
                        new NSString(error.Message),
                        NSError.LocalizedDescriptionKey
                    );
                    using var nativeError = new NSError(
                        new NSString("Doroti.WebView.Content"),
                        1,
                        info
                    );
    ...
    {
        _closed = true;
        foreach (var cancellation in _tasks.Values)
        {
            cancellation.Cancel();
        }
