...
public sealed class DorotiAndroidVulkanViewHandler
    : ViewHandler<DorotiGraphiteView, DorotiAndroidViewContainer>
{
    private static readonly CommandMapper<
        DorotiGraphiteView,
        DorotiAndroidVulkanViewHandler
    > Commands = new(ViewCommandMapper)
    {
        [nameof(ISKGLView.InvalidateSurface)] = (handler, _, _) =>
            handler.PlatformView.RequestFrame(),
    };

    public DorotiAndroidVulkanViewHandler()
...
public sealed class DorotiAndroidViewContainer : Android.Widget.FrameLayout
{
    ...
    public DorotiAndroidViewContainer(Context context)
        : base(context)
    {
        Surface = new(context);
...
public sealed class DorotiAndroidVulkanView : SurfaceView, ISurfaceHolderCallback
{
    ...
    protected override bool DispatchHoverEvent(MotionEvent? e) =>
        (e is not null && SemanticsHelper?.DispatchHoverEvent(e) == true)
        ...
        _allocationProfileCallback = new(() =>
        {
            foreach (var entry in FrameworkWorkProfile.CaptureEntries())
            {
                Android.Util.Log.Info(
                    "DorotiAllocation",
                    $"kind={entry.Kind} calls={entry.Calls} allocated={entry.AllocatedBytes} self={entry.SelfAllocatedBytes} type={entry.Type}"
                );
            }
        });
    ...
    internal void Connect(DorotiGraphiteView owner)
    {
        ...
        _platformFrameTiming =
            Microsoft.Maui.ApplicationModel.Platform.CurrentActivity?.Intent?.GetStringExtra(
                "DOROTI_PLATFORM_FRAME_PROFILE"
            ) == "1";
        Holder!.AddCallback(this);
        _live = Holder.Surface?.IsValid == true;
        if (_live && Holder.SurfaceFrame is { } bounds)
        {
            _width = bounds.Width();
    ...
    private void DrawFrame()
    {
        using var allocationProfile = FrameworkWorkProfile.AllocationEnabled
            ? FrameworkWorkProfile.Begin(GetType(), 10)
            : default;
        _pending = false;
        if (!_live || _faulted || _retirement is not null || _owner is null)
        {
            return;
        }
        ...
        try
        {
            if (_window is null)
            {
                ...
                if (Volatile.Read(ref _retiringGenerations) != 0)
                {
                    RequestFrame();
                    return;
                }
                if (!OperatingSystem.IsAndroidVersionAtLeast(24))
                {
                    throw new PlatformNotSupportedException(
                        "Doroti Graphite requires Android API 24 or newer and a Vulkan 1.2 device for the official profile."
                    );
                }
                ...
                if (_nativeWindow == 0)
                {
                    throw new InvalidOperationException("ANativeWindow_fromSurface failed.");
                }

                _window = GraphiteVulkanWindow.CreateAndroid(_nativeWindow);
                _window.EnableFrameTiming =
                    Microsoft.Maui.ApplicationModel.Platform.CurrentActivity?.Intent?.GetStringExtra(
            ...
            var presented = _window.Render(
                ...
                (surface, width, height) =>
                {
                    paint = new(
                        surface,
                        _window.ContextIdentity,
                        width,
                        height,
                        _density,
                        (_generation << 32) | _window.Generation,
                        GetType().FullName!,
            ...
            if (presented && _window.EnableFrameTiming)
            {
                RecordFrameTiming(_window.LastFrameTiming);
            }

            if (presented && _platformFrameTiming)
            {
                Android.Util.Log.Info(
                    "DorotiPlatformTiming",
            ...
            if (!presented || paint?.Completion is null)
            {
                RequestFrame();
            }
        }
    ...
    internal Task<Skia.Rendering.SkiaGraphiteReadback> RequestPlatformReadback(
        SKSurface surface,
        SKImageInfo info
    ) =>
        (
            _window ?? throw new InvalidOperationException("Android Vulkan window is unavailable.")
    ...
    internal VulkanSharedRaster CreateHardwareBufferRaster(nint buffer, int width, int height) =>
        (
            _window ?? throw new InvalidOperationException("Android Vulkan window is unavailable.")
        ).CreateHardwareBufferRaster(buffer, width, height);

    private void ScheduleGpuCompletion()
    {
        if (_gpuCompletionPending || !_live || _window?.WindowFramesInFlight is not > 0)
        {
            return;
        }
    ...
    private void RecordFrameTiming(VulkanWindowFrameTiming timing)
    {
        ...
        var means = string.Join(
            ",",
            _timingSums.Select(value =>
                (value / _timingFrames).ToString(
                    "F3",
                    System.Globalization.CultureInfo.InvariantCulture
                )
            )
        );
        var maxima = string.Join(
            ",",
            _timingMaxima.Select(value =>
                value.ToString("F3", System.Globalization.CultureInfo.InvariantCulture)
            )
        );
        Android.Util.Log.Info(
            "DorotiFrameTiming",
            $"frames={_timingFrames} phases=acquire,paint,submit,copy,fence,present,total meanMs={means} maxMs={maxima} submitted={_window?.SubmittedWindowFrames} completed={_window?.CompletedWindowFrames} busy={_window?.BusyWindowFrames} unavailable={_window?.UnavailableWindowImages} maxInFlight={_window?.MaximumWindowFramesInFlight}"
        );
        _timingFrames = 0;
        Array.Clear(_timingSums);
        Array.Clear(_timingMaxima);
    }
    ...
    )
    {
        ...
        {
            if (
                await Task.WhenAny(retirement, Task.Delay(TimeSpan.FromSeconds(5)))
                    .ConfigureAwait(false) != retirement
            )
            {
                Android.Util.Log.Error(
                    "DorotiGraphite",
                    "Surface retirement exceeded five seconds; retaining generation and blocking replacement (not device loss)."
                );
            }
            ...
            if (window.EnableFrameTiming)
            {
                Android.Util.Log.Info(
                    "DorotiFrameTiming",
                    $"drained submitted={window.SubmittedWindowFrames} completed={window.CompletedWindowFrames} outstanding={window.WindowFramesInFlight}"
                );
            }
        ...
        catch (Exception exception)
        {
            ...
            lock (FailedRetirements)
            {
                FailedRetirements.Add((window, nativeWindow));
            }
    ...
    {
        using var allocationProfile = FrameworkWorkProfile.AllocationEnabled
            ? FrameworkWorkProfile.Begin(GetType(), 9)
            : default;
        if (e is null || _owner?.EnableTouchEvents != true)
        {
            return false;
        }
        ...
        if (_inputTiming)
        {
            Android.Util.Log.Info(
                "DorotiInputTiming",
                $"action={e.ActionMasked} eventMs={e.EventTime} queuedMs={queuedMs} history={e.HistorySize} x={e.GetX():F1} y={e.GetY():F1} dispatchMs={Stopwatch.GetElapsedTime(started).TotalMilliseconds:F3}"
            );
        }
    ...
    public override bool OnGenericMotionEvent(MotionEvent? e)
    {
        ...
        if (change is null || e.PointerCount == 0)
        {
            return base.OnGenericMotionEvent(e);
        }
    ...
    private void DispatchPointer(MotionEvent e, int index, PointerChange change)
    {
        ...
        var horizontalFactor =
            scroll && OperatingSystem.IsAndroidVersionAtLeast(26)
        ...
        _owner?.DispatchNativePointer(
            new(
                ...
                Tilt: e.GetAxisValue(Axis.Tilt, index)
            )
        );
    }
    ...
    protected override void Dispose(bool disposing)
    {
        if (disposing)
        {
            Disconnect();
            RemoveCallbacks(_allocationProfileCallback);
            _allocationProfileCallback.Dispose();
            _drawCallback.Dispose();
            _gpuCompletionCallback.Dispose();
        }
