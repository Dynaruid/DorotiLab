...
internal sealed class AndroidPointerSubscription : IDisposable
{
    ...
    {
        _view = view; _dispatch = dispatch; _gesture = new(dispatch);
    ...
    private void Unfocused(object? sender, FocusEventArgs args) => _gesture.Cancel(DorotiFrameClock.Now);
    private void Unloaded(object? sender, EventArgs args) => _gesture.Cancel(DorotiFrameClock.Now);
    private void Attach()
    {
        Detach();
        if (_view.Handler?.PlatformView is not Android.Views.View native) return;
        _native = native;
        native.Touch += Touch;
    ...
    private void Touch(object? sender, Android.Views.View.TouchEventArgs args)
    {
        ...
        for (var i = 0; i < e.PointerCount; i++)
        {
            ...
            _dispatch(new(DorotiFrameClock.Now, change.Value, kind, device, e.GetX(i), e.GetY(i),
                AndroidPointerMapping.Buttons(kind, (int)e.ButtonState), 0, 0, PointerSignalKind.none, e.GetPressure(i),
                Orientation: e.GetOrientation(i), Tilt: e.GetAxisValue(Axis.Tilt, i)));
        }
        args.Handled = true;
    }
    ...
    {
        var e = args.Event;
        if (e is null || _native is null || e.PointerCount == 0) return;
        var change = e.ActionMasked switch
        {
        ...
        _dispatch(new(DorotiFrameClock.Now, change.Value, kind, device, e.GetX(), e.GetY(),
            ...
            scroll ? PointerSignalKind.scroll : PointerSignalKind.none, e.GetPressure(0),
            Orientation: e.GetOrientation(0), Tilt: e.GetAxisValue(Axis.Tilt, 0)));
        args.Handled = true;
    }
    private void Detach()
    {
        _gesture.Cancel(DorotiFrameClock.Now);
        if (_native is not null) { _native.Touch -= Touch; _native.GenericMotion -= Generic; }
        _native = null;
    }
    public void Dispose()
    {
        _view.HandlerChanged -= Changed; _view.Unfocused -= Unfocused; _view.Unloaded -= Unloaded;
        Detach();
    }
}
#endif
