...
internal sealed class AndroidPlatformViewHost(
    DorotiGraphiteView owner,
    MauiTextInputBridge textInput
) : IDisposable
{
    private static readonly PlatformEffectSupport NativeEffects = new(
        true,
        1,
        32,
        Saturation: true,
    ...
    private readonly bool _optimize =
        Environment.GetEnvironmentVariable("DOROTI_ANDROID_PLATFORM_RASTER_MODE") != "baseline";
    private readonly bool _routeRaster =
        Environment.GetEnvironmentVariable("DOROTI_ANDROID_PLATFORM_RASTER_MODE")
        is not ("baseline" or "full-regions");
    ...
    internal IEnumerable<IPlatformViewFactory> CreateFactories(
        ...
        [
            new Factory(this, false),
            new Factory(this, true),
            new Factory(this, true, webView: true, resources),
        ];
    ...
    internal void Bind(DorotiAndroidVulkanView surface, FrameLayout container)
    {
        ...
        if (_container is not null && _container != container)
        {
            throw new InvalidOperationException(
                "A live Android PlatformView owner cannot change containers."
            );
        }
    ...
    private FrameLayout Container
    {
        get
        {
            ...
            if (
                _container is null
                && owner.Handler?.PlatformView is DorotiAndroidViewContainer container
            )
            {
            ...
            return _container
                ?? throw new InvalidOperationException(
                    "Android PlatformView owner is not attached."
                );
        }
    }
    ...
    private static bool HasCommands(IReadOnlyList<SceneCommand> commands, int depth = 0)
    {
        if (depth > 256)
        {
            throw new InvalidDataException("Retained platform scene depth exceeded.");
        }

        return commands.Any(c =>
            c.Operation is "platformView" or "inputShield"
            || (
                c.HostPayload is SceneRetainedPayload retained
                && HasCommands(retained.Commands, depth + 1)
            )
        );
    }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("Unretired Android platform frame.");
        }
        ...
        var cacheScope = SkiaPlatformRasterContent.CacheScope.From(
            token,
            width,
            height,
        ...
        try
        {
            plan = PlatformCompositionPlanner.Build(
                commands,
                token,
                _coordinator!,
                mode,
        ...
        try
        {
            ...
            {
                _instances[placement.Handle].Validate(placement);
            }

            foreach (var part in plan.Parts.OfType<PlatformShieldSegment>())
            {
                ValidateTransform(part.Shield.Transform);
            }

            var handles = placements
                .Select(p => p.Handle)
                .Concat(_visible.Select(p => p.Handle).Where(IsLive))
                .Distinct();
            ...
            var backdropSample = backdropPart is not null
                ? BackdropSample(backdropPart, token, width, height)
                : default;
            if (
                !plan.HasNativeContent
                || (_optimize && mode == PlatformViewComposition.NativeOverlay)
            )
            {
                foreach (var raster in allRasters)
                {
                    renderer.DrawPlatformRasterSegment(canvas, raster.Commands, width, height);
                }

                _pending = new(plan, batch!, null, null, [], width, height, Scope: cacheScope);
                return;
            }
            ...
            for (var i = 0; i < allRasters.Length; i++)
            {
                ...
                {
                    renderer.DrawPlatformRasterSegment(canvas, raster.Commands, width, height);
                    if (backdropPart is null || backdropSample.IsEmpty)
                    {
                        continue;
                    }
                }
                ...
                {
                    overlayBounds = RasterOverlayBounds(
                        plan,
                        raster.PaintOrder,
                        token,
                        width,
                    ...
                    {
                        canvas.ClipRect(overlayBounds, SKClipOperation.Difference, false);
                    }
                ...
                if (
                    ...
                    && !(
                        i == 0
                        && backdropPart is not null
                        && renderer.PlatformBackgroundColor.Alpha != 0
                    )
                )
                {
                    continue;
                ...
                var slices =
                    _optimize && i == 0 && backdropPart is not null
                        ? BackdropRasterSlices(
                            plan,
                            backdropPart,
                            raster.Commands,
                            StableBackdropSourceBounds(
                                raster.Commands,
                                backdropSample,
                                width,
                                height
                    ...
                    :
                    [
                        new SkiaPlatformRasterContent.Slice(
                            raster.Commands,
                            new(0, 0, width, height)
                        ),
                    ];
                if (
                    slices.Count >= SliceStride
                    || cached.Count + changed.Count + slices.Count > SliceStride
                )
                {
                    throw new InvalidOperationException(
                        "Android platform raster slice count exceeds 128."
                    );
                }
                ...
                for (var index = 0; index < slices.Count; index++)
                {
                    ...
                    var existing = _optimize
                        ? _rasters.FirstOrDefault(r =>
                            r.PixelWidth == width
                            && r.PixelHeight == height
                            && r.Epoch == _rasterEpoch
                            && !cached.ContainsValue(r)
                            && SkiaPlatformRasterContent.CanReuse(
                                r.Scope,
                                new(r.Commands, r.Bounds),
                                cacheScope,
                                slice
                            )
                        )
                    ...
                    {
                        changed.Add(new(new(key, slice.Commands), slice.Bounds));
                    }
                }
            }
            ...
            if (
                ...
                && _surface!.SupportsHardwareBuffer
            )
            {
                var bytes =
                    _rasters.Sum(r => r.Bytes)
                    + rasters.Sum(r => (long)r.Bounds.Width * r.Bounds.Height * 8);
                if (bytes > 256L * 1024 * 1024)
                {
                    throw new InvalidOperationException(
                        "Android shared raster banks exceed 256 MiB."
                    );
                }
                ...
                try
                {
                    ...
                    {
                        var output = new AndroidSharedRaster(
                            _surface,
                            raster.Bounds.Width,
                        ...
                        if (raster.Segment.PaintOrder / SliceStride == 0)
                        {
                            output.Canvas.DrawColor(renderer.PlatformBackgroundColor);
                        }

                        renderer.DrawPlatformRasterSegment(
                            output.Canvas,
                            raster.Segment.Commands,
                            width,
                            height
                        );
                    }
            ...
            var atlasWidth = Math.Max(
                rasters.Max(r => r.Bounds.Width),
                Math.Min(width, rasters.Sum(r => r.Bounds.Width))
            );
            var atlasY = 0;
            var atlasX = 0;
            var shelfHeight = 0;
            foreach (var raster in rasters.OrderByDescending(r => r.Bounds.Height))
            {
                ...
                {
                    atlasY = checked(atlasY + shelfHeight);
                    atlasX = 0;
                    shelfHeight = 0;
                }
                raster.AtlasX = atlasX;
                raster.AtlasY = atlasY;
            ...
            var reservedBytes =
                _rasters.Sum(r => r.Bytes)
                + cached.Values.Sum(r => r.Bytes)
                + rasters.Sum(r => (long)r.Bounds.Width * r.Bounds.Height * 12)
                + ((long)atlasWidth * atlasHeight * 8);
            if (atlasWidth > 16384 || atlasHeight > 16384 || reservedBytes > 256L * 1024 * 1024)
            {
                throw new InvalidOperationException(
                    $"Android platform raster banks exceed 256 MiB / 16384 pixels: atlas={atlasWidth}x{atlasHeight}, slices={rasters.Length}, bytes={reservedBytes}."
                );
            }
            ...
            for (var i = 0; i < rasters.Length; i++)
            {
                ...
                atlas.Canvas.Translate(
                    raster.AtlasX - raster.Bounds.Left,
                    raster.AtlasY - raster.Bounds.Top
                );
                if (raster.Segment.PaintOrder / SliceStride == 0)
                {
                    atlas.Canvas.DrawColor(renderer.PlatformBackgroundColor);
                }
            ...
            _pending = new(
                ...
                batch!,
                atlas,
                _surface!.RequestPlatformReadback(atlas, info),
                rasters,
                width,
                height,
                cached,
    ...
    private bool IsLive(PlatformViewHandle handle)
    {
        try
        {
            return _coordinator!.Resolve(handle.InstanceId) == handle;
        }
    ...
    private IReadOnlyList<SkiaPlatformRasterContent.Slice> BackdropRasterSlices(
        PlatformCompositionPlan plan,
        PlatformBackdropSegment effect,
        IReadOnlyList<SceneCommand> commands,
        SKRectI sample,
    ...
    )
    {
        ...
        foreach (
            var native in plan
                .Parts.OfType<PlatformNativeSegment>()
                .Where(n => n.PaintOrder < effect.PaintOrder && n.Placement.Visible)
        )
        {
            if (!_instances[native.Placement.Handle].IsOpaque)
            {
                continue;
            }
            ...
            var opaque = new SKRectI(
                (int)Math.Ceiling(rect.left * token.DeviceScaleX),
                (int)Math.Ceiling(rect.top * token.DeviceScaleY),
                (int)Math.Floor(rect.right * token.DeviceScaleX),
                (int)Math.Floor(rect.bottom * token.DeviceScaleY)
            );
            regions = regions
                .SelectMany(region => SkiaPlatformRasterContent.SubtractOpaque(region, opaque))
                .ToArray();
            if (regions.Count == 0)
            {
                break;
        ...
        return regions
            .Select(region => new SkiaPlatformRasterContent.Slice(commands, region))
            .ToArray();
    }

    private SKRectI StableBackdropSourceBounds(
        IReadOnlyList<SceneCommand> commands,
        SKRectI sample,
        int width,
        int height
    )
    {
        ...
        if (width > 4096 || height > 4096 || (long)width * height * 4 > 64L * 1024 * 1024)
        {
            return sample;
        }
        ...
        foreach (
            var previous in _rasters.Where(r =>
                r.PaintOrder / SliceStride == 0
                && r.PixelWidth == width
                && r.PixelHeight == height
                && r.Epoch == _rasterEpoch
            )
        ...
        {
            if (
                SkiaPlatformRasterContent.Equivalent(previous.Commands, commands)
                || SkiaPlatformRasterContent.EquivalentTranslation(
                    new(previous.Commands, viewport),
                    new(commands, viewport)
                )
            )
            {
                return viewport;
    ...
    private static SKRectI RasterOverlayBounds(
        PlatformCompositionPlan plan,
        int order,
        PlatformCompositionToken token,
        int width,
        int height,
        SKRectI backdropSample
    )
    {
        ...
        var result = plan
            .Parts.OfType<PlatformBackdropSegment>()
            .Any(effect =>
                order < effect.PaintOrder
                || plan.Parts.OfType<PlatformRasterSegment>()
                    .Any(raster =>
                        raster.PaintOrder > 0
                        && raster.PaintOrder < effect.PaintOrder
                        && raster.PaintOrder < order
                        && SkiaPlatformRasterContent.HasDrawing(raster.Commands)
                    )
            )
            ? backdropSample
        ...
        foreach (
            var native in plan
                .Parts.OfType<PlatformNativeSegment>()
                .Where(n => n.PaintOrder < order && n.Placement.Visible)
        )
        {
            var p = native.Placement;
            var rect = Map(p.Bounds, p.Transform, p.Clip);
            ...
            var pixels = Intersect(
                new(
                    (int)Math.Floor(rect.left * token.DeviceScaleX),
                    (int)Math.Floor(rect.top * token.DeviceScaleY),
                    (int)Math.Ceiling(rect.right * token.DeviceScaleX),
                    (int)Math.Ceiling(rect.bottom * token.DeviceScaleY)
                ),
                new(0, 0, width, height)
            );
            if (pixels.IsEmpty)
            ...
            result = result.IsEmpty
                ? pixels
                : new(
                    Math.Min(result.Left, pixels.Left),
                    Math.Min(result.Top, pixels.Top),
                    Math.Max(result.Right, pixels.Right),
                    Math.Max(result.Bottom, pixels.Bottom)
                );
        }
        return result.IsEmpty ? default : result;
    }

    private static SKRectI BackdropSample(
        PlatformBackdropSegment backdrop,
        PlatformCompositionToken token,
        int width,
        int height
    )
    {
        var sx = token.DeviceScaleX;
        var sy = token.DeviceScaleY;
        var bounds = backdrop.Bounds;
        var rect = Intersect(
            new(
                (int)Math.Floor((bounds.left - (backdrop.SigmaX * 3)) * sx),
                (int)Math.Floor((bounds.top - (backdrop.SigmaY * 3)) * sy),
                (int)Math.Ceiling((bounds.right + (backdrop.SigmaX * 3)) * sx),
                (int)Math.Ceiling((bounds.bottom + (backdrop.SigmaY * 3)) * sy)
            ),
            new(0, 0, width, height)
        );
        if (
            rect.Width > 4096
            || rect.Height > 4096
            || (long)rect.Width * rect.Height * 4 > 64L * 1024 * 1024
        )
        {
            throw new InvalidOperationException(
                "Android backdrop sample exceeds its 4096px / 64 MiB limit."
            );
        }
    ...
    private bool HasStaleNative(IReadOnlyList<SceneCommand> commands) =>
        commands.Any(c =>
    ...
    {
        AndroidPlatformViewDispatcher.VerifyThread();
        if (!accepted || _pending is null || _disposed)
        {
            return FinishCore(accepted);
        }
    ...
    private bool FinishCore(bool accepted)
    {
        ...
        try
        {
            ...
            var parent = Container;
            if (frame.Width != parent.Width || frame.Height != parent.Height)
            {
                return false;
            }
            ...
            if (frame.Shared is { } shared)
            {
                foreach (var slice in frame.Rasters)
                {
                    ...
                    var slot = _rasters.FirstOrDefault(r =>
                        r.PaintOrder == slice.Segment.PaintOrder && !desired.ContainsValue(r)
                    );
                    if (slot is not null)
                    {
                    ...
                    {
                        var raster = new RasterView(
                            parent.Context!,
                            bitmap,
                            slice.Segment.Commands,
                            frame.Width,
                            frame.Height,
                            _rasterEpoch,
                            slice.Bounds,
            ...
            if (frame.Readback is { } task)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Platform readback outlived its completion fence."
                    );
                }

                var pixels = task.GetAwaiter().GetResult();
                _readbackFrames++;
                _readbackBytes += pixels.Pixels.LongLength;
                for (var i = 0; i < frame.Rasters.Length; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new InvalidOperationException(
                                "Android bitmap pixels are unavailable."
                            );
                        }
                        ...
                        try
                        {
                            ...
                            if (
                                slice.AtlasX == 0
                                && pixels.RowBytes == rowBytes
                                && destinationRowBytes == rowBytes
                            )
                            {
                                System.Runtime.InteropServices.Marshal.Copy(
                                    pixels.Pixels,
                                    checked(slice.AtlasY * pixels.RowBytes),
                                    destination,
                                    checked(rowBytes * slice.Bounds.Height)
                                );
                            }
                            else
                            {
                                for (var y = 0; y < slice.Bounds.Height; y++)
                                {
                                    System.Runtime.InteropServices.Marshal.Copy(
                                        pixels.Pixels,
                                        checked(
                                            ((slice.AtlasY + y) * pixels.RowBytes)
                                            + (slice.AtlasX * 4)
                                        ),
                                        IntPtr.Add(destination, checked(y * destinationRowBytes)),
                                        rowBytes
                                    );
                                }
                            }
                        ...
                        var slot = _optimize
                            ? _rasters.FirstOrDefault(r =>
                                r.PaintOrder == slice.Segment.PaintOrder
                                && !desired.ContainsValue(r)
                            )
                            : null;
                        if (slot is not null)
                        {
            ...
            var next = frame
                .Plan.Parts.OfType<PlatformNativeSegment>()
                .Select(p => p.Placement)
                .ToArray();
            _scaleX = frame.Plan.Token.DeviceScaleX;
            _scaleY = frame.Plan.Token.DeviceScaleY;
            foreach (var p in next)
            {
                CompleteNow(frame.Batch.AttachAsync(p));
            }

            var nextHandles = next.Select(p => p.Handle).ToHashSet();
            foreach (
                var old in _visible.Where(p =>
                    !nextHandles.Contains(p.Handle) && frame.Batch.Contains(p.Handle)
                )
            )
            {
            ...
            updates.Clear();
            foreach (var raster in _rasters.Where(r => !desired.ContainsValue(r)))
            {
                parent.RemoveView(raster);
                raster.Dispose();
            }
            _rasters.Clear();
            ...
            {
                var shield = _shields[^1];
                _shields.RemoveAt(_shields.Count - 1);
                parent.RemoveView(shield);
                shield.Dispose();
            }
            while (_shields.Count < shieldParts.Length)
            {
                var shield = new ShieldView(parent.Context!, _surface!)
                {
                    ImportantForAccessibility = ImportantForAccessibility.No,
                };
                _shields.Add(shield);
                parent.AddView(shield);
            }
            foreach (var raster in prepared)
            {
                parent.AddView(
                    raster,
                    new FrameLayout.LayoutParams(raster.Bounds.Width, raster.Bounds.Height)
                    {
                        LeftMargin = raster.Bounds.Left,
                        TopMargin = raster.Bounds.Top,
            ...
            {
                if (frame.Translated?.TryGetValue(order, out var slice) == true)
                {
                    raster.Move(slice);
                }
            ...
            var backdropOrder = frame
                .Plan.Parts.OfType<PlatformBackdropSegment>()
                .SingleOrDefault()
                ?.PaintOrder;
            var shieldIndex = 0;
            if (
                OperatingSystem.IsAndroidVersionAtLeast(31)
                && _backdrop is not null
                && !frame.Plan.Parts.OfType<PlatformBackdropSegment>().Any()
            )
            {
                parent.RemoveView(_backdrop);
                _backdrop.Dispose();
                _backdrop = null;
            }
            foreach (var part in frame.Plan.Parts.OrderBy(p => p.PaintOrder))
            {
                switch (part)
                {
                    case PlatformRasterSegment raster:
                        foreach (
                            var slice in _rasters
                                .Where(r => r.PaintOrder / SliceStride == raster.PaintOrder)
                                .OrderBy(r => r.PaintOrder)
                        )
                        {
                    ...
                    case PlatformNativeSegment native:
                        ...
                        if (
                            OperatingSystem.IsAndroidVersionAtLeast(31)
                            && nativeClip is AndroidPlatformBackdropSourceView source
                        )
                        {
                            source.SetBackdropSampling(
                                backdropOrder is { } effectOrder && native.PaintOrder < effectOrder
                            );
                        }

                        paintOrder.Add(nativeClip);
                        break;
                    case PlatformShieldSegment shield:
                        var bounds = Map(
                            shield.Shield.Bounds,
                            shield.Shield.Transform,
                            shield.Shield.Clip
                        );
                        ...
                        Place(
                            view,
                            bounds,
                            frame.Plan.Token.DeviceScaleX,
                            frame.Plan.Token.DeviceScaleY
                        );
                        paintOrder.Add(view);
                        break;
                    case PlatformBackdropSegment backdrop
                        when OperatingSystem.IsAndroidVersionAtLeast(31):
                        if (!parent.IsHardwareAccelerated)
                        {
                            throw new PlatformNotSupportedException(
                                "Native backdrop requires Android hardware acceleration."
                            );
                        }

                        if (_backdrop is null)
                        {
                            _backdrop = new(parent.Context!);
                            parent.AddView(_backdrop);
                        }
                        var viewport = Rect.fromLTWH(
                            0,
                            0,
                            frame.Width / _scaleX,
                            frame.Height / _scaleY
                        );
                        var visible = backdrop.Bounds.intersect(viewport);
                        Place(_backdrop, visible, _scaleX, _scaleY);
                        _backdrop.UpdateFrame(
                            BackdropSample(backdrop, frame.Plan.Token, frame.Width, frame.Height),
                            paintOrder.Where(v => v is not ShieldView).ToArray(),
                            (float)(backdrop.SigmaX * _scaleX),
                            (float)(backdrop.SigmaY * _scaleY),
                            (float)(backdrop.Style?.Saturation ?? 1)
                        );
                        paintOrder.Add(_backdrop);
                        break;
                }
            }
            if (!paintOrder.OrderBy(parent.IndexOfChild).SequenceEqual(paintOrder))
            {
                foreach (var view in paintOrder)
                {
                    view.BringToFront();
                }
            }
            ...
            if (_profile)
            {
                Android.Util.Log.Info(
                    "DorotiPlatformFrame",
                    $"mode={(_optimize ? "optimized" : "baseline")} frame={++_commits} readbackFrames={_readbackFrames} readbackBytes={_readbackBytes} sharedSlices={_sharedSlices} reusedSlices={_reusedSlices} changed={frame.Rasters.Length} cached={frame.Cached?.Count ?? 0} uiMs={System.Diagnostics.Stopwatch.GetElapsedTime(started).TotalMilliseconds:F3}"
                );
            }
    ...
    private static void CompleteNow(ValueTask task)
    {
        ...
        {
            throw new InvalidOperationException(
                "Reserved Android UI operation unexpectedly suspended."
            );
        }
    ...
    private sealed record Pending(
        ...
        SKSurface? Atlas,
        Task<SkiaGraphiteReadback>? Readback,
        RasterSlice[] Rasters,
        int Width,
        int Height,
        ...
        Dictionary<int, AndroidSharedRaster>? Shared = null
    ) : IDisposable
    {
        public void Dispose()
        {
    ...
    private sealed class RasterView(
        Context context,
        Bitmap bitmap,
        IReadOnlyList<SceneCommand> commands,
        int width,
    ...
    ) : NativeView(context), IAndroidPlatformBackdropSource
    {
        ...
        internal bool BackdropSourceOnly
        {
            set
            {
                if (_backdropSourceOnly == value)
                {
                    return;
                }
                _backdropSourceOnly = value;
                Invalidate();
            }
        }
        ...
        internal void Adopt(
            ...
            int pixelHeight,
            long rasterEpoch,
            SkiaPlatformRasterContent.CacheScope nextScope
        )
        {
            var old = _bitmap;
            _bitmap = next;
            commands = slice.Segment.Commands;
            width = pixelWidth;
            height = pixelHeight;
            epoch = rasterEpoch;
            bounds = slice.Bounds;
        ...
        public void DrawBackdropSource(Android.Graphics.Canvas canvas)
        {
            if (_bitmap is { } value)
            {
                canvas.DrawBitmap(value, 0, 0, null);
            }
        }
    ...
    private sealed class ShieldView(Context context, DorotiAndroidVulkanView surface)
        : NativeView(context)
    {
    ...
    private static void ValidateTransform(PlatformViewTransform transform)
    {
        if (!transform.IsAxisAligned || transform.M11 != 1 || transform.M22 != 1)
        {
            throw new NotSupportedException(
                "Android platform views support translation and rectangular clipping only."
            );
        }
    }
    ...
    {
        var left = (int)Math.Ceiling(bounds.left * sx);
        var top = (int)Math.Ceiling(bounds.top * sy);
        var right = (int)Math.Floor(bounds.right * sx);
        var bottom = (int)Math.Floor(bounds.bottom * sy);
        if (
            view.LayoutParameters is FrameLayout.LayoutParams current
            && current.Width == Math.Max(0, right - left)
            && current.Height == Math.Max(0, bottom - top)
        ...
        )
        {
            ...
            if (current.LeftMargin != left)
            {
                current.LeftMargin = left;
            }

            if (current.TopMargin != top)
            {
            ...
            if (view.Top != top)
            {
                view.OffsetTopAndBottom(top - view.Top);
            }
    ...
    ) : IPlatformViewFactory
    {
        public string ViewType =>
            webView ? "doroti/webview"
            : editor ? "doroti/native-editor"
        ...
        public PlatformViewSupport QuerySupport(PlatformViewRequest request)
        {
            var supported =
                ...
                && request.Composition
                    is PlatformViewComposition.NativeOverlay
                        or PlatformViewComposition.InterleavedComposition
                && (request.Effects & ~PlatformViewEffects.RectClip) == 0;
            return new(
                "Android/View/Graphite-readback",
                ...
                Reason: supported
                    ? null
                    : "Only native button/editor, translation and rect clip are supported.",
                NativeBackdropBlur: supported
                ...
                Capabilities: new(
                    ...
                    host._optimize
                    && OperatingSystem.IsAndroidVersionAtLeast(29)
                    && host._surface?.SupportsHardwareBuffer == true
                        ? PlatformViewTransport.GpuShared
                        : PlatformViewTransport.BoundedReadback,
        ...
        public async ValueTask<IPlatformViewInstance> CreateAsync(
    ...
    private sealed class Instance : IPlatformViewInstance, IPlatformWebViewInstance
    {
        ...
        public Task<WebViewResult> ExecuteAsync(
            ...
            ?? throw new WebViewException(WebViewError.Unsupported, "Not a WebView.");
        ...
        internal Instance(
            ...
            WebViewOptions? webOptions = null,
        ...
        )
        {
            ...
            {
                _web = new AndroidWebViewSession(
                    parent.Context!,
                    handle,
                    webOptions,
                    content
                        ?? throw new InvalidOperationException("WebView content was not prepared."),
            ...
            {
                TextView control = editor
                    ? new EditText(parent.Context!)
                    : new Android.Widget.Button(parent.Context!);
                control.Text = text;
                _control = control;
            }
            ...
            _control.ContentDescription =
                $"doroti-platform-view-{handle.InstanceId}-{handle.InstanceGeneration}";
            ...
            if (_control is TextView textControl)
            {
                textControl.TextChanged += TextChanged;
            }

            if (!editor && webOptions is null)
            {
                _control.Click += Click;
            }

            Clip.AddView(_control);
            parent.AddView(Clip);
            Android.Util.Log.Info(
                "DorotiPlatformView",
                $"create handle={handle} native={_control.Handle}"
            );
        }

        private void Click(object? sender, EventArgs e)
        {
            if (!_disabled && !_disposed && _control is TextView textControl)
            {
                textControl.Text = $"Native clicks: {++_clicks}";
                Android.Util.Log.Info(
                    "DorotiPlatformView",
                    $"click handle={_handle} count={_clicks}"
                );
            }
        }
        ...
        private void FocusChanged(object? sender, NativeView.FocusChangeEventArgs e)
        {
            ...
            try
            {
                _host.YieldTextFocus();
                _focused(_handle);
            }
            catch (Exception error)
            {
                Android.Util.Log.Error("DorotiPlatformView", error.ToString());
            }
        }
        ...
        public ValueTask ApplyAsync(PlatformViewPlacement placement)
        {
            ...
            var sx = _host._scaleX;
            var sy = _host._scaleY;
            visible = visible.intersect(
                Rect.fromLTWH(0, 0, _host.Container.Width / sx, _host.Container.Height / sy)
            );
            if (
                !placement.Visible
                || visible.isEmpty
            ...
            Place(
                _control,
                Rect.fromLTWH(
                    bounds.left - (Math.Ceiling(visible.left * sx) / sx),
                    bounds.top - (Math.Ceiling(visible.top * sy) / sy),
                    bounds.width,
                    bounds.height
                ),
                sx,
        ...
        private void ReleaseFocus()
        {
            ...
            _control.ClearFocus();
            _host._surface?.RequestFocus();
            var ime =
                _control.Context?.GetSystemService(Context.InputMethodService)
                as Android.Views.InputMethods.InputMethodManager;
            ime?.HideSoftInputFromWindow(
                _control.WindowToken,
        ...
        {
            AndroidPlatformViewDispatcher.VerifyThread();
            if (focused && !_disabled && !_disposed && Clip.Visibility == ViewStates.Visible)
            {
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            finally
            {
                _control.Dispose();
                Clip.Dispose();
                _host._instances.Remove(_handle);
            }
    ...
    public void Dispose()
    {
        ...
        {
            CompleteNow(session.DisposeAsync());
        }
        ...
        {
            _container?.RemoveView(backdrop);
            backdrop.Dispose();
            _backdrop = null;
        }
        foreach (var raster in _rasters)
        {
            _container?.RemoveView(raster);
            raster.Dispose();
        }
        foreach (var shield in _shields)
        {
            _container?.RemoveView(shield);
            shield.Dispose();
        }
        _rasters.Clear();
        _shields.Clear();
