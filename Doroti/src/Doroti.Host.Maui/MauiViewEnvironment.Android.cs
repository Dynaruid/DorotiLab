...
internal sealed partial class MauiViewEnvironment
{
    ...
    private sealed class ContrastListener(Action changed)
        : Java.Lang.Object,
            Android.App.UiModeManager.IContrastChangeListener
    {
    ...
    {
        while (
            context is Android.Content.ContextWrapper wrapper && context is not Android.App.Activity
        )
        {
    ...
    private sealed class LayoutConsumer(Action<WindowLayoutInfo> changed)
        : Java.Lang.Object,
    ...
    private sealed class InsetsListener(Action<WindowInsets> changed)
        : Java.Lang.Object,
            Android.Views.View.IOnApplyWindowInsetsListener
    {
        public WindowInsets OnApplyWindowInsets(Android.Views.View? view, WindowInsets? insets)
        {
            if (insets is null)
            {
                throw new ArgumentNullException(nameof(insets));
            }
    ...
    private sealed class InsetsAnimation(Action<WindowInsets> changed)
        : WindowInsetsAnimation.Callback((int)WindowInsetsAnimationDispatchMode.ContinueOnSubtree)
    {
        public override WindowInsets OnProgress(
            WindowInsets insets,
            IList<WindowInsetsAnimation> runningAnimations
        )
        {
            changed(insets);
            return insets;
    ...
    {
        if (_element.Handler?.PlatformView is not Android.Views.View native)
        {
            return;
        }
        ...
        var listener = new InsetsListener(value =>
        {
        ...
        _detach.Add(() =>
        {
            native.SetOnApplyWindowInsetsListener(null);
            _currentInsets = null;
            listener.Dispose();
        });
        if (OperatingSystem.IsAndroidVersionAtLeast(30))
        {
            var animation = new InsetsAnimation(value =>
            {
                _currentInsets = value;
                Refresh();
            });
            ...
            _detach.Add(() =>
            {
                if (OperatingSystem.IsAndroidVersionAtLeast(30))
                {
                    native.SetWindowInsetsAnimationCallback(null);
                }
                animation.Dispose();
            });
        }
        if (native.Context?.ContentResolver is { } resolver)
        {
            ...
            resolver.RegisterContentObserver(
                Android.Provider.Settings.Secure.ContentUri!,
                true,
                settings
            );
            resolver.RegisterContentObserver(
                Android.Provider.Settings.Global.ContentUri!,
                true,
                settings
            );
            _detach.Add(() =>
        ...
        if (FindActivity(native.Context) is { } activity)
        {
            ...
            if (
                ...
                && activity.GetSystemService(Android.Content.Context.UiModeService)
                    is Android.App.UiModeManager modes
            )
            {
                var contrast = new ContrastListener(() => Refresh());
                modes.AddContrastChangeListener(
                ...
                _detach.Add(() =>
                {
                    ...
                    {
                        modes.RemoveContrastChangeListener(contrast);
                    }
                    contrast.Dispose();
                });
            }
            var tracker = new WindowInfoTrackerCallbackAdapter(
                WindowInfoTracker.Companion.GetOrCreate(activity)
            );
            var consumer = new LayoutConsumer(info =>
            {
                _windowLayout = info;
                Refresh();
            });
            ...
            _detach.Add(() =>
            {
                tracker.RemoveWindowLayoutInfoListener(consumer);
                consumer.Dispose();
                tracker.Dispose();
                _windowLayout = null;
            });
    ...
    {
        if (
            _element.Handler?.PlatformView is not Android.Views.View native
            || (_currentInsets ?? native.RootWindowInsets) is not { } insets
        )
        {
            return;
        }
        ...
        if (
            ...
            && FindActivity(native.Context)?.WindowManager?.CurrentWindowMetrics
                is { } windowMetrics
        )
        {
        ...
        ViewPadding bars,
            ime,
            gestures = ViewPadding.zero;
        if (OperatingSystem.IsAndroidVersionAtLeast(30))
        {
            static ViewPadding Convert(Android.Graphics.Insets value) =>
...
#pragma warning restore CS0618, CA1422
        ...
        if (FontScaler is null || TextScale != configuration.FontScale)
        {
            ...
            FontScaler = OperatingSystem.IsAndroidVersionAtLeast(34)
                ? size =>
                    TypedValue.ApplyDimension(ComplexUnitType.Sp, (float)size, metrics)
                    / metrics.Density
                : size => size * scale;
        }
        GestureSettings = new(
            Android.Views.ViewConfiguration.Get(native.Context!)?.ScaledTouchSlop
        );
        Use24Hour = Android.Text.Format.DateFormat.Is24HourFormat(native.Context);
        var manager =
            native.Context?.GetSystemService(Android.Content.Context.AccessibilityService)
        ...
        var bold =
            ...
            && configuration.FontWeightAdjustment != int.MaxValue
            && configuration.FontWeightAdjustment >= 300;
        var contrast =
            OperatingSystem.IsAndroidVersionAtLeast(34)
            && (
                native.Context.GetSystemService(Android.Content.Context.UiModeService)
        ...
        native.GetLocationInWindow(windowOrigin);
        if (_windowLayout is not null)
        {
            foreach (var feature in _windowLayout.DisplayFeatures)
            {
                ...
                var local = new Rect(
                    (b.Left - windowOrigin[0]) / density,
                    (b.Top - windowOrigin[1]) / density,
                    (b.Right - windowOrigin[0]) / density,
                    (b.Bottom - windowOrigin[1]) / density
                );
                if (
                    local.right < 0
                    || local.bottom < 0
                    || local.left > native.Width / density
                    || local.top > native.Height / density
                )
                {
                    continue;
                }
                ...
                features.Add(
                    new(
                        ...
                        folding?.State.ToString() == "HALF_OPENED"
                                ? DisplayFeatureState.postureHalfOpened
                            : folding is not null ? DisplayFeatureState.postureFlat
        ...
        if (OperatingSystem.IsAndroidVersionAtLeast(28) && insets.DisplayCutout is { } cutout)
        {
            foreach (var b in cutout.BoundingRects)
            {
                ...
                {
                    features.Add(
                        new(local, DisplayFeatureType.cutout, DisplayFeatureState.unknown)
                    );
                }
            }
        }
        ...
        {
            Corners = new(
                insets.GetRoundedCorner((int)RoundedCornerPosition.TopLeft)?.Radius ?? 0,
