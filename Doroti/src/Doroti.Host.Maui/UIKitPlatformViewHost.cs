...
internal sealed class UIKitPlatformViewHost : IDisposable
{
    ...
    private (UIKitPlatformRasterSurface Slot, int Order)[] _visibleRasters = [];
    ...
    private void ApplyLayers((UIKitPlatformRasterSurface Slot, int Order)[] rasters, PlatformInputShield[] shields, PlatformBackdropSegment[] effects)
    {
        ...
        foreach (var child in parent.Subviews.OrderBy(child => child.Layer.ZPosition)) parent.BringSubviewToFront(child);
        ...
        UIKitPlatformRasterSurface.RasterFrame[] frames, IDisposable[] gpuLeases) : IDisposable, IPreparedPlatformComposition
    {
        ...
        private readonly (UIKitPlatformRasterSurface Slot, int Order)[] _previousRasters = host._visibleRasters;
