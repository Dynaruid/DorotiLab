...
internal sealed class UIKitTrackpadInput : IDisposable
{
    ...
    private UIPanGestureRecognizer? _pan, _wheel;
    ...
    private MauiTrackpadGesture? _panStream, _pinchStream;
    private CGPoint _lastWheel, _lastHover;
    ...
    private static ulong Device(UIGestureRecognizer recognizer) => unchecked((ulong)(nint)recognizer.Handle);
    ...
    private void Attach()
    {
        ...
        _native = native;
        _pan = new UIPanGestureRecognizer(Continuous) { AllowedScrollTypesMask = UIScrollTypeMask.Continuous };
        _wheel = new UIPanGestureRecognizer(Discrete) { AllowedScrollTypesMask = UIScrollTypeMask.Discrete };
        ...
        _delegate.OnScroll = () =>
        {
            if (DorotiFrameClock.Now < _inertiaDeadline) _panStream.CancelInertia(DorotiFrameClock.Now);
    ...
    {
        if (_panStream is null || _native is null) return;
        ...
        switch (recognizer.State)
        {
            ...
            case UIGestureRecognizerState.Failed:
                ...
                _inertiaDeadline = time + TimeSpan.FromSeconds(Math.Max(0,
                    .1821 * Math.Log(Math.Max(1, Math.Max(Math.Abs(velocity.X), Math.Abs(velocity.Y)))) - .4825));
                break;
        }
    }
    ...
    {
        if (_pinchStream is null) return;
        if (recognizer.State is UIGestureRecognizerState.Began or UIGestureRecognizerState.Changed)
        {
            ...
            if (recognizer.State == UIGestureRecognizerState.Changed)
                _pinchStream.Update(DorotiFrameClock.Now, 0, 0, (double)(_pinch?.Scale ?? 1), (double)(_rotation?.Rotation ?? 0));
        }
        else if (recognizer.State is UIGestureRecognizerState.Ended or UIGestureRecognizerState.Cancelled or UIGestureRecognizerState.Failed)
        {
            ...
            if (_pinchParts == 0)
            {
                ...
                if (_pinch is not null) _pinch.Scale = 1;
                if (_rotation is not null) _rotation.Rotation = 0;
            }
        }
    }
    ...
    private void Discrete(UIPanGestureRecognizer recognizer)
    {
        ...
        if (recognizer.State is UIGestureRecognizerState.Began or UIGestureRecognizerState.Changed or UIGestureRecognizerState.Ended)
            _dispatch(new(DorotiFrameClock.Now, PointerChange.hover, PointerDeviceKind.mouse, Device(recognizer),
                position.X * Ratio, position.Y * Ratio, 0, x, y, PointerSignalKind.scroll, 0));
        _lastWheel = recognizer.State is UIGestureRecognizerState.Ended or UIGestureRecognizerState.Cancelled or UIGestureRecognizerState.Failed
            ? CGPoint.Empty : translation;
    }
    ...
    {
        Cancel();
        foreach (var recognizer in new UIGestureRecognizer?[] { _pan, _wheel, _pinch, _rotation, _hover })
        {
        ...
        _pan = _wheel = null;
        _pinch = null; _rotation = null; _hover = null; _native = null;
    }
    ...
    private sealed class GestureDelegate : UIGestureRecognizerDelegate
    {
        ...
        {
            if (@event.Type == UIEventType.Scroll) OnScroll?.Invoke();
            return true;
        }
    }
}
