...
internal sealed partial class MauiViewEnvironment
{
    ...
    {
        if (_element.Handler?.PlatformView is not NSView native)
        {
            return;
        }
        ...
        var voiceOver = NSWorkspace.SharedWorkspace.AddObserver(
            "voiceOverEnabled",
        ...
        var accessibility = workspace.AddObserver(
            new NSString("NSWorkspaceAccessibilityDisplayOptionsDidChangeNotification"),
            _ => Refresh()
        );
        _detach.Add(() =>
        {
            workspace.RemoveObserver(accessibility);
    ...
    {
        if (_element.Handler?.PlatformView is not NSView native || native.Window is not { } window)
        {
            return;
        }

        NativePhysicalSize = new(
            Math.Round(native.Bounds.Width * window.BackingScaleFactor),
            Math.Round(native.Bounds.Height * window.BackingScaleFactor)
        );
        var safe = native.SafeAreaInsets;
        // NSEdgeInsets uses named top/bottom independent of NSView's flipped drawing coordinates.
        Padding = ViewOcclusion.Scale(
