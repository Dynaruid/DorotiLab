...
internal sealed class AndroidTrackpadGesture(Action<MauiSurfacePointerData> dispatch)
{
    ...
    )
    {
        ...
        {
            if (_pans.Remove(device, out var old))
            {
                old.Gesture.Remove(time);
            }
        ...
        if (!_pans.TryGetValue(device, out var pan))
        {
            return false;
        }

        if (change == PointerChange.move)
        {
            pan.Gesture.Update(time, x - pan.X, y - pan.Y);
        }
    ...
    {
        foreach (var pan in _pans.Values)
        {
            pan.Gesture.Remove(time);
        }
