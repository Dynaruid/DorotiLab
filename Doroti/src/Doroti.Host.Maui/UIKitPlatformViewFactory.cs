...
public sealed class UIKitPlatformViewDispatcher : IPlatformViewDispatcher
{
    ...
    internal static void VerifyThread()
    {
        if (!NSThread.IsMain)
        {
            throw new InvalidOperationException("UIKit PlatformView requires the main thread.");
        }
    }
}
...
) : IPlatformViewFactory
{
    ...
    public PlatformViewSupport QuerySupport(PlatformViewRequest request)
    {
        ...
        return new(
            ...
            Capabilities: new(
                ...
                UIKitPlatformViewHost.MaterialEffects
            ),
            Reason: supported
                ? null
    ...
    private sealed class Instance : IPlatformViewInstance
    {
        ...
        private readonly UIView _clip = new()
        {
            ClipsToBounds = true,
            Hidden = true,
        ...
        )
        {
            ...
            if (type == "doroti/native-editor")
            {
                _control = new NativeEditor(BeforeFocus, Focused)
                {
                    ...
                    AccessibilityLabel = "Native editor",
                };
            }
            ...
            _control.AccessibilityIdentifier =
        ...
        {
            if (_disabled || _disposed || _clip.Hidden)
            {
                return;
            }
        ...
        public ValueTask ApplyAsync(PlatformViewPlacement placement)
        {
            ...
            )
            {
                throw new InvalidOperationException(
                    "UIKit requires live owner-local identity, translation and rectangular clipping."
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    "A live UIKit PlatformView cannot move between owners."
                );
            }
        ...
        public ValueTask SetFocusAsync(bool focused)
        {
            ...
            if (focused)
            {
                if (_disabled || _clip.Hidden || _clip.Window is null)
                {
                    throw new InvalidOperationException(
                        "Native focus requires a visible attachment."
                    );
                }

                if (!_control.BecomeFirstResponder())
                {
                    throw new InvalidOperationException("UIKit rejected native focus.");
                }
            }
            else
            {
        ...
        public ValueTask DisposeAsync()
        {
            ...
            DisableInputAsync();
            _disposed = true;
            if (_control is UIButton button)
            {
                button.TouchUpInside -= Activated;
            }

            if (_control is WKWebView web)
            {
                web.StopLoading();
            }
