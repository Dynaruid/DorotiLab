...
internal sealed class MauiUIKitContextMenuChannel : IPlatformMessageHostCapability
{
    ...
    public MauiUIKitContextMenuChannel(
        IPlatformMessageHostCapability fallback,
        MauiTextInputBridge input
    )
    {
        _fallback = fallback;
        _input = input;
        ...
        {
            if (_frameworkHandler is not { } handler)
            {
                return;
            }
            ...
            Dispatch?.Invoke(() =>
            {
                _ = handler(bytes, CancellationToken.None);
            });
        };
    }
    ...
    )
    {
        if (channel == "flutter/platform" && data is { } bytes)
        {
            ...
            {
                if (!_input.SupportsSystemContextMenu)
                {
                    return "[\"unsupported\",\"iOS system menu unavailable\",null]"u8.ToArray();
                }
                ...
                await _input.RunUIKitMenuMutation(() =>
                    _input.ShowSystemContextMenu(target, items)
                );
                return "[null]"u8.ToArray();
            }
        }
...
public sealed partial class MauiTextInputBridge
{
    ...
    internal void ShowSystemContextMenu(CGRect target, IReadOnlyList<UIKitMenuItem> items)
    {
        ...
        if (
            ...
            || _visualHost?.Handler?.PlatformView is not UIView view
            || _active?.Handler?.PlatformView is not UIView input
            || !SupportsSystemContextMenu
        )
        {
            return;
        }
        ...
        _systemMenuView = new UIKitEditMenuView(input)
        {
            Frame = view.Bounds,
        ...
        _systemMenu.PresentEditMenu(
            UIEditMenuConfiguration.Create(null, new CGPoint(target.GetMidX(), target.GetMidY()))
        );
    }
    ...
    {
        if (!ReferenceEquals(_systemMenuDelegate, menu))
        {
            return;
        }
    ...
    private sealed class UIKitEditMenuDelegate(
        ...
        CGRect target,
        IReadOnlyList<UIKitMenuItem> items,
        UIView input
    ) : UIEditMenuInteractionDelegate
    {
        public override CGRect GetTargetRect(
            UIEditMenuInteraction interaction,
            UIEditMenuConfiguration configuration
        ) => target;

        public override UIMenu GetMenu(
            UIEditMenuInteraction interaction,
            UIEditMenuConfiguration configuration,
        ...
        )
        {
            ...
            foreach (var item in items)
            {
                var selector = item.Type switch
                {
                    ...
                    "paste" => "paste:",
                    "selectAll" => "selectAll:",
                    "captureTextFromCamera" => "captureTextFromCamera:",
                    _ => null,
                };
                if (selector is not null)
                {
                    ...
                    {
                        commands.Add(suggested);
                    }
                }
                else
                {
                    commands.Add(
                        UIAction.Create(item.Title ?? item.Type, null, null, _ => Perform(item))
                    );
                }
            }
        ...
        private static UICommand? FindCommand(IEnumerable<UIMenuElement> elements, string selector)
        {
            ...
            {
                if (element is UICommand command && command.Action.Name == selector)
                {
                    return command;
                }

                if (element is UIMenu menu && FindCommand(menu.Children, selector) is { } nested)
                {
                    return nested;
                }
            }
        ...
        private void Perform(UIKitMenuItem item)
        {
            if (
                owner._disposed
                || !owner._hasClient
                || owner.HasPendingClientChange
                || !ReferenceEquals(owner._active?.Handler?.PlatformView, input)
            )
            {
                return;
            }
            ...
            if (item.Type == "custom")
            {
                owner.SystemContextMenuEvent?.Invoke(
            ...
            var selected = text.Substring(
                start,
                Math.Clamp(owner._active.SelectionLength, 0, text.Length - start)
            );
            var presenter = input.Window?.RootViewController;
            while (presenter?.PresentedViewController is { } next)
            {
                presenter = next;
            }

            if (presenter is null || selected.Length == 0)
            {
                return;
            }
            ...
            if (item.Type == "lookUp")
            {
                presenter.PresentViewController(
                    new UIReferenceLibraryViewController(selected),
                    true,
                    null
                );
            }
            else if (item.Type == "share")
            {
                ...
                {
                    popover.SourceView = (UIView)owner._visualHost!.Handler!.PlatformView!;
                    popover.SourceRect = target;
                }
            ...
            else if (item.Type == "searchWeb")
            {
                UIApplication.SharedApplication.OpenUrl(
                    new NSUrl("https://www.google.com/search?q=" + Uri.EscapeDataString(selected)),
        ...
        public override void WillDismissMenu(
            UIEditMenuInteraction interaction,
            UIEditMenuConfiguration configuration,
