// Microsoft.UI.Composition: live WinUI VisualSurface sampling and raster inputs.
#include <windows.h>
#include <d3d11.h>
#include <d2d1_1.h>
...
extern "C" __declspec(dllexport) int32_t __cdecl doroti_windows_winui_effect_v1(
  ...
  *brush = nullptr;
  try {
    ...
    // The caller binds an explicit live VisualSurface prefix, excluding foreground content.
    *brush = reinterpret_cast<IUnknown*>(detach_abi(result));
    return S_OK;
  } catch (...) { return to_hresult(); }
}

extern "C" __declspec(dllexport) int32_t __cdecl doroti_windows_winui_graphics_create_v1(
...
extern "C" __declspec(dllexport) int32_t __cdecl doroti_windows_winui_surface_v1(
