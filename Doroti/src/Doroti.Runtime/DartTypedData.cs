...
public sealed class ByteBuffer
{
    ...
    public List<long> asUint32List(long offsetInBytes = 0, long? length = null)
    {
        ...
        {
            result.Add(
                BinaryPrimitives.ReadUInt32LittleEndian(Slice(offsetInBytes + (index * 4), 4))
            );
        }
        return result;
    }
...
public sealed class ByteData
{
    public ByteData(Uint8List bytes)
        : this(
            bytes?.buffer ?? throw new ArgumentNullException(nameof(bytes)),
            bytes.offsetInBytes,
            bytes.lengthInBytes
        ) { }
    ...
    public ByteData(ByteBuffer buffer, long offsetInBytes = 0, long? lengthInBytes = null)
    {
        ...
        if (
            ...
            || offsetInBytes + this.lengthInBytes > buffer.lengthInBytes
        )
        {
    ...
    public static implicit operator ByteData(ReadOnlyMemory<byte> value) =>
        new(new Uint8List(value.ToArray()));
}
...
public sealed class Uint8List : IList<long>, IReadOnlyList<long>
{
    ...
    public Uint8List(long length)
        : this(new ByteBuffer(new byte[checked((int)length)]), 0, length) { }
    ...
    public Uint8List(byte[] values)
        : this(new ByteBuffer(values.ToArray()), 0, values.LongLength) { }
    ...
    public Uint8List(ByteData data)
        : this(
            data?.buffer ?? throw new ArgumentNullException(nameof(data)),
            data.offsetInBytes,
            data.lengthInBytes
        ) { }
    ...
    public int IndexOf(long item) =>
        this.Select((value, index) => (value, index))
            .FirstOrDefault(pair => pair.value == item, (0, -1))
            .index;
    ...
    public void Add(long item) =>
        throw new NotSupportedException("A typed-data list has fixed length.");
...
public abstract class DartTypedList<T> : IList<T>, IReadOnlyList<T>
{
    ...
    {
        get => _values[checked((int)index)];
        set => _values[checked((int)index)] = value;
    }
...
{
    public Int32List(long length)
        : base(length) { }

    public Int32List(IEnumerable<int> values)
        : base(values) { }

    public Int32List(IEnumerable<long> values)
        : base(values.Select(value => checked((int)value))) { }
}

public sealed class Int64List : DartTypedList<long>
{
    public Int64List(long length)
        : base(length) { }

    public Int64List(IEnumerable<long> values)
        : base(values) { }
}

public sealed class Float32List : DartTypedList<float>
{
    public Float32List(long length)
        : base(length) { }

    public Float32List(IEnumerable<float> values)
        : base(values) { }
}

public sealed class Float64List : DartTypedList<double>
{
    public Float64List(long length)
        : base(length) { }

    public Float64List(IEnumerable<double> values)
        : base(values) { }
}
...
public sealed class SendPort
{
    ...
    internal void Listen(Action<object?> listener) =>
        _listener = listener ?? throw new ArgumentNullException(nameof(listener));

    public void send(object? message) =>
        (
            _listener
            ?? throw new InvalidOperationException("No ReceivePort listener is registered.")
        )(message);
}
    ...
    where T : class
{
    ...
    public T? this[object key]
    {
        ...
        set
        {
            ...
            {
                _values.Add(key, new Holder(value));
            }
        }
    }
