...
public static class MaterialColorSchemeRuntime
{
    ...
    )
    {
        if (contrastLevel is < -1 or > 1 || !double.IsFinite(contrastLevel))
        {
            throw new ArgumentOutOfRangeException(
                ...
                "Material contrast must be between -1 and 1."
            );
        }

        var schemeKey = new SchemeKey(unchecked((uint)seedArgb), isDark, variant, contrastLevel);
        var colorKey = new ColorKey(schemeKey, role);
        return ColorCache.GetOrAdd(
            colorKey,
            static key =>
            {
                var scheme = SchemeCache.GetOrAdd(
                    key.Scheme,
                    static item =>
                        BuildScheme(
                            Hct.FromInt(item.SeedArgb),
                            item.IsDark,
                            item.Variant,
                            item.ContrastLevel
                        )
    ...
    private static Scheme BuildScheme(Hct source, bool dark, string variant, double contrast)
    {
        ...
        var normalized = variant switch
        {
            ...
            or "expressive"
            or "rainbow"
            or "fruitSalad" => variant,
            _ => throw new NotSupportedException(
                $"Unsupported Material dynamic scheme variant '{variant}'."
            ),
        };

        var palettes = normalized switch
        {
            "tonalSpot" => new Palettes(
                P(hue, 36),
                P(hue, 16),
                P(hue + 60, 24),
                P(hue, 6),
            ...
            "vibrant" => new Palettes(
                P(hue, 200),
                P(RotatedHue(hue, VibrantHues, VibrantSecondaryRotations), 24),
                P(RotatedHue(hue, VibrantHues, VibrantTertiaryRotations), 32),
                P(hue, 10),
                P(hue, 12)
            ),
            "expressive" => new Palettes(
                P(hue + 240, 40),
                P(RotatedHue(hue, ExpressiveHues, ExpressiveSecondaryRotations), 24),
                P(RotatedHue(hue, ExpressiveHues, ExpressiveTertiaryRotations), 32),
                P(hue + 15, 8),
                P(hue + 15, 12)
            ),
            "rainbow" => new Palettes(
                P(hue, 48),
                P(hue, 16),
                P(hue + 60, 24),
                P(hue, 0),
            ...
            "content" => new Palettes(
                P(hue, chroma),
                P(hue, Math.Max(chroma - 32, chroma * 0.5)),
                PaletteFromHct(FixIfDisliked(Analogous(source))),
                P(hue, chroma / 8),
                P(hue, (chroma / 8) + 4)
            ),
            "fidelity" => new Palettes(
                P(hue, chroma),
                P(hue, Math.Max(chroma - 32, chroma * 0.5)),
                PaletteFromHct(FixIfDisliked(Complement(source))),
                P(hue, chroma / 8),
                P(hue, (chroma / 8) + 4)
            ),
    ...
    {
        if (role is "shadow" or "scrim")
        {
            return 0;
        }

        if (role is "surfaceTint")
        {
            return scheme.IsDark ? 80 : 40;
        }

        if (role is "primary" or "primaryContainer")
        {
            return ResolvePair(scheme, role, "primaryContainer", "primary");
        }

        if (role is "secondary" or "secondaryContainer")
        {
            return ResolvePair(scheme, role, "secondaryContainer", "secondary");
        }

        if (role is "tertiary" or "tertiaryContainer")
        {
            return ResolvePair(scheme, role, "tertiaryContainer", "tertiary");
        }
        ...
        var answer =
            RatioOfTones(backgroundTone, initial) >= ratio && scheme.Contrast >= 0
                ? initial
    ...
    private static double ResolvePair(
        Scheme scheme,
    ...
    )
    {
        ...
        var nearer =
            !decreasing && RatioOfTones(backgroundTone, nearerInitial) >= nearerRatio
                ? nearerInitial
                : ForegroundTone(backgroundTone, nearerRatio);
        var farther =
            !decreasing && RatioOfTones(backgroundTone, fartherInitial) >= fartherRatio
                ? fartherInitial
                : ForegroundTone(backgroundTone, fartherRatio);
        ...
        if (nearer is >= 50 and < 60)
        {
            ...
            farther =
                direction > 0 ? Math.Max(farther, nearer + 10) : Math.Min(farther, nearer - 10);
        }
    ...
    private static double InitialTone(Scheme scheme, string role)
    {
        ...
        return role switch
        {
            ...
            "surfaceContainerLow" => dark
                ? Curve(scheme.Contrast, 10, 10, 11, 12)
                : Curve(scheme.Contrast, 96, 96, 96, 95),
            "surfaceContainer" => dark
                ? Curve(scheme.Contrast, 12, 12, 16, 20)
                : Curve(scheme.Contrast, 94, 94, 92, 90),
            "surfaceContainerHigh" => dark
                ? Curve(scheme.Contrast, 17, 17, 21, 25)
                : Curve(scheme.Contrast, 92, 92, 88, 85),
            "surfaceContainerHighest" => dark
                ? Curve(scheme.Contrast, 22, 22, 26, 30)
                : Curve(scheme.Contrast, 90, 90, 84, 80),
            ...
            "primaryContainer" => monochrome
                ? (dark ? 85 : 25)
                : (
                    scheme.Variant is "fidelity" or "content" ? scheme.Source.Tone
                    : dark ? 30
                    : 90
                ),
            "onPrimaryContainer" => monochrome ? (dark ? 0 : 100)
            : IsFidelity(scheme) ? ForegroundTone(InitialTone(scheme, "primaryContainer"), 4.5)
            : (dark ? 90 : 30),
            "inversePrimary" => dark ? 40 : 80,
            "secondary" => dark ? 80 : 40,
            "onSecondary" => monochrome ? (dark ? 10 : 100) : (dark ? 20 : 100),
            "secondaryContainer" => monochrome ? (dark ? 30 : 85)
            : IsFidelity(scheme)
                ? FindDesiredChromaByTone(scheme.Palettes.Secondary, dark ? 30 : 90, !dark)
            : (dark ? 30 : 90),
            "onSecondaryContainer" => monochrome ? (dark ? 90 : 10)
            : IsFidelity(scheme) ? ForegroundTone(InitialTone(scheme, "secondaryContainer"), 4.5)
            : (dark ? 90 : 30),
            "tertiary" => monochrome ? (dark ? 90 : 25) : (dark ? 80 : 40),
            "onTertiary" => monochrome ? (dark ? 10 : 90) : (dark ? 20 : 100),
            "tertiaryContainer" => monochrome ? (dark ? 60 : 49)
            : IsFidelity(scheme)
                ? FixIfDisliked(
                    Hct.From(
                        scheme.Palettes.Tertiary.Hue,
                        scheme.Palettes.Tertiary.Chroma,
                        scheme.Source.Tone
                    )
                ).Tone
            : (dark ? 30 : 90),
            "onTertiaryContainer" => monochrome ? (dark ? 0 : 100)
            : IsFidelity(scheme) ? ForegroundTone(InitialTone(scheme, "tertiaryContainer"), 4.5)
            : (dark ? 90 : 30),
            "error" => dark ? 80 : 40,
            "onError" => dark ? 20 : 100,
    ...
    private static Palette PaletteForRole(Scheme scheme, string role) =>
        role switch
        {
            "primary"
            or "onPrimary"
            ...
            or "primaryFixedDim"
            or "onPrimaryFixed"
            or "onPrimaryFixedVariant"
            or "surfaceTint" => scheme.Palettes.Primary,
            "secondary"
            or "onSecondary"
            or "secondaryContainer"
            or "onSecondaryContainer"
            or "secondaryFixed"
            or "secondaryFixedDim"
            or "onSecondaryFixed"
            or "onSecondaryFixedVariant" => scheme.Palettes.Secondary,
            "tertiary"
            or "onTertiary"
            or "tertiaryContainer"
            or "onTertiaryContainer"
            or "tertiaryFixed"
            or "tertiaryFixedDim"
            or "onTertiaryFixed"
            or "onTertiaryFixedVariant" => scheme.Palettes.Tertiary,
            "error" or "onError" or "errorContainer" or "onErrorContainer" => new(25, 84),
            "surfaceVariant" or "onSurfaceVariant" or "outline" or "outlineVariant" => scheme
                .Palettes
                .NeutralVariant,
            _ => scheme.Palettes.Neutral,
        };

    private static string? BackgroundRole(string role, bool dark) =>
        role switch
        {
            ...
            or "tertiaryContainer"
            or "error"
            or "errorContainer" => dark ? "surfaceBright" : "surfaceDim",
            "onPrimary" => "primary",
    ...
    private static double ContrastFor(string role, double contrast) =>
        role switch
        {
            "primary" or "secondary" or "tertiary" or "error" or "inversePrimary" => Curve(
                contrast,
                3,
                4.5,
                7,
            ...
            or "tertiaryContainer"
            or "errorContainer"
            or "outlineVariant" => Curve(contrast, 1, 1, 3, 4.5),
            "onPrimary"
            or "onSecondary"
            or "onTertiary"
            or "onError"
            or "onSurface"
            or "inverseOnSurface" => Curve(contrast, 4.5, 7, 11, 21),
            "onPrimaryContainer"
            or "onSecondaryContainer"
            or "onTertiaryContainer"
            or "onErrorContainer"
            or "onSurfaceVariant" => Curve(contrast, 3, 4.5, 7, 11),
    ...
    private static double ForegroundTone(double backgroundTone, double ratio)
    {
        ...
        if (preferLighter)
        {
            var negligible =
                ...
                && lighterRatio < ratio
                && darkerRatio < ratio;
            return lighterRatio >= ratio || lighterRatio >= darkerRatio || negligible
                ? lighter
                : darker;
        }
    ...
    private static double YFromLstar(double lstar) =>
        lstar > 8 ? 100 * Math.Pow((lstar + 16) / 116, 3) : 100 * lstar / 903.2962962962963;

    private static double LstarFromY(double y)
    {
        var normalized = y / 100;
        var f =
            normalized > 216d / 24389d
                ? Math.Pow(normalized, 1d / 3d)
                : ((24389d / 27d * normalized) + 16) / 116;
        return (116 * f) - 16;
    }
    ...
    private static double RotatedHue(double sourceHue, double[] hues, double[] rotations)
    {
        ...
        {
            if (sourceHue >= hues[index] && sourceHue < hues[index + 1])
            {
                return Sanitize(sourceHue + rotations[index]);
            }
        }
        return sourceHue;
    }
    ...
    private static double FindDesiredChromaByTone(Palette palette, double tone, bool decreasing)
    {
        ...
        if (closest.Chroma >= palette.Chroma)
        {
            return answer;
        }
        ...
        while (closest.Chroma < palette.Chroma)
        {
            ...
            if (peak > candidate.Chroma)
            {
                break;
            }

            if (Math.Abs(candidate.Chroma - palette.Chroma) < 0.4)
            {
                break;
            }

            if (
                Math.Abs(candidate.Chroma - palette.Chroma)
                < Math.Abs(closest.Chroma - palette.Chroma)
            )
            {
                closest = candidate;
            }
    ...
    private static Hct FixIfDisliked(Hct hct) =>
        Math.Round(hct.Hue) is >= 90 and <= 111
        && Math.Round(hct.Chroma) > 16
        && Math.Round(hct.Tone) < 65
    ...
    private static Hct Complement(Hct input)
    {
        ...
        for (var addend = 0; addend <= 360; addend++)
        {
            ...
            var error = Math.Abs(
                desired - RelativeTemperature(RawTemperature(candidate), temperatures)
            );
            if (error >= bestError)
            {
                continue;
    ...
    private static Hct[] HuesAtSourceTone(Hct input) =>
        Enumerable.Range(0, 360).Select(hue => Hct.From(hue, input.Chroma, input.Tone)).ToArray();

    private static double RelativeTemperature(
        double temperature,
    ...
    private static double RawTemperature(Hct color)
    {
        ...
        return -0.5
            + (0.02 * Math.Pow(chroma, 1.07) * Math.Cos(Sanitize(hue - 50) * Math.PI / 180));
    }
    ...
    private static readonly double[] VibrantSecondaryRotations =
    [
        18,
        15,
        10,
    ...
    private static readonly double[] VibrantTertiaryRotations =
    [
        35,
        30,
        20,
    ...
    private static readonly double[] ExpressiveSecondaryRotations =
    [
        45,
        95,
        45,
    ...
    private static readonly double[] ExpressiveTertiaryRotations =
    [
        120,
        120,
        20,
    ...
    private sealed record Palettes(
        Palette Primary,
        Palette Secondary,
        Palette Tertiary,
        Palette Neutral,
    ...
    private readonly record struct SchemeKey(
        uint SeedArgb,
        bool IsDark,
        string Variant,
        double ContrastLevel
