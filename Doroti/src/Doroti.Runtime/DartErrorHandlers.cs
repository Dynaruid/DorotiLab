...
{
    public static Func<Exception, StackTrace, object?> Adapt<TError, TResult>(
        Func<TError, TResult> callback
    )
    ...
    public static Func<Exception, StackTrace, object?> Adapt<TError, TResult>(
        Func<TError, StackTrace, TResult> callback
    )
        where TError : Exception
    {
        ArgumentNullException.ThrowIfNull(callback);
    ...
    public static Func<Exception, StackTrace, Task<object?>> AdaptTask<TError, TResult>(
        Func<TError, Task<TResult>> callback
    )
        where TError : Exception
    {
        ArgumentNullException.ThrowIfNull(callback);
    ...
    private static object InvokeAction(
        Action<Exception, StackTrace> callback,
        Exception error,
        StackTrace stack
    )
    {
    ...
    internal static async Task<T> Recover<T>(Delegate handler, Exception error)
    {
        ...
        if (ReferenceEquals(result, Void))
        {
            return default!; // Explicit legacy Action recovery contract.
        }

        if (result is Future future)
        {
            result = await future.asObjectTask().ConfigureAwait(false);
        }
        else if (result is Task<T> task)
        {
            return await task.ConfigureAwait(false);
        }
        else if (result is Task<object?> objectTask)
        {
            result = await objectTask.ConfigureAwait(false);
        }
        ...
        throw new InvalidCastException(
            $"The Future error handler did not return {typeof(T)} or an awaitable of that type."
        );
    }
