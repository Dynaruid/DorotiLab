...
public readonly record struct Duration(long microseconds) : IComparable<Duration>
{
    public Duration(double seconds)
        : this(checked((long)(seconds * 1_000_000d))) { }
    ...
    public static Duration operator +(Duration left, Duration right) =>
        new(left.inMicroseconds + right.inMicroseconds);
...
public sealed class StringBuffer
{
    ...
    public StringBuffer(object? content = null)
    {
    ...
    public void writeCharCode(long value) =>
        _builder.Append(char.ConvertFromUtf32(checked((int)value)));
...
public static class DartCoreExtensions
{
    ...
    public static DateTime subtract(this DateTime value, Duration duration) =>
        value.Subtract(duration);

    public static Duration difference(this DateTime value, DateTime other) => value - other;

    public static long ToDartWeekday(this DayOfWeek value) =>
        value == DayOfWeek.Sunday ? 7 : (long)value;
    ...
    public static List<T> toList<T>(this IEnumerable<T> values, bool growable = true) =>
        values.ToList();

    public static T? weakTarget<T>(WeakReference<T>? reference)
        where T : class =>
    ...
    public static double roundToDouble(this double value) =>
    ...
    public static string toRadixString(this long value, long radix)
    {
        ...
        while (remaining != 0)
        {
            buffer.Push(digits[(int)(remaining % (ulong)radix)]);
    ...
    public static string substring(this string value, long start, long? end = null) =>
            ...
            ? value[checked((int)start)..]
            : value[checked((int)start)..checked((int)end.Value)];

    public static bool contains(this string value, string other) =>
    ...
    public static T elementAt<T>(this IEnumerable<T> values, long index) =>
        values.ElementAt(checked((int)index));
    ...
    public static IEnumerable<long> runes(this string value) =>
        value.EnumerateRunes().Select(rune => (long)rune.Value);
    ...
    public static List<string> split(this string value, string pattern) =>
        [.. value.Split(pattern, StringSplitOptions.None)];
    ...
    public static IEnumerable<TResult> map<TKey, TValue, TResult>(
        this IReadOnlyDictionary<TKey, TValue> values,
        Func<TKey, TValue, TResult> transform
    ) => values.Select(pair => transform(pair.Key, pair.Value));

    public static DartMap<TResultKey, TResultValue> map<TKey, TValue, TResultKey, TResultValue>(
        this IReadOnlyDictionary<TKey, TValue> values,
        Func<TKey, TValue, MapEntry<TResultKey, TResultValue>> transform
    ) =>
        new(
            values
                .Select(pair => transform(pair.Key, pair.Value))
                .Select(entry => new KeyValuePair<TResultKey, TResultValue>(entry.key, entry.value))
        );
    ...
    ) => values.Aggregate(initial, combine);

    public static IEnumerable<T> reversed<T>(this IEnumerable<T> values) => values.Reverse();

    public static T reduce<T>(this IEnumerable<T> values, Func<T, T, T> combine) =>
        values.Aggregate(combine);

    public static IEnumerable<TResult> expand<T, TResult>(
        this IEnumerable<T> values,
        Func<T, IEnumerable<TResult>> transform
    ) => values.SelectMany(transform);

    public static IEnumerable<T> where<T>(this IEnumerable<T> values, Func<T, bool> predicate) =>
    ...
    public static IEnumerable<T> skip<T>(this IEnumerable<T> values, long count) =>
        values.Skip(checked((int)count));

    public static IEnumerable<T> take<T>(this IEnumerable<T> values, long count) =>
    ...
    public static IEnumerable<TResult> cast<TResult>(this IEnumerable values) =>
        values.Cast<TResult>();

    public static HashSet<T> toSet<T>(this IEnumerable<T> values) => new(values);

    public static IEnumerable<T> followedBy<T>(this IEnumerable<T> values, IEnumerable<T> other) =>
        values.Concat(other);
    ...
    )
    {
        ...
        return found ? result
            : orElse is null ? throw new InvalidOperationException("No element")
            : orElse();
    }
    ...
    public static long indexWhere<T>(
        this IEnumerable<T> values,
        Func<T, bool> predicate,
        long start = 0
    )
    {
        ...
        foreach (var value in values)
        {
            if (index >= start && predicate(value))
            {
                return index;
            }
    ...
    private static bool InvokePredicate<T>(Delegate predicate, T value) =>
        predicate switch
        {
            ...
            _ => throw new ArgumentException(
                "Use Func<T, bool> or Predicate<T>; adapt custom delegates with a typed lambda.",
                nameof(predicate)
            ),
        };

    public static T _merge<T>(this T value, T other)
        where T : struct, Enum =>
        Convert.ToInt64(value, CultureInfo.InvariantCulture)
        >= Convert.ToInt64(other, CultureInfo.InvariantCulture)
            ? value
            : other;
    ...
    public static int lastIndexWhere<T>(
        this List<T> values,
        Func<T, bool> predicate,
        int? start = null
    )
    {
        for (
            var index = Math.Min(start ?? (values.Count - 1), values.Count - 1);
            index >= 0;
            index--
        )
        {
    ...
    public static TKey? firstKeyAfter<TKey, TValue>(
        this SortedDictionary<TKey, TValue> values,
        TKey key
    )
        where TKey : notnull =>
        values.Keys.FirstOrDefault(candidate => Comparer<TKey>.Default.Compare(candidate, key) > 0);

    public static TKey? lastKeyBefore<TKey, TValue>(
        this SortedDictionary<TKey, TValue> values,
        TKey key
    )
        where TKey : notnull =>
    ...
    public static TKey? firstKey<TKey, TValue>(this SortedDictionary<TKey, TValue> values)
        where TKey : notnull => values.Count == 0 ? default : values.Keys.First();

    public static TKey? lastKey<TKey, TValue>(this SortedDictionary<TKey, TValue> values)
        where TKey : notnull => values.Count == 0 ? default : values.Keys.Last();
...
public static class Dart_coreLibrary
{
    ...
    public static int identityHashCode(object? value) =>
        value is null ? 0 : System.Runtime.CompilerServices.RuntimeHelpers.GetHashCode(value);
...
public sealed class Characters : IReadOnlyCollection<string>
{
    ...
    public Characters(string value) =>
        _elements = value.EnumerateRunes().Select(rune => rune.ToString()).ToArray();
...
public sealed class CharacterRange
{
    ...
    public CharacterRange(string value)
        : this(value.EnumerateRunes().Select(rune => rune.ToString()).ToArray()) { }
    ...
    public bool moveNext(long count = 1)
    {
        _start = _end;
        _end = Math.Min(elements.Length, _end + checked((int)count));
        return _start < elements.Length;
    }

    public bool MoveNext() => moveNext();

    public bool moveBack(long count = 1)
    {
        _end = _start;
        _start = Math.Max(0, _start - checked((int)count));
        return _end > 0;
    }

    public void expandNext(long count = 1) =>
        _end = Math.Min(elements.Length, _end + checked((int)count));
