...
{
    public static IReadOnlyDictionary<long, long> Quantize(
        IEnumerable<long> argbPixels,
        int maxColors = 128
    )
    {
    ...
    private static IReadOnlyDictionary<long, long> QuantizeWsmeans(uint[] pixels, int maxColors)
    {
        ...
        {
            countsByColor[pixel] = countsByColor.GetValueOrDefault(pixel) + 1;
        }
        ...
        var clusters = new MaterialImageQuantizerWu()
            .Quantize(pixels, (uint)maxColors)
            .ColorToCount.Keys.Select(provider.FromInt)
            .ToList();
        ...
        for (var iteration = 0; iteration < 5; iteration++)
        {
            ...
            for (var i = 0; i < points.Length; i++)
            {
                ...
                {
                    if (distances[previous][j] >= 4 * previousDistance)
                    {
                        continue;
                    }
            ...
            for (var i = 0; i < points.Length; i++)
            {
                ...
                {
                    sums[cluster][component] += points[i][component] * counts[i];
                }
            }
            for (var i = 0; i < count; i++)
            {
                clusters[i] =
                    populations[i] == 0
                        ? [0, 0, 0]
                        : sums[i].Select(value => value / populations[i]).ToArray();
            }
        }
        ...
        for (var i = 0; i < count; i++)
        {
            if (populations[i] > 0)
            {
                result.TryAdd(provider.ToInt(clusters[i]), populations[i]);
            }
        }
    ...
    public static IReadOnlyList<long> Score(IReadOnlyDictionary<long, long> colors, int desired = 1)
    {
        ...
        var entries = colors
            ...
            .Select(entry =>
                (Color: Hct.FromInt(unchecked((uint)entry.Key)), Population: entry.Value)
            )
            .ToArray();
        ...
        {
            population[(int)Math.Floor(entry.Color.Hue)] += entry.Population;
        }
        ...
        for (var separation = 90; separation >= 15; separation--)
        {
            ...
            foreach (var entry in ranked)
            {
                if (
                    chosen.Any(color =>
                        180 - Math.Abs(Math.Abs(color.Hue - entry.Color.Hue) - 180) < separation
                    )
                )
                {
                    continue;
        ...
        return chosen.Count == 0
            ? [0xff4285f4L]
            : chosen.Select(color => (long)color.ToInt()).ToArray();
    }
    ...
    public static long[] ArgbFromRgba(ReadOnlySpan<byte> bytes)
    {
        if (bytes.Length % 4 != 0)
        {
            throw new ArgumentException("RGBA input must contain complete pixels.", nameof(bytes));
        }
