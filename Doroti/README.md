...
`DOROTI_WINDOWS_PRESENTER=Vulkan` selects an experimental Vulkan/Skia presenter; ANGLE remains the default and there is no silent fallback. The product requires System32 Vulkan 1.1, a matching adapter LUID, and dedicated importable D3D11-texture external memory. It presents through Windows Presentation with zero Vulkan WSI swapchains. SkiaSharp packages are pinned to `4.152.0-rc.1.26426.14`, and the Windows host uses `SkiaSharp.Vulkan.Silk.NET` with `GRSilkNetBackendContext`. A native topmost DirectComposition target on the top-level HWND owns the full-capacity identity source, and the top-level client is the only visible clip. Moving-origin `Left`/`Top` resize prepares an exact raster and copied buffer without exposing it. After a bounded compositor-clock wait in `WM_WINDOWPOSCHANGING`, USER32 applies geometry; `WM_WINDOWPOSCHANGED` immediately submits the reserved buffer on the platform thread and waits up to 50 ms for its matching CompositionFrame receipt. Matching `WM_SIZE` acknowledges that commit. Fixed-origin `Right`/`Bottom` keeps its pre-geometry submission and DWM wait. Programmatic or mismatched geometry uses a bounded exact-render fallback, and the separate 100 ms terminal bound is only a fail-safe. On Windows 11 24H2+, Vulkan may be combined with `experimentalAcrylic`; a host-backdrop-enabled non-topmost `DesktopWindowTarget` owns an active `DesktopAcrylicController`, an HWND-wide transient system backdrop remains below the content, and the topmost Vulkan overlay is premultiplied. The controller accepts the shared kind/theme/tint/luminosity option channel. Automated WGC/current-monitor qualification is available through `validate-windows-vulkan-live-resize.ps1`; its strict app-raster pixel gate is not reclassified when material underlay is visible. The preceding standard-HWND candidates failed direct physical testing. The current clock/geometry/receipt candidate passed 16 focused automatic resize cases on AMD at 96 DPI/165 Hz, but moving-origin throughput measured only 52.5–59.58 presentations/s. This cost comes from synchronization, with no fixed FPS limiter. On 2026-09-05, the user confirmed that window resizing now works well in the observed Vulkan+AMD+Acrylic environment. This is PASS-observed for that resize issue; other environment combinations remain unverified. The app-owned fixed-envelope/custom-non-client alternative is not being adopted, and Vulkan remains experimental opt-in. The completed problem/work documents are summarized in the [September 5 resize history](../history/26-09-05/windows-vulkan-acrylic-resize-summary.md), including earlier failures and the separate initial-FPS/R2R observations.
