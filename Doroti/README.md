...
`DOROTI_WINDOWS_PRESENTER=Vulkan` selects an experimental, separate direct Vulkan/Skia presenter. It requires System32 Vulkan 1.1 plus `VK_KHR_surface`, `VK_KHR_win32_surface`, and `VK_KHR_swapchain`; swapchain-maintenance is not required. Stale work is rejected before acquire, and every successful acquire commits to copy/present. Set `DOROTI_WINDOWS_VULKAN_DEVICE` when more than one capable GPU is eligible or deterministic adapter selection is required. It has no automatic fallback and conflicts with `experimentalAcrylic`. ANGLE remains the default. Automated WGC/current-monitor resize qualification is available through `validate-windows-vulkan-live-resize.ps1`, but physical resize/scan-out acceptance is still `notVerified`.

The Material demo opts in only when `DOROTI_DEMO_EXPERIMENTAL_ACRYLIC=1`. Runtime kind/theme/tint updates use the `doroti/windows/experimental-acrylic` platform channel; query it with an empty payload to read effective mode, adapter LUID, budgets, and option counters. The dedicated validator is `pwsh -File ./Doroti/eng/validate-windows-experimental-acrylic.ps1`. Current 200%-DPI/165-Hz TopLeft three-second geometry and 600 ms responsiveness cases report `PASS-automated-partial`, but the full edge/DPI/refresh three-run matrix and physical validation are incomplete, so this is not stable qualification.
