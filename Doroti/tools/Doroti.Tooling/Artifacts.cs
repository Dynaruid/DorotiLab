...
public static class ProcessRunner
{
    ...
    private static string ResolveExecutable(string fileName)
    {
        ...
        foreach (
            var directory in (Environment.GetEnvironmentVariable("PATH") ?? string.Empty).Split(
                Path.PathSeparator,
                StringSplitOptions.RemoveEmptyEntries
            )
        )
        {
            foreach (var extension in extensions)
            {
                var candidate = Path.Combine(
                    directory.Trim('"'),
                    fileName + extension.ToLowerInvariant()
                );
...
public static class RepositoryLocalStorage
{
    ...
    public static string ResolveRoot(string dorotiRoot)
    {
        ...
        var workspaceRoot = Directory.Exists(
            Path.Combine(Path.GetDirectoryName(resolvedDorotiRoot)!, "tools", "Doroti.DartToCSharp")
        )
            ? Path.GetDirectoryName(resolvedDorotiRoot)!
            : resolvedDorotiRoot;
        var configured = Environment.GetEnvironmentVariable(EnvironmentVariable);
        var root =
            string.IsNullOrWhiteSpace(configured) ? Path.Combine(workspaceRoot, ".doroti")
    ...
    public static string CreateTemporaryDirectory(string dorotiRoot, string name)
    {
        ...
        )
        {
            throw new ArgumentException(
                $"Invalid Doroti temporary directory name: {name}",
                nameof(name)
            );
        }
    ...
    {
        var temporaryRoot =
            Path.GetFullPath(GetTemporaryRoot(dorotiRoot)).TrimEnd(Path.DirectorySeparatorChar)
            + Path.DirectorySeparatorChar;
        var resolved = Path.GetFullPath(path);
        if (!resolved.StartsWith(temporaryRoot, StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                $"Refusing to clean a path outside the Doroti temporary root: {resolved}"
            );
        }
        for (var attempt = 1; attempt <= 5; attempt++)
        {
            try
            {
                ...
                when (attempt < 5 && exception is IOException or UnauthorizedAccessException)
            {
                Thread.Sleep(200);
            }
        }
