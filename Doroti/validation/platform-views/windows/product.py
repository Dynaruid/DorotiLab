"""Windows product HWND/GPU composition gate. Run via record.py / 1200s wrapper."""
import ctypes as c
...
OUT = Path(os.environ.get('DOROTI_PLATFORM_VIEW_GATE_OUTPUT') or
    ROOT / 'Doroti/artifacts/validation/platform-views/windows-product' / datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
OUT.mkdir(parents=True, exist_ok=True)
EXE = ROOT / 'DorotiTestbedApp/windowsappsdk/bin/Debug/net10.0-windows10.0.19041.0/win-x64/DorotiTestbedApp.WindowsAppSdk.exe'
...
def click(hwnd, x, y, scale, native_hit_test=True):
    if native_hit_test:
        ...
        u.SetCursorPos(origin.x + round(x * scale), origin.y + round(y * scale))
        u.mouse_event(2, 0, 0, 0, 0x444f5250); time.sleep(.025); u.mouse_event(4, 0, 0, 0, 0x444f5250)
    else:
        position = round(x * scale) | (round(y * scale) << 16)
        u.SendMessageW(hwnd, 0x200, 0, position)
        u.SendMessageW(hwnd, 0x201, 1, position)
        u.SendMessageW(hwnd, 0x202, 0, position)
...
def main():
    ...
    with (OUT / 'product.log').open('w', encoding='utf-8') as log:
        ...
        try:
            ...
            if os.environ.get('DOROTI_PROBE_REDIRECTION') == '1':
                u.SetWindowLongPtrW(hwnd, -20, u.GetWindowLongPtrW(hwnd, -20) & ~0x200000)
            ...
            origin = w.POINT(); u.ClientToScreen(hwnd, c.byref(origin))
            scale = frame['dpi'] / 96
            pixel = image.getpixel((origin.x - rect.left + round(150 * scale), origin.y - rect.top + round(158 * scale)))[:3]
            ...
            def current_stage(stage):
                ...
                return value if value and probe(value)[0] == stage else None
            def taps():
                return (read(OUT / 'frame.json.input.json') or {}).get('scriptPointerDowns', 0)
            ...
            result = dict(productLive='passed', scenarios=samples, nativePointerDowns=taps(), physical='notVerified',
                ime='notVerified', modalBarrier='passed', productCreateDisposeCycles=20,
            ...
            try: code = process.wait(timeout=20)
                ...
                process.kill()
                process.wait()
                raise RuntimeError('Product close did not complete within 20 seconds')
