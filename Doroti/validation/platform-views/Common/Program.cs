...
await Check(
    "host receipt tracks attachment and rejects late resurrection",
    async () =>
    {
        ...
        Assert(
            owner.RecordPlacementReceipt(first)
                && owner.GetState(handle) == PlatformViewState.Attached
        );
        using var hidden = PlatformCompositionPlanner.Build(
            Array.Empty<SceneCommand>(),
            new(13, 0, 2, 0),
            owner
        );
        Assert(
            owner.RecordPlacementReceipt(hidden)
                && owner.GetState(handle) == PlatformViewState.Hidden
        );
        Assert(
            !owner.RecordPlacementReceipt(first)
                && owner.GetState(handle) == PlatformViewState.Hidden
...
await Check(
    "owner isolation and stale generation",
    async () =>
    {
        await using var a = Owner(1);
        await using var b = Owner(2);
        ...
        await a.DisposeAsync(x);
        Assert(a.Resolve(1) == next);
    }
);
await Check(
    "pure analysis holds no native resource",
    async () =>
    {
        ...
        using var plan = PlatformCompositionPlanner.Analyze(
            scene.Commands,
            new(3, 0, 1, 0),
            owner.CaptureSnapshot(PlatformViewComposition.InterleavedComposition)
        );
...
await Check(
    "admission retains until retirement",
    async () =>
    {
...
await Check(
    "R/N/R/N/R order and snapshot immutability",
    async () =>
    {
        await using var owner = Owner(5);
        var a = await owner.CreateAsync(
            new PlatformViewRequest(1, "test", PlatformViewComposition.InterleavedComposition)
        );
        var b = await owner.CreateAsync(
            new PlatformViewRequest(2, "test", PlatformViewComposition.InterleavedComposition)
        );
        ...
        Assert(
            string.Join("", plan.Parts.Select(p => p is PlatformNativeSegment ? "N" : "R"))
                == "RNRNR"
        );
        await owner.DisposeAsync(b);
...
await Check(
    "unknown foreground never silently lowered",
    async () =>
    {
        ...
        await Reject(() =>
        {
            using var plan = PlatformCompositionPlanner.Analyze(
                commands,
                new(6, 0, 1, 0),
                owner.CaptureSnapshot(PlatformViewComposition.NativeOverlay)
            );
            return Task.CompletedTask;
        });
    }
...
await Check(
    "session rejects old frame and releases failed plans",
    async () =>
    {
        ...
        await Reject(() => session.SubmitAsync(stale).AsTask());
        Assert(stale.IsDisposed);
        presenter.Fail = true;
        var failed = PlatformCompositionPlanner.Build(scene, new(7, 0, 3, 0), owner);
        await Reject(() => session.SubmitAsync(failed).AsTask());
        Assert(failed.IsDisposed);
        Assert(session.LastCommit!.Token.FrameNumber == 2);
    }
);
await Check(
    "post-commit cancellation preserves retirement lease",
    async () =>
    {
        ...
        await session.SubmitAsync(plan, cancellation.Token);
        cancellation.Cancel();
...
await Check(
    "late native creation reclaimed on cancellation",
    async () =>
    {
        ...
        await using var owner = new PlatformViewCoordinator(
            9,
            "fake",
            new([factory]),
            new Dispatcher()
        ...
        using var cancellation = new CancellationTokenSource();
        var creation = owner
            .CreateAsync(new PlatformViewRequest(1, "test"), cancellation.Token)
            .AsTask();
        cancellation.Cancel();
        await Reject(() => creation);
        var instance = new Instance();
        factory.Pending.SetResult(instance);
        await owner.DisposeAsync();
        Assert(instance.Disposed);
    }
);
await Check(
    "failed commit still waits for submitted GPU retirement",
    async () =>
    {
        ...
        var retirement = new TaskCompletionSource(
        ...
        var removed = owner.DisposeAsync(handle).AsTask();
        Assert(!removed.IsCompleted);
        retirement.SetResult();
        await removed.WaitAsync(TimeSpan.FromSeconds(2));
        Assert(plan.IsDisposed);
    }
);
await Check(
    "client disposal includes late factory recovery",
    async () =>
    {
        var factory = new Factory
        {
...
await Check(
    "native effect intent survives retained scene planning",
    async () =>
    {
        ...
        foreach (
            var match in new[]
            {
                PlatformEffectMatchPolicy.MatchCommon,
...
await Check(
    "native saturation is negotiated and supports zero-radius color adjustment",
    async () =>
    {
        await using var owner = Owner(13);
        var handle = await owner.CreateAsync(
        ...
        foreach (var sigma in new[] { 0.0, 8.0 })
        {
            ...
            builder.pushBackdropFilter(
                new ImageFilter(sigma, sigma)
                {
                    PlatformEffectIntent = new(
            ...
            await Reject(() =>
            {
                using var rejected = PlatformCompositionPlanner.Build(
                    scene,
                    new(13, 0, 1, 0),
                    owner,
                    effects: new(true, 1, 64)
                );
                return Task.CompletedTask;
            });
            using var plan = PlatformCompositionPlanner.Build(
...
static PlatformViewCoordinator Owner(ulong id) =>
    new(id, "fake", new([new Factory()]), new Dispatcher());
...
sealed class Factory : IPlatformViewFactory
{
    ...
    public ValueTask<IPlatformViewInstance> CreateAsync(
        ...
        Pending is { } pending
            ? new(pending.Task)
            : ValueTask.FromResult<IPlatformViewInstance>(new Instance());
}
