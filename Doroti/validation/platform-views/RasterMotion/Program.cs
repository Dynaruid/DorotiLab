...
Check(
    "changed viewport clipping redraws",
    !Same(start, Slice(picture, 70, 90, viewportRight: 100))
);
...
Check(
    "resized bitmap redraws",
    !Same(start, Slice(picture, 70, 90) with { Bounds = new SKRectI(70, 90, 151, 150) })
);
Check(
    "stationary background cannot follow moving crop",
    !Same(start, start with { Bounds = new SKRectI(70, 90, 150, 150) })
);
...
Check(
    "coincident small foreground pictures share one bitmap",
...
emptyCanvas.save();
emptyCanvas.translate(20, 30);
emptyCanvas.clipRect(Rect.fromLTWH(0, 0, 80, 60));
emptyCanvas.restore();
using var emptyPicture = emptyRecorder.endRecording();
var emptyBuilder = new SceneBuilder();
emptyBuilder.addPicture(Offset.zero, emptyPicture);
using var emptyScene = emptyBuilder.build();
Check(
    "state-only picture has no pixels",
    !SkiaPlatformRasterContent.HasDrawing(emptyScene.Commands)
);
Check(
    "state-only picture allocates no bitmap",
    SkiaPlatformRasterContent.Split(emptyScene.Commands, 1000, 1000).Count == 0
);
var effectRecorder = new PictureRecorder();
var effectCanvas = new Canvas(effectRecorder);
effectCanvas.saveLayer(null, new Paint { blendMode = BlendMode.src });
effectCanvas.restore();
using var effectPicture = effectRecorder.endRecording();
var effectBuilder = new SceneBuilder();
effectBuilder.addPicture(Offset.zero, effectPicture);
using var effectScene = effectBuilder.build();
Check(
    "empty layer with destructive blend is retained",
    SkiaPlatformRasterContent.HasDrawing(effectScene.Commands)
);
var sample = new SKRectI(0, 0, 100, 80);
var pieces = SkiaPlatformRasterContent.SubtractOpaque(sample, new(10, 20, 90, 70));
Check(
    "opaque native interior preserves all kernel edge strips",
    pieces.Count == 4
        && pieces.Sum(r => r.Width * r.Height) == 100 * 80 - 80 * 50
        && pieces.All(r => r.Left >= 0 && r.Top >= 0 && r.Right <= 100 && r.Bottom <= 80)
);
Check(
    "opaque native coverage eliminates hidden background capture",
    SkiaPlatformRasterContent.SubtractOpaque(sample, new(-1, -1, 101, 81)).Count == 0
);
Check(
    "disjoint native content preserves full background capture",
    SkiaPlatformRasterContent.SubtractOpaque(sample, new(110, 0, 200, 80)).SequenceEqual([sample])
);
Check(
    "kernel pieces do not overlap",
    pieces
        .SelectMany(
            (a, i) =>
                pieces
...
Check(
    "same resource domain permits translated reuse",
    SkiaPlatformRasterContent.CanReuse(scope, start, scope, Slice(picture, 70, 90))
);
Check(
    "HWND path can explicitly forbid translated reuse",
...
Check(
    "new frame number does not invalidate pixels",
    scope
        == SkiaPlatformRasterContent.CacheScope.From(
            new(1, 2, 99, 4, 2, 2),
            1000,
            800,
            SKColors.Transparent,
...
foreach (
    ...
    Check(
        "changed owner/epoch/extent/scale/background/resource invalidates reuse",
        !SkiaPlatformRasterContent.CanReuse(scope, start, changedScope, start)
    );
Check(
    "coverage retains enforced clip and pixel halo",
    SkiaPlatformRasterContent.Coverage(layeredScene.Commands, 1000, 1000)
        == new SKRectI(18, 28, 102, 92)
);
Check(
    "empty content has no coverage",
    SkiaPlatformRasterContent.Coverage(emptyScene.Commands, 1000, 1000).IsEmpty
);
...
Check(
    "panel and stationary navigation remain separate slices",
    splitStart.Count == 2 && splitEnd.Count == 2
);
Check(
    "moving panel and stationary navigation independently reuse pixels",
    splitStart
        .Zip(splitEnd)
        .All(pair => SkiaPlatformRasterContent.CanReuse(scope, pair.First, scope, pair.Second))
);
Check(
    "combined moving and stationary content cannot reuse one bitmap",
    !SkiaPlatformRasterContent.CanReuse(
        scope,
        new(combinedStart, new(0, 0, 1000, 800)),
        scope,
        new(combinedEnd, new(0, 0, 1000, 800))
    )
);
...
static Picture Picture(uint color)
{
    ...
    new Canvas(recorder).drawRect(
        ...
        new Paint { color = new Color(color) }
    );
    return recorder.endRecording();
}
static SkiaPlatformRasterContent.Slice Slice(
    Picture picture,
    double x,
    double y,
    bool changing = false,
    double viewportRight = 500,
