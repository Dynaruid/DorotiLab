...
foreach (var pointerId in new[] { 0, 1, 31 })
    Assert(
        androidIds.Add(
            checked(
                (long)Doroti.Host.Maui.AndroidPointerMapping.DeviceIdentifier(device, pointerId)
            )
        ),
        "Android device/pointer identity must fit signed Flutter IDs and remain unique"
    );
Console.WriteLine("PASS Android virtual/physical device identifiers and multi-pointer isolation");
await using (
    var rejectedOwner = new PlatformViewCoordinator(
        3,
        "test",
        new([
            new Factory
            {
                Failure = new WebViewException(
                    WebViewError.Unsupported,
                    "provider feature missing"
                ),
            },
        ]),
        new Dispatcher()
...
)
{
    try
    {
        await rejectedOwner.CreateAsync(new PlatformViewRequest(1, "test"));
        throw new Exception("Expected typed factory rejection.");
    }
...
await a.AttachAsync(
        new(handle, Rect.fromLTWH(0, 0, 20, 20), PlatformViewTransform.Identity, null, 0)
    )
    .AsTask()
...
await Reject(() => a.ExecuteWebViewAsync(handle, new(WebViewOperation.State)));
Assert(
    (await a.ExecuteWebViewAsync(replacement, new(WebViewOperation.State))).RequestId == 1,
    "replacement command"
);
new WebViewOptions(Html: "한글", AllowedOrigins: ["https://example.com"]).Encode();
await Reject(() =>
    Task.FromResult(new WebViewOptions(AllowedOrigins: ["https://example.com/path"]).Encode())
);
Console.WriteLine(
    "PASS owner isolation; event lifetime; async command does not block placement/close; pending cancellation; stale generation; options policy"
);
...
sealed class Factory : IPlatformViewFactory
{
    ...
    public PlatformViewSupport QuerySupport(PlatformViewRequest request) =>
        new("test", "test", "test", true, request.Composition, PlatformViewEffects.RectClip);
    ...
    public ValueTask<IPlatformViewInstance> CreateAsync(
        ...
        Failure is { } error
            ? ValueTask.FromException<IPlatformViewInstance>(error)
            : ValueTask.FromResult<IPlatformViewInstance>(Instance = new(handle));
}

sealed class Instance(PlatformViewHandle handle) : IPlatformViewInstance, IPlatformWebViewInstance
{
    private readonly TaskCompletionSource<WebViewResult> _pending = new(
        TaskCreationOptions.RunContinuationsAsynchronously
    );
    public event Action<WebViewEvent>? WebViewChanged;

    public void Send() =>
        WebViewChanged?.Invoke(new(handle, 1, 1, WebViewEventKind.Completed, "about:blank"));

    public Task<WebViewResult> ExecuteAsync(
        WebViewCommand command,
        CancellationToken cancellationToken
    ) =>
        command.Operation == WebViewOperation.EvaluateJavaScript
            ? _pending.Task
            : Task.FromResult(new WebViewResult(1, 1, 1));
    ...
    {
        _pending.TrySetException(new WebViewException(WebViewError.Closed, "closed"));
