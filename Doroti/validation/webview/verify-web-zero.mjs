...
try{
 ...
 try{
  await cdp('Page.navigate',{url:`http://127.0.0.1:${proxy.address().port}/?dorotiTestbedMode=webview-workload&dorotiWebViewCount=0&dorotiRenderer=${mode}`});
  let ready=false;for(let i=0;i<30;i++){ready=await evaluate(`document.querySelector('.doroti-root')?.dataset.dorotiWorkerRuntime==='ready'&&JSON.parse(__dorotiResizeDiagnostics.presenter('doroti-surface')).frontRequestId>0`);if(ready)break;await wait(300)}
  if(!ready)throw Error('Zero-view product not ready');await wait(300);
  const state=await evaluate(`({extraNodes:document.querySelectorAll('iframe,[data-doroti-raster],[data-doroti-platform-effect]').length,modules:performance.getEntriesByType('resource').map(r=>r.name).filter(n=>/doroti.web.(composition|platform-views|webview)/.test(n)),presenter:JSON.parse(__dorotiResizeDiagnostics.presenter('doroti-surface'))})`);
  if(state.extraNodes!==0||state.modules.length!==0)throw Error(JSON.stringify(state));
  const version=await cdp('Browser.getVersion');await writeFile(join(out,'result.json'),JSON.stringify({status:'PASS',state,version,scope:'Product boot and zero-view lazy module/resource boundary'},null,2));console.log('PASS zero view: no composition modules, DOM resources or uploads');
