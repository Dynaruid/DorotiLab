...
def memory(pid):
    seen=set();total=0;pending=[pid];unreadable=[]
    while pending:
        current=pending.pop()
    ...
    return dict(observableTreePssBytes=total,processCount=len(seen),unreadablePssCount=len(unreadable))
reports=[]
for count in (0,1,4):
    for mode in ('idle','animation','scroll','modal'):
        ...
        if samples:report['peakObservableTreePssBytes']=max(s['observableTreePssBytes'] for s in samples);report['maxProcesses']=max(s['processCount'] for s in samples);report['maxUnreadablePssCount']=max(s['unreadablePssCount'] for s in samples)
