"""Build and run an external AppKit package-only desktop consumer.
...
consumer = Path(tempfile.mkdtemp(prefix='doroti-desktop-appkit-'))
TFM = os.environ.get('DOROTI_MACOS_PACKAGE_TFM', 'net10.0-macos27.0')
...
for name in ['Doroti.Skia.RuntimeEffects', 'Doroti.Skia.Rendering', 'Doroti.Host.Maui',
             'Doroti.Target.MacOS.Maui.osx-arm64', 'Doroti.App.Sdk', 'Doroti.Runner.Sdk']:
    args = ['dotnet', 'pack', str(ROOT / f'Doroti/src/{name}/{name}.csproj'), '-c', 'Release', '-o', str(FEED), '--nologo', '-v:q']
    if name in ['Doroti.Host.Maui', 'Doroti.Target.MacOS.Maui.osx-arm64']:
        args += ['-p:RuntimeIdentifier=osx-arm64', f'-p:DorotiMacOSTargetFramework={TFM}']
    run(args)
...
for name in ['DesktopStartup.cs', 'DesktopProbe.cs']:
    shutil.copyfile(ROOT / 'DorotiTestbedApp/desktop' / name, consumer / 'desktop' / name)
runner = consumer / 'macos/Probe.MacOS.csproj'
runner.write_text(f'''<Project Sdk="Microsoft.NET.Sdk"><Sdk Name="Doroti.Runner.Sdk" Version="0.3.0-beta"/>
<PropertyGroup>
<TargetFramework>{TFM}</TargetFramework><RuntimeIdentifier>osx-arm64</RuntimeIdentifier>
<AssemblyName>DorotiTestbedApp.MacOS</AssemblyName><RootNamespace>DorotiTestbedApp.MacOS</RootNamespace>
<DorotiAppProject>../Probe.csproj</DorotiAppProject><DorotiTarget>macOS</DorotiTarget>
...
<DorotiHostKind>Maui</DorotiHostKind><DorotiNativeEntryKind>AppKit-Main</DorotiNativeEntryKind><UseMaui>true</UseMaui>
<DefineConstants>$(DefineConstants);MACOS;DOROTI_MAUI</DefineConstants>
...
</PropertyGroup><ItemGroup><PackageReference Include="Doroti.Target.MacOS.Maui.osx-arm64" Version="0.3.0-beta"/>
<DorotiNativeBindingProject Include="binding/DorotiTestbedApp.MacOS.Native.csproj"/>
</ItemGroup></Project>''')
(consumer / 'macos/AppKitDelegate.cs').write_text('''using Foundation;
using Doroti.Host.Maui;
namespace DorotiTestbedApp.MacOS;
[Register("DorotiDesktopPackageDelegate")]
public sealed class AppKitDelegate : DorotiMacOSMauiApplication {
 ...
 public override void DidFinishLaunching(NSNotification notification) {
  ...
  _ = MacOSDesktopWindowEvidence.RunAsync();
 }
}''')
shutil.copyfile(ROOT / 'DorotiTestbedApp/macos/MacOSDesktopWindowEvidence.cs', consumer / 'macos/MacOSDesktopWindowEvidence.cs')
shutil.copyfile(ROOT / 'DorotiTestbedApp/macos/DorotiNativePlatformBridge.MacOS.cs', consumer / 'macos/DorotiNativePlatformBridge.MacOS.cs')
shutil.copyfile(ROOT / 'DorotiTestbedApp/macos/Info.MacOS.plist', consumer / 'macos/Info.plist')
...
(consumer / 'macos/application-manifest.json').write_text(json.dumps(dict(schemaVersion='doroti.application-capabilities/v1', applicationId='dev.doroti.desktop.probe', targetRid='osx-arm64', resources=[], plugins=[])))
env = dict(os.environ, NUGET_PACKAGES=str(consumer / 'packages'))
run(['dotnet', 'build', str(runner), '-c', 'Release', f'-p:DorotiMacOSTargetFramework={TFM}', '--nologo', '-v:q'], cwd=consumer, env=env)
...
probe_output = run(['python3', str(Path(__file__).with_name('verify-macos.py')), '--app', str(app), '--output', str(consumer / 'probe')], env=env)
result = dict(status='PASS', consumer=str(consumer), appkitExecution='PASS', packageOnly=True, tfm=TFM, probeOutput=probe_output)
(OUT / 'macos-result.json').write_text(json.dumps(result, indent=2))
