...
try {
  ...
  await page.goto((process.env.DOROTI_WEB_BASE_URL??'http://127.0.0.1:5088')+'/?dorotiTestbedMode=sample&dorotiResizeDiagnostics='+(diagnostics?'1':'0')+(traced?'&dorotiCanvasKitTrace=1':'')+(process.env.DOROTI_PERF_QUERY??''));
  ...
  const rasters=directDiagnostics[0]?.managed?.frame?.Skia?.Trace?.filter(e=>e.Phase===13 && !e.Reason)??[];
  const causalCommit=stimulus&&directCommits.find(commit=>rasters.some(frame=>
  ...
  const result={label,traced,diagnostics,sweep,progress,restart,onset,onsetWindows,causalOnset,stimuli,directDiagnostics,frameworkFrameMilliseconds,steadyCommitIntervals,cold:process.argv.includes('--cold'),elapsed:Date.now()-start,errors,managedProbes,before,after,stages,resizeTrace,uiFrameMilliseconds,directSurfaceMilliseconds,directCommitIntervals,
