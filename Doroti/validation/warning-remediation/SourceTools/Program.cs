...
if (args[0] == "index")
{
    ...
    foreach (
        var name in new[]
        {
            "diagnostics-debug.json",
            "diagnostics-release.json",
            "warning-ledger.json",
        }
    )
    {
        ...
        foreach (var row in rows.Select(r => r!.AsObject()))
        {
            ...
            if (!cache.TryGetValue(file, out var root))
            {
                root = CSharpSyntaxTree
                    .ParseText(File.ReadAllText(Path.Combine(run, "isolated", file)))
                    .GetRoot();
                cache.Add(file, root);
            }
            ...
            {
                "CS0693" or "CS8981" or "CS8609" or "CS8613" or "CS8765" or "CS8767" or "CS8714" =>
...
var workspaceProperties = args.Where(a => a.StartsWith("--property=", StringComparison.Ordinal))
    ...
    .ToDictionary(
        p => p[0],
        p => p.Length == 2 ? p[1] : throw new ArgumentException("Expected --property=Name=Value.")
    );
...
var projectIds = workspace
    .CurrentSolution.GetProjectDependencyGraph()
    .GetTopologicallySortedProjects()
    .ToArray();
if (args[0] == "rename-intent-action")
{
    var project = workspace.CurrentSolution.Projects.Single(p =>
        p.Name == "Doroti.Framework.Widgets"
    );
    var compilation = await project.GetCompilationAsync() ?? throw new InvalidOperationException();
    var symbol =
        compilation.GetTypeByMetadataName("Doroti.Framework.Widgets.Action`1")
        ?? throw new InvalidOperationException(
            "The framework Action<T> declaration was not found."
        );
    if (!symbol.Locations.Any(l => l.IsInSource))
        throw new InvalidOperationException("Expected a source declaration.");
    ...
    if (!workspace.TryApplyChanges(renamed))
        throw new InvalidOperationException("IntentAction rename could not be applied.");
    File.WriteAllText(
        args[2],
...
if (args[0] == "simplify-icon-data-names")
{
    ...
    foreach (var projectId in projectIds)
    {
        ...
        if (project.Name != "Doroti.Framework.Material")
            continue;
        foreach (var documentId in project.DocumentIds)
        {
            ...
            if (
                !headerText.Contains("/animated_icons/data/", StringComparison.Ordinal)
                && !headerText.Contains("/material/icons.dart", StringComparison.Ordinal)
            )
                continue;
            var root = (CompilationUnitSyntax)(
                await document.GetSyntaxRootAsync() ?? throw new InvalidOperationException()
            );
            var names = root.DescendantNodes()
                .OfType<QualifiedNameSyntax>()
                .Where(n =>
                    n.ToString()
                        is "global::Doroti.Ui.Offset"
                            or "global::Doroti.Ui.Size"
                            or "global::Doroti.Framework.Widgets.IconData"
                )
                .ToArray();
            if (names.Length == 0)
                continue;
            ...
            var model =
                await document.GetSemanticModelAsync() ?? throw new InvalidOperationException();
            ...
            var expected = names
                .Select(n =>
                    model.Compilation.GetTypeByMetadataName(n.ToString()[8..])
                    ?? throw new InvalidOperationException("Icon data name is not a resolved type.")
                )
                .ToArray();
            var indices = names
                .Select((node, index) => (node, index))
                .ToDictionary(p => p.node, p => p.index);
            const string annotationKind = "DorotiIconDataType";
            var updated = root.ReplaceNodes(
                names,
                (original, _) =>
                    SyntaxFactory
                        .IdentifierName(original.Right.Identifier)
                        .WithTriviaFrom(original)
                        .WithAdditionalAnnotations(
                            new SyntaxAnnotation(annotationKind, indices[original].ToString())
                        )
            );
            ...
            if (
                names.Any(n =>
                    n.ToString().StartsWith("global::Doroti.Ui.", StringComparison.Ordinal)
            ...
            )
            {
                ...
                updated = updated
                    ...
                    .AddUsings(
                        SyntaxFactory
                            .UsingDirective(SyntaxFactory.ParseName("Doroti.Ui"))
                            .WithUsingKeyword(
                                SyntaxFactory
            ...
            var candidateRoot =
                await candidate.GetSyntaxRootAsync() ?? throw new InvalidOperationException();
            var candidateModel =
                await candidate.GetSemanticModelAsync() ?? throw new InvalidOperationException();
            var checkedNames = 0;
            foreach (var node in candidateRoot.GetAnnotatedNodes(annotationKind))
            {
                ...
                var actual = candidateModel.LookupNamespacesAndTypes(
                    node.SpanStart,
                    name: expected[index].Name
                );
                if (
                    actual.Length != 1
                    || !SymbolEqualityComparer.Default.Equals(expected[index], actual[0])
                )
                    throw new InvalidOperationException(
                        $"Icon data type binding changed in {document.Name} at {index}."
                    );
                checkedNames++;
            }
            if (
                checkedNames != names.Length
                || candidateModel.GetDiagnostics().Any(d => d.Severity == DiagnosticSeverity.Error)
            )
                throw new InvalidOperationException(
                    $"Icon data validation failed in {document.Name}."
                );
            if (!workspace.TryApplyChanges(candidate.Project.Solution))
                throw new InvalidOperationException(
                    "Icon data simplification could not be applied."
                );
...
if (args[0] is "fix-ide0001" or "check-ide0001")
{
    ...
    var nameAnalyzerType = featuresAssembly.GetType(
        "Microsoft.CodeAnalysis.CSharp.Diagnostics.SimplifyTypeNames.CSharpSimplifyTypeNamesDiagnosticAnalyzer",
        true
    )!;
    var nameAnalyzer = (DiagnosticAnalyzer)Activator.CreateInstance(nameAnalyzerType, true)!;
    var nameProviderType = featuresAssembly
        .GetTypes()
        .Single(t => t.Name == "SimplifyTypeNamesCodeFixProvider");
    var nameProvider = (CodeFixProvider)Activator.CreateInstance(nameProviderType, true)!;
    var report = new List<object>();
    var module = args.FirstOrDefault(a => a.StartsWith("--module=", StringComparison.Ordinal))?[
        9..
    ];
    foreach (var projectId in projectIds)
    {
        var project = workspace.CurrentSolution.GetProject(projectId)!;
        if (module is not null && project.Name != module)
            continue;
        Console.WriteLine($"Analyzing IDE0001: {project.Name}");
        var compilation =
            await project.GetCompilationAsync() ?? throw new InvalidOperationException();
        ...
        foreach (var documentId in project.DocumentIds)
        {
            ...
            var originalTree =
                await project.GetDocument(documentId)!.GetSyntaxTreeAsync()
                ?? throw new InvalidOperationException();
            var documentDiagnostics = await analysis.GetAnalyzerSemanticDiagnosticsAsync(
                compilation.GetSemanticModel(originalTree),
                null,
                CancellationToken.None
            );
            if (documentDiagnostics.Any(d => d.Id == "AD0001"))
                throw new InvalidOperationException(
                    "IDE0001 analyzer failed: "
                        + string.Join("\n", documentDiagnostics.Where(d => d.Id == "AD0001"))
                );
            var diagnostics = documentDiagnostics
                .Where(d => d.Id == "IDE0001" && d.Location.SourceTree == originalTree)
                .ToImmutableArray();
            if (diagnostics.IsEmpty)
                continue;
            report.Add(new { file = document.FilePath, count = diagnostics.Length });
            Console.WriteLine($"  {document.Name}: {diagnostics.Length}");
            if (args[0] != "fix-ide0001")
                continue;
            var actions = new List<CodeAction>();
            await nameProvider.RegisterCodeFixesAsync(
                new CodeFixContext(
                    document,
                    diagnostics[0],
                    (a, _) => actions.Add(a),
            ...
            var context = new FixAllContext(
                ...
                ["IDE0001"],
                new DocumentDiagnostics(documentId, diagnostics),
                CancellationToken.None
            );
            var action =
                await nameProvider.GetFixAllProvider()!.GetFixAsync(context)
                ?? throw new InvalidOperationException("No IDE0001 fix-all action.");
            var operation = (await action.GetOperationsAsync(CancellationToken.None))
                .OfType<ApplyChangesOperation>()
                .Single();
            if (!workspace.TryApplyChanges(operation.ChangedSolution))
                throw new InvalidOperationException("Name simplification could not be applied.");
        }
    }
    File.WriteAllText(
        args[2],
    ...
    if (args[0] == "check-ide0001" && report.Count != 0)
        ...
        foreach (var documentId in project.DocumentIds)
        {
            while (true)
            {
                ...
                var parameter = root.DescendantNodes()
                    ...
                    .FirstOrDefault(p =>
                        p.Ancestors()
                            ...
                            .Any(t =>
                                t.TypeParameterList?.Parameters.Any(tp =>
                                    tp.Identifier.Text == p.Identifier.Text
                                ) == true
                            )
                    );
                ...
                var renamed = owner switch
                {
                    ...
                    or "_CupertinoSheetRouteTransitionMixin__sheet"
                    or "ModalRoute" => "TRouteResult",
                    _ => "TValue",
                };
                var model =
                    await document.GetSemanticModelAsync() ?? throw new InvalidOperationException();
                var symbol =
                    model.GetDeclaredSymbol(parameter) ?? throw new InvalidOperationException();
                ...
                if (!workspace.TryApplyChanges(solution))
                    throw new InvalidOperationException("Rename could not be applied.");
                changes.Add(
                    new
                    {
                        file = document.FilePath,
                        owner,
                        before = parameter.Identifier.Text,
                        after = renamed,
                    }
                );
                Console.WriteLine(
                    $"Renamed {document.Name}: {owner}.{symbol.ContainingSymbol.Name} -> {renamed}"
                );
            }
        }
    }
...
{
    var locations = System
        .Text.RegularExpressions.Regex.Matches(
            File.ReadAllText(args[3]),
            @"^(.*?)\((\d+),(\d+)\): error CS8600:",
        ...
        .ToDictionary(
            ...
            g =>
                g.Select(m =>
                        (Line: int.Parse(m.Groups[2].Value), Column: int.Parse(m.Groups[3].Value))
                    )
                    .Distinct()
                    .ToArray(),
    ...
    {
        var document = workspace.CurrentSolution.GetDocument(documentId)!;
        if (IsGenerated(document) || !locations.TryGetValue(document.FilePath!, out var positions))
            continue;
        ...
        foreach (var position in positions)
        {
            var token = root.FindToken(
                sourceText.Lines[position.Line - 1].Start + position.Column - 1
            );
            var cast = token
                .Parent?.AncestorsAndSelf()
                .OfType<CastExpressionSyntax>()
                .FirstOrDefault();
            if (cast is null || cast.Type is NullableTypeSyntax)
                continue;
            if (
                model.GetTypeInfo(cast.Type).Type is not { IsReferenceType: true } type
        ...
        var updated = root.ReplaceNodes(
            ...
            (_, rewritten) =>
                rewritten.WithType(
                    SyntaxFactory
                        .NullableType(rewritten.Type.WithoutTrivia())
                        .WithTriviaFrom(rewritten.Type)
                )
        );
        if (!workspace.TryApplyChanges(document.WithSyntaxRoot(updated).Project.Solution))
            throw new InvalidOperationException("Nullable cast annotation failed.");
    ...
    foreach (var projectId in projectIds)
    {
        ...
        foreach (var documentId in project.DocumentIds)
        {
            ...
            if (
                module is null
                && !root.ToFullString()
                    .Contains("#pragma warning disable", StringComparison.Ordinal)
            )
                continue;
            var model =
                await document.GetSemanticModelAsync() ?? throw new InvalidOperationException();
            ...
            {
                if (Unwrap(cast.Expression) is not CastExpressionSyntax bridge)
                    continue;
                if (model.GetTypeInfo(bridge.Type).Type?.SpecialType != SpecialType.System_Object)
                    continue;
                var source = model.GetTypeInfo(bridge.Expression).Type;
                var target = model.GetTypeInfo(cast.Type).Type;
                if (
                    Unwrap(bridge.Expression).IsKind(SyntaxKind.NullLiteralExpression)
                    && target is { IsReferenceType: true }
                    && target.TypeKind is not (TypeKind.Dynamic or TypeKind.Error)
                )
                {
                ...
                if (
                    source is null
                    || target is null
                    || source.TypeKind is TypeKind.Dynamic or TypeKind.Error
                ...
                if (!(conversion.IsIdentity || conversion.IsReference) || conversion.IsUserDefined)
                    continue;
                if (
                    bridge
                        .Type.DescendantTrivia()
                        .Any(t =>
            ...
            var updated = root.ReplaceNodes(
                ...
                (original, rewritten) =>
                {
                    var result = rewritten.WithExpression(
                        ((CastExpressionSyntax)Unwrap(rewritten.Expression)).Expression
                    );
                    return
                        nullCandidates.Contains(original) && result.Type is not NullableTypeSyntax
                        ? result.WithType(
                            SyntaxFactory
                                .NullableType(result.Type.WithoutTrivia())
                                .WithTriviaFrom(result.Type)
                        )
                        : result;
                }
            );
            ...
            changes.Add(
                new
                {
                    ...
                    nullReferences = nullCandidates.Count,
                }
            );
...
var analyzerType = features.GetType(
    "Microsoft.CodeAnalysis.CSharp.Diagnostics.SimplifyTypeNames.CSharpSimplifyTypeNamesDiagnosticAnalyzer",
    true
)!;
var analyzer = (DiagnosticAnalyzer)Activator.CreateInstance(analyzerType, true)!;
var predicate = analyzerType.GetMethod(
    "CanSimplifyTypeNameExpression",
    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly
)!;
var optionsType = Assembly
    .Load("Microsoft.CodeAnalysis.CSharp.Workspaces")
    .GetType("Microsoft.CodeAnalysis.CSharp.Simplification.CSharpSimplifierOptions", true)!;
var options = optionsType
    .GetField("Default", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)!
    .GetValue(null);
...
foreach (var projectId in projectIds)
{
    ...
    foreach (var documentId in project.DocumentIds)
    {
        ...
        foreach (var node in root.DescendantNodes().OfType<MemberAccessExpressionSyntax>())
        {
            ...
            if (
                (bool)predicate.Invoke(analyzer, parameters)!
                && (string?)parameters[4] == "IDE0002"
            )
            {
                var span = (TextSpan)parameters[3]!;
                diagnostics.Add(
                    Diagnostic.Create(descriptor, Location.Create(root.SyntaxTree, span))
                );
            }
        }
        ...
        evidence.Add(
            new
            {
                ...
                locations = diagnostics
                    .Select(d => d.Location.GetLineSpan().StartLinePosition.ToString())
                    .ToArray(),
            }
        );
        Console.WriteLine($"  {document.Name}: {diagnostics.Count}");
        if (args[0] != "fix-ide0002")
            continue;
        var actions = new List<CodeAction>();
        ...
        var context = new FixAllContext(
            ...
            key,
            ["IDE0002"],
            new DocumentDiagnostics(documentId, diagnostics.ToImmutableArray()),
            CancellationToken.None
        );
        var action =
            await provider.GetFixAllProvider()!.GetFixAsync(context)
            ?? throw new InvalidOperationException("No fix-all action.");
        var operation = (await action.GetOperationsAsync(CancellationToken.None))
            .OfType<ApplyChangesOperation>()
            .Single();
        if (!workspace.TryApplyChanges(operation.ChangedSolution))
            throw new InvalidOperationException("Simplification could not be applied.");
    }
}
File.WriteAllText(
    args[2],
...
if (args[0] == "check-ide0002" && evidence.Count != 0)
    Environment.ExitCode = 1;

static bool IsGenerated(Document document) =>
    document.FilePath is null
    || document.FilePath.Replace('\\', '/').Contains("/obj/", StringComparison.Ordinal)
...
sealed class DocumentDiagnostics(DocumentId id, ImmutableArray<Diagnostic> diagnostics)
