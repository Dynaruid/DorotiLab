...
try{
 ...
 const seek=async(predicate,label)=>{for(let n=0;n<55;n++){const visible=await nodes();if(visible.some(x=>(x.label??'').includes('Date picker')))console.log('picker area '+JSON.stringify(visible.filter(x=>x.label).map(x=>[x.role,x.label])));const v=visible.find(predicate);if(v){await wait(650);const settled=(await nodes()).find(predicate);if(settled)return settled;}await cdp('Input.dispatchMouseEvent',{type:'mouseWheel',x:750,y:400,deltaX:0,deltaY:240});await wait(350);}throw Error('Not found: '+label);};
 ...
 try {
   await shot('initial');
   const defaults=await nodes();check(defaults.some(x=>x.role==='button'&&x.label==='Filled'&&x.disabled==='true'),'disabled button stays disabled');
