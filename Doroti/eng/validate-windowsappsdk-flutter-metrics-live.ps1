...
[CmdletBinding()]
param(
    [string] $ProjectPath = (Join-Path $PSScriptRoot '../validation/windowsappsdk-flutter-metrics/Doroti.Validation.WindowsAppSdkFlutterMetrics.csproj'),

    [string] $ContractValidatorPath = (Join-Path $PSScriptRoot 'validate-windowsappsdk-flutter-metrics.ps1'),

    [string] $OutputRoot = (Join-Path $PSScriptRoot '../artifacts/windowsappsdk-flutter-metrics')
)
...
function Invoke-ProcessWithinTestTimeout(
    ...
    if ($RestrictPath) {
        # The fixture must resolve only its published native artifacts; retain
        # system DLL lookup for the no-PATH-fallback launch probe.
...
function Invoke-ContractValidator([string] $EvidencePath, [bool] $RequireEvidence) {
    ...
    if ($null -ne $LASTEXITCODE -and $LASTEXITCODE -ne 0) {
        throw 'F3 static metrics contract validation failed.'
    }
...
$publishDirectory = Join-Path $outputRootFullPath (
    'f3-live-' + [DateTimeOffset]::UtcNow.ToString('yyyyMMdd-HHmmss') + '-' + [Guid]::NewGuid().ToString('N'))
Assert-True (-not (Test-Path -LiteralPath $publishDirectory)) "F3 publish directory already exists: $publishDirectory"

$restoreOutput = Invoke-ProcessWithinTestTimeout 'dotnet' @(
    'restore', $projectFullPath, '--force-evaluate', '--nologo', '-m:1'
) 'F3 clean restore' $false
...
) 'F3 self-contained publish' $false
Write-Host $publishOutput

$fixtureBaseName = 'Doroti.Validation.WindowsAppSdkFlutterMetrics'
...
foreach ($requiredPath in @($executablePath, $depsPath, $anglePath, $skiaPath, $windowsAppRuntimePath)) {
    Assert-True (Test-Path -LiteralPath $requiredPath -PathType Leaf) "F3 publish is missing: $requiredPath"
}
Assert-True (@(Get-ChildItem -LiteralPath $publishDirectory -File -Filter 'av_libglesv2.dll').Count -eq 1) (
    'F3 publish must contain exactly one selected ANGLE DLL.')
Assert-True (@(Get-ChildItem -LiteralPath $publishDirectory -File -Filter 'libSkiaSharp.dll').Count -eq 1) (
    'F3 publish must contain exactly one selected Skia native DLL.')
...
) 'F3 physical metrics/display live validation' $true
...
# The F3 fixture uses GetGuiResources after warmup to bound the 100-cycle HWND/metrics probe.
$summary = [ordered]@{
    schemaVersion = 'doroti.windowsappsdk-flutter-metrics-live-validation/v1'
    status = 'PASS'
    ...
    dpiMatrix = $evidence.dpiMatrix
    validation = $evidence.validation
    resources = $evidence.resources
    startup = $evidence.startup
    evidencePath = $evidencePath
    notVerified = @(
        'F4 raster-thread EGL child-window surface creation, recreation, and actual swap/present',
        'F5 bounded resize transaction completion, F6 cadence, F7 input, F8 lifecycle, and F9 product-runner selection',
        'output-level visible resize, blank-frame, and compositor acceptance (FG)'
    )
}
$summaryPath = Join-Path $publishDirectory 'f3-live-validation.json'
