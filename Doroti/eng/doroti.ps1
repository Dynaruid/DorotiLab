...
param(
    ...
    [ValidateSet('android', 'ios', 'linux', 'macos', 'web', 'windows', 'all')]
    [string] $Platform,
...
function Resolve-DorotiWorkspace {
    ...
    try {
        ...
        $allowedAliases = @('android', 'ios', 'linux', 'macos', 'web', 'windows')
...
function Resolve-DorotiNativeWorkspace {
    ...
    if ($Platform -cnotin @('android', 'ios', 'macos')) { throw 'native commands require --platform android, ios, or macos.' }
    $workspace = Resolve-DorotiWorkspace $App
    $platformRoot = Join-Path $workspace.Root $Platform
    $runner = $workspace.Runners[$Platform]
...
function Write-InteropMigrationDiagnostic {
    throw 'scaffold-interop is obsolete because Android, iOS, and Mac Catalyst native bridges are generated by default. Use: doroti native doctor|build|open|add --app <path> --platform <name>.'
}
