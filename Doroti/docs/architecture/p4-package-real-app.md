...
The retired P4 external gate covered restore, build/run, trimmed self-contained single-file ReadyToRun publish, published executable run, source-linked diagnostic audit, package-content audit, and provider rebase. The checked-in `migration/releases/p4-provider-upgrade-rebase.json` is the normalized upgrade review evidence from that workflow.
