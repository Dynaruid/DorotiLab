...
Current-status note: A0's macOS selection was promoted later by G7-3M into `Doroti.Vendor.Avalonia.Native` and `Doroti.Target.macOS.osx-arm64`. Its independent provenance and target evidence are recorded in `migration/avalonia-shell/g7-macos-source-port-provenance.json` and `migration/macos/g7-macos-shell-evidence.json`; this later result does not retroactively change the A0 milestone claim.
