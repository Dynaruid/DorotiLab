...
The default Windows presenter remains `AngleD3D11`. Maintainers can explicitly test the experimental Vulkan path with `DOROTI_WINDOWS_PRESENTER=Vulkan`; set `DOROTI_WINDOWS_VULKAN_DEVICE` when multiple capable GPUs are eligible or deterministic selection is required. The product path uses System32 Vulkan 1.1, dedicated D3D11-texture external memory, Windows Presentation, and zero Vulkan WSI swapchains. SkiaSharp is pinned to `4.152.0-rc.1.26426.14` and the Vulkan context uses `SkiaSharp.Vulkan.Silk.NET`. A native topmost DirectComposition target on the top-level HWND owns a full-capacity identity raster, and the HWND client clip is the sole exact viewport. Moving-origin left/top edges keep the current raster attached to that HWND and coalesce actual `WM_SIZE` work to the latest frame without blocking each interactive step; fixed-origin edges retain their bounded exact-frame handshake. On Windows 11 24H2+, explicit Vulkan may be combined with `experimentalAcrylic`: a host-backdrop-enabled non-topmost `DesktopWindowTarget` runs an active `DesktopAcrylicController`, while the topmost Vulkan overlay remains premultiplied. Physical resize/scan-out, Acrylic quality, and the full GPU/DPI/monitor matrix remain `notVerified`, so Vulkan stays experimental opt-in.
