...
The default Windows presenter remains `AngleD3D11`. Maintainers can explicitly test the experimental direct Vulkan path with `DOROTI_WINDOWS_PRESENTER=Vulkan`; set `DOROTI_WINDOWS_VULKAN_DEVICE` when multiple capable GPUs are eligible or deterministic selection is required. Vulkan requires System32 Vulkan 1.1 plus the surface, Win32-surface, and swapchain extensions; swapchain-maintenance is not required. It never falls back silently and cannot be combined with `experimentalAcrylic`. On the tested AMD Radeon 780M, a retained oversized-child surface removes normal per-size swapchain recreation. The Vulkan-only path prepares each proposed logical viewport and completes its DWM synchronization inside `WM_SIZING`, before the top-level geometry is exposed. It does not paint a parent fallback that could conceal a remaining exposure. Iteration validation is now one focused top-left probe, while the repeated eight-edge matrix is opt-in. Physical resize/scan-out and the full GPU/DPI/monitor matrix remain `notVerified`, so Vulkan stays experimental opt-in.
