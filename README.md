...
The default Windows presenter remains `AngleD3D11`. Maintainers can explicitly test the experimental direct Vulkan path with `DOROTI_WINDOWS_PRESENTER=Vulkan`; set `DOROTI_WINDOWS_VULKAN_DEVICE` when multiple capable GPUs are eligible or deterministic selection is required. Vulkan requires System32 Vulkan 1.1 plus the surface, Win32-surface, and swapchain extensions; swapchain-maintenance is not required. It never falls back silently and cannot be combined with `experimentalAcrylic`. On the tested AMD Radeon 780M, the retained oversized-child surface removed normal per-size swapchain recreation: all 24 automated 600 px/600 ms reverse border/corner cases passed with zero blank frames or Vulkan errors, at least 119.15 presentations/s during motion, and target-to-present p95 at most 4.95 ms. Physical resize/scan-out and the full GPU/DPI/monitor matrix remain `notVerified`, so Vulkan stays experimental opt-in.
