...
- G0–G4 최초 실행: 첫 제품 출력 뒤 정상 종료·장치 손실 gate가 실패하여
  당시 후보를 보존하고 WebGL S2로 원복했다. 그 실패 기록은 아래에 보존한다.
  이후 후속 전환으로 **현재 제품 기본값은 `worker-direct-webgpu`**이며,
  `worker-direct-webgl`은 명시적 선택 경로다. 현재 기본값과 최초 원복 기록을 구분한다.
- 병렬 레이아웃: 사용자 판단에 따라 제거. 기존 계산 비중 0.3% 미만의 측정과
  이후 실험의 결과를 보존하며 성능 개선으로 판정하지 않는다.

현재 후속 범위는 WebGPU 기본값과 명시적 WebGL의 수명·기능 회귀, 성능 검증이다.
패키지 전환과 WebGPU의 효과를 구분한다. 네이티브 Graphite 전환은 [work.md](work.md)와
[Linux 후속 보고서](Doroti/docs/validation/native-graphite-linux-2026-09-09.md)를 따른다.
아래 최초 계획·원복·실패 기록을 현재 기본값 설명으로 사용하지 않는다.
