...
`DOROTI_WINDOWS_PRESENTER=Vulkan` selects the separate experimental Vulkan/Skia path for this same `HwndExactCpp` runner. Stale work is rejected before image acquisition; a successful acquire commits to unconditional copy/present, so acquired-image release is not required. The resize path submits Skia asynchronously and defers its one copy fence to next use. A required native feature keeps the visible child HWND at a grow-only surface capacity initially matching the current monitor work area; the exact parent client clips it with `WS_CLIPCHILDREN`. Mirroring Flutter's Windows handshake, `WM_SIZING` does not render a provisional layout: the actual top-client `WM_SIZE` publishes the authoritative logical extent and holds the HWND thread for up to 100 ms until the raster worker presents that exact generation. A successful interactive present crosses one `DwmFlush` before the transaction returns. This is render-driven USER32 backpressure, not a fixed 30/60 fps limiter. No app-colored parent fallback is painted. Logical viewport changes that fit the retained capacity do not resize the child, recreate the Win32 surface/swapchain, or scale the raster. The full capacity is cleared to the application background and scene drawing is clipped to the logical viewport; a capacity overrun performs one grow/recreate.
