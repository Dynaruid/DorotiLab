...
`DOROTI_WINDOWS_PRESENTER=Vulkan` selects the separate experimental Vulkan/Skia path for this same `HwndExactCpp` runner. Stale work is rejected before image acquisition; a successful acquire commits to unconditional copy/present, so acquired-image release is not required. The resize path submits Skia asynchronously, defers its one copy fence to next use, and runs `DwmFlush` only for final settlement. A required native feature keeps the visible child HWND at a grow-only surface capacity initially matching the current monitor work area; the exact parent client clips it with `WS_CLIPCHILDREN`. Logical viewport changes that fit this capacity therefore redraw and present without resizing the child, recreating the Win32 surface/swapchain, or scaling the raster. The full capacity is cleared to the application background and scene drawing is clipped to the logical viewport; a capacity overrun performs one grow/recreate.

On the tested AMD Radeon 780M, a ten-resize product run used one 1280x752 surface for a final 950x670 viewport with nine retained reuses and ten accepted/presented generations. An `ANGLE → Vulkan → ANGLE` automated run of all eight borders/corners, three 600 px/600 ms reverse passes each, passed 24/24 cases with zero blank frames or Vulkan errors, one surface creation per process, at least 119.15 presentations/s during motion, target-to-present p95 at most 4.95 ms, and maximum at most 9.00 ms. The old exact-per-extent design's 19–25 ms `vkCreateSwapchainKHR` cost is now paid only before first show or at rare capacity growth. The device lacks swapchain-maintenance present fences, so unextended present-resource retirement is a separate specification-level `notVerified` boundary. Automated WGC and terminal evidence does not substitute for physical scan-out, human resize, IME, accessibility, larger-monitor memory/startup, or the full DPI/GPU matrix; Vulkan therefore remains opt-in. See [ADR-027](../Doroti/docs/adr/ADR-027-windows-optional-vulkan.md) and the [implementation checkpoint](../history/26-09-02/windows-appsdk-vulkan-implementation-checkpoint.md).
