...
Mac Catalyst cross-build is verified from Windows; native publish/run still requires Apple Silicon macOS. Android 16/API 36 arm64 physical-device automation verifies SkiaSharp 4.151.1, custom SkSL and `ImageFilter.shader` GPU execution, repeated-scroll rendering, and visible-content screenshots; the x86_64 package build also passes. Physical-device manual persistent-display, IME, TalkBack, stylus, and mouse acceptance remain separate. A manual Chromium smoke verified the published WebGL2 canvas and basic pointer state change, while a fresh browser-live `ImageFilter.shader` check remains `notVerified`.
