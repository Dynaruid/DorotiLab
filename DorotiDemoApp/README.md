...
`DOROTI_WINDOWS_PRESENTER=Vulkan` selects the separate experimental Vulkan/Skia path for this same `HwndExactCpp` runner. Stale work is rejected before image acquisition; a successful acquire commits to unconditional copy/present, so acquired-image release is not required. The resize path submits Skia asynchronously and defers its one copy fence to next use. A required native feature keeps the visible child HWND at a grow-only surface capacity initially matching the current monitor work area; the exact parent client clips it with `WS_CLIPCHILDREN`. During interactive resize, Vulkan-only `WM_SIZING` handling prepares the proposed logical viewport and completes one `DwmFlush` before returning, so the retained frame reaches the compositor before USER32 can expose the proposed top-level geometry. A matching actual `WM_SIZE` reuses that frame; a mismatch republishes the actual client extent. This is pre-geometry compositor synchronization, not a fixed 30/60 fps limiter. No app-colored parent fallback is painted because it would hide rather than eliminate a top/child exposure. Logical viewport changes that fit the retained capacity do not resize the child, recreate the Win32 surface/swapchain, or scale the raster. The full capacity is cleared to the application background and scene drawing is clipped to the logical viewport; a capacity overrun performs one grow/recreate.

On the tested AMD Radeon 780M, a ten-resize product run used one 1280x752 surface for a final 950x670 viewport with nine retained reuses and ten accepted/presented generations. The first retained-surface automation passed 24/24 cases, but subsequent physical testing reported transient black/white regions and left/top raster jitter; that observation remains the authoritative pre-fix result. After proposed-size admission was added, a final source-locked `ANGLE → Vulkan → ANGLE` run of all eight borders/corners, three 600 px/600 ms reverse passes each, passed 24/24 with 1,140/1,140 motion targets presented, zero supersedes, platform-wait timeouts, or marker regressions, at least 77.75 presentations/s, and worst target-to-present p95/maximum of 14.54/16.65 ms. Capability/WSI, injected-result, reset, lifecycle, resize, and start/close gates also passed. The old exact-per-extent design's 19–25 ms `vkCreateSwapchainKHR` cost is paid only before first show or at rare capacity growth. The device lacks swapchain-maintenance present fences, so unextended present-resource retirement is a separate specification-level `notVerified` boundary. Automated WGC and terminal evidence does not substitute for post-fix physical scan-out, human resize, IME, accessibility, larger-monitor memory/startup, or the full DPI/GPU matrix; Vulkan therefore remains opt-in. See [ADR-027](../Doroti/docs/adr/ADR-027-windows-optional-vulkan.md) and the [implementation checkpoint](../history/26-09-02/windows-appsdk-vulkan-implementation-checkpoint.md).
