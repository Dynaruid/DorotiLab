...
Future<void> runExtractor(List<String> args) async {
  ...
  try {
    final output = await extractAnalyzerOutput(
      path,
    ...
    stderr.writeln('Dart analyzer could not resolve $path: $error');
    exitCode = 65;
  }
}

Future<Map<String, Object?>> extractAnalyzerOutput(
  String path, {
...
String? _nodeElementId(AstNode node) {
  final element = switch (node) {
    ...
    DeclaredIdentifier(:final declaredFragment) => declaredFragment?.element,
    DeclaredVariablePattern(:final declaredFragment) =>
      declaredFragment?.element,
...
Map<String, String?> _nodeProperties(AstNode node) {
  ...
  switch (node) {
    ...
    case FunctionExpression(:final parameters, :final body):
      ...
      properties['isAsync'] = body.isAsynchronous.toString();
    ...
    case RepresentationDeclaration(:final fieldName, :final constructorName):
      properties['name'] = fieldName.lexeme;
      properties['constructor'] = constructorName?.name.lexeme ?? 'new';
    ...
    case ParenthesizedExpression(:final expression):
      properties['expressionOffset'] = expression.offset.toString();
    case ForPartsWithExpression(:final initialization, :final condition):
      properties['initializerOffset'] = initialization?.offset.toString();
      properties['conditionOffset'] = condition?.offset.toString();
    case ForPartsWithDeclarations(:final condition):
      properties['conditionOffset'] = condition?.offset.toString();
    ...
    case DeclaredVariablePattern(:final name):
      properties['name'] = name.lexeme;
    case AssignedVariablePattern(:final name):
      properties['name'] = name.lexeme;
    case PatternFieldName(:final name):
      properties['name'] = name?.lexeme;
    ...
    case RecordTypeAnnotation(:final question):
      properties['isNullable'] = (question != null).toString();
    case RecordTypeAnnotationNamedField(:final name):
      properties['name'] = name.lexeme;
...
String _canonicalElementId(Element element, {String? fallbackLibraryUri}) {
  ...
  final localSuffix = element is LocalVariableElement
      ? '@local${element.firstFragment.offset}'
      : '';
  return '$libraryUri#${names.reversed.join('.')}$localSuffix';
}
...
Future<SomeResolvedUnitResult> _resolveFileWithPackages(
  ...
  final resolvedPackagesPath = await materializeFrameworkPackageConfig(
    packagesPath,
  );
...
Future<String> materializeFrameworkPackageConfig(String packagesPath) async {
  ...
  final cache = dorotiCacheDirectory('flutter-package-config');
  final key = dorotiStableKey('${source.absolute.path}\n$content');
  final materialized = File('${cache.path}${Platform.pathSeparator}$key.json');
  if (await materialized.exists()) {
    if (await materialized.readAsString() != content) {
      throw StateError('Doroti package-config cache key collision: $key');
    }
  } else {
    final staging = File(
      '${materialized.path}.staging-$pid-'
      '${DateTime.now().microsecondsSinceEpoch}',
    );
    await staging.writeAsString(content);
    try {
      await staging.rename(materialized.path);
...
Future<Directory> _materializeSkyEngineWithHitTestOverlay(
  Directory hostSkyEngine,
) async {
  final hostDispatcher = File(
    '${hostSkyEngine.path}${Platform.pathSeparator}lib'
    '${Platform.pathSeparator}ui${Platform.pathSeparator}platform_dispatcher.dart',
  );
  final alreadyPresent =
      hostDispatcher.existsSync() &&
      hostDispatcher.readAsStringSync().contains('class HitTestRequest');
  if (alreadyPresent) {
    return hostSkyEngine;
  }
  if (!hostDispatcher.existsSync()) {
    return hostSkyEngine;
  }

  var source = await hostDispatcher.readAsString();
  final cache = dorotiCacheDirectory('sky-engine-hit-test');
  final key = dorotiStableKey(
    'doroti-sky-hit-test/v1\n${hostSkyEngine.absolute.path}\n$source',
  );
  final overlayRoot = Directory('${cache.path}${Platform.pathSeparator}$key');
  final overlayDispatcher = File(
    '${overlayRoot.path}${Platform.pathSeparator}lib'
    '${Platform.pathSeparator}ui${Platform.pathSeparator}platform_dispatcher.dart',
  );
  final readyMarker = File(
    '${overlayRoot.path}${Platform.pathSeparator}.doroti-hit-test-ready',
  );
  if (readyMarker.existsSync() && overlayDispatcher.existsSync()) {
    return overlayRoot;
  }
  if (overlayRoot.existsSync()) {
    await overlayRoot.delete(recursive: true);
  }

  final stagingRoot = cache.createTempSync('$key.staging-$pid-');
  await _mirrorSkyEngineExceptPlatformDispatcher(hostSkyEngine, stagingRoot);
  final stagingDispatcher = File(
    '${stagingRoot.path}${Platform.pathSeparator}lib'
    '${Platform.pathSeparator}ui${Platform.pathSeparator}platform_dispatcher.dart',
  );
  final stagingMarker = File(
    '${stagingRoot.path}${Platform.pathSeparator}.doroti-hit-test-ready',
  );
  if (!source.contains('typedef HitTestCallback')) {
  ...
  if (!source.contains('HitTestCallback? get onHitTest') &&
      source.contains(
        'set onPointerDataPacket(PointerDataPacketCallback? callback)',
      )) {
    source = source.replaceFirstMapped(
      RegExp(
        r'set onPointerDataPacket\(PointerDataPacketCallback\? callback\) \{'
        r'[\s\S]*?_onPointerDataPacketZone = Zone\.current;\r?\n  \}',
      ),
      (match) =>
          '${match.group(0)}\n\n'
  ...
  if (!source.contains('class HitTestRequest')) {
    source =
        '$source\n'
        '\n'
        ...
        '  static const HitTestResponse empty =\n'
        '      HitTestResponse(hasPlatformView: false);\n'
        '  final bool hasPlatformView;\n'
        '}\n';
  }
  await stagingDispatcher.parent.create(recursive: true);
  await stagingDispatcher.writeAsString(source);
  await stagingMarker.writeAsString('ok\n');
  try {
    await stagingRoot.rename(overlayRoot.path);
  } on FileSystemException {
    if (!readyMarker.existsSync() || !overlayDispatcher.existsSync()) {
      rethrow;
    }
...
Future<void> _mirrorSkyEngineExceptPlatformDispatcher(
  Directory source,
  ...
  await for (final entity in source.list(recursive: true, followLinks: false)) {
    ...
    if (relative.replaceAll('\\', '/') == 'lib/ui/platform_dispatcher.dart') {
      continue;
    }
    ...
    if (entity is Directory) {
      ...
      try {
        ...
        // Ignore locked/unreadable host files unused for framework analysis.
      }
    }
  }
}
