...
internal sealed partial class FrameworkCSharpLowerer
{
    ...
    {
        using var syntaxRegion = builder.BeginRegion(
            CsSyntaxRegionKind.Expression,
            ToCsOrigin(node.Origin)
        );
        ...
        switch (node.Kind)
        {
            case CoreNodeKind.SimpleIdentifier:
                if (
                    node.Text(CoreProperty.name) == "debugFormatDouble"
            ...
            case CoreNodeKind.IntegerLiteral:
            {
                ...
                if (
                    long.TryParse(literal, out var number)
                    && (
                        number > int.MaxValue
                        || number < int.MinValue
            ...
            case CoreNodeKind.SimpleStringLiteral:
                builder
                    .Append('"')
                    .Append(Escape(node.Text(CoreProperty.value) ?? string.Empty))
                    .Append('"');
                return;
            ...
            case CoreNodeKind.NamedExpression:
                if (
                    node.Children.FirstOrDefault(item =>
                        item.Category == "expression" && item.Kind != CoreNodeKind.Label
                    ) is
                    { } namedValue
                )
                {
            ...
            case CoreNodeKind.AssignmentExpression:
            {
                ...
                if (
                    ...
                    && left is not null
                    && right is not null
                    && left.Kind == CoreNodeKind.SimpleIdentifier
                )
                {
                    var leftName = left.Text(CoreProperty.name);
                    var leftType = left.StaticType;
                    if (
                        string.IsNullOrEmpty(leftType) && declaration.Element.Parameters is not null
                    )
                    {
                        var parameter = declaration.Element.Parameters.FirstOrDefault(item =>
                            item.Name == leftName
                        );
                ...
                if (
                    ...
                    && left is not null
                    && right is not null
                    && left.Kind == CoreNodeKind.IndexExpression
                )
                {
                    var indexParts = left
            ...
            case CoreNodeKind.IfNullExpression:
            {
                ...
                var right = node
                    .Children.Skip(1)
                    .FirstOrDefault(item => item.Category == "expression");
                if (left is not null && right is not null)
                {
                    if (
                        DescendantsAndSelf(left)
                            .Any(item => item.Text(CoreProperty.name) == "index")
                    )
                    {
                        LowerExpression(
                            builder,
                    ...
                    if (
                        (
                            IsValueType(mappedLeft)
                            && !mappedLeft.EndsWith("?", StringComparison.Ordinal)
                        )
                        || DescendantsAndSelf(left)
                            .Any(item => item.Text(CoreProperty.name) == "isInScribbleRect")
                    )
                    {
                        LowerExpression(
                            builder,
                    ...
                    if (
                        (MapType(node.StaticType ?? "object").TrimEnd('?') is "object" or "dynamic")
            ...
            case CoreNodeKind.FunctionReference:
            {
                ...
                if (target is null)
                {
                    AddUnsupportedDiagnostic(
                        ...
                        "Provide the resolved function tear-off target."
                    );
            ...
            case CoreNodeKind.FunctionExpressionInvocation:
            {
                ...
                if (
                    function?.Kind == CoreNodeKind.ConstructorReference
                    && DescendantsAndSelf(function)
                        .Any(item =>
                            item.Text(CoreProperty.name)
                                ?.Contains("GlobalKey", StringComparison.Ordinal) == true
                        )
                )
                {
                    var constructorName =
                        function.Child(CoreChildRole.constructorOffset)
                        ?? function.Children.FirstOrDefault(item =>
                            item.Kind == CoreNodeKind.ConstructorName
                        );
                    var globalKeyType = constructorName is null
                        ? "GlobalKey<IState>"
                ...
                {
                    var requiresDelegateCast =
                        function.StaticType?.Contains(" Function", StringComparison.Ordinal) == true
                        && function.Kind
                            is CoreNodeKind.FunctionExpression
                                or CoreNodeKind.SwitchExpression;
                    if (requiresDelegateCast)
                    {
                        builder
                            .Append("((")
                            .Append(MapType(function.StaticType ?? "object Function()"))
                            .Append(")");
                    }
                    LowerExpression(
                        builder,
                ...
                var functionArguments =
                    ...
                    ?? [];
                if (
                    function?.Text(CoreProperty.name) == "successCallback"
                ...
                )
                {
                    ...
                    function?.StaticType?.Contains("bool", StringComparison.Ordinal) == true
                    && functionArguments.Any(item =>
                        item.Kind == CoreNodeKind.NamedExpression
                        && item.Text(CoreProperty.name) == "allowUpscaling"
                    )
                )
                {
                    var orderedNames = new[] { "allowUpscaling", "cacheHeight", "cacheWidth" };
                    var positional = functionArguments.FirstOrDefault(item =>
                        item.Kind != CoreNodeKind.NamedExpression
                    );
                    ...
                    foreach (var orderedName in orderedNames)
                    {
                        ...
                        var named = functionArguments.FirstOrDefault(item =>
                            item.Kind == CoreNodeKind.NamedExpression
                            && item.Text(CoreProperty.name) == orderedName
                        );
                        var value = named?.Children.FirstOrDefault(item =>
                            item.Category == "expression"
                        );
                        if (value is null)
                        {
                            builder.Append(orderedName == "allowUpscaling" ? "false" : "null");
                        }
                        else
                        {
                            LowerExpression(
                ...
                else
                {
                    ...
                    if (
                        (
                            function?.Text(CoreProperty.name) == "onStateChange"
                            || (
                                declaration.Name == "AppLifecycleListener"
                                && functionArguments.Length == 1
                                && MapType(functionArguments[0].StaticType ?? string.Empty)
                                    .TrimEnd('?')
                                    .EndsWith("AppLifecycleState", StringComparison.Ordinal)
                            )
                        )
                        && functionArguments.Length == 1
                    )
            ...
            case CoreNodeKind.AsExpression:
            {
                ...
                if (expression is not null && typeNode is not null)
                {
                    ...
                    if (
                        (_session.ActiveDonorDeclaration ?? declaration).Name == "IndexedSlot"
                    ...
                    if (
                        (
                            nonNullableTargetType == "Size"
                            || nonNullableTargetType.EndsWith(".Size", StringComparison.Ordinal)
                        )
                        && (
                            sourceType == "Offset"
                            || sourceType.EndsWith(".Offset", StringComparison.Ordinal)
                        )
                    )
                    {
                    ...
                    if (
                        (
                            nonNullableTargetType == "Offset"
                            || nonNullableTargetType.EndsWith(".Offset", StringComparison.Ordinal)
                        )
                        && (
                            sourceType == "Size"
                            || sourceType.EndsWith(".Size", StringComparison.Ordinal)
                        )
                    )
                    {
                        builder.Append(
                            "global::Doroti.Ui.Offset.fromSize(DartRuntimePrimitives.RequireReference("
                        );
                        LowerExpression(
                            builder,
                            expression,
                    ...
                    if (
                        nonNullableTargetType is "Size" or "Offset"
                        || nonNullableTargetType.EndsWith(".Size", StringComparison.Ordinal)
                        || nonNullableTargetType.EndsWith(".Offset", StringComparison.Ordinal)
                    )
                    {
                    ...
                    if (targetType.StartsWith("DartMap<", StringComparison.Ordinal))
                    {
                        ...
                        builder
                            .Append("DartRuntimePrimitives.ConvertMap<")
                            .Append(string.Join(", ", mapArguments))
                            .Append(">((System.Collections.IDictionary)");
                        LowerExpression(
                            builder,
                            expression,
                            declaration,
                    ...
                    else if (IsValueType(targetType))
                    {
                        ...
                        if (IsUnboundTypeParameterName(sourceType))
                        {
                            builder.Append("(object)");
                        }
                ...
                if (expression is not null && typeNode is not null)
                {
                    ...
                    )
                    {
                        var declaredArguments =
                            ...
                            ?? [];
                        mappedIsType =
                            ResolveEmittedTypeName(typeNode, declaredTypeName)
                            + (
                    ...
                    if (
                        !mappedIsType.Contains('<', StringComparison.Ordinal)
                        && FindGlobalDeclaration(mappedIsType)
                            is { Element.TypeParameters.Length: > 0 } rawGenericType
                    )
                    {
                        mappedIsType +=
                            $"<{string.Join(", ", rawGenericType.Element.TypeParameters.Select(_ => "object"))}>";
                    }
                    var isClipStorage =
                        expression.Text(CoreProperty.name) == "_clip"
                    ...
                    if (
                        (
                            mappedIsType is "null" or "Null"
                            || typeNode.Text(CoreProperty.name) == "Null"
                        ) && isClipStorage
                    )
                    {
                        if (isNot)
                    ...
                    var mappedExpressionType = MapType(expression.StaticType ?? string.Empty)
                        .TrimEnd('?');
                    ...
                    if (
                        mappedIsType == "Enum"
                        && (
                            MapType(expression.StaticType ?? string.Empty)
                                .TrimEnd('?')
                                .StartsWith('_')
                            || FindGlobalDeclaration(
                                StripLibraryPrefix(
                                    (expression.StaticType ?? string.Empty).TrimEnd('?')
                                )
            ...
            {
                var typeNode = node.Children.FirstOrDefault(item =>
                ...
                if (typeNode is not null)
                {
                    ...
                    var isRuntimeTypeParameter =
                        !string.IsNullOrEmpty(literalName)
                        && owningDeclaration
                            .Members.Where(member => ContainsOffset(member.Ast, node.Offset))
                    ...
                    builder
                        .Append("typeof(")
                        .Append(literalType.TrimEnd('?') == "dynamic" ? "object" : literalType)
                        .Append(')');
                    return;
                }
                break;
            ...
            case CoreNodeKind.AdjacentStrings:
            {
                var values = node
                    .Children.Where(item =>
                        item.Kind
                            is CoreNodeKind.SimpleStringLiteral
                                or CoreNodeKind.StringInterpolation
                    )
                    .ToArray();
            ...
            case CoreNodeKind.AwaitExpression:
            {
                ...
                var handlerInvocation =
                    expression?.Kind == CoreNodeKind.FunctionExpressionInvocation
                    && DescendantsAndSelf(expression)
                        .Any(item =>
                            item.Kind == CoreNodeKind.SimpleIdentifier
                            && item.Text(CoreProperty.name) == "handler"
                        )
                ...
                if (handlerInvocation)
                {
                    ...
                    builder
                        .Append("((")
                        .Append(resultType)
                        .Append(")await DartAsyncRuntime.AwaitObject(");
                    LowerExpression(
                        builder,
                        expression!,
                        declaration,
                ...
                var awaitable =
                    expressionType == "Future"
                    || expressionType.StartsWith("Future<", StringComparison.Ordinal)
                    || futureOr;
                if (awaitable)
                {
                ...
                {
                    builder
                        .Append("DartAsyncRuntime.AwaitFutureOrValue<")
                        .Append(MapGenericArgument(node.StaticType ?? "object"))
                        .Append(">(");
                }
            ...
            {
                var expression = node.Children.FirstOrDefault(item =>
                    item.Category == "expression"
                );
                ...
                if (expression is not null)
                {
                    ...
                    var wrapThrownValue =
                        thrownType is "object" or "dynamic"
                        || !thrownType.EndsWith("Exception", StringComparison.Ordinal);
                    if (wrapThrownValue)
                    {
                        builder.Append("DartRuntimePrimitives.AsException(");
                    }
        ...
        AddUnsupportedDiagnostic(
            ...
            node,
            "expression",
            "Add a typed expression visitor for this AST node before selecting the containing library."
        );
    ...
    )
    {
        ...
        if (
            name == "simulation"
            && declaration.Name == "DrivenScrollActivity"
        ...
        if (
            name == "_disabledGeometry"
            && (_session.ActiveDonorDeclaration ?? declaration).Name == "_SelectionContainerState"
        )
        {
            builder
                .Append(MapType((_session.ActiveDonorDeclaration ?? declaration).Name))
        ...
        if (
            ...
            && string.Equals(
                currentLibrary,
                "package:flutter/src/widgets/_window.dart",
        ...
        if (
            name is "InheritedModel" or "ModalRoute"
        ...
        if (_session.EmittingAssignmentLeft && FindGlobalMember(elementId) is { IsStatic: false })
        {
            builder
                .Append(_session.ExplicitThisExpression ?? "this")
                .Append('.')
                .Append(SafeIdentifier(name));
            return;
        }
        ...
        if (elementId?.LastIndexOf("@local", StringComparison.Ordinal) is >= 0)
        {
            ...
            if (
                !_session.EmittingAssignmentLeft
                && NeedsReferenceTypePromotion(node, owningDeclaration)
            )
            {
                builder
                    .Append("((")
                    .Append(MapType(node.StaticType!))
                    .Append(')')
                    .Append(emittedName)
                    .Append(')');
            }
        ...
        if (
            DescendantsAndSelf(identifierOwner.Ast)
                .Any(item =>
        ...
        )
        {
            builder
                .Append("typeof(")
                .Append(EmittedTypeName(currentLibrary, declaration.Name))
                .Append('<')
                .Append(
                    string.Join(
                        ", ",
                        currentTypeParameters.Select(item => SafeIdentifier(item.Name))
                    )
                )
                .Append(">)");
            return;
        ...
        if (
            ...
            && StripLibraryPrefix(viewportBase)
                .StartsWith("RenderViewportBase<", StringComparison.Ordinal)
        )
        {
        ...
        if (
            _session.ActiveDonorDeclaration is { } donorDeclaration
            && donorDeclaration.Members.Any(member =>
                (member.Element.Parameters ?? []).Any(parameter =>
                    ...
                    && (
                        string.Equals(
                            elementId,
                            member.Element.CanonicalId + "." + parameter.Name,
                            StringComparison.Ordinal
                        )
        ...
        if (
            _session.ActiveDonorDeclaration is { } donorWithMember
        ...
        if (
            declaration.Ast.Kind == CoreNodeKind.EnumDeclaration
            && _session.ExplicitThisExpression is { } enumReceiver
            && name == "name"
        )
        {
        ...
        if (
            _session.ActiveSourceLibrary is { } sourceLibrary
            && _semanticIndex.FindDeclaration(sourceLibrary, name)
                is {
                    Ast.Kind: CoreNodeKind.FunctionDeclaration
                        or CoreNodeKind.TopLevelVariableDeclaration
                }
        )
        {
            builder
                .Append(QualifiedLibraryStaticClassName(sourceLibrary, currentLibrary))
                .Append('.')
                .Append(SafeIdentifier(name));
            return;
        }
        ...
        if (
            FindGlobalMember(elementId) is { IsStatic: false } inheritedMember
        ...
        if (
            _semanticIndex.DeclarationsBySimpleName.TryGetValue(name, out var namedDeclarations)
            && namedDeclarations.FirstOrDefault(candidate =>
                candidate.Ast.Kind == CoreNodeKind.TopLevelVariableDeclaration
            )
                is { } topLevelVariable
        )
        {
            ...
            builder
                .Append(QualifiedLibraryStaticClassName(ownerLibrary, currentLibrary))
                .Append('.')
                .Append(SafeIdentifier(name));
            return;
        }
        if (
            _session.ExplicitEnumDeclaration is { } explicitEnum
            && explicitEnum.Ast.Kind == CoreNodeKind.EnumDeclaration
            && elementId?.StartsWith(
                explicitEnum.Element.CanonicalId + ".",
                StringComparison.Ordinal
            ) == true
        )
        {
            builder
                .Append(
                    EmittedTypeName(
                        LibraryUriFromElementId(explicitEnum.Element.CanonicalId),
                        explicitEnum.Name
                    )
                )
                .Append('.')
        ...
        if (
            name == "kLongPressTimeout"
            && elementId?.Contains(
                "/gestures/constants.dart#kLongPressTimeout",
                StringComparison.Ordinal
            ) == true
        )
        {
        ...
        if (
            name == "Timer"
            && string.Equals(
                LibraryUriFromElementId(elementId),
                "dart:async",
                StringComparison.Ordinal
            )
        )
        ...
        {
            builder.Append(
                "global::Doroti.Framework.Services.DebugLibrary.debugProfilePlatformChannels"
            );
            return;
        }
        if (name == "utf8")
        ...
        if (name == "fromStandardMessageCodecMessage" && elementId is not null)
        {
            ...
            if (separator > 0)
            {
                builder
                    .Append(
                        EmittedTypeName(LibraryUriFromElementId(elementId), symbol[..separator])
                    )
        ...
        {
            builder
                .Append(LibraryStaticClassName(currentLibrary))
                .Append('.')
                .Append(SafeIdentifier(name));
            return;
        }
        ...
        if (
            ...
            && elementId != currentLibrary
            && IsTopLevelElement(elementId, "objectRuntimeType")
        )
        {
            builder.Append(
        ...
        if (
            name == "runtimeType"
            && elementId is not null
            && elementId.Contains("Object.runtimeType", StringComparison.Ordinal)
        )
        {
        ...
        if (
            name == "iterator"
            && (
                node.StaticType?.StartsWith("Iterator", StringComparison.Ordinal) == true
                || node.StaticType?.StartsWith("IEnumerator", StringComparison.Ordinal) == true
            )
        )
        {
        ...
        {
            var owningDeclaration = _session.ActiveDonorDeclaration ?? declaration;
            var isRuntimeTypeValue =
                elementId?.Contains("@local", StringComparison.Ordinal) == true
                || owningDeclaration
                    .Members.Where(member => ContainsOffset(member.Ast, node.Offset))
                    .SelectMany(member => member.Element.Parameters ?? [])
                    .Any(parameter =>
        ...
        if (elementId is not null)
        {
            ...
            if (marker >= 0)
            {
                ...
                {
                    var isLocalFunction = DescendantsAndSelf(declaration.Ast)
                        .Any(item =>
                            item.Kind == CoreNodeKind.FunctionDeclaration
                            && string.Equals(
                                item.Text(CoreProperty.name),
                    ...
                    builder
                        .Append(QualifiedLibraryStaticClassName(elementLibrary, currentLibrary))
                        .Append('.')
                        .Append(SafeIdentifier(name));
                    return;
                }
    ...
    private string EmittedLocalIdentifier(CoreAstNode node, string fallbackName)
    {
            ...
            if (owner is not null)
            {
                var scopes = DescendantsAndSelf(owner.Ast)
                    .Where(candidate =>
                        candidate.Kind
                            is CoreNodeKind.FunctionExpression
                                or CoreNodeKind.FunctionDeclarationStatement
                ...
                foreach (var scope in scopes)
                {
                    var resolvedLocal = DescendantsAndSelf(scope)
                        .Where(candidate =>
                            candidate.Offset <= node.Offset
                            && string.Equals(
                                candidate.Text(CoreProperty.name),
                                fallbackName,
                                StringComparison.Ordinal
                            )
                            && candidate.ElementId?.Contains("@local", StringComparison.Ordinal)
                                == true
                        )
                        .OrderByDescending(candidate => candidate.Offset)
                        .FirstOrDefault();
                    if (resolvedLocal is null)
                    {
                        continue;
                    }
        ...
        return offset.Length > 0 && offset.All(char.IsDigit) ? $"{safeName}__{offset}" : safeName;
    }
    ...
    )
    {
        ...
        if (condition is null || thenExpression is null || elseExpression is null)
        {
            AddUnsupportedDiagnostic(
                ...
                "Provide all resolved conditional expression operands in analyzer output."
            );
            builder.Append("throw new NotSupportedException(\"DOTF0001\")");
            return;
        }
        var conditionalDelegateDartType = new[]
        {
            node.StaticType,
            thenExpression.StaticType,
            elseExpression.StaticType,
        ...
        builder.Append(" ? ");
        var recordTargetType =
            thenExpression.Kind == CoreNodeKind.RecordLiteral
        ...
        var nullableTypeParameter = IsUnboundTypeParameterName(conditionalMappedType.TrimEnd('?'))
            ...
            : null;
        if (recordTargetType is not null)
        {
            builder.Append("((").Append(recordTargetType).Append(")");
        }
    ...
    )
    {
        ...
        foreach (var child in node.Children)
        {
            ...
            else if (child.Kind == CoreNodeKind.InterpolationExpression)
            {
                builder.Append('{');
                var expression =
                    child.Child(CoreChildRole.expressionOffset)
                    ?? child.Children.FirstOrDefault(item => item.Category == "expression");
                if (expression is null)
                {
                    AddUnsupportedDiagnostic(
                        ...
                        declaration,
                        child,
                        "interpolation-expression-shape",
                        "Provide the typed interpolation expression."
                    );
    ...
    )
    {
        ...
        if (left is null || right is null)
        {
            AddUnsupportedDiagnostic(
                ...
                node,
                "expression-shape",
                "The analyzer protocol must provide both typed operands."
            );
            builder.Append("throw new NotSupportedException(\"DOTF0001\")");
            return;
        }
        ...
        {
            while (
                value.Kind == CoreNodeKind.ParenthesizedExpression
        ...
        if (
            opToken == "-"
            && unwrappedRight.Kind == CoreNodeKind.IntegerLiteral
        ...
        var leftPropertyTarget =
            left.Kind == CoreNodeKind.PropertyAccess ? left.Child(CoreChildRole.targetOffset)
            : left.Kind == CoreNodeKind.PrefixedIdentifier
                ? left.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.SimpleIdentifier
        ...
        if (
            left.Kind == CoreNodeKind.IndexExpression
            && left.Children.FirstOrDefault(candidate => candidate.Category == "expression")
                is { } indexedTarget
            && TryGetGenericTypeArguments(
                MapType(indexedTarget.StaticType ?? string.Empty).TrimEnd('?'),
                out var indexedArguments
            )
        ...
        if (
            string.IsNullOrEmpty(leftDartType) && left.Text(CoreProperty.name) is { } assignmentName
        )
        {
            if (left.ElementId?.Contains("@local", StringComparison.Ordinal) == true)
            {
                leftDartType = DescendantsAndSelf(declaration.Ast)
                    .Where(candidate =>
                        string.Equals(candidate.ElementId, left.ElementId, StringComparison.Ordinal)
            ...
            if (
                !string.IsNullOrWhiteSpace(targetDartType)
                && FindGlobalDeclaration(targetDartType) is { } targetOwner
            )
            {
            ...
            if (
                ...
                && (
                    ...
                    || (
                        ...
                        && DescendantsAndSelf(left)
                            .Any(candidate => candidate.Text(CoreProperty.name) == "_errorText")
                    )
                )
            )
            {
            ...
            if (
                leftPropertyTarget is null
                || leftPropertyTarget.Kind
                    is CoreNodeKind.ThisExpression
                        or CoreNodeKind.SuperExpression
            )
            {
            ...
            if (
                string.IsNullOrWhiteSpace(leftDartType)
        ...
        if (
            opToken == "="
            && activeAssignmentOwner.Name == "PrioritizedAction"
        ...
        )
        {
            ...
            opToken == "="
            && activeAssignmentOwner.Name == "_HighlightModeManager"
            && left.Text(CoreProperty.name) == "_listeners"
        )
        {
            mappedLeftType =
                "global::Doroti.Framework.Foundation.HashedObserverList<global::System.Action<FocusHighlightMode>>";
        }
        if (
            opToken == "="
            && activeAssignmentOwner.Name == "FormFieldState"
        ...
        if (
            opToken == "="
            && right.Kind == CoreNodeKind.InstanceCreationExpression
            && !DescendantsAndSelf(right)
                .Any(candidate =>
                    candidate.Kind == CoreNodeKind.ArgumentList
        ...
        if (
            opToken == "="
            && right.Kind == CoreNodeKind.NullLiteral
        ...
        if (
            opToken == "="
            && left.Text(CoreProperty.name) == "length"
            && leftPropertyTarget
                ?.StaticType?.TrimEnd('?')
                .StartsWith("List<", StringComparison.Ordinal) == true
        )
        {
            LowerExpression(
                builder,
        ...
        if (
            opToken == "="
            && left.Kind == CoreNodeKind.IndexExpression
            && left.Children.FirstOrDefault(item => item.Category == "expression")
                ?.StaticType?.TrimEnd('?') == "Int32List"
        )
        {
            _session.EmittingAssignmentLeft = true;
            try
        ...
        if (
            opToken == "="
            && mappedLeftType.StartsWith("DartMap<", StringComparison.Ordinal)
            && mappedRightType.StartsWith("DartMap<", StringComparison.Ordinal)
            && mappedLeftType != mappedRightType
        )
        {
            LowerAssignmentTarget();
            builder.Append(" = ");
        ...
        if (
            opToken == "="
            && mappedLeftType.StartsWith("List<", StringComparison.Ordinal)
        ...
        if (
            opToken == "="
            && TryGetDelegateAdapterParameter(
                mappedLeftType,
                mappedRightType,
                out var delegateParameter
            )
        )
        {
            LowerAssignmentTarget();
        ...
        if (
            opToken == "??"
            && left.Kind == CoreNodeKind.MethodInvocation
            && left.Text(CoreProperty.name) == "call"
            && left.Child(CoreChildRole.targetOffset) is { } nullableCallable
        ...
        )
        {
            ...
            EmitArguments(
                builder,
                left.Child(CoreChildRole.argumentsOffset),
                declaration,
                package,
                library,
                inputPath,
        ...
        while (
            coalesceComparison.Kind == CoreNodeKind.ParenthesizedExpression
        ...
        if (
            opToken == "??"
            && coalesceComparison.Kind == CoreNodeKind.BinaryExpression
        ...
        if (
            opToken == "="
            && IsValueType(mappedLeftType.TrimEnd('?'))
        ...
        if (
            opToken == "="
            && !IsValueType(mappedLeftType.TrimEnd('?'))
            && !IsValueType(mappedRightType.TrimEnd('?'))
            && mappedLeftType.Length > 0
            && mappedRightType.Length > 0
            ...
            && !string.Equals(
                mappedLeftType.TrimEnd('?'),
                mappedRightType.TrimEnd('?'),
                StringComparison.Ordinal
            )
        )
        {
        ...
        if (
            opToken == "="
            && right.Kind == CoreNodeKind.SimpleIdentifier
        ...
        if (
            opToken == "??"
            && left.StaticType is { } coalesceType
            && IsValueType(MapType(coalesceType))
            && !MapType(coalesceType).EndsWith("?", StringComparison.Ordinal)
        )
        {
            LowerExpression(builder, left, declaration, package, library, inputPath, diagnostics);
            return;
        ...
        if (
            opToken == "??"
            && mappedLeftType.TrimEnd('?').StartsWith("DartMap<", StringComparison.Ordinal)
        ...
        )
        {
            ...
            builder
                .Append(".cast<")
                .Append(DartMapTypeArguments(mappedLeftType.TrimEnd('?')))
                .Append(">())");
            return;
        }
        if (
            opToken == "??"
            && mappedLeftType.TrimEnd('?') is not ("object" or "dynamic" or "void" or "")
            && (
                right.StaticType?.Contains(" Function", StringComparison.Ordinal) == true
                || FindGlobalMember(right.ElementId)?.Kind == "method"
            )
        ...
        var unrelatedCoalesceTypes =
            coalesceLeftDeclaration is not null
            && coalesceRightDeclaration is not null
        ...
        if (
            ...
            && mappedLeftType.TrimEnd('?') != mappedRightType.TrimEnd('?')
            && (
                (MapType(node.StaticType ?? string.Empty).TrimEnd('?') is "object" or "dynamic")
                || unrelatedCoalesceTypes
            )
        )
        {
            builder.Append("((object?)");
        ...
        if (opToken is "==" or "!=")
        {
            ...
            if (
                IsPrimitiveEqualityType(leftEqualityType)
                && IsPrimitiveEqualityType(rightEqualityType)
            )
            {
                LowerExpression(
                    builder,
        ...
        var assignmentOperatorDeclaration =
            FindGlobalDeclaration((left.StaticType ?? mappedLeftType).TrimEnd('?'))
            ?? FindGlobalDeclaration((right.StaticType ?? mappedRightType).TrimEnd('?'));
        var hasUserAssignmentOperator =
            userOperator is not null
            && (opToken == "+=" || opToken == "-=")
            && (
                ...
                || assignmentOperatorDeclaration?.Members.Any(member =>
                    member.IsOperator && member.Name == opToken[..1]
                ) == true
            );
        ...
        if (opToken == "~/")
        {
            if (
                mappedLeftType.TrimEnd('?')
                is not ("int" or "long" or "double" or "num" or "float" or "decimal")
            )
            {
                LowerExpression(
                    builder,
        ...
        var containsDeclaredTypeParameter = left
            .Children.Append(left)
            .Concat(right.Children.Append(right))
            .SelectMany(DescendantsAndSelf)
            .Any(candidate =>
                candidate.StaticType is { } candidateType
        ...
        if (
            ...
            && (
                ...
                || (
                    IsUnboundTypeParameterName(
                        MapType(node.StaticType ?? string.Empty).TrimEnd('?')
        ...
        if (
            ...
            && (
                string.Equals(
                    (left.StaticType ?? string.Empty).TrimEnd('?'),
                    declaration.Name,
                    StringComparison.Ordinal
                )
                || FindGlobalDeclaration(mappedLeftType.TrimEnd('?'))
                    ?.Members.Any(member => member.IsOperator && member.Name == opToken) == true
                || FindGlobalDeclaration(mappedRightType.TrimEnd('?'))
                    ?.Members.Any(member => member.IsOperator && member.Name == opToken) == true
            )
        )
        {
        ...
        var assignment =
            node.Text(CoreProperty.@operator) is { } op && (op == "=" || op.EndsWith('='));
        if (assignment)
        {
            _session.EmittingAssignmentLeft = true;
        }
    ...
    )
    {
        ...
        if (
            string.IsNullOrWhiteSpace(appliedOwnerType)
    ...
    private string? ResolvedExpressionValueType(CoreAstNode expression)
    {
        ...
        var storageType =
            member is not null && owner is not null
        ...
        var resolvedType =
            member?.IsGetter == true && storageType is not null
    ...
    private bool IsPrimitiveEqualityType(string type) =>
        type is "bool" or "int" or "double" or "num" or "String" or "string";

    private static bool TryGetDelegateAdapterParameter(
        string leftType,
        string rightType,
    ...
    )
    {
        ...
        var leftGesture =
            left.EndsWith("GestureTapCallback", StringComparison.Ordinal)
            || left.EndsWith("GestureLongPressCallback", StringComparison.Ordinal);
        var rightGesture =
            right.EndsWith("GestureTapCallback", StringComparison.Ordinal)
        ...
        var leftDragAction =
            left.StartsWith("Action<", StringComparison.Ordinal)
            && left.Contains("DragUpdateDetails", StringComparison.Ordinal);
        var rightDragAction =
            right.StartsWith("Action<", StringComparison.Ordinal)
            && right.Contains("DragUpdateDetails", StringComparison.Ordinal);
    ...
    {
        if (
            DescendantsAndSelf(operand)
                .Any(item =>
                    item.Text(CoreProperty.name) is "presentError" or "_reportStructuredError"
                )
        )
        {
    ...
    )
    {
        ...
        if (left is null || right is null)
        {
            AddUnsupportedDiagnostic(
                ...
                node,
                "expression-shape",
                "The analyzer protocol must provide both typed operands for equality."
            );
            builder.Append("throw new NotSupportedException(\"DOTF0001\")");
            return;
        }
        if (left.Kind == CoreNodeKind.NullLiteral || right.Kind == CoreNodeKind.NullLiteral)
        {
            ...
            var isClipStorage =
                nonNull.Text(CoreProperty.name) == "_clip"
                || DescendantsAndSelf(nonNull)
                    .Any(item =>
                        item.Kind == CoreNodeKind.SimpleIdentifier
                        && item.Text(CoreProperty.name) == "_clip"
                    );
        ...
        if (
            !IsTypeParameter(left.StaticType, declaration)
            && !IsTypeParameter(right.StaticType, declaration)
        )
        {
            builder.Append('(');
