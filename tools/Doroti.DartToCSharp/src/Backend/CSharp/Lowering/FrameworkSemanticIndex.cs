...
internal sealed class FrameworkSemanticIndex
{
    ...
    public FrameworkSemanticIndex(IEnumerable<CoreResolvedDeclaration> declarations)
    {
        AllDeclarations = declarations
            ...
            .ToArray();
        DeclarationsBySimpleName = new ReadOnlyDictionary<string, CoreResolvedDeclaration[]>(
            AllDeclarations
        ...
        _declarationsByCanonicalId = new ReadOnlyDictionary<string, CoreResolvedDeclaration>(
            AllDeclarations
                .GroupBy(item => item.Element.CanonicalId, StringComparer.Ordinal)
                .ToDictionary(group => group.Key, group => group.First(), StringComparer.Ordinal)
        );
        _declarationsByLibraryAndName = new ReadOnlyDictionary<
            (string Library, string Name),
            CoreResolvedDeclaration
        >(
            AllDeclarations
        ...
        _declarationsByEmittedName = new ReadOnlyDictionary<string, CoreResolvedDeclaration[]>(
            AllDeclarations
                .GroupBy(
                    item => EmittedName(Library(item.Element.CanonicalId), item.Name),
                    StringComparer.Ordinal
                )
                .ToDictionary(
                    group => group.Key,
        ...
        var members = AllDeclarations
            .SelectMany(declaration =>
                declaration.Members.Select(member => (Declaration: declaration, Member: member))
            )
            .Where(item => !string.IsNullOrEmpty(item.Member.Element.CanonicalId))
            .ToArray();
        _membersByCanonicalId = new ReadOnlyDictionary<string, CoreResolvedMember>(
            members
                .GroupBy(item => item.Member.Element.CanonicalId, StringComparer.Ordinal)
                .ToDictionary(
                    group => group.Key,
                    group => group.First().Member,
                    StringComparer.Ordinal
                )
        );
        _memberOwnersByCanonicalId = new ReadOnlyDictionary<string, CoreResolvedDeclaration>(
            members
                .GroupBy(item => item.Member.Element.CanonicalId, StringComparer.Ordinal)
                .ToDictionary(
                    group => group.Key,
                    group => group.First().Declaration,
                    StringComparer.Ordinal
                )
        );
        ...
        var typeUsers = new Dictionary<string, List<CoreResolvedDeclaration>>(
            StringComparer.Ordinal
        );
        foreach (var declaration in AllDeclarations)
        {
            foreach (
                var type in DirectReferencedTypes(declaration)
                    .SelectMany(TypeIdentifiers)
                    .Where(item => item.Length > 0)
                    .Distinct(StringComparer.Ordinal)
            )
            {
                if (!typeUsers.TryGetValue(type, out var users))
                {
        ...
        _typeUsersBySimpleName = new ReadOnlyDictionary<string, CoreResolvedDeclaration[]>(
            typeUsers.ToDictionary(
                item => item.Key,
                item =>
                    item.Value.Distinct()
                        .OrderBy(value => value.Element.CanonicalId, StringComparer.Ordinal)
                        .ToArray(),
                StringComparer.Ordinal
            )
        );
        ...
        foreach (var declaration in AllDeclarations)
        {
            ...
            while (pending.Count > 0)
            {
                ...
                if (!found.TryAdd(candidate.Element.CanonicalId, candidate))
                {
                    continue;
                }

                foreach (var child in TypeUsers(candidate.Name))
                {
                    pending.Enqueue(child);
                }
            }
            descendants[declaration.Name] = found
                .Values.OrderBy(item => item.Element.CanonicalId, StringComparer.Ordinal)
                .ToArray();
        }
        _descendantsBySimpleName = new ReadOnlyDictionary<string, CoreResolvedDeclaration[]>(
            descendants
        );
        MixinDeclarations = AllDeclarations
            .Where(item => item.Ast.Kind == CoreNodeKind.MixinDeclaration)
            .ToArray();
    }
    ...
    public CoreResolvedDeclaration? FindDeclarationByCanonicalId(string? canonicalId) =>
        ...
        && _declarationsByCanonicalId.TryGetValue(canonicalId, out var declaration)
            ? declaration
            : null;

    public CoreResolvedDeclaration? FindDeclaration(string? simpleName) =>
        simpleName is not null
        && DeclarationsBySimpleName.TryGetValue(SimpleTypeName(simpleName), out var matches)
        && matches.Length == 1
            ? matches[0]
            : null;
    ...
    public CoreResolvedMember? FindMember(string? canonicalId) =>
        canonicalId is not null && _membersByCanonicalId.TryGetValue(canonicalId, out var member)
            ? member
            : null;

    public CoreResolvedDeclaration? FindMemberOwner(CoreResolvedMember member) =>
        _memberOwnersByCanonicalId.TryGetValue(member.Element.CanonicalId, out var declaration)
            ? declaration
            : null;

    public CoreResolvedDeclaration? FindDeclaration(string library, string name) =>
        _declarationsByLibraryAndName.TryGetValue((library, name), out var declaration)
            ? declaration
            : null;

    public bool HasDeclaration(string library, string name) =>
        _declarationsByLibraryAndName.ContainsKey((library, name));
    ...
    public bool IsEnumGetter(string? memberId) =>
        memberId is not null && _enumGetterIds.Contains(memberId);

    public CoreResolvedDeclaration[] TypeUsers(string typeName) =>
        _typeUsersBySimpleName.TryGetValue(SimpleTypeName(typeName), out var declarations)
            ? declarations
            : [];

    public CoreResolvedDeclaration[] Descendants(string typeName) =>
        _descendantsBySimpleName.TryGetValue(SimpleTypeName(typeName), out var declarations)
            ? declarations
            : [];
