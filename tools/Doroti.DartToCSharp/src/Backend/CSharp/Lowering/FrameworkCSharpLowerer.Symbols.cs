...
internal sealed partial class FrameworkCSharpLowerer
{
    ...
    private IEnumerable<CoreResolvedDeclaration> AppliedMixinDeclarations(
    ...
    private string? ConcreteImplementedClass(CoreResolvedDeclaration declaration) =>
        (declaration.Element.Interfaces ?? []).FirstOrDefault(interfaceType =>
        {
    ...
    private CoreResolvedDeclaration? DisplacedStructuralSuperclass(
        CoreResolvedDeclaration declaration
    )
    {
        if (
            !declaration.Name.StartsWith("_TransformedPointer", StringComparison.Ordinal)
    ...
    private IEnumerable<CoreResolvedDeclaration> ImplementationDonorDeclarations(
        CoreResolvedDeclaration declaration
    ) =>
    ...
    )
    {
        ...
        var appliedType = (application.Element.Mixins ?? [])
            ...
            .FirstOrDefault(type =>
                string.Equals(type.Split('<')[0], donor.Name, StringComparison.Ordinal)
            );
    ...
    )
    {
        ...
        if (declaration.Element.Supertype is { } supertype)
        {
            ...
            )
            {
                ...
                for (
                    var index = 0;
                    index < Math.Min(superParameters.Length, arguments.Length);
                    index++
                )
                {
    ...
    private bool IsPublicMixinStorageType(string name)
    {
        if (
            name
            is "_CreationParams"
                or "_MutableTextRange"
                or "_NoopMouseCursorSession"
                or "_NoopMouseCursor"
        )
        {
            return false;
        ...
        if (
            declarations.Any(candidate =>
                !IsDartPrivate(candidate)
                && DirectBaseNames(candidate)
        ...
        return declarations
            .Where(candidate =>
                !IsDartPrivate(candidate)
                || _semanticIndex
                    .TypeUsers(candidate.Name)
                    .Any(user =>
                        !string.Equals(user.Name, candidate.Name, StringComparison.Ordinal)
                        && user.Members.Where(member =>
                                !IsDartPrivate(member)
                                || (
                                    user.Ast.Kind == CoreNodeKind.MixinDeclaration
                                    && WillEmitAsInterface(user)
                                )
                            )
                            .SelectMany(member =>
            ...
            .SelectMany(candidate =>
                candidate.Members.Where(member =>
                    !IsDartPrivate(member)
                    || (
                        candidate.Ast.Kind == CoreNodeKind.MixinDeclaration
                        && WillEmitAsInterface(candidate)
            ...
            .SelectMany(member =>
                new[] { member.Element.Type, member.Element.ReturnType }.Concat(
                    (member.Element.Parameters ?? []).Select(parameter => parameter.Type)
                )
            )
            .Where(type => !string.IsNullOrWhiteSpace(type))
            .Any(type => Regex.IsMatch(type!, typePattern, RegexOptions.CultureInvariant));
    ...
    private bool HasSelectedMixinApplication(CoreResolvedDeclaration mixin) =>
        _semanticIndex
            .TypeUsers(mixin.Name)
            .Any(candidate =>
                (candidate.Element.Mixins ?? []).Any(type =>
                    string.Equals(
    ...
    private static bool HasPromotedClassRepresentation(string declarationName) =>
        declarationName
            is "GestureBinding"
                or "SchedulerBinding"
                or "ServicesBinding"
                or "DiagnosticableTreeMixin"
                or "RenderObjectWithLayoutCallbackMixin"
                or "CustomClipper"
                or "TextScaler"
                or "ValueNotifier"
    ...
    private static bool HasPromotedInterfaceRepresentation(string declarationName) =>
        declarationName
            is "Diagnosticable"
                ...
                or "Listenable"
                or "ValueListenable"
                or "AutofillScopeMixin"
                or "TextSelectionDelegate"
                or "TextInputClient"
                or "DeltaTextInputClient"
                or "SelectionRegistrant"
                or "UndoManagerClient"
                or "RenderSliverBoxChildManager"
                or "PaintingBinding"
                or "RendererBinding"
                or "SemanticsBinding"
                or "ViewportNotificationMixin";
        ...
        || (
            ...
            && (
                ...
                || (
                    ...
                    && HasSelectedMixinApplication(declaration)
                )
                || (
                    ...
                    && declaration.Members.All(member =>
                        member.IsStatic || member.IsAbstract || member.Kind == "constructor"
                    )
                )
            )
        );
    ...
    private string QualifiedLibraryStaticClassName(string libraryUri, string currentLibrary)
    {
        ...
        if (
            libraryUri.Contains("/widgets/", StringComparison.Ordinal)
    ...
    private bool HasNullableValueStorage(CoreAstNode node, CoreResolvedDeclaration declaration)
    {
        ...
        if (!IsValueType(mappedType.TrimEnd('?')))
        {
            return false;
        }
        ...
        if (
            FindGlobalMember(elementId) is { } resolvedMember
            && (resolvedMember.Element.ReturnType ?? resolvedMember.Element.Type)?.EndsWith(
                "?",
                StringComparison.Ordinal
            ) == true
        )
        ...
        if (HasNullableMemberStorage(node, name))
        {
            return true;
        }
        ...
        if (
            ...
            && AssignmentStorageType(_session.ActiveDonorDeclaration ?? declaration, name, null)
    ...
    {
        if (
            node.StaticType is null
            || node.StaticType.EndsWith("?", StringComparison.Ordinal)
        ...
        if (
            declaration
                .Members.SelectMany(member => member.Element.Parameters ?? [])
                .Any(parameter =>
    ...
    private bool HasNullableMemberStorage(CoreAstNode node, string? name)
    {
        ...
        var target =
            node.Kind == CoreNodeKind.PropertyAccess ? node.Child(CoreChildRole.targetOffset)
        ...
        var owner =
            FindGlobalDeclaration(StripLibraryPrefix(targetType.TrimEnd('?')))
            ?? FindGlobalDeclaration(MapType(targetType).TrimEnd('?'));
        return owner is not null
            && AssignmentStorageType(owner, name, targetType)
                ?.EndsWith("?", StringComparison.Ordinal) == true;
    }

    private static bool IsLocalOrParameter(CoreAstNode node, CoreResolvedDeclaration declaration)
    {
        if (node.ElementId?.Contains("@local", StringComparison.Ordinal) == true)
        {
            return true;
        }
        ...
        return name is not null
            && (declaration.Element.Parameters ?? [])
                .Concat(declaration.Members.SelectMany(member => member.Element.Parameters ?? []))
    ...
    private bool NeedsReferenceTypePromotion(CoreAstNode node, CoreResolvedDeclaration declaration)
    {
        ...
        if (
            mapped is "object" or "object?"
            || mapped.StartsWith("Func<", StringComparison.Ordinal)
            || mapped.StartsWith("Action<", StringComparison.Ordinal)
            || mapped is "Action" or "Action?"
        ...
        if (
            FindGlobalMember(node.ElementId) is { } resolvedMember
        ...
        return DescendantsAndSelf(declaration.Ast)
            .Any(item =>
                item.Kind == CoreNodeKind.SimpleIdentifier
                ...
                && (
                    ...
                    || (
                        item.StaticType is { } candidateType
                        && ShouldCastInvocationArgument(MapType(candidateType), mapped)
                    )
                )
            );
    }
    ...
    private bool HasOverrideAnnotation(CoreAstNode node) =>
        node.Children.Any(child =>
            child.Kind == CoreNodeKind.Annotation
    ...
    private bool HasAwaitOutsideNestedFunctions(CoreAstNode node)
    {
        foreach (var child in node.Children)
        {
            ...
            if (child.Kind == CoreNodeKind.AwaitExpression || HasAwaitOutsideNestedFunctions(child))
            {
                return true;
            }
        }
    ...
    {
        if (
            node.Text(CoreProperty.isAsync) == "true"
            || node.Children.Any(child => child.Text(CoreProperty.isAsync) == "true")
        )
        {
            return true;
        }

        if (HasAwaitOutsideNestedFunctions(node))
        {
            return true;
        }

        var body =
            node.Kind is CoreNodeKind.BlockFunctionBody
                ? node
        ...
        return int.TryParse(contentOffsetText, out var contentOffset)
            && contentOffset > body.Offset;
    }

    private bool IsInsideDartAsyncFunction(
        CoreResolvedDeclaration declaration,
    ...
    )
    {
        ...
        var enclosingFunction = DescendantsAndSelf(owningDeclaration.Ast)
            .Where(item =>
                ...
                && ContainsOffset(item, statement.Offset)
            )
        ...
        var localFunction = DescendantsAndSelf(owningDeclaration.Ast)
            .Where(item =>
                item.Kind == CoreNodeKind.FunctionDeclarationStatement
                && ContainsOffset(item, statement.Offset)
            )
            .OrderBy(item => item.Length)
            .FirstOrDefault();
        ...
        {
            return DescendantsAndSelf(localFunction)
                .Any(item =>
                    item.Kind == CoreNodeKind.FunctionExpression
                    && item.Text(CoreProperty.isAsync) == "true"
                );
        }
        var member = owningDeclaration
            .Members.Where(item => ContainsOffset(item.Ast, statement.Offset))
    ...
    private string ContainingReturnType(CoreResolvedDeclaration declaration, CoreAstNode statement)
    {
        ...
        var localFunction = DescendantsAndSelf(owningDeclaration.Ast)
            ...
            .OrderBy(item => item.Length)
            .FirstOrDefault();
        var localFunctionType = localFunction
            ?.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.FunctionDeclaration)
        ...
        var owningMember = owningDeclaration
            .Members.Where(item => ContainsOffset(item.Ast, statement.Offset))
            .OrderBy(item => item.Ast.Length)
            .FirstOrDefault();
        var contractMember = owningMember is null
            ? null
            : FindBaseContractMember(owningDeclaration, owningMember);
        return MapType(
            owningMember?.Element.ReturnType
    ...
    private bool ExpressionProducesFuture(CoreAstNode expression) =>
        expression.StaticType is { } type
        && (type == "Future" || type.StartsWith("Future<", StringComparison.Ordinal));

    private bool HasOverridableBaseMember(
        CoreResolvedDeclaration declaration,
        CoreResolvedMember member
    ) => FindOverriddenBaseMember(declaration, member) is not null;

    private CoreResolvedMember? FindOverriddenBaseMember(
        CoreResolvedDeclaration declaration,
        CoreResolvedMember member
    ) => FindBaseMember(declaration, member, includeInterfaces: false);

    private CoreResolvedMember? FindBaseContractMember(
        CoreResolvedDeclaration declaration,
        CoreResolvedMember member
    ...
    )
    {
        ...
        return superDeclaration?.Members.FirstOrDefault(candidate =>
    ...
    )
    {
        ...
        while (pending.Count > 0)
        {
            ...
            if (string.IsNullOrEmpty(baseName) || baseName == "Object" || !visited.Add(baseName))
            {
                continue;
            }
            ...
            if (generic >= 0)
            {
                simpleName = simpleName[..generic];
            }

            var baseDeclaration =
                _currentDeclarations?.FirstOrDefault(candidate => candidate.Name == simpleName)
            ...
            var effectiveMembers = baseDeclaration.Members.Concat(
                AppliedMixinDeclarations(baseDeclaration).SelectMany(mixin => mixin.Members)
            );
            ...
            foreach (var parent in ClrBaseNames(baseDeclaration, includeInterfaces))
            {
    ...
    private IEnumerable<string> ClrBaseNames(
        CoreResolvedDeclaration declaration,
        bool includeInterfaces
    )
    {
        ...
        if (
            ...
            && ConcreteImplementedClass(declaration) is { } structuralBase
        )
        {
        ...
        )
        {
            ...
            (declaration.Element.Mixins ?? []).FirstOrDefault(type =>
                FindGlobalDeclaration(MapType(type)) is { } mixin && !WillEmitAsInterface(mixin)
            ) is
            { } classMixin
        )
        {
        ...
        foreach (
            var candidate in (declaration.Element.Mixins ?? []).Concat(
    ...
    private IEnumerable<string> DirectBaseNames(CoreResolvedDeclaration declaration)
    {
        ...
        foreach (var mixin in declaration.Element.Mixins ?? [])
        {
            yield return mixin;
        }
    ...
    private IEnumerable<CoreResolvedDeclaration> InterfaceContractDeclarations(
        CoreResolvedDeclaration declaration
    )
    {
        yield return declaration;
        foreach (
            var baseDeclaration in DirectBaseNames(declaration)
    ...
    private bool IsKnownExternalOverride(
        CoreResolvedDeclaration declaration,
        CoreResolvedMember member
    ) =>
        DirectBaseNames(declaration)
            .Any(baseName =>
            {
                ...
                return simpleBase switch
                {
                    ...
                    "BindingBase" when HasOverrideAnnotation(member.Ast) => member.Name
                        is "initInstances"
                            or "initServiceExtensions"
                            or "unlocked"
                            or "performReassemble"
                            or "debugCheckZone"
                            or "reassembleApplication",
                    "GestureRecognizer" when HasOverrideAnnotation(member.Ast) => member.Name
                        is "debugDescription"
                            or "dispose"
                            or "acceptGesture"
                            or "rejectGesture",
                    "OneSequenceGestureRecognizer" when HasOverrideAnnotation(member.Ast) =>
                        member.Name
                            is "debugDescription"
                                or "addAllowedPointer"
                                or "didStopTrackingLastPointer"
                                or "handleEvent"
                                or "acceptGesture"
                                or "rejectGesture"
                                or "stopTrackingPointer"
                                or "dispose",
                    _ => false,
                };
            });
    ...
    private CoreResolvedDeclaration? FindGlobalDeclaration(string mappedType)
    {
        ...
        if (
            simpleName == "IntentAction"
            && _semanticIndex.DeclarationsBySimpleName.TryGetValue("Action", out var actions)
        )
        {
            return actions.FirstOrDefault(d =>
                LibraryUriFromElementId(d.Element.CanonicalId)
    ...
    private CoreResolvedMember? FindGlobalMember(string? elementId) =>
        _semanticIndex.FindMember(elementId);
    ...
    private bool IsRequiredByMixinConstraint(CoreResolvedMember member)
    {
        ...
        foreach (var mixin in _semanticIndex.MixinDeclarations)
        {
            foreach (var constraintName in DirectBaseNames(mixin))
            {
                ...
                if (
                    constraint?.Members.Any(candidate =>
                        candidate.Element.CanonicalId == member.Element.CanonicalId
                    ) == true
                )
                {
                    return true;
    ...
    private bool IsInterfaceDeclaration(CoreResolvedDeclaration declaration) =>
        ...
        && (
            ...
            || (
                ...
                && declaration.Members.All(candidate =>
                    candidate.IsStatic || candidate.IsAbstract || candidate.Kind == "constructor"
                )
            )
        );
    ...
    private bool TryResolveEmittedNamedConstructor(
        string typeName,
    ...
    )
    {
        ...
        {
            var representationConstructor = declaration
                .Ast.Children.FirstOrDefault(item =>
    ...
    private CoreResolvedMember? PrimaryGenerativeConstructor(CoreResolvedMember[] constructors) =>
        constructors.FirstOrDefault(item =>
            !item.IsFactory
            && !DescendantsAndSelf(item.Ast)
