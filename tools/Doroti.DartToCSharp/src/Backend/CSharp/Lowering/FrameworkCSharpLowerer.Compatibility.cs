...
{
    private bool IsPinnedG31CompatibilityLibrary(string library) =>
        library
            is "package:flutter/src/foundation/object.dart"
                or "package:flutter/src/foundation/annotations.dart"
                or "package:flutter/src/physics/tolerance.dart";

    private bool TryEmitPinnedG31Declaration(
    ...
    )
    {
        ...
        if (
            library == "package:flutter/src/physics/tolerance.dart"
            && declaration.Name == "Tolerance"
        )
        {
    ...
    )
    {
        ...
        var invocationElementId =
            node.ElementId
            ?? node.Children.FirstOrDefault(child =>
                child.Kind == CoreNodeKind.SimpleIdentifier
                && string.Equals(
                    child.Text(CoreProperty.name),
                    methodName,
                    StringComparison.Ordinal
                )
            )?.ElementId;
        var invocationArguments = node.Child(CoreChildRole.argumentsOffset);
        var thenArguments = invocationArguments
            ?.Children.Where(item => item.Category is "expression" or "argument")
            .ToArray();
        if (
            methodName == "Create"
            && target is not null
            && DescendantsAndSelf(target)
                .Any(item =>
                    item.Text(CoreProperty.name)
                        ?.Contains("DiagnosticsProperty", StringComparison.Ordinal) == true
                )
        )
        {
            builder.Append("new ");
        ...
        if (
            declaration.Name == "HtmlElementView"
        ...
        )
        {
            ...
            builder.Append(
                "throw new NotSupportedException(\"HtmlElementView is only available on Flutter Web\")"
            );
            return;
        }
        if (
            ...
            && methodName == "of"
            && DescendantsAndSelf(target)
                .Any(candidate => candidate.Text(CoreProperty.name) == "CreationLocation")
        )
        {
        ...
        {
            builder.Append(
                "global::Doroti.Framework.Rendering.RenderViewportBase<ParentDataClass>.showInViewport("
            );
            EmitArguments(
                builder,
                invocationArguments,
        ...
        if (methodName == "addPointer" && target is not null && thenArguments is { Length: 1 })
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append(
                ".addPointer((global::Doroti.Framework.Gestures.PointerDownEvent)(object)"
            );
            LowerExpression(
                builder,
                thenArguments[0],
        ...
        if (
            ...
            && MapType(thenArguments[0].StaticType ?? string.Empty)
                .Contains("Future", StringComparison.Ordinal)
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append(
        ...
        )
        {
            ...
            _session.ActiveFunctionReturnType =
                callbackType.StartsWith("Action", StringComparison.Ordinal) ? "void"
        ...
        if (
            methodName == "invokeLayoutCallback"
            && invocationArguments?.Children.FirstOrDefault(item => item.Category == "expression")
                is { Kind: CoreNodeKind.FunctionExpression } layoutCallback
            && layoutCallback
                .Children.FirstOrDefault(item => item.Kind == CoreNodeKind.FormalParameterList)
                ?.Children.FirstOrDefault(item => item.Category == "parameter")
                is { } callbackParameter
            && layoutCallback
                .Children.FirstOrDefault(item => item.Kind == CoreNodeKind.ExpressionFunctionBody)
                ?.Child(CoreChildRole.expressionOffset)
                is { } callbackExpression
        )
        {
            ...
            var parameterName = SafeIdentifier(
                callbackParameter.Text(CoreProperty.name) ?? "constraints"
            );
            builder
                .Append("invokeLayoutCallback<Constraints>((Constraints ")
        ...
        if (
            methodName == "debugGetOpenHandleStackTraces"
            && node.ElementId?.Contains("dart:ui#Image.", StringComparison.Ordinal) == true
        )
        {
        ...
        {
            var values = invocationArguments
                .Children.Where(item => item.Category is "expression" or "argument")
                .Select(item =>
                    item.Kind == CoreNodeKind.NamedExpression
                        ? item.Child(CoreChildRole.expressionOffset)
                        : item
                )
            ...
            if (
                values.Length >= 2
                && values[1].StaticType?.StartsWith("void Function()", StringComparison.Ordinal)
                    == true
            )
            {
                if (target is not null)
        ...
        if (
            methodName == "_lerp"
            && target is null
            && node.StaticType is { } nullableLerpType
        ...
        if (
            target?.Kind == CoreNodeKind.SuperExpression
            && methodName is "toStringShort" or "toDiagnosticsNode"
        )
        {
        ...
        var hasChildOrderArgument =
            invocationArguments?.Children.Any(argument =>
                argument.Kind == CoreNodeKind.NamedExpression
                && argument.Text(CoreProperty.name) == "childOrder"
            ) == true;
        if (
            methodName is "toStringShort" or "toDiagnosticsNode"
            && target is not null
            && target.Kind != CoreNodeKind.SuperExpression
            && !hasChildOrderArgument
        )
        {
        ...
        if (
            ...
            && invocationArguments?.Children.Any(argument =>
                argument.Kind == CoreNodeKind.NamedExpression
                && argument.Text(CoreProperty.name) == "minLevel"
            ) == true
        )
        {
            if (target is not null)
        ...
        if (
            methodName == "findLockByLogicalKey"
            && target is not null
            && target.Text(CoreProperty.name) == "KeyboardLockMode"
        )
        {
            var keyArgument = node.Child(CoreChildRole.argumentsOffset)
            ...
            {
                builder.Append("default(LogicalKeyboardKey)");
            }

            builder.Append(
                ".keyId switch { var id when id == LogicalKeyboardKey.numLock.keyId => KeyboardLockMode.numLock, var id when id == LogicalKeyboardKey.scrollLock.keyId => KeyboardLockMode.scrollLock, var id when id == LogicalKeyboardKey.capsLock.keyId => KeyboardLockMode.capsLock, _ => (KeyboardLockMode?)null })"
            );
            return;
        }
        ...
        if (
            ...
            && AppliedMixinDeclarations(declaration)
                .Any(mixin =>
                    mixin.Members.Any(member =>
                        member.Kind == "method"
                        && member.Name == methodName
                        && !DescendantsAndSelf(member.Ast)
                            .Any(item =>
                                item.Category == "statement" && item.Kind != CoreNodeKind.Block
                            )
                    )
                )
        )
        ...
        {
            var foldArguments =
                invocationArguments?.Children.Where(item => item.Category == "expression").ToArray()
                ?? [];
        ...
        if (
            methodName is "insert" or "Insert"
            && target is not null
            && (
        ...
        )
        {
            var insertArguments =
                node.Child(CoreChildRole.argumentsOffset)
                    ?.Children.Where(item => item.Category == "expression")
                    .ToArray()
                ?? [];
        ...
        {
            var removeArgument = node.Child(CoreChildRole.argumentsOffset)
                ?.Children.FirstOrDefault(item => item.Category == "expression");
        ...
        if (methodName is "indexOf" or "IndexOf" && target is not null)
        {
            var indexArguments =
                ...
                ?? [];
            if (
                (target.StaticType ?? string.Empty).TrimEnd('?') == "String"
                && indexArguments.Length == 2
            )
            {
                LowerExpression(
                    builder,
            ...
            if (
                indexArgument?.StaticType?.EndsWith("?", StringComparison.Ordinal) == true
        ...
        if (
            methodName is "min" or "max"
            && (
                target is null
                || target.Text(CoreProperty.name) == "math"
                || target.Text(CoreProperty.prefix) == "math"
            )
        )
        {
            var resultType = MapType(node.StaticType ?? string.Empty).TrimEnd('?');
            builder.Append(
                IsUnboundTypeParameterName(resultType)
                    ? methodName == "min"
                        ? "DartRuntimePrimitives.Min("
                        : "DartRuntimePrimitives.Max("
                    : methodName == "min"
                        ? "Math.Min("
                        : "Math.Max("
            );
        ...
        if (
            methodName is "encodeFull" or "parse"
            && (
                (target is not null && target.Text(CoreProperty.name) == "Uri")
                || node.ElementId?.Contains("#Uri.", StringComparison.Ordinal) == true
            )
        )
        {
        ...
        if (
            target is not null
            && methodName is "firstKey" or "lastKey"
            ...
            && TryGetGenericTypeArguments(
                MapType(target.StaticType ?? string.Empty).TrimEnd('?'),
                out var mapArguments
            )
            && mapArguments.Length == 2
        )
        {
            builder
                .Append("DartCollectionRuntime.")
                .Append(methodName == "firstKey" ? "FirstKeyOrNull" : "LastKeyOrNull")
                .Append('<')
                .Append(nullableKeyType.TrimEnd('?'))
                .Append(", ")
                .Append(mapArguments[1])
                .Append(">(");
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append(')');
            return;
        ...
        var mappedExplicitTypeArguments =
            node.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.TypeArgumentList)
                ?.Children.Where(item => item.Category == "type")
                .Select(MapTypeFromAst)
                .ToArray()
            ?? [];
        var explicitTypeArguments =
            methodName == "then"
            && mappedExplicitTypeArguments.Any(type => type is "void" or "Null")
                ? []
        ...
        if (
            string.IsNullOrEmpty(typeArguments)
            && target is null
            && _currentDeclarations is not null
        )
        {
            var called = FindDeclaration(node.ElementId);
            if (
                called is not null
                && called.Element.TypeParameters is { Length: > 0 }
                && declaration.Element.TypeParameters is { Length: > 0 }
            )
            {
                typeArguments =
                    $"<{string.Join(", ", declaration.Element.TypeParameters.Select(item => SafeIdentifier(item.Name)))}>";
            }
        }
        if (
            string.IsNullOrEmpty(typeArguments)
        ...
        if (
            string.IsNullOrEmpty(typeArguments)
            && methodName == "then"
            && node.StaticType is { } inferredThenType
        ...
        )
        {
            ...
            if (
                start >= 0
                && end > start
                && inferredThenType[(start + 1)..end] is not ("void" or "Null")
            )
            {
        ...
        if (
            string.IsNullOrEmpty(typeArguments)
            && methodName == "then"
            && declaration.Name == "TickerFuture"
        )
        {
            var containingGenericMember = declaration.Members.FirstOrDefault(member =>
                ContainsOffset(member.Ast, node.Offset)
                && member.Element.TypeParameters is { Length: 1 }
            );
            if (
                containingGenericMember?.Element.TypeParameters is
                { Length: 1 } memberTypeParameters
            )
            {
        ...
        if (
            target is not null
            && target.Text(CoreProperty.name) is { } staticTargetName
            && TryResolveEmittedNamedConstructor(
                MapType(staticTargetName),
                methodName,
                out var staticFactoryMethod
            )
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
        ...
        )
        {
            ...
            if (start >= 0 && end > start)
            {
                var arguments = SplitGenericArguments(castType[(start + 1)..end])
                    .Select(MapType)
                    .ToArray();
                if (arguments.Length > 0)
                {
                    typeArguments = $"<{string.Join(", ", arguments)}>";
                }
            }
        }
        if (
            methodName == "map"
            && target?.StaticType is { } mapTargetType
            && node.StaticType is { } mapResultType
        )
        {
            ...
            {
                typeArguments =
                    $"<{mappedTarget[(targetStart + 1)..targetEnd]}, {mappedResult[(resultStart + 1)..resultEnd]}>";
            }
        }
        ...
        if (
            methodName == "toString"
            && target?.Kind == CoreNodeKind.PrefixedIdentifier
            && target.Text(CoreProperty.name) == "runtimeType"
        )
        {
        ...
        if (
            methodName == "toString"
            && target?.Kind == CoreNodeKind.PropertyAccess
            && target.Text(CoreProperty.name) == "runtimeType"
        )
        {
        ...
        if (
            ...
            && (
                (
                    target?.Kind == CoreNodeKind.SimpleIdentifier
                    && target.Text(CoreProperty.name) == "Object"
                )
                || (
                    target?.Kind == CoreNodeKind.PrefixedIdentifier
                    && target.Text(CoreProperty.name) == "hash"
                    && target.Text(CoreProperty.prefix) == "Object"
                )
        ...
        var intrinsicArguments =
            args?.Children.Where(item => item.Category == "expression").ToArray() ?? [];
        if (
            target is not null
            && methodName == "insert"
        ...
        )
        {
            ...
            EmitArguments(
                ...
                inputPath,
                diagnostics,
                expectedParameters: ResolveInvocationMember(
                    node,
                    target,
                    declaration,
                    methodName
                )?.Element.Parameters,
                expectedArgumentTypes: ResolveInvocationParameterTypes(
                    node,
                    target,
                    declaration,
                    methodName
        ...
        if (
            target is null
            && methodName == "_computeIntrinsics"
            && intrinsicArguments.Length > 0
            && FindGlobalDeclaration(intrinsicArguments[0].StaticType ?? string.Empty)
                is { Ast.Kind: CoreNodeKind.EnumDeclaration } enumArgumentDeclaration
        ...
        )
        {
            ...
            builder
                ...
                .Append("(new ")
                .Append(enumName)
                .Append("InterfaceAdapter(");
            LowerExpression(
                builder,
                intrinsicArguments[0],
                declaration,
                package,
                library,
                inputPath,
        ...
        var resolvedTargetStorageType = target is null ? null : ResolvedExpressionValueType(target);
        if (
            string.IsNullOrWhiteSpace(resolvedTargetStorageType)
        ...
        if (
            ...
            && target is not null
            && (
                IsDartCollectionType(target.StaticType)
        ...
        if (
            ...
            && target is not null
            && (
                MapType(target.StaticType ?? string.Empty)
                    .TrimEnd('?')
                    .EndsWith("ParagraphBuilder", StringComparison.Ordinal)
                || DescendantsAndSelf(target)
                    .Any(candidate =>
        ...
        if (
            string.IsNullOrWhiteSpace(targetStaticType)
            && (
                methodName
                is "add"
                    or "addAll"
        ...
        string CurrentInvocationName()
        {
            if (target is not null)
            {
                ...
                var rawTargetType = StripLibraryPrefix(
                    (targetStaticType ?? string.Empty).TrimEnd('?')
                );
                var rawGeneric = rawTargetType.IndexOf('<');
                if (rawGeneric >= 0)
                {
                    rawTargetType = rawTargetType[..rawGeneric];
                }

                var targetDeclaration =
                    FindGlobalDeclaration(rawTargetType)
                    ?? FindGlobalDeclaration(
                        MapType(targetStaticType ?? string.Empty).TrimEnd('?')
                    );
                ...
                if (
                    targetMember is not null
                    && targetDeclaration is not null
                    && !LibraryUriFromElementId(targetDeclaration.Element.CanonicalId)
                        .StartsWith("dart:", StringComparison.Ordinal)
                )
                {
                ...
                resolvedMember ??= targetDeclaration?.Members.FirstOrDefault(member =>
                    member.Kind == "method"
                    && string.Equals(member.Name, methodName, StringComparison.Ordinal)
                );
                var resolvedOwner = resolvedMember is null
                    ? null
                    : FindDeclaringDeclaration(resolvedMember);
                if (
                    resolvedMember is not null
                    && resolvedOwner is not null
                    && !LibraryUriFromElementId(resolvedOwner.Element.CanonicalId)
                        .StartsWith("dart:", StringComparison.Ordinal)
                )
                {
        ...
        {
            targetStaticType =
                collectionElementId.Contains("dart:core#List.", StringComparison.Ordinal)
                    ? "List<object>"
                : collectionElementId.Contains("dart:core#Set.", StringComparison.Ordinal)
                    ? "Set<object>"
                : collectionElementId.Contains("dart:core#Map.", StringComparison.Ordinal)
                    ? "Map<object, object>"
                : collectionElementId.Contains("dart:core#Stopwatch.", StringComparison.Ordinal)
                    ? "Stopwatch"
                : null;
        }
        var targetNullableReferenceType =
            !string.IsNullOrEmpty(targetStaticType)
            && targetStaticType.EndsWith("?", StringComparison.Ordinal)
            && !IsValueType(targetStaticType.TrimEnd('?'));
        ...
        if (
            ...
            && (
                explicitNullAware
                || (
                    targetNullableReferenceType
                    && (
                        node.StaticType == "void"
        ...
        )
        {
            ...
            )
            {
                ...
                builder
                    .Append(", __target => __target.")
                    .Append(CurrentInvocationName())
                    .Append(typeArguments)
                    .Append('(');
            ...
            var dynamicDispatch =
                ...
                || (
                    ...
                    && DescendantsAndSelf(target)
                        .Any(candidate => candidate.Text(CoreProperty.name) == "renderObject")
                );
            dynamicDispatch |=
                methodName == "markNeedsLayout"
                && invocationArguments?.Children.Any(argument =>
                    argument.Kind == CoreNodeKind.NamedExpression
                    && argument.Text(CoreProperty.name) == "withDelegateRebuild"
                ) == true;
            var dynamicResultType =
                dynamicDispatch
                && node.StaticType is { } nullableStaticType
                && nullableStaticType.TrimEnd('?')
                    is not ("void" or "dynamic" or "Object" or "object")
                    ? MapType(nullableStaticType)
                    : null;
            if (dynamicResultType is not null)
            {
                builder.Append("((").Append(dynamicResultType).Append(')');
            }
        ...
        var usesDynamicDispatch =
            ...
            && (
                ...
                || (
                    methodName == "insert"
                    && MapType(targetStaticType ?? string.Empty).TrimEnd('?') == "RenderObject"
                )
                || (
                    methodName is "insert" or "remove" or "_updateCallback"
                    && DescendantsAndSelf(target)
        ...
        var dynamicInvocationResultType =
            usesDynamicDispatch
            && node.StaticType is { } invocationStaticType
            && invocationStaticType.TrimEnd('?')
                is not ("void" or "dynamic" or "Object" or "object")
                ? MapType(invocationStaticType)
                : null;
        if (dynamicInvocationResultType is not null)
        {
            builder.Append("((").Append(dynamicInvocationResultType).Append(')');
        }
        ...
        if (
            target is null
            && staticInvocationMember is { IsStatic: true }
            && staticInvocationOwner is not null
        )
        {
            builder
                .Append(MapStaticOwnerType(staticInvocationOwner.Name, declaration))
                .Append('.')
            ...
            && node.ElementId?.StartsWith(
                declaration.Element.CanonicalId + ".",
                StringComparison.Ordinal
            ) == true
        ...
        {
            builder
                .Append(_session.ExplicitThisExpression)
                .Append('.')
                .Append(CurrentInvocationName())
                .Append(typeArguments);
        }
        ...
        {
            builder.Append(
                "global::Doroti.Framework.Foundation.objectRuntimeTypeFunctions.objectRuntimeType"
            );
        }
        else if (target is null && !string.IsNullOrEmpty(node.ElementId))
        {
            ...
            if (
                !string.IsNullOrEmpty(elementLibrary)
                && !symbol.Contains('.', StringComparison.Ordinal)
                && !elementLibrary.StartsWith("dart:", StringComparison.Ordinal)
                && (
                    calledDeclaration?.Ast.Kind
                        is CoreNodeKind.FunctionDeclaration
                            or CoreNodeKind.TopLevelVariableDeclaration
                    || !string.Equals(elementLibrary, library, StringComparison.Ordinal)
                )
            )
            {
                builder
                    .Append(QualifiedLibraryStaticClassName(elementLibrary, library))
                    .Append('.');
            }
            var mappedName = CurrentInvocationName();
            builder.Append(mappedName).Append(typeArguments);
        ...
        if (methodName != "toList")
        {
            var argumentInvocationName =
                methodName == "call" && target is not null
                    ? target.Text(CoreProperty.name)
                        ?? target.Children.LastOrDefault()?.Text(CoreProperty.name)
                        ?? methodName
                    : methodName;
            EmitArguments(
                builder,
    ...
    )
    {
        ...
        if (target is null || argumentList.Length < 4)
        {
            AddUnsupportedDiagnostic(
                ...
                node,
                "setrange-shape",
                "setRange requires a target, start, end, source and sourceOffset."
            );
    ...
    private string MapMethodInvocationName(string name, string? targetType)
    {
        ...
        if (
            name == "remove"
            && targetType?.TrimEnd('?') is { } mapType
            && (
                mapType.StartsWith("Map<", StringComparison.Ordinal)
                || mapType.StartsWith("DartMap<", StringComparison.Ordinal)
            )
        )
        {
            return "remove";
        ...
        if (name == "add")
        {
            ...
            if (value.StartsWith("Queue<", StringComparison.Ordinal))
            {
                return "Enqueue";
            }

            if (
                value.StartsWith("PriorityQueue<", StringComparison.Ordinal)
                || value.StartsWith("HeapPriorityQueue<", StringComparison.Ordinal)
            )
            {
                return "Add";
            }
        ...
        {
            return collection
                ? targetType?.Contains("Set", StringComparison.Ordinal) == true
                    ? "UnionWith"
                    : "AddRange"
                : "addAll";
        }
        ...
        if (name == "clear")
        {
            return collection ? "Clear" : "clear";
        }

        if (name == "contains")
        {
            return collection ? "Contains" : "contains";
        }

        if (name == "indexOf")
        {
            return collection ? "IndexOf" : "indexOf";
        }

        if (name == "lastIndexOf")
        {
            return collection ? "LastIndexOf" : "lastIndexOf";
        }

        if (
            name is "firstKey" or "lastKey"
            && targetType?.TrimEnd('?') is { } sortedMapType
            && (
        ...
        if (
            name == "removeFirst"
            && targetType?.TrimEnd('?').StartsWith("Queue<", StringComparison.Ordinal) == true
        )
        {
            return "Dequeue";
        }
        ...
        if (
            ...
            && (
                targetType.Contains("HashSet", StringComparison.Ordinal)
    ...
    private bool IsDartTypedDataList(string? type) =>
        type?.TrimEnd('?')
            is "Float32List"
                or "Float64List"
                or "Int8List"
    ...
    private bool IsDartEnumerableType(string? type)
    {
        ...
        if (FindGlobalDeclaration(MapType(value)) is { } declaration)
        {
        ...
        while (pending.Count > 0)
        {
            ...
            if (!visited.Add(candidate.Element.CanonicalId))
            {
                continue;
            }
            ...
            foreach (var baseName in DirectBaseNames(candidate))
            {
                ...
                if (
                    simpleBase.StartsWith("Iterable<", StringComparison.Ordinal)
                ...
                {
                    pending.Enqueue(baseDeclaration);
                }
            }
        }
        return false;
    ...
    {
        var constructorName = node.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.ConstructorName
        );
        var typeName = constructorName is null
            ? MapType(node.StaticType ?? "object")
            : MapTypeFromAst(constructorName);
        var staticTypeName = MapType(node.StaticType ?? string.Empty);
        if (
            typeName == "dynamic"
            && constructorName is not null
            && DescendantsAndSelf(constructorName)
                .Any(item => item.Text(CoreProperty.name) == "Router")
        )
        {
        ...
        {
            var owner = _session.ActiveDonorDeclaration ?? declaration;
            var contextualType = owner
                .Members.Where(member =>
                    member.Kind == "field" && ContainsOffset(member.Ast, node.Offset)
                )
                .Select(member => MapType(member.Element.Type ?? string.Empty))
                .FirstOrDefault(candidate =>
                    !Regex.IsMatch(candidate, @"\bT\b", RegexOptions.CultureInvariant)
                );
            contextualType ??= DescendantsAndSelf(owner.Ast)
                .Where(candidate =>
                    candidate.Kind == CoreNodeKind.VariableDeclaration
            ...
            if (
                !string.IsNullOrEmpty(contextualType)
                && string.Equals(
                    typeName.Split('<')[0],
                    contextualType.Split('<')[0],
                    StringComparison.Ordinal
                )
            )
            {
        ...
        var constructor =
            node.Text(CoreProperty.constructor)
            ?? (
                constructorName?.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.SimpleIdentifier
                )
                    is { } nameNode
        ...
        if (
            (
                ...
                || typeName.StartsWith("DartMap<", StringComparison.Ordinal)
            )
        ...
        if (
            DescendantsAndSelf(node)
                .Any(item =>
                    item.Text(CoreProperty.name)?.Contains("GlobalKey", StringComparison.Ordinal)
                    == true
                )
        )
        {
            var globalKeyType = staticTypeName.Contains("GlobalKey<", StringComparison.Ordinal)
                ? staticTypeName
                : typeName;
        ...
        if (
            typeName.Contains("DiagnosticsProperty<", StringComparison.Ordinal)
            && argumentList.Length > 0
        )
        {
            builder.Append("new ").Append(typeName).Append('(');
        ...
        )
        {
            ...
            builder
                .Append(")).Select(__index => ((Func<long, ")
                .Append(typeName[12..^1])
                .Append(">)");
            LowerExpression(
                builder,
                argumentList[1],
        ...
        if (
            ...
            && staticTypeName.EndsWith("EdgeInsets", StringComparison.Ordinal)
        )
        {
            typeName = staticTypeName;
        }
        ...
        if (
            typeName.EndsWith("TextSelection", StringComparison.Ordinal)
            && constructor is "collapsed" or "fromPosition"
        )
        {
            builder
                .Append(typeName)
                .Append(constructor == "collapsed" ? ".CreateCollapsed(" : ".CreateFromPosition(");
            EmitArguments(
                builder,
                arguments,
                declaration,
        ...
        if (
            typeName == "ImageStreamListener"
            && _currentLibrary.EndsWith("/image_cache.dart", StringComparison.Ordinal)
            && argumentList.Length == 1
        )
        {
            builder.Append(
                "new ImageStreamListener((Action<ImageInfo, bool>)((image, synchronousCall) => listener(image, synchronousCall)))"
            );
            return;
        }
        ...
        )
        {
            ...
            if (constructor == "filled" && argumentList.Length >= 2)
            {
                builder
                    .Append("new ")
                    .Append(typeName)
                    .Append("(System.Linq.Enumerable.Repeat<")
                    .Append(elementType)
                    .Append(">(");
                LowerExpression(
                    builder,
            ...
            if (constructor == "generate" && argumentList.Length >= 2)
            {
                builder
                    ...
                    .Append(typeName)
                    .Append(
                        "(System.Linq.Enumerable.Select(System.Linq.Enumerable.Range(0, checked((int)"
                    );
                LowerExpression(
                    builder,
                    argumentList[0],
        ...
        if (
            ...
            && (
                MapType(
                        ResolvedExpressionValueType(argumentList[0])
                            ?? argumentList[0].StaticType
                            ?? string.Empty
                    )
                ...
                || MapType(
                    ...
                    .StartsWith("IReadOnlyList<", StringComparison.Ordinal)
                || MapType(
                        ResolvedExpressionValueType(argumentList[0])
                            ?? argumentList[0].StaticType
                            ?? string.Empty
        ...
        )
        {
            ...
            builder
                .Append("new ")
                .Append(typeName)
                .Append("(DartRuntimePrimitives.ConvertEnumerable<")
                .Append(elementType)
                .Append(">(");
            LowerExpression(
                builder,
        ...
        if (
            typeName is "Vector2" or "global::System.Numerics.Vector2"
            && constructor == "array"
        ...
        if (typeName.Contains("GlobalKey<", StringComparison.Ordinal))
        {
            ...
            typeName.StartsWith("DefaultTransitionDelegate<", StringComparison.Ordinal)
            && constructor is null or "" or "new"
        )
        {
            builder.Append("new ").Append(typeName).Append('(');
        }
        ...
        )
        {
            ...
            constructor == "all"
            && typeName.EndsWith("EdgeInsetsGeometry", StringComparison.Ordinal)
        )
        {
        ...
        )
        {
            ...
            constructor == "fromLTRB"
            && typeName.EndsWith("EdgeInsets", StringComparison.Ordinal)
        )
        {
            builder.Append("new ").Append(typeName).Append('(');
        }
            ...
            && TryResolveEmittedNamedConstructor(
                typeName,
                constructor,
                out var namedConstructorMethod
            )
        )
        {
        ...
        if (
            typeName == "Tween<double>"
            && argumentList.Any(item =>
                item.Kind == CoreNodeKind.NamedExpression
                && item.Child(CoreChildRole.expressionOffset)
                    ?.StaticType?.EndsWith("?", StringComparison.Ordinal) == true
            )
        )
        {
            for (var index = 0; index < argumentList.Length; index++)
            {
                ...
                {
                    builder
                        .Append(SafeIdentifier(argument.Text(CoreProperty.name) ?? "value"))
                        .Append(": ");
                }
        ...
        else
        {
            var resolvedConstructor =
                FindGlobalMember(constructorName?.ElementId) ?? FindGlobalMember(node.ElementId);
            if (
                resolvedConstructor is null
                && FindGlobalDeclaration(typeName) is { } constructorDeclaration
            )
            {
                ...
                var positionalCount = argumentList.Count(argument =>
                    argument.Kind != CoreNodeKind.NamedExpression
                );
                var compatibleConstructors = constructorDeclaration
                    .Members.Where(member =>
                        member.Kind == "constructor"
                        && (member.Element.Parameters ?? []).Count(parameter =>
                            parameter.Kind is not "optional-named" and not "required-named"
                        ) >= positionalCount
            ...
            var resolvedConstructorOwner = resolvedConstructor is null
                ? null
                : FindDeclaringDeclaration(resolvedConstructor);
            ...
            if (
                ...
                && constructorTypeParameters.Length > 0
                    ...
                    .Select(
                        (parameter, index) =>
                            new KeyValuePair<string, string>(
                                parameter.Name,
                                constructorTypeArguments[index]
                            )
                    )
            ...
            var hasUnsubstitutedConstructorParameter =
                !substitutedConstructorParameters
                && constructorParameters?.Any(parameter =>
                    constructorTypeParameters.Any(typeParameter =>
                        Regex.IsMatch(
                            parameter.Type,
                            $@"\b{Regex.Escape(typeParameter.Name)}\b",
            ...
            EmitArguments(
                ...
                inputPath,
                diagnostics,
                expectedParameters: hasUnsubstitutedConstructorParameter
                    ? null
    ...
    {
        var typeArgumentList = node.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.TypeArgumentList
        );
        var typeName = typeArgumentList is not null
            ? MapType(
                $"List<{string.Join(", ", typeArgumentList.Children.Where(item => item.Category == "type").Select(MapTypeFromAst))}>"
            )
            : MapType(node.StaticType ?? "List<object>");
        var collectionElements = node
            .Children.Where(item => item.Category is "expression" or "collection-element")
            .ToArray();
        ...
        var elementType =
            typeName.StartsWith("List<", StringComparison.Ordinal) && typeName.EndsWith('>')
                ? typeName[5..^1]
                : string.Empty;
        ...
        else
        {
            ...
            for (var index = 0; index < elements.Length; index++)
            {
                ...
                if (elements[index].Kind == CoreNodeKind.AssignmentExpression)
                {
                    builder.Append('(');
                }

                EmitListCollectionValue(
                    builder,
                    elements[index],
                    elementType,
                    declaration,
    ...
    )
    {
        if (mapCollection && element.Kind == CoreNodeKind.MapLiteralEntry)
        {
            ...
            if (expressions.Length != 2)
            {
                AddUnsupportedDiagnostic(
                    ...
                    element,
                    "map-entry-shape",
                    "Resolve the key and value expressions for the Dart map literal entry."
                );
                return;
            }
        ...
        if (element.Kind == CoreNodeKind.SpreadElement)
        {
            ...
            if (spread is null)
            {
                AddUnsupportedDiagnostic(
                    ...
                    element,
                    "collection-spread",
                    "Expose the typed spread expression."
                );
                return;
            }
            ...
            if (isNullAware)
            {
                ...
                builder
                    ...
                    .Append(" is not null) { ")
                    .Append(listName)
                    .Append(setCollection ? ".UnionWith(" : ".AddRange(")
                    .Append(spreadName)
                    .Append("); } ");
            }
            else
        ...
        if (element.Kind == CoreNodeKind.NullAwareElement)
        {
            ...
            if (value is null)
            {
                AddUnsupportedDiagnostic(
                    ...
                    "Expose the typed null-aware collection expression."
                );
                return;
            }
        ...
        {
            var condition =
                element.Child(CoreChildRole.conditionOffset)
                ?? element.Children.FirstOrDefault(item => item.Category == "expression");
            var caseClause = element.Children.FirstOrDefault(item =>
                item.Kind == CoreNodeKind.CaseClause
            );
            ...
            if (condition is null || branches.Length == 0)
            {
                AddUnsupportedDiagnostic(
                    ...
                    element,
                    "collection-if",
                    "Expose the condition and collection branch elements."
                );
                return;
            }
            builder.Append("if (");
            ...
            {
                var guardedPattern = caseClause.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.GuardedPattern
                );
                EmitIfCaseCondition(
                    builder,
                    condition,
        ...
        if (element.Kind == CoreNodeKind.ForElement)
        {
            var parts = element.Children.FirstOrDefault(item =>
                item.Kind
                    is CoreNodeKind.ForEachPartsWithDeclaration
                        or CoreNodeKind.ForEachPartsWithIdentifier
                        or CoreNodeKind.ForPartsWithDeclarations
                        or CoreNodeKind.ForPartsWithExpression
            );
            var body = element.Children.LastOrDefault(item =>
                item.Kind == CoreNodeKind.MapLiteralEntry
                || item.Category is "expression" or "collection-element"
            );
            if (parts is null || body is null)
            {
                AddUnsupportedDiagnostic(
                    ...
                    element,
                    "collection-for",
                    "Expose the typed loop parts and collection body."
                );
                return;
            }
            if (
                parts.Kind
                is CoreNodeKind.ForPartsWithDeclarations
                    or CoreNodeKind.ForPartsWithExpression
            )
            {
                var variableList = parts.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.VariableDeclarationList
                );
                var expressions = parts
                    .Children.Where(item => item.Category == "expression")
                    .ToArray();
                var initializer = variableList is null
                    ? parts.Child(CoreChildRole.initializerOffset)
                    : null;
                ...
                if (initializer is null && condition is null)
                {
                    ...
                    condition = variableList is null
                        ? expressions.Skip(1).FirstOrDefault()
                        : expressions.FirstOrDefault();
                }
                var updaters = expressions
                    .Where(item =>
                        item.Offset != initializer?.Offset && item.Offset != condition?.Offset
                    )
                    .ToArray();
                builder.Append("for (");
            ...
            else
            {
                ...
                if (identifier is null || iterable is null)
                {
                    AddUnsupportedDiagnostic(
                        ...
                        element,
                        "collection-for",
                        "Expose the declared identifier and iterable."
                    );
                    return;
                }
                var identifierName = EmittedLocalIdentifier(
                    identifier,
                    identifier.Text(CoreProperty.name) ?? "value"
                );
        ...
        AddUnsupportedDiagnostic(
            ...
            element,
            "collection-element",
            "Add typed lowering for this collection element kind."
        );
    }
    ...
    )
    {
        ...
        var mapEntries = node
            .Children.Where(item => item.Kind == CoreNodeKind.MapLiteralEntry)
            .ToArray();
        var collectionElements = node
            .Children.Where(item => item.Category is "expression" or "collection-element")
            .ToArray();
        ...
        var mapValueType =
            TryGetGenericTypeArguments(typeName.TrimEnd('?'), out var mapArguments)
            && mapArguments.Length == 2
                ? mapArguments[1]
                : string.Empty;
        ...
        if (isSet && collectionElements.Any(item => item.Category != "expression"))
        {
            ...
            var elementType =
                TryGetGenericTypeArguments(typeName.TrimEnd('?'), out var setArguments)
        ...
        if (!isSet && collectionElements.Any(item => item.Category != "expression"))
        {
            ...
            builder
                ...
                .Append("(); ");
            foreach (
                var element in node.Children.Where(item =>
                    item.Kind == CoreNodeKind.MapLiteralEntry
                    || item.Category is "expression" or "collection-element"
                )
            )
        ...
        for (var index = 0; index < mapEntries.Length; index++)
        {
            ...
            var expressions = mapEntries[index]
                .Children.Where(item => item.Category == "expression")
                .ToArray();
            if (expressions.Length != 2)
            {
            ...
            if (
                mapValueType.StartsWith("List<", StringComparison.Ordinal)
        ...
        {
            if (mapEntries.Length > 0 || index > 0)
            {
                builder.Append(", ");
            }
    ...
    {
        var parameters =
            node.Children.FirstOrDefault(item =>
        ...
        var parameterNames = parameters
            .Select(
                (parameter, index) =>
                {
                    ...
                    var identifier = DescendantsAndSelf(parameter)
                        .FirstOrDefault(item => item.Kind == CoreNodeKind.SimpleIdentifier);
                    return identifier is not null
                        ? SafeIdentifier(identifier.Text(CoreProperty.name) ?? $"arg{index}")
                        : $"arg{index}";
                }
            )
            .ToArray();
        var expressionBody = node.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.ExpressionFunctionBody
        );
        var blockBody = node.Children.FirstOrDefault(item =>
        ...
        var normalizedFunctionType = mappedFunctionType.Replace(
            "global::System.",
            string.Empty,
            StringComparison.Ordinal
        );
        ...
        {
            if (
                TryGetGenericTypeArguments(normalizedFunctionType, out var contextualArguments)
            ...
            !string.IsNullOrEmpty(contextualLambdaReturnType)
            && normalizedFunctionType.StartsWith("Func<", StringComparison.Ordinal)
        ...
        {
            if (
                normalizedFunctionType is "Action"
                || normalizedFunctionType.StartsWith("Action<", StringComparison.Ordinal)
            )
            {
                builder.Append("{ ");
                if (
                    expression.Kind != CoreNodeKind.NullLiteral
                    && MapType(expression.StaticType ?? string.Empty) != "void"
                )
                {
                    builder.Append("_ = ");
                }
        ...
        else if (block is not null)
        {
            ...
            if (contextualLambdaReturnType is null)
            {
                ...
                normalizedFunctionType = mappedFunctionType.Replace(
                    "global::System.",
                    string.Empty,
                    StringComparison.Ordinal
                );
            ...
            var dartReturnType =
                !string.IsNullOrEmpty(contextualLambdaReturnType) ? contextualLambdaReturnType
                : normalizedFunctionType is "Action"
                || normalizedFunctionType.StartsWith("Action<", StringComparison.Ordinal)
                    ? "void"
                : functionMarker > 0 ? dartFunctionType[..functionMarker].Trim()
                : string.Empty;
            var previousReturnType = _session.ActiveFunctionReturnType;
            _session.ActiveFunctionReturnType =
                dartReturnType.Length > 0 ? MapType(dartReturnType) : null;
            try
            {
                EmitBlockBody(
                    builder,
            ...
            var hasValueReturn = DescendantsExcludingNestedFunctions(block)
                .Any(statement =>
                    statement.Kind == CoreNodeKind.ReturnStatement
                    && statement.Children.Any(child => child.Category == "expression")
                );
            var asyncReturnsValue =
                isAsync
                && TryGetGenericTypeArguments(
                    mappedFunctionType.TrimEnd('?'),
                    out var functionArguments
                )
                && functionArguments.Length > 0
                && functionArguments[^1].StartsWith("Future<", StringComparison.Ordinal);
            var requiresClrValue = normalizedFunctionType.StartsWith(
                "Func<",
                StringComparison.Ordinal
            );
            if (requiresClrValue && !returnsValue && !asyncReturnsValue)
            {
                ...
                || (
                    dartReturnType != "void"
                    && ((!isAsync && (returnsValue || hasValueReturn)) || asyncReturnsValue)
                )
            )
            {
    ...
    )
    {
        ...
        var target = hasCascadeSection
            ? node.Children.FirstOrDefault(item =>
                item.Category == "expression" && item.Kind != CoreNodeKind.CascadeSection
            )
        ...
        if (target is null)
        {
            AddUnsupportedDiagnostic(
                ...
                node,
                "cascade-shape",
                "Provide the typed cascade target and sections."
            );
            builder.Append("throw new NotSupportedException(\"DOTF0001\")");
            return;
        }
        ...
        {
            var expression =
                section.Kind == CoreNodeKind.CascadeSection
            ...
            if (expression is not null)
            {
                ...
                var cascadeAssignmentLeft =
                    expression.Kind == CoreNodeKind.AssignmentExpression
                        ...
                        : null;
                if (
                    cascadeAssignmentLeft?.Kind == CoreNodeKind.IndexExpression
                    && cascadeAssignmentLeft.Child(CoreChildRole.targetOffset) is null
                )
                {
                    var index = cascadeAssignmentLeft.Children.FirstOrDefault(item =>
                    ...
                    builder
                        .Append("] ")
                        .Append(MapOperator(expression.Text(CoreProperty.@operator) ?? "="))
                        .Append(' ');
                ...
                {
                    var right = expression.Child(CoreChildRole.rightOffset);
                    var assignmentOperator = MapOperator(
                        expression.Text(CoreProperty.@operator) ?? "="
                    );
                    builder.Append('.');
                    LowerExpression(
                        builder,
                        cascadeAssignmentLeft,
                        declaration,
                        package,
                        library,
                        inputPath,
                    ...
                    {
                        var mappedLeftType = MapType(cascadeAssignmentLeft.StaticType ?? "object")
                            .TrimEnd('?');
                        var mappedRightType = MapType(right.StaticType ?? "object");
                        var hasNullableRightValue =
                            mappedRightType == mappedLeftType + "?"
                            || HasNullableValueStorage(right, declaration)
                            || DescendantsAndSelf(right)
                                .Any(candidate =>
                                    MapType(candidate.StaticType ?? string.Empty)
                                    == mappedLeftType + "?"
                                );
                        var pairedGetterRequiresValue =
                            !string.IsNullOrEmpty(cascadeAssignmentLeft.ElementId)
                        ...
                        var requiresNullableValue =
                            assignmentOperator == "="
                            && IsValueType(mappedLeftType)
                            && hasNullableRightValue
                            && (
                                pairedGetterRequiresValue
                                || !MapType(cascadeAssignmentLeft.StaticType ?? "object")
                ...
                {
                    var index = expression.Children.FirstOrDefault(item =>
                        item.Category == "expression"
                    );
                    builder.Append('[');
                    if (index is not null)
                    ...
                    expression.Kind == CoreNodeKind.MethodInvocation
                ...
                )
                {
                    ...
                    builder
                        .Append('.')
                        .Append(MapMethodInvocationName(cascadeMethod, target.StaticType))
                        .Append('(');
                    EmitArguments(
                        builder,
    ...
    )
    {
        ...
        if (targetName is not null && targetName is not ("bool" or "String" or "int"))
        {
            return false;
        }
        var arguments = node.Child(CoreChildRole.argumentsOffset);
        var literals =
            arguments
                ?.Children.Where(item =>
                    item.Category == "expression" && item.Kind == CoreNodeKind.SimpleStringLiteral
                )
                .ToArray()
            ?? [];
    ...
    private bool TryEmitFromEnvironmentLiteral(
        CsSyntaxBuilder builder,
        CoreAstNode[] argumentExpressions,
    ...
    )
    {
        ...
        {
            builder
                .Append("Environment.GetEnvironmentVariable(")
                .Append('"')
                .Append(Escape(key))
                .Append("\")");
            return true;
        ...
        {
            builder
                .Append("(long.TryParse(Environment.GetEnvironmentVariable(")
                .Append('"')
                .Append(Escape(key))
                .Append("\"), out var __environmentValue) ? __environmentValue : 0L)");
            return true;
        }
        builder.Append(
            key switch
    ...
    private bool IsCSharpConstant(CoreAstNode? node) =>
        node?.Kind
            is CoreNodeKind.BooleanLiteral
                or CoreNodeKind.IntegerLiteral
                or CoreNodeKind.DoubleLiteral
                or CoreNodeKind.SimpleStringLiteral;
    ...
    private CoreResolvedDeclaration? FindDeclaration(string? elementId) =>
        _currentDeclarations?.FirstOrDefault(declaration =>
            declaration.Element.CanonicalId == elementId
        );

    private bool TryResolvePromotedMemberOwner(
        CoreAstNode node,
        string targetType,
        string memberName,
    ...
    )
    {
        ...
        if (
            separator <= 0
            || !string.Equals(symbol[(separator + 1)..], memberName, StringComparison.Ordinal)
        )
        {
            return false;
        }
        ...
        var owner =
            _semanticIndex.FindDeclarationByCanonicalId($"{elementLibrary}#{ownerName}")
            ?? FindGlobalDeclaration(MapType(ownerName));
        var target = FindGlobalDeclaration(MapType(targetType));
        if (
            owner?.Name == "ViewportNotificationMixin"
        ...
        if (
            ...
            && (
                _currentDeclarations?.Any(declaration =>
                    declaration.Element.TypeParameters?.Any(parameter =>
    ...
    private bool IsCurrentTypeParameter(string typeName) =>
        _currentDeclarations?.Any(declaration =>
            declaration.Element.TypeParameters?.Any(parameter => parameter.Name == typeName) == true
        ) == true;

    private bool RequiresDynamicDispatch(string? dartType, string? memberElementId)
    {
        if (dartType?.TrimEnd('?') is "Object" or "object")
        {
            return true;
        }
    ...
    private bool RequiresDynamicInvocationDispatch(string? dartType, string methodName)
    {
        ...
        if (
            methodName
            is "debugValidateChild"
                or "getPositionForPoint"
                or "scheduleLayoutCallback"
                or "globalToLocal"
                or "localToGlobal"
                or "getRectForComposingRange"
                or "getEndpointsForSelection"
                or "setChild"
                or "setFlatChildren"
                or "_insertChild"
                or "_removeChild"
                ...
                or "indexOf"
                or "insert"
                or "prepareInitialFrame"
                or "triggerRebuild"
                or "beginActivity"
                or "disposePostFrame"
                or "move"
                or "_dragCancelCallback"
                or "_updateCallback"
                or "toStringDeep"
        )
        {
        ...
        )?.Element.TypeParameters?.FirstOrDefault(parameter =>
            string.Equals(parameter.Name, typeParameterName, StringComparison.Ordinal)
        );
        var mappedBound = MapType(typeParameter?.Bound ?? string.Empty).TrimEnd('?');
        return mappedBound == "RenderObject"
            || mappedBound.EndsWith(".RenderObject", StringComparison.Ordinal);
    }

    private bool RequiresDynamicPropertyDispatch(
        string? dartType,
        string? memberElementId,
        string memberName
    )
    {
        var mappedType = MapType(dartType ?? string.Empty).TrimEnd('?');
        return RequiresDynamicDispatch(dartType, memberElementId)
            ...
            || memberName
                is "child"
                    or "center"
                    or "debugChildIntegrityEnabled"
                    or "_element"
                    or "_updateCallback"
                    or "_callback"
                    or "_deferredLayoutChild"
                    or "_dragCancelCallback"
                    or "mouseTracker"
                    or "simulation"
                    or "invoke"
                    or "textDirection"
                    or "_currentDrag"
                    or "lastOverlapsContent"
                    or "lastShrinkOffset"
            || (
                memberName == "list"
                && memberElementId?.StartsWith("dart:ui#SemanticsRole.", StringComparison.Ordinal)
                    != true
            );
    }
    ...
    private bool IsTopLevelElement(string? elementId, string name)
    {
        var marker = elementId?.LastIndexOf('#') ?? -1;
        return marker >= 0
            && string.Equals(elementId![(marker + 1)..], name, StringComparison.Ordinal);
    }

    private string MapMemberName(string name) =>
        name switch
        {
            "toString" => "ToString",
            "add" => "Add",
    ...
    private string MapPropertyAccessName(string? elementId, string name, string? targetType)
    {
        ...
        if (
            ...
            && (
                ...
                || (
                    ...
                    parameter.Name == (targetType ?? string.Empty).TrimEnd('?')
                    && MapType(parameter.Bound ?? string.Empty).TrimEnd('?') == "Enum"
                ) == true
            )
        )
        {
        ...
        if (
            IsDartCollectionType(targetType)
        ...
        return
            member is not null
            && owner is not null
            && !LibraryUriFromElementId(owner.Element.CanonicalId)
                .StartsWith("dart:", StringComparison.Ordinal)
    ...
    {
        if (
            targetType == "AnimationStatus"
            && memberName
                is "isDismissed"
                    or "isCompleted"
                    or "isAnimating"
                    or "isForwardOrCompleted"
        )
        {
            builder
                .Append("global::Doroti.Framework.Animation.AnimationStatusMembers.")
                .Append(SafeIdentifier(memberName))
                .Append('(');
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append(')');
            return true;
        ...
        if (
            enumDeclaration?.Ast.Kind != CoreNodeKind.EnumDeclaration
    ...
    private string EscapeInterpolated(string value) =>
        Escape(value)
            .Replace("{", "{{", StringComparison.Ordinal)
            .Replace("}", "}}", StringComparison.Ordinal);

    private static CsOrigin ToCsOrigin(SourceOrigin origin) =>
        new(origin.Source, origin.Offset, origin.Length, origin.SymbolId?.Value);
}
