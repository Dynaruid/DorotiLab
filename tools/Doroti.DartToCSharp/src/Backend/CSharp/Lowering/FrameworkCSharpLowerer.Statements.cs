...
internal sealed partial class FrameworkCSharpLowerer
{
    ...
    )
    {
        ...
        foreach (
            var variable in block
                .Children.Where(item => item.Kind == CoreNodeKind.VariableDeclarationStatement)
                .SelectMany(item =>
                    DescendantsAndSelf(item)
                        .Where(child => child.Kind == CoreNodeKind.VariableDeclaration)
                )
        )
        {
            ...
            {
                body = body.RenameIdentifier(
                    SafeIdentifier(name),
                    $"{SafeIdentifier(name)}__{variable.Offset}"
                );
            }
        }
        builder.Append(body);
    ...
    )
    {
        ...
        switch (node.Kind)
        {
            ...
            case CoreNodeKind.ReturnStatement:
            {
                ...
                var returnMember = returnOwner
                    .Members.Where(candidate => ContainsOffset(candidate.Ast, node.Offset))
                    .OrderBy(candidate => candidate.Ast.Length)
                    .FirstOrDefault();
                if (
                    expression is null
                    && returnMember is not null
                    && DescendantsExcludingNestedFunctions(returnMember.Ast)
                        .Any(candidate => candidate.Kind == CoreNodeKind.YieldStatement)
                )
                {
                ...
                if (
                    ...
                    && string.Equals(
                        MapType(sourceContainingReturn),
                        "void",
                ...
                )
                {
                    ...
                    if (
                        ...
                        && expression.Kind
                            is not (CoreNodeKind.ThrowExpression or CoreNodeKind.RethrowExpression)
                    )
                    {
                        builder.Append("_ = ");
                    }
                ...
                var mappedExpressionType = MapType(
                    expression is null
                        ? string.Empty
                        : ResolvedExpressionValueType(expression)
                ...
                if (
                    ...
                    && DescendantsAndSelf(expression)
                        .Any(item =>
                            ...
                            && item.Text(CoreProperty.name) == "parser"
                        )
                )
                {
                    ...
                    var resultType =
                        start >= 0 && end > start ? containingReturn[(start + 1)..end] : "object";
                    builder
                        .Append(prefix)
                        .Append("return await DartAsyncRuntime.AwaitFutureOrValue<")
                        .Append(resultType)
                        .Append(">(");
                    LowerExpression(
                        builder,
                ...
                if (
                    expression is not null
                    && expression.Kind != CoreNodeKind.AwaitExpression
                    && IsInsideDartAsyncFunction(declaration, node)
                ...
                if (
                    expression is not null
                    && MapType(sourceContainingReturn) == "void"
                    && expression.StaticType == "void"
                )
                {
                    builder.Append(prefix);
                    LowerExpression(
                ...
                if (expression is not null)
                {
                    ...
                    if (returnOwner.Name == "_LerpSides" && returnMember?.Name == "resolve")
                    {
                        ...
                        returnOwner.Name == "ContextAction"
                        && returnMember?.Name == "_makeOverridableAction"
                        && returnOwner.Element.TypeParameters
                            is { Length: 1 } contextActionParameters
                    )
                    {
                        mappedReturn =
                            $"ContextAction<{SafeIdentifier(contextActionParameters[0].Name)}>";
                    }
                    if (
                        IsInsideDartAsyncFunction(declaration, node)
                        && mappedReturn.TrimEnd('?').StartsWith("Future<", StringComparison.Ordinal)
                        && TryGetGenericTypeArguments(
                            mappedReturn.TrimEnd('?'),
                            out var asyncReturnArguments
                        )
                    ...
                    var returnsTypeParameter =
                        expression.Kind == CoreNodeKind.NullLiteral
                        && (
                            (declaration.Element.TypeParameters ?? []).Any(parameter =>
                                containingReturn.Contains(parameter.Name, StringComparison.Ordinal)
                            ) || containingReturn.Contains("T?", StringComparison.Ordinal)
                        );
                    if (returnsTypeParameter)
                    {
                    ...
                    else if (expression.Kind == CoreNodeKind.AssignmentExpression)
                    {
                        ...
                        && (
                            expression.StaticType?.Contains(" Function", StringComparison.Ordinal)
                                == true
                    ...
                    )
                    {
                        ...
                        IsValueType(mappedReturn)
                        && !mappedReturn.EndsWith("?", StringComparison.Ordinal)
                        && (
                            ...
                            || (
                                ...
                                && (
                                    ...
                                    ?? expression.Children.FirstOrDefault(item =>
                                ...
                                && (
                                    ...
                                    || DescendantsAndSelf(assignmentTarget)
                                        .Any(item =>
                                            item.Text(CoreProperty.name) == "_cachedLineBreakCount"
                                        )
                                )
                            )
                        )
                    )
                    {
                        ...
                        mappedExpressionType.TrimEnd('?') is "object" or "dynamic"
                    ...
                    )
                    {
                        ...
                        mappedReturn.StartsWith("List<", StringComparison.Ordinal)
                        && mappedExpressionType.StartsWith(
                            "IReadOnlyList<",
                            StringComparison.Ordinal
                        )
                        && TryGetGenericTypeArguments(mappedReturn, out var returnListArguments)
                        && TryGetGenericTypeArguments(
                            mappedExpressionType,
                            out var expressionListArguments
                        )
                        && returnListArguments.SequenceEqual(
                    ...
                    )
                    {
                        ...
                        !IsValueType(mappedReturn.TrimEnd('?'))
                        && !IsValueType(mappedExpressionType.TrimEnd('?'))
                        && mappedReturn.Length > 0
                        && mappedExpressionType.Length > 0
                        && mappedReturn.TrimEnd('?') is not ("object" or "dynamic" or "void")
                        && mappedExpressionType.TrimEnd('?')
                            is not ("object" or "dynamic" or "void")
                        && !string.Equals(
                            mappedReturn.TrimEnd('?'),
                            mappedExpressionType.TrimEnd('?'),
                            StringComparison.Ordinal
                    ...
                    else
                    {
                        ...
                        if (
                            containingReturn.Contains("List<", StringComparison.Ordinal)
                            && expression.Kind == CoreNodeKind.MethodInvocation
                            && expression.Text(CoreProperty.name) == "cast"
                        )
                        {
                            builder.Append(".ToList()");
                        }
            ...
            case CoreNodeKind.ExpressionStatement:
            {
                ...
                if (expression is not null)
                {
                    if (
                        ...
                        && AppliedMixinDeclarations(_session.ActiveDonorDeclaration ?? declaration)
                            ...
                            .LastOrDefault(candidate =>
                                ...
                                && candidate.Member.Name == superMethodName
                            )
                            is { } mixinSuper
                        && mixinSuper.Member.Ast.Children.FirstOrDefault(item =>
                    ...
                    )
                    {
                        ...
                        var superArguments =
                            expression
                                .Child(CoreChildRole.argumentsOffset)
                                ?.Children.Where(item => item.Category == "expression")
                                .Select(item =>
                                    item.Kind == CoreNodeKind.NamedExpression
                                        ? item.Children.FirstOrDefault(child =>
                                            child.Category == "expression"
                                        ) ?? item
                                        : item
                                )
                                .ToArray()
                        ...
                        foreach (
                            var mixinStatement in mixinBlock.Children.Where(item =>
                                item.Category == "statement"
                                && item.Kind != CoreNodeKind.ReturnStatement
                            )
                        )
                        {
                            ...
                            for (
                                ...
                                parameterIndex < donorParameters.Length
                                    && parameterIndex < superArguments.Length;
                                parameterIndex++
                            )
                            {
                                ...
                                if (argument.Kind == CoreNodeKind.SimpleIdentifier)
                                {
                                    mixinSyntax = mixinSyntax.RenameIdentifier(
                                        ...
                                        EmittedLocalIdentifier(
                                            argument,
                                            argument.Text(CoreProperty.name)
                                                ?? donorParameters[parameterIndex].Name
                                        )
                                    );
                                }
                            }
                    ...
                    if (
                        expression.Kind == CoreNodeKind.MethodInvocation
                        && expression.Text(CoreProperty.name) == "setRange"
                    )
                    {
                        builder.Append(prefix);
                        EmitSetRange(
                    ...
                    while (
                        statementExpression.Kind == CoreNodeKind.ParenthesizedExpression
                        && statementExpression.Child(CoreChildRole.expressionOffset)
                            is { } unwrappedStatementExpression
                    )
                    {
                    ...
                    if (ExpressionProducesFuture(expression))
                    {
                        ...
                        expression.Kind
                        is not (
                            CoreNodeKind.MethodInvocation
                            or CoreNodeKind.FunctionExpressionInvocation
                            or CoreNodeKind.AssignmentExpression
                            or CoreNodeKind.PrefixExpression
                            or CoreNodeKind.PostfixExpression
                            or CoreNodeKind.InstanceCreationExpression
                            or CoreNodeKind.AwaitExpression
                            or CoreNodeKind.ThrowExpression
                            or CoreNodeKind.RethrowExpression
                        )
                    )
                    {
            ...
            {
                var condition = node.Child(CoreChildRole.conditionOffset);
                var caseClause = node.Children.FirstOrDefault(item =>
                ...
                if (caseClause is null)
                {
                    var promotionCondition =
                        condition.Kind == CoreNodeKind.ParenthesizedExpression
                ...
                if (promotions.Count > 0)
                {
                    var assignmentRoots = DescendantsAndSelf(thenStatement)
                        ...
                        .Cast<CoreAstNode>()
                        .Concat(
                            DescendantsAndSelf(thenStatement)
                        ...
                        .Concat(
                            DescendantsAndSelf(thenStatement)
                                .Where(item =>
                                    (
                                        item.Kind
                                        is CoreNodeKind.PrefixExpression
                                            or CoreNodeKind.PostfixExpression
                                ...
                                .Where(child =>
                                    child.Kind
                                        is CoreNodeKind.SimpleIdentifier
                                            or CoreNodeKind.AssignedVariablePattern
                                )
                                .Select(child =>
                                    SafeIdentifier(child.Text(CoreProperty.name) ?? string.Empty)
                                )
                        )
                        .ToHashSet(StringComparer.Ordinal);
                ...
                var loopParts = node.Children.FirstOrDefault(item =>
                    item.Kind
                        is CoreNodeKind.ForPartsWithDeclarations
                            ...
                            or CoreNodeKind.ForEachPartsWithDeclaration
                            or CoreNodeKind.ForEachPartsWithIdentifier
                            or CoreNodeKind.ForEachPartsWithPattern
                );
            ...
            {
                var declarationList = node.Children.FirstOrDefault(item =>
                ...
                var typeNode = declarationList.Children.FirstOrDefault(item =>
                    item.Category == "type"
                );
                var variables = declarationList
                    .Children.Where(item => item.Kind == CoreNodeKind.VariableDeclaration)
                    .ToArray();
                foreach (var variable in variables)
                {
                    var name = EmittedLocalIdentifier(
                        variable,
                        variable.Text(CoreProperty.name) ?? "missing"
                    );
                    ...
                    {
                        var contextualInitializerType = MapType(
                            initializer.StaticType ?? string.Empty
                        );
                        if (
                            !string.IsNullOrEmpty(contextualInitializerType)
                    ...
                    if (
                        ...
                        && initializer?.StaticType?.Contains(" Function", StringComparison.Ordinal)
                            == true
                        && (
                    ...
                    if (
                        (_session.ActiveDonorDeclaration ?? declaration).Name == "TreeSliver"
                    ...
                    if (initializer is not null)
                    {
                        ...
                        var initializerType = MapType(
                            ResolvedExpressionValueType(initializer)
                                ?? initializer.StaticType
                                ?? string.Empty
                        );
                        var needsListConversion =
                            ...
                            && (
                                ...
                                || (
                                    initializer.Kind == CoreNodeKind.MethodInvocation
                                    && initializer.Text(CoreProperty.name) == "cast"
                                )
                            );
                        var needsMapConversion =
                            type.StartsWith("DartMap<", StringComparison.Ordinal)
                            && initializerType.StartsWith("DartMap<", StringComparison.Ordinal)
                            && type != initializerType;
                        var needsCheckedReferenceCast =
                            type != "var"
                            && !needsListConversion
                            && !needsMapConversion
                            ...
                            && !string.Equals(
                                type.TrimEnd('?'),
                                initializerType.TrimEnd('?'),
                                StringComparison.Ordinal
                            );
                        var needsRequiredValue =
                            type != "var"
                            && IsValueType(type.TrimEnd('?'))
                            && !type.EndsWith("?", StringComparison.Ordinal)
                            && initializerType == type + "?";
                        var needsObjectValueCast =
                            type != "var"
                            && IsValueType(type.TrimEnd('?'))
                            && initializerType.TrimEnd('?') is "object" or "dynamic";
                        var wrapAwait =
                            initializer.Kind == CoreNodeKind.AwaitExpression
                        ...
                        if (
                            initializer.Kind == CoreNodeKind.FunctionExpression
                            && (
                        ...
                        {
                            while (
                                expressionToLower.Kind
                                    is CoreNodeKind.AsExpression
                                        or CoreNodeKind.ParenthesizedExpression
                                        or CoreNodeKind.PostfixExpression
                            )
                            {
                                var unwrapped =
                                    expressionToLower.Child(CoreChildRole.expressionOffset)
                        ...
                        {
                            if (
                                TryGetGenericTypeArguments(type, out var targetArguments)
                                && TryGetGenericTypeArguments(
                                    initializerType,
                                    out var sourceArguments
                                )
                        ...
                        else if (needsMapConversion)
                        {
                            builder
                                .Append(".cast<")
                                .Append(DartMapTypeArguments(type))
                                .Append(">()");
                        }
                        if (needsRequiredValue || needsCheckedReferenceCast)
                        {
                            builder.Append(')');
                        }
                    }
            ...
            {
                var patternDeclaration = node.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.PatternVariableDeclaration
                );
                var pattern = patternDeclaration?.Children.FirstOrDefault(item =>
                    item.Category == "pattern"
                );
                var initializer = patternDeclaration?.Children.FirstOrDefault(item =>
                    item.Category == "expression"
                );
                ...
                if (pattern.Kind == CoreNodeKind.ObjectPattern)
                {
                    ...
                    builder.AppendLine(";");
                    foreach (
                        var field in pattern.Children.Where(item =>
                            item.Kind == CoreNodeKind.PatternField
                        )
                    )
                    {
                        var fieldPattern = field.Children.FirstOrDefault(item =>
                            item.Category == "pattern"
                        );
                        var variablePattern = fieldPattern is null
                            ? null
                            : DescendantsAndSelf(fieldPattern)
                                .FirstOrDefault(item =>
                                    item.Kind == CoreNodeKind.DeclaredVariablePattern
                                );
                        ...
                        var fieldName =
                            field
                                .Children.FirstOrDefault(item =>
                                    item.Kind == CoreNodeKind.PatternFieldName
                                )
                        ...
                        var variableTypeNode = variablePattern.Children.FirstOrDefault(item =>
            ...
            case CoreNodeKind.TryStatement:
            {
                ...
                var catchClauses = node
                    .Children.Where(item => item.Kind == CoreNodeKind.CatchClause)
                    .ToArray();
                var finallyClause = node.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.FinallyClause
                );
                var directBlocks = node
                    .Children.Where(item => item.Kind == CoreNodeKind.Block)
                    .ToArray();
                var directFinallyBody =
                    catchClauses.Length == 0 && directBlocks.Length > 1 ? directBlocks[^1] : null;
                if (body is null)
                {
                    break;
                }
                ...
                {
                    var catchTypeNode = clause.Children.FirstOrDefault(item =>
                        item.Kind == CoreNodeKind.NamedType
                    );
                    var catchType = catchTypeNode is null
                        ? "Exception"
                        : MapTypeFromAst(catchTypeNode);
                    var catchParameters = clause
                        .Children.Where(item => item.Kind == CoreNodeKind.CatchClauseParameter)
                        .ToArray();
                    ...
                    var exceptionName = exceptionParameter is null
                        ? null
                        : exceptionParameter.Text(CoreProperty.name);
                    var stackName = stackParameter is null
                        ? null
                        : stackParameter.Text(CoreProperty.name);
                    var emittedExceptionName =
                        exceptionParameter is null || exceptionName is null
                            ? null
                            : $"{SafeIdentifier(exceptionName)}__{exceptionParameter.Offset}";
                    var emittedStackName =
                        stackParameter is null || stackName is null
                            ? null
                            : $"{SafeIdentifier(stackName)}__{stackParameter.Offset}";
                    var catchBody = clause.Children.FirstOrDefault(item =>
                        item.Kind == CoreNodeKind.Block
                    );
                    ...
                    else
                    {
                        builder
                            .Append(prefix)
                            .AppendLine($"catch ({catchType} {emittedExceptionName})");
                    }
                    builder.AppendLine($"{prefix}{{");
                    if (stackName is not null and not "_")
                    {
                        builder.AppendLine(
                            $"{new string(' ', (indent + 1) * 4)}var {emittedStackName} = new System.Diagnostics.StackTrace();"
                        );
                    }
                    EmitBlockBody(
                        builder,
                ...
                {
                    var finallyBody = finallyClause.Children.FirstOrDefault(item =>
                        item.Kind == CoreNodeKind.Block
                    );
        ...
        AddUnsupportedDiagnostic(
            ...
            node,
            "statement",
            "Add a typed statement visitor for this AST node before selecting the containing library."
        );
    }
    ...
    )
    {
        var prefix = new string(' ', indent * 4);
        var expression =
            node.Child(CoreChildRole.expressionOffset)
            ?? node.Children.FirstOrDefault(item => item.Category == "expression");
        ...
        builder.AppendLine(prefix + "{");
        foreach (
            var @case in node.Children.Where(item => item.Kind == CoreNodeKind.SwitchExpressionCase)
        )
        {
            var pattern =
                @case.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.GuardedPattern)
            ...
            var result =
                @case.Child(CoreChildRole.expressionOffset)
    ...
    )
    {
        ...
        builder.AppendLine(")");
        builder.AppendLine($"{prefix}{{");
        var switchMembers = node
            .Children.Where(item =>
                item.Kind
                    is CoreNodeKind.SwitchCase
                        or CoreNodeKind.SwitchPatternCase
                        or CoreNodeKind.SwitchDefault
            )
            .ToArray();
        ...
        for (var memberIndex = 0; memberIndex < switchMembers.Length; memberIndex++)
        {
            ...
            var casePrefix = new string(' ', (indent + 1) * 4);
            var pattern =
                member.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.GuardedPattern)
                ?? member.Children.FirstOrDefault(item =>
                    item.Category is "pattern" or "expression"
                );
            var statements = member.Children.Where(item => item.Category == "statement").ToArray();
            if (
                member.Kind != CoreNodeKind.SwitchDefault
                && statements.Length == 0
                && memberIndex + 1 < switchMembers.Length
                && switchMembers[memberIndex + 1]
                    is { Kind: not CoreNodeKind.SwitchDefault } nextMember
            )
            {
                var nextPattern =
                    nextMember.Children.FirstOrDefault(item =>
                        item.Kind == CoreNodeKind.GuardedPattern
                    )
                    ?? nextMember.Children.FirstOrDefault(item =>
                        item.Category is "pattern" or "expression"
                    );
                var nextStatements = nextMember
                    .Children.Where(item => item.Category == "statement")
                    .ToArray();
                var bindings = pattern is null
                    ? []
                    : DescendantsAndSelf(pattern)
                        .Where(item => item.Kind == CoreNodeKind.DeclaredVariablePattern)
                        .GroupBy(
                            item => item.Text(CoreProperty.name) ?? "value",
                            StringComparer.Ordinal
                        )
                        .ToDictionary(
                            group => group.Key,
                            group => group.OrderBy(item => item.Offset).First(),
                            StringComparer.Ordinal
                        );
                var nextBindings = nextPattern is null
                    ? []
                    : DescendantsAndSelf(nextPattern)
                        .Where(item => item.Kind == CoreNodeKind.DeclaredVariablePattern)
                        .GroupBy(
                            item => item.Text(CoreProperty.name) ?? "value",
                            StringComparer.Ordinal
                ...
                if (
                    ...
                    && bindings
                        ...
                        .SequenceEqual(
                            nextBindings.Keys.OrderBy(item => item, StringComparer.Ordinal),
                            StringComparer.Ordinal
                        )
            ...
            if (
                member.Kind == CoreNodeKind.SwitchDefault
                || (pattern is not null && IsCatchAllPattern(pattern))
            )
            {
                builder.AppendLine($"{casePrefix}default:");
            }
            ...
            if (pendingCaseBodyRenames.TryGetValue(memberIndex, out var caseBodyRenames))
            {
                ...
                {
                    renderedCaseBody = renderedCaseBody.RenameIdentifier(
                        rename.Key,
                        rename.Value,
                        renameAssignments: false
                    );
        ...
        var hasDefault = switchMembers.Any(member =>
            member.Kind == CoreNodeKind.SwitchDefault
        ...
        if (
            !hasDefault
            && nonEmptyCaseStatements.Length > 0
    ...
    private bool IsTerminatingStatement(CoreAstNode statement) =>
        statement.Kind switch
        {
            CoreNodeKind.ReturnStatement
            or CoreNodeKind.BreakStatement
            or CoreNodeKind.ContinueStatement => true,
            CoreNodeKind.ExpressionStatement => statement.Children.Any(item =>
                item.Kind is CoreNodeKind.ThrowExpression or CoreNodeKind.RethrowExpression
            ),
            _ => false,
        };
    ...
    )
    {
        var prefix = new string(' ', indent * 4);
        var function =
            node.Child(CoreChildRole.functionOffset)
            ?? node.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.FunctionDeclaration);
        var functionExpression = function?.Children.FirstOrDefault(item =>
        ...
        if (function is null || functionExpression is null)
        {
            AddUnsupportedDiagnostic(
                ...
                "Resolve the local function declaration and signature."
            );
            return;
        }
        ...
        var parameterCount = int.TryParse(node.Text(CoreProperty.parameterCount), out var count)
            ? count
            : 0;
        var formalParameters =
            functionExpression
                .Child(CoreChildRole.parametersOffset)
        ...
        var parameters = Enumerable
            ...
            .Select(index =>
            {
                ...
                if (
                    index < formalParameters.Length
                    && formalParameters[index].Kind == CoreNodeKind.DefaultFormalParameter
                )
                {
                    var defaultExpression = formalParameters[index]
                        .Children.FirstOrDefault(item => item.Category == "expression");
                    suffix = defaultExpression?.Kind switch
                    {
                        CoreNodeKind.BooleanLiteral => " = "
                            + (defaultExpression.Text(CoreProperty.value) ?? "false"),
                        CoreNodeKind.IntegerLiteral or CoreNodeKind.DoubleLiteral => " = "
                            + (defaultExpression.Text(CoreProperty.value) ?? "0"),
                        CoreNodeKind.NullLiteral => " = null",
                        _ => string.Empty,
                    };
                    if (
                        defaultExpression is null
                        && MapType(node.ParameterType(index) ?? "object")
        ...
        else
        {
            ...
            if (expression is null)
            {
                AddUnsupportedDiagnostic(
                    ...
                    node,
                    "local-function-body",
                    "Resolve the local function body."
                );
            }
            else
            {
    ...
    )
    {
        ...
        if (
            condition.Kind == CoreNodeKind.BinaryExpression
            && condition.Text(CoreProperty.@operator) == "&&"
        )
        {
            if (condition.Child(CoreChildRole.leftOffset) is { } left)
            {
                CollectConditionPromotions(left, declaration, promotions);
            }

            if (condition.Child(CoreChildRole.rightOffset) is { } right)
            {
                CollectConditionPromotions(right, declaration, promotions);
            }
        ...
        if (
            condition.Kind == CoreNodeKind.BinaryExpression
            && condition.Text(CoreProperty.@operator) == "!="
            && condition.Child(CoreChildRole.leftOffset)
                is { Kind: CoreNodeKind.SimpleIdentifier } nullableExpression
            && condition.Child(CoreChildRole.rightOffset) is { Kind: CoreNodeKind.NullLiteral }
            && nullableExpression.StaticType is { } nullableType
            && IsValueType(MapType(nullableType).TrimEnd('?'))
            && IsLocalOrParameter(nullableExpression, declaration)
        )
        {
            var name = EmittedLocalIdentifier(
                nullableExpression,
                nullableExpression.Text(CoreProperty.name) ?? "value"
            );
            promotions.Add(
                (name, $"{name}__value{condition.Offset}", MapType(nullableType).TrimEnd('?'), true)
            );
            return;
        }
        if (
            condition.Kind == CoreNodeKind.IsExpression
            && condition.Text(CoreProperty.@operator) != "is!"
            && condition.Text(CoreProperty.isNot) != "true"
            && condition.Child(CoreChildRole.expressionOffset)
                is { Kind: CoreNodeKind.SimpleIdentifier } promotedExpression
        ...
        )
        {
            var name = EmittedLocalIdentifier(
                promotedExpression,
                promotedExpression.Text(CoreProperty.name) ?? "value"
            );
    ...
    )
    {
        ...
        foreach (var promotion in promotions.DistinctBy(item => item.Name))
        {
            ...
            else
            {
                ...
                if (IsTypeParameter(promotion.Type, declaration))
                {
                    builder.Append("(object)");
                }
    ...
    )
    {
        if (loopParts.Kind == CoreNodeKind.ForEachPartsWithPattern)
        {
            ...
            var patternIterable = loopParts.Children.FirstOrDefault(item =>
                item.Category == "expression"
            );
            if (pattern is null || patternIterable is null)
            {
                AddUnsupportedDiagnostic(
                    ...
                    "Provide the typed foreach pattern and iterable."
                );
                return;
            }
        ...
        var loopVariable = loopParts.Children.FirstOrDefault(item =>
            item.Kind is CoreNodeKind.DeclaredIdentifier or CoreNodeKind.SimpleIdentifier
        );
        var iterable = loopParts.Children.FirstOrDefault(item =>
            item != loopVariable && item.Category == "expression"
        );
        ...
        if (name is null || iterable is null)
        {
            AddUnsupportedDiagnostic(
                ...
                loopParts,
                "for-each-shape",
                "Provide the typed foreach variable and iterable."
            );
            return;
        }
        var typeNode =
            loopVariable?.Kind == CoreNodeKind.DeclaredIdentifier
                ? loopVariable.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.NamedType)
                : null;
        var type = typeNode is null ? "var" : MapTypeFromAst(typeNode);
        builder
            .Append(prefix)
            .Append($"foreach ({type} {EmittedLocalIdentifier(loopVariable!, name)} in ");
    ...
    {
        var variableList = loopParts.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.VariableDeclarationList
        );
        var expressions = loopParts.Children.Where(item => item.Category == "expression").ToArray();
        var initExpression = variableList is null
            ? loopParts.Child(CoreChildRole.initializerOffset)
            : null;
        ...
        var updaters = expressions
            .Where(item =>
                item.Offset != initExpression?.Offset && item.Offset != condition?.Offset
            )
            .ToArray();
    ...
    {
        var typeNode = variableList.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.NamedType
        );
    ...
    )
    {
        ...
        var body = DescendantsAndSelf(function)
            .FirstOrDefault(item => item.Kind == CoreNodeKind.Block);
        if (body is null)
        {
            AddUnsupportedDiagnostic(
                ...
                function,
                "closure-body",
                "Add typed expression-body closure lowering."
            );
        }
        else
        {
            ...
            try
            {
                ...
                for (
                    var index = 0;
                    index < formalParameters.Length && index < argumentValues.Length;
                    index++
                )
                {
                    var parameter = formalParameters[index];
                    var parameterName = SafeIdentifier(
                        parameter.Text(CoreProperty.name) ?? $"arg{index}"
                    );
                    var parameterTypeNode = parameter.Children.FirstOrDefault(item =>
                        item.Category == "type"
                    );
