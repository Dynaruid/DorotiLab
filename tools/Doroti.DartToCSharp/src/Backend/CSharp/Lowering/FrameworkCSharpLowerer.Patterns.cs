...
internal sealed partial class FrameworkCSharpLowerer
{
    ...
    )
    {
        ...
        var cases = node
            .Children.Where(item => item.Kind == CoreNodeKind.SwitchExpressionCase)
            .ToArray();
        var mappedSwitchResultType = MapType(node.StaticType ?? string.Empty);
        var targetTypeRecordArms =
            mappedSwitchResultType.StartsWith('(') && mappedSwitchResultType.EndsWith(')');
        var patternExtensionDeclaration =
        ...
        var representation =
            patternExtensionDeclaration?.Ast.Kind == CoreNodeKind.ExtensionTypeDeclaration
                ? patternExtensionDeclaration.Ast.Children.FirstOrDefault(item =>
        ...
        var unwrapExtensionValue =
            representationName is not null
            && cases
                .SelectMany(DescendantsAndSelf)
                .Any(item =>
                    item.Kind is CoreNodeKind.RecordPattern or CoreNodeKind.NullCheckPattern
                );
        var unwrapExtensionRecord =
            unwrapExtensionValue && expression?.Kind == CoreNodeKind.RecordLiteral;
        ...
        if (expression is not null)
        {
            if (unwrapExtensionRecord)
            {
                ...
                for (var index = 0; index < fields.Length; index++)
                {
                    ...
                    var field =
                        fields[index].Kind == CoreNodeKind.NamedExpression
                            ? fields[index]
                                .Children.FirstOrDefault(item => item.Category == "expression")
                                ?? fields[index]
                            : fields[index];
                    var fieldType = StripLibraryPrefix(field.StaticType ?? string.Empty)
                        .TrimEnd('?');
                    var unwrapField =
                        field.Kind == CoreNodeKind.ThisExpression
                        || string.Equals(
                            fieldType,
                            patternExtensionDeclaration!.Name,
        ...
        var switchInputName =
            expression?.Kind == CoreNodeKind.SimpleIdentifier
                ? EmittedLocalIdentifier(
                    expression,
                    expression.Text(CoreProperty.name) ?? string.Empty
                )
                : null;
        ...
        for (var index = 0; index < cases.Length; index++)
        {
            ...
            {
                expansionPattern = expansionPattern.Children.FirstOrDefault(item =>
                    item.Category == "pattern"
                );
            }
            var guardedHasWhenClause =
                pattern?.Kind == CoreNodeKind.GuardedPattern
                && pattern.Children.Any(item => item.Kind == CoreNodeKind.WhenClause);
            var armPatterns =
                !guardedHasWhenClause && expansionPattern?.Kind == CoreNodeKind.LogicalOrPattern
            ...
            foreach (var armPattern in armPatterns)
            {
                ...
                {
                    if (targetTypeRecordArms)
                    {
                        caseBuilder.Append("((").Append(mappedSwitchResultType).Append(")(");
                    }

                    var mappedResultType = MapType(result.StaticType ?? string.Empty).TrimEnd('?');
                    if (
                        result.Kind == CoreNodeKind.NullLiteral
                        && (
                            IsTypeParameter(
                                mappedSwitchResultType,
                    ...
                    else
                    {
                        if (mappedSwitchResultType.TrimEnd('?') is "object" or "dynamic")
                        {
                            ...
                            mappedSwitchResultType.Length > 0
                            && mappedSwitchResultType.TrimEnd('?') != "void"
                            && result.Kind
                                is not (
                                    CoreNodeKind.ThrowExpression
                                    or CoreNodeKind.RethrowExpression
                                )
                            && !string.Equals(
                                mappedSwitchResultType.TrimEnd('?'),
                                mappedResultType,
                                StringComparison.Ordinal
                            )
                        )
                        {
                            caseBuilder
                                ...
                                .Append(mappedSwitchResultType.TrimEnd('?'))
                                .Append(">(");
                            LowerExpression(
                                caseBuilder,
                                result,
                ...
                )
                {
                    ...
                    foreach (
                        var branchDeclaration in DescendantsAndSelf(armPattern)
                            .Where(item => item.Kind == CoreNodeKind.DeclaredVariablePattern)
                    )
                    {
                ...
                var armEffectivePattern =
                    armPattern.Kind == CoreNodeKind.GuardedPattern
                        ? armPattern.Children.FirstOrDefault(item => item.Category == "pattern")
                            ?? armPattern
                        : armPattern;
                if (
                    switchInputName is { Length: > 0 }
                    && armEffectivePattern.Kind == CoreNodeKind.ObjectPattern
                )
                {
                    caseSyntax = caseSyntax.RenameIdentifier(
                        switchInputName,
                        $"__object{armEffectivePattern.Offset}",
                        renameAssignments: false
                    );
                }
                if (
                ...
                )
                {
                    caseSyntax = caseSyntax.RenameIdentifier(
                        switchInputName,
                        $"__typed{armEffectivePattern.Offset}",
                        renameAssignments: false
                    );
                }
                foreach (
                    var declared in DescendantsAndSelf(armPattern)
                        .Where(item => item.Kind == CoreNodeKind.DeclaredVariablePattern)
                )
                {
                    ...
                    caseSyntax = caseSyntax.RenameIdentifier(
                        declaredName,
                        $"{declaredName}__pattern{declared.Offset}",
                        renameAssignments: false
                    );
                }
    ...
    {
        var guarded = switchCase.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.GuardedPattern
        );
        var pattern =
            guarded?.Children.FirstOrDefault(item => item.Category == "pattern")
    ...
    {
        var hasObject = cases.Any(switchCase =>
            DescendantsAndSelf(switchCase)
                .Any(node =>
                    node.Kind == CoreNodeKind.NamedType
                    && node.Text(CoreProperty.name) is "Object" or "object" or "dynamic"
                )
        );
        var hasNull = cases.Any(switchCase =>
            DescendantsAndSelf(switchCase).Any(node => node.Kind == CoreNodeKind.NullLiteral)
        );
    ...
    {
        while (
            pattern.Kind is CoreNodeKind.ParenthesizedPattern or CoreNodeKind.GuardedPattern
        ...
        if (
            pattern.Kind == CoreNodeKind.SimpleIdentifier
        ...
        if (
            pattern.Kind
                is CoreNodeKind.AssignedVariablePattern
                    or CoreNodeKind.DeclaredVariablePattern
            && !pattern.Children.Any(item => item.Kind == CoreNodeKind.NamedType)
            && pattern.Children.Any(item =>
        ...
        if (pattern.Kind == CoreNodeKind.RecordPattern)
        {
            var fields = pattern
                .Children.SelectMany(item =>
                    item.Category == "pattern"
                        ? [item]
                        : item.Children.Where(child => child.Category == "pattern")
                )
                .ToArray();
    ...
    private bool ClosedRecordSwitchIsExhaustive(CoreAstNode? expression, CoreAstNode[] cases)
    {
        ...
        if (
            string.IsNullOrEmpty(staticType)
            || !staticType.StartsWith('(')
            || !staticType.EndsWith(')')
        )
        {
            return false;
            ...
            for (var state = 0; state < stateCount; state++)
            {
                if (
                    fields
                        ...
                        .All(item =>
                            ClosedRecordFieldMatches(
                                item.field,
                                fieldTypes[item.index].Trim(),
                                (state & (1 << item.index)) != 0
                            )
                        )
                )
                {
    ...
    private bool ClosedNullableSwitchIsExhaustive(CoreAstNode? expression, CoreAstNode[] cases)
    {
        ...
        foreach (var switchCase in cases)
        {
            ...
            coversNull |=
                pattern is not null
                && DescendantsAndSelf(pattern).Any(item => item.Kind == CoreNodeKind.NullLiteral);
            coversValue |=
                pattern?.Kind == CoreNodeKind.DeclaredVariablePattern
                && pattern.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.NamedType)
                    is { } type
                && string.Equals(
                    StripLibraryPrefix(type.Text(CoreProperty.name) ?? string.Empty),
                    underlyingType,
                    StringComparison.Ordinal
                );
        }
    ...
    {
        while (
            pattern.Kind == CoreNodeKind.ParenthesizedPattern
            && pattern.Children.FirstOrDefault(item => item.Category == "pattern") is { } nested
        )
        {
            pattern = nested;
        ...
        if (fieldType == "bool")
        {
            var literal = DescendantsAndSelf(pattern)
                .FirstOrDefault(item => item.Kind == CoreNodeKind.BooleanLiteral);
        ...
        if (fieldType == "double?")
        {
            var isNull = DescendantsAndSelf(pattern)
                .Any(item => item.Kind == CoreNodeKind.NullLiteral);
            if (isNull)
            {
                return !secondState;
            }
            var typedNonNull = DescendantsAndSelf(pattern)
                .Any(item =>
    ...
    )
    {
        ...
        )
        {
            ...
            var namedOrder = SplitGenericArguments(fieldText)
                ...
                .ToArray();
            fields = fields
                .OrderBy(item => item.Kind == CoreNodeKind.NamedExpression ? 1 : 0)
                .ThenBy(item =>
                            ...
                            item.Text(CoreProperty.name)
                                ?? DescendantsAndSelf(item)
                                    .FirstOrDefault(candidate =>
                                        candidate.Kind == CoreNodeKind.Label
                                    )
        ...
        var namedFieldNames = fields
            .Where(item => item.Kind == CoreNodeKind.NamedExpression)
            .Select(item =>
                item.Text(CoreProperty.name)
                ?? DescendantsAndSelf(item)
                    .FirstOrDefault(candidate => candidate.Kind == CoreNodeKind.Label)
                    ?.Children.FirstOrDefault(candidate =>
                        candidate.Kind == CoreNodeKind.SimpleIdentifier
                    )
                    ?.Text(CoreProperty.name)
        ...
        {
            fields = fields
                .OrderBy(item => item.Text(CoreProperty.name) == "boundaryEnd" ? 0 : 1)
                .ToArray();
        }
        else if (namedFieldNames.SetEquals(["paragraph", "localPosition"]))
        {
            fields = fields
                .OrderBy(item => item.Text(CoreProperty.name) == "paragraph" ? 0 : 1)
                .ToArray();
        }
        else if (namedFieldNames.SetEquals(["startGlyphHeight", "endGlyphHeight"]))
        {
            fields = fields
                .OrderBy(item => item.Text(CoreProperty.name) == "startGlyphHeight" ? 0 : 1)
                .ToArray();
        }
        builder.Append('(');
        for (var index = 0; index < fields.Length; index++)
        {
            ...
            {
                var name =
                    field.Text(CoreProperty.name)
                    ?? DescendantsAndSelf(field)
                        .FirstOrDefault(candidate => candidate.Kind == CoreNodeKind.Label)
                        ?.Children.FirstOrDefault(candidate =>
                            candidate.Kind == CoreNodeKind.SimpleIdentifier
                        )
                ...
                {
                    builder.Append(SafeIdentifier(name)).Append(": ");
                }
    ...
    private void EmitDeconstructionPattern(
        CsSyntaxBuilder builder,
        CoreAstNode pattern,
        CoreResolvedDeclaration declaration
    )
    {
        ...
        {
            var fields =
                pattern.Kind == CoreNodeKind.RecordPattern
                    ? pattern
                        .Children.Where(item => item.Kind == CoreNodeKind.PatternField)
                        .ToArray()
            ...
            for (var index = 0; index < fields.Length; index++)
            {
                ...
                var nested =
                    fields[index].Kind == CoreNodeKind.PatternField
        ...
        if (
            pattern.Kind
            is CoreNodeKind.DeclaredVariablePattern
                or CoreNodeKind.AssignedVariablePattern
        )
        {
            var name = pattern.Text(CoreProperty.name) ?? "_";
            if (
                pattern.Kind == CoreNodeKind.AssignedVariablePattern
            ...
            )
            {
                ...
                var declarationNode = DescendantsAndSelf(owner.Ast)
                    ...
                    .FirstOrDefault();
                builder.Append(
                    declarationNode is null
                        ? SafeIdentifier(name)
                        : EmittedLocalIdentifier(declarationNode, name)
                );
            }
            else
            {
    ...
    )
    {
        ...
        switch (node.Kind)
        {
            case CoreNodeKind.DeclaredVariablePattern:
                var declaredType = node.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.NamedType
                );
                var isExtensionRepresentationPattern =
                ...
                builder
                    .Append(
                        isExtensionRepresentationPattern ? "var"
                        : declaredType is null ? "object"
                        : MapTypeFromAst(declaredType).TrimEnd('?')
                    )
                    .Append(' ')
            ...
            case CoreNodeKind.NullCheckPattern:
                EmitPattern(
                    builder,
                    node.Children.FirstOrDefault(item => item.Category == "pattern"),
                    declaration,
                    package,
                    library,
                    inputPath,
            ...
            case CoreNodeKind.AssignedVariablePattern:
                {
                    ...
                    var declarationNode = DescendantsAndSelf(owner.Ast)
                        ...
                        .FirstOrDefault();
                    builder.Append(
                        node.ElementId?.Contains("@local", StringComparison.Ordinal) == true
                            ? EmittedLocalIdentifier(node, name)
                        : declarationNode is null ? SafeIdentifier(name)
                        : EmittedLocalIdentifier(declarationNode, name)
                    );
                }
                return;
            case CoreNodeKind.RecordPattern:
            {
                ...
                for (var index = 0; index < recordFields.Length; index++)
                {
                    ...
                    EmitPattern(
                        builder,
                        recordFields[index]
                            .Children.FirstOrDefault(item => item.Category == "pattern"),
                        declaration,
                        package,
                        library,
                        inputPath,
            ...
            case CoreNodeKind.GuardedPattern:
                ...
                var guard =
                    node.Children.FirstOrDefault(item => item.Category == "expression")
                    ?? node.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.WhenClause)
                        ?.Children.FirstOrDefault(item => item.Category == "expression");
            ...
            case CoreNodeKind.WildcardPattern:
                var wildcardType = node.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.NamedType
                );
                if (wildcardType is not null)
                {
                    builder
                        .Append(MapTypeFromAst(wildcardType).TrimEnd('?'))
                        .Append(" __typed")
                        .Append(node.Offset);
                }
                else
            ...
            case CoreNodeKind.RelationalPattern:
            {
                ...
                var relationalOperand = node.Children.FirstOrDefault(item =>
            ...
            {
                var type = node.Children.FirstOrDefault(item =>
                    item.Kind == CoreNodeKind.NamedType
                );
                ...
                if (fields.Length == 1)
                {
                    ...
                    if (runtimeTypeName == "runtimeType")
                    {
                        ...
                        EmitDeconstructionPattern(
                            builder,
                            runtimeTypeField.Children.FirstOrDefault(item =>
                                item.Category == "pattern"
                            ) ?? runtimeTypeField,
                            declaration
                        );
                ...
                for (var index = 0; index < fields.Length; index++)
                {
                    ...
                    var field = fields[index];
                    var fieldName = field
                        .Children.FirstOrDefault(item => item.Kind == CoreNodeKind.PatternFieldName)
                        ?.Text(CoreProperty.name);
                    ...
                    {
                        fieldName =
                            fieldPattern?.Text(CoreProperty.name)
                            ?? (
                                fieldPattern is null
                                    ? null
            ...
            case CoreNodeKind.LogicalOrPattern:
                var alternatives = node
                    .Children.Where(item => item.Category == "pattern")
                    .ToArray();
                if (
                    alternatives.Length > 0
                    && alternatives.All(item => item.Kind == CoreNodeKind.ConstantPattern)
                    && alternatives.Any(item =>
                        item.Children.FirstOrDefault(child => child.Category == "expression")
                            is { } value
                        && !IsCSharpPatternConstant(value)
                    )
                )
                {
                    ...
                    for (var index = 0; index < alternatives.Length; index++)
                    {
                        ...
                        var value = alternatives[index]
                            .Children.FirstOrDefault(child => child.Category == "expression");
                    ...
                    alternatives.Length == 0
                        ...
                        : DescendantsAndSelf(alternatives[0])
                            ...
                            .ToDictionary(
                                ...
                                group =>
                                    EmittedLocalIdentifier(
                                        group.OrderBy(item => item.Offset).First(),
                                        group.Key
                                    ),
                                StringComparer.Ordinal
                            );
                for (var index = 0; index < alternatives.Length; index++)
                {
                    ...
                    foreach (
                        var declared in DescendantsAndSelf(alternatives[index])
                            .Where(item => item.Kind == CoreNodeKind.DeclaredVariablePattern)
                    )
                    {
    ...
    private bool IsCSharpPatternConstant(CoreAstNode expression)
    {
        if (
            expression.Kind
            is CoreNodeKind.NullLiteral
                or CoreNodeKind.BooleanLiteral
        ...
        if (
            expression.Kind == CoreNodeKind.PrefixExpression
            && expression.Children.Any(item =>
                item.Kind is CoreNodeKind.IntegerLiteral or CoreNodeKind.DoubleLiteral
            )
        )
        {
            return true;
    ...
    )
    {
        ...
        if (pattern?.Kind == CoreNodeKind.MapPattern)
        {
            var matchName =
                "__match"
                + pattern.Offset.ToString(System.Globalization.CultureInfo.InvariantCulture);
            LowerExpression(builder, value, declaration, package, library, inputPath, diagnostics);
            builder
                .Append(" is var ")
                .Append(matchName)
                .Append(" && DartPatternRuntime.IsMap(")
                .Append(matchName)
                .Append(')');
            var entryIndex = 0;
            foreach (
                var entry in pattern.Children.Where(item =>
                    item.Kind == CoreNodeKind.MapPatternEntry
                )
            )
            {
                var key = entry.Children.FirstOrDefault(item => item.Category == "expression");
                var entryPattern = entry.Children.FirstOrDefault(item =>
                    item.Category == "pattern"
                );
                var entryName = $"__entry{pattern.Offset}_{entryIndex++}";
                builder
                    .Append(" && DartPatternRuntime.TryGetMapValue(")
                    .Append(matchName)
                    .Append(", ");
                if (key is null)
                {
        ...
        var whenClause = guardedPattern?.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.WhenClause
        );
