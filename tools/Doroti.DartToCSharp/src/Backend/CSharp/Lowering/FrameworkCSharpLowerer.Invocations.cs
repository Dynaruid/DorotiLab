...
internal sealed partial class FrameworkCSharpLowerer
{
    ...
    {
        var constructorName =
            node.Child(CoreChildRole.constructorOffset)
            ?? node.Children.FirstOrDefault(item => item.Kind == CoreNodeKind.ConstructorName);
        ...
        var named = constructorName.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.SimpleIdentifier
        );
        var constructor =
            named?.Text(CoreProperty.name) ?? constructorName.Text(CoreProperty.name) ?? "new";
        ...
        var parameterCount =
            parameterStart >= 0 && parameterEnd > parameterStart
        ...
        {
            var member = TryResolveEmittedNamedConstructor(
                type,
                constructor,
    ...
    {
        var identifier = node.Children.FirstOrDefault(item =>
            item.Kind == CoreNodeKind.SimpleIdentifier
        );
        ...
        )
        {
            ...
            {
                builder
                    .Append(MapType(symbol[..separator]).TrimEnd('?'))
                    .Append('.')
    ...
    )
    {
        ...
        if (name == "dispatchPointerEvent")
        {
            builder
                ...
                .Append(
                    "__event.pointer, __event.embedderId, __event.platformData, __event.timeStamp, __event.position, __event.kind, "
                )
                .Append(
                    "__event.orientation, __event.pressure, __event.size, __event.radiusMajor, __event.radiusMinor)))"
                );
            return;
        }
        ...
        {
            builder.Append(
                "global::Doroti.Framework.Rendering.RenderViewportBase<global::Doroti.Framework.Rendering.SliverPhysicalContainerParentData>.showInViewport"
            );
            return;
        }
        ...
        {
            builder.Append(
                "(value => global::Doroti.Framework.Foundation.DebugLibrary.debugFormatDouble(value))"
            );
            return;
        }
        ...
        {
            builder
                .Append("global::Doroti.Framework.Semantics.SemanticsBinding.")
        ...
        if (
            name is "isEmpty" or "isNotEmpty"
            && prefixNode?.StaticType?.TrimEnd('?') == "CharacterRange"
        )
        {
            builder.Append('(');
            LowerExpression(
        ...
        if (
            name == "values"
            && FindGlobalDeclaration(prefix)?.Ast.Kind == CoreNodeKind.EnumDeclaration
        )
        {
        ...
        if (
            ...
            && prefixedTypeDeclaration.Ast.Kind
                is CoreNodeKind.ClassDeclaration
                    or CoreNodeKind.MixinDeclaration
                    or CoreNodeKind.EnumDeclaration
                    or CoreNodeKind.ExtensionTypeDeclaration
        )
        {
            builder
                .Append(MapStaticOwnerType(prefix, declaration))
                .Append('.')
                .Append(SafeIdentifier(name));
            return;
        }
        ...
        if (
            StripLibraryPrefix(prefix) == "_CachedLayoutCalculation"
        ...
        if (!string.IsNullOrEmpty(node.ElementId))
        {
            ...
            )
            {
                ...
                {
                    builder
                        .Append(MapStaticOwnerType(ownerDeclaration.Name, declaration))
                        .Append('.')
                        .Append(SafeIdentifier(name));
                    return;
                }
            ...
            )
            {
                ...
                if (
                    referencedDeclaration?.Ast.Kind
                    is CoreNodeKind.ClassDeclaration
                        or CoreNodeKind.MixinDeclaration
                        or CoreNodeKind.EnumDeclaration
                        or CoreNodeKind.ExtensionTypeDeclaration
        ...
        if (
            prefixNode is not null
            && name == "_depth"
            && node.ElementId?.Contains(
                "#ViewportNotificationMixin._depth",
        ...
        if (
            ...
            && (
                ...
                || IsDartEnumerableType(MapType(prefixType))
                || node.ElementId is "dart:core#Iterable.isEmpty" or "dart:core#Iterable.isNotEmpty"
            )
        )
        {
            var iterableNullAware =
                node.Text(CoreProperty.@operator) == "?."
            ...
            if (iterableNullAware)
            {
                ...
                builder
                    .Append("System.Linq.Enumerable.Any(")
                    .Append(promoted)
                    .Append(") : (bool?)null)");
                return;
            }
            builder.Append(
                name == "isEmpty" ? "!System.Linq.Enumerable.Any(" : "System.Linq.Enumerable.Any("
            );
            LowerExpression(
                builder,
                prefixNode,
        ...
        if (
            ...
            && name == "flattened"
            && node.ElementId?.Contains(
                "#IterableIterableExtension.flattened",
                StringComparison.Ordinal
            ) == true
        )
        {
        ...
        if (
            prefixNode is not null
            && TryResolvePromotedMemberOwner(node, prefixType, name, out var promotedOwner)
        )
        {
            builder.Append("((").Append(promotedOwner).Append(")");
            if (IsCurrentTypeParameter(prefixType))
            {
        ...
        if (
            prefixNode is not null
            && prefixType is "Vector4" or "global::System.Numerics.Vector4"
            && name == "storage"
        )
        {
        ...
        if (
            prefixNode is not null
            && prefixType is "PointerEvent" or "PointerPanZoomUpdateEvent"
        ...
        if (
            name is "isOdd" or "isEven"
            && prefixNode is not null
            && prefixType is "int" or "double" or "num"
        )
        {
            builder.Append("((checked((long)(");
            LowerExpression(
        ...
        var isTypeParameter =
            declaration.Element.TypeParameters?.Any(item =>
                string.Equals(item.Name, prefixType, StringComparison.Ordinal)
            ) == true;
        ...
        if (
            ...
            && (
                isTypeParameter
                || prefixType is "dynamic" or "object" or "Object" or ""
                || IsEnumType(prefixType)
                || prefixDeclaration?.Ast.Kind == CoreNodeKind.EnumDeclaration
            )
        )
        {
        ...
        if (
            name == "length"
            && (prefixType is "String" or "string" || node.ElementId == "dart:core#String.length")
        )
        {
            if (prefixNode is not null)
            {
        ...
        if (
            name == "first"
            && prefixNode is not null
            && prefixType.StartsWith("Queue<", StringComparison.Ordinal)
        )
        {
            LowerExpression(
                builder,
        ...
        if (
            name is "stackTrace" or "stacktrace"
            && prefixNode is not null
            && (
        ...
        if (
            prefixNode is not null
            && prefixType == "Stopwatch"
            && name is "elapsedMilliseconds" or "elapsedMicroseconds" or "elapsedTicks"
        )
        {
            LowerExpression(
                builder,
            ...
            builder.Append(
                name switch
                {
                    ...
                    "elapsedMicroseconds" =>
                        ".ElapsedTicks / (TimeSpan.TicksPerMillisecond / 1000)",
                    _ => ".ElapsedTicks",
                }
            );
            return;
        ...
        if (
            mapped.StartsWith("Count", StringComparison.Ordinal)
            && (
        ...
        )
        {
            ...
            if (wrapComparison && nullableCount)
            {
                ...
                builder
                    .Append("?.Count)) is { } ")
                    .Append(promotedCount)
                    .Append(" ? ")
                    .Append(promotedCount)
                    .Append(mapped["Count".Length..])
                    .Append(" : (bool?)null)");
                return;
            }
            if (wrapComparison)
            ...
            if (wrapComparison)
            {
                builder.Append(mapped["Count".Length..]).Append(')');
            }
        ...
        {
            builder
                .Append("FoundationRuntimePorts.Length(")
                .Append(SafeIdentifier(prefix))
                .Append(')');
            return;
        }
        var dynamicPrefixDispatch =
            prefixNode is not null
            && RequiresDynamicPropertyDispatch(prefixNode.StaticType, node.ElementId, name);
        var dynamicPrefixResultType =
            dynamicPrefixDispatch
            && !_session.EmittingAssignmentLeft
            && node.StaticType is { } prefixStaticType
            && prefixStaticType.TrimEnd('?') is not ("void" or "dynamic" or "Object" or "object")
                ? MapType(prefixStaticType)
                : null;
        if (dynamicPrefixResultType is not null)
        {
            builder.Append("((").Append(dynamicPrefixResultType).Append(')');
        }
    ...
    )
    {
        ...
        var name = node.Text(CoreProperty.name) ?? "missing";
        var targetType = (
            target is null ? string.Empty : ResolvedExpressionValueType(target) ?? string.Empty
        ).TrimEnd('?');
        var resolvedPropertyElementId =
            node.ElementId
        ...
        if (
            ...
            && (_session.ActiveDonorDeclaration ?? declaration).Name
        ...
        if (
            name == "name"
            && target is not null
            && targetType.StartsWith("_", StringComparison.Ordinal)
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append(".ToString()");
        ...
        if (
            (target?.StaticType == "Type" || target?.Kind == CoreNodeKind.TypeLiteral)
            && target.Kind is CoreNodeKind.SimpleIdentifier or CoreNodeKind.TypeLiteral
            && (
                target.Text(CoreProperty.name)
                ?? DescendantsAndSelf(target)
                    .FirstOrDefault(item => item.Kind == CoreNodeKind.NamedType)
                    ?.Text(CoreProperty.name)
            )
                is { } staticPropertyOwner
        )
        {
            builder
                .Append(MapType(staticPropertyOwner).TrimEnd('?'))
                .Append('.')
                .Append(SafeIdentifier(name));
            return;
        }
        if (
            target is not null
            && targetType.EndsWith("Constraints", StringComparison.Ordinal)
        ...
        if (
            target is not null
            && name == "_depth"
            && resolvedPropertyElementId?.Contains(
                "#ViewportNotificationMixin._depth",
                StringComparison.Ordinal
            ) == true
        )
        ...
        if (
            ...
            && (
                ...
                || IsDartEnumerableType(MapType(targetType))
                || resolvedPropertyElementId
                    is "dart:core#Iterable.isEmpty"
                        or "dart:core#Iterable.isNotEmpty"
            )
        )
        {
            var nullAware =
        ...
        if (
            target?.Text(CoreProperty.name) == "double"
            && name is "infinity" or "negativeInfinity" or "nan" or "maxFinite" or "minPositive"
        )
        {
            builder.Append(MapDoubleConstant(name));
        ...
        )
        {
            builder.Append("DartRuntimePrimitives.StackTraceFrom(");
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            if (MapType(targetType).Contains("PlatformException", StringComparison.Ordinal))
            {
        ...
        if (
            target is not null
            && TryResolvePromotedMemberOwner(node, targetType, name, out var promotedOwner)
        )
        {
            builder.Append("((").Append(promotedOwner).Append(")");
            if (IsCurrentTypeParameter(targetType))
            {
                builder.Append("(object)");
            }
        ...
        if (
            target is not null
            && targetType is "PointerEvent" or "PointerPanZoomUpdateEvent"
            && name is "pan" or "localPan" or "panDelta" or "localPanDelta"
        )
        {
            builder.Append("(((PointerPanZoomUpdateEvent?)(object?)");
        ...
        if (
            name is "isOdd" or "isEven"
            && target is not null
            && targetType is "int" or "double" or "num"
        )
        {
            builder.Append("((checked((long)(");
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
        ...
        if (
            ...
            && DescendantsAndSelf(target)
                .Any(item =>
                    item.Kind == CoreNodeKind.SimpleIdentifier
                    && item.Text(CoreProperty.name) == "details"
                )
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
        ...
        if (name == "values" && target is not null && target.StaticType == "Type")
        {
            ...
            builder
                .Append("System.Enum.GetValues<")
                .Append(MapType(enumName))
                .Append(">().ToList()");
            return;
        }
        if (
            name is "characters" or "runes"
            && target is not null
            && targetType is "String" or "string"
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append('.').Append(name).Append("()");
        ...
        if (
            name == "length"
            && target is not null
            && targetType is "dynamic" or "object" or "Object"
        )
        {
        ...
        if (
            ...
            && (
                ...
                || FindGlobalDeclaration(
                    StripLibraryPrefix((target.StaticType ?? string.Empty).TrimEnd('?'))
                )?.Ast.Kind == CoreNodeKind.EnumDeclaration
            )
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
        ...
        if (name == "index" && target is not null)
        {
            ...
            var targetDeclaration =
                FindGlobalDeclaration(targetType)
                ?? FindGlobalDeclaration(
                    StripLibraryPrefix((target.StaticType ?? string.Empty).TrimEnd('?'))
                );
            if (
                ...
                || IsEnumType(target.StaticType ?? string.Empty)
        ...
        if (
            name == "sign"
            && target is not null
            && target.StaticType?.TrimEnd('?') is "int" or "double" or "num"
        )
        {
            builder.Append("Math.Sign(");
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
        ...
        if (name == "length" && target is not null && target.StaticType?.TrimEnd('?') == "String")
        {
            ...
            builder.Append(
                node.Text(CoreProperty.@operator) == "?."
                || target.StaticType.EndsWith("?", StringComparison.Ordinal)
                    ? "?.Length"
                    : ".Length"
            );
            return;
        ...
        if (
            name == "name"
            && target is not null
            && target.StaticType is { Length: > 0 } targetNameType
            && char.IsUpper(targetNameType.TrimEnd('?')[0])
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append(".ToString()");
        ...
        if (
            name == "first"
            && target is not null
            && targetType.StartsWith("Queue<", StringComparison.Ordinal)
        )
        {
            LowerExpression(builder, target, declaration, package, library, inputPath, diagnostics);
            builder.Append(".Peek()");
        ...
        if (
            ...
            && target is not null
            && (
                targetType.TrimEnd('?') == "DateTime"
                || MapType(target.StaticType ?? string.Empty).TrimEnd('?') == "DateTime"
            )
        )
        {
        ...
        var dynamicPropertyDispatch =
            target is not null
            && RequiresDynamicPropertyDispatch(target.StaticType, resolvedPropertyElementId, name);
        var dynamicPropertyResultType =
            dynamicPropertyDispatch
            && !_session.EmittingAssignmentLeft
            && node.StaticType is { } propertyStaticType
            && propertyStaticType.TrimEnd('?') is not ("void" or "dynamic" or "Object" or "object")
                ? MapType(propertyStaticType)
                : null;
        if (dynamicPropertyResultType is not null)
        {
            builder.Append("((").Append(dynamicPropertyResultType).Append(')');
        }

        var nullableBooleanExpression =
            mapped.Contains(' ', StringComparison.Ordinal)
            && target?.StaticType?.EndsWith("?", StringComparison.Ordinal) == true;
        if (nullableBooleanExpression)
        {
            builder.Append("((bool?)(");
        }
        ...
        if (
            node.ElementId is { } propertyElementId
            && _semanticIndex.IsEnumGetter(propertyElementId)
        )
        {
            builder.Append("()");
        }
    ...
    )
    {
        ...
        if (expressions.Length < 2)
        {
            AddUnsupportedDiagnostic(
                ...
                "Provide the typed target and index operands."
            );
        ...
        if (
            !_session.EmittingAssignmentLeft
            && targetType.TrimEnd('?') is "dynamic" or "object" or "Object"
        )
        {
        ...
        if (
            isMapGet
            && mappedResultType.EndsWith("?", StringComparison.Ordinal)
            && IsValueType(mappedResultType.TrimEnd('?'))
        )
        {
            builder
                .Append("DartCollectionRuntime.NullableMapValue<")
                .Append(mappedResultType.TrimEnd('?'))
                .Append(">(");
            LowerExpression(
                builder,
                expressions[0],
        ...
        var isSequenceIndex =
            ...
            || targetType.StartsWith("string", StringComparison.Ordinal)
            || targetType == "String"
        ...
        var nullableIndex =
            expressions[1].StaticType?.EndsWith("?", StringComparison.Ordinal) == true
            || (
                ...
                && DescendantsAndSelf(declaration.Ast)
                    ...
                    .Any(list =>
                        list.Children.Any(variable =>
                            variable.Kind == CoreNodeKind.VariableDeclaration
                            && variable.Text(CoreProperty.name)
                                == expressions[1].Text(CoreProperty.name)
                        )
                        && list.Children.FirstOrDefault(item => item.Category == "type")
                            is { } localType
    ...
    private bool IsTypeParameter(string? type, CoreResolvedDeclaration declaration)
    {
        ...
        return declaration.Element.TypeParameters?.Any(parameter => parameter.Name == name)
            ?? false;
    }
    ...
    )
    {
        ...
        {
            var argumentValue =
                values[index].Kind == CoreNodeKind.NamedExpression
                    ? values[index].Children.FirstOrDefault(item => item.Category == "expression")
                        ?? values[index]
                    : values[index];
            var namedArgument =
                values[index].Kind == CoreNodeKind.NamedExpression
                    ? values[index].Text(CoreProperty.name)
                        ?? DescendantsAndSelf(values[index])
                            ...
                            ?.Children.FirstOrDefault(item =>
                                item.Kind == CoreNodeKind.SimpleIdentifier
                            )
                            ?.Text(CoreProperty.name)
                    : null;
            var expectedParameter = namedArgument is not null
                ? expectedParameters?.FirstOrDefault(parameter =>
            ...
            if (
                nullAsGenericDefault
                && (
                    argumentValue.Kind == CoreNodeKind.NullLiteral
            ...
            if (
                ...
                && (
                    _session.ActiveDonorDeclaration ?? declaration
                ).Element.TypeParameters?.FirstOrDefault()
                    is { } nullTypeParameter
            )
            {
                builder
                    .Append("default(")
                    .Append(SafeIdentifier(nullTypeParameter.Name))
                    .Append(')');
                continue;
            }
            ...
            {
                if (
                    decoderType.Contains("bool", StringComparison.Ordinal)
                    && decoderType.Contains("int?", StringComparison.Ordinal)
                )
                {
            ...
            var argumentName = argumentValue.Kind
                is CoreNodeKind.SimpleIdentifier
                    or CoreNodeKind.PropertyAccess
                    or CoreNodeKind.PrefixedIdentifier
                ? argumentValue.Text(CoreProperty.name)
                : null;
            if (
                argumentName == "showOnScreen"
                && argumentValue.StaticType?.Contains(" Function", StringComparison.Ordinal) == true
            )
            {
                builder.Append("() => ");
                EmitFunctionTearOffReceiver(
                    builder,
                    argumentValue,
                    declaration,
                    package,
            ...
            var expectedArgumentType =
                namedArgument is not null && expectedParameter is not null
                    ? MapType(expectedParameter.Type)
                    : expectedArgumentTypes?.ElementAtOrDefault(index)
                        ?? (
                            expectedParameter is not null
                                ? MapType(expectedParameter.Type)
                                : string.Empty
                        );
            if (
            ...
            )
            {
                ...
                builder.Append(
                    ContainsUnboundTypeParameter(expectedArgumentType) ? "default" : "null"
                );
                continue;
            }
            if (
                invocationName is "maybePop" or "pop" or "restorablePushNamed" or "pushNamed"
            ...
            var resolvedArgumentStorageType =
                argumentValue.StaticType?.Contains(" Function", StringComparison.Ordinal) == true
                    ? null
                    : ResolvedExpressionValueType(argumentValue);
            var actualArgumentType = MapType(
                resolvedArgumentStorageType ?? argumentValue.StaticType ?? string.Empty
            );
            var expectedDelegateType = expectedArgumentType.Replace(
                "global::System.",
                string.Empty,
                StringComparison.Ordinal
            );
            var actualDelegateType = actualArgumentType.Replace(
                "global::System.",
                string.Empty,
                StringComparison.Ordinal
            );
            if (
                IsValueType(expectedArgumentType)
                && !expectedArgumentType.EndsWith("?", StringComparison.Ordinal)
            ...
            if (
                expectedArgumentType
                    is "Widget"
                        or "Widget?"
                        or "global::Doroti.Framework.Widgets.Widget"
                        or "global::Doroti.Framework.Widgets.Widget?"
                && actualArgumentType
                    is not (
                        "Widget"
                ...
                && actualArgumentType.TrimEnd('?')
                    is "PreferredSizeWidget"
                        or "ObstructingPreferredSizeWidget"
                        or "global::Doroti.Framework.Widgets.PreferredSizeWidget"
                        or "global::Doroti.Framework.Cupertino.ObstructingPreferredSizeWidget"
            )
            {
                builder.Append(
            ...
            if (
                invocationName == "setMessageHandler"
                && argumentValue.Kind == CoreNodeKind.FunctionExpression
            ...
            )
            {
                ...
                _session.ActiveFunctionReturnType =
                    TryGetGenericTypeArguments(
                        expectedDelegateType.TrimEnd('?'),
                        out var messageHandlerArguments
                    )
            ...
            {
                builder.Append(
                    "DartRuntimePrimitives.ConvertValue<ParentDataWidget<global::Doroti.Framework.Rendering.ParentData>>("
                );
                LowerExpression(
                    builder,
                    argumentValue,
            ...
            if (
                actualDelegateType.Contains("TickerFuture", StringComparison.Ordinal)
                && actualDelegateType.StartsWith("Func<", StringComparison.Ordinal)
            ...
            )
            {
                ...
                if (
                    actualDelegateType.TrimEnd('?') == "Action"
                ...
                )
                {
                    ...
                    actualDelegateType.StartsWith("Func<", StringComparison.Ordinal)
                    && TryGetGenericTypeArguments(
                        actualDelegateType.TrimEnd('?'),
                        out var actualCallbackArguments
                    )
                )
                {
                    expectedClrDelegateType =
                        actualCallbackArguments.Length <= 1
                            ? "global::System.Action"
                            : $"global::System.Action<{string.Join(", ", actualCallbackArguments[..^1])}>";
                }
                else
                {
                    expectedClrDelegateType = expectedDelegateType.StartsWith(
                        "Action",
                        StringComparison.Ordinal
                    )
            ...
            {
                var callbackBlock = argumentValue
                    .Children.FirstOrDefault(item => item.Kind == CoreNodeKind.BlockFunctionBody)
                    ?.Child(CoreChildRole.blockOffset);
                if (
                    invocationName is "firstWhere" or "putIfAbsent"
                ...
                var previousContextualReturn = _session.ContextualLambdaReturnType;
                _session.ContextualLambdaReturnType =
                    invocationName == "whenComplete" ? "void" : "object";
                try
                {
                    LowerExpression(
                        builder,
            ...
            if (
                invocationName == "registerNumericServiceExtension"
            ...
            if (
                invocationName == "registerBoolServiceExtension"
                && namedArgument is "getter" or "setter"
                && argumentValue.Kind == CoreNodeKind.FunctionExpression
            )
            {
                var callbackType =
                    namedArgument == "getter" ? "Func<Future<bool>>" : "Func<bool, Future>";
                builder.Append("((").Append(callbackType).Append(')');
                var previousFunctionReturnType = _session.ActiveFunctionReturnType;
                _session.ActiveFunctionReturnType =
                    namedArgument == "getter" ? "Future<bool>" : "Future";
                try
                {
                    LowerExpression(
                        builder,
            ...
            if (
                argumentName == "decodeResize"
                && argumentValue.StaticType?.Contains("bool", StringComparison.Ordinal) == true
            )
            {
                ...
                builder.Append(
                    "((Func<ImmutableBuffer, bool, long?, long?, Future<Codec>>)((__buffer, __allowUpscaling, __cacheWidth, __cacheHeight) => decodeResize(__buffer, __cacheWidth, __cacheHeight, __allowUpscaling)))"
                );
                continue;
            }
            if (
            ...
            )
            {
                var callbackArguments =
                    ...
                    : TryGetGenericTypeArguments(expectedDelegateType, out var callbackTypes)
                        ? Enumerable
                            .Range(0, callbackTypes.Length)
                            .Select(argumentIndex => $"__arg{argumentIndex}")
                            .ToArray()
                    : [];
                builder
                    .Append('(')
                    .Append(string.Join(", ", callbackArguments))
                    .Append(") => { _ = ");
                LowerExpression(
                    builder,
                    argumentValue,
            ...
            {
                var lambdaNames = Enumerable
                    .Range(0, actionArguments.Length)
                    .Select(argumentIndex => $"__arg{argumentIndex}")
                    .ToArray();
                builder
                    .Append('(')
                    .Append(string.Join(", ", lambdaNames))
                    .Append(") => { _ = ((")
                    .Append(actualArgumentType)
                    .Append(")");
                LowerExpression(
                    builder,
                    argumentValue,
            ...
            {
                var lambdaNames = Enumerable
                    .Range(0, expectedActionTypes.Length)
                    .Select(argumentIndex => $"__arg{argumentIndex}")
                    .ToArray();
                builder
                    .Append('(')
                ...
                for (var argumentIndex = 0; argumentIndex < lambdaNames.Length; argumentIndex++)
                {
                    ...
                    if (
                        string.Equals(
                            expectedActionTypes[argumentIndex],
                            actualActionTypes[argumentIndex],
                            StringComparison.Ordinal
                        )
                    )
                    {
                    ...
                    else
                    {
                        builder
                            ...
                            .Append(actualActionTypes[argumentIndex].TrimEnd('?'))
                            .Append(">(")
                            .Append(lambdaNames[argumentIndex])
                            .Append(')');
                    }
                }
                builder.Append(')');
            ...
            if (
                ...
                && TryGetGenericTypeArguments(
                    actualDelegateType.TrimEnd('?'),
                    out var optionalActionArguments
                )
            )
            {
                ...
                builder
                    .Append('(')
                    .Append(string.Join(", ", optionalActionArguments.Select(_ => "default")))
                    .Append(')');
                continue;
            }
            if (
            ...
            {
                var lambdaNames = Enumerable
                    .Range(0, actualActionArguments.Length)
                    .Select(argumentIndex => $"__arg{argumentIndex}")
                    .ToArray();
                builder
                    .Append('(')
            ...
            if (
                expectedParameters is not null
                && expectedDelegateType.StartsWith("Func<", StringComparison.Ordinal)
                && actualDelegateType.StartsWith("Func<", StringComparison.Ordinal)
                ...
                && string.Equals(
                    expectedFuncArguments[^1],
                    actualFuncArguments[^1],
                    StringComparison.Ordinal
                )
                && !string.Equals(
                    expectedArgumentType,
            ...
            {
                var lambdaNames = Enumerable
                    .Range(0, expectedFuncArguments.Length - 1)
                    .Select(argumentIndex => $"__arg{argumentIndex}")
                    .ToArray();
                builder
                    .Append('(')
                    .Append(expectedArgumentType)
                    .Append(")((")
                    .Append(string.Join(", ", lambdaNames))
                    .Append(") => ");
                LowerExpression(
                ...
                for (var argumentIndex = 0; argumentIndex < lambdaNames.Length; argumentIndex++)
                {
                    ...
                    if (
                        string.Equals(
                            expectedFuncArguments[argumentIndex],
                            actualFuncArguments[argumentIndex],
                            StringComparison.Ordinal
                        )
                    )
                    {
                    ...
                    else
                    {
                        builder
                            .Append("DartRuntimePrimitives.ConvertValue<")
                            .Append(actualFuncArguments[argumentIndex].TrimEnd('?'))
                            .Append(">(")
                            .Append(lambdaNames[argumentIndex])
                            .Append(')');
                    }
            ...
            if (
                argumentName == "_computeDryLayout"
                && argumentValue.StaticType?.Contains("BoxConstraints", StringComparison.Ordinal)
                    == true
            )
            {
            ...
            if (
                expectedArgumentType.Contains("AnimationStatusListener", StringComparison.Ordinal)
                && actualDelegateType.StartsWith("Func<", StringComparison.Ordinal)
            )
            {
                builder.Append(
                    "(global::Doroti.Framework.Animation.AnimationStatus __status) => { _ = "
                );
                LowerExpression(
                    builder,
                    argumentValue,
            ...
            if (
                ...
                && TryGetGenericTypeArguments(
                    expectedDelegateType.TrimEnd('?'),
                    out var contextualFuncArguments
                )
            ...
            var restoresNonConstValueDefault =
                argumentName is not null
                && declaration
                    .Members.Where(member => member.Kind == "constructor")
                    .SelectMany(member => member.Element.Parameters ?? [])
                    .Any(parameter =>
                        parameter.Name == argumentName && NeedsNonConstValueDefault(parameter)
                    );
            if (
                argumentName?.StartsWith("lockMode", StringComparison.Ordinal) == true
                || argumentName == "previousState"
            ...
            else if (expectedArgumentType == "long" && actualArgumentType == "ulong")
            {
                ...
                invocationName == "dispatchLocalesChanged"
                && DescendantsAndSelf(argumentValue)
                    .Any(item => item.Text(CoreProperty.name) == "locales")
            )
            {
                ...
                invocationName == "_updateResolvedLocale"
                && DescendantsAndSelf(argumentValue)
                    .Any(item => item.Text(CoreProperty.name) == "locales")
            )
            {
            ...
            else if (invocationName == "registerForRestoration" && index == 0)
            {
                ...
                invocationName is "addRenderView" or "removeRenderView"
                && DescendantsAndSelf(argumentValue)
                    .Any(item => item.Text(CoreProperty.name) == "renderObject")
            )
            {
                // RenderObjectElement.renderObject is typed at its base contract,
                // but _RawViewElement's Dart invariant guarantees RenderView.
                builder.Append(
                    "DartRuntimePrimitives.ConvertValue<global::Doroti.Framework.Rendering.RenderView>("
                );
                LowerExpression(
                    builder,
                    argumentValue,
            ...
            )
            {
                ...
                TryGetGenericTypeArguments(
                    expectedArgumentType.TrimEnd('?'),
                    out var expectedCollectionArguments
                )
            ...
            )
            {
                ...
                builder
                    .Append(".Cast<")
                    .Append(expectedCollectionArguments[0])
                    .Append(">().ToList()");
            }
            else if (
                expectedArgumentType.TrimEnd('?').StartsWith("List<", StringComparison.Ordinal)
                ...
                && TryGetGenericTypeArguments(
                    expectedArgumentType.TrimEnd('?'),
                    out var expectedListArguments
                )
                && TryGetGenericTypeArguments(
                    actualArgumentType.TrimEnd('?'),
                    out var actualReadOnlyArguments
                )
                && expectedListArguments.Length == 1
                && actualReadOnlyArguments.Length == 1
            )
            {
                var wrapAwaitedCollection = argumentValue.Kind == CoreNodeKind.AwaitExpression;
                if (wrapAwaitedCollection)
                ...
                if (
                    !string.Equals(
                        expectedListArguments[0],
                        actualReadOnlyArguments[0],
                        StringComparison.Ordinal
                    )
                )
                {
                ...
                && TryGetGenericTypeArguments(
                    expectedArgumentType.TrimEnd('?'),
                    out var expectedEnumerableListArguments
                )
                && TryGetGenericTypeArguments(
                    actualArgumentType.TrimEnd('?'),
                    out var actualEnumerableArguments
                )
                && expectedEnumerableListArguments.Length == 1
                && actualEnumerableArguments.Length == 1
            )
            {
                var wrapAwaitedCollection = argumentValue.Kind == CoreNodeKind.AwaitExpression;
                if (wrapAwaitedCollection)
                ...
                if (
                    !string.Equals(
                        expectedEnumerableListArguments[0],
                        actualEnumerableArguments[0],
                        StringComparison.Ordinal
                    )
                )
                {
                    builder
                        .Append(".Cast<")
                        .Append(expectedEnumerableListArguments[0])
                        .Append(">()");
                }
                builder.Append(".ToList()");
            }
                ...
                && TryGetGenericTypeArguments(
                    expectedArgumentType.TrimEnd('?'),
                    out var expectedEnumerableArguments
                )
            ...
            )
            {
                ...
                expectedArgumentType.TrimEnd('?').StartsWith("DartMap<", StringComparison.Ordinal)
                && actualArgumentType.TrimEnd('?').StartsWith("DartMap<", StringComparison.Ordinal)
                && expectedArgumentType.TrimEnd('?') != actualArgumentType.TrimEnd('?')
                && TryGetGenericTypeArguments(
                    expectedArgumentType.TrimEnd('?'),
                    out var expectedMapArguments
                )
            ...
            )
            {
                ...
                builder
                    .Append(".cast<")
                    .Append(DartMapTypeArguments(expectedArgumentType.TrimEnd('?')))
                    .Append(">()");
            }
            else if (
                actualArgumentType.TrimEnd('?') is "object" or "dynamic"
            ...
            )
            {
                ...
                (castDynamicArguments || invocationName == "_invokeMethod")
                && actualArgumentType == "dynamic"
            )
            {
                builder.Append("(object?)");
                LowerExpression(
            ...
            )
            {
                builder
                    .Append("DartRuntimePrimitives.ConvertValue<")
                    .Append(expectedArgumentType.TrimEnd('?'))
                    .Append(">(");
                LowerExpression(
                    builder,
                    values[index],
    ...
    private bool CallableReturnsValue(CoreAstNode argument, CoreResolvedDeclaration declaration)
    {
        ...
        var name =
            argument.Text(CoreProperty.name)
            ?? argument
                .Children.FirstOrDefault(candidate =>
                    candidate.Kind == CoreNodeKind.SimpleIdentifier
                )
                ?.Text(CoreProperty.name);
        ...
        var localFunction = DescendantsAndSelf(owner.Ast)
        ...
        var functionType =
            localFunction
                ?.Children.FirstOrDefault(candidate =>
                    candidate.Kind == CoreNodeKind.FunctionExpression
                )
                ?.StaticType
    ...
    )
    {
        ...
        if (methodName == "setMessageHandler" && target is not null)
        {
            ...
            var channelTargetType = MapType(
                    target.StaticType
                        ?? channelTargetMember?.Element.ReturnType
                        ?? channelTargetMember?.Element.Type
                        ?? string.Empty
                )
                .TrimEnd('?');
            if (
                channelTargetType.StartsWith("BasicMessageChannel<", StringComparison.Ordinal)
        ...
        var analyzerParameterTypes = Enumerable
            .Range(0, 64)
        ...
        if (target is not null)
        {
            ...
            var targetType = MapType(
                    target.StaticType
                        ?? targetMember?.Element.ReturnType
                        ?? targetMember?.Element.Type
                        ?? string.Empty
                )
                .TrimEnd('?');
            if (TryGetGenericTypeArguments(targetType, out var targetArguments))
            {
                ...
                if (
                    ...
                    && TryGetGenericTypeArguments(
                        MapType(invocation.StaticType ?? string.Empty).TrimEnd('?'),
                        out var resultArguments
                    )
                ...
                if (
                    methodName is "remove" or "containsKey"
        ...
        var callableType = invocation
            .Children.FirstOrDefault(child =>
                ...
                && child.StaticType?.Contains(" Function", StringComparison.Ordinal) == true
            )
            ?.StaticType;
        ...
        if (
            target is not null
            && FindDeclaringDeclaration(resolvedMember) is { } memberOwner
        ...
        )
        {
            ...
            if (genericStart > 0 && targetType.EndsWith('>'))
            {
                ...
                if (
                    targetDeclaration?.Element.CanonicalId == memberOwner.Element.CanonicalId
        ...
        if (resolvedMember.Element.TypeParameters is { Length: > 0 } methodTypeParameters)
        {
            ...
            if (
                ...
                && methodName == "resolveWith"
                && _session.ActiveFunctionReturnType is { } contextualReturn
                && TryGetGenericTypeArguments(
                    contextualReturn.TrimEnd('?'),
                    out var contextualArguments
                )
            ...
            if (
                ...
                && TryGetGenericTypeArguments(
                    MapType(invocation.StaticType ?? string.Empty).TrimEnd('?'),
                    out var inferredReturnArguments
                )
            ...
            for (
                var index = 0;
                index < methodTypeParameters.Length && index < methodArguments.Length;
                index++
            )
            {
    ...
    private bool ShouldCastInvocationArgument(string actualType, string expectedType)
    {
        ...
        while (pending.Count > 0)
        {
            ...
            if (string.Equals(baseName, actual, StringComparison.Ordinal))
            {
                return true;
            }
