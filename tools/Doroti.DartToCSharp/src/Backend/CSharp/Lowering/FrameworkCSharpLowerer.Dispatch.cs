...
{
    private void ValidateOverrideContract(
        CoreResolvedDeclaration declaration,
        CoreResolvedMember member,
        CoreResolvedMember? contract,
        bool isInterface,
        string package,
        string library,
        string inputPath,
    ...
    )
    {
        if (
            ...
            || !HasOverrideAnnotation(member.Ast)
            || member.Name is "toString" or "hashCode" or "=="
            || IsKnownExternalOverride(declaration, member)
            || FindDeclaringDeclaration(member)?.Element.CanonicalId
