...
{
    public static CsTypeReference Named(string name, bool isNullable = false) =>
        new(string.Empty, name, [], isNullable);
}

internal sealed record CsParameter(
...
internal sealed record CsField(
    ...
    bool IsConst = false
) : CsMember(Origin, Accessibility, Name);
...
internal sealed record CsExpressionStatement(CsOrigin Origin, CsExpression Expression)
    : CsStatement(Origin);
...
internal sealed record CsMemberAccess(CsOrigin Origin, CsExpression Target, string Name)
    : CsExpression(Origin);

internal sealed record CsInvocation(CsOrigin Origin, CsExpression Target, CsExpression[] Arguments)
    : CsExpression(Origin);
