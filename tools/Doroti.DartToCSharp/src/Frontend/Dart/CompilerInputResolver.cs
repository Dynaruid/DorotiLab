    ...
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(manifest.PackageRoot))
        {
            throw new InvalidDataException(
                $"Package URI input requires packageRoot: {logicalPath}"
            );
        }
        ...
        {
            throw new InvalidDataException(
                $"Package configuration is missing. Run pub get first: {configPath}"
            );
        }
        ...
        {
            throw new InvalidDataException(
                $"Package URI is not present in the resolved lock graph: {logicalPath}"
            );
        }
        ...
        if (!File.Exists(resolved))
        {
            throw new FileNotFoundException(
                $"Resolved package URI does not exist: {logicalPath}",
                resolved
            );
        }
