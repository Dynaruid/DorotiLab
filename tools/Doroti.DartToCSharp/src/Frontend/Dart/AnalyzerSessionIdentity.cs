...
internal sealed class AnalyzerSessionIdentity
{
    ...
    private AnalyzerSessionIdentity(
        string analyzerRoot,
        string runtimeDigest,
        string packageConfigDigest
    )
    {
    ...
    public static AnalyzerSessionIdentity Create(string analyzerRoot, string? packageConfigPath) =>
        new(
    ...
    public string HashFile(string path) =>
        _fileHashes.GetOrAdd(Path.GetFullPath(path), ArtifactFiles.Sha256);
}
