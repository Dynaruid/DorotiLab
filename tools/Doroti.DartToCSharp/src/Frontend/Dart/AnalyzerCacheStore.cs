...
internal sealed record AnalyzerCacheStatus(
    int EntryCount,
    long Bytes,
...
internal sealed class AnalyzerCacheStore(
...
)
{
    ...
    public bool TryRead(string cacheKey, out string payload)
    {
        ...
        try
        {
            ...
            if (separator <= 0)
            {
                throw new InvalidDataException("Analyzer cache header delimiter is missing.");
            }

            var header =
                JsonSerializer.Deserialize<AnalyzerCacheEntryHeader>(
                    uncompressed.AsSpan(0, separator)
                ) ?? throw new InvalidDataException("Analyzer cache header is empty.");
            if (
                header.SchemaVersion != "doroti.dart-analyzer-cache/v2"
            ...
            foreach (var dependency in header.Dependencies)
            {
                if (
                    ...
                    || !string.Equals(
                        identity.HashFile(dependency.Path),
                        dependency.Sha256,
            ...
            when (exception is InvalidDataException or IOException or JsonException)
        {
            Quarantine(path);
            payload = string.Empty;
            return false;
    ...
    public void Write(
        string cacheKey,
        string payload,
        IReadOnlyList<AnalyzerDependencyFingerprint> dependencies
    )
    {
    ...
    public AnalyzerCacheStatus Status() => ReadStatus(_root);
    ...
    public static AnalyzerCacheStatus ReadStatus(string cacheDirectory)
    {
        ...
        var files = Directory
            .GetFiles(root, "*.entry.br", SearchOption.TopDirectoryOnly)
    ...
    public static int Prune(string cacheDirectory, long maximumBytes, TimeSpan maximumAge)
    {
        ...
        {
            if (file.LastWriteTimeUtc >= cutoff && bytes <= maximumBytes)
            {
                continue;
            }
