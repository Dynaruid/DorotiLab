...
internal static class AnalyzerCacheKey
{
    ...
    public static string Create(AnalyzerSessionIdentity session, string inputPath, bool syntaxOnly)
    {
        var packageRoot = FindDartPackageRoot(inputPath);
        var packageState = new[]
        {
            "pubspec.yaml",
            "pubspec.lock",
            Path.Combine(".dart_tool", "package_config.json"),
        }
            .Select(name => Path.Combine(packageRoot, name))
            .Where(File.Exists)
            .Select(path =>
                $"{ArtifactFiles.NormalizePath(Path.GetRelativePath(packageRoot, path))}:{session.HashFile(path)}"
            );
        var identity =
            $"input:{session.HashFile(inputPath)}\n"
