...
{
    private sealed record ImportDirective(
        string Directive,
        string[] Candidates,
        string Selected,
        string? Condition
    ...
    )
    {
        var application =
            ...
            ?? throw new InvalidDataException(
                "Application graph expansion requires an application manifest."
            );
        if (
        ...
        {
            throw new InvalidDataException(
                "Application compilation requires packageRoot and entryPoint."
            );
        }
        if (manifest.Inputs.Length != 0)
        {
            throw new InvalidDataException(
                "Application compilation discovers its package closure from entryPoint; fixture-specific inputs are not allowed."
            );
        }
        ...
        {
            throw new InvalidDataException(
                $"Application package configuration is missing: {packageConfigPath}"
            );
        }
        ...
        foreach (var pair in application.Conditions ?? [])
        {
        ...
        {
            if (physicalByLibrary.ContainsKey(library))
            {
                continue;
            }
            ...
            {
                var selectedLibrary = CanonicalizeImport(
                    packageConfigPath,
                    library,
                    physical,
                    import.Selected
                ...
                var candidates = import
                    .Candidates.Select(candidate =>
                        CanonicalizeImport(packageConfigPath, library, physical, candidate)
                    )
                    ...
                    .ToArray();
                edges.Add(
                    new(library, selectedLibrary, import.Directive, candidates, import.Condition)
                );
                foreach (
                    var candidate in candidates.Where(candidate =>
                        !candidate.StartsWith("package:flutter/", StringComparison.Ordinal)
                    )
                )
                {
        ...
        var libraries = physicalByLibrary
            .OrderBy(item => item.Key, StringComparer.Ordinal)
            .Select(item => new ApplicationLibraryNode(
                item.Key,
                item.Value,
                ArtifactFiles.Sha256(item.Value)
            ))
            .ToArray();
        var localEdges = edges
            .SelectMany(edge =>
                edge.Candidates.Where(physicalByLibrary.ContainsKey)
                    .Select(candidate => new ApplicationImportEdge(
                        edge.From,
                        candidate,
                        edge.Directive,
                        edge.Candidates,
        ...
        var inputs = libraries
            .Select(item => new SelectionInput(item.Library, ["*"], item.Library, "generate"))
            .ToArray();
        return (
            manifest with
            {
    ...
    private static string ResolveWithinPackage(string packageRoot, string relative)
    {
        ...
        {
            throw new InvalidDataException("Application manifest paths must be package-relative.");
        }
    ...
    private static string ResolveLibrary(string packageConfigPath, string library)
    {
        ...
        {
            throw new InvalidDataException(
                $"Application libraries must use package URIs: {library}"
            );
        }
        ...
        if (slash < 0)
        {
            throw new InvalidDataException($"Invalid package URI: {library}");
        }
        ...
        {
            throw new InvalidDataException(
                $"Package is absent from package_config.json: {packageName}"
            );
        }
        ...
        var packageUri = package.TryGetProperty("packageUri", out var value)
            ? value.GetString() ?? "lib/"
            : "lib/";
        ...
        return File.Exists(path)
            ...
            : throw new FileNotFoundException(
                $"Application library does not exist: {library}",
                path
            );
    }

    private static string? CanonicalizeImport(
        string packageConfigPath,
        string ownerLibrary,
        string ownerPath,
        string import
    )
    {
        ...
        if (import.StartsWith("package:", StringComparison.Ordinal))
        {
            return import;
        }
        ...
        foreach (var package in config.RootElement.GetProperty("packages").EnumerateArray())
        {
            ...
            var packageUri = package.TryGetProperty("packageUri", out var value)
                ...
                : "lib/";
            var libRoot =
                new Uri(root, EnsureSlash(packageUri)).LocalPath.TrimEnd(
                    Path.DirectorySeparatorChar
                ) + Path.DirectorySeparatorChar;
            if (!path.StartsWith(libRoot, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
        ...
        throw new InvalidDataException(
            $"Relative import from {ownerLibrary} resolves outside the package graph: {import}"
        );
    }

    private static ImportDirective[] ParseImports(
        string source,
    ...
    )
    {
        ...
        foreach (Match match in ImportDirectiveRegex().Matches(source))
        {
            ...
            var uris = QuotedUriRegex()
                .Matches(body)
                .Select(item => item.Groups["uri"].Value)
                .ToArray();
            if (uris.Length == 0)
            {
                continue;
    ...
    private static ApplicationSccNode[] ComputeSccs(
        string[] libraries,
        ApplicationImportEdge[] edges
    )
    {
        ...
        void Visit(string library)
        {
            ...
            foreach (var dependency in adjacency[library])
            {
                ...
                {
                    Visit(dependency);
                    low[library] = Math.Min(low[library], low[dependency]);
                }
                else if (onStack.Contains(dependency))
                {
                    low[library] = Math.Min(low[library], indices[dependency]);
                }
            }
            if (low[library] != indices[library])
            {
                return;
            }
        ...
        return components
            ...
            .Select(
                (item, position) =>
                    new ApplicationSccNode(
                        $"scc-{position:D3}",
                        item,
                        item.Length > 1
                            || edges.Any(edge => edge.From == item[0] && edge.To == item[0])
                    )
            )
            .ToArray();
    }
    ...
    )
    {
        ...
        {
            if (!hashes.TryGetValue(library.Library, out var hash) || hash != library.Sha256)
            {
                changed.Add(library.Library);
            }
        }
        foreach (
            var removed in hashes.Keys.Except(
        ...
        var affected = new HashSet<string>(
            changed.Where(item => libraries.Any(library => library.Library == item)),
            StringComparer.Ordinal
        );
        ...
        {
            foreach (
                var importer in edges.Where(edge => edge.To == dependency).Select(edge => edge.From)
            )
            {
                if (affected.Add(importer))
                {
                    queue.Enqueue(importer);
                }
            }
            var component = sccs.SingleOrDefault(item =>
                item.Libraries.Contains(dependency, StringComparer.Ordinal)
            );
            ...
            foreach (var member in component.Libraries)
            {
                if (affected.Add(member))
                {
    ...
    [GeneratedRegex(
        "if\\s*\\(\\s*(?<condition>[A-Za-z0-9_.]+)\\s*\\)\\s*['\"](?<uri>[^'\"]+)['\"]",
