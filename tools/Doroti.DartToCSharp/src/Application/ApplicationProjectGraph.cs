...
{
    private sealed record ResourceSourceDocument(
        string SchemaVersion,
        ResourceSourceEntry[] Resources
    );

    private sealed record ResourceSourceEntry(
        string Key,
        string Path,
        string Kind,
    ...
    private sealed record PluginSourceEntry(
        string Id,
        string Channel,
        string Codec,
        string CapabilityId,
        NativePackageSource[] NativePackages
    );

    private sealed record NativePackageSource(
        string Rid,
        string PackageId,
        string Version,
        string AbiVersion,
    ...
    )
    {
        ...
        if (resources.SchemaVersion != "doroti.application-resources/v1")
        {
            throw new InvalidDataException(
                $"Unsupported application resource schema: {resources.SchemaVersion}"
            );
        }
        ...
        {
            throw new InvalidDataException(
                $"Unsupported application plugin schema: {plugins.SchemaVersion}"
            );
        }

        if (
            resources.Resources.Select(item => item.Key).Distinct(StringComparer.Ordinal).Count()
        ...
        if (
            plugins.Plugins.Select(item => item.Channel).Distinct(StringComparer.Ordinal).Count()
        ...
        for (var index = 0; index < resources.Resources.Length; index++)
        {
            ...
            embeddedItems.Add(
                $"    <EmbeddedResource Include=\"{relative}\" LogicalName=\"{embeddedName}\" />"
            );
        ...
        foreach (
            var plugin in plugins.Plugins.OrderBy(item => item.Channel, StringComparer.Ordinal)
        )
        {
            var native = plugin.NativePackages.SingleOrDefault(item =>
                item.Rid == application.TargetRid
            );
            if (native is null)
            {
                diagnostics.Add(
        ...
        var runtimeManifestPath = Path.Combine(
            projectDirectory,
            "doroti.application-capabilities.json"
        );
        ArtifactFiles.WriteJson(runtimeManifestPath, runtimeManifest);
        embeddedItems.Add(
            "    <EmbeddedResource Include=\"doroti.application-capabilities.json\" LogicalName=\"Doroti.Application.Manifest\" />"
        );

        var frameworkReferences = BuildApplicationFrameworkReferences(
            application.FrameworkPackages,
            identity.RuntimeBindingVersion
        );
        var hostReference = BuildApplicationPackageReference(
            application.HostBootstrapPackage,
            identity.RuntimeBindingVersion
        );
        ...
        ArtifactFiles.WriteUtf8(
            ...
            string.Join(
                ...
                new[]
                {
                    ...
                    .Concat(application.FrameworkPackages)
                    .Distinct(StringComparer.Ordinal)
                    .Select(package => $"global using Doroti.Framework.{package};")
            )
                + "\n"
        ...
        ArtifactFiles.WriteJson(
            Path.Combine(outputDirectory, "application-graph.json"),
            new
            {
                schemaVersion = "doroti.application-graph/v1",
                entryPoint = plan.EntryPoint,
                targetRid = application.TargetRid,
                frameworkPackages = application
                    .FrameworkPackages.Order(StringComparer.Ordinal)
                    .ToArray(),
                hostBootstrapPackage = application.HostBootstrapPackage,
                contractSha256 = plan.ContractSha256,
                libraries = plan
                    .Libraries.Select(item => new
                    {
                        item.Library,
                        path = ArtifactFiles.NormalizePath(
                            Path.GetRelativePath(packageRoot, item.Path)
                        ),
                        item.Sha256,
                    })
                    .ToArray(),
                ...
                incremental = new
                {
                    ...
                    regeneratedOutputs = outputs
                        .Where(output =>
                            plan.AffectedLibraries.Contains(
                ...
                resources = embeddedResources,
                plugins = pluginEntries,
                directReferences = application
                    .FrameworkPackages.Select(package => $"Doroti.Framework.{package}")
                    .Append(application.HostBootstrapPackage)
                    .Order(StringComparer.Ordinal)
                    .ToArray(),
            }
        );
    ...
    private static string ResolveApplicationResource(string packageRoot, string relative)
    {
        ...
        {
            throw new InvalidDataException("Resource paths must be package-relative.");
        }
        ...
        {
            throw new InvalidDataException(
                $"Application resource is missing or escapes packageRoot: {relative}"
            );
        }
    ...
    private static string BuildApplicationFrameworkReferences(string[] packages, string version)
    {
        ...
        {
            throw new InvalidDataException(
                "Application frameworkPackages contains an unsupported or empty package selection."
            );
        }
