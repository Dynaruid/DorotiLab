...
public sealed class PortAdoption
{
    ...
    )
    {
        ...
        if (workspace.State.Mode != PortSchemas.RuntimeAdoption)
        {
            throw Error(
                "DORPORT011",
                $"Adoption requires port mode '{PortSchemas.RuntimeAdoption}'."
            );
        }
        var ir = ArtifactFiles.ReadJson<MigrationIr>(
            Path.Combine(workspace.GeneratedBasePath, "migration-ir.json")
        );
        var matches = ir
            .Inputs.Where(input => library is null || input.Library == library)
            .SelectMany(input =>
                input
                    .Declarations.Where(declaration => declaration.Name == symbol)
        ...
        if (matches.Length != 1)
        {
            throw Error(
                "DORPORT011",
                $"Adoption symbol must resolve exactly once: {symbol}; found {matches.Length}."
            );
        }
        var match = matches[0];
    ...
    )
    {
        ...
        var targets = root.DescendantNodes()
            ...
            .Where(node =>
                node.Identifier.ValueText == symbol
        ...
        if (targets.Length != 1)
        {
            throw Error(
                "DORPORT011",
                $"Generated adoption candidate must be a unique type declaration: {symbol}."
            );
        }
        var target = targets[0].WithoutLeadingTrivia().WithoutTrailingTrivia();
        MemberDeclarationSyntax member = target;
        if (
            targets[0].Ancestors().OfType<BaseNamespaceDeclarationSyntax>().FirstOrDefault() is
            { } containingNamespace
        )
        {
        ...
        var candidate = root.WithMembers(SyntaxFactory.SingletonList(member))
            .NormalizeWhitespace()
            .ToFullString();
        var header =
    ...
    private static void CopyManualSnapshot(
        string workspacePath,
        string staging,
        IEnumerable<PortManualInput> inputs
    )
    {
