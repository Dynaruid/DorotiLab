...
public sealed class PortCompiler
{
    ...
    )
    {
        ...
        if (
            !string.Equals(
                selection.CompatibilityProfile,
        ...
        if (Path.GetPathRoot(root) == root)
        {
            throw Error(
                "DORPORT003",
                "A filesystem root cannot be used as the port workspace root."
            );
        }
        ...
        try
        {
            ...
            ArtifactFiles.WriteJson(
                ...
                stateArtifacts.Provenance
            );
            ArtifactFiles.WriteJson(
                Path.Combine(staging, "source-map.json"),
                stateArtifacts.SourceMap
            );
            ownership = ownership with
            {
                ...
                WorkspaceFiles = new[] { "port-state.json", "provenance.json", "source-map.json" }
                    .Select(path => new PortGeneratedFile(
                        path,
                        ArtifactFiles.Sha256(Path.Combine(staging, path))
                    ))
                    .ToArray(),
            };
            ...
            return new(
                target,
                Path.Combine(target, "generated-base"),
                Path.Combine(target, "effective"),
                report,
                ownership,
                stateArtifacts.State
            );
        }
        finally
        {
    ...
    )
    {
        ...
        try
        {
            ownership = ArtifactFiles.ReadJson<PortWorkspaceDocument>(
                Path.Combine(target, "port-workspace.json")
            );
        }
            ...
            when (exception is IOException or System.Text.Json.JsonException)
        {
            throw Error(
                "DORPORT004",
                $"Compiler-owned workspace metadata was edited or removed: {exception.Message}"
            );
        }
        if (
        ...
        )
        {
            throw Error(
                "DORPORT004",
                "Compiler-owned workspace metadata no longer matches its content identity."
            );
        }

        var generatedBase = Path.Combine(target, "generated-base");
        var actual = EnumerateGeneratedFiles(generatedBase);
        var expected = ownership.GeneratedFiles.ToDictionary(
            item => item.Path,
            item => item.Sha256,
        ...
        {
            var changedSet = actual
                .Keys.Except(expected.Keys, StringComparer.Ordinal)
                .Concat(expected.Keys.Except(actual.Keys, StringComparer.Ordinal))
                .OrderBy(value => value, StringComparer.Ordinal);
            throw Error(
                "DORPORT004",
                $"Compiler-owned generated file set was edited: {string.Join(", ", changedSet)}"
            );
        }
        ...
        var expectedEffective = ownership.EffectiveFiles.ToDictionary(
            item => item.Path,
            item => item.Sha256,
            StringComparer.Ordinal
        );
        ...
        {
            var changedSet = actualEffective
                .Keys.Except(expectedEffective.Keys, StringComparer.Ordinal)
                .Concat(expectedEffective.Keys.Except(actualEffective.Keys, StringComparer.Ordinal))
                .OrderBy(value => value, StringComparer.Ordinal);
            throw Error(
                "DORPORT004",
                $"Compiler-owned effective file set was edited: {string.Join(", ", changedSet)}"
            );
        }
        foreach (var path in actualEffective.Keys)
        {
            if (
                !string.Equals(
                    actualEffective[path],
                    expectedEffective[path],
                    StringComparison.Ordinal
                )
            )
            {
        ...
        ValidateInventory(
            Path.Combine(target, "manual-snapshot"),
            ownership.ManualSnapshotFiles,
            "manual snapshot"
        );
        foreach (var file in ownership.WorkspaceFiles)
        {
            ...
            if (
                !File.Exists(path)
                || !string.Equals(ArtifactFiles.Sha256(path), file.Sha256, StringComparison.Ordinal)
            )
            {
                throw Error(
                    "DORPORT004",
                    $"Compiler-owned workspace artifact was edited or removed: {file.Path}"
                );
            }
        }

        var report = ArtifactFiles.ReadJson<ConverterReport>(
            Path.Combine(generatedBase, "converter-report.json")
        );
        var state = ArtifactFiles.ReadJson<PortStateDocument>(
            Path.Combine(target, "port-state.json")
        );
        if (state.SchemaVersion != PortSchemas.State || state.WorkspaceId != workspaceId)
        {
            throw Error(
                "DORPORT004",
                "Compiler-owned port state no longer matches the workspace identity."
            );
        }
    ...
    )
    {
        ...
        var replacementsBySymbol = replacements
            ...
            .ToDictionary(
                item => PortManifestLoader.TargetKey(item.Library, item.Symbol, null),
                StringComparer.Ordinal
            );
        ...
        foreach (var input in ir.Inputs.OrderBy(item => item.Library, StringComparer.Ordinal))
        {
            ...
            foreach (
                var symbol in input.SelectedSymbols.OrderBy(value => value, StringComparer.Ordinal)
            )
            {
                        ...
                        new(
                            ...
                            ArtifactFiles.NormalizePath(
                                Path.GetRelativePath(
                                    portRoot,
                                    Path.GetFullPath(replacement.Source, portRoot)
                                )
                            )
                        )
                    );
                ...
                {
                    ownership.Add(
                        new(input.Library, symbol, null, PortSchemas.Generated, output.Sha256, null)
                    );
                }
            }
        }
        ...
        )
        {
            ...
            if (!declaration.Members.Any(item => item.Name == replacement.Member))
            {
                throw Error(
                    "DORPORT007",
                    $"Replacement member does not exist: {replacement.Library}#{replacement.Symbol}.{replacement.Member}"
                );
            }
    ...
    private static void ValidateSelection(PortManifest manifest, MigrationIr ir)
    {
        ...
        )
        {
            throw Error(
                "DORPORT008",
                "Generated Migration IR profile differs from the port manifest."
            );
        }
        ...
        if (!expected.SequenceEqual(actual, StringComparer.Ordinal))
        {
            throw Error(
                "DORPORT008",
                "Port inputs do not exactly match the resolved compiler selection."
            );
        }
    }
    ...
    private static void ValidateReplacementHash(PortReplacement replacement, string actualHash)
    {
        if (
            !string.Equals(
                replacement.GeneratedBaseSha256,
                actualHash,
    ...
    private static void ValidateInventory(
        string root,
        PortGeneratedFile[] expectedFiles,
        string description
    )
    {
    ...
    )
    {
        ...
        var path = PortManifestLoader.ResolveUserPath(
            portRoot,
            relativePath,
            requireDirectory: false
        );
        inputs.Add(
            $"{kind}:{ArtifactFiles.NormalizePath(relativePath)}:{ArtifactFiles.Sha256(path)}"
        );
    }
    ...
    )
    {
        ...
        foreach (
            var path in Directory
                ...
                .OrderBy(
                    path => ArtifactFiles.NormalizePath(Path.GetRelativePath(portRoot, path)),
                    StringComparer.Ordinal
                )
        )
        {
            inputs.Add(
                $"{kind}:{ArtifactFiles.NormalizePath(Path.GetRelativePath(portRoot, path))}:{ArtifactFiles.Sha256(path)}"
            );
        }
    }
