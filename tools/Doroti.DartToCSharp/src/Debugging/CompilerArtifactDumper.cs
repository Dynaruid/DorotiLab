...
internal static class CompilerArtifactDumper
{
    ...
    )
    {
        ...
        if (options.Stages.HasFlag(CompilerDumpStage.AnalyzerProtocol))
        {
            ArtifactFiles.WriteJson(
                Path.Combine(directory, "analyzer-protocol.json"),
                new
                {
                    schemaVersion = "doroti.analyzer-protocol-dump/v1",
                    identity,
                    inputs = ordered
                        .Select(item => new { source = item.Source, output = item.AnalyzerOutput })
                        .ToArray(),
                }
            );
        }
        ...
        {
            ArtifactFiles.WriteJson(
                Path.Combine(directory, "dart-ir.json"),
                new
                {
                    ...
                    inputs = ordered
                        .Select(item => new
                        {
                            ...
                            declarations = item
                                .DartDeclarations.OrderBy(value => value.Offset)
                                .Select(DumpDeclaration)
                                .ToArray(),
                        })
                        .ToArray(),
                }
        ...
        {
            ArtifactFiles.WriteJson(
                Path.Combine(directory, "core-ir.json"),
                new
                {
                    ...
                    inputs = ordered
                        .Select(item => new
                        {
                            ...
                            library = item.Library,
                            declarations = item
                                .CoreDeclarations.OrderBy(value => value.Offset)
                                .Select(declaration => new
                                {
                                    symbolId = declaration.Element.CanonicalId,
                                    declarationKind = declaration.Kind,
                                    memberSymbols = declaration
                                        .Members.Select(member => member.Element.CanonicalId)
        ...
        {
            ArtifactFiles.WriteJson(
                Path.Combine(directory, "csharp-ir.json"),
                new
                {
                    schemaVersion = "doroti.csharp-ir/v1",
                    identity,
                    inputs = ordered
                        .Where(item => item.GeneratedFile is not null)
                        .Select(item => new
                        {
                            source = item.Source,
                            ...
                            declarations = item
                                .Mappings.OrderBy(mapping => mapping.SourceOffset)
                                .Select(mapping => new
                                {
                                    mapping.Symbol,
                                    origin = new
                                    ...
                                    printedSpan = new
                                    {
                                        mapping.GeneratedLineStart,
                                        mapping.GeneratedLineEnd,
                                    },
                                })
                                .ToArray(),
                        })
    ...
    private static object DumpDeclaration(DartResolvedDeclaration declaration) =>
        new
        {
            declaration.Kind,
            declaration.Name,
            ...
            origin = new
            {
                declaration.Ast.Origin.Source,
                declaration.Offset,
                declaration.Length,
            },
            node = DumpNode(declaration.Ast),
            members = declaration
                .Members.OrderBy(member => member.Offset)
                .Select(member => new
                {
                    member.Kind,
                    member.Name,
    ...
    private static object DumpNode(DartAstNode node) =>
        new
        {
            node.Kind,
            node.Category,
