...
{
    public static CoreResolvedDeclaration[] Lower(
        IEnumerable<DartResolvedDeclaration> declarations
    ) =>
        declarations
            .Select(declaration => new CoreResolvedDeclaration(
                ...
                declaration
                    .Members.Select(member => new CoreResolvedMember(
                        member.Kind,
                        member.Name,
                        member.Offset,
                        member.Length,
    ...
    private static CoreResolvedElement LowerElement(DartResolvedElement element) =>
        new(
            element.Kind,
            element.Name,
            element.CanonicalId,
            ...
            element.Interfaces,
            element
                .TypeParameters?.Select(parameter => new CoreResolvedTypeParameter(
                    parameter.Name,
                    parameter.Bound,
                    LowerType(parameter.ResolvedBound)
                ))
                .ToArray(),
            element.ReturnType,
            LowerType(element.ResolvedReturnType),
            element
                .Parameters?.Select(parameter => new CoreResolvedParameter(
                    parameter.Name,
                    parameter.Type,
    ...
    private static CoreType? LowerType(DartType? type) =>
        type switch
        {
            null => null,
            ...
            DartInterfaceType { Symbol.Name: "bool" } item => new CoreBooleanType(
                item.ValueNullability != Nullability.NonNullable
            ),
            DartInterfaceType { Symbol.Name: "int" } item => new CoreIntegerType(
                item.ValueNullability != Nullability.NonNullable
            ),
            DartInterfaceType { Symbol.Name: "double" or "num" } item => new CoreFloatingType(
                item.ValueNullability != Nullability.NonNullable
            ),
            DartInterfaceType { Symbol.Name: "String" or "string" } item => new CoreStringType(
                item.ValueNullability != Nullability.NonNullable
            ),
            ...
            DartTypeParameterType item => new CoreNominalType(
                item.Symbol,
                [],
                item.ValueNullability != Nullability.NonNullable
            ),
            DartRecordType item => new CoreNominalType(
                ...
                item.Positional.Select(field => LowerType(field)!)
                    .Concat(item.Named.Values.Select(field => LowerType(field)!))
                    .ToArray(),
                item.ValueNullability != Nullability.NonNullable
            ),
            _ => throw new InvalidDataException(
                $"Unregistered Dart type lowering: {type.GetType().Name}"
            ),
        };
}
