...
internal sealed record DartLiteral(SourceOrigin Origin, DartType StaticType, object? Value)
    : DartExpression(Origin, StaticType);

internal sealed record DartIdentifier(SourceOrigin Origin, DartType StaticType, SymbolId Symbol)
    : DartExpression(Origin, StaticType);
...
internal sealed record DartConditional(
    ...
    DartExpression WhenFalse
) : DartExpression(Origin, StaticType);

internal sealed record DartAwait(SourceOrigin Origin, DartType StaticType, DartExpression Value)
    : DartExpression(Origin, StaticType);
