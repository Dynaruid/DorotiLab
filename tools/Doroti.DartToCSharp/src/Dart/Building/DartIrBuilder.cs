...
{
    public static LoweredDeclaration[] Lower(
        ...
        declarations
            .Select(declaration => new LoweredDeclaration(
                ...
                declaration.Element,
                declaration
                    .Members.Select(member => new LoweredMember(
                        member.Kind,
                        member.Name,
                        member.Offset,
                        member.Length,
    ...
    private static string Slice(string source, int offset, int length, string symbol)
    {
        if (offset < 0 || length < 0 || offset > source.Length - length)
        {
            throw new InvalidDataException(
                $"Analyzer span for {symbol} is outside its source file: {offset}+{length} of {source.Length}."
            );
        }
...
{
    public static DartResolvedDeclaration[] Build(
        MigrationIrDeclaration[] declarations,
        string source
    )
    {
        return declarations
            .Select(declaration =>
            {
                var analyzerElement =
                    declaration.Element
                    ?? throw new InvalidDataException(
                        $"Typed framework declaration {declaration.Name} has no resolved element identity."
                    );
                var element = ResolveElement(analyzerElement);
                var ast =
                    declaration.Ast
                    ?? throw new InvalidDataException(
                        $"Typed framework declaration {declaration.Name} has no AST."
                    );
                var typedAst = BuildNode(ast, source, element.CanonicalId);
                var members = declaration
                    .Members.Select(member => new DartResolvedMember(
                        member.Kind,
                        member.Name,
                        member.Offset,
                        member.Length,
                        ResolveElement(
                            member.Element
                                ?? throw new InvalidDataException(
                                    $"Typed framework member {declaration.Name}.{member.Name} has no resolved element identity."
                                )
                        ),
                        BuildNode(
                            member.Ast
                                ?? throw new InvalidDataException(
                                    $"Typed framework member {declaration.Name}.{member.Name} has no AST."
                                ),
                            source,
    ...
    private static DartResolvedElement ResolveElement(AnalyzerElement element) =>
        new(
            element.Kind,
            element.Name,
            element.CanonicalId,
            ...
            element.Interfaces,
            element
                .TypeParameters?.Select(parameter => new DartResolvedTypeParameter(
                    parameter.Name,
                    parameter.Bound,
                    DecodeOptionalType(parameter.Bound)
                ))
                .ToArray(),
            element.ReturnType,
            DecodeOptionalType(element.ReturnType),
            element
                .Parameters?.Select(parameter => new DartResolvedParameter(
                    parameter.Name,
                    parameter.Type,
    ...
    private static DartType? DecodeOptionalType(string? value) =>
        string.IsNullOrWhiteSpace(value) || value == "InvalidType"
            ? null
            : DartTypeDecoder.Decode(value);

    private static DartAstNode BuildNode(
        MigrationIrNode node,
        string source,
    ...
    )
    {
        ...
        if (
            !string.IsNullOrWhiteSpace(node.StaticType)
            && node.StaticType is not "dynamic" and not "Type" and not "InvalidType"
        )
        {
            try
            {
