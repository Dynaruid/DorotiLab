...
internal static class DartTypeDecoder
{
    ...
    private static DartType DecodeCore(string value, string original)
    {
        ...
        if (value == "dynamic")
        {
            return new DartDynamicType();
        }

        if (value == "void")
        {
            return new DartVoidType();
        }

        if (value == "Never")
        {
            return new DartNeverType(nullability);
        }

        if (value == "Null")
        {
            return new DartNullType();
        }
        ...
        if (genericStart >= 0)
        {
            ...
            {
                throw new DartTypeDecodeException(
                    original,
                    "generic argument list is not balanced"
                );
            }
    ...
    private static DartRecordType DecodeRecordType(
        string fieldsText,
        string original,
    ...
    )
    {
        ...
        foreach (var field in SplitTopLevel(fieldsText, ',').Where(item => item.Length > 0))
        {
            ...
            {
                foreach (
                    var namedField in SplitTopLevel(trimmed[1..^1], ',')
                ...
                )
                {
                    ...
                    if (separator < 0)
                    {
                        throw new DartTypeDecodeException(
                            original,
                            $"named record field is missing its name: '{namedField}'"
                        );
                    }
                    ...
                    if (!IsIdentifier(name))
                    {
                        throw new DartTypeDecodeException(
                            original,
                            $"invalid named record field '{name}'"
                        );
                    }
                    if (!named.TryAdd(name, DecodeCore(namedField[..separator].Trim(), original)))
                    {
                        throw new DartTypeDecodeException(
                            original,
                            $"duplicate named record field '{name}'"
                        );
                    }
                }
            }
    ...
    private static DartFunctionParameter[] DecodeFunctionParameters(string value, string original)
    {
        ...
        foreach (var segment in SplitTopLevel(value, ',').Where(item => item.Length > 0))
        {
            ...
            )
            {
                var open = trimmed[0];
                foreach (
                    var grouped in SplitTopLevel(trimmed[1..^1], ',').Where(item => item.Length > 0)
                )
                {
                    result.Add(
    ...
    private static int FunctionParametersStart(string signature, string original)
    {
        ...
        if (!signature.StartsWith('<'))
        {
            throw new DartTypeDecodeException(
                original,
                "function type is missing its parameter list"
            );
        }
        var depth = 0;
        for (var index = 0; index < signature.Length; index++)
        {
            ...
            else if (signature[index] == '>' && --depth == 0)
            {
                ...
                if (parameterStart >= signature.Length || signature[parameterStart] != '(')
                {
                    throw new DartTypeDecodeException(
                        original,
                        "generic function type is missing its parameter list"
                    );
                }
        ...
        throw new DartTypeDecodeException(
            original,
            "generic function type parameters are not balanced"
        );
    }
    ...
    private static int FindTopLevel(string value, string token)
    {
        ...
        for (var index = 0; index <= value.Length - token.Length; index++)
        {
            ...
            if (angle == 0 && value.AsSpan(index).StartsWith(token, StringComparison.Ordinal))
            {
                return index;
            }
        }
    ...
    private static int FindTopLevelCharacter(string value, char target)
    {
        ...
        for (var index = 0; index < value.Length; index++)
        {
            ...
            else if (value[index] == target && round == 0)
            {
                return index;
            }
        }
    ...
    private static int FindLastTopLevelSpace(string value)
    {
        ...
        for (var index = value.Length - 1; index >= 0; index--)
        {
            ...
            else if (value[index] == ' ' && angle == 0 && round == 0)
            {
                return index;
            }
        }
    ...
    private static bool IsTypeName(string value) =>
        value.Split('.', StringSplitOptions.RemoveEmptyEntries).All(IsIdentifier);
