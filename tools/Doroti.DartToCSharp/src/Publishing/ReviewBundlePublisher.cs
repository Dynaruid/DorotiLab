...
internal static class ReviewBundlePublisher
{
    public static void Publish(string outputDirectory, Action<string> write)
    {
        ...
        var parent =
            ...
            ?? throw new PortContractException(
                "DORPORT003",
                "Review output must have a parent directory."
            );
        if (Path.GetPathRoot(target) == target)
        {
            throw new PortContractException(
                "DORPORT003",
                "A filesystem root cannot be used as review output."
            );
        }
        Directory.CreateDirectory(parent);
        var staging = Path.Combine(
            parent,
            $".{Path.GetFileName(target)}.doroti-review-{Guid.NewGuid():N}"
        );
        try
        {
    ...
    private static void EnsureEqual(string expectedRoot, string actualRoot)
    {
        ...
        if (
            ...
            || expected.Any(item =>
                !string.Equals(item.Value, actual[item.Key], StringComparison.Ordinal)
            )
        )
        {
            throw new PortContractException(
                "DORPORT004",
                $"Existing review bundle differs from deterministic output: {actualRoot}"
            );
        }
    }
