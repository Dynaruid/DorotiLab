...
Windows presenter 기본값은 계속 `AngleD3D11`입니다. Maintainer는 `DOROTI_WINDOWS_PRESENTER=Vulkan`으로 실험적 direct Vulkan 경로를 명시적으로 시험할 수 있으며, capable GPU가 둘 이상이거나 선택을 고정할 때 `DOROTI_WINDOWS_VULKAN_DEVICE`를 지정합니다. Vulkan은 System32 Vulkan 1.1과 surface/Win32-surface/swapchain extension을 요구하지만 swapchain-maintenance는 요구하지 않습니다. 자동 fallback이 없으며 `experimentalAcrylic`과 함께 선택할 수 없습니다. AMD Radeon 780M에서는 retained oversized-child surface가 일반적인 per-size swapchain 재생성을 제거합니다. Flutter Windows resize handshake처럼 Vulkan 전용 경로는 실제 `WM_SIZE`를 exact logical frame이 present될 때까지 최대 100ms 유지한 뒤 DWM 경계를 통과합니다. `WM_SIZING` provisional layout과 parent fallback은 사용하지 않습니다. 반복 검증은 축소했고 8방향 반복 matrix는 opt-in입니다. 실제 resize/scan-out과 전체 GPU/DPI/monitor matrix는 계속 `notVerified`이므로 Vulkan은 실험적 opt-in으로 유지합니다.
