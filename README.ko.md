...
Windows presenter 기본값은 계속 `AngleD3D11`입니다. Maintainer는 `DOROTI_WINDOWS_PRESENTER=Vulkan`으로 실험적 Vulkan 경로를 명시적으로 시험할 수 있으며, capable GPU가 둘 이상이거나 선택을 고정할 때 `DOROTI_WINDOWS_VULKAN_DEVICE`를 지정합니다. 제품 경로는 System32 Vulkan 1.1, dedicated D3D11-texture external memory, Windows Presentation을 사용하며 Vulkan WSI swapchain은 0개입니다. SkiaSharp는 `4.152.0-rc.1.26426.14`로 고정하고 Vulkan context는 `SkiaSharp.Vulkan.Silk.NET`으로 구성합니다. Vulkan은 top-level HWND에 native topmost DirectComposition target 하나를 직접 붙이고 full-capacity source를 identity로 유지합니다. 모든 interactive edge가 제안 크기의 Skia raster, Vulkan copy, Presentation 제출을 geometry 전에 완료합니다. 원점이 고정되는 우측·하단은 DWM 표시 경계까지 기다립니다. 원점이 움직이는 좌측·상단은 제출 직후 반환해 새 frame이 다음 USER32 geometry와 합쳐질 수 있게 하며 이전 screen origin에서 강제로 한 번 표시하지 않습니다. Programmatic/mismatch geometry는 bounded exact fallback을 유지하고 별도의 100 ms 제한은 terminal wait의 fail-safe입니다. Windows 11 24H2 이상에서는 explicit Vulkan과 `experimentalAcrylic`을 함께 선택할 수 있습니다. Host-backdrop-enabled non-topmost `DesktopWindowTarget`의 active `DesktopAcrylicController`와 HWND-wide transient system-backdrop underlay 아래에 topmost premultiplied Vulkan overlay를 합성합니다. 실제 물리 확인에서는 standard-HWND moving-origin transaction의 떨림이 남았으므로 이 동작은 acceptance하지 않습니다. App-owned fixed-envelope/custom-non-client 대안은 채택하지 않으며, 좌측·상단 물리 acceptance가 해결되지 않은 상태로 Vulkan을 실험적 opt-in으로 유지합니다.
