...
public static partial class TimerValidationExport
{
    ...
    {
        if (milliseconds is < 100 or > 120000)
            throw new ArgumentOutOfRangeException(nameof(milliseconds));
        var owner = Environment.CurrentManagedThreadId;
        var context =
            SynchronizationContext.Current
            ?? throw new InvalidOperationException("No JS owner context.");
        ...
        using var microtaskScope = DartAsyncRuntime.enterMicrotaskScheduler(action =>
        ...
        try
        {
            for (var i = 0; i < 24; i++)
                timers.Add(
                    DartTimer.periodic(Duration.Create(milliseconds: 1 + i % 7), _ => Observe())
                );
            var canceledCalls = 0;
            ...
            var delayed = await new Future<int>(Duration.Create(milliseconds: 2)).then<int>(_ =>
            {
                Observe();
                return 42;
            });
            var completer = new Completer<int>();
            var timeoutResult = await completer.future.timeout(
                Duration.Create(milliseconds: 5),
                (Func<object>)(
                    () =>
                    {
                        Observe();
            ...
            var timedOut = false;
            using (
                var cancellation = new CancellationTokenSource(TimeSpan.FromMilliseconds(5), clock)
            )
            {
                try
                {
                    await Task.Delay(TimeSpan.FromSeconds(10), clock, cancellation.Token);
                }
            ...
            await Task.Run(() =>
            {
                using var timer = clock.CreateTimer(
                    _ => throw new InvalidOperationException("Canceled timer fired"),
                    null,
            ...
            return JsonSerializer.Serialize(
                new
                {
                    ...
                    peakSystemTimers,
                    owner,
                    diagnostics = JsonSerializer.Deserialize<JsonElement>(
                        BrowserTimeProvider.CaptureDiagnostics()
                    ),
                }
            );
        }
