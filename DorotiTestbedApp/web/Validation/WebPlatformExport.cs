...
public static partial class WebPlatformExport
{
    private static Task<string> OnOwner(Func<Task<string>> action)
    {
        ...
        var owner =
            PlatformEffectFixtureProbe.Owner
            ?? throw new InvalidOperationException("Platform fixture is not mounted.");
        owner.DispatchPlatformEvent(async () =>
        {
            try
            {
    ...
    [JSExport]
    public static Task<string> Stage(int stage) =>
        OnOwner(() =>
        {
    ...
    [JSExport]
    public static Task<string> Style(double strength, double saturation, int tint, bool raster) =>
        OnOwner(() =>
        {
    ...
    [JSExport]
    public static Task<string> Command(int operation, string text, int documentGeneration) =>
        OnOwner(async () =>
        {
            var web =
                PlatformEffectFixtureProbe.WebView
                ?? throw new InvalidOperationException("WebView is not mounted.");
            await web.Ready;
            try
            {
                return JsonSerializer.Serialize(
                    await web.ExecuteAsync(
                        new((WebViewOperation)operation, text, documentGeneration)
                    )
                );
            }
    ...
    [JSExport]
    public static Task<string> Policy(string origin) =>
        OnOwner(async () =>
        {
            ...
            await using (var rejected = new WebViewController(owner, new(Html: "private")))
            {
                try
                {
                    ...
                    throw new Exception("Private iframe profile was accepted.");
                }
                catch (WebViewException error) when (error.Code == WebViewError.Unsupported)
                {
                    passed.Add("private profile rejected");
                }
            }
            await using var web = new WebViewController(
                owner,
                new(
                    Html: """
            ...
            web.Changed += e =>
            {
                if (e.Kind == WebViewEventKind.Message)
                    messages++;
            };
            await web.Ready;
            for (var i = 0; i < 30 && messages == 0; i++)
                await Task.Delay(50);
            if (messages != 1)
                throw new Exception($"Expected one trusted message, got {messages}.");
            ...
            if (messages != 1)
                throw new Exception("Invalid or repeated message accepted.");
            passed.Add("replayed wrong nonce stale and oversized messages rejected");
            try
            {
                await web.ExecuteAsync(new(WebViewOperation.Navigate, "https://example.invalid/"));
                throw new Exception("Origin policy ignored.");
            }
            catch (WebViewException error) when (error.Code == WebViewError.InvalidRequest)
            {
                passed.Add("command navigation allowlist");
            }
            ...
            {
                passed.Add("caller cancellation");
            }
            var pending = web.ExecuteAsync(
                new(WebViewOperation.EvaluateJavaScript, "new Promise(()=>{})")
            );
            await web.DisposeAsync();
            try
            {
                await pending;
                throw new Exception("Close did not cancel JavaScript.");
            }
            catch (WebViewException error) when (error.Code == WebViewError.Closed)
            {
                passed.Add("close cancels pending JavaScript");
            }
