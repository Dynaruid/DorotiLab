    ...
    protected override void OnCreate(global::Android.OS.Bundle? savedInstanceState)
    {
        ...
        if (Intent?.GetStringExtra("doroti_webview_evidence") == "1")
        {
            Environment.SetEnvironmentVariable(
                "DOROTI_ANDROID_WEBVIEW_EVIDENCE",
                System.IO.Path.Combine(EvidenceDirectory(), "webview-evidence.txt")
            );
        }
        ...
        if (Intent?.GetStringExtra("doroti_effect_calibration") == "1")
        {
            Environment.SetEnvironmentVariable(
                ...
                System.IO.Path.Combine(EvidenceDirectory(), "effect-calibration")
            );
        }
