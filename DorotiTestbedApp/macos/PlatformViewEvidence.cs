...
internal static class PlatformViewEvidence
{
    internal static async Task CaptureAsync()
    {
        ...
        try
        {
            ...
            if (
                Environment.GetEnvironmentVariable("DOROTI_PLATFORM_VIEW_COMPOSITION")
                == "interleaved"
            )
            {
            ...
            for (var attempt = 0; attempt < 300 && result is null; attempt++)
            {
                ...
                await dispatcher.InvokeAsync(() =>
                {
                    ...
                    if (
                        controls.Length != 2
                        || controls.Any(control => control.Superview?.Hidden != false)
                    )
                        return ValueTask.CompletedTask;
                    var button = controls.OfType<NSButton>().Single();
                    var editor = controls.OfType<NSTextField>().Single();
                    button.PerformClick(button);
                    if (button.Title != "Native clicks: 1")
                        throw new InvalidOperationException(
                            "Product button activation was not delivered once."
                        );
                    if (!editor.Window!.MakeFirstResponder(editor))
                        throw new InvalidOperationException("Product editor rejected focus.");
                    var fieldEditor =
                        editor.CurrentEditor as NSTextView
                        ?? throw new InvalidOperationException("No product field editor.");
                    fieldEditor.InsertText(
                        new NSString("product-input"),
                        new NSRange(NSRange.NotFound, 0)
                    );
                    if (!editor.StringValue.Contains("product-input"))
                        throw new InvalidOperationException("Product editor lost inserted text.");
                    var bounds = controls.Select(control => control.Superview!.Frame).ToArray();
                    if (bounds.Any(rect => rect.Width != 240 || rect.Height != 64))
                        throw new InvalidOperationException("Scene/native layout size mismatch.");
                    result = new(
                        true,
                        controls.Select(control => control.Identifier!).ToArray(),
                        bounds
                            .Select(rect =>
                                new double[] { rect.X, rect.Y, rect.Width, rect.Height }
                            )
                            .ToArray(),
                        (double)editor.Window.BackingScaleFactor,
                        button.Title,
                        editor.StringValue,
                        (int)editor.Window.WindowNumber,
            ...
            if (result is null)
                throw new TimeoutException(
                    "The product widget/Metal path did not attach both native controls."
                );
            File.WriteAllText(
                path,
                JsonSerializer.Serialize(
                    result,
                    PlatformViewEvidenceJsonContext.Default.ProductPlatformViewResult
                )
            );
        ...
        {
            File.WriteAllText(path + ".exception.txt", error.ToString());
        }
    }
    ...
    private static async Task CaptureInterleavedAsync(string path)
    {
            ...
            await dispatcher.InvokeAsync(() =>
            {
                controls = Microsoft
                    ...
                    .Where(control =>
                        ...
                        && control.Superview?.Hidden == false
                    )
                    .ToArray();
                return ValueTask.CompletedTask;
            });
        ...
        if (controls.Length != 2)
            throw new TimeoutException("Interleaved product controls did not attach.");
        var editor = controls.OfType<NSTextField>().Single();
        var button = controls.OfType<NSButton>().Single();
        await dispatcher.InvokeAsync(() =>
        {
        ...
        for (var attempt = 0; attempt < 1200; attempt++)
        {
            await Task.Delay(100);
            if (
                !File.Exists(path + ".stage")
                || !int.TryParse(File.ReadAllText(path + ".stage"), out var stage)
                || stage == previous
            )
                continue;
            await dispatcher.InvokeAsync(() =>
            {
                (
                    PlatformViewFixtureProbe.SetStage
                    ?? throw new InvalidOperationException("Fixture stage controller unavailable.")
                )(stage);
                return ValueTask.CompletedTask;
            });
            ...
            await dispatcher.InvokeAsync(() =>
            {
                if (
                    editor.StringValue != "preserved-native-state"
                    || editor.Superview?.Superview is null
                    || button.Superview?.Superview is null
                )
                    throw new InvalidOperationException(
                        "Native identity/state was lost while changing paint order."
                    );
                ...
                if (shouldShield ? hit is not DorotiMacOSMetalView : hit != editor)
                    throw new InvalidOperationException(
                        $"Stage {stage} hit target mismatch: {hit?.GetType().Name}"
                    );
                if (shouldShield)
                {
                    ...
                    using var down = NSEvent.MouseEvent(
                    ...
                    using var up = NSEvent.MouseEvent(
                ...
                var slots = overlay
                    .Subviews.Where(view =>
                        view.GetType().Name == "AppKitPlatformRasterSurface" && !view.Hidden
                    )
                    .ToArray();
                if (stage == 5 && slots.Length < 3)
                    throw new InvalidOperationException(
                        "Missing intermediate/foreground Metal surfaces."
                    );
                stageResult = new InterleavedStageResult(
                    true,
                    stage,
                    controls.Select(control => control.Identifier!).ToArray(),
                    editor.StringValue,
                    hit!.GetType().Name,
                    slots.Select(view => (double)view.Layer!.ZPosition).ToArray(),
                    (int)editor.Window!.WindowNumber,
                    (double)editor.Window.BackingScaleFactor,
                    new double[] { origin.X - 20, origin.Y - 20 },
                    new double[]
                    {
                        overlay.ConvertPointToView(CoreGraphics.CGPoint.Empty, null!).X,
                        editor.Window.Frame.Height
                            - overlay.ConvertPointToView(CoreGraphics.CGPoint.Empty, null!).Y,
                    }
                );
                return ValueTask.CompletedTask;
            });
            ...
            if (clicks != (stage is 1 or 2 or 5 or 6 or 9 ? 1 : 0))
                throw new InvalidOperationException(
                    $"Stage {stage}: shield delivered {clicks} foreground taps."
                );
            File.WriteAllText(
...
internal sealed record ProductPlatformViewResult(
    bool Passed,
    string[] Identifiers,
    double[][] Bounds,
    double BackingScale,
...
internal sealed record InterleavedStageResult(
    bool Passed,
    int Stage,
    string[] Identifiers,
    string EditorText,
