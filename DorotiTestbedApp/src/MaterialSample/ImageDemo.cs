...
internal sealed class SampleImageDemoState : State<SampleImageDemo>
{
    ...
    {
        using var stream =
            typeof(SampleImageDemo).Assembly.GetManifestResourceStream("MaterialSample.image.webp")
            ?? throw new InvalidOperationException("Material sample image resource is missing.");
        using var bytes = new MemoryStream();
        stream.CopyTo(bytes);
        return new Uint8List(bytes.ToArray());
    });
    private bool _url,
        _cover,
        _loading;
    ...
    private M.ColorScheme? _light,
        _dark;

    private IImageProvider Provider() =>
        _url ? new NetworkImageIo(Url) : new MemoryImage(LocalBytes.Value);
        ...
        new M.Card(
            child: new Padding(
                ...
                child: new Column(
                    ...
                    children:
                    [
                        new Text("Image demo", style: M.Theme.of(context).textTheme.titleLarge),
                        new SizedBox(height: 12),
                        new M.SegmentedButton<string>(
                            segments:
                            [
                                new("Local", label: new Text("Local asset")),
                                new("URL", label: new Text("Image URL")),
                            ],
                            selected: [_url ? "URL" : "Local"],
                            onSelectionChanged: values => SelectSource(values.Contains("URL"))
                        ),
                        new SizedBox(height: 12),
                        new Text(_url ? "Unsplash · remote image" : "Mae Mu · Unsplash"),
                        ...
                        new ClipRRect(
                            ...
                            child: new ColoredBox(
                                ...
                                child: new SizedBox(
                                    ...
                                    height: 240,
                                    child: new Image(
                                        key: new Doroti.Framework.Foundation.ValueKey<int>(_reload),
                                        image: Provider(),
                                        ...
                                        frameBuilder: (_, child, frame, synchronous) =>
                                            frame is not null || synchronous
                                                ? child
                                                : new Center(child: new Text("Loading image…")),
                                        errorBuilder: (_, _, _) =>
                                            new Center(
                                                child: new Column(
                                                    ...
                                                    children:
                                                    [
                                                        new Text("Image could not be loaded."),
                                                        new M.TextButton(
                                                            child: new Text("Retry image"),
                                                            onPressed: RetryImage
                                                        ),
                                                    ]
                                                )
                                            )
                        ...
                        new Wrap(
                            ...
                            [
                                new M.ChoiceChip(
                                    label: new Text("Contain"),
                                    selected: !_cover,
                                    onSelected: _ => setState(() => _cover = false)
                                ),
                                new M.ChoiceChip(
                                    label: new Text("Cover"),
                                    selected: _cover,
                                    onSelected: _ => setState(() => _cover = true)
                                ),
                                M.FilledButton.CreateTonal(
                                    child: new Text(_loading ? "Extracting…" : "Extract colors"),
                        ...
                        .. _error is null
                            ...
                            [
                                new SizedBox(height: 12),
                                new Text(
                                    "Could not extract colors. Check the image and try again."
                                ),
                            ],
                        .. _light is null || _dark is null
                            ? Array.Empty<Widget>()
                            :
                            [
                                new SizedBox(height: 16),
                                Palette(context, "Light palette", _light),
                                new SizedBox(height: 12),
                                Palette(context, "Dark palette", _dark),
                            ],
                    ]
                )
            )
    ...
    {
        (string Label, Color Color, Color On)[] colors =
        [
        ...
        return new Column(
            ...
            children:
            [
                new Text(title, style: M.Theme.of(context).textTheme.titleSmall),
                new SizedBox(height: 8),
                new Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: colors
                        .Select(role =>
                            (Widget)
                                new Container(
                                    width: 108,
                                    padding: EdgeInsets.CreateAll(10),
                                    decoration: new BoxDecoration(
                                        color: role.Color,
                                        borderRadius: BorderRadius.CreateCircular(8)
                                    ),
                                    child: new Text(
                                        $"{role.Label}\n#{role.Color.red:X2}{role.Color.green:X2}{role.Color.blue:X2}",
                                        style: new TextStyle(color: role.On)
                                    )
                                )
                        )
                        .ToList()
