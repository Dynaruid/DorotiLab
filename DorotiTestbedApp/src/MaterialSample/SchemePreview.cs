...
internal sealed class SchemePreview(string label, M.ColorScheme scheme) : StatelessWidget
{
    ...
    {
        Widget Box(string name, Color color, Color on, double height, double width) =>
            new ColorBox(name, color, on, height, width);
        Widget RowOf(params Widget[] children) =>
            new Row(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: children.ToList()
            );
        Widget ColumnOf(params Widget[] children) =>
            new Column(mainAxisSize: MainAxisSize.min, children: children.ToList());
        Widget Quartet(string name, Color color, Color on, Color container, Color onContainer) =>
            ColumnOf(
                Box(name, color, on, 87, 208),
                Box("On " + name, on, color, 40, 208),
                new SizedBox(height: 5),
                Box(name + " Container", container, onContainer, 87, 208),
                Box("On " + name + " Container", onContainer, container, 40, 208)
            );
        ...
        return new M.Theme(
            ...
            child: new FittedBox(
                ...
                child: new Container(
                    width: 902,
                    padding: EdgeInsets.CreateOnly(top: 16, left: 16, right: 16),
                    decoration: new BoxDecoration(
                        color: scheme.surface,
                        borderRadius: BorderRadius.CreateCircular(12),
                        border: Border.CreateAll(
                            color: theme.brightness == scheme.brightness
                            ...
                            RowOf(
                                ColumnOf(
                                    ...
                                    RowOf(
                                        ...
                                        Box("Outline", scheme.outline, scheme.surface, 40, 158.59),
                                        Box(
                                            "Outline Variant",
                                ...
                                ColumnOf(
                                    ...
                                    RowOf(
                                        Box("Scrim", scheme.scrim, M.Colors.white, 40, 96.31),
                                        new SizedBox(width: 20),
                                        Box("Shadow", scheme.shadow, M.Colors.white, 40, 96.31)
                                    ),
...
internal sealed class ColorBoxState : State<ColorBox>
{
    ...
    private async void Copy()
    {
        ...
        messenger.showSnackBar(new M.SnackBar(content: new Text($"Copied {hex} to clipboard")));
    }
            ...
            child: new Container(
                ...
                child: new DefaultTextStyle(
                    style: M.Theme.of(context)
                        .textTheme.labelSmall!.copyWith(color: widget.OnColor),
                    child: new Stack(
                        children:
                        [
                            new Positioned(top: 10, left: 10, child: new Text(widget.Label)),
                            _hovered
                                ? new Positioned(
                                    top: 0,
                                    right: 0,
                                    child: new M.IconButton(
                                        ...
                                        icon: new Icon(M.Icons.copy, size: 24),
                                        onPressed: Copy
                                    )
                                )
