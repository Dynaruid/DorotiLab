...
internal static class WindowsEffectCalibration
{
    ...
    internal static void Start(DorotiView owner, WebViewController controller, string variable = "DOROTI_WINDOWS_EFFECT_CALIBRATION")
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                {
                    await controller.ExecuteAsync(new(WebViewOperation.LoadHtml,
                        "<!doctype html><title>calibration</title><style>html,body{margin:0;width:100%;height:100%;" + style + "}</style>"));
                    for (var i = 0; i < 100; i++)
                    {
                        var state = await controller.ExecuteAsync(new(WebViewOperation.State));
                        if (!state.IsLoading && state.Title == "calibration") return;
                        await Task.Delay(50);
                    }
                    throw new TimeoutException("Calibration HTML did not load.");
                }
                await Load("background:linear-gradient(to right,black 0%,black 50%,white 50%,white 100%)");
                async Task Stage(string name, double sigma, double saturation = 1, bool raster = false, uint tint = 0)
                {
                    var completion = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
                    owner.DispatchPlatformEvent(() =>
                    {
                        PlatformEffectFixtureProbe.SetRasterSource?.Invoke(raster);
                        PlatformEffectFixtureProbe.SetStyle?.Invoke(new(Match: PlatformEffectMatchPolicy.ExactSigma,
                            ExactSigma: sigma, Saturation: saturation, Tint: tint));
                    ...
                    for (var i = 0; i < 200; i++)
                    {
                        if (System.IO.File.Exists(path + ".ack") && System.IO.File.ReadAllText(path + ".ack") == name) return;
                        await Task.Delay(50);
                    }
                ...
                {
                    await Load("background:linear-gradient(to right,rgb(180,80,60) 0%,rgb(180,80,60) 50%,rgb(60,80,180) 50%,rgb(60,80,180) 100%)");
                    await Stage("color-edge", 16, 2);
                    await Stage("color-edge-zero", 0, 2);
                }
                System.IO.File.WriteAllText(path + ".done", "PASS");
            }
            catch (Exception error) { System.IO.File.WriteAllText(path + ".done", "FAIL " + error); }
        });
    }
}
