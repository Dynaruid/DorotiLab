...
internal static class QtWebViewEvidence
{
    ...
    {
        var lines = new List<string> { "Linux Qt product WebViewController / Quick scene", "physical=notVerified; nativeAot=notVerified" };
        ...
        try
        {
            ...
            Check(features is { Navigation: true, JavaScript: true, EphemeralProfile: true, SharedPersistentProfile: true, ClearAllData: false }, "feature query");
            ...
            Check((await Js("/*" + new string('한', 600_000) + "*/7")).Json == "7", "UTF-8 command limit survives JSON escaping");
            ...
            await Reject(() => primary.ExecuteAsync(new(WebViewOperation.ClearData)), WebViewError.Unsupported);
            ...
            await content.ExecuteAsync(new(WebViewOperation.Navigate, "doroti-app://content/index.html?second=1")); await Loaded(content, "App content");
            Check((await content.ExecuteAsync(new(WebViewOperation.State))).CanGoBack, "navigation history");
            await content.ExecuteAsync(new(WebViewOperation.Back)); await Loaded(content, "App content");
            Check((await content.ExecuteAsync(new(WebViewOperation.State))).CanGoForward, "back enables forward");
            await content.ExecuteAsync(new(WebViewOperation.Forward)); await Loaded(content, "App content");
            Check((await content.ExecuteAsync(new(WebViewOperation.State))).Url!.EndsWith("?second=1"), "forward restores URL");
            var beforeReload = (await content.ExecuteAsync(new(WebViewOperation.State))).DocumentGeneration;
            await content.ExecuteAsync(new(WebViewOperation.Reload)); await Loaded(content, "App content");
            Check((await content.ExecuteAsync(new(WebViewOperation.State))).DocumentGeneration > beforeReload, "reload advances document generation");
            await Reject(() => ContentJs("'x'.repeat(2*1024*1024+1)"), WebViewError.JavaScript);
            await ContentJs("window.rangeResult=null;fetch('style.css',{headers:{Range:'bytes=0-3'}}).then(async r=>window.rangeResult=[r.status,(await r.text()).length],()=>window.rangeResult='denied');undefined");
            for (var i = 0; i < 30 && (await ContentJs("window.rangeResult")).Json == "null"; i++) await Task.Delay(50);
            Check((await ContentJs("window.rangeResult")).Json == "\"denied\"", "unsupported Range rejected explicitly: " + (await ContentJs("window.rangeResult")).Json);
            ...
            Check(messages.TryDequeue(out var message) && message.MessageName == "probe" && message.MessageJson?.Contains("한글") == true, "trusted app isolated-world transport");
            await ContentJs("doroti.postMessage('oversized','x'.repeat(70000));undefined");
            await ContentJs("window.dispatchEvent(new CustomEvent('doroti-message',{detail:JSON.stringify({generation:0,requestId:1,name:'stale',payload:null})}));undefined");
            ...
            Check((await ContentJs("typeof doroti")).Json == "\"undefined\"", "untrusted document has no message facade");
            ...
            {
                await shared.Ready; await Loaded(shared);
                Check((await shared.ExecuteAsync(new(WebViewOperation.Features))).Features!.SharedPersistentProfile, "shared persistent profile creation");
            }
