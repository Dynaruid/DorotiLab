...
internal sealed class MediaQueryFixture : StatefulWidget
{
    ...
    private sealed class FixtureState : State<MediaQueryFixture>
    {
        ...
        public override Widget build(BuildContext context)
        {
            ...
            var report =
                $"view={view.viewId} metrics={raw.generation} surface={raw.surfaceGeneration}\n"
            ...
            Console.WriteLine(
                "MQ-FIXTURE "
                    + JsonSerializer.Serialize(
                        new MediaQueryReport(
                            view.viewId,
                            raw.physicalSize.width,
                            raw.physicalSize.height,
                            raw.devicePixelRatio,
                            raw.generation,
                            raw.surfaceGeneration,
                            raw.viewPadding,
                            raw.viewInsets,
                            raw.systemGestureInsets,
                            media.size.width,
                            media.size.height,
                            media.padding.ToString(),
                            media.viewInsets.ToString(),
                            _safe,
                            _resize,
                            _maintain
                        ),
                        MediaQueryJsonContext.Default.MediaQueryReport
                    )
            );
                    ...
                    child: new Column(
                        ...
                        [
                            ...
                            new Wrap(
                                ...
                                [
                                    new M.TextButton(
                                        onPressed: () => setState(() => _safe = !_safe),
                                        child: new Text($"SafeArea: {_safe}")
                                    ),
                                    new M.TextButton(
                                        onPressed: () => setState(() => _resize = !_resize),
                                        child: new Text($"Scaffold resize: {_resize}")
                                    ),
                                    new M.TextButton(
                                        onPressed: () => setState(() => _maintain = !_maintain),
                                        child: new Text($"Maintain bottom: {_maintain}")
                                    ),
                                    new M.TextButton(
                                        onPressed: () => setState(() => _sliver = !_sliver),
                                        child: new Text($"SliverSafeArea: {_sliver}")
                                    ),
                                    new M.TextButton(
                                        onPressed: () =>
                            ...
                            new M.TextField(
                                decoration: new M.InputDecoration(
                                    labelText: "Keyboard / caret / focus"
                                )
                            ),
                            new M.TextField(
                                minLines: 2,
                                maxLines: 4,
                                decoration: new M.InputDecoration(labelText: "Multiline editor")
                            ),
                            new SizedBox(height: 180),
                            new M.TextField(
                                decoration: new M.InputDecoration(labelText: "Bottom input")
                            ),
                        ]
                    )
                );
            Widget body = _sliver
                ? new CustomScrollView(
                    ...
                    [
                        new SliverSafeArea(
                            sliver: new SliverToBoxAdapter(child: new Builder(builder: Content))
                        ),
                    ]
                )
            ...
            if (_safe && !_sliver)
            {
                body = new SafeArea(maintainBottomViewPadding: _maintain, child: body);
            }

            return new M.Scaffold(
                backgroundColor: new Color(0xffe6f3ff),
                resizeToAvoidBottomInset: _resize,
                appBar: new M.AppBar(title: new Text("MediaQuery / SafeArea")),
                body: body,
                bottomNavigationBar: new SafeArea(
                    maintainBottomViewPadding: _maintain,
                    child: new SizedBox(
                        height: 40,
                        child: new Center(child: new Text("Bottom edge marker"))
                    )
                )
            );
        }
