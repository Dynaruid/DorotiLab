...
internal static class WindowsWebViewEvidence
{
    private static int _started;

    internal static void Start(DorotiView owner, WebViewController primary)
    {
        var path = Environment.GetEnvironmentVariable("DOROTI_WINDOWS_WEBVIEW_EVIDENCE");
    ...
    private static async Task Run(DorotiView owner, WebViewController primary, string path)
    {
        ...
        {
            "WindowsAppSDK product WebViewController / CompositionController",
        ...
        async Task Loaded(WebViewController controller, string? title = null)
        {
            ...
            throw new TimeoutException("Document did not load: " + title);
        }
        ...
        try
        {
            ...
            await ContentJs(
                "chrome.webview.postMessage({version:1,documentGeneration:"
                    + state.DocumentGeneration
                    + ",requestId:1,name:'forged',payload:'bad'})"
            );
            ...
            lines.Add("PASS Windows WebView product commands");
        }
        catch (Exception error)
        {
