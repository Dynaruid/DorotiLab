...
internal static class AndroidWebViewEvidence
{
    ...
    private static async Task Run(DorotiView owner, WebViewController primary, string path)
    {
        ...
        {
            "Android product WebViewController / native hierarchy",
        ...
        try
        {
            ...
            Check(
                ...
                "trusted native-origin main-frame bridge"
            );
            await ContentJs("doroti.postMessage('oversized','x'.repeat(70000));undefined");
            await ContentJs(
                "dorotiNative.postMessage(JSON.stringify({version:1,documentGeneration:0,requestId:1,name:'stale',payload:null}));undefined"
            );
            var messageGeneration = (
                await content.ExecuteAsync(new(WebViewOperation.State))
            ).DocumentGeneration;
            await ContentJs(
                "(()=>{const f=document.createElement('iframe');f.id='message-frame';f.srcdoc=\"<script>dorotiNative.postMessage(JSON.stringify({version:1,documentGeneration:"
                    + messageGeneration
                    + ",requestId:1,name:'child-frame',payload:null}))<"
                    + "/script>\";document.body.append(f)})()"
            );
            ...
            await ContentJs(
                "window.rangeResult=null;fetch('style.css',{headers:{Range:'bytes=0-3'}}).then(async r=>window.rangeResult=[r.status,(await r.text()).length]);undefined"
            );
            string? range = null;
            for (var i = 0; i < 30; i++)
            {
                range = (await ContentJs("window.rangeResult")).Json;
                if (range != "null")
                {
                    break;
                }
            ...
            await Loaded(content, "App content");
            Check(
                (await ContentJs("localStorage.getItem('doroti-probe')")).Json == "null",
            ...
            Check(
                (await ContentJs("typeof dorotiNative")).Json == "\"undefined\"",
                "untrusted document has no native message object"
            );
            ...
            await using (
                var sharedA = await Create(
                    options with
                    {
                        Profile = WebViewProfile.SharedPersistent,
                    }
                )
            )
            await using (
                var sharedB = await Create(
                    options with
                    {
                        Profile = WebViewProfile.SharedPersistent,
                    }
            ...
            )
            {
                ...
                await sharedA.LoadAppContentAsync("webview/index.html");
                await Loaded(sharedA, "App content");
                await sharedA.ExecuteAsync(
                    new(
                        WebViewOperation.EvaluateJavaScript,
                        "localStorage.setItem('shared-probe','shared');undefined"
                    )
                );
                await sharedB.LoadAppContentAsync("webview/index.html");
                await Loaded(sharedB, "App content");
                Check(
                    (
                        await sharedB.ExecuteAsync(
                            new(
                                WebViewOperation.EvaluateJavaScript,
                                "localStorage.getItem('shared-probe')"
                            )
                        )
                    ).Json == "\"shared\"",
                    "explicit shared persistent profile"
                );
