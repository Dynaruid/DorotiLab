...
- 작성일: 2026-09-07
- 상태: **실행 PARTIAL — direct 기본 전환·CSS·캐시 수정 및 자동 검증 반영 / resize·onset 성능 gate FAIL**
- 저장소: `C:\Users\parti\Labo\DorotiLab`
- 검토 HEAD: `207010a48eb7040bed4475ace53c5363d819de6f` + 현재 미커밋 변경 전체.
- 최초 계획 작성 요청은 문서만 산출했다. 2026-09-07의 `전체작업` 요청으로 아래 구현·검증을 실행했다. 기존 `work.md`와 과거 실패 이력은 보존했다.
...
- [x] loader와 manifest의 기본값을 `worker-direct-webgl`로 일치시킨다.
  생략/`auto`/미인식 값의 현재 선택 규칙은 유지하면서 결과만 direct로 변경한다.
- [x] 명시적 `worker-canvaskit-webgl`, direct 및 기존 backend 선택은 보존한다.
  direct 초기화 실패 시 다른 backend로 조용히 넘어가지 않고 실제 실패를 표시한다.
- [x] 기본 선택 테스트를 direct의 ownership/main .NET 0·Worker .NET 1·visible Offscreen WebGL2,
...
- [x] README/ADR/활성 실행 문서와 기본 검증 경로를 갱신한다. 과거 결과·CanvasKit 전용 시험은 이름과 범위를 유지한다.
...
- [x] P0 증거에 따라 CSS 크기 소유 규칙을 통일한다. 우선 후보는 root가 viewport를 clip하고
...
- [x] cache 승격이 원인이라면 frame별 생성 수/픽셀/시간 예산을 두고 제한한다.
...
- [x] Release 빌드·TypeScript·변경한 scheduler/cache/resize 계약 검증. 모든 테스트 프로세스 timeout **20분**.
...
- [x] 사용자의 실제 브라우저 창 drag와 첫 scroll/animation 체감을 확인한다.
  자동 증거와 사람 관찰을 별도 기록하며 관찰이 없으면 `notVerified`다.
- [x] `history/26-09-07/`의 별도 실행 보고서에 최종 diff/명령/결과/남은 항목을 기록하고,
...
상세 diff/명령/실패 원본/최종 측정은 [실행 보고서](history/26-09-07/web-direct-default-execution.md)에 있다.
체크된 구현 항목은 성능 acceptance의 PASS를 의미하지 않는다. 원래 수치 목표는 유지한다.

| 단계 | 현재 결과 |
| --- | --- |
| P0 | PARTIAL. direct의 두 실패 재현, callback/raster/paragraph/cache/scene 입력 상관관계 계측. native/독립 시나리오 전체 matrix 미완료. |
| P1 | 기본 전환 구현, source 8 선택과 publish 8 선택 PASS. sample/diagnostics/restart PASS. 별도 package 소비 앱 notVerified. |
| P2 | CSS 배율 충돌 수정, DPR1/2 최종 배율 오차 0. active front 1개 및 settle 1531.9/1459.7ms로 연속성·지연 FAIL. |
| P3 | cache frame 예산/128 metadata 상한/O(1) 항목 관리·만료·해제 구현 및 GPU 회귀 PASS. cold input→새 scene 694.8~755.8ms, 5초 idle 뒤 재시작 570.9~632.5ms로 시작 성능 FAIL. |
| P4 | Release Web/publish/Windows host/native 계약/브라우저 기능 검증 반영. strict resize 2 FAIL 및 미확인 항목 보존. |
| 사용자 관찰 | **일부 개선됐지만 지연이 남음**. 실제 관찰을 자동 결과와 별도로 기록. |

**전체 완료 아님.** 큰 동기 framework build/새 section 구성, live resize 추종, native drag·page zoom·IME/물리 matrix와 package 소비 검증이 남았다.
